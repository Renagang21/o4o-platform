 ...
 *
 * 매장 경영자가 KPA HUB 에 진열된 운영자 발행 동영상을 보고, "가져가기" 로 자기 매장
 * 동영상 사본 (author_role='store') 으로 가져온다. 가져온 사본은 QR 생성 대상으로 쓸 수 있다.
...
import { importOperatorVideo } from '../../api/videoStaff';
import { getActiveServiceShortKey } from '../../lib/serviceContext';

const SERVICE_KEY = () => getActiveServiceShortKey();
const PAGE_LIMIT = 20;

export function HubVideoLibraryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || 'HUB 동영상을 불러올 수 없습니다');
    } finally {
      setIsLoading(false);
    }
  }, [page]);
  ...
  const batchImportItems = useCallback(
    async (
      ...
      const settled = await Promise.allSettled(ids.map((id) => importOperatorVideo(slug, id)));
      ...
      {!error && (
        <>
          ...
          <DataTable<HubContentItemResponse>
            ...
            rowKey="id"
            loading={isLoading}
            emptyMessage="아직 운영자 게시 동영상이 없습니다"
            tableId="store-hub-video"
            selectable
...
export default HubVideoLibraryPage;
