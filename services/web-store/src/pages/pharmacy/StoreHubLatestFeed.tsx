...
import { getActiveServiceKey, getActiveServiceShortKey } from '../../lib/serviceContext';
...
const DIGITAL_META: Record<DigitalFeedItem['kind'], { label: string; icon: ReactNode; route: string; color: string; bg: string }> = {
  pop: { label: 'POP', icon: <ImageIcon size={14} />, route: '/hub/pop', color: '#B45309', bg: '#FEF3C7' },
  qr: { label: 'QR 템플릿', icon: <QrCode size={14} />, route: '/hub/qr', color: '#4338CA', bg: '#E0E7FF' },
  video: { label: '동영상', icon: <Video size={14} />, route: '/hub/video', color: '#BE185D', bg: '#FCE7F3' },
  signage: { label: '사이니지', icon: <MonitorPlay size={14} />, route: '/hub/signage', color: '#047857', bg: '#D1FAE5' },
  'screen-set': { label: '태블릿 화면', icon: <MonitorSmartphone size={14} />, route: '/hub/screen-set', color: '#7C3AED', bg: '#EDE9FE' },
};
...
export function StoreHubLatestFeed() {
  ...
  const loadContents = useCallback(() => {
    ...
    return Promise.allSettled([
      ...
      cmsApi.getContents({ serviceKey: getActiveServiceShortKey(), status: 'published', limit: PREVIEW_ROWS, offset: 0 }),
      hubContentApi.list({ serviceKey: getActiveServiceShortKey(), producer: 'operator', sourceDomain: 'blog', page: 1, limit: PREVIEW_ROWS }),
    ]).then((results) => {
  ...
  const loadDigital = useCallback(() => {
    setDigital(INIT);
    return Promise.allSettled([
      hubContentApi.list({ serviceKey: getActiveServiceShortKey(), producer: 'operator', sourceDomain: 'pop', page: 1, limit: PREVIEW_ROWS }),
      hubContentApi.list({ serviceKey: getActiveServiceShortKey(), producer: 'operator', sourceDomain: 'qr', page: 1, limit: PREVIEW_ROWS }),
      hubContentApi.list({ serviceKey: getActiveServiceShortKey(), producer: 'operator', sourceDomain: 'video', page: 1, limit: PREVIEW_ROWS }),
      hubContentApi.list({ serviceKey: getActiveServiceKey(), sourceDomain: 'signage-media', page: 1, limit: PREVIEW_ROWS }),
      ...
      <FeedSection
        ...
        headerExtra={
          <span style={st.viewAllGroup}>
            <button type="button" style={st.viewAll} onClick={() => navigate('/hub/content')}>
              콘텐츠 전체 보기 →
            </button>
            <button type="button" style={st.viewAll} onClick={() => navigate('/hub/blog')}>
              블로그 전체 보기 →
            </button>
          </span>
        }
