...
import { useAuth, useUnifiedStore } from '../../contexts';
...
export function EventOfferContentPanel({ compact = false }: EventOfferContentPanelProps) {
  const { user } = useAuth();
  const { organizationId: selectedStoreId } = useUnifiedStore();
  ...
  // Unified Store: 매장 보유 = 선택된 매장(organization) 이 있다 (KPA 의 user.isStoreOwner 플래그 대신)
  const hasStore = !!user && !!selectedStoreId;
