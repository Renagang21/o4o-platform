...
export default function StoreCockpitPage() {
  ...
  // store.neture.co.kr 이식(§21-15): 매장은 통합 공간에서 이미 골랐다(상단 매장 선택).
  //   원본은 /cosmetics/stores/me 의 첫 매장을 자동 선택했다 — 선택 매장과 다른 매장이 보이지 않도록
  //   organization_id 가 선택 매장인 것만 쓴다(운영 실측 2026-09-26: 매장 2 · 미연결 0).
  ...
  const loadStores = useCallback(async () => {
    ...
    try {
      const myStores = (await storeApi.getMyStores())?.filter((s) => s.organization_id === organizationId) ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      <StoreHomeActivityPanel
        title="최근 주문"
        moreLabel="전체 보기 →"
        moreTo="/work/k-cosmetics/commerce/orders"
        loading={loading}
