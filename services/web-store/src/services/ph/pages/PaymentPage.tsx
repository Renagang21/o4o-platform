...
} from '../api/pharmacyHubOrders';
import { BRAND } from '../config';

/** Unified Store 서비스 업무(PharmacyHub) 경로 base — PH 앱의 `/store-owner` 자리 */
const PH_WORK_BASE = '/work/pharmacy-hub';
    ...
    (async () => {
      try {
        ...
        const result = await preparePayment({
          paymentGroupId,
          successUrl: `${origin}${PH_WORK_BASE}/payment/success?paymentGroupId=${g}`,
          failUrl: `${origin}${PH_WORK_BASE}/payment/fail?paymentGroupId=${g}`,
        });
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      ...
      await toss.requestPayment('카드', {
        ...
        successUrl: `${origin}${PH_WORK_BASE}/payment/success?paymentGroupId=${g}&paymentId=${encodeURIComponent(prep.paymentId)}`,
        failUrl: `${origin}${PH_WORK_BASE}/payment/fail?paymentGroupId=${g}`,
      });
    } catch (err) {
      ...
      {!loading && error && (
        <>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => navigate(`${PH_WORK_BASE}/cart`)}
            ...
            <button
              type="button"
              onClick={() => navigate(`${PH_WORK_BASE}/orders`)}
              className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700"
            >
              주문 내역
            </button>
      ...
      {!loading && !error && prep && (
        <div className="space-y-4">
          ...
          <p className="text-center text-sm">
            <Link to={`${PH_WORK_BASE}/orders`} className="text-gray-500 underline">
              나중에 결제하기 (주문 내역으로)
            </Link>
          </p>
        </div>
