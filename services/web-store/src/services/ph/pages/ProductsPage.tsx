 ...
 *   담기 · 주문 · 결제는 상세(`${PH_WORK_BASE}/products/:offerId`) 이후 단계에서 처리한다 (이번 범위 밖).
 */
  ...
  const columns: SupplyProductExplorerColumn<ProductRow>[] = [
    {
      ...
      render: (_v, row) => (
        <div className="flex flex-col gap-0.5 min-w-0">
          <Link
            to={`${PH_WORK_BASE}/products/${row.offerId}`}
      ...
      <SupplyProductExplorer<ProductRow>
        ...
        footer={
          <p className="mt-6 text-sm">
            <Link to={`${PH_WORK_BASE}`} className="text-gray-500 underline">
              약국 경영자 홈
            </Link>
          </p>
        }
