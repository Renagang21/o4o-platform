...
import { addCartItem, errorMessage } from '../api/pharmacyHubOrders';
  ...
  if (error || !data) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <Link to={`${PH_WORK_BASE}/products`} className="text-sm text-gray-500 underline">
          상품 목록
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="mt-6 rounded-lg border border-gray-200 bg-white p-4">
        ...
        {added && (
          <p className="mb-2 text-sm text-emerald-700">
            장바구니에 담았습니다.{' '}
            <Link to={`${PH_WORK_BASE}/cart`} className="underline">
              장바구니로 이동
            </Link>
          </p>
        )}
      ...
      <p className="mt-6 flex gap-4 text-sm">
        <Link to={`${PH_WORK_BASE}/products`} className="text-gray-500 underline">
          상품 목록
        </Link>
        <Link to={`${PH_WORK_BASE}/cart`} className="text-gray-500 underline">
          장바구니
        </Link>
      </p>
    </div>
