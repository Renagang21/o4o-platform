/**
 * InstructorDashboardPage — 강사 대시보드
 *
 ...
 *  3. 내 강의 목록 (편집 · 수강자 진입)
 *
 * 강사 신청·승인 동선은 두지 않는다 — 해당 backend(`/lms/instructor/apply*`)는
 * `requireKpaAdmin` 로 KPA 전용이라 PH 에서는 dead navigation 이 된다.
 */

import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { BookOpen, Users, GraduationCap, Clock, Plus, ChevronRight } from 'lucide-react';
import { lmsApi, type InstructorDashboardCourse, type PendingEnrollment } from '../../api/lms';

const C = { primary: '#0d9488' };
...
function KpiCard({ label, value, sub, accent, icon }: {
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: 12, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', borderLeft: `4px solid ${accent}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <div style={{ padding: 6, borderRadius: 8, backgroundColor: `${accent}18` }}>{icon}</div>
...
export default function InstructorDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [courseRes, enrollRes] = await Promise.all([
        ...
        lmsApi.instructorPendingEnrollments(),
      ]);
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 20px' }}>
      ...
      {loading && <div style={{ textAlign: 'center', padding: '60px 0', color: '#9ca3af' }}>불러오는 중...</div>}

      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 28 }}>
          <KpiCard label="총 강의" value={courses.length} sub={`공개 ${publishedCount}개`} accent={C.primary} icon={<BookOpen size={16} color={C.primary} />} />
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{ padding: '1px 8px', borderRadius: 10, fontSize: 11, fontWeight: 600, flexShrink: 0, ...(STATUS_COLOR[course.status ?? 'draft'] ?? STATUS_COLOR.draft) }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }} onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={() => navigate(`/instructor/courses/${course.id}`)}
                      style={{ padding: '5px 12px', backgroundColor: '#ccfbf1', color: '#0f766e', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                    >
                      편집
                    </button>
                    <button
