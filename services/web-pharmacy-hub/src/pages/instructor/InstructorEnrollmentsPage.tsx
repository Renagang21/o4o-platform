...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; bg: string; color: string }> = {
    ...
    completed: { label: '완료', bg: '#ccfbf1', color: '#0f766e' },
...
export default function InstructorEnrollmentsPage() {
  ...
  useEffect(() => {
    if (!courseId) return;
    lmsApi
      .instructorGetParticipantsSummary(courseId)
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const params: { page: number; limit: number; status?: string; query?: string } = { page, limit: LIMIT };
      if (statusFilter !== 'all') params.status = statusFilter;
      if (nameSearch) params.query = nameSearch;

      const res = await lmsApi.instructorGetParticipants(courseId, params);
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {courseTitle && <p style={{ fontSize: 13, color: '#6b7280', margin: '4px 0 0' }}>{courseTitle}</p>}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          ...
          <button onClick={load} style={{ marginTop: 12, padding: '8px 20px', backgroundColor: C.primary, color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}>
            다시 시도
          </button>
        </div>
      ) : items.length === 0 ? (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.enrollmentId} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  <td style={{ padding: '12px 16px', fontSize: 14, fontWeight: 500, color: '#111827' }}>{item.userName}</td>
                  <td style={{ padding: '12px 16px', fontSize: 13, color: '#6b7280' }}>{fmt(item.enrolledAt)}</td>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: 13, color: '#6b7280' }}>{fmt(item.completedAt)}</td>
                  <td style={{ padding: '12px 16px' }}>
                    {item.certificateIssued
                      ? <span style={{ color: '#0f766e', fontSize: 12, fontWeight: 600 }}>발급됨</span>
                      : <span style={{ color: '#9ca3af', fontSize: 12 }}>없음</span>}
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    {item.credited ? (
                      <div>
                        <span style={{ color: '#0369a1', fontSize: 12, fontWeight: 600 }}>지급됨 / {item.creditAmount} C</span>
                        {item.creditedAt && <div style={{ color: '#9ca3af', fontSize: 11, marginTop: 2 }}>{fmt(item.creditedAt)}</div>}
                      </div>
                    ) : (
