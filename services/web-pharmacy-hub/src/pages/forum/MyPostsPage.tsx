/**
 * MyPostsPage — Pharmacy-Hub 내가 쓴 글 (/forum/my-posts)
 *
 * WO-O4O-COMMUNITY-PHARMACYHUB-BASELINE-AND-CROSSSERVICE-MYPOSTS-ADOPTION-V1 §9·§11
 *
 * - 화면은 공통 `MyForumPostsTemplate`. wrapper 는 fetch / route / config 만 담당한다.
 * - 서비스 경계와 author 필터는 서버(route 의 canonical service scope + author=me)가 전담한다.
 */
  ...
  return (
    <main className="min-h-screen bg-slate-50 py-8">
      <div className="mx-auto max-w-5xl px-4">
        <button className="mb-2 text-sm text-teal-700" onClick={() => navigate('/forum')} type="button">
          ← 커뮤니티 홈
        </button>
        <MyForumPostsTemplate
          description="PharmacyHub 커뮤니티에 내가 작성한 글입니다."
          posts={posts}
          totalCount={total}
          currentPage={page}
          ...
          renderUnauthenticated={() => (
            <div className="rounded-lg border border-slate-200 bg-white p-8 text-center">
              ...
              <button
                className="mt-3 rounded-md bg-teal-700 px-4 py-2 text-sm font-medium text-white"
