 ...
 *
 * KPA 화면(services/web-kpa-society/src/pages/pharmacy/ForeignVisitorPartnersPage.tsx)의 Pharmacy-Hub 채택본이다.
...
} from '../../lib/api/pharmacyHubForeignVisitorPartners';
...
export function ForeignVisitorPartnersPage() {
  ...
  useEffect(() => {
    checkSubscription({ serviceKey: 'pharmacy-hub', planCode: PLAN_CODE })
  ...
  const handleToggleStatus = async (p: ForeignVisitorPartner) => {
    ...
    try {
      ...
      if (e?.response?.data?.code === 'ENTITLEMENT_REQUIRED') {
        toast.error('이 기능은 외국인 관광객 판매 지원 이용권이 필요합니다.');
        setEntitled(false);
      } else {
        toast.error(e?.response?.data?.error || e?.message || '상태 변경에 실패했습니다.');
      }
    }
  };
                ...
                <tr key={p.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/40">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        type="button"
                        onClick={() => navigate(`/store-owner/foreign-visitor/partners/${p.id}/qr-codes`)}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (e?.response?.data?.code === 'ENTITLEMENT_REQUIRED') {
        ...
        toast.error(e?.response?.data?.error || e?.message || '저장에 실패했습니다.');
      }
    } finally {
      setSaving(false);
    }
