 ...
 *
 * KPA 화면(services/web-kpa-society/src/pages/pharmacy/ForeignVisitorPartnerQrCodesPage.tsx)의 Pharmacy-Hub 채택본이다.
 * backend 는 서비스 공용 /api/v1/foreign-visitor · /api/v1/store-entitlements 를 그대로 쓰고
 * serviceKey 만 pharmacy-hub 로 바뀐다 — 신규 table·신규 endpoint·서비스 분기 없음.
 * 매장 경로는 PH 셸 기준 /store-owner/foreign-visitor/* 다.
 */
import { useCallback, useEffect, useState } from 'react';
import { Link, useParams } from 'react-router-dom';
import { Loader2, Plus, Pencil, QrCode, Download, Copy, Check, AlertCircle, Lock, Globe } from 'lucide-react';
import { toast } from '@o4o/error-handling';
import { checkSubscription } from '../../lib/api/pharmacyHubStoreServiceSubscription';
import { getForeignVisitorPartner, type ForeignVisitorPartner } from '../../lib/api/pharmacyHubForeignVisitorPartners';
import {
  getPartnerQrCodes,
  createPartnerQrCode,
  updatePartnerQrCode,
  ...
  type QrWritePayload,
} from '../../lib/api/pharmacyHubForeignVisitorPartnerQrCodes';

const PLAN_CODE = 'FOREIGN_VISITOR_SALES_SUPPORT';
const ENTRY_PATH = '/store-owner/foreign-visitor';
const PARTNERS_PATH = '/store-owner/foreign-visitor/partners';
...
export function ForeignVisitorPartnerQrCodesPage() {
  ...
  useEffect(() => {
    checkSubscription({ serviceKey: 'pharmacy-hub', planCode: PLAN_CODE }).then((r) => setEntitled(r.active)).catch(() => setEntitled(false));
  ...
  const handleToggle = async (q: ForeignVisitorPartnerQrCode) => {
    ...
    try {
      ...
      if (e?.response?.data?.code === 'ENTITLEMENT_REQUIRED') { toast.error('이 기능은 외국인 관광객 판매 지원 이용권이 필요합니다.'); setEntitled(false); }
      else toast.error(e?.response?.data?.error || e?.message || '상태 변경에 실패했습니다.');
    }
  };
...
function QrFormModal({ partnerId, initial, onClose, onSaved, onEntitlementDenied }: FormProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (e?.response?.data?.code === 'ENTITLEMENT_REQUIRED') { toast.error('이 기능은 외국인 관광객 판매 지원 이용권이 필요합니다.'); onEntitlementDenied(); onClose(); }
      else toast.error(e?.response?.data?.error || e?.message || '저장에 실패했습니다.');
...
function QrViewModal({ qr, onClose }: { qr: ForeignVisitorPartnerQrCode; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    getPartnerQrSvg(qr.id, 512)
      ...
      .catch((e) => { if (!cancelled) setErr(e?.response?.data?.error || e?.message || 'QR 이미지를 불러오지 못했습니다.'); });
