/**
 * StoreProductMultilingualContentPage — 매장 상품별 다국어 QR 콘텐츠 저작 (Pharmacy-Hub)
 *
...
import { MultilingualPublicActions } from '../../components/store-owner/MultilingualPublicActions';
...
function draftFromPage(p?: MlcPageDetail): PageDraft {
  return {
    title: p?.title ?? '',
    summary: p?.summary ?? '',
...
function isTargetKind(v: string | undefined): v is MlcTargetKind {
...
export default function StoreProductMultilingualContentPage() {
  ...
  const [group, setGroup] = useState<MlcGroupDetail | null>(null);
  const [title, setTitle] = useState('');
  const [defaultLocale, setDefaultLocale] = useState<MlcLocale>('ko');

  const [activeLocale, setActiveLocale] = useState<MlcLocale>('ko');
    ...
    (async () => {
      ...
      try {
        const groups = await listMyMlcGroups({
          targetKind: targetKind as MlcTargetKind,
          targetId: targetId!,
        ...
        if (canceled) return;
        if (existing) {
          const full = await getMlcGroup(existing.id);
          if (canceled) return;
          hydrate(full);
        } else {
          // 신규: 기본 제목을 상품명으로 프리필.
  ...
  const publishedLocaleCount = useMemo(
    () => MLC_LOCALES.filter((loc) => drafts[loc]?.status === 'published').length,
    [drafts],
  );

  const handleSaveGroup = async (): Promise<MlcGroupDetail | null> => {
    ...
    try {
      const saved = await createMlcGroup({
        targetKind: targetKind as MlcTargetKind,
        targetId: targetId!,
        title: title.trim(),
        defaultLocale,
  ...
  const handleSavePage = async (publish: boolean) => {
    ...
    try {
      const g = await upsertMlcPage(group.id, activeLocale, {
        title: d.title.trim(),
      ...
      toast.success(
        `${MLC_LOCALE_LABELS[activeLocale]} 페이지가 ${publish ? '발행' : '저장'}되었습니다`,
      );
    } catch (e: any) {
  ...
  const handleTogglePageStatus = async () => {
    ...
    setIsSavingPage(true);
    try {
      const g = await setMlcPageStatus(group.id, activeLocale, next);
      hydrate(g);
      setActiveLocale(activeLocale);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
        ...
        <button onClick={() => navigate('/store-owner/handled-products')} className="text-sm text-blue-600 hover:underline">
          상품 목록으로 돌아가기
        </button>
      </div>
    );
    ...
    <div className="space-y-6 max-w-4xl p-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/store-owner/handled-products')} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500" title="상품 목록">
      ...
      <div className="space-y-4 bg-white rounded-xl border border-slate-100 p-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              value={defaultLocale}
              onChange={(e) => setDefaultLocale(e.target.value as MlcLocale)}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              disabled={isSavingGroup}
            >
              {MLC_LOCALES.map((loc) => (
                <option key={loc} value={loc}>{MLC_LOCALE_LABELS[loc]}</option>
              ))}
            </select>
          </div>
        </div>
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm text-slate-500">
                <span className="font-semibold text-slate-700">{MLC_LOCALE_LABELS[activeLocale]}</span> 페이지
            ...
            <div>
              ...
              <input
                ...
                placeholder={`${MLC_LOCALE_LABELS[activeLocale]} 제목`}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                disabled={isSavingPage}
              />
            </div>
            ...
            <div>
              ...
              <RichTextEditor
                ...
                placeholder={`${MLC_LOCALE_LABELS[activeLocale]} 본문을 작성하세요`}
                minHeight="360px"
