/**
 * MultilingualProductPublicLandingPage — 외국인 고객용 다국어 상품 안내 (public/QR landing, Pharmacy-Hub)
 *
 * WO-O4O-PHARMACYHUB-COMMUNITY-AND-MY-STORE-FULL-PARITY-CLOSURE-V1 §8 (#76)
 * 계약 원본: WO-O4O-MULTILINGUAL-PRODUCT-QR-LANDING-V1 (KPA 와 동일한 공통 API)
 *
 * 매장이 저작한 store-scoped 다국어 콘텐츠를 인증 없이 QR/URL 로 여는 모바일 우선 화면.
 *   /multilingual-products/:publicKey?locale=en   (mode=tablet 이면 매장 응대용 큰 화면)
 *
 * - published page 만 노출 / 요청 locale 없으면 fallback(en -> defaultLocale -> ko).
...
export function MultilingualProductPublicLandingPage() {
  ...
  const localeParam = searchParams.get('locale') as MlcLocale | null;
  ...
  const load = useCallback(async (loc?: MlcLocale) => {
    setLoading(true);
    setNotFound(false);
    try {
  ...
  const switchLocale = (loc: MlcLocale) => {
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col">
        <header className="bg-white border-b border-slate-200 px-8 py-5 sticky top-0 z-10">
          <div className="max-w-4xl mx-auto flex flex-col gap-4">
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-400">Pharmacy-Hub</p>
        ...
        <footer className="px-8 py-8 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Please ask our staff for assistance. · Pharmacy-Hub</p>
        </footer>
      </div>
    );
  }
