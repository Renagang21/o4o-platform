/**
 * ForumPage - 포럼 게시글 목록 (테이블 + 페이지네이션)
 *
    ...
    async function load() {
      ...
      try {
        ...
        if (isFiltering) {
          ...
          setTotalPages(Math.max(1, Math.ceil((typeFilter ? results.length : res.totalCount) / PAGE_SIZE)));
        } else {
    ...
    <div style={s.container}>
      ...
      <header style={s.header}>
        <div style={s.headerRow}>
          <div>
            <h1 style={s.title}>{customTitle || 'o4o · 네뚜레 의견 나누기'}</h1>
      ...
      {customNotice && (
        <div style={s.notice}>
          <span>ℹ️</span>
          <p style={s.noticeText}>{customNotice}</p>
        </div>
      )}
      ...
      {/* Footer */}
      <div style={s.footer}>
        <Link to="/" style={s.backLink}>← 홈으로 돌아가기</Link>
      </div>
    </div>
  );
}
...
const s: Record<string, React.CSSProperties> = {
  ...
  pinnedRow: {
    cursor: 'pointer', backgroundColor: '#fffbeb', transition: 'background-color 0.1s',
  },
  badge: {
    display: 'inline-block', padding: '2px 8px', fontSize: '11px', fontWeight: 500, borderRadius: '4px',
  },
  pinnedTag: {
