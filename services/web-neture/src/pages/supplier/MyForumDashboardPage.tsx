/**
 * MyForumDashboardPage - 내 포럼 관리 대시보드 (Neture)
 *
 * WO-O4O-FORUM-MY-FORUM-EXPANSION-V1
 * GlycoPharm MyForumDashboardPage 패턴 복제 — Neture 공급자 공간
 */
...
export default function MyForumDashboardPage() {
  ...
  const loadRequests = async () => {
    ...
    setError(null);
    try {
      const response = await fetchMyForumRequests();
      if (!response.success) {
  ...
  const handleSaveEdit = async () => {
    ...
    setEditError(null);
    try {
      const res = await updateMyCategory(editingCategory.id, {
        name: trimmedName,
      ...
      if (!res.success) {
        setEditError(res.error || '저장에 실패했습니다.');
      } else {
  ...
  const handleDeleteRequest = async () => {
    ...
    setDeleteError(null);
    try {
      const res = await requestDeleteCategory(deletingCategory.id, {
        reason: deleteReason.trim() || undefined,
      });
      if (!res.success) {
        setDeleteError(res.error || '삭제 요청에 실패했습니다.');
      } else {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        <Link
          to="/supplier/forum/request-category"
        ...
        >
          <div className="w-10 h-10 rounded-lg bg-emerald-100 flex items-center justify-center">
            <MessageSquarePlus className="w-5 h-5 text-emerald-700" />
          </div>
          <div>
            <div className="font-semibold text-emerald-800">포럼 개설 신청</div>
            <div className="text-sm text-emerald-600">새로운 포럼을 요청합니다</div>
          </div>
        </Link>
      ...
      <div className="mb-8">
        ...
        {error && (
          <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
            ...
            <button onClick={loadRequests} className="mt-3 px-4 py-2 text-sm bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
              다시 시도
            </button>
          </div>
        )}
        ...
        {!isLoading && !error && requests.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
            ...
            <Link
              to="/supplier/forum/request-category"
              className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm"
            >
                      ...
                      <div className="pt-4 space-y-3">
                        ...
                        {request.reviewComment && (
                          <div className={`p-3 rounded-lg ${
                            ...
                            request.status === 'revision_requested' ? 'bg-orange-50' : 'bg-red-50'
                          }`}>
                            <div className={`text-xs font-medium mb-1 ${
                              request.status === 'approved' ? 'text-green-700' :
                              request.status === 'revision_requested' ? 'text-orange-700' : 'text-red-700'
                            }`}>
                              {request.status === 'revision_requested' ? '보완 요청 사항' : '관리자 의견'}
                            </div>
                            <p className={`text-sm ${
                              request.status === 'approved' ? 'text-green-600' :
                              request.status === 'revision_requested' ? 'text-orange-600' : 'text-red-600'
                            }`}>
                              {request.reviewComment}
                            </p>
                            {request.reviewedAt && (
                              <p className="text-xs text-slate-500 mt-1">{formatDate(request.reviewedAt)} 검토</p>
                            )}
                          </div>
                        )}
                        {request.status === 'approved' && request.createdCategorySlug && (
                          <Link
                            to={`/supplier/forum?category=${request.createdCategorySlug}`}
                            className="inline-flex items-center gap-2 px-3 py-2 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition-colors text-sm"
                          >
          ...
          <div className="space-y-3">
            {myCategories.map((cat) => (
              <div key={cat.id} className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200">
                <Link
                  to={`/supplier/forum?category=${cat.slug}`}
                ...
                <div className="flex items-center gap-2 shrink-0 ml-3">
                  ...
                  <button onClick={() => openEdit(cat)} className="p-2 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="포럼 정보 수정">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => !deleteSaving && setDeletingCategory(null)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setDeletingCategory(null)} disabled={deleteSaving} className="flex-1 px-4 py-2.5 text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors text-sm font-medium disabled:opacity-50">
                취소
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => !editSaving && setEditingCategory(null)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            {editError && (
            ...
            <div>
              ...
              <input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} maxLength={50}
                className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm" />
            ...
            <div>
              ...
              <textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} rows={3}
                className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm resize-none" />
            </div>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setEditingCategory(null)} disabled={editSaving} className="flex-1 px-4 py-2.5 text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors text-sm font-medium disabled:opacity-50">
                취소
              </button>
              <button onClick={handleSaveEdit} disabled={editSaving} className="flex-1 px-4 py-2.5 text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
