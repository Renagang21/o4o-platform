/**
 * MarketTrialDetailPage — Neture 유통참여형 펀딩(Market Trial) 상세
 *
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (active) setError('유통참여형 펀딩을 찾을 수 없습니다.');
      } finally {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (msg.includes('Already participated')) {
        setJoinError('이미 참여 신청한 유통참여형 펀딩입니다.');
        ...
        {!hasJoined && isRecruiting && !isFull && (
          <div>
            <p style={{ fontSize: '0.8125rem', color: '#6B7280', lineHeight: 1.5, marginBottom: '14px' }}>
              {isAuthenticated
                ? '이 유통참여형 펀딩에 참여하시겠습니까? 보상 방식을 선택한 후 신청해 주세요.'
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <p style={{ fontSize: '0.8125rem', color: '#6B7280', margin: 0 }}>
                이 유통참여형 펀딩에 관한 공지 및 토론을 포럼에서 확인하세요.
              </p>
            </div>
            <Link
