...
export async function fetchStorePlaylists(): Promise<StorePlaylist[]> {
  const res = await api.get('/cosmetics/store-playlists');
...
export async function createStorePlaylist(
  ...
  const res = await api.post('/cosmetics/store-playlists', { name, playlistType });
  return (res.data as { success: boolean; data: StorePlaylist }).data;
}
...
export async function updateStorePlaylist(
  ...
  const res = await api.patch(`/cosmetics/store-playlists/${id}`, updates);
  return (res.data as { success: boolean; data: StorePlaylist }).data;
}

export async function deleteStorePlaylist(id: string): Promise<void> {
  await api.delete(`/cosmetics/store-playlists/${id}`);
}
...
export async function fetchPlaylistItems(playlistId: string): Promise<StorePlaylistItem[]> {
  const res = await api.get(`/cosmetics/store-playlists/${playlistId}/items`);
...
export async function addPlaylistItem(
  ...
  const res = await api.post(`/cosmetics/store-playlists/${playlistId}/items`, { snapshotId });
  return (res.data as { success: boolean; data: StorePlaylistItem }).data;
}
...
export async function reorderPlaylistItems(
  ...
  await api.patch(`/cosmetics/store-playlists/${playlistId}/items/reorder`, { order });
}
...
export async function deletePlaylistItem(
  ...
  await api.delete(`/cosmetics/store-playlists/${playlistId}/items/${itemId}`);
}
