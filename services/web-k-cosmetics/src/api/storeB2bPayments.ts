/**
 * K-Cosmetics 매장 → 공급자 B2B 결제 API client
 *
 * WO-O4O-SUPPLIER-ORDER-PAYMENT-FULFILLMENT-SETTLEMENT-CANONICALIZATION-V1 §2-E-4
 *
 * backend: `/api/v1/cosmetics/b2b/payments/{prepare,confirm,order/:orderId}`
 ...
 *
 * ⚠️ 소비자 → 매장 판매 결제(`/api/v1/cosmetics/payments/*`)는 은퇴(410)다. 그 경로를 되살리지 않는다.
...
function unwrap<T>(body: ApiEnvelope<T> | undefined, fallback: string): T {
...
export async function prepareStoreB2bPayment(params: {
  ...
  const res = await api.post('/cosmetics/b2b/payments/prepare', {
    ...(params.orderId ? { orderId: params.orderId } : {}),
    ...(params.paymentGroupId ? { paymentGroupId: params.paymentGroupId } : {}),
    successUrl: params.successUrl,
    failUrl: params.failUrl,
  });
  return unwrap(res.data as ApiEnvelope<StoreB2bPreparePaymentResult>, '결제 준비에 실패했습니다.');
}
...
export async function confirmStoreB2bPayment(params: {
  ...
  const res = await api.post('/cosmetics/b2b/payments/confirm', {
    paymentId: params.paymentId,
    paymentKey: params.paymentKey,
    ...(params.orderId ? { orderId: params.orderId } : {}),
    ...(params.paymentGroupId ? { paymentGroupId: params.paymentGroupId } : {}),
  });
  return unwrap(res.data as ApiEnvelope<StoreB2bConfirmPaymentResult>, '결제 확인에 실패했습니다.');
}

/** 결제창 렌더링용 주문 정보 */
export async function getStoreB2bPaymentInfo(orderId: string): Promise<StoreB2bPaymentInfo> {
  const res = await api.get(`/cosmetics/b2b/payments/order/${orderId}`);
  return unwrap(res.data as ApiEnvelope<StoreB2bPaymentInfo>, '결제 정보를 불러오지 못했습니다.');
}

/**
 * Toss v1 결제 SDK 로더 (CDN 스크립트 주입 — npm dependency/lockfile 무변경).
 * web-neture · web-kpa-society 와 같은 방식이다(새 공통 위젯 패키지를 만들지 않는다).
 */
