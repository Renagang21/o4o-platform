/**
 * CosmeticsTable - K-Cosmetics 축약 Table UI (Spike)
 *
 * 설계 원칙:
 * - Cosmetics 브랜드에 맞는 시각적 스타일
 * - 컬럼 최소화 (3-4개)
 * - 정렬/선택 기능 제거
 */
...
export interface CosmeticsTableColumn {
  id: string;
  label: string;
  width?: string;
...
export interface CosmeticsTableRow {
  id: string;
...
interface CosmeticsTableProps {
  columns: CosmeticsTableColumn[];
  rows: CosmeticsTableRow[];
  loading?: boolean;
  emptyMessage?: string;
  className?: string;
}
...
/**
 * CosmeticsTable Component (Condensed Version)
 */
export const CosmeticsTable: FC<CosmeticsTableProps> = ({
  columns,
  rows,
  loading = false,
