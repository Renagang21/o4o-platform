...
const STATUS_MESSAGES: Record<Exclude<MembershipStatus, 'active'>, { title: string; message: string }> = {
  none:      { title: '서비스 가입이 필요합니다',  message: 'K-Cosmetics 서비스를 이용하려면 먼저 가입 신청을 해주세요.' },
      ...
      <div className="bg-white rounded-2xl p-10 text-center max-w-md w-full shadow-sm border border-slate-200">
        ...
        <div className="flex justify-center gap-2 flex-wrap">
          {canApply && applyPath && (
            <button
              type="button"
              className="px-5 py-2.5 bg-pink-600 text-white border-none rounded-lg text-sm font-medium cursor-pointer"
