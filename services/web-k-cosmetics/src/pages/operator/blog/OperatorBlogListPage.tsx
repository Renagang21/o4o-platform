...
const blogActionPolicy = defineActionPolicy<OperatorBlogPost>('cosmetics:operator-blog', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    { key: 'publish', label: '발행', variant: 'primary', visible: (p) => p.status !== 'published' },
    {
      ...
      confirm: { title: '블로그 보관', message: '이 블로그를 보관하시겠습니까? HUB 노출이 중단됩니다.', confirmText: '보관' },
    },
    {
      key: 'delete',
      label: '삭제',
      variant: 'danger',
      confirm: { title: '블로그 삭제', message: '이 블로그를 삭제하시겠습니까? 되돌릴 수 없습니다.', variant: 'danger', confirmText: '삭제' },
    },
  ],
});
...
export default function OperatorBlogListPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listOperatorBlogPosts({ page, limit: PAGE_LIMIT, status: statusFilter || undefined });
      setItems(res.data);
  ...
  const handlePublish = useCallback(async (post: OperatorBlogPost) => {
    if (!window.confirm(`"${post.title}" 블로그를 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.`)) return;
  ...
  const batchBlogOp = useCallback(
    async (
      ...
      const fn = op === 'publish' ? publishOperatorBlogPost : op === 'archive' ? archiveOperatorBlogPost : deleteOperatorBlogPost;
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      <header className="mb-6 pb-5 border-b-2 border-slate-200 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            운영자가 K-Cosmetics 매장 HUB 에 게시할 블로그 콘텐츠를 작성·관리합니다.
      ...
      {error && (
        <div className="text-center py-16 text-red-600 text-sm">
          <p>{error}</p>
          <button onClick={() => loadData()} className="mt-3 px-4 py-1.5 text-xs text-pink-600 border border-pink-400 rounded-lg hover:bg-pink-50">
            다시 시도
          </button>
        </div>
      )}
      ...
      {!error && (
        <>
            ...
            <ActionBar
              ...
              actions={[
                ...
                },
                {
                  ...
                  onClick: () => runBulk(selectedNotArchivedIds, 'archive', { confirm: `선택한 ${selectedNotArchivedIds.length}개 블로그를 보관하시겠습니까?` }),
                ...
                },
                {
                  ...
                  onClick: () => runBulk(selectedAllIds, 'delete', { confirm: `선택한 ${selectedAllIds.length}개 블로그를 삭제하시겠습니까? 되돌릴 수 없습니다.` }),
          ...
          <DataTable<OperatorBlogPost>
            ...
            rowKey="id"
            loading={isLoading}
            emptyMessage={statusFilter ? '해당 상태의 블로그가 없습니다' : '아직 작성한 블로그가 없습니다'}
