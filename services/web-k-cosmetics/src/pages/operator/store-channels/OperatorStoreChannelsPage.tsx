 ...
 *
 * Reference: services/web-glycopharm/src/pages/operator/store-channels/OperatorStoreChannelsPage.tsx
 *
...
const STATUS_BADGE: Record<string, { label: string; bg: string; text: string }> = {
  APPROVED:   { label: '활성', bg: 'bg-emerald-100', text: 'text-emerald-700' },
...
const channelActionPolicy = defineActionPolicy<ChannelData>('cosmetics:store-channels', {
  rules: [
    {
      key: 'approve',
      label: '승인',
...
export default function OperatorStoreChannelsPage() {
  ...
  const fetchChannels = useCallback(async () => {
    ...
    try {
      ...
      if (typeFilter)   params.channelType = typeFilter;

      const { data } = await api.get('/operator/stores/channels', { params });
      if (data.success) {
  ...
  const handleStatusChange = async (channel: ChannelData, newStatus: string) => {
    ...
    try {
      await api.put(
        `/operator/stores/${channel.storeId}/channels/${channel.id}/status`,
        { status: newStatus },
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">K-Cosmetics 매장의 채널 상태를 관리합니다</p>
        </div>
        <button
        ...
        <div className="p-4 border-b border-slate-100 flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }}
            className="px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500"
          >
              ...
              {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${
                    currentPage === page ? 'bg-pink-600 text-white' : 'hover:bg-slate-100 text-slate-600'
                  }`}
                >
                  {page}
                </button>
