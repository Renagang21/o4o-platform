...
export default function OperatorPopWritePage() {
  ...
  const handleSave = async (): Promise<OperatorPopPost | null> => {
    ...
    try {
      ...
      const saved = isNew ? await createOperatorPopPost(body) : await updateOperatorPopPost(id!, body);
      setPost(saved);
      toast.success(isNew ? 'POP 이 생성되었습니다 (초안)' : '저장되었습니다');
      if (isNew) navigate(`/operator/pop/${saved.id}/edit`, { replace: true });
      return saved;
    } catch (e: any) {
  ...
  const handlePublish = async () => {
    ...
    if (!saved) return;
    if (saved.status === 'published') { toast.info('이미 발행된 POP 입니다'); return; }
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
      ...
      <button onClick={() => navigate('/operator/pop')} className="text-sm text-pink-600 hover:underline">목록으로 돌아가기</button>
    </div>
  );
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/operator/pop')} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500" title="목록">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <h1 className="text-xl font-bold text-slate-800">{isNew ? '새 POP 작성' : 'POP 수정'}</h1>
