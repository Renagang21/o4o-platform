...
const STATUS_BADGE_CLASS: Record<OperatorPopPost['status'], string> = {
  draft: 'bg-slate-100 text-slate-600',
  published: 'bg-pink-50 text-pink-700',
  archived: 'bg-amber-50 text-amber-700',
};

const popActionPolicy = defineActionPolicy<OperatorPopPost>('cosmetics:operator-pop', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    { key: 'publish', label: '발행', variant: 'primary', visible: (p) => p.status !== 'published' },
    {
      key: 'archive',
      label: '보관',
      visible: (p) => p.status !== 'archived',
      confirm: { title: 'POP 보관', message: '이 POP 을 보관하시겠습니까? HUB 노출이 중단됩니다.', confirmText: '보관' },
    },
    {
      key: 'delete',
      label: '삭제',
      variant: 'danger',
      confirm: { title: 'POP 삭제', message: '이 POP 을 삭제하시겠습니까? 되돌릴 수 없습니다.', variant: 'danger', confirmText: '삭제' },
    },
  ],
});

const POP_ACTION_ICONS: Record<string, ReactNode> = {
...
export default function OperatorPopListPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listOperatorPopPosts({ page, limit: PAGE_LIMIT, status: statusFilter || undefined });
      setItems(res.data);
  ...
  const handlePublish = useCallback(async (post: OperatorPopPost) => {
    if (!window.confirm(`"${post.title}" POP 을 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.`)) return;
    setActionLoading(post.id);
    try {
      await publishOperatorPopPost(post.id);
  ...
  const handleArchive = useCallback(async (post: OperatorPopPost) => {
    setActionLoading(post.id);
    try {
      await archiveOperatorPopPost(post.id);
  ...
  const handleDelete = useCallback(async (post: OperatorPopPost) => {
    setActionLoading(post.id);
    try {
      await deleteOperatorPopPost(post.id);
  ...
  const batchPopOp = useCallback(
    async (
      ...
      const fn = op === 'publish' ? publishOperatorPopPost : op === 'archive' ? archiveOperatorPopPost : deleteOperatorPopPost;
  ...
  const runBulk = useCallback(
    async (ids: string[], op: BulkOp, opts?: { confirm?: string }) => {
      ...
      const result = await batch.executeBatch(batchPopOp, ids, { op });
      if (result.successCount > 0) { setSelectedIds(new Set()); await loadData(); }
    },
    [batch, batchPopOp, loadData],
  );

  const columns: ListColumnDef<OperatorPopPost>[] = useMemo(() => [
    {
      key: 'title',
      header: '제목',
      sortable: true,
    ...
    },
    {
      ...
      header: '슬러그',
      width: '180px',
      render: (_v, p) => <span className="text-xs text-slate-500 font-mono truncate">/{p.slug}</span>,
    },
    {
      key: 'status',
      header: '상태',
    ...
    },
    {
      ...
      render: (_v, p) => <span className="text-xs text-slate-500">{new Date(p.updatedAt).toLocaleDateString('ko-KR')}</span>,
    },
    {
      key: 'publishedAt',
      header: '발행일',
      ...
      render: (_v, p) => <span className="text-xs text-slate-500">{p.publishedAt ? new Date(p.publishedAt).toLocaleDateString('ko-KR') : '-'}</span>,
    },
    {
      key: '_actions',
      header: '액션',
      ...
      render: (_v, p) => (
        <RowActionMenu
          actions={buildRowActions(popActionPolicy, p, {
            edit: () => navigate(`/operator/pop/${p.id}/edit`),
            publish: () => handlePublish(p),
            archive: () => handleArchive(p),
            delete: () => handleDelete(p),
          }, {
            icons: POP_ACTION_ICONS,
            loading: actionLoading === p.id ? { edit: true, publish: true, archive: true, delete: true } : undefined,
          })}
          inlineMax={popActionPolicy.inlineMax}
        />
      ),
    },
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      <header className="mb-6 pb-5 border-b-2 border-slate-200 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            운영자가 K-Cosmetics 매장 HUB 에 게시할 POP 콘텐츠를 작성·관리합니다.
            발행 시 모든 K-Cosmetics 매장의 HUB 에 노출됩니다.
          </p>
        </div>
        <button
      ...
      {!error && (
        <>
              ...
              actions={[
                {
                  ...
                  tooltip: '선택한 초안/보관 POP 을 일괄 발행합니다',
                },
                {
                  key: 'bulk-archive',
                  label: `일괄 보관 (${selectedNotArchivedIds.length})`,
                  onClick: () => runBulk(selectedNotArchivedIds, 'archive', { confirm: `선택한 ${selectedNotArchivedIds.length}개 POP 을 보관하시겠습니까?` }),
                  variant: 'default' as const,
                  icon: <Archive className="w-3.5 h-3.5" />,
                  loading: batch.loading,
                  group: 'actions',
                  visible: selectedNotArchivedIds.length > 0,
                  tooltip: '선택한 POP 을 일괄 보관',
                },
                {
                  key: 'bulk-delete',
                  ...
                  onClick: () => runBulk(
                    ...
                    'delete',
                    { confirm: `선택한 ${selectedIds.size}개 POP 을 삭제하시겠습니까? 되돌릴 수 없습니다.` },
                  ),
                  ...
                  tooltip: '선택한 POP 을 일괄 삭제 (되돌릴 수 없음)',
                },
              ]}
            />
          </div>
          ...
          <DataTable<OperatorPopPost>
            columns={columns}
            data={items}
            rowKey="id"
            loading={isLoading}
            emptyMessage={statusFilter ? '해당 상태의 POP 이 없습니다' : '아직 작성한 POP 이 없습니다'}
