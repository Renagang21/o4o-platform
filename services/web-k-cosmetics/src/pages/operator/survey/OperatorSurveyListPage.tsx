/**
 * OperatorSurveyListPage — 설문조사 관리 (K-Cosmetics 운영자)
 *
 * WO-O4O-KCOSMETICS-OPERATOR-SURVEYS-V1
 *
 * Backend: GET /api/v1/surveys?serviceKey=k-cosmetics
...
export default function OperatorSurveyListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await kcosSurveyApi.list({ page, limit: LIMIT });
  ...
  const handleAction = useCallback(async (key: string, row: SurveyItem) => {
    try {
      if (key === 'activate') {
        await kcosSurveyApi.update(row.id, { status: 'active' });
        toast.success('활성화되었습니다');
        load();
      } else if (key === 'close') {
        await kcosSurveyApi.update(row.id, { status: 'closed' });
        toast.success('종료되었습니다');
        load();
      } else if (key === 'delete') {
        await kcosSurveyApi.delete(row.id);
  ...
  const columns = useMemo((): ListColumnDef<SurveyItem>[] => [
    ...
    },
    {
      ...
      render: (_v, row) => row.rewardEnabled
        ? <span className="text-xs font-semibold text-pink-700">{row.rewardAmount}P</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 text-sm text-white bg-pink-600 rounded hover:bg-pink-700"
          >
