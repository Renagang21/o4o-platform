/**
 * OperatorSurveyCreatePage — 설문 만들기 (K-Cosmetics 운영자)
 *
 * WO-O4O-KCOSMETICS-OPERATOR-SURVEYS-V1
 *
 * Backend: POST /api/v1/surveys (serviceKey='k-cosmetics', ownerType='service_operator')
 */

import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ClipboardList, Plus, Trash2, ChevronLeft } from 'lucide-react';
import { kcosSurveyApi, type CreateSurveyPayload } from '../../../api/survey';
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="space-y-4 p-4 border rounded-lg bg-white">
          ...
          <div>
            ...
            <input
              ...
              placeholder="설문 제목"
              className="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-pink-500"
            />
          </div>
          <div>
        ...
        {/* 포인트 보상 */}
        <section className="p-4 border rounded-lg bg-pink-50 space-y-3">
          ...
          {rewardEnabled && (
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-28 border rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-pink-500"
              />
          ...
          {questions.map((q, qIdx) => (
            <div key={q.id} className="p-4 border rounded-lg bg-white space-y-3">
              ...
              <select
                ...
                className="w-full border rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-pink-500"
              >
                  ...
                  {q.options.map((opt, oIdx) => (
                    <div key={oIdx} className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 border rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-pink-500"
                      />
                      {q.options.length > 2 && (
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={saving}
            className="flex-1 py-2 text-sm text-white bg-pink-600 rounded hover:bg-pink-700 disabled:opacity-50">
