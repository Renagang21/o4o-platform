/**
 * TabletStorePage — In-Store Tablet Kiosk View (K-Cosmetics)
 *
...
export default function TabletStorePage() {
  ...
  const handleSubmitInterest = async () => {
    ...
    if (selectedProduct.type === 'local') return;
    ...
    return (
      <div style={styles.fullscreen}>
        <div style={styles.centerMessage}>
          ...
          <h2 style={{ fontSize: '28px', fontWeight: 700, marginBottom: '8px' }}>{st.label}</h2>
          <p style={{ color: '#64748b', fontSize: '16px', marginBottom: '32px' }}>{st.message}</p>
          ...
          {isDone && (
            <button onClick={resetToDefault} style={styles.primaryBtn}>처음으로</button>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div style={styles.fullscreen}>
          ...
          <div style={styles.detailInfo}>
            ...
            {!isLocal && (
              <div style={{ margin: '16px 0' }}>
                <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="이름 (선택사항)" style={styles.input} maxLength={100} />
                <input type="text" value={customerNote} onChange={(e) => setCustomerNote(e.target.value)} placeholder="요청 사항 (선택사항)" style={{ ...styles.input, marginTop: '8px' }} maxLength={200} />
              </div>
            )}
            {isLocal && (
        ...
        <div style={styles.actionBar}>
          <button onClick={() => { setViewMode('browse'); setSelectedProduct(null); setCustomerName(''); setCustomerNote(''); }} style={styles.backBtn}>돌아가기</button>
          {!isLocal && (
            <button onClick={handleSubmitInterest} disabled={submitting} style={{ ...styles.interestBtn, opacity: submitting ? 0.6 : 1 }}>
  ...
  return (
    <div style={styles.fullscreen}>
      ...
      <div style={styles.body}>
        {loading ? (
          <div style={styles.centerMessage}><p style={{ color: '#94a3b8' }}>상품을 불러오는 중...</p></div>
        ) : displayProducts.length === 0 ? (
          <div style={styles.centerMessage}><p style={{ color: '#94a3b8' }}>표시할 상품이 없습니다.</p></div>
        ) : (
          <div style={styles.grid}>
            {displayProducts.map((p) => (
              <div key={`${p.type}-${p.id}`} onClick={() => { setSelectedProduct(p); setViewMode('detail'); }} style={styles.productCard}>
                ...
                <div style={styles.productInfo}>
                  ...
                  <span style={styles.productPrice}>{p.price ? formatPrice(p.price) : p.priceDisplay || '가격 문의'}</span>
                </div>
                {p.type === 'local' && <div style={styles.localBadge}>자체</div>}
              </div>
            ))}
          </div>
        )}
...
const styles: Record<string, React.CSSProperties> = {
  fullscreen: { position: 'fixed', inset: 0, backgroundColor: '#f1f5f9', display: 'flex', flexDirection: 'column', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' },
  header: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', backgroundColor: '#fff', borderBottom: '1px solid #e2e8f0', flexShrink: 0 },
  body: { flex: 1, overflowY: 'auto', padding: '16px' },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' },
  productCard: { position: 'relative', backgroundColor: '#fff', borderRadius: '12px', border: '2px solid #e2e8f0', cursor: 'pointer', overflow: 'hidden', transition: 'border-color 0.15s, transform 0.1s' },
  productImgArea: { height: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f8fafc' },
  productImg: { maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' as const },
  productInfo: { padding: '10px 12px', display: 'flex', flexDirection: 'column' as const, gap: '4px' },
  productName: { fontSize: '14px', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const },
  productPrice: { fontSize: '15px', fontWeight: 700, color: '#2563eb' },
  localBadge: { position: 'absolute' as const, top: '8px', right: '8px', padding: '2px 8px', borderRadius: '4px', backgroundColor: '#f59e0b', color: '#fff', fontSize: '11px', fontWeight: 700 },
  detailImageArea: { height: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f8fafc', flexShrink: 0 },
  detailInfo: { padding: '24px', flex: 1 },
  categoryBadge: { display: 'inline-block', padding: '4px 12px', borderRadius: '20px', backgroundColor: '#e0f2fe', color: '#0369a1', fontSize: '13px', fontWeight: 500 },
  actionBar: { display: 'flex', gap: '12px', padding: '16px 24px', backgroundColor: '#fff', borderTop: '1px solid #e2e8f0', flexShrink: 0 },
  backBtn: { padding: '14px 24px', borderRadius: '12px', border: '2px solid #e2e8f0', backgroundColor: '#fff', color: '#334155', fontSize: '16px', fontWeight: 600, cursor: 'pointer', flex: 1 },
  interestBtn: { padding: '14px 24px', borderRadius: '12px', border: 'none', backgroundColor: '#f59e0b', color: '#fff', fontSize: '16px', fontWeight: 700, cursor: 'pointer', flex: 2 },
  input: { width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', outline: 'none', boxSizing: 'border-box' as const },
  primaryBtn: { padding: '14px 24px', borderRadius: '12px', border: 'none', backgroundColor: '#3b82f6', color: '#fff', fontSize: '16px', fontWeight: 700, cursor: 'pointer' },
  centerMessage: { display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center' as const, padding: '48px' },
};
