 ...
 *
 * GlycoPharm StoreChannelsPage 패턴 동일 구조:
          ...
          {loadingList ? (
            ...
            <div className="space-y-2">
              {available.map(product => (
                <div
                  key={product.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-slate-200 hover:border-pink-300 hover:bg-pink-50/30 transition-colors"
                >
                  ...
                  <button
                    ...
                    className="ml-3 px-3 py-1.5 text-xs font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700 disabled:opacity-50 flex items-center gap-1"
                  >
                    {addingId === product.id ? (
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="text-sm text-slate-500 mb-1">
          <Link to="/store" className="text-pink-600 hover:underline">&larr; 대시보드</Link>
        </div>
        ...
        <div className="text-center py-16 bg-white rounded-lg border border-slate-200">
          ...
          <button
            ...
            disabled={creating}
            className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700 disabled:opacity-50"
          >
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="mb-6 p-5 rounded-xl border border-slate-200 bg-slate-50">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {([
            // Phase 3 K-Cosmetics: emoji → lucide (CHANNEL_TABS 와 채널 타입별 동일, SIGNAGE=Tv)
            ...
            >
              <Icon size={20} className={activeTab === item.type ? 'text-pink-700' : 'text-slate-500'} />
      ...
      {isProductChannel && currentChannel?.status === 'APPROVED' && !productLoading && (() => {
          ...
          return (
            <div className="flex items-start justify-between gap-4 p-5 mb-6 rounded-xl border border-amber-200 bg-amber-50">
              ...
              <button
                ...
                className="shrink-0 px-4 py-2 text-sm font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700 transition-colors"
              >
                이 채널에 상품 추가하기
              </button>
            </div>
        ...
        return (
          <div className="flex items-start justify-between gap-4 p-5 mb-6 rounded-xl border border-slate-200 bg-slate-50">
            ...
            <Link to="/store-hub/b2b" className="shrink-0 px-4 py-2 text-sm font-medium text-pink-700 bg-white border border-pink-200 rounded-lg hover:bg-pink-50 transition-colors">
              상품 보러가기
            </Link>
          </div>
        );
        ...
        <div className="rounded-lg border border-slate-200 p-4 bg-white">
          ...
          {currentChannel ? (
            ...
            <button
              onClick={handleCreateChannel}
              disabled={creating}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700 disabled:opacity-50"
            >
              {creating ? (
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className="px-3 py-1.5 text-xs font-medium text-pink-700 bg-pink-50 border border-pink-200 rounded-lg hover:bg-pink-100"
        >
          대시보드로 이동
        </button>
        <button
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            {currentChannel && currentChannel.status === 'APPROVED' && (
              <button
                onClick={() => setShowAddModal(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700"
              >
          ...
          {productLoading ? (
            ...
            <div className="text-center py-8 bg-white rounded-lg border border-slate-200">
              ...
              <button
                onClick={handleCreateChannel}
                disabled={creating}
                className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-xs font-medium text-white bg-pink-600 rounded-lg hover:bg-pink-700 disabled:opacity-50"
              >
                {creating ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                ) : (
      ...
      {currentTab.assetKey ? (
        channelAssets.length === 0 ? (
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-xs mt-1">자산 관리에서 채널 배치를 설정하거나, 콘텐츠를 가져오세요.</p>
          </div>
        ) : (
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <table className="w-full text-sm">
