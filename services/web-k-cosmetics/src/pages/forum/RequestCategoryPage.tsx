/**
 * RequestCategoryPage - 포럼 개설 신청 (K-Cosmetics)
 *
...
export default function RequestCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSubmitting(false);
      setIsSuccess(true);
      setTimeout(() => navigate('/forum/my-dashboard'), 3000);
    } catch {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="mb-8">
        <NavLink to="/forum/my-dashboard" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-800 mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
          <div>
            ...
            <input type="text" id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="예: K-뷰티 트렌드"
              className={`w-full px-4 py-3 rounded-lg border ${errors.name ? 'border-red-300 focus:ring-red-500' : 'border-slate-200 focus:ring-pink-500'} focus:outline-none focus:ring-2 focus:border-transparent`} />
          ...
          <div>
            ...
            <textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="이 포럼에서 어떤 주제를 다루나요?" rows={4}
              className={`w-full px-4 py-3 rounded-lg border ${errors.description ? 'border-red-300 focus:ring-red-500' : 'border-slate-200 focus:ring-pink-500'} focus:outline-none focus:ring-2 focus:border-transparent resize-none`} />
        ...
        <div className="flex gap-3">
          <NavLink to="/forum/my-dashboard" className="flex-1 px-6 py-3 text-center text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors font-medium">
            취소
          </NavLink>
          <button type="submit" disabled={isSubmitting}
            className="flex-1 px-6 py-3 text-white bg-pink-600 rounded-lg hover:bg-pink-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isSubmitting ? (
