...
import { Copy, X, ExternalLink, QrCode } from 'lucide-react';
...
export function HubQrLibraryPage() {
  ...
  //   HUB 조회와 가져오기 API(importOperatorQr) 는 adapter 로 그대로 주입한다 — backend · 계약 무변경.
  //   HUB 목록은 운영자 **원본**의 읽기 전용 진열이고, 가져오기가 만드는 것은 매장 소유 **사본**이다.
  const fetchPage = useCallback(
    ({ page: nextPage, limit }: { page: number; limit: number }) =>
      hubContentApi
        .list({ sourceDomain: 'qr', page: nextPage, limit })
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      <header className="mb-6 pb-5 border-b-2 border-slate-200">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          K-Cosmetics 운영자가 발행한 QR 콘텐츠입니다. 선택해 일괄 가져가기 또는 행 클릭으로 단건 가져가기를 할 수 있습니다.
          가져온 QR 은 매장 소유이며, 내 매장 QR 사본으로 복사되어 자유롭게 관리할 수 있습니다.
        </p>
      </header>
              ...
              actions={[
                {
                  ...
                  group: 'actions',
                  tooltip: '선택한 QR 을 내 매장 QR 사본으로 일괄 가져갑니다',
