/**
 * HubSignagePage — K-Cosmetics Store HUB 사이니지 라이브러리
 *
 * WO-O4O-STOREHUB-STRUCTURE-ALIGNMENT-V1: wrapper page 초기 추가
 * WO-O4O-KCOSMETICS-STORE-HUB-SIGNAGE-CANONICAL-ALIGNMENT-V1:
 *   GlycoPharm / KPA HubSignageLibraryPage canonical 패턴 이식.
 *   - hubContentApi (SERVICE_KEY='k-cosmetics' 자동 주입, src/lib/api/hubContent)
 *   - assetSnapshotApi.copy({ assetType: 'signage' }) → /cosmetics/assets/copy
...
export function HubSignagePage() {
  ...
  // Fetch media — K-Cosmetics hubContentApi는 SERVICE_KEY='k-cosmetics' 자동 주입
  ...
  // Single add
  // K-Cosmetics assetSnapshotApi.copy()는 sourceService 파라미터 없음
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <header className="mb-6 pb-5 border-b-2 border-slate-200">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          매장 화면에 송출할 콘텐츠와 플레이리스트를 탐색하고 내 매장에 추가합니다.
      ...
      {error && (
        <div className="text-center py-16 text-red-600 text-sm">
          ...
          <button
            ...
            className="mt-3 px-4 py-1.5 text-xs text-pink-600 border border-pink-400 rounded-lg hover:bg-pink-50"
          >
            다시 시도
          </button>
        </div>
      ...
      <div className="flex items-start gap-3 mt-8 p-5 bg-pink-50/60 border border-pink-100 rounded-xl text-sm text-slate-600 leading-relaxed">
        ...
        <span>
          추가한 콘텐츠는{' '}
          <Link to="/store/marketing/signage" className="text-pink-600 underline underline-offset-2">
            디지털사이니지 운영 화면
          </Link>
