...
export default function ContentHubPage() {
  ...
  const loadContent = async () => {
    ...
    try {
      if (contentType === 'playlists') {
        const result = await globalContentApi.listPlaylists(activeSource, 'k-cosmetics', { page: 1, limit: 50 });
        ...
        const result = await globalContentApi.listMedia(activeSource, 'k-cosmetics', { page: 1, limit: 50 });
        if (result.success && result.data) {
  ...
  const handleClonePlaylist = async (playlistId: string, playlistName: string) => {
    try {
      const result = await globalContentApi.clonePlaylist(playlistId, 'k-cosmetics');
      if (result.success) {
  ...
  const handleCloneMedia = async (mediaId: string, mediaName: string) => {
    try {
      const result = await globalContentApi.cloneMedia(mediaId, 'k-cosmetics');
      if (result.success) {
