 ...
 * 피드백 작성 권한 확인
 */
export function canWriteFeedback(userRole?: string): boolean {
  return userRole === 'district_admin' || userRole === 'branch_admin';
}

/**
 * 피드백 관리 권한 확인 (상태 변경, 고정 등)
 */
export function canManageFeedback(userRole?: string): boolean {
  return userRole === 'district_admin' || userRole === 'platform_admin';
}
