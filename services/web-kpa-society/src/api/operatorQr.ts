/**
 * Operator QR Template API Client — KPA Operator HUB QR Write
 *
 * WO-O4O-KPA-OPERATOR-QR-WRITE-PAGE-V1 (2026-05-24)
 *
 * 운영자가 KPA 매장 HUB 에 게시할 QR "템플릿" 을 작성/수정/게시하는 API client.
 * Backend 는 server-side 에서 authorRole='operator', serviceKey='kpa' 강제.
 * → 프론트는 title / description / targetType / target_* 만 전송한다.
 *
 * Backend: WO-O4O-KPA-OPERATOR-QR-PUBLISHING-PHASE2-BACKEND-V1
 *   /api/v1/kpa/operator/qr/templates (CRUD + publish + archive)
 *
 * 권한 (backend 검증):
 *   kpa:operator / kpa:admin / platform:super_admin
 *
 * 주의:
 *   - 운영자 단계에서는 slug / organizationId 없음 (entity 자체 부재)
 *   - 매장 가져가기 시 기존 store_qr_codes 에 매장 사본 INSERT (Phase 3-B 후속)
 *
 * 패턴: operatorPop.ts mirror — 다만 QR 도메인 차이로:
 *   - slug 없음 (운영자 단계 미발급)
 *   - excerpt/content → description + targetType + target_* 로 대체
 *   - publish/archive endpoint 동일 패턴
 */
...
function getApiBase(): string {
  ...
  return `${base}/api/v1/kpa/operator/qr`;
}
...
/**
 * 생성 input — targetType 별 union 으로 정합 보장.
 *   - target_type='url'     → targetUrl 필수, targetContent* 미사용
 *   - target_type='content' → targetContentKind + targetContentRef 필수, targetUrl 미사용
 */
