...
export function LmsLessonPage() {
  ...
  const [assignment, setAssignment] = useState<AssignmentLearner | null>(null);
  ...
  const [live, setLive] = useState<LiveLesson | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedAnswers({});
    setQuizResult(null);
    // WO-O4O-LMS-ASSIGNMENT-MINIMAL-V1: reset assignment state too
    setAssignment(null);
        ...
        {isLiveLesson ? (
          live && live.liveStartAt && live.liveEndAt && live.liveUrl ? (
            <Card padding="large" style={{ marginTop: '24px' }}>
              <h3 style={{ ...typography.headingS, color: colors.neutral900, marginBottom: '8px' }}>라이브 일정</h3>
              <p style={{ ...typography.bodyM, color: colors.neutral700 }}>
              ...
              {liveStatus === 'scheduled' && (
                <div style={{ marginTop: '16px', padding: '14px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px' }}>
                  <p style={{ ...typography.bodyM, color: '#1e40af' }}>
              ...
              {liveStatus === 'in_progress' && (
                <div style={{ marginTop: '16px' }}>
                  <button
                    style={{ ...styles.submitButton, opacity: liveJoining ? 0.6 : 1 }}
              ...
              {liveStatus === 'ended' && (
                <div style={{ marginTop: '16px' }}>
                  <a
                    ...
                    style={{ ...styles.navButton, display: 'inline-block', textDecoration: 'none' }}
                  >
                    ▶ 다시보기
                  </a>
                </div>
            ...
            <Card padding="large" style={{ marginTop: '24px', backgroundColor: '#fef3c7', border: '1px solid #fde68a' }}>
              <h3 style={{ ...typography.headingS, color: '#92400e', marginBottom: '8px' }}>라이브가 아직 준비되지 않았습니다</h3>
              <p style={{ ...typography.bodyM, color: '#92400e' }}>
                강사가 라이브 일정과 URL을 등록하면 참여할 수 있습니다.
              </p>
            </Card>
          )
        ) : isAssignmentLesson ? (
          /* WO-O4O-LMS-ASSIGNMENT-MINIMAL-V1: 과제 제출 영역 */
          assignment ? (
            <>
              <Card padding="large" style={{ marginTop: '24px' }}>
                <h3 style={{ ...typography.headingS, color: colors.neutral900, marginBottom: '8px' }}>과제 안내</h3>
                {assignment.instructions ? (
                  <p style={{ ...typography.bodyM, color: colors.neutral700, whiteSpace: 'pre-wrap' }}>
                    {assignment.instructions}
                  </p>
                ) : (
                  <p style={{ ...typography.bodyS, color: colors.neutral500 }}>설명이 등록되지 않았습니다.</p>
                )}
                {assignment.dueDate && (
                  <p style={{ ...typography.bodyS, color: '#92400e', marginTop: '8px' }}>
                    마감일: {new Date(assignment.dueDate).toLocaleString('ko-KR')}
                  </p>
                )}
              </Card>

              <Card padding="large" style={{ marginTop: '16px' }}>
            ...
            <Card padding="large" style={{ marginTop: '24px', backgroundColor: '#fef3c7', border: '1px solid #fde68a' }}>
              <h3 style={{ ...typography.headingS, color: '#92400e', marginBottom: '8px' }}>과제가 아직 준비되지 않았습니다</h3>
              <p style={{ ...typography.bodyM, color: '#92400e' }}>
                강사가 과제 내용을 등록하면 제출할 수 있습니다.
              </p>
            </Card>
          )
        ) : isQuizLesson ? (
