/**
 * OperatorBlogListPage — 운영자 매장 HUB 블로그 목록 (표준 테이블)
 *
 ...
 *   BulkResultModal + RowActionMenu + defineActionPolicy). MemberManagementPage mirror.
 *
...
// MemberManagementPage 패턴 mirror.
const blogActionPolicy = defineActionPolicy<OperatorBlogPost>('kpa:operator-blog', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    {
      ...
      confirm: {
        title: '블로그 발행',
        message: '이 블로그를 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.',
        confirmText: '발행',
      },
    },
    {
...
export default function OperatorBlogListPage() {
  ...
  // ── Single row actions ──────────────────────────────────────────
  // 확인은 blogActionPolicy.publish.confirm (RowActionMenu → ConfirmActionDialog) 가 담당.
  ...
  const handleBulkPublish = useCallback(() => {
    ...
    setPendingBulk({
      op: 'publish', ids,
      title: '일괄 발행',
      message: `선택한 ${ids.length}개 블로그를 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.`,
      confirmText: '발행',
    });
  ...
  const handleBulkArchive = useCallback(() => {
    ...
    setPendingBulk({
      op: 'archive', ids,
      title: '일괄 보관',
      message: `선택한 ${ids.length}개 블로그를 보관하시겠습니까?`,
      confirmText: '보관',
    });
  ...
  const handleBulkDelete = useCallback(() => {
    ...
    setPendingBulk({
      op: 'delete', ids,
      title: '일괄 삭제',
      message: `선택한 ${ids.length}개 블로그를 삭제하시겠습니까? 되돌릴 수 없습니다.`,
      variant: 'danger',
      confirmText: '삭제',
    });
  }, [selectedAllIds]);
