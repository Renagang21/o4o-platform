/**
 * OperatorPopListPage — 운영자 매장 HUB POP 목록 (표준 테이블)
 *
 * WO-O4O-KPA-OPERATOR-POP-WRITE-PAGE-V1 (초기 카드형)
 ...
 *
 * 운영자가 KPA 매장 HUB 에 게시한 POP 목록. 상태별 필터 + 행 액션 + 일괄 발행/보관/삭제.
 *
...
// ─── Action Policy ─────────────────────
const popActionPolicy = defineActionPolicy<OperatorPopPost>('kpa:operator-pop', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    {
      ...
      confirm: {
        title: 'POP 발행',
        message: '이 POP 을 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.',
        confirmText: '발행',
      },
    },
    {
      ...
      confirm: {
        title: 'POP 보관',
        message: '이 POP 을 보관하시겠습니까? HUB 노출이 중단됩니다.',
        confirmText: '보관',
      },
    },
    {
      ...
      confirm: {
        title: 'POP 삭제',
        message: '이 POP 을 삭제하시겠습니까? 되돌릴 수 없습니다.',
        variant: 'danger',
        confirmText: '삭제',
      },
    },
...
export default function OperatorPopListPage() {
  ...
  // ── Single row actions ──
  // 확인은 popActionPolicy.publish.confirm (RowActionMenu → ConfirmActionDialog) 가 담당.
  ...
  const batchPopOp = useCallback(
    async (
      ...
      const fn =
        op === 'publish' ? publishOperatorPopPost
        : op === 'archive' ? archiveOperatorPopPost
        : deleteOperatorPopPost;
  ...
  const handleBulkPublish = useCallback(() => {
    ...
    setPendingBulk({
      op: 'publish', ids,
      title: '일괄 발행',
      message: `선택한 ${ids.length}개 POP 을 발행하시겠습니까? 발행 즉시 매장 HUB 에 노출됩니다.`,
      confirmText: '발행',
    });
  ...
  const handleBulkArchive = useCallback(() => {
    ...
    setPendingBulk({
      op: 'archive', ids,
      title: '일괄 보관',
      message: `선택한 ${ids.length}개 POP 을 보관하시겠습니까?`,
      confirmText: '보관',
    });
  }, [selectedNotArchivedIds]);
  const handleBulkDelete = useCallback(() => {
    const ids = items.filter((p) => selectedIds.has(p.id)).map((p) => p.id);
    if (ids.length === 0) return;
    setPendingBulk({
      op: 'delete', ids,
      title: '일괄 삭제',
      message: `선택한 ${ids.length}개 POP 을 삭제하시겠습니까? 되돌릴 수 없습니다.`,
      variant: 'danger',
      confirmText: '삭제',
    });
  }, [items, selectedIds]);
