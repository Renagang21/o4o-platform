 ...
 *
 * 콘텐츠 허브의 "설문조사" 전용 목록.
 * 설문 데이터는 별도 도메인(participation_sets) — participationApi 재사용.
 * /participation 단순 redirect 대신, 콘텐츠 허브 안의 하위 섹션으로 노출.
 *
 * API:
 *   - GET /participation/sets (participationApi.getParticipationSets)
 *
...
/**
 * 클릭 시 이동 경로.
 * ACTIVE 상태: 응답 페이지 / 그 외: 결과 페이지 (DRAFT는 결과가 없을 수 있으나
 * ParticipationListPage 기존 패턴 그대로 유지).
 */
...
export function ContentSurveysPage() {
  ...
  const load = useCallback((pageNum: number) => {
    ...
    participationApi.getParticipationSets({ page: pageNum, limit: PAGE_LIMIT })
      .then((res: any) => {
        // PaginatedResponse 형태가 환경에 따라 다를 수 있어 양쪽 모두 수용
        const payload = res?.data ?? res;
        const list = payload?.items ?? payload?.data?.items ?? [];
        const t = payload?.total ?? payload?.data?.total ?? 0;
