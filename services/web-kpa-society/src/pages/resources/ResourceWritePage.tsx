            ...
            {USAGE_TYPE_OPTIONS.map((opt) => (
              <label
                ...
                style={{
                  ...
                  gap: 6,
                  padding: '8px 14px',
                  border: `1.5px solid ${usageType === opt.value ? '#2563eb' : '#e2e8f0'}`,
                  borderRadius: 8,
                  backgroundColor: usageType === opt.value ? '#dbeafe' : '#f8fafc',
                  cursor: 'pointer',
                  fontSize: '0.8125rem',
                  fontWeight: usageType === opt.value ? 700 : 400,
                  color: usageType === opt.value ? '#1d4ed8' : '#64748b',
                  transition: 'all 0.15s',
                  boxShadow: usageType === opt.value ? '0 0 0 3px rgba(37,99,235,0.1)' : 'none',
                }}
              >
                <input
                  type="radio"
...
const styles: Record<string, React.CSSProperties> = {
  ...
  // WO-O4O-KPA-RESOURCES-CREATE-MODAL-UX-V1: chip 스타일 개선 (파란색 계열, 테두리 추가)
  tagInputWrap: {
    display: 'flex',
