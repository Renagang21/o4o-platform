/**
 * Store Local Products Management Page (KPA)
 *
 ...
 *   목록 화면(헤더·검색·활성필터·loading/error/empty·table·pagination·toast·CRUD 흐름)을
 *   공통 StoreLocalProductsManager(@o4o/store-ui-core)로 위임한다.
 *   KPA 고유는 슬롯으로 유지 — BaseTable 렌더(tableVariant='base') · 다국어 컬럼(extraColumns) ·
 *   리치 등록/수정 폼(renderFormModal: 미디어 라이브러리 · RichTextEditor · 콘텐츠 가져오기 · 바코드).
 *   API·route·권한 변경 없음.
 *
...
export default function StoreLocalProductsPage() {
  ...
  const api = useMemo(
    () => ({ fetchLocalProducts, createLocalProduct, updateLocalProduct, deleteLocalProduct }),
    [],
  );

  // KPA 고유 컬럼 — 다국어 콘텐츠 연결 상태
  const extraColumns = useMemo<Array<StoreLocalProductsExtraColumn<LocalProduct>>>(
    () => [
      {
        key: 'multilingual',
        header: '다국어',
        width: 160,
        render: (product) => <MultilingualContentBadge summary={mlcSummary.get(product.id)} />,
      },
    ],
    [mlcSummary],
  );
  ...
  return (
    <StoreLocalProductsManager<LocalProduct, LocalProductInput>
      ...
      renderFormModal={({ product, saving, error, onSave, onClose }) => (
        <ProductFormModal
          product={product}
          saving={saving}
          error={error}
