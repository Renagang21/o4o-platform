...
export default function ServiceSwitcher({ currentServiceKey }: ServiceSwitcherProps) {
  ...
  const fetchServices = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<{ success: boolean; data: { services: ServiceInfo[] } }>('/auth/services');
      const data = res.data;
  ...
  const handleOpen = async (serviceKey: string) => {
    ...
    try {
      const res = await api.post<{ success: boolean; data: { targetUrl: string } }>('/auth/handoff', { targetServiceKey: serviceKey });
      const data = res.data;
  ...
  const handleJoin = async (serviceKey: string) => {
    setActionKey(serviceKey);
    try {
      const res = await api.post<{ success: boolean }>(`/auth/services/${serviceKey}/join`, {});
      const data = res.data;
      if (data.success) {
        setLoaded(false);
        await fetchServices();
