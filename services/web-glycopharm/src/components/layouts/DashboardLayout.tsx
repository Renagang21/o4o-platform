...
const roleConfig: Record<string, { title: string; icon: typeof Building2; color: string; menuItems: Array<{ path: string; label: string; icon: typeof LayoutDashboard }> }> = {
  ...
  operator: {
    ...
    menuItems: [
      { path: '/admin', label: '대시보드', icon: LayoutDashboard },
      { path: '/admin/applications', label: '신청 관리', icon: FileCheck },
      { path: '/admin/products', label: '상품 관리', icon: Package },
      { path: '/admin/orders', label: '주문 관리', icon: ShoppingCart },
      { path: '/admin/inventory', label: '재고/공급', icon: Boxes },
      { path: '/admin/settlements', label: '정산 관리', icon: CreditCard },
      { path: '/admin/analytics', label: '분석/리포트', icon: BarChart3 },
      { path: '/admin/reports', label: '청구 리포트', icon: FileText },
      { path: '/admin/billing-preview', label: '청구 미리보기', icon: Briefcase },
      { path: '/admin/invoices', label: '인보이스', icon: CreditCard },
      { path: '/admin/marketing', label: '마케팅', icon: Megaphone },
      { path: '/admin/forum-requests', label: '포럼 신청', icon: MessageSquare },
      { path: '/admin/forum-management', label: '포럼 관리', icon: FileText },
      { path: '/admin/market-trial', label: 'Trial 관리', icon: Tag },
      { path: '/admin/signage/content', label: '콘텐츠 허브', icon: Monitor },
      { path: '/admin/signage/library', label: '콘텐츠 라이브러리', icon: Monitor },
      { path: '/admin/signage/my', label: '내 사이니지', icon: Tv },
      { path: '/admin/signage/hq-media', label: 'HQ 미디어', icon: Monitor },
      { path: '/admin/signage/hq-playlists', label: 'HQ 플레이리스트', icon: Monitor },
      { path: '/admin/signage/templates', label: '템플릿', icon: Monitor },
      { path: '/admin/support', label: '고객지원', icon: HelpCircle },
      { path: '/admin/ai-report', label: 'AI 리포트', icon: BarChart3 },
      { path: '/admin/users', label: '회원 관리', icon: Users },
    ],
  },
};
