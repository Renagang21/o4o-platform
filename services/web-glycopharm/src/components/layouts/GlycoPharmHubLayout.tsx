 ...
 *
 * WO-O4O-GLYCOPHARM-STORE-HUB-PORT-V1: wrapper 최초 추가
 * WO-O4O-STORE-HUB-COMMON-VIEW-AND-SHELL-UNIFICATION-V1:
 *   KPA·K-Cosmetics 와 동일했던 사이드바/드로어 마크업을 공통 `StoreHubShell` 로 이관.
 *   이 파일은 이제 **메뉴 config + accent + label + 서비스 푸터** 만 소유한다.
 *   메뉴 항목 / 라우트 / 권한 / accent(teal) 무변경.
 */
...
/** GlycoPharm 은 그룹 헤더 없는 평면 메뉴 — 단일 무명 그룹으로 표현한다. */
const HUB_MENU: StoreHubNavGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        key: 'content',
        label: '콘텐츠',
        description: '플랫폼 콘텐츠를 탐색하고 내 매장에 적용합니다',
        icon: FileText,
      ...
      },
      {
        key: 'event-offers',
        label: '이벤트/특가',
        description: 'GlycoPharm 이벤트 상품을 확인하고 신청합니다',
        icon: Tag,
