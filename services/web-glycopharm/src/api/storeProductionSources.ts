 ...
 * - QR: GET /glycopharm/pharmacy/qr (store-qr-landing controller) → { data: { items } }
 * - direct: GET /glycopharm/store-contents → { data: [...] } 중 sourceType='direct' 필터 (KPA 동일)
...
export async function getStoreQrCodes(opts?: { limit?: number }): Promise<any[]> {
  ...
  const res = await api.get(`/glycopharm/pharmacy/qr${qs ? `?${qs}` : ''}`);
...
export async function getStoreDirectContents(): Promise<any[]> {
  const res = await api.get('/glycopharm/store-contents');
