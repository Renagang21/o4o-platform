/**
 * QR Staff API Client — Authenticated (GlycoPharm Store Hub QR Import)
 *
 ...
 * 기존 storeQr.ts (또는 동등) 가 cover.
 *
 * Backend: WO-O4O-GLYCOPHARM-STORE-HUB-POP-QR-STAFF-BACKEND-V1
 *   POST /api/v1/glycopharm/stores/:slug/qr/staff/import
 *
 * 패턴: KPA qrStaff.ts mirror — sourceId 만 전송.
 ...
 * 매장 QR 페이지가 store_qr_codes 목록 조회로 표시.
 */
 ...
 *
 * Backend 가:
 *   - 원본 검증: author_role='operator' AND status='published' AND service_key='glycopharm'
