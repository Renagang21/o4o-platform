/**
 * Store Analytics API Client — GlycoPharm
 *
 * WO-O4O-STORE-MARKETING-ANALYTICS-CROSSSERVICE-V1
 *
 * 매장 마케팅 성과 분석 데이터 조회.
 * Backend: /api/v1/glycopharm/pharmacy/analytics/marketing
 *   (createStoreAnalyticsController with serviceKey='glycopharm')
 */
...
export async function getMarketingAnalytics(): Promise<{
  ...
  const res = await api.get<{ success: boolean; data: MarketingAnalyticsData }>(
    '/glycopharm/pharmacy/analytics/marketing',
  );
  return res.data;
}
