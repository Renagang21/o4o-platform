...
export default function ForumMemberManagementPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setJoinRequests((reqRes as any).data?.data || (reqRes as any).data || []);
      setMembers((memRes as any).data?.data || (memRes as any).data || []);
    } catch (err: any) {
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      setActionError(err?.response?.data?.error || err?.message || '승인에 실패했습니다.');
    } finally {
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      ...
      setActionError(err?.response?.data?.error || err?.message || '거절에 실패했습니다.');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  const handleRemoveMember = async (userId: string, userName: string | null) => {
    ...
    try {
      ...
      setActionError(err?.response?.data?.error || err?.message || '회원 삭제에 실패했습니다.');
    } finally {
      setActionLoading(null);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <Link to="/forum/my-dashboard" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 transition-colors mb-4">
      ...
      {!loading && error && (
        <div className="p-4 bg-red-50 rounded-xl border border-red-200 flex items-start gap-3">
          ...
          <div>
            ...
            <Link to="/forum/my-dashboard" className="text-sm text-red-600 font-medium hover:text-red-700 mt-1 inline-block">
              ← 내 포럼으로 돌아가기
            </Link>
          </div>
        </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="font-medium text-slate-800">
                          {req.user_display_name || req.requester_name || req.user_name || '(이름 없음)'}
                        </div>
                        <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                          {(req.requester_email || req.user_email) && (
                            <span>{req.requester_email || req.user_email}</span>
                          )}
                      ...
                      {rejectingId === req.id ? (
                        ...
                        <div className="flex items-center gap-2 shrink-0 ml-3">
                          <button
                            ...
                            disabled={actionLoading === req.id}
                            className="px-3 py-1.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center gap-1"
                          >
