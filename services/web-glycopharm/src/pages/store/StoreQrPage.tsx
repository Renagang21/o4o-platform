 ...
 *
 * GlycoPharm Store Execution 마지막 축.
 * 약국 오너가 QR 코드를 생성/관리/다운로드.
 *
 * 기능:
...
export default function StoreQrPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await qrFetch<{ success: boolean; data: { items: QrItem[] } }>(
        '/pharmacy/qr',
      );
  ...
  const handleDownload = async (qr: QrItem) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/v1/glycopharm/pharmacy/qr/${qr.id}/image?format=png&size=512`,
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', paddingBottom: 80 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>
            약국 QR 코드 생성 및 관리 — 총 {items.length}개
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={loadItems} style={refreshBtnStyle}>
            <RefreshCw size={13} />
          </button>
      ...
      {error && !loading && (
        <div style={{ textAlign: 'center', padding: 60 }}>
          ...
          <button onClick={loadItems} style={retryBtnStyle}>
      ...
      {!loading && !error && items.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8' }}>
          ...
          <p style={{ fontSize: 13, color: '#cbd5e1', marginBottom: 20 }}>
            새 QR 코드를 만들어 약국 자료, 홈페이지, 이벤트 페이지로 연결하세요
          </p>
          <button onClick={openCreate} style={createBtnStyle}>
            <Plus size={14} /> 첫 QR 만들기
          </button>
        </div>
      )}
          ...
          {items.map((qr) => {
            ...
            return (
              <div key={qr.id} style={qrCardStyle}>
                {/* Left: icon */}
                <div style={{ width: 48, height: 48, borderRadius: 10, backgroundColor: '#f0fdfa', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${typeConf.cls}`}>
                      {typeConf.label}
                    </span>
                    {!qr.isActive && (
                      <span style={{ fontSize: 11, backgroundColor: '#f1f5f9', color: '#94a3b8', padding: '2px 8px', borderRadius: 4 }}>
                        비활성
                      </span>
                    )}
                  </div>
                  ...
                  {(Object.keys(LANDING_TYPE_CONFIG) as LandingType[]).map((type) => (
                    <button
                      ...
                      style={{
                        padding: '6px 14px',
                        borderRadius: 8,
                        border: `2px solid ${form.landingType === type ? '#0d9488' : '#e2e8f0'}`,
                        backgroundColor: form.landingType === type ? '#f0fdfa' : '#fff',
                        color: form.landingType === type ? '#0d9488' : '#64748b',
...
const createBtnStyle: React.CSSProperties = {
  ...
  padding: '8px 16px', backgroundColor: '#0d9488', color: '#fff',
