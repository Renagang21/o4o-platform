 ...
 *
 * KPA StoreChannelsPage 패턴 동일 구조:
  ...
  if (guidance) {
    return (
      <div className="flex items-center gap-3 p-4 mb-6 rounded-lg border border-slate-200 bg-slate-50">
        ...
        {guidanceLink && (
          <Link to={guidanceLink} className="flex-shrink-0 inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-blue-700 bg-white border border-blue-200 rounded-lg hover:bg-blue-50">
  ...
  return (
    <div className="flex items-center gap-3 p-4 mb-6 rounded-lg border border-slate-200 bg-white">
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        <a href={url!} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100" title="새 탭에서 열기">
      ...
      {currentTab.assetKey ? (
        channelAssets.length === 0 ? (
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-xs mt-1">자산 관리에서 채널 배치를 설정하거나, 매장 HUB에서 콘텐츠를 가져오세요.</p>
          </div>
        ) : (
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <table className="w-full text-sm">
