 ...
 *
 * GlycoPharm 약국 경영자 (glycopharm:store_owner / pharmacy_owner subRole) 가 가입 후
 * 자신의 약국·사업자 정보를 확인하고 수정할 수 있는 화면.
 *
...
function dataToForm(data: PharmacyBusinessInfo): FormState {
  return {
...
export default function PharmacyInfoPage() {
  const [data, setData] = useState<PharmacyBusinessInfo | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const result = await mypageApi.getBusinessInfo();
      setData(result);
      setLoadState('loaded');
    } catch (e: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: UpdatePharmacyBusinessInfoPayload = {
      ...
      const updated = await mypageApi.updateBusinessInfo(payload);
      setData(updated);
      setIsEditMode(false);
      setSuccessMsg('약국/사업자 정보가 저장되었습니다.');
    ...
    return (
      <div className="max-w-3xl mx-auto p-6">
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-900">약국 경영자만 이용 가능합니다</p>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <header className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">약국/사업자 정보</h1>
        <p className="text-sm text-gray-500 mt-1">
          가입 및 운영에 사용되는 약국 사업자 정보를 확인하고 수정합니다.
        </p>
      </header>
          ...
          {isEditMode ? (
            <>
              ...
              <Field label="약사면허번호">
                <input
                  className={`${inputCls} bg-gray-100 cursor-not-allowed`}
                  value={data.pharmacistLicenseNumber || ''}
                  disabled
                />
                <p className="text-xs text-gray-500 mt-1">
                  면허번호 변경은 자격 확인 절차를 통해 진행됩니다.
                </p>
              </Field>
            ...
            <>
              <ViewRow label="약국명" value={data.pharmacyName} />
              <ViewRow
                label="사업자등록번호"
                value={data.businessRegistrationNumber}
                badge="변경 불가"
...
const inputCls =
  'w-full px-3 py-2 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500';
