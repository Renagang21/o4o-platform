/**
 * B2BTableList — GlycoPharm 로컬 B2B 테이블 컴포넌트
 *
 * WO-O4O-HUB-EXPLORATION-CORE-REMOVAL-PREP-V1
 *
 * @o4o/hub-exploration-core에서 분리. 타입·테마·스타일 주입 모두 자체 포함.
 * 카테고리 탭, 정렬, 페이지네이션, 데스크톱/모바일 반응형 내장.
 */
...
const NEUTRALS: Record<number, string> = {
...
export function B2BTableList({
  ...
  const sorted = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      else if (sortKey === 'supplierName') cmp = (a.supplierName || '').localeCompare(b.supplierName || '', 'ko');
  ...
  function handleSort(key: B2BTableSortKey) {
    if (onSortChange) { onSortChange(key); }
      ...
      if (internalSortKey === key) setInternalSortOrder(prev => prev === 'asc' ? 'desc' : 'asc');
      else { setInternalSortKey(key); setInternalSortOrder('asc'); }
    }
    setPage(0);
  }
  ...
  const sortLabels: Record<B2BTableSortKey, string> = { name: '상품명', supplierName: '공급처', createdAt: '등록일' };
  ...
  return (
    <div style={S.container}>
      ...
      {categories && categories.length > 0 && (
        <div style={S.tabs}>
          {categories.map(cat => (
            <button key={cat} type="button" style={activeCategory === cat ? { ...S.tab, ...S.tabActive } : S.tab} onClick={() => handleCategoryChange(cat)}>
              {cat}
            </button>
          ))}
        </div>
      ...
      {/* Info Bar */}
      <div style={S.infoBar}>
        <span style={S.infoCount}>총 <strong style={{ color: NEUTRALS[900] }}>{items.length}</strong>개 상품</span>
        <div style={S.infoRight}>
          {activeCategory && activeCategory !== '전체' && <span style={S.infoBadge}>{activeCategory}</span>}
          <span style={S.infoSort}>{sortLabels[sortKey]} {sortOrder === 'asc' ? '▲' : '▼'}</span>
        </div>
      </div>

      {items.length === 0 ? (
        <div style={S.empty}>{emptyMessage || '등록된 B2B 상품이 없습니다.'}</div>
      ) : (
        <>
          {/* Desktop Table */}
          <div className="b2b-table-desktop" style={S.tableWrap}>
            <table style={S.table}>
              <thead>
                <tr>
                  <th style={sortKey === 'name' ? { ...S.th, ...S.thSortable, ...S.thActive } : { ...S.th, ...S.thSortable }} onClick={() => handleSort('name')}>상품명{sortIcon('name')}</th>
                  <th style={S.th}>단위</th>
                  <th style={S.th}>단가</th>
                  <th style={{ ...S.th, color: PRIMARY, fontWeight: 700 }}>공급가</th>
                  <th style={sortKey === 'supplierName' ? { ...S.th, ...S.thSortable, ...S.thActive } : { ...S.th, ...S.thSortable }} onClick={() => handleSort('supplierName')}>공급처{sortIcon('supplierName')}</th>
                  <th style={S.th}>법적분류</th>
                  <th style={sortKey === 'createdAt' ? { ...S.th, ...S.thSortable, ...S.thActive } : { ...S.th, ...S.thSortable }} onClick={() => handleSort('createdAt')}>등록일{sortIcon('createdAt')}</th>
                  <th style={{ ...S.th, textAlign: 'center' as const, minWidth: '140px' }}>액션</th>
                </tr>
              </thead>
              <tbody>
                {paged.map(item => (
                  <tr key={item.id} className="b2b-table-row" style={rowStyle(item)}>
                    ...
                    <td style={S.td}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        {item.discountRate != null && item.discountRate > 0 && <span style={S.discountBadge}>-{item.discountRate}%</span>}
                      </div>
                    </td>
                    <td style={S.td}>
                      {item.supplierName ? (
                    ...
                    <td style={S.td}>{item.legalCategory ? <span style={S.categoryBadge}>{item.legalCategory}</span> : '-'}</td>
                    <td style={S.td}>{formatDate(item.createdAt)}</td>
                    <td style={{ ...S.td, textAlign: 'center' as const }}>
                      <div style={S.actionGroup}>
                        {item.onClick && <button type="button" style={S.detailBtn} onClick={(e) => { e.stopPropagation(); item.onClick!(); }}>상세 보기</button>}
                        {renderAction(item)}
                      </div>
                    </td>
                  </tr>
            ...
            {paged.map(item => (
              <div key={item.id} className="b2b-table-row" style={isSoldout(item) ? { ...S.mobileCard, opacity: 0.5 } : S.mobileCard}>
                ...
                <div style={S.mobileRow2}>
                  <span style={S.mobileMeta}>{item.unit || '-'} · {item.supplierName || '-'}{item.supplierVerified && <span style={S.verifiedIcon}> ✓</span>}</span>
                </div>
                <div style={S.mobileRow3}>
                  <div style={S.mobilePriceGroup}>
                    {item.supplyPrice != null && <span style={S.mobileSupplyPrice}>{formatPrice(item.supplyPrice)}</span>}
                    {item.discountRate != null && item.discountRate > 0 && <span style={S.discountBadge}>-{item.discountRate}%</span>}
                    {item.price != null && item.discountRate != null && item.discountRate > 0 && <span style={S.mobilePrice}>{formatPrice(item.price)}</span>}
                  </div>
                </div>
                <div style={S.mobileActionRow}>
                  {item.onClick && <button type="button" style={S.detailBtnMobile} onClick={(e) => { e.stopPropagation(); item.onClick!(); }}>상세 보기</button>}
          ...
          {totalPages > 1 && (
            <div style={S.pagination}>
              <button type="button" style={page === 0 ? { ...S.pageBtn, ...S.pageBtnDisabled } : S.pageBtn} disabled={page === 0} onClick={() => setPage(p => p - 1)}>‹</button>
              <div className="b2b-table-desktop" style={{ display: 'flex', gap: '4px' }}>
                {getPageNumbers().map(p => (
                  <button key={p} type="button" style={p === page ? { ...S.pageNumBtn, ...S.pageNumActive } : S.pageNumBtn} onClick={() => setPage(p)}>{p + 1}</button>
                ))}
              </div>
              <span className="b2b-table-mobile" style={S.pageInfo}>{page + 1} / {totalPages}</span>
              <button type="button" style={page >= totalPages - 1 ? { ...S.pageBtn, ...S.pageBtnDisabled } : S.pageBtn} disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)}>›</button>
            </div>
          )}
        </>
      )}
...
const S: Record<string, React.CSSProperties> = {
  container: { width: '100%' },
  title: { margin: '0 0 16px', fontSize: '1.25rem', fontWeight: 700, color: NEUTRALS[900] },
  tabs: { display: 'flex', gap: '4px', marginBottom: '0', overflowX: 'auto' as const, borderBottom: `1px solid ${NEUTRALS[200]}`, paddingBottom: '0' },
  tab: { padding: '8px 16px', background: 'transparent', border: 'none', borderBottom: '2px solid transparent', fontSize: '0.875rem', fontWeight: 500, color: NEUTRALS[500], cursor: 'pointer', whiteSpace: 'nowrap' as const, fontFamily: 'inherit', marginBottom: '-1px' },
  tabActive: { color: PRIMARY, borderBottomColor: PRIMARY, fontWeight: 600 },
  infoBar: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', marginBottom: '4px' },
  infoCount: { fontSize: '0.8125rem', color: NEUTRALS[500] },
  infoRight: { display: 'flex', alignItems: 'center', gap: '8px' },
  infoBadge: { display: 'inline-block', padding: '2px 8px', fontSize: '0.6875rem', fontWeight: 600, borderRadius: '10px', backgroundColor: `${PRIMARY}10`, color: PRIMARY },
  infoSort: { fontSize: '0.75rem', color: NEUTRALS[400], fontWeight: 500 },
  empty: { padding: '48px 20px', textAlign: 'center' as const, color: NEUTRALS[400], fontSize: '0.875rem' },
  tableWrap: { overflowX: 'auto' as const, borderRadius: '8px', border: `1px solid ${NEUTRALS[200]}` },
  table: { width: '100%', borderCollapse: 'collapse' as const, fontSize: '0.8125rem' },
  th: { padding: '10px 12px', textAlign: 'left' as const, fontSize: '0.75rem', fontWeight: 600, color: NEUTRALS[500], backgroundColor: NEUTRALS[50], borderBottom: `1px solid ${NEUTRALS[200]}`, whiteSpace: 'nowrap' as const, userSelect: 'none' as const },
  thSortable: { cursor: 'pointer' },
  thActive: { color: PRIMARY, backgroundColor: `${PRIMARY}06` },
  tr: { borderBottom: `1px solid ${NEUTRALS[100]}` },
  td: { padding: '10px 12px', color: NEUTRALS[600], whiteSpace: 'nowrap' as const, verticalAlign: 'middle' as const },
  tdDesc: { fontSize: '0.6875rem', color: NEUTRALS[400], marginTop: '2px', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '220px' },
  supplyPriceDesktop: { fontWeight: 700, color: PRIMARY, fontSize: '0.8125rem' },
  ...
  mobileCard: { padding: '14px 16px', backgroundColor: '#ffffff', border: `1px solid ${NEUTRALS[200]}`, borderRadius: '10px' },
  mobileRow1: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px', marginBottom: '4px' },
  mobileName: { fontSize: '0.875rem', fontWeight: 600, color: NEUTRALS[800], overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const },
  mobileDesc: { fontSize: '0.6875rem', color: NEUTRALS[400], marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const },
  mobileRow2: { marginBottom: '8px' },
  mobileMeta: { fontSize: '0.75rem', color: NEUTRALS[500] },
  mobileRow3: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' },
  mobilePriceGroup: { display: 'flex', alignItems: 'baseline', gap: '6px' },
  mobileSupplyPrice: { fontSize: '1.0625rem', fontWeight: 700, color: PRIMARY },
  ...
  pageNumBtn: { padding: '4px 10px', background: 'transparent', border: '1px solid transparent', borderRadius: '6px', fontSize: '0.8125rem', fontWeight: 500, color: NEUTRALS[500], cursor: 'pointer', fontFamily: 'inherit', minWidth: '32px', textAlign: 'center' as const },
  pageNumActive: { background: PRIMARY, color: '#ffffff', fontWeight: 600, borderColor: PRIMARY },
  pageInfo: { fontSize: '0.8125rem', color: NEUTRALS[500], padding: '0 8px' },
};
