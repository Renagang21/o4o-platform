 ...
 *   KPA HubSignageLibraryPage canonical 패턴 이식.
 *   - hubContentApi (SERVICE_KEY='glycopharm' 자동 주입)
 *   - assetSnapshotApi.copy({ assetType: 'signage' }) — sourceService 불필요
 *
...
export function HubSignageLibraryPage() {
  ...
  // Fetch media — GlycoPharm hubContentApi는 SERVICE_KEY='glycopharm' 자동 주입
  ...
  // Single add
  // GlycoPharm assetSnapshotApi.copy()는 sourceService 파라미터 없음 (service-specific endpoint)
  const handleCopySingle = useCallback(async (item: HubContentItemResponse) => {
    try {
      ...
      if (msg.includes('DUPLICATE') || msg.includes('already') || e?.code === 'DUPLICATE_SNAPSHOT') {
        toast.error('이미 약국에 추가된 항목입니다.');
      } else if (e?.status === 403) {
        toast.error('추가 권한이 없습니다. 약국 계정으로 로그인되어 있는지 확인하세요.');
      } else {
        toast.error(`약국 추가에 실패했습니다. (${msg || '서버 오류'})`);
      }
    }
  }, []);
