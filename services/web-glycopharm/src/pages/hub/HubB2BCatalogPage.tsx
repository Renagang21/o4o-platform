/**
 * HubB2BCatalogPage — GlycoPharm Store HUB 상품 카탈로그
 *
 * WO-O4O-B2B-OPERATION-TABLE-STRUCTURE-V1
 * WO-O4O-GLYCOPHARM-STORE-HUB-B2B-CATALOG-KPA-ALIGNMENT-V1:
 *   KPA-Society canonical 정렬 — 로컬 B2BTableList(품목 탭) → @o4o/operator-ux-core DataTable
 *   + checkbox multi-select + ActionBar(bulk 내 매장에 추가). 유통유형 탭(전체/B2B/운영자/판매자 모집).
 *   품목(의약품/건기식/…) 탭 제거 — 품목·대상·노출 제어는 Neture 공급자/운영자 영역 책임.
      ...
      render: (_v, row) => (
        <div className="flex flex-col gap-0.5 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {row.isAdded && (
              <span className="inline-block px-1.5 py-0.5 text-[10px] font-semibold rounded bg-teal-50 text-teal-700">내 매장</span>
            )}
          </div>
          {row.description && (
        ...
        if (row.isAdded) {
          return (
            <div className="flex items-center justify-center gap-1">
              <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-teal-50 text-teal-600" title="이미 내 매장에 추가됨">
        ...
        return (
          <div className="flex items-center justify-center">
            <button
              ...
              className="inline-flex items-center justify-center w-7 h-7 rounded-full text-teal-600 hover:bg-teal-50 disabled:opacity-60"
            >
    ...
    <div className="px-1 py-2">
      ...
      {error ? (
        ...
        <>
          ...
          <DataTable<CatalogProduct>
            ...
            tableId="glyco-store-hub-b2b-products"
            selectable
      ...
      <div className="flex items-start gap-3 px-5 py-4 bg-teal-50/60 border border-teal-100 rounded-xl mt-6 text-sm text-slate-600 leading-relaxed">
        ...
        <span>
          ...
          추가된 상품은 <a href="/store/channels" className="text-teal-700 underline underline-offset-2 hover:text-teal-800">채널 관리</a>에서 진열하면 고객에게 보여집니다.
        </span>
      </div>
      ...
      {products.some(p => p.isAdded) && (
        <div className="flex items-start gap-3 px-5 py-4 bg-green-50 border border-green-200 rounded-xl mt-3 text-sm text-slate-600 leading-relaxed">
          ...
          <span>
            ...
            <a href="/store/channels" className="text-teal-700 font-semibold underline underline-offset-2 hover:text-teal-800">채널 관리로 이동 →</a>
          </span>
        </div>
      )}
    </div>
