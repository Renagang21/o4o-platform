/**
 * HubEventOffersPage — GlycoPharm 이벤트 오퍼 (StoreHub 진입점)
 *
 * WO-O4O-GLYCOPHARM-EVENT-OFFERS-HUB-CANONICAL-ALIGNMENT-V1
 *
 * /store-hub/event-offers → Event Offer 목록 표시.
 * K-Cosmetics HubEventOffersPage와 동일 진입 구조.
 */
...
export function HubEventOffersPage() {
  ...
  const loadOffers = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await glycopharmEventOfferApi.listActive(1, 50);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Link
          to="/store-hub/cart"
          className="shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 text-sm font-semibold rounded-lg border border-teal-200 text-teal-700 bg-white hover:bg-teal-50 transition-colors"
        >
