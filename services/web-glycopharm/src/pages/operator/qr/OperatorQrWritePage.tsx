/**
 * OperatorQrWritePage — 운영자 약국 HUB QR 템플릿 작성/수정
 *
 * WO-O4O-GLYCOPHARM-OPERATOR-QR-WRITE-FRONTEND-V1 (2026-05-27)
 *
 * 운영자가 GlycoPharm 약국 HUB 에 게시할 QR 템플릿을 작성·수정.
 * URL 패턴:
 ...
 *   POST   /api/v1/glycopharm/operator/qr/templates
 *   PUT    /api/v1/glycopharm/operator/qr/templates/:id
 *   PATCH  /api/v1/glycopharm/operator/qr/templates/:id/publish
 *
 * 프론트는 authorRole / serviceKey 를 보내지 않는다.
 * Backend 가 authorRole='operator', serviceKey='glycopharm' 강제.
 */
...
export default function OperatorQrWritePage() {
  ...
  const handlePublish = async () => {
    ...
    if (!window.confirm('지금 발행하시겠습니까? 발행 즉시 GlycoPharm 약국 HUB 에 노출됩니다.')) return;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              ...
              {' · 약국 HUB 노출 대상 (GlycoPharm) · 운영자 단계에서는 실제 QR slug 미발급'}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4 bg-white rounded-xl border border-slate-100 p-6">
        <div>
          ...
          <input
            ...
            placeholder="QR 템플릿 제목 (약국 HUB 목록 표시)"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="이 QR 이 어떤 용도인지 짧게 설명하세요 (약국이 가져갈 때 참고)"
            rows={2}
        ...
        {targetType === 'url' && (
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              약국이 가져갈 때 이 URL 이 약국 사본 QR 의 landing target 으로 박힙니다.
            </p>
          </div>
        )}
        ...
        {targetType === 'content' && (
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-slate-400 mt-1">
                약국이 가져갈 때 backend 가 약국 사본 store_qr_codes 의 landing_target_id 로 변환합니다 (Phase 3-B).
              </p>
            </div>
          </div>
        )}
