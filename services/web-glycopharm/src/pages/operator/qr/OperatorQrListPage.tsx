/**
 * OperatorQrListPage — 운영자 약국 HUB QR 템플릿 목록
 *
 * WO-O4O-GLYCOPHARM-OPERATOR-QR-WRITE-FRONTEND-V1 (2026-05-27):
 *   KPA OperatorQrListPage port. API endpoint: /api/v1/glycopharm/operator/qr/templates
 *
 * QR 도메인 차이 (Blog/POP 와):
 *   - slug 컬럼 부재 (운영자 단계 미발급)
...
const qrActionPolicy = defineActionPolicy<OperatorQrTemplate>('glycopharm:operator-qr', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    {
...
export default function OperatorQrListPage() {
  ...
  const handlePublish = useCallback(async (q: OperatorQrTemplate) => {
    if (!window.confirm(`"${q.title}" QR 템플릿을 발행하시겠습니까? 발행 즉시 약국 HUB 에 노출됩니다.`)) return;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <header className="mb-6 pb-5 border-b-2 border-slate-200 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">약국 HUB QR-code</h1>
          <p className="mt-1.5 text-sm text-slate-500">
            운영자가 GlycoPharm 약국 HUB 에 게시할 QR "템플릿" 을 작성·관리합니다.
            실제 QR-code 는 약국 경영자가 가져갈 때 약국별로 발급됩니다.
          </p>
        </div>
        <button
