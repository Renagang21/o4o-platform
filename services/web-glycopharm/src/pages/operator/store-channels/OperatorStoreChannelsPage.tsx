/**
 * OperatorStoreChannelsPage — Cross-store 채널 관리 (GlycoPharm 운영자)
 *
 * WO-O4O-GLYCOPHARM-OPERATOR-STORE-CHANNELS-V1
 *
 * Backend: GET /api/v1/operator/stores/channels (injectServiceScope → glycopharm 자동 격리)
 *          PUT /api/v1/operator/stores/:storeId/channels/:channelId/status
 * Guard: stores.routes.ts에서 glycopharm:operator 허용, serviceKey 자동 필터링
 */
...
const channelActionPolicy = defineActionPolicy<ChannelData>('glycopharm:store-channels', {
  rules: [
    {
      key: 'approve',
      label: '승인',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">GlycoPharm 약국의 채널 상태를 관리합니다</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 flex items-center gap-3 flex-wrap">
          ...
          <button onClick={handleSearch} className="px-4 py-2 bg-emerald-700 text-white rounded-lg hover:bg-emerald-800 text-sm font-medium">
            검색
          </button>
        </div>
              ...
              {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${
                    currentPage === page ? 'bg-emerald-700 text-white' : 'hover:bg-slate-100 text-slate-600'
                  }`}
                >
                  {page}
                </button>
