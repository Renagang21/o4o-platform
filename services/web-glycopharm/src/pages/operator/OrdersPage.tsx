/**
 * Operator Orders Page (Order Management) — view-only
 *
 * WO-O4O-OPERATOR-ORDER-VIEW-FRONTEND-WIRE-V1:
 *   checkout_orders 기반 view-only operator orders API(/glycopharm/operator/orders)에 연결.
 *   조회 전용 — 주문 상태변경/배송/취소/환불/송장/정산/bulk action/selectable 없음.
 *   상태 처리 기능은 후속 store/seller scope WO 에서 제공 예정.
 *   선행: WO-O4O-OPERATOR-ORDER-VIEW-API-V1 (backend), WO-O4O-OPERATOR-ORDER-MOCK-SURFACE-GUARD-V1.
 */
...
import { glycopharmApi, type OperatorOrder, type OperatorOrderStats } from '@/api/glycopharm';
...
export default function OrdersPage() {
  ...
  const columns: ListColumnDef<OperatorOrder>[] = [
    ...
    },
    {
      ...
      header: '상태',
      width: '110px',
      render: (_v, o) => <StatusBadge status={o.status} label={ORDER_STATUS_LABEL[o.status]} />,
    },
    {
      key: 'createdAt',
      header: '주문일시',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-blue-800">주문 조회 전용</p>
