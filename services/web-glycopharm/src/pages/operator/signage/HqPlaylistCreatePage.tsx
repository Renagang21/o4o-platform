 ...
 *     canonical /api/signage/glycopharm/hq/* 다단계(URL별 HQ media → 플레이리스트 → 항목 일괄).
 */
...
export default function HqPlaylistCreatePage() {
  ...
  const handleSubmit = useCallback(async (
    ...
    for (let i = 0; i < values.items.length; i++) {
      ...
      const mediaResult = await apiFetch(`${BASE}/hq/media`, {
    ...
    const playlistResult = await apiFetch(`${BASE}/hq/playlists`, {
      name: values.name,
    ...
    // Step 3: 항목 일괄 추가
    setProgress('항목 추가 중...');
    await apiFetch(`${BASE}/playlists/${playlist.id}/items/bulk`, {
  ...
  return (
    <div className="space-y-6">
      <div>
        <button onClick={() => navigate(LIST_PATH)} className="text-sm text-primary-600 hover:underline mb-2">← 목록으로</button>
        <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
          <ListMusic className="w-6 h-6" /> 플레이리스트 등록
        </h1>
