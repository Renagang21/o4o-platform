/**
 * OperatorBlogWritePage — 운영자 약국 HUB 블로그 작성/수정
 *
 * WO-O4O-GLYCOPHARM-OPERATOR-STORE-HUB-WRITE-CAPABILITY-V1 (2026-05-27)
 *
 * 운영자가 GlycoPharm 약국 HUB 에 게시할 블로그를 RichTextEditor 로 작성·수정.
 * URL 패턴:
 ...
 * Backend 가 author_role='operator', service_key='glycopharm', store_id=null 강제.
 */
...
export default function OperatorBlogWritePage() {
  ...
  const handleSave = async (): Promise<OperatorBlogPost | null> => {
    ...
    try {
      ...
      const saved = isNew
        ? await createOperatorBlogPost(body)
        : await updateOperatorBlogPost(id!, body);
      setPost(saved);
      toast.success(isNew ? '블로그가 생성되었습니다 (초안)' : '저장되었습니다');
      if (isNew) {
        navigate(`/operator/blog/${saved.id}/edit`, { replace: true });
      }
      return saved;
    } catch (e: any) {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
        ...
        <button
          onClick={() => navigate('/operator/blog')}
      ...
      <div className="space-y-4 bg-white rounded-xl border border-slate-100 p-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="예: spring-pharmacy-tips"
