/**
 * OperatorPopListPage — 운영자 약국 HUB POP 목록
 *
 * WO-O4O-GLYCOPHARM-OPERATOR-STORE-HUB-WRITE-CAPABILITY-V1 (2026-05-27):
 *   KPA OperatorPopListPage port. API endpoint: /api/v1/glycopharm/operator/pop/posts
...
const popActionPolicy = defineActionPolicy<OperatorPopPost>('glycopharm:operator-pop', {
  inlineMax: 2,
  rules: [
    { key: 'edit', label: '수정' },
    {
...
export default function OperatorPopListPage() {
  ...
  const handlePublish = useCallback(async (post: OperatorPopPost) => {
    if (!window.confirm(`"${post.title}" POP 을 발행하시겠습니까? 발행 즉시 약국 HUB 에 노출됩니다.`)) return;
  ...
  const handleBulkArchive = useCallback(
    () => runBulk(selectedNotArchivedIds, 'archive', {
      confirm: `선택한 ${selectedNotArchivedIds.length}개 POP 을 보관하시겠습니까?`,
    }),
      ...
      <header className="mb-6 pb-5 border-b-2 border-slate-200 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            운영자가 GlycoPharm 약국 HUB 에 게시할 POP 콘텐츠를 작성·관리합니다.
