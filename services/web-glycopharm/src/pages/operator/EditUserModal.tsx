...
const GLYCOPHARM_MEMBERSHIP_OPTIONS = [
...
const GLYCOPHARM_ADMIN_ROLE_OPTIONS = [
  { value: '', label: '일반 회원' },
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const gpMembership = (data.memberships || []).find((m: any) => m.serviceKey === 'glycopharm');
        setMembershipRole(gpMembership?.role || '');
        setOriginalMembershipRole(gpMembership?.role || '');
        // 운영 권한 (role_assignments — 편집 가능)
        const activeRoles: string[] = (data.roles || [])
          .filter((r: any) => r.isActive)
          .map((r: any) => r.role);
        const adminRole = activeRoles.find((r: string) => r === 'glycopharm:admin' || r === 'glycopharm:operator') || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (membershipRole !== originalMembershipRole) {
        ...
        payload.membershipServiceKey = 'glycopharm';
      }
      if (hasBusinessInfo) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-3 border-t">
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
                {GLYCOPHARM_MEMBERSHIP_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
              {membershipRole !== originalMembershipRole && (
                <p className="text-xs text-amber-600 mt-1">
                  회원 유형이 변경됩니다: {GLYCOPHARM_MEMBERSHIP_OPTIONS.find(o => o.value === originalMembershipRole)?.label || originalMembershipRole || '미지정'} → {GLYCOPHARM_MEMBERSHIP_OPTIONS.find(o => o.value === membershipRole)?.label || membershipRole}
                </p>
              )}
            </div>
            ...
            <div className="pt-3 border-t">
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
                {GLYCOPHARM_ADMIN_ROLE_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
              {selectedRole !== currentRole && (
                <p className="text-xs text-amber-600 mt-1">
                  권한이 변경됩니다: {currentRole ? GLYCOPHARM_ADMIN_ROLE_OPTIONS.find(o => o.value === currentRole)?.label || currentRole : '일반 회원'} → {selectedRole ? GLYCOPHARM_ADMIN_ROLE_OPTIONS.find(o => o.value === selectedRole)?.label || selectedRole : '일반 회원'}
                </p>
              )}
            </div>
