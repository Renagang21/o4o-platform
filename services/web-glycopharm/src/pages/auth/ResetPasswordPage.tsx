...
export default function ResetPasswordPage() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center py-12 px-4">
        <div className="max-w-md w-full text-center space-y-4">
          ...
          <Link
            to="/forgot-password"
            className="inline-block px-6 py-2.5 text-sm font-medium bg-primary-600 text-white rounded-xl hover:bg-primary-700"
          >
            비밀번호 찾기로 이동
          </Link>
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post<{ success: boolean }>('/auth/reset-password', { token, password });
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2.5 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                placeholder="새 비밀번호"
                required
              />
              <button
