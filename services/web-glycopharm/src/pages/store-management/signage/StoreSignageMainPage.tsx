 ...
 *
 * WO-O4O-GLYCOPHARM-SIGNAGE-STORE-UX-V1   — KPA canonical 이식
 * WO-O4O-GLYCOPHARM-SIGNAGE-SCHEDULE-V1   — Schedule 탭 활성화
 * WO-O4O-SIGNAGE-PHASE2-HUB-STORE-BOUNDARY-V1 — Explore 탭 제거 + DataTable 적용
 *
 * /store/signage: 3탭 구조
 *  [Tab 1] 내 동영상      (assets)    — storeAssetControlApi 기반
...
// WO-O4O-GLYCOPHARM-STORE-SIGNAGE-MEDIA-REGISTRATION-FRONTEND-PARITY-V1: 직접 동영상 등록(SignageMedia)
import {
  fetchSignageMedia,
  createSignageMedia,
  deleteSignageMedia,
...
function computeSignageKpi(items: StoreAssetItem[]) {
  let published = 0, draft = 0, hidden = 0, forcedActive = 0, signageChannel = 0;
...
export default function StoreSignageMainPage() {
  ...
  // ── 동영상 등록 state (WO-O4O-GLYCOPHARM-STORE-SIGNAGE-MEDIA-REGISTRATION-FRONTEND-PARITY-V1) ──
  const [registeredMedia, setRegisteredMedia] = useState<SignageMediaItem[]>([]);
  ...
  const openEditSchedule = (sch: SignageScheduleItem) => {
    ...
    setSchStartTime(sch.startTime.slice(0, 5));
  ...
  // ── 동영상 등록 handlers (WO-O4O-GLYCOPHARM-STORE-SIGNAGE-MEDIA-REGISTRATION-FRONTEND-PARITY-V1) ──
  const resetVideoRegForm = () => {
    setRegVideoTitle('');
    setRegVideoUrl('');
  ...
  const handleRegisterVideo = async () => {
    ...
    if (!organizationId) { setRegVideoError('매장 정보를 불러오는 중입니다. 잠시 후 다시 시도해 주세요.'); return; }
    const sourceType = detectVideoSource(regVideoUrl.trim());
    if (!sourceType) { setRegVideoError('올바른 YouTube 또는 Vimeo URL을 입력해 주세요.'); return; }
  ...
  const handleAddSignageToPlaylist = async (mediaId: string) => {
    if (!selectedPlaylistId || !organizationId) return;
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="flex gap-1 mb-1 border-b border-slate-200">
        {([
          { key: 'assets', label: '내 동영상', Icon: Monitor },
          { key: 'playlist', label: '내 플레이리스트', Icon: ListVideo },
          { key: 'schedules', label: '스케줄', Icon: Calendar },
      ...
      <div className={`flex items-start gap-4 mb-6 px-4 py-3 rounded-xl border ${
        currentSchedule ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'
      }`}>
            ...
            <div className="border border-slate-200 rounded-lg bg-white">
              ...
              {showAddPicker && (
                <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                  ...
                  {signageSnapshots.length === 0 && registeredMedia.length === 0 ? (
                    <p className="text-xs text-slate-400">사이니지 자산이 없습니다. '동영상 등록'으로 직접 추가하거나 Hub/Library에서 선택하세요.</p>
                  ) : (
                    <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto">
                      {/* WO-O4O-GLYCOPHARM-STORE-SIGNAGE-MEDIA-REGISTRATION-FRONTEND-PARITY-V1: 직접 등록 동영상 */}
                      {registeredMedia.map(m => (
                        <button
                          key={`media-${m.id}`}
                          onClick={() => handleAddSignageToPlaylist(m.id)}
                          className="flex items-center gap-1 px-3 py-1.5 text-xs bg-white border border-blue-200 rounded-md hover:bg-blue-50 hover:border-blue-300 truncate max-w-[200px]"
                          title="등록한 동영상"
                        >
                  ...
                  {playlistItems.map((item, idx) => (
                    <div key={item.id} className={`flex items-center gap-3 px-4 py-2.5 ${item.isForced ? 'bg-red-50/30' : ''}`}>
                      ...
                      <div className="flex flex-col gap-0.5">
                        <button onClick={() => handleMoveItem(idx, -1)} disabled={idx === 0} className="text-slate-300 hover:text-slate-600 disabled:opacity-30">
                          <GripVertical className="w-3 h-3 rotate-180" />
                        </button>
                        <button onClick={() => handleMoveItem(idx, 1)} disabled={idx === playlistItems.length - 1} className="text-slate-300 hover:text-slate-600 disabled:opacity-30">
                      ...
                      {item.isLocked ? (
                        ...
                        <button onClick={() => handleDeleteItem(item.id)} className="p-1 text-slate-300 hover:text-red-500 rounded" title="삭제">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      )}
                    </div>
                ...
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      ...
                      {playlists.filter(pl => !signagePlaylists.find(sp => sp.id === pl.id)).map(pl => (
                        <option key={pl.id} value={pl.id}>{pl.name}</option>
                      ))}
                    </select>
                ...
                <DataTable<SignageScheduleItem>
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (_v, sch) => (
                        <span className="text-slate-600 text-xs">
                          {sch.playlist?.name
                            || signagePlaylists.find(p => p.id === sch.playlistId)?.name
                            || playlists.find(p => p.id === sch.playlistId)?.name
                            || sch.playlistId.slice(0, 8)}
                        </span>
                      ),
                    },
                    {
                    ...
                    },
                    {
                      ...
                      render: (_v, sch) => (
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={e => { e.stopPropagation(); openEditSchedule(sch); }} className="p-1 text-slate-400 hover:text-blue-600 rounded" title="수정">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={e => { e.stopPropagation(); handleDeleteSchedule(sch); }} className="p-1 text-slate-400 hover:text-red-500 rounded" title="삭제">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      ),
      ...
      {/* WO-O4O-GLYCOPHARM-STORE-SIGNAGE-MEDIA-REGISTRATION-FRONTEND-PARITY-V1:
          KPA 정렬 — '동영상 등록'(직접 URL 등록 모달)을 탭 헤더 우측 1순위로. 허브 가져오기는 '콘텐츠 허브'로 보존. */}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-slate-800">내 동영상</h2>
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate('/store/marketing/signage/library')}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-500 border border-slate-200 rounded-lg hover:bg-slate-50"
          >
      ...
      {showVideoRegForm && (
        <div
          ...
          onClick={e => { if (e.target === e.currentTarget) resetVideoRegForm(); }}
          onKeyDown={e => { if (e.key === 'Escape') resetVideoRegForm(); }}
          role="dialog"
          aria-modal="true"
          tabIndex={-1}
        >
      ...
      {registeredMedia.length > 0 && (
        <div className="mb-6 rounded-lg border border-slate-200 bg-white">
          <div className="px-4 py-2.5 border-b border-slate-100 text-sm font-medium text-slate-700">
            등록한 동영상 <span className="text-slate-400 font-normal">({registeredMedia.length})</span>
          </div>
            ...
            {registeredMedia.map(m => (
              <li key={m.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-400 truncate">{m.sourceType} · {m.sourceUrl}</div>
                </div>
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${m.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                  {m.status === 'active' ? '공개' : '초안'}
                </span>
                <button onClick={() => handleDeleteMedia(m.id)} className="p-1 text-slate-300 hover:text-red-500 rounded" title="삭제">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </li>
            ))}
            ...
            {([
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                  statusFilter === opt.key ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-100'
                }`}
              >
                {opt.label}
              </button>
        ...
        <>
          <DataTable<StoreAssetItem>
            columns={[
              ...
              },
              {
                ...
                title: '채널 배치',
                render: (_v, item) => {
                  const isChannelDisabled = item.isForced || item.isLocked || channelUpdatingId === item.id;
                  return (
                    <div className="flex gap-1.5">
                      {CHANNEL_DEFS.map(ch => {
                        ...
                        return (
                          <button
                            ...
                            } ${isChannelDisabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:opacity-80'}`}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4 text-sm text-slate-500">
              ...
              <div className="flex gap-2">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} className="flex items-center gap-1 px-3 py-1.5 border border-slate-300 rounded-md hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">
                  <ChevronLeft className="w-4 h-4" /> 이전
                </button>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="flex items-center gap-1 px-3 py-1.5 border border-slate-300 rounded-md hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">
