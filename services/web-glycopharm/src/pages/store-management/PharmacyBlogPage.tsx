 ...
 * Adapted from KPA PharmacyBlogPage (canonical reference).
 *
 * 경로: /store/content/blog
 * 인증 필수 (상위 ProtectedRoute + StoreLayoutWrapper).
 *
 * Glycopharm-specific override:
 *  - store-resolver: pharmacyApi.getPharmacyStatus().storeSlug (KPA 의 getStoreSlug 대체)
 *  - getAccessToken: '@o4o/auth-client' (KPA 는 contexts/AuthContext)
 *  - service param: 'glycopharm' default
 *  - 이미지 업로드 핸들러는 본 WO 범위 외 (RichTextEditor URL 직접 입력 가능)
 *
 * 흐름:
 *  - list ViewMode: 게시글 목록 + 발행/보관/삭제/URL복사/미리보기
 *  - editor ViewMode: RichTextEditor 본문 + AI 보조 배너 + 임시저장
...
export default function PharmacyBlogPage() {
  ...
  // Resolve slug from Glycopharm pharmacy status
  useEffect(() => {
    (async () => {
      try {
        const res = await pharmacyApi.getPharmacyStatus();
        const resolved = res?.data?.storeSlug ?? null;
        if (resolved) {
          setSlug(resolved);
        } else {
          setError('연결된 매장이 없습니다. 약국 신청을 먼저 진행하세요.');
          setLoading(false);
        }
      } catch (e: any) {
        setError(e?.message || '약국 정보를 불러올 수 없습니다.');
        setLoading(false);
      }
    })();
  }, []);
              ...
              >
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0, marginLeft: '12px' }}>
                  {post.status === 'published' && (
                    <>
                      <button onClick={() => handleCopyUrl(post.slug)} style={{ ...smallBtn, color: '#0f172a' }} title="공개 URL 복사">URL 복사</button>
                      <button onClick={() => handlePreview(post.slug)} style={{ ...smallBtn, color: '#0f172a' }} title="새 탭에서 열기">미리보기</button>
                    </>
                  )}
