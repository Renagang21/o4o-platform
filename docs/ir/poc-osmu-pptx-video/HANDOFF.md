...
**완료:** EN 번역 + 애니메이션(v4 수용) + R11 단일프레임 병합(균일간격) 적용 → `자일리톨그린_동영상_EN_16x9.pptx`.

**검증 대기 (사용자가 확인하기로 함):**
- EN 파일을 열어 **제목 클릭 시 부제까지 한 박스로 잡히면 = 병합 새버전**(클릭 테스트).
- slide 7·10·11 제목–부제 **간격이 균일한지**.
- ⚠️ 이전 PC 에서 "간격 안 바뀜" 이슈는 **OneDrive 캐시로 옛 파일을 보던 것**으로 추정(디스크 파일은 병합본 확인됨). 새 PC 에선 동기화 후 새로 열면 해결될 것.

**다음 단계 후보 (사용자와 택1):**
1. 슬라이드별 폰트 축소율 차이로 spcBef 시각 간격이 미세하게 달라지는지 점검 → 필요시 간격을 폰트배율과 무관하게 고정.
2. **세로 9:16** 비율 제작 (배경 비율별 별도 등록 + 전경 리플로우 실증).
3. 다른 언어(중국어=짧아짐 / 일본어) 대조.
