...
- **비율 매트릭스 완성 (✅ 2026-06-23): 7언어 × 4비율 = 28개.** `portrait.py`(OSMU_RATIO 파라미터) — (1) `sldSz` 목표비율, (2) 배경 `<p:bg>` 중앙 크롭(§10.3, 별도 이미지 없음), (3) 전경 2-모드: **STACK**(세로/정사각 9:16·1:1 = 상단 텍스트+하단 이미지 세로 스택) / **FIT**(가로 16:10 = 16:9 레이아웃 균일 축소·중앙, 폰트도 축소). 파이프라인 `trans→merge→[portrait]→inject_dyn→repack`(16:9 는 portrait 생략).
  - **언어 7**: ko(원본, trans 생략)·en·zh·ja·vi·th·id. **비율 4**: 16x9(PC/가로사이니지)·9x16(폰/세로)·16x10(태블릿 가로)·1x1(SNS 파트너). 파일 `자일리톨그린_동영상_{LANG}_{16x9,9x16,16x10,1x1}.pptx` (28개, python-pptx 로드 OK).
  - 다른 비율 추가: portrait.py `RATIO_SIZE` 에 항목 추가(3x4·4x5 이미 정의). aspect≥1.2=FIT, 미만=STACK 자동.
