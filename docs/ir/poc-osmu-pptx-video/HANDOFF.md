...
- **간격 표준화 + CJK orphan 수정 (✅ 2026-06-23, 사용자 검수 반영, EN/ZH/JA 재빌드):** "제목–부제 간격이 슬라이드마다 다름"의 진짜 원인 = PowerPoint 가 spcBef 위에 **폰트크기 비례 leading** 을 더함 → 상수 spcBef 로는 절대 균일 안 됨. **해법:** `spcBef = 22 − 0.2·(prev_fs+next_fs)` 폰트 역보정(시각간격 상수화: 44→24=8 / 36→44=6 / 24↔28=11pt). + CJK 1글자 orphan 제목 폰트 미세축소(slide1 36→33.8 / slide2 44→39.6). `merge.py` `comp_spcbef`/`deorphan`.

**검증 대기 (사용자가 확인하기로 함 — ⚠️ 추정 기반이라 시각 확인 필수):**
- EN/ZH/JA 열어 **제목–부제 시각 간격이 전 슬라이드 균일**한지(특히 slide1·8·12 폰트 다른 조합).
- ZH slide1 "...制成" / slide2 "...分泌" 가 **한 줄에** 들어오는지(orphan 사라짐).
- 간격이 너무 넓/좁으면 `merge.py` `GAP_VISUAL`(현재 22) 조정 / orphan 과축소면 floor(0.82) 조정.
- ⚠️ OneDrive 캐시 주의 — 동기화 후 새로 열 것.

**다음 단계 후보 (사용자와 택1):**
1. **세로 9:16** 비율 제작 (배경 비율별 별도 등록 + 전경 리플로우 실증).
2. 부제 **문단별 등장** 애니(현재는 블록 단위) — paragraph-level build.
3. (검수 후 필요시) 간격/orphan param 미세 튜닝.
