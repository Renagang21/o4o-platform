...
| 이메일 | 역할 | 비밀번호 | 용도 |
|--------|------|----------|------|
| `pharmacist@test.test` | user | TestPassword | 약사 테스트 |
| `admin@test.test` | admin | TestPassword | 관리자 테스트 |

### 3.2 K-Cosmetics

| 이메일 | 역할 | 비밀번호 | 용도 |
|--------|------|----------|------|
| `consumer@k-cosmetics.test` | user | TestPassword | 소비자 테스트 |
| `seller@k-cosmetics.test` | seller | TestPassword | 판매자 테스트 |
| `supplier@k-cosmetics.test` | supplier | TestPassword | 공급자 테스트 |
| `admin@k-cosmetics.test` | admin | TestPassword | 관리자 테스트 |
...
WHERE email LIKE '%@test.test'
   OR email LIKE '%@k-cosmetics.test';
```
