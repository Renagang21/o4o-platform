...
>
> 3 service (KPA / Neture / K-Cos) 의 Operator Members list-side 구조 비교 → **공통 wrapper 가능 여부 + 설계안 + 후속 WO 순서** 확정.
...
> **Option C — KPA 별도 유지 + Neture/K-Cos 만 단일 wrapper 통합.**
...
> Neture/K-Cos 의 List 구조는 공통도 ≥ 75% → `OperatorMembersConsolePage` wrapper 1 개 + 2 service thin wrapper. K-Cos bulk action 부재는 wrapper 도입과 함께 자동 해소.
...
→ **Live wrappers:** KPA 1 + Neture/K-Cos 각 1 = 총 4 페이지 (`UsersPage` 명명 통일 안 됨).
...
### 3.2 Neture/K-Cos `UserData` (공통 User entity)
...
| role / membership_type (badge) | KPA: membership_type 인라인 | Neture: RoleBadge: RoleBadge | K-Cos: RoleBadge | 4/4 (라벨/소스 다름) |
...
**Neture+K-Cos 만 비교 시:** name / email / status / createdAt / actions / role / services = **7/9 (78%)**

### 4.2 Filter (Tabs)

| Tab | KPA | Neture | K-Cos |
|---|---|---|---|---|
| 전체 (all) | ✅ | ✅ | ✅ | ✅ |
...
→ **공통 패턴:** Neture/K-Cos 2 service 가 4 카드 / 동일 라벨·아이콘·컬러로 100% 일치. **KPA 만 3 카드 (커스텀 라벨)**.
...
### Option C — KPA 별도 유지 + Neture/K-Cos 만 단일 wrapper ✅ **권장**
...
operator-core-ui:
  packages/operator-core-ui/src/modules/members/
    OperatorMembersConsolePage.tsx // 공통 wrapper (Neture/K-Cos 용)
...
**장점:**
- Neture/K-Cos 의 공통 7/9 columns + 4 stats card + ActionBar/batch + Drawer (직전 WO 정합) 모두 한 곳에서 처리
...
packages/operator-core-ui/
  src/modules/members/
    OperatorMembersConsolePage.tsx # 공통 wrapper (Neture/K-Cos)
    components/
...
interface OperatorMembersConsoleProps {
  ...
  // delete 모드 (선택)
  deleteMode?: 'simple' | 'risk' | 'soft-and-hard';     // K-Cos: simple: risk, Neture: soft-and-hard
...
| Neture UsersManagementPage.tsx | ~917 → ~80 lines (thin wrapper + editModalRenderer slot + drawerExtraSections + custom delete logic) |
 UsersPage.tsx | ~957 → ~80 lines (thin wrapper + risk delete modal slot) |
...
| `apiFetch<T>` wrapper | path replace `^/api/v1` |
...
- 범위: Option C — Neture/K-Cos 2 service 만 통합
...
| 같은 Capability → 같은 UI/UX | ✅ Neture/K-Cos (User entity 기반) 완전 정렬 | 없음 |
...
|---|---|
| 즉시 코드 변경 | 0 |
| 큰 결정 | **Option C — KPA 별도 유지 + Neture/K-Cos 통합** 확정 |
...
*Status: ✅ 결정 — Option C (KPA 별도 유지 + Neture/K-Cos 단일 wrapper). Priority 1-4 후속 WO 4건 제안.*
