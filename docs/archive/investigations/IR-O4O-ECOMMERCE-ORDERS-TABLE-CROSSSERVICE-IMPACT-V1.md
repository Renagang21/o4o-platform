...
> - `cef1dadcc` store 라벨 KPA canonical 정렬
> **작성일**: 2026-05-31
> **범위**: `ecommerce_orders` 테이블 부재의 cross-service 영향 분석. KPA-Society / K-Cosmetics + Platform Admin + Neture (informational).
...
백엔드 코드 베이스에 **두 개의 주문 테이블 모델이 혼재**하는 상태:

- **Legacy 가정 (`ecommerce_orders`)**: 백엔드 7개 파일이 raw SQL 로 직접 참조 — K-Cosmetics + Platform Admin 광역.
...
- **즉시 가능한 WO 영역**: 프론트엔드 UI safe-fallback + 백엔드 try/catch 방어.
...
| API 방어 부족인가? | K-Cosmetics `/operator/dashboard` 는 일부 `.catch()` 가 safe default 반환 (silent 0). |
...
1. 사용자 자신의 매장이 실제로 운영 중이라면, 이 화면이 거짓 정보를 정상 상태로 위장. **운영 의사결정 위험**.
...
4. 그러나 transaction rollback 이 정상 작동하므로 사용자에게 "주문 실패" 정도로 표면화.
5. 결제 흐름이 살아있다면 차단.
...
2. PR-2: `CosmeticsStoreSummaryService` 의 raw SQL 클러스터 → `@o4o/ecommerce-core` 의 entity / repository 패턴으로 통합. KPA 의 패턴 mirror.
...
K-Cosmetics 가 silent 거짓 0 노출. 둘 다 "준비중" 표기 / 비활성 처리가 더 적절.
