...
- KPA만 독립 1400줄 구현.
- **남은 GAP은 UI 라벨/탭/bulk action 비대칭.** Neture bulk 전무, K-Cos 상태 탭 2개만, 컬럼 "유형"/"역할" 라벨 불일치 등.
- **완전삭제 구현은 4개 서비스 모두 완료.** 단, Delete Flow UI는 4개 서비스 모두 **개별 커스텀** — 공통화 가장 큰 ROI.

**핵심 권고**: KPA는 도메인 복잡성 때문에 별도 유지가 정당하므로, **2서비스(Neture/K-Cos) 정합 + Delete Flow 공통화** 우선 진행. KPA는 `OperatorMembersConsolePage` 옵션 prop을 통한 점진 정합을 선택지로 검토.
...
| ------ | ------------ | ------------ | -------- |
| 구현 방식 | 독립 1400줄 | OperatorMembersConsolePage thin wrapper | OperatorMembersConsolePage thin wrapper |
| 코드 라인 수 | ~1421 | ~280 | ~243 |
| role 탭 | 약사 / 약대생 (2개) | 판매자 / 소비자 (2개) | 공급자 / 파트너 / 셀러 (3개) |
| 상태 탭 | **5개** (pending/active/suspended/rejected/withdrawn) | 2개 (suspended/withdrawn) | 4개 (active/suspended/rejected/withdrawn) |
| 가입 신청 탭 | ✅ 별도 | ❌ | ❌ (별도 RegistrationRequestsPage) |
| 컬럼 헤더 | "유형" (membership_type) | "역할" (RoleBadge) | "역할" (RoleBadge) |
| Drawer 진입 | ✅ BaseDetailDrawer | ✅ BaseDetailDrawer | ✅ BaseDetailDrawer |
| EditModal | KpaEditUserModal (도메인 특화) | EditUserModal (generic) | EditUserModal (generic) |
...
| ------ | ----- | ------------ | -------- |
| 분리 페이지? | ✅ 분리 (`AdminMemberManagementPage`) | ✅ 분리 (`KCosmeticsAdminMembersPage`) | ✅ 분리 (`AdminMemberManagementPage`) |
| 주 용도 | 완전삭제 + 위험 작업 | 완전삭제 + soft/hard 선택 DeleteFlow | 완전삭제 |
| 권한 | `kpa:admin` | `cosmetics:admin` | `neture:admin` |
| Delete 확인 UX | `MemberDeleteRiskModal` (2단계) | `KCosAdminDeleteFlow` (동일) | `AdminMemberDeleteModal` (risk 평가) |
| API endpoint | `DELETE /api/v1/operator/members/:userId?mode=hard` | 동일 | 동일 |
| audit log | ActionLogService | ActionLogService | ActionLogService |
...
| 항목 | KPA | K-Cosmetics | Neture |
| ------ | ----- | ------------ | -------- |
| DataTable 소스 | `@o4o/operator-ux-core` | `@o4o/operator-ux-core` (via wrapper) | `@o4o/operator-ux-core` (via wrapper) |
| 검색 입력 | ✅ debounce | ✅ debounce | ✅ debounce |
| 상태 탭 | 5개 ✅ | 2개 ⚠ | 4개 ⚠ |
| role 탭 | 2개 | 2개 | 3개 |
| 상태 Badge | `StatusBadge` | `StatusBadge` | `StatusBadge` |
| 역할 Badge | `RoleBadge` 또는 "유형" 텍스트 | `RoleBadge` | `RoleBadge` |
| 체크박스 | ✅ | ✅ (wrapper 내장) | ✅ (wrapper 내장) |
| row action | RowActionMenu | RowActionMenu | RowActionMenu |
| 빈 목록 / 로딩 / 에러 | 기본 | 기본 (wrapper) | 기본 (wrapper) |
| 페이징 | ✅ | ✅ | ✅ |
| 고아 컬럼 | (없음) | (없음) | **"대시보드 접근"** chip column (Neture only) |
...
- K-Cos는 정지/복원/탈퇴 bulk 있으나 승인/거절은 없음. RegistrationRequestsPage 별도 흐름이 그 자리를 일부 대체하나 통합 회원 관리 화면에서 bulk 승인이 빠진 상태
...
| 항목 | KPA | K-Cosmetics | Neture |
| ------ | ----- | ------------ | -------- |
| 모달 컴포넌트 | `KpaEditUserModal` | `EditUserModal` (서비스 로컬) | `EditUserModal` (서비스 로컬) |
| 위치 | packages/operator-core-ui | services/web-k-cosmetics | services/web-neture |
| 방식 | Modal | Modal (slot-based) | Modal (slot-based) |
| 도메인 필드 | 약사/약대생/약국명/면허번호/활동유형/capabilities/사업자정보 | 기본 정보 + 연락처 | 기본 정보 + 대시보드 접근 (read-only) |
| 수정 가능 필드 | 이름/약국정보/면허/사업정보 (이메일 불가) | 이름/연락처 (이메일 불가) | 이름/연락처 |
| 저장 API | `PATCH /api/v1/kpa/members/{id}` | 동일 | 동일 |
| operator vs admin 권한 | 동일 모달 + 권한별 버튼 노출 | 동일 모달 | 동일 모달 |

**핵심 관찰**:
- ⚠ **EditUserModal이 서비스별로 3종 각자 구현됨** (K-Cos/Neture) — 동일 generic 패턴인데 공통화 안 됨. `CommonEditUserModal` (이미 packages/operator-core-ui에 존재)으로 통합 가능
...
| 항목 | KPA | K-Cosmetics | Neture |
| ------ | ----- | ------------ | -------- |
| 사용 상태값 | active/suspended/rejected/withdrawn/pending | active/approved/suspended/pending/rejected/withdrawn | active/suspended/rejected/pending/withdrawn |
| 승인 (pending → active) | Drawer footer + bulk | ⚠ 불가 (동일) | ⚠ 불가 (`/neture/operator/registrations/{id}/approve` 별도 endpoint) |
| 거절 (pending → rejected) | Drawer footer + bulk | ⚠ 불가 | ⚠ 불가 |
| 정지 (active → suspended) | row + bulk | row + bulk | ❌ 미구현 |
| 복원 (suspended → active) | row + bulk | row + bulk | ❌ |
| 탈퇴 (→ withdrawn) | bulk + row | bulk + row | ❌ |
| 복구 (withdrawn → active) | ✅ (admin 재가입) | ✅ admin | ✅ admin (AdminMemberManagementPage) |
| 사유 입력 요구? | rejection 시만 | rejection 시만 | rejection 시만 |
| 확인 Modal | ✅ ConfirmActionDialog | ✅ ConfirmActionDialog | ⚠ 일부 |
| service_memberships 갱신? | ✅ | ✅ | ✅ |
| 서비스 profile 동시 갱신? | ✅ kpa_members | ✅ cosmetics profile | ✅ neture_suppliers (supplier만) |
| 알림/이메일 발송 | ⚠ 일부 | ⚠ 일부 | ⚠ 일부 |

**상태값 SSOT**: 모든 서비스에서 `service_memberships.status`가 SSOT (에서 정합 완료).

**핵심 관찰**:
- 상태 enum은 5개 모두 공유: `pending | active | suspended | rejected | withdrawn` ✅
- 다만 **`approved` 상태가 K-Cos에 잔존** (active의 alias로 추정)
...
| 항목 | KPA | K-Cosmetics | Neture |
| ------ | ----- | ------------ | -------- |
| 표현 라벨 | "탈퇴 처리" | 동일 혼용 | "비활성화" |
| 실제 동작 | soft delete (`status='withdrawn'` + isActive=false) | 동일 | 동일 |
| API | `DELETE /api/v1/operator/members/:userId` (default mode=soft) | 동일 | 동일 |
| operator 가능? | ✅ | ✅ | ⚠ Drawer 내 단건만 |
| bulk 가능? | ✅ | ✅ | ❌ |
| 복구 가능? | ✅ admin | ✅ admin | ✅ admin |
...
| ------ | ----- | ------------ | -------- |
| 구현 여부 | ✅ | ✅ | ✅ |
| Page | AdminMemberManagementPage | KCosmeticsAdminMembersPage | AdminMemberManagementPage |
| 메뉴 라벨 | "완전삭제" | "완전삭제" | "회원 완전삭제" |
| 라우트 | admin/members | admin/members | admin/members |
| 권한 | kpa:admin | cosmetics:admin | neture:admin |
| 단건 | ✅ | ✅ | ✅ |
| bulk | ❌ (정책상 금지) | ❌ | ❌ |
| 확인 UX | MemberDeleteRiskModal 2단계 | KCosAdminDeleteFlow (soft/hard 선택) | AdminMemberDeleteModal |
| API | `DELETE /api/v1/operator/members/:userId?mode=hard` | 동일 | 동일 |
| cascade 정책 | service_memberships + role_assignments 삭제, users status='deleted' + isActive=false, kpa_members status='withdrawn' 동기화, organization_members 정리 | 동일 base | 동일 base + neture_suppliers 정리 |
| audit log | ActionLogService (`${serviceKey}.operator.member_delete`) | 동일 | 동일 |
...
| **EditUserModal generic** | CommonEditUserModal 존재, 2서비스가 자기 EditUserModal 별도 구현 | **K-Cos/Neture의 EditUserModal을 CommonEditUserModal로 정합** |
...
2. EditUserModal 정합 (K-Cos/Neture)
...
| 회원 유형 옵션 (membership role) | 도메인 차이 — KPA(약사/약대생) / Neture(공급자/파트너/셀러) / K-Cos(판매자/소비자) |
...
| **MemberEditForm (generic)** | ⚠ | `CommonEditUserModal` (이미 존재) | ⚠ 미사용 | K-Cos/Neture가 자기 EditUserModal 유지 — 정합 필요 |
...
| Service scope boundary violation | K-Cos/Neture | `isPlatformAdmin` 여부에 따라 scope 필터링 달라짐. 누락 시 cross-service 접근 가능. 현재 구현됨 (`:264-270, 972-985`) — 회귀 주의 | HIGH |
| 상태값 `approved` 잔존 | K-Cos | `active`의 alias로 보이나 잔존 데이터 가능성. 정리 필요 | MED |
...
| 라벨 혼용 ("탈퇴"/"비활성화") | K-Cos | 운영자 혼동 위험 | LOW |
...
5. **Neture bulk 활성화 + K-Cos status 탭 확장** — IR-O4O-NETURE-KPA의 W1과 일부 중복. 이미 main에서 부분 처리됨 (`a37edb78d`, `743995e88`, `84d4e2771`).
...
- KPA `MemberDeleteRiskModal` `GpAdminDeleteFlow`, KCos `KCosAdminDeleteFlow`, Neture `AdminMemberDeleteModal` 통합
...
**범위**: K-Cos/Neture의 EditUserModal을 `CommonEditUserModal`로 정합.
...
**범위**: Neture `UsersManagementPage`에 `extraBulkActions` (정지/복원/탈퇴) 추가 — K-Cos 패턴 차용.
- **위험도**: LOW (`84d4e2771` 인프라 이미 존재)

### W-E — WO-O4O-OPERATOR-MEMBERS-STATUS-TAB-EXPANSION-V1 (K-Cos)
**범위**: K-Cos에 pending/active/rejected 상태 탭 추가.
...
## 부록 C. 2026-06-01 현황 업데이트

> HEAD `af9db233f` 기준 재조사. 2026-05-28 이후 완료된 WO 반영.

### 원래 IR에서 지적된 GAP 중 해소된 항목

| Gap | 구 상태 (2026-05-28) | 현재 상태 (2026-06-01) | 해결 WO |
|-----|---------------------|----------------------|---------|
| Neture bulk action 전무 | ❌ bulk 없음 | ✅ extraBulkActions 3개(정지/복원/탈퇴) | WO-O4O-OPERATOR-MEMBERS-LIST-COMMONIZATION-V1 |
 status 탭 2개 | ❌ (suspended/withdrawn) | ✅ 4개(승인/반려/정지/탈퇴) | WO-O4O-OPERATOR-MEMBERS-LIST-COMMONIZATION-V1 |
| KCOS status 탭 2개 | ❌ (suspended/withdrawn) | ✅ 5개(가입 신청/활성/거절/정지/탈퇴) | WO-O4O-KCOSMETICS-MEMBERSHIP-APPROVAL-FLOW-STABILIZATION-V1 |
| KCOS/Neture EditUserModal 별도 | ⚠ 서비스-로컬 구현 | ✅ CommonEditUserModal 공통 적용 완료 | WO-O4O-OPERATOR-EDITUSER-MODAL-PHASE1-NETURE-GP-KCOS-V1 |
| KPA 독립 1400줄 구현 | ⚠ wrapper 미사용 | ✅ thin wrapper 전환 완료 | WO-O4O-KPA-OPERATOR-MEMBER-MANAGEMENT-WRAPPER-MIGRATION-V1 |
| KCOS pharmacist/supplier role 잔존 | ⚠ 잔존 | ✅ 제거 완료 (0건 확인) | WO-O4O-KCOSMETICS-OPERATOR-VOCABULARY-PHARMACY-CLEANUP-V2 |
| 회원 유형/운영 권한 컬럼 분리 | ⚠ 미적용 | ✅ KCOS/Neture 모두 extraColumns로 분리 |  |
| Neture 완전삭제 별도 admin 페이지 | ⚠ 미구현 | ✅ AdminMemberManagementPage 구현 | WO-O4O-NETURE-ADMIN-MEMBER-HARD-DELETE-V1 |

### 아직 미해소된 GAP (신규 발견 포함)

| Gap | 우선도 | 내용 | 권장 WO |
|-----|:-----:|------|--------|
| **KCOS admin roleTabs 불일치** | 🔴 | operator: `cosmetics:store_owner/seller/store_owner` ↔ admin: `seller`만. buyer key != consumer | WO-O4O-MEMBER-MANAGEMENT-ADMIN-ROLETAB-STATUSTAB-ALIGNMENT-V1 |
| **KCOS admin statusTabs pending 없음** | 🔴 | operator에는 있으나 admin에 없음 → 가입 신청 회원 admin 화면에서 미노출 | WO-O4O-MEMBER-MANAGEMENT-ADMIN-ROLETAB-STATUSTAB-ALIGNMENT-V1 |
| `searchPlaceholder` prop 미노출 | 🟡 | MemberListLayout에는 있으나 OperatorMembersConsolePageProps에 없음 | WO-O4O-MEMBER-MANAGEMENT-WRAPPER-SEARCH-PLACEHOLDER-PROP-V1 |
 EditUserModal `membershipRoleOptions` 어휘 불일치 | 🟡 | — | IR (데이터 확인) → WO |
| W-A Delete Flow 공통화 | 🟢 | 4서비스 Delete UX 각자 구현 유지 중 → 공통 OperatorMemberDeleteFlow 신규 | WO-O4O-OPERATOR-MEMBERS-DELETE-FLOW-COMMONIZATION-V1 |
| Neture `/operator/users` 라우트 비표준 | 🟢 | canonical: `/operator/members` | WO-O4O-NETURE-OPERATOR-MEMBERS-NAMING-CANONICALIZATION-V1 |
...
W-A(Delete Flow), searchPlaceholder prop, admin 탭 정렬 완료 시 **95% 이상** 도달.

### 즉시 권장 WO

**WO-O4O-MEMBER-MANAGEMENT-ADMIN-ROLETAB-STATUSTAB-ALIGNMENT-V1** (최고 우선도):
- `services/web-k-cosmetics/src/pages/admin/KCosmeticsAdminMembersPage.tsx`
- 2개 파일, backend 무변경, 위험도 낮음
