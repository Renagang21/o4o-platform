...
> **조사 목적**: 3개 서비스(KPA, K-Cosmetics, Neture)의 multi-role dashboard redirect / workspace entry 구조 현황을 파악하고, 공통화 가능 영역과 서비스 고유 영역을 분류한다.
>
> **상태**: COMPLETE
> **날짜**: 2026-05-15
...
**Gap**: K-Cosmetics는 LoginPage 내부에서 redirect를 처리한다. App.tsx 수준 PostLoginRedirect가 없으므로 로그인 후 URL 직접 접근 시 redirect 로직이 발동하지 않을 수 있다.
...
**Gap**: K-Cosmetics, Neture는 로그인 후 헤더 드롭다운에 스토어/워크스페이스 직접 링크가 없다. 대시보드 redirect가 성공해도 헤더에서 재진입이 불편하다.
...
| 영역 | 이유 |
|------|------|
| `{SERVICE}_ROLE_PRIORITY` 배열 | 서비스마다 역할 체계가 다름 (kpa:pharmacist:pharmacist vs neture:supplier) |
...
| Drift 유형 | 서비스 | 심각도 |
|-----------|--------|--------|
| LoginPage 내부 redirect (PostLoginRedirect 미적용) | K-Cosmetics | Medium |
| 헤더 드롭다운 워크스페이스 링크 없음 | K-Cosmetics, Neture | Low |
| Dual-source 역할 감지 없음 | K-Cosmetics, Neture | Low (현재 증상 없음) |
| `getKCosmeticsDashboardRoute([result.role])` 버그 | K-Cosmetics | High (multi-role 판정 오류) |
| Role label helper 없음 | KPA, K-Cosmetics | Low |
...
- **중기 정렬**: K-Cosmetics를 KPA 수준으로 끌어올리는 PostLoginRedirect 이관 (P2)
