...
| # | 흐름 | KPA-Society | Neture | K-Cosmetics |
| --- | ------ | :-----------: | :------: | :-----------: |
| 1 | 커뮤니티에서 전체 포럼 보기 | ✅ ForumHomePage | ✅ ForumHubPage | ✅ ForumHubPage |
| 2 | 포럼 상세 진입 | ✅ ForumDetailPage | ✅ ForumPostPage | ✅ PostDetailPage |
| 3 | 게시글 보기 / 글쓰기 | ✅ ForumWritePage | ✅ ForumWritePage | ✅ ForumWritePage |
| 4 | 포럼 개설 신청 진입 | ✅ (ForumRequestPanel) | ❌ 없음 | ❌ 없음 |
| 5 | 내 신청 목록 확인 | ❌ 전용 화면 없음 | ❌ 없음 | ❌ 없음 |
| 6 | "내 포럼 관리" 화면 존재 | ❌ 없음 | ❌ 없음 | ❌ 없음 |
| 7 | 내 운영 포럼 목록 표시 | ❌ | ❌ | ❌ |
| 8 | 포럼 기본 정보 수정 | ❌ | ❌ | ❌ |
| 9 | 포럼 삭제 요청 | ❌ | ❌ | ❌ |

다른 서비스는 커뮤니티 이용(1~3)만 구현됨.
...
| # | 흐름 | KPA-Society | Neture | K-Cosmetics |
| --- | ------ | :-----------: | :------: | :-----------: |
| 1 | 포럼 신청 목록 조회 | ✅ ForumManagementPage | ✅ ForumManagementPage | ❌ 없음 |
| 2 | 승인 / 반려 / 보완 | ✅ 3-action (분회 기반) | ⚠️ 2-action (approve/reject) | ❌ |
| 3 | 승인 시 포럼 생성 흐름 | ✅ 자동 생성 (분회 scope) | ✅ (공통 서비스 경유) | ❌ |
| 4 | 삭제 요청 목록 조회 | ❌ | ❌ | ❌ |
| 5 | 삭제 요청 승인/반려 | ❌ | ❌ | ❌ |
| 6 | 운영자 메뉴/라우트 접근 | ✅ `/operator/forum-*` | ✅ `/operator/forum-management` | ❌ |
| 7 | 포럼 분석 대시보드 | ✅ ForumAnalyticsDashboard | ❌ | ❌ |

**요약**: 운영자 신청 검토는 KPA, Neture 2개 서비스에서 구현. 삭제 요청 검토와 포럼 분석은 각각 KPA에만 존재.
...
| API | KPA-Society | Neture | K-Cosmetics |
| ----- | :-----------: | :------: | :-----------: |
| **서비스별 forumRouter** | ✅ `/api/v1/kpa/forum/*` | ❌ (공통만 사용) | ❌ (공통만 사용) |
| **포럼 CRUD (posts/comments)** | ✅ 서비스별 | ✅ 공통 `/api/v1/forum/*` | ✅ 공통 `/api/v1/forum/*` |
| **카테고리 목록** | ✅ | ✅ | ✅ |
| **카테고리 인기순** | ✅ | ✅ | ✅ |
...
| **포럼 신청 생성** | ✅ KPA 자체 | ✅ 공통 API | ✅ 공통 API |
| **내 신청 목록** | ✅ KPA 자체 | ✅ 공통 API | ✅ 공통 API |
| **운영자 신청 검토** | ✅ `/kpa/branches/:id/forum-requests/*` | ❌ 서비스별 없음 | ❌ |
| **운영자 삭제요청 검토** | ❌ | ❌ | ❌ |
| **포럼 분석** | ✅ `/kpa/operator/forum-analytics` | ❌ | ❌ |

### 프론트엔드 API 클라이언트

| API 메서드 | KPA-Society | Neture | K-Cosmetics |
| ----------- | :-----------: | :------: | :-----------: |
| `getMyCategories()` | ❌ | ❌ | ❌ |
| `updateMyCategory()` | ❌ | ❌ | ❌ |
| `requestDeleteCategory()` | ❌ | ❌ | ❌ |
| `forumDeleteRequestApi.*` | ❌ | ❌ | ❌ |
| Forum 신청 생성/조회 | ✅ (내장) | ✅ ForumManagementPage 내부 | ❌ |
| 운영자 신청 리뷰 | ✅ 분회 API | ✅ ForumManagementPage 내부 | ❌ |

---

## 5. 역할 분리 상태 비교

| 역할 | KPA-Society | Neture | K-Cosmetics |
| ------ | :-----------: | :------: | :-----------: |
| **커뮤니티 Forum = 일반 이용** | ✅ 분리됨 (ForumHomePage) | ✅ 분리됨 (ForumHubPage) | ✅ 분리됨 (ForumHubPage) |
| **대시보드 Forum = 내 포럼 관리** | ❌ 미구현 | ❌ 미구현 | ❌ 미구현 |
| **운영자 Forum = 승인/검토** | ✅ ForumManagementPage | ⚠️ ForumManagementPage (부분) | ❌ 미구현 |
...
| 기능 | KPA-Society | Neture | K-Cosmetics |
| ------ | :-----------: | :------: | :-----------: |
| 커뮤니티 포럼 이용 (조회/글쓰기/댓글) | 🟢 완료 | 🟢 완료 | 🟢 완료 |
| 포럼 개설 신청 | 🟢 완료 (분회 기반) | 🟡 부분 (UI만) | 🔴 미구현 |
| 내 신청 목록 확인 | 🔴 미구현 | 🔴 미구현 | 🔴 미구현 |
| 내 포럼 관리 대시보드 | 🔴 미구현 | 🔴 미구현 | 🔴 미구현 |
| 소유자 기본 정보 수정 | 🔴 미구현 | 🔴 미구현 | 🔴 미구현 |
| 소유자 삭제 요청 | 🔴 미구현 | 🔴 미구현 | 🔴 미구현 |
| 운영자 신청 검토 | 🟢 완료 | 🟡 부분 | 🔴 미구현 |
| 운영자 삭제 요청 검토 | 🔴 미구현 | 🔴 미구현 | 🔴 미구현 |
| 포럼 분석 대시보드 | 🟢 완료 | 🔴 미구현 | 🔴 미구현 |
...
1. **공통 API는 이미 존재**: `/api/v1/forum/categories/mine`, `/categories/:id/owner`, `/categories/:id/delete-request`는 공통 forum.routes.ts에 이미 등록됨.
...
다른 서비스에는 대시보드 Forum 자체가 없어 중복 문제 없음.
...
| 차이 유형 | 내용 |
|----------|------|
| **라우트 구조 차이** | KPA는 서비스별 forumRouter 보유, Neture/K-Cosmetics는 공통 API만 사용 |
| **신청 시스템 차이** | — |
| **소유자 API 차이** | — |
...
| 운영자 포럼 삭제 요청을 공통 레이어로 | 서비스별 operator 컨트롤러마다 복제하는 대신, 공통 forum routes에 통합 고려 |
| 포럼 분석 대시보드 공통화 | KPA만 보유. Neture에도 필요한지 판단 |
