...
O4O 3개 서비스(KPA-Society, K-Cosmetics, Neture)의 운영자/관리자, 매장 허브, 내 매장 영역에서 UI-UX 표준화 가능성을 조사한다.
...
| ------ | :---: | :-----------: | :------: |
| 매장 관리 | `OperatorStoresList` thin wrapper | `OperatorStoresList` thin wrapper | `OperatorStoresList` thin wrapper |
| 회원 관리 | `OperatorMembersConsolePage` | `OperatorMembersConsolePage` | `OperatorMembersConsolePage` |
| CMS 콘텐츠 | `CmsContentManager` | (별도) | (별도) |
| 포럼 관리 | 커스텀 | N/A | 커스텀 |

커스텀 batch UI 사용(ActionBar 미사용). → **표준화 대상**.
...
KPA는 자료실(Resources) 추가.

### 5-3. Hub 각 섹션 표준화 수준

| 섹션 | K-Cosmetics | KPA | 표준화 도구 |
| ------ | :-----------: | :---: | ------------ |
| B2B 카탈로그 | ✅ (KPA 기준 정렬 완료) | ✅ canonical | `HubB2BCatalogPage` |
...
| 개념 | KPA | K-Cosmetics | Neture |
| ------ | ----- | ------------ | -------- |
| 매장 운영자 역할 | `storeOwner` | `storeManager` | N/A (메뉴 없음) |
| 허브 vs 내 매장 순서 | 내 매장 first | HUB first | N/A |
| 매장 허브 진입 | 허브 탭 | 허브 탭 | 별도 랜딩 |
| 운영자 대시보드 | `/operator` | `/operator` | `/operator` |
...
- **KPA/KCOS**: community / store_hub / common 3축
...
| `CommonEditUserModal` | 3개 (KCOS/Neture) | ✅ |
| `KpaEditUserModal` | KPA | ✅ (정당 분리) |
| `OperatorMemberDeleteFlow` | KCOS | ✅ 완료 |
...
| 8-item 고정 메뉴 | KPA/Neture flat, KCOS section | ⚠️ KCOS 편차 |

### 10-4. @o4o/shared-space-ui

| 컴포넌트 | 사용 서비스 | 표준화 수준 |
|---------|-----------|:----------:|
| `StoreHubTemplate` | KCOS | ⚠️ KPA/Neture 미사용 |
| `ContentHubTemplate` | KCOS, (KPA 일부) | ⚠️ |
| `StandardHomeTemplate` | 부분 | ⚠️ |
| `SignageHubTemplate` | KCOS | ⚠️ KPA 미적용 여부 확인 필요 |
...
| Neture admin 콘솔 공통화 | 22 pages의 목적/역할이 KPA/KCOS admin과 질적으로 다름 |
...
| 순서 | WO 후보 | 위험도 | 설명 |
|:---:|--------|:-----:|------|
| 1 | — | 낮음 | ActionBar + useBatchAction 패턴 적용 |
...
| **서비스별 차이 = 도메인 차이 vs 구현 편차** | K-Cosmetics store section-based 메뉴 PharmaciesPage 커스텀 batch UI, KPA Hub 비스포크 — 이 세 가지는 **도메인 차이가 아닌 구현 편차**로 판단. | ⚠️ 정합 권장 |
...
| **Neture supplier/partner 구조** | Neture의 supplier/partner는 조직 중심 구조로 KPA/KCOS의 store owner와 목적이 다름. Nav 통합은 가능하지만 구조 강제 통일은 불가. | ✅ 의도된 분리 |
| **KPA canonical 기준** | 5-block dashboard, DomainIASidebar, DataTable, OperatorMembersConsolePage 모두 KPA 기준으로 타 서비스 정렬됨. | ✅ |
| **1인 개발 생산성** | thin wrapper 패턴으로 서비스 페이지 규모 최소화됨. | ⚠️ 정합 권장 |

**결론**: 운영자 영역은 5-block + thin wrapper 기반으로 고도로 표준화됨. Store Hub/My Store 영역은 별도 채팅방 관할.
