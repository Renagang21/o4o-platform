...
>
> KPA-Society / K-Cosmetics 의 operator layout wrapper 3개는 **layout shell 이 byte-identical** 이며, 차이는 (1) 컴포넌트명, (2) 렌더하는 `*GlobalHeader`, (3) `isAdminOrAbove` 에 넘기는 serviceKey 문자열 **3가지뿐**이다. 서비스별 복잡도(profile dropdown / NotificationBell / brand)는 전부 `*GlobalHeader` **브릿지 내부**에 있어 wrapper 공통화와 직교한다.
>
...
### 2.1 KPA / K-Cosmetics (동일 패턴 — 본 공통화 대상)
...
마운트: route element layout Outlet 기반.
...
## 3. KPA / K-Cosmetics wrapper 비교표

| 축 | KPA | K-Cosmetics |
| ---- | ----- | ------------- |
| 컴포넌트명 | `KpaOperatorLayoutWrapper` | `OperatorLayoutWrapper` |
| export | default | default |
| header import | `KpaGlobalHeader` | `KCosGlobalHeader` |
| isAdmin serviceKey | `'kpa'` | `'cosmetics'` |
| isAdmin helper | `isAdminOrAbove` | `isAdminOrAbove` |
| menuItems | `filterMenuByRole(UNIFIED_MENU, isAdmin)` | 동일 |
| capabilities | `ENABLED_CAPABILITIES` | 동일 |
| useAuth source | `../../contexts/AuthContext` | `../../contexts/AuthContext` |
| 외곽 div | `min-h-screen flex flex-col bg-gray-50` | 동일 |
| container | `max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6` | 동일 |
| inner flex | `flex gap-6` | 동일 |
| sidebar | `DomainIASidebar` (offset `top-20`) | 동일 |
| main | `flex-1 min-w-0` + `<Outlet/>` | 동일 |
| useMemo deps | `[isAdmin]` | 동일 |
...
| 차이 | 처리 방식 |
|------|-----------|
| serviceKey (`kpa`/`cosmetics`) | wrapper 가 isAdmin 계산 후 menuItems 만 전달 → 공통 컴포넌트는 serviceKey 불필요 (또는 prop) |
...
| 항목 | KPA | K-Cosmetics |
| ------ | ----- | ------------- |
| 강의 대시보드 (`/instructor`) | isInstructor | isInstructor (showInstructor) |
| 관리자 대시보드 (`/admin`) | isAdmin | isAdmin |
| 운영 대시보드 (`/operator`) | isOperator | isOperator |
| fallback 대시보드 | 없음 | **있음** (`!isAdmin&&!isOperator` → `getKCosmeticsDashboardRoute`) |
| 내 매장 (`/store`) | **있음** (isStoreOwner) | 없음 |
| 마이페이지 icon | LayoutDashboard | **UserCircle** |
| 설정 (`/mypage/settings`) | 공통 | 공통 |
| utilitySlot | **credit 뱃지(⭐C)** + NotificationBell | NotificationBell |
| register 진입 | `openRegisterModal` (useAuthModal) | `navigate('/register')` (모달 없음) |
| login 진입 | `openLoginModal` (useAuthModal) | `useLoginModal` |
| notif serviceKey | `'kpa-society'` | `'k-cosmetics'` |
| logout | `await logout()` → `/` | `logout()` → `/` |
| brand | 💊 KPA-Society / #2563eb | 💄 K-Cosmetics / #db2777 |
...
- 포함하려면 **선행 WO**: `WO-O4O-NETURE-OPERATOR-SIDEBAR-DOMAIN-IA-MIGRATION-V1`
...
| 우선순위 | WO (가칭) | 범위 |
|:---:|-----------|------|
| 1 | **WO-O4O-CROSSSERVICE-OPERATOR-LAYOUT-WRAPPER-COMMON-COMPONENT-V1** | operator-ux-core 에 `OperatorAreaLayout` 추가 + 3 서비스 wrapper 를 축소형으로 교체. menuItems/capabilities/header slot props. |
...
| Twin Axis (KPA + Neture canonical) | ⚠️ 부분 — 현재 공통화는 KPA만. **Neture 축은 OperatorShell 잔류로 미합류** → 완전한 twin-axis 정합은 Neture migration(후속 WO) 후 달성. 본 IR 은 이를 명시적 후속으로 분리 |
