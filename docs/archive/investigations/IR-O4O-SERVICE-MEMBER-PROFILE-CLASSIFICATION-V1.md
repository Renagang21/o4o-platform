...
>
> KPA / K-Cos 2 service 의 회원 프로필 분류 (직역 / 참여자 유형) 체계를 조사하고, **role (권한) 과 분리된 profile classification** 의 canonical 설계를 확정한다.
...
| 조사 방법 | 4 회 병렬 Explore agent (KPA / K-Cos / backend) |
| 조사 범위 | — |
...
| 차원 | KPA | K-Cos |
|---|---|---|---|
| **Profile entity** | `KpaMember` + `KpaPharmacistProfile` | — | 없음 (User entity + cosmetics_store_members 매장 관계만) |
| **Profile classification 컬럼** | `kpa_members.activity_type` (enum 11종) + `membership_type` + `fee_category` + `sub_role` (예정) | `membership_type='pharmacist'` 고정 | **부재** (cosmetics_store_members.role 은 매장 내 직급, user 프로필 아님) |
...
| `pharmacy_owner` / `staff_pharmacist` (2종) | 약사 직역 | ❌ |
...
| 2종 (근무약사 / 약국 경영자) | 2종 1:1 일치 (이미 구현 — `staff_pharmacist` / `pharmacy_owner`) |
...
| EditUserModal 에 subRole 필드 부재 | 운영자가 약국 경영자 / 근무약사 변경 불가 |
| PUT endpoint payload 에 `subRole` 미수용 | backend 확장 필요 |
| — | role 분리 원칙 위배 (정리 대상) |
...
판정: ⚪ 가능하나 KPA 의 기존 entity 와 중복 — 의미 분리 혼란. Drift 위험.
...
판정: ❌ 타입 안정성 약함 + KPA 의 기존 enum entity 와 분리 — drift 큼.

### Option D — Service-specific Profile Entity (KPA 패턴 mirror)
...
| role / profile 분리 | 강함  / 약함 (KPA) | 중간 | 약함 | **강함** |
...
| 사용자 directive — 최소 선택지 시작 | ✅ K-Cos 2종 2종, KPA 기존 유지 | 없음 |
...
- API client 통일 (별건 IR)
- Multi-service profile classification (1 user 가 KPA pharmacy_owner staff_pharmacist 동시 보유 가능성)
...
| 사용자 directive 1:1 매핑 | ✅ KPA 7종 2종 / K-Cos 2종 모두 |
