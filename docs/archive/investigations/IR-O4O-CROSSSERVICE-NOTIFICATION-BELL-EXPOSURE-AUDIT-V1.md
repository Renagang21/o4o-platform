...
Operator/Admin 역할의 대시보드는 모두 GlobalHeader를 상속하므로, GlobalHeader에서 활성화되면 operator/admin에게도 노출된다. K-Cosmetics 관리자 승인 시 in-app notification을 생성하는 기능이 이미 구현되어 있으나, 사용자가 알림을 볼 수 없다.
...
K-Cosmetics에서 회원 승인/거절 시 createNotification 호출 추가
...
|---|---|---|---|
| NotificationBell impl | 예 | 예(미사용) | 예 |
| API impl | 예 | 예 | 예 |
| GlobalHeader 사용 | 예 | 예 | 예 |
| utilitySlot 활성화 | 예 | 아니오 | 예 |
