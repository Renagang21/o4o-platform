...
**조사 범위**: KPA-Society / K-Cosmetics / Neture **operator/admin dashboard**
...
> 2. **Dead Link 와 Vocabulary Drift 가 KPA·K-Cosmetics 에 산재** — KPA `/operator/signage/content` (config 에는 있으나 route 없음), K-Cosmetics `pharmacy/pharmacist/supplier` 어휘 노출 환자/케어 KPI 잔재.
...
| 영역 | KPA | K-Cosmetics | Neture (baseline) |
| ------ | :---: | :-----------: | :----------------: |
| 5-Block unified dashboard API | ❌ (summary only) | ✅ | ✅ |
| `OperatorDashboardLayout` 사용 | △ (커스텀 5-block) | ✅ | ✅ |
| Action Queue backend 응답 | ❌ (frontend 생성) | ✅ (3건) | ✅ (4건) |
| `adminOnly` 메뉴 플래그 | △ (Dashboard prop) | ❌ | ✅ |
| Dead link 0개 | ❌ (≥1) | ✅ | ✅ |
| Vocabulary drift 0개 | △ (supplier 잔재) | ❌ (약국/약사) | △ (약국 placeholder) |
...
|------|---------|--------------|---------|------|
| Approval Backlog | pending > 0 | neture / cosmetics / kpa 모두 | "{항목} {count}건이 있습니다." | ✅ 구체적 |
| Growth Trend | orders.growth ≠ 0 | 모두 | "주문이 전주 대비 {%} {증감}했습니다." | ✅ |
| Activity Drop | inactive/total > 0.3 | neture(stores) | "비활성 매장이 {count}곳, 전체의 {%}입니다." | ✅ |
...
| 분류 | 발견 항목 | 영향 서비스 |
|------|----------|-----------|
| **A. 이미 정렬됨** | Neture 8 KPI + 4 action items + 5 activity, K-Cos pass-through pattern Settlements admin 이동 | (baseline) |
| **B. KPI label drift** | (없음 — 라벨은 대체로 도메인 정합) | — |
| **C. KPI count/source 오류 가능성** | — | K-Cosmetics |
...
| **F. Quick Action route mismatch** | KPA admin "서비스 신청 검토" → `/operator/pharmacy-requests` (label 과 link 의미 불일치 의심) `/operator/lms/courses` | KPA |
| **G. operator/admin scope mismatch** | K-Cosmetics UNIFIED_MENU `adminOnly` 플래그 미적용 Event Offer Approval operator 권한 (재검토 필요) | K-Cosmetics |
| **H. 서비스 도메인 어휘 혼입** | K-Cosmetics StoresPage `pharmacy:'약국'`, UsersPage `pharmacist/supplier`; Neture RecruitingProductsOverviewPage '약국' placeholder | K-Cosmetics, Neture |
...
| W5 | — | Admin KPI `total-patients`/`high-risk-patients`/`open-care-alerts` 제거 또는 옵셔널 + capabilities.CARE 선언 정리 | H | 중 |
...
| §5 HUB 철학 (매장 HUB 운영, 매장 실행 자산 게시) | ✅ | Neture 는 공급자 중심이므로 무관. |
...
| §7 Drift 방지 (도메인 어휘 격리) | ❌ | H 분류 다수: K-Cosmetics pharmacy/약사 환자/케어, Neture 약국 placeholder 등 |
