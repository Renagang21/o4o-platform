...
> **목표**: KPA-Society를 기준으로 매장 HUB / 내 매장 구조를 조사하고 K-Cosmetics 공통화 범위와 예외를 분리한다.
> **제약**: 코드 수정 없음. 조사 전용.
> **날짜**: 2026-05-26
...
| 패키지 | 역할 | 사용 서비스 |
|--------|------|-------------|
| `@o4o/store-ui-core` | 내 매장 사이드바 레이아웃 (`StoreDashboardLayout`), 서비스별 메뉴 config (`KPA_SOCIETY_STORE_CONFIG`, `COSMETICS_STORE_CONFIG`), capability 기반 메뉴 필터 (`resolveStoreMenu`), AI 인사이트 엔진 (`computeStoreInsights`) | KPA, K-Cosmetics |
| `@o4o/store-asset-policy-core` | 매장 자산 정책 UI (`StoreAssetsPanel`), 상태 게이트 (`canToggleStatus`), 미디어 추출 유틸, 라이프사이클 뱃지 | KPA(풀 사용), K-Cosmetics(타입만 사용) |
...
| `@o4o/store-products-ui` | 내 매장 상품 관리 (`StoreProductsManagerPage`) | KPA, K-Cosmetics |
| `@o4o/operator-ux-core` | 서비스별 terminology·uiText config (`kpaConfig`, `kcosmeticsConfig`) | 전 서비스 |
...
`kpa_store_contents` 테이블은 3개 서비스가 공용으로 사용 중.
...
| ------ | :-----------: | :-----------: |
| **HUB 경로** | `/store-hub` | `/store-hub` |
| **레거시 redirect** | `/hub → /store-hub` ✅ | `/hub → /store-hub` ✅ |
| **랜딩 템플릿** | `StoreHubTemplate` ✅ | `StoreHubTemplate` ✅ |
| **HUB 레이아웃** | `PharmacyHubLayout` (로컬) | `KCosmeticsHubLayout` (로컬) |
| **접근 보호** | `HubGuard` + `PharmacyOwnerOnlyGuard` ✅ | storeManager만 노출 (메뉴) |
| **홈 탭** | ✅ | ✅ |
| **B2B 상품 탭** | ✅ `/store-hub/b2b` | ✅ `/store-hub/b2b` |
| **사이니지 탭** | ✅ `HubSignageLibraryPage` (풀 구현) | ⚠️ `HubSignagePage` (플레이스홀더) |
| **콘텐츠/자료 탭** | ✅ `HubContentLibraryPage` (목록 탐색) | ⚠️ `HubContentPage` (버튼 1개만) |
| **이벤트/특가 탭** | ✅ `/store-hub/event-offers` | ✅ `/store-hub/event-offers` |
| **블로그 탭** | ✅ `HubBlogLibraryPage` | ❌ 없음 |
| **POP 탭** | ✅ `HubPopLibraryPage` | ❌ 없음 |
| **QR 탭** | ✅ `HubQrLibraryPage` | ❌ 없음 |
| **AI 추천 블록** | ⚠️ "준비 중" | ⚠️ "준비 중" |
| **HUB API 네임스페이스** | `/kpa/store-hub/...` | `/cosmetics/store-hub/...` |

**핵심 차이**: KPA HUB = 8개 탭(홈+B2B+사이니지+이벤트특가+콘텐츠+블로그+POP+QR). K-Cosmetics HUB = 4~5개 탭. 블로그/POP/QR 탐색 탭 없음.
...
| ------ | :---: | :-----------: |
| **라우트 루트** | `/store` | `/store` |
| **레이아웃** | `StoreDashboardLayout` (@o4o/store-ui-core) ✅ | `StoreDashboardLayout` ✅ |
| **메뉴 Config** | `KPA_SOCIETY_STORE_CONFIG` | `COSMETICS_STORE_CONFIG` |
| **메뉴 섹션 수** | 8섹션 | 5섹션 |
| **capability 기반 메뉴 필터** | `resolveStoreMenu` ✅ | `resolveStoreMenu` ✅ |

### 3-B. 핵심 기능 항목 비교

| 기능 | KPA | K-Cosmetics |
| ------ | :---: | :-----------: |
| **내 매장 홈 (index)** | `StoreHomePage` ✅ | `StoreCockpitPage` ✅ |
| **약국/매장 정보 관리** | `/store/info` (`PharmacyInfoPage`) | `/store/settings` |
| **내 매장 상품** | `StoreProductsManagerPage` ✅ | `StoreProductsManagerPage` ✅ |
| **자체 상품 (local-products)** | `/store/commerce/local-products` ✅ | `/store/local-products` ✅ |
| **태블릿 진열** | `/store/commerce/tablet-displays` ✅ | `/store/tablet-displays` ✅ |
| **채널 관리** | `/store/channels` (hidden) ✅ | `/store/channels` ✅ |
| **사이니지** | `/store/marketing/signage/playlist` ✅ | `/store/signage` ✅ |
| **블로그** | `/store/content/blog` ✅ | `/store/content/blog` ✅ |
| **POP** | `/store/content/pop` ✅ | `/store/pop` ✅ |
| **QR 코드** | `/store/marketing/qr` ✅ | `/store/qr` ✅ |
| **매장 자산 제어 (`StoreAssetsPanel`)** | `/store/content` (`StoreAssetsPage`) ✅ | ❌ 없음 (타입만 import) |
...
| **B2B 주문** | ❌ | ❌ |
| **AI Copilot (내 매장 홈)** | 홈에 통합 | 홈에 통합 |
| **스토어 설정** | `/store/settings` + `/store/settings/template` | `/store/settings` |

### 3-C. 내 매장 내 HUB 가져오기(Import) 흐름

| 기능 | KPA | K-Cosmetics |
| ------ | :---: | :-----------: |
| **HUB 블로그 가져오기** | `POST /kpa/stores/:slug/blog/staff/import` ✅ | ❌ |
| **HUB POP 가져오기** | `POST /kpa/stores/:slug/pop/staff/import` ✅ | ❌ |
| **HUB QR 가져오기** | `POST /kpa/stores/:slug/qr/staff/import` ✅ | ❌ |
| **사이니지 복사** | `StoreAssetsPanel` 내에서 처리 ✅ | ❌ |
...
3. `StoreAssetsPanel` + policy 함수 (store-asset-policy-core) — KPA 동일 사용
...
| HUB 탭별 라이브러리 페이지 | KPA만 완성 (Blog/POP/QR) | KPA 구현을 기준으로 K-Cosmetics에 적용 |
| 내 자료함 (Library) | KPA만 존재 | K-Cosmetics 확장 대상 |
| StoreAssetsPanel 사용 | KPA 사용, K-Cosmetics 미사용 | K-Cosmetics에 `StoreAssetsPage` 추가 |
| HUB 접근 보호 (Guard) | KPA만 `HubGuard` 적용 | K-Cosmetics에 동일 Guard 적용 |

### 4-C. O4O 철학 기준 장기 공통화 후보

- 블로그/POP/QR Staff API 패턴 (현재 KPA 전용 `/kpa/stores:slug/*`) → K-Cosmetics에도 동일 패턴 확장
- 마케팅 분석 (`/store/analytics/marketing`) → K-Cosmetics에도 추가
...
| K-Cosmetics Blog Staff API 없음 | `/cosmetics/stores/:slug/blog/staff` 미구현 | KPA 패턴으로 추가 |
...
| K-Cosmetics StoreAssetsPanel 미사용 | ⚠️ 불일치 | — |
...
- K-Cosmetics에 내 자료함(Library) 기능 추가
...
### B. KPA를 기준으로 K-Cosmetics 확장이 가능한가?

**가능**: HUB 탭 구조, 내 자료함, HUB Import API 모두 KPA 구현을 기준으로 확장 가능하다. 서비스별 예외만 분리 유지한다.
...
API prefix 정리(pharmacy→store)는 breaking change를 수반하므로 P3으로 연기한다.

### E. 메뉴/라우트만 정리하면 되는가?

**아니다**: HUB 탭 구현이 실질적으로 누락(K-Cosmetics 블로그/POP/QR 탭)되어 있어, API 및 페이지 컴포넌트 추가가 모두 필요하다.
