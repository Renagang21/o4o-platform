...
| 바로 WO 진행 가능 여부 | **K-Cosmetics: 정책 논의 필요** |
...
| 상태값 | service_memberships || cosmetics_members | kpa_members |
|--------|---------------------|--------------------|-------------------|-------------|
...
| Action | Neture Operator | KPA Operator || K-Cosmetics Operator |
|--------|----------------|--------------|---------------------|----------------------|
...
|--------|------------|-------------|
| `pending` | 가입 신청 / 승인대기 | KPA, Neture (자동)
| `active` | 활성 | Neture, K-Cos (추가 가능) |
| `approved` | 승인됨 |
| `rejected` | 반려 / 거절 | KPA, Neture
| `suspended` | 정지 | KPA, Neture, K-Cos |
...
| R2 | — | 중간 | wrapper 전환 시 status 매핑 확인 필요 |
| R3 | — | 중간 | 안 A(wrapper 전환) 로 해결 |
...
| R6 | — | 낮음 | `{ key: 'all', label: '전체' }` — roleFilter 없음. wrapper는 roleFilter: [] 를 '필터 없음'으로 처리하여 문제 없음 |
...
표현 방식 차이이며 기능 동등. 단, 공통 UX 표준(`OperatorMembersConsolePage` wrapper)과 불일치 — Drift 가능성.

### O4O-OPERATOR-CANONICAL-WORKFLOW-V1 (검수·승인 UX)

Admin 페이지는 wrapper 사용 중이므로 불일치 상태.

### USER-OPERATOR-FREEZE-V1 (F11) — service_memberships.status SSOT

`service_memberships.status`가 회원 상태 SSOT. 일관성 유지됨.

### OPERATOR-DATATABLE-POLICY-V1

공통 wrapper 미사용 서비스는 DataTable 정책 적용 여부가 개별 구현에 의존. 안 A(wrapper 전환) 수행 시 DataTable 정책 자동 적용.
