...
**`kpa_members` 단독.** `memberRepo.findOne({ where: { license_number:.. } })` — KpaMember repository.
...
| 차원 | KPA |
|---|---|---|
| license 저장 컬럼 | `kpa_members.license_number` (VARCHAR, Partial UNIQUE) | — |
...
KPA 차단 원인은 **kpa_members 내부 데이터** 100%.
...
| 3. 서비스별 unique | KPA 각자 독립 | 서비스 격리 강화 | 약사 한 명이 서비스에서 다른 라이센스 사용 가능 — 식별자 일관성 깨짐 |
...
차단 원인 = KPA 내부 (시나리오 B/C, DB 검증 1회 필요).*
