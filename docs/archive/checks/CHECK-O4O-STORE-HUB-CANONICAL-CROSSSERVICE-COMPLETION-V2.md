...
**날짜:** 2026-06-01  
**유형:** read-only 최종 검증  
**목적:** KPA / K-Cosmetics Store Hub 7개 영역 cross-service canonical 정렬 최종 확인  
...
| hub layout 유지 | ✅ |
| live API (`signage-media`) | ✅ HTTP 200 |
...
단, 선택적 개선 후보:
- Event Offers: KPA `PharmacyOwnerOnlyGuard > KpaEventOfferPage` 수준으로 K-Cosmetics를 업그레이드하는 것은 별도 WO 가능 (현재 FUNCTIONAL 수준은 운영 지장 없음)
