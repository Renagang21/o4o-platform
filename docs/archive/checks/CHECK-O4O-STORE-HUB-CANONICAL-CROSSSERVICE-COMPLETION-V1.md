...
**날짜:** 2026-06-01  
**유형:** read-only 검증  
**목적:** KPA-Society / K-Cosmetics Store Hub 하위 영역 KPA canonical 정렬 최종 점검  
**코드 수정:** 없음
...
| 영역 | KPA | K-Cosmetics | 전체 판정 |
| ------ | ----- | ------------ | --------- |
| **Blog** | FULL | FULL | ✅ ALIGNED |
| **POP** | FULL | FULL | ✅ ALIGNED |
| **QR** | FULL | FULL | ✅ ALIGNED |
| **B2B** | FULL | FULL | ✅ ALIGNED |
| **Content** | FULL | ⚠️ WRAPPER | ⚠️ PARTIAL |
| **Event Offers** | FULL | FUNCTIONAL | ✅ ALIGNED |
| **Signage** | FULL | FULL | ✅ ALIGNED |
...
| 항목 | KPA | K-Cosmetics |
| ------ | ----- | ------------ |
| hubContentApi serviceKey | `kpa-society` | `k-cosmetics` |
| assetSnapshotApi endpoint | `/kpa/assets/copy` | `/cosmetics/assets/copy` |
| signage sourceDomain | `signage-media`, `signage-playlist` | `signage-media`, `signage-playlist` |
| blog sourceDomain | `blog` | `blog` |
| pop sourceDomain | `pop` | `pop` |
| qr sourceDomain | `qr` | `qr` |
| signage assetType | `signage` | `signage` |
| KPA sourceService hardcoding | `sourceService:'kpa'` (KPA only) | ❌ 없음 |

**판정: ✅** K-Cosmetics에 `sourceService:'kpa'` hardcoding 없음.
...
| 영역 | KPA | K-Cosmetics |
| ------ | ----- | ------------ |
| Blog | `importOperatorBlog` | `importOperatorBlog` |
| POP | `importOperatorPop` | `importOperatorPop` |
| QR | `importOperatorQr` | `importOperatorQr` |
| Signage | `assetSnapshotApi.copy({assetType:'signage'})` | `assetSnapshotApi.copy({assetType:'signage'})` |
| Content | `assetSnapshotApi.copy({assetType:'cms'})` | ⚠️ 없음 (wrapper) |
| B2B | 취급 신청 전용 | 취급 신청 전용 |
| Event Offers | 신청 flow | `cosmeticsEventOfferApi` |
...
| 항목 | K-Cosmetics |
| ------ | ------------ |
| route | `/store-hub/signage` → `HubSignagePage` |
| redirect 제거 | ✅ (이전 static card wrapper 교체) |
| 미디어/플레이리스트 탭 | ✅ |
| DataTable + checkbox | ✅ |
| ActionBar 일괄 추가 | ✅ |
| `assetSnapshotApi.copy` | ✅ `{ sourceAssetId, assetType:'signage' }` |
| `sourceService` hardcoding | ✅ 없음 |
| hub layout 유지 | ✅ |
| empty state | ✅ |
| live API | ✅ HTTP 200 (data:[]) |
| 브라우저 smoke | ✅ PASS (K-Cos admin) |

---

## 7. 회귀 여부

최근 K-Cosmetics Signage 정렬 작업 후 다른 영역 회귀 확인:
...
| QR | route 존재, 320줄 | ✅ |
| B2B | route 존재, 369줄 | ✅ |
| Content | route 존재 (wrapper) | ✅ 기존 상태 유지 |
| Event Offers | route 존재, 172줄 | ✅ |
| Signage | route 존재, 579줄 | ✅ 정렬 완료 |
