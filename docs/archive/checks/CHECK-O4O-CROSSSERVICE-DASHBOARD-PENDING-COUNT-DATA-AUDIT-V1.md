...
>
> **결론**: Tier 2 W4 (K-Cos 어휘 정리) / W6 (Neture placeholder) 는 데이터 의존성 없이 **즉시 진행 가능**. W5 는 본 CHECK 범위 외이나 데이터 의존성 적음.
>
> 다른 세션의 hotfix commit `de0f96ddc` 로 처리 중 (§10 별도 항목).
...
| 2 | — | STUB=0 일치, ecommerce_orders 부재 | **A** |
...
| `IR-O4O-ECOMMERCE-ORDERS-VS-CHECKOUT-ORDERS-ALIGNMENT-V1` | §5 — Neture(neture_orders/checkout_orders) vs Cosmetics(ecommerce_orders) 의 주문 source 정합성. 구조적 정책 결정 IR | 중간 |
