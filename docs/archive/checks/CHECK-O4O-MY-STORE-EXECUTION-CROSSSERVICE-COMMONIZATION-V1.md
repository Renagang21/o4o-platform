...
**날짜:** 2026-06-01  
**유형:** read-only 검증  
**목적:** KPA / K-Cosmetics 내 매장·내 약국 실행 영역 cross-service 공통화 수준 조사  
**코드 수정:** 없음
...
**근거:** Store Hub → 내 매장 실행 흐름은 3개 서비스 모두 작동. 사용자-facing 문구 drift 1건 존재.
...
| 영역 | KPA | K-Cosmetics | 판정 |
| ------ | ----- | ------------ | ------ |
| **대시보드** | FULL (StoreHomePage 347줄) | FULL (StoreCockpitPage @o4o/store-ui-core 공통) | ✅ |
| **내 자료함 콘텐츠** | FULL (StoreLibraryContentsPage 236줄) | FULL (263줄, assetSnapshotApi + StartProductionModal) | ✅ |
| **제작 자료** | FULL (StoreProductionMaterialsPage 632줄 + AI editor) | FUNCTIONAL (252줄 + AI editor) | ✅ |
| **POP** | FULL (StorePopPage 876줄 + AI + template + PDF export) | FUNCTIONAL (482줄, AI 있음) | ✅ |
| **QR** | FULL (StoreQRPage 1334줄 + AI + analytics) | FUNCTIONAL (524줄) | ✅ |
| **블로그** | FULL | FUNCTIONAL (StoreBlogManagePage 684줄) | ✅ |
| **사이니지** | FULL (2356줄, 3탭 + schedule) | PARTIAL (401줄, playlist CRUD만, schedule 미구현) | ⚠️ |
| **상품 설명** | FULL (StoreProductDescriptionsPage 742줄) | MISSING | ⚠️ |
| **내 매장 상품** | FULL (StoreProductsManagerPage) | FUNCTIONAL | ✅ |
| **채널 배포** | FULL (StoreChannelsPage 2014줄) | FUNCTIONAL (1126줄) | ✅ |
| **태블릿/디스플레이** | FULL (StoreTabletDisplaysPage 843줄) | FUNCTIONAL (536줄 + InterestRequestsPage) | ✅ |
...
| API | KPA | K-Cosmetics | 비고 |
| ----- | ----- | ------------ | ------ |
| `assetSnapshotApi.copy` | ✅ `{ sourceService:'kpa', assetType }` | ✅ `{ assetType }` | canonical 경로 |
| `assetSnapshotApi.list` | ✅ store library | ✅ store library | |
...
`HubB2BCatalogPage`는 B2B 상품 카탈로그로 "상품을 약국에 추가"하는 개념이므로 "내 약국에 추가"가 맞습니다.
...
| 상품 설명 기능 | K-Cosmetics | MISSING / INTENTIONAL_DIFF | 확인 필요 |
...
KPA canonical 대비 K-Cosmetics는 Signage depth, 상품 설명 기능 등에서 경량화된 구조이나 운영상 치명적 문제는 없음.

**PARTIAL 판정 근거:** 핵심 흐름은 연결되어 있으나 Signage schedule(K-Cosmetics), 상품 설명(K-Cosmetics) 등 일부 영역이 PARTIAL/MISSING 상태.
