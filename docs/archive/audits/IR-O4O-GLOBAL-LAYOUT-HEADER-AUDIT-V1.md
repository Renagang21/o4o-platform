...
> **조사 일자**: 2026-04-17
> **조사 범위**: kpa-society, neture, k-cosmetics
...
| 패키지 | 컴포넌트 | 사용 서비스 | 역할 |
|--------|----------|-------------|------|
| `@o4o/ui` | **OperatorShell** | kpa, neture, k-cosmetics | Operator 대시보드 공통 Shell (사이드바+헤더) |
| `@o4o/store-ui-core` | **StoreDashboardLayout** | kpa, k-cosmetics | Store Owner 대시보드 공통 Shell |
| `@o4o/store-ui-core` | **StoreTopBar**, **StoreSidebar** | kpa, k-cosmetics | Store Shell 내부 컴포넌트 |
...
| 항목 | kpa-society | neture | k-cosmetics |
| ------ | ------------ | -------- | ------------- |
| **공개 메뉴** | 홈/포럼/강의 | Home/Community/Supplier/Partner/Contact/About | 홈/허브/커뮤니티 |
| **조건부 메뉴** | 약국HUB/내약국/운영대시보드 | 없음 (역할별 별도 Layout) | 매장관리 |
| **메뉴 정의 위치** | Header.tsx 상수 | NetureLayout.tsx 내장 | Header.tsx 내장 |
| **메뉴 관리 방식** | 하드코딩 + 역할 필터 | 하드코딩 | 하드코딩 |
| **공유 메뉴 시스템** | 없음 | 없음 | 없음 |
...
- **Admin Header → 공유 X** (kpa 각각 자체 구현)
...
1. **스타일링 방식 불일치**: kpa/k-cosmetics는 inline style, neture는 Tailwind CSS — 공통 컴포넌트 구축 시 어느 방식을 채택할지 결정 필요
2. **브랜드 아이덴���티**: 각 서비스의 로고/컬러/아이콘이 다름 (kpa: 💊/blue: Activity/green, neture: text/green, k-cosmetics: 💄/pink) — 공통 Header는 서비스별 props로 주입해야 함
