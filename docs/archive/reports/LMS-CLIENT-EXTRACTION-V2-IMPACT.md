...
| 패턴 | 정의 | KPA | K-Cos |
|---|---|---|---|
| **A** | `(res as any).data?.X ?? res.data ?? null` (envelope + defensive fallback + `as any` 캐스팅) | **10** | **5** |
| **B** | `res.data` (단일 unwrap) | 0 | 0 |
| **C** | 직접 객체 사용 / 이중 unwrap (`.data.data.X`) | 0 | 0 |
| **D** | try/catch 후 null fallback | 2 | 0 |
...
| 메서드 | KPA 사용 파일 | K-Cos | 시그니처 변경 영향 |
|---|---|---|---|
| `getCourse(id)` | 2 | 2 | 반환형 envelope 통일. |
| `getCourses(params)` | 5 | 1 | `PaginatedResponse<T>` 타입 수렴 |
| `getLessons(courseId)` | 2 | 2 | []` 폴백 제거 가능 |
| `getLesson(courseId, lessonId)` | 1 | 1 | — |
| `getEnrollmentByCourse(courseId)` | 4 | 4 | — |
| `enrollCourse(courseId)` | 2 | 1 | 일관됨 |
| `updateProgress(courseId, lessonId, completed)` | 1 | 1 | ✅ 시그니처 통일 완료 |
| `getQuizForLesson(lessonId)` | 1 | 1 | — |
| `submitQuiz(quizId, answers)` | 1 | 1 | 일관됨 |
...
| 2 | — | 중간 | 중 | Phase 5 별도 분리. V2 Step 1 에서는 lms.ts 가 envelope→객체 unwrap 책임 유지 |
