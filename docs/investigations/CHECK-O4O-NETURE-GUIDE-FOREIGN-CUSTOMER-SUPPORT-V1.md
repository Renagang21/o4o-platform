...
### 5.2 배포 후 UI smoke (2026-06-23, neture.co.kr, 비로그인 게스트)
배포: Deploy Web (Cloud Run) `3e479f65c` success.

| # | 항목 | 결과 |
|---|------|------|
| 1 | /guide "외국인 고객 응대 가이드" 카드 노출 | ✅ href=`/guide/foreign-customer-support` |
| 2 | /guide/foreign-customer-support 상세 렌더 | ✅ hero + 9섹션 + 운영정책 + CTA (스샷) |
| 3 | 용어/콘텐츠 정확 | ✅ 매장 취급 상품/O4O 주문 가능 상품/Store Hub/QR·태블릿 보기 |
| 4 | 모바일 폭(390px) 레이아웃 | ✅ 깨짐 없음, 정상 스택 (스샷) |
| 5 | console error / 4xx·5xx | ✅ 상세 0 (/guide 의 401 auth/me 는 비로그인 게스트 — benign) |
| 6 | 기존 /guide 카드 회귀 | ✅ /guide 정상 로드 |
