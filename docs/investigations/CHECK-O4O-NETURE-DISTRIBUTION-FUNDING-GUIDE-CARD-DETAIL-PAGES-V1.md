...
배포 후 확인 권장:
- `/guide` → 유통참여형 펀딩 카드(가이드 04) → `/guide/features/market-trial` 진입
- 카드 목차 8개 클릭 시 각 섹션 anchor 스크롤 동작
- Hero CTA `/market-trial` 이동 유지
- 모바일 폭 카드 그리드 깨짐 없음
- DB 편집 콘텐츠(GuideEditableSection)로 인한 기존 override 가 있으면 reseed/clear 검토 (pageKey=guide/features/market-trial). 현 단계 데이터는 disposable.
