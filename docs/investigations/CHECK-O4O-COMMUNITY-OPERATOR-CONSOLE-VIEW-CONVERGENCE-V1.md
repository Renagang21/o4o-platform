...
| 화면군 | 대상 서비스 | 착수 시점 판정 |
|---|---|---|
| **B21 운영자 커뮤니티 콘솔** | KPA · Neture↔Neture `VIEW_DUPLICATED` · KPA `CORE_ONLY` (MUST_FIX_BEFORE_CLOSE) |
| **C7 운영자 콘텐츠 허브** | KPA | — |
...
|---|---|---|---|---|---:|---|---|
| KPA | `/operator/community` (`OperatorRoutes.tsx:102`) | `pages/operator/CommunityManagementPage.tsx` | `src/api/community.ts` `communityManageApi` | `@o4o/operator-ux-core` (DataTable) · `@o4o/ui` | 629 | Neture (업무 동일) | `CORE_ONLY` |
| Neture | `/admin/community-admin` (`App.tsx:1121`) | `pages/admin/CommunityManagementPage.tsx` | `src/lib/api/communityAdmin.ts` | 없음 | 454 | KPA | `VIEW_DUPLICATED` |

**업무 정체 확정**: B21 은 forum moderation 콘솔이 아니라 **Home 편집 콘솔**(광고 / 스폰서 / (KPA) 하단 링크 CRUD)이다.
삭제 확인 방식(`ConfirmActionDialog` vs `window.confirm`) · 안내 배너 유무 · API 모듈 경로뿐이다.
...
| 차이 | 성격 | 처리 |
|---|---|---|
| accent color (KPA/Neture blue emerald) | 표현 | `accent` preset config |
...
| 이미지 선택 — KPA 만 미디어 라이브러리 | 서비스 자산 | `renderImageField` slot (+ 공통 `ImageFieldShell`) |
 제한적 제공 안내 배너 | 서비스 고유 공지 | `notice` slot |
...
| 차이 | 성격 | 처리 |
|---|---|---|
| status enum (KPA draft/ready draft/published/private) | 서비스 계약 | `statusOptions[]` + `defaultStatus` config |
...
- `ContentHubClient` — `list/get?/create/update/remove/uploadImage?`
  KPA 각각 wrapper 안의 로컬 `apiFetch` 로 구성. **backend endpoint·DTO·error semantics 무변경** (§10).
...
Neture 는 이전에 없던 **retry** 를 는 이전에 없던 **ConfirmActionDialog 기반 삭제 확인**을 공통화로 획득했다.

### 6-1. 관측된 표시 차이 1건 (의도된 것)

기존 KPA 구현은 `statusOptions` 에 없는 status 값을 **무조건 '초안' 배지로 표시**했다(`STATUS_BADGE[status] || STATUS_BADGE.draft`).
...
- CI `Deploy Web Services (Cloud Run)` run `32322639212` — 7 job 전부 success (kpa-society · neture · k-cosmetics · kpa-branch · pharmacy-hub)
