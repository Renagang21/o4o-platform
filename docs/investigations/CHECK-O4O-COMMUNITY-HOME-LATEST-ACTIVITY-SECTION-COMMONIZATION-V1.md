...
|---|---|---|
| 홈 page 파일 | `src/pages/CommunityHomePage.tsx` (385L) | `src/pages/HomePage.tsx` (309L) |
| 섹션 위치 | `StandardHomeTemplate` `latestSlot` | 동일 |
| 데이터 source | `homeApi.getLatest` → `/api/v1/kpa/home/latest` | `/api/v1/cosmetics/home/latest` |
| 응답 shape | `{ success, data: LatestItem[] }` | 동일 (client 가 `data` 만 반환) |
| 표시 개수 | 6 (`LATEST_SUMMARY_LIMIT`) | 6 |
| 표시 항목 | 종류 배지 / 제목 / 작성자(sm↑) / 날짜(`ko-KR` month+day) | 동일 |
| 정렬 | backend 위임 (프론트 정렬 없음) | 동일 |
| 탭 | all / forum / course / content / signage / resource | 동일 |
| 탭 바로가기 | `/forum` `/lms` `/content` `/signage` `/resources` | content=`/store-hub/content` · signage=`/store/marketing/signage/playlist` |
| loading | `Loader2` 스피너 | 동일 |
| error | **있음** (문구 + 재시도) | **없음** (실패를 빈 목록으로 위장) |
| empty | "등록된 글이 없습니다" | 동일 |
| 클릭 destination | `item.href` (backend 제공) `<Link to>` | 동일 |
| accent | blue | pink |
| 모바일/데스크톱 | 작성자 `hidden sm:block` 만 차이 | 동일 |
| 로그인 필요 여부 | 없음 (공개 섹션) | 없음 |
| 기존 shared 소비 | 없음 (인라인 100%) | 없음 |
...
- accent class 3곳 · error 문구/버튼 스타일 · prop 이름(`error` vs `loadError`) · 주석 1줄. **그 외 전 라인 동일.**
...
| content `/store-hub/content` · signage `/store/marketing/signage/library` | `.emerald` | 기존 유지 | `useNavigate` 신규 |
...
backend 변경 아님(프론트 실패 전달 계약 정합). KPA 는 기존 동작 그대로.
...
| KCos `HomePage.tsx` | 309 | 233 | **-76** |
 `CommunityMainPage.tsx` | 481 | 377 | **-104** |
| 서비스 합계 | 1,175 | 889 | **-286** |
| 공통 View (신규) | 0 | 289 | +289 |
| 삭제된 duplicated JSX 블록 | 2벌 (KPA 120L · KCos 104L 121L = 345L) | **0** | — |
...
| 3 | — | VIEW_DUPLICATED | **FULLY_COMMON** |
...
1. **backend `/home/latest` 2벌 인라인 핸들러**(kpa / cosmetics)는 그대로다 — 본 WO 범위 밖(§6). 별도 WO.
