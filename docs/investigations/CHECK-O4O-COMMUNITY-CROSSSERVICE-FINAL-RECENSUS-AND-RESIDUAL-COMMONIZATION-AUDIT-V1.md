...
| Backend route·controller | `apps/api-server/src/routes/forum/service-forum.routes.ts`(공통 factory) + `kpa/` 자체 라우터 | Core 공통 여부 |
...
| 자체 라우터 + 공통 `ForumController` facade | — |

공통 factory 라우트 표(`service-forum.routes.ts:103-142`): `GET/POST/PUT/DELETE /posts`, `/posts:id`, `POST /posts:id/like`, `PATCH /posts:id/pin`, 댓글 CRUD, `/categories/*`, `/categories:id/join-requests`, `/members`, `/membership-status`. KPA 자체 라우터도 동일 표를 노출한다(본 WO 에서 KPA `PUT /comments:id` 댓글 라우트 remount 로 정렬 완료).
...
| # | 기능 | 라우트·구현 근거 | KPA | KCos | PH | NET | 판정 근거 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|---|
| F01 | 포럼 허브 | KPA /forum · KCos /forum · forum · PH /forum · NET /forum·/partner·/supplier·/workspace | FC | FC | FC | FC | FC | 4서비스 전부 ForumHubTemplate(shared-space-ui) 소비. PH 는 허브·목록 진입 2곳 모두 템플릿. |
| F02 | 전체 게시글 목록 | KPA /forum/all(ForumFeedPage) · KCos·PH·NET /forum/posts | CO | FC | FC | FC | FC | KCos·PH·NET = ForumListTemplate. KPA 만 회원 피드(ForumFeedPage) UX 별도 유지 — backend·API 계약은 공통 Core. |
...
| F06 | 게시글 수정 | forum/edit:id (4서비스) | FC | FC | FC | FC | FC | 본 WO 에서 KCos·PH 에 edit 라우트 + ForumWriteForm 편집 모드 추가 → 5/5 정렬. |
| F07 | 게시글 삭제 | 상세 화면 작성자 액션 | FC | FC | FC | FC | FC | 본 WO 에서 KCos·PH 에 작성자 전용 삭제 추가. 최종 권한은 DELETE /posts:id backend. |
...
| F12 | 좋아요 | 상세 화면 | FC | FC | FC | FC | FC | 본 WO 에서 KCos 에 ForumLikeButton + POST /posts:id/like 배선 → 5/5. |
...
| F15 | 포럼 소유자 대시보드 | KPA /mypage/my-forums · KCos·forum/my-dashboard · NET /supplier/my-forum | FC | FC | FC | NI | FC | ForumOwnerDashboard + 서비스 adapter(29~52L wrapper). PH 는 소유자 개설 동선 자체가 없다. |
| F16 | 포럼 회원관리 | KPA·KCos 소유자 회원관리 | FC | FC | FC | NI | NI | ForumOwnerMemberManagement 소비. NET 은 폐쇄형 회원관리 동선 없음(코드 주석 명시), PH 는 소유자 동선 없음. |
| F17 | 포럼 개설·카테고리 요청 | KPA /forum/request · KCos·forum/request-category · NET /forum/request | FC | FC | FC | NI | FC | ForumRequestForm 소비. |
...
| # | 기능 | 라우트·구현 근거 | KPA | KCos | PH | NET | 판정 근거 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|---|
| L01 | LMS 허브/목록 | KPA /lms · KCos /lms · lms | FC | FC | FC | NI | NI | LmsHubTemplate 3/3. PH·NET 은 교육 제품축 자체가 없음. |
...
| # | 기능 | 라우트·구현 근거 | KPA | KCos | PH | NET | 판정 근거 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|---|
| C01 | 커뮤니티 콘텐츠 목록 | KPA /content · KCos·content · NET /content | CO | FC | FC | NI | FC | KCos=63L CommunityContentListTemplate wrapper, NET=ContentHubTemplate. KPA 는 문서형+설문 2섹션 + 자료함 가져가기로 정보구조가 다름 → CORE_ONLY(§7-A). |
| C02 | 콘텐츠 상세 | /content/:id | FC | FC | FC | NI | NI | CommunityContentDetailView 3/3. NET 은 공개 콘텐츠 상세 라우트 없음. |
| C03 | 콘텐츠 검색·필터 | 목록 화면 내 | FC | FC | FC | NI | CO | KPA CommunityContentSearchBar(1곳), KCos 템플릿 내장. NET 은 자체 정렬/출처 필터. |
| C04 | 콘텐츠 사용자 작성 | content/new · /content/write | CO | FC | FC | NI | NI | KCos 122L wrapper(공통 write 템플릿). KPA 는 설문/강의 타입 분기 포함 178L → CORE_ONLY. |
| C05 | 콘텐츠 수정·삭제(작성자) | /content/:id/edit | CO | FC | FC | NI | NI | C04 와 동일 근거. |
| C06 | 자료실 허브 | resources | FC | FC | FC | NI | FC | ResourcesHubTemplate 4/4(KPA·KCos·NET). PH 자료실은 /store-owner 매장 실행자산 축. |
...
| C10 | 감사(Appreciation) 연결 | AppreciationPanel | FC | FC | FC | NI | NI | KPA 3 · KCos 4 4 소비. PH·NET 은 감사 축 미도입. |
...
| # | 기능 | 라우트·구현 근거 | KPA | KCos | PH | NET | 판정 근거 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|---|
| D01 | 커뮤니티 홈 | KPA /community · KCos / · community · NET /community | FC | FC | FC | NI | FC | KPA·KCos=LatestActivitySection 기반, NET=StandardHomeTemplate. PH 홈은 역할 진입 랜딩(103L)으로 커뮤니티 홈이 아니다. |
...
| E04 | 내 수강 | mypage/enrollments | CO | FC | FC | NI | NI | 본 WO 에서 MyEnrollmentsView(account-ui) 추출 → KCos wrapper. KPA 는 문서화된 hybrid list UX 유지(WO-O4O-KPA-MY-ENROLLMENTS-HYBRID-LIST-ALIGN-V1). |
...
| E10 | 멤버십 게이트·접근 판정 | MembershipGate | FC | FC | FC | FC | FC | KPA 6 · KCos 2 4 · PH 8 · NET 2 소비. |
...
| G04 | 서비스 고유 커뮤니티 확장 | KPA 연차보고·자격 forum/feedback · KCos partners · NET service-update forum | SS | SS | SS | NI | SS | 각 서비스 사업 고유 기능. |
...
| 선행 축 | 재확인 결과 | 근거 |
|---|---|---|
| Forum service boundary | 유지 | 2서비스 공통 factory + KPA facade. `forumContextMiddleware` 로 serviceCode·organizationId 강제 |
...
| Content/Resource frontend View | 유지 | KCos 63L/89L/122L wrapper |
...
| 7 | KPA C01 콘텐츠 목록 | A | 문서형 + 설문 2섹션 + 자료함 가져가기(BaseTable/Drawer). KCos 의 단일 리스트와 정보구조가 다르다. |
| 8 | NET C03 검색·필터 | A | NET 은 출처(sourceType) 배지·정렬 토글 축이 따로 있다(APP-CONTENT Phase 2). |
| 9 | KPA C04 콘텐츠 작성 | A | 설문/강의 타입 분기 포함. KCos 는 문서형 단일. |
...
**분류 B(잔존 duplication) = 0건**, **분류 C(adoption gap) = 0건**. 본 감사 시작 시점의 C 유형 6건(KCos 좋아요, KCos·PH 수정/삭제)은 §7 방침대로 후속 WO 로 미루지 않고 이번에 직접 해소했다(아래 §7).
...
| R3 | — | adoption gap | 댓글 라우트 remount 로 공통 factory 라우트 표와 정렬 |
| R4 | KCos `services/forumApi.ts` | adoption gap | `toggleForumPostLike` / `updateForumPost` / `deleteForumPost` 추가 |
...
| R7 `pages/forum/ForumPostDetailPage.tsx` | adoption gap | 동일(좋아요 카운트를 서버 응답 state 로 전환) |
| R8 | PH `pages/forum/ForumDetailPage.tsx` | adoption gap | 작성자 전용 수정/삭제 액션 추가 |
| R9 | KCos·PH `App.tsx` | 라우트 누락 | `forum/edit:postId` 추가(KCos=ProtectedRoute, PH=MembershipGate) |
| R10 | KCos·PH `pages/forum/ForumWritePage.tsx` | adoption gap | 공통 `ForumWriteForm` 편집 모드(prefill + `handleUpdate`) — 패키지 변경 없이 기존 props(`initialTitle`/`initialContentHtml`) 사용 |
| R11 | `packages/account-ui` | 잔존 duplication | `MyEnrollmentsView` / `MyCreditsView` / `MyCertificatesView` 추출 → KCos mypage 를 thin wrapper 로 전환 |
...
| obsolete service wrapper | 0건 (KCos mypage 3화면은 wrapper 로 전환) | — |
...
- **INTENDED_ABSENCE** — 제품축 자체가 없는 경우. PH·NET 의 LMS 10 cell(교육 제품 미보유), PH 의 콘텐츠·자료실 11 cell(PH 콘텐츠는 `/store-owner` 매장 실행자산 축이며 커뮤니티 콘텐츠가 아니다), NET F16(코드 주석에 "폐쇄형 회원 관리 동선이 없다" 명시), PH F15~F17(포럼 개설·소유 동선 자체가 없음), KPA·전서비스 L10(퀴즈·과제 화면 미구현), KCos 의 KPA 고유 진입점 대응(L08·C08·D05·E09·G01).
...
- NOT_IMPLEMENTED 145 → 82: 최초 census 시점에 미구현이던 PH·NET·KCos 의 forum interaction/owner/membership 축이 실제로 구현·배선됐다.
...
- KCos 커뮤니티 목록(포럼/콘텐츠/강의/자료실)은 **정상 empty state**(`총 0개` + 안내문)로 렌더된다. 데이터 0건은 serviceKey 격리 결과이며 오류가 아니다.
...
KCos·PH 는 커뮤니티 목록이 0건이라 기존 데이터로 상세 상호작용을 실행할 수 없었다.
...
| 영향 범위 | content 를 **HTML string 으로 보내는 서비스 전부** — KPA-Society · K-Cosmetics · PharmacyHub 의 글 작성/수정. Neture 만 클라이언트에서 `htmlToBlocks` 로 Block[] 변환 후 전송해 영향 없음 |
...
| 영향 범위 | forum 을 지정하지 않고 보내던 모든 진입점 — K-Cosmetics `/forum/write` `/forum/write`, KPA-Society slug 없는 `/forum/write`(목록 상단 "글쓰기" 링크), Neture(`categorySlug` 만 전송 → 백엔드가 해석하지 않던 키). PharmacyHub 만 `forumId` 를 보내 영향 없음 |
...
| 조치 (frontend) | KPA(slug 없는 진입)·K-Cosmetics 글쓰기에 **PharmacyHub 와 같은 축**의 게시판 선택(`GET /forum/categories` 소비)을 도입하고 `forumId` 를 전송. Neture 는 `categorySlug` 에 대응하는 forum 을 조회해 `forumId` 를 확정 후 전송. 등록 가능한 게시판이 없으면 빈 상태 문구로 안내(무응답 제출 없음) |
...
> 부수 관측(코드 결함 아님): K-Cosmetics · PharmacyHub 는 `forum_category_requests` 에 **completed forum 이 0건**이다.
...
| # | 항목 | 상태 |
|---|---|---|
| R15 | 공통 write 계약(`POST/PUT /forum/posts`) 이 HTML string content 에서 500 (`DOMParser is not defined`) — KPA·KCos·PH 3개 서비스 글쓰기 실패 | **본 WO 에서 수정 완료** (§14-4) |
| R14 | KPA 댓글 인라인 수정 버튼 미렌더 — 공통 `PUT /comments/:id` 도달 불가 | **본 WO 에서 수정 완료** (§14-2) |
| R16 | forum 미지정 작성이 `forum_id = NULL` 로 저장돼 그 서비스에서 영구 비가시(작성 201 → 목록·상세·수정·삭제 전부 불가) — KPA·KCos·NET | **본 WO 에서 수정 완료** (§14-5) — backend fail-closed(`FORUM_REQUIRED`) + 4개 글쓰기 화면 게시판 선택 도입 |
...
|---|---|---|---|
| L09 | 강사 인접 사용자 기능 | KPA · KCos · 강사 운영 화면은 서비스별 교육 운영 정책에 종속 |
| E09 | 내 콘텐츠 | NET (1) | KPA=커뮤니티 작성물 / NET=워크스페이스 자산 — 업무 축이 다름 |
| G02 | 기능 가이드 | KPA · KCos · NET (4) | 서비스별 안내 문구·화면 |
| G04 | 서비스 고유 커뮤니티 확장 | KPA · KCos · NET (4) | 각 서비스 사업 고유 기능 (KPA 연차보고·자격 forum/feedback, KCos partners, NET service-update forum) |

공통 부품을 강제하면 업무 의미가 왜곡되므로 **전건 유지**한다 (공통화 미달이 아니다).

### 15-3. VALID_NOT_IMPLEMENTED (82)

- `INTENDED_ABSENCE` 76 — 해당 서비스에 그 기능 축 자체가 없다(예: PharmacyHub LMS/자료실, KCos 인증서 발급 백엔드 미구성). 공통 부품은 이미 존재하므로 도입 시 재구현이 아니라 배선이다.
- `PRODUCT_DECISION_REQUIRED` 6 — 도입 여부가 제품 결정 사안(예: PH 커뮤니티 홈 활동 피드, KCos 강의 수강/진도).
  - 여기에 §14-5 부수 관측이 더해진다: **KCos · PH 는 `forum_category_requests` 에 completed forum 이 0건**이다.
