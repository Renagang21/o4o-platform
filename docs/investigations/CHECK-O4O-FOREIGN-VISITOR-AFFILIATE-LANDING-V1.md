...
- 배포: api `deploy-api`(push) + web `deploy-web-services`(kpa-society dispatch) **둘 다 success**.
- **API (public, no auth):**
  - `GET /api/v1/foreign-visitor/affiliate/fvq_nonexistent99/resolve` → **404 `AFFILIATE_QR_NOT_FOUND`** ✅ (401 아님 → route 마운트 + public 확인)
  - 형식 오류(60자) shortCode → **404 `AFFILIATE_QR_NOT_FOUND`** ✅
- **Frontend (Playwright):** `https://kpa-society.co.kr/foreign-visitor/affiliate/fvq_nonexistent99` →
  - `landing_rendered: true` / `affiliate_notfound_state: true`("안내를 찾을 수 없습니다 / This guide is not available") / **`spa_404: false`** ✅ — route 매칭 + landing 렌더(SPA 404 아님). 시각 확인(KPA-Society 헤더/푸터).
  - console: resolve 404 1건(unknown shortCode 설계상 정상), pageError 0.
- 양성 경로(실 ACTIVE shortCode → 환영 landing + storeSlug 링크)는 store 가 ACTIVE entitlement 없어 QR 발급 불가 → **발급 후 후속 검증**. resolve 계약·landing 렌더는 코드/404 경로로 확인됨.
