...
> **유형:** forum write 공통화 선행 정렬 — KCos editor(textarea→RichTextEditor) content format(HTML string→blocks)
> **결과: PASS — KCos textarea→RichTextEditor content→blocks(htmlToBlocks) 정렬. 3서비스 write editor=RichTextEditor·content=blocks 통일. postType/create-only 유지, KPA/Neture 미수정.**
...
shared ForumWriteForm 생성·postType 정책·edit route·detail 은 범위 밖.
...
KPA/Neture write 의 canonical 패턴: `RichTextEditor`(@o4o/content-editor) `onChange={(c)=>setState(c.html)}` → submit `htmlToBlocks(html)`(@o4o/forum-core/utils) → content=Block[]. KCos 를 이에 맞춤.
...
- `services/web-k-cosmetics/src/pages/forum/ForumWritePage.tsx`:
  ...
  - content empty 검증: `!content.trim` → html 기준(`content==='<p></p>' || strip(html).trim===''` 동일).
...
- KCos 에 `@o4o/forum-core` workspace dep 추가(htmlToBlocks 사용 위함; 기존 미링크). `pnpm install` 로 symlink + lockfile 갱신.
- **pnpm-lock.yaml diff = forum-core link 2건만**(@o4o/web-k-cosmetics → link:../../packages/forum-core). 타 dep 변동 없음.
- forum-core dist/utils 기빌드(Neture 소비 중) — 런타임 import 안전.

## 7. 검증

- web-k-cosmetics · web-kpa-society · web-neture **각각 0 errors** ✅.
- **정적:**
  - KCos `<textarea>` 0, RichTextEditor + htmlToBlocks 사용 확인. content blocks 전송.
  - **postType select 유지**(KCos POST_TYPES), **create-only 유지**.
...
**PASS.** KCos editor parity(textarea→RichTextEditor) content blocks 정렬 완료. 3서비스 write 가 RichTextEditor·blocks 로 통일됨. postType/create-only 유지, KPA/Neture·backend 무변경, typecheck(4) 통과. **`WO-O4O-FORUM-WRITE-FORM-COMMONIZATION-V1`(create-only, optional showPostType, 확장 slot) 진입 기반 확보.**
...
3. (선택) browser smoke — KCos `/forum/write`(editor 렌더) `/forum/write`(blocks payload).
4. (잔여) KCos `styles.textarea` 미사용(객체 property, tsc 무오류) — 선택적 cleanup.

---

*Date: 2026-06-12 · WO-O4O-FORUM-WRITE-EDITOR-CONTENT-PARITY-V1 · KCos editor content blocks 정렬 PASS. postType/create-only 유지. KPA/Neture·backend 무변경. editor UI 변경은 후속 smoke 권장.*
