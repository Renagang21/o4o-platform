...
- **즉시 깨지는(모바일 미대응) 영역 1곳**: `store-hub`(KCos `*HubLayout`) / `supplier`(Neture `SupplierSpaceLayout`) — sticky만 있고 mobile drawer/hamburger 부재 → 모바일에서 사이드바가 본문 압박·가로스크롤 위험. **최우선 수정 대상**.
- **breakpoint 불일치**: operator·KPA admin·Neture = `md`(768) / KCos admin·Store = `lg`(1024). 표준화의 핵심 결정 변수.
...
| 2 | **DashboardLayout** (내부 sidebar) | `services/web-k-cosmetics/.../layouts/DashboardLayout.tsx` | **admin · KCos /admin** | 서비스별 **복제본 2개** |
...
| 4 | **StoreDashboardLayout** + **StoreSidebar** | `packages/store-ui-core/src/components/StoreSidebar.tsx` | `/store`(KCos·KPA 공유) | **공통** |
| 5 | **\*HubLayout** | `KCosmeticsHubLayout` / KPA `PharmacyHubLayout` | `/store-hub` | 서비스별 독자 |
...
> operator 3개 서비스(KPA/KCos/Neture)는 `OperatorLayoutWrapper` 가 동일 `OperatorAreaShell`+`DomainIASidebar` 를 쓰고 header·menuItems·domainIAConfig 만 주입한다(Neture 는 4-domain 커스텀 IA).
...
|------|-------|--------|-----------------|:--------:|
| operator | `/operator/*` | KPA·KCos·Neture | DomainIASidebar (#1) | ✓ |
| admin | `/admin/*` | Neture | DomainIASidebar (#1, menu만 admin) | ✓ |
| admin | `/admin/*` | KCos | DashboardLayout (#2) | ✓ |
| admin | `/admin/*` | KPA | AdminSidebar (#3) | ✓ |
| admin platform | `/admin/platform/*` | Neture | PlatformSectionLayout (local nav) | ✗(탭) |
| 내 매장 | `/store/*` | KCos·KPA | StoreDashboardLayout (#4) | ✓ |
| 매장 허브 | `/store-hub/*` | KCos | \*HubLayout (#5) | ✓ |
...
| **#2 DashboardLayout(KCos)** | `fixed w-64 lg:translate-x-0` | 고정 사이드바 | drawer(`-translate-x-full`)+`bg-black/50`+`lg:hidden` 햄버거 | **lg(1024)** | ✓/✓/✓ | — (group toggle만) | **높음**(`lg:ml-64` 본문 margin) | Full(단 본문margin·lg) |
...
| **#5a \*HubLayout(KCos)** | `sticky top-20 w-52` | sticky(미대응) | **drawer 없음** | 없음 | ✗/✗/✗ | — | **높음**(모바일 압박·가로스크롤) | **Broken(mobile)** |
...
| #2 KCos Admin | `px-4 py-2.5` | `py-2.5` | `ml-8`(32) | ✗ 없음 |
...
1. **모바일 미대응 2영역(최우선)**: `store-hub`(KCos `*HubLayout`), `supplier`(Neture) — drawer 부재로 모바일에서 사이드바가 본문을 가리거나 가로스크롤 유발.
2. **Breakpoint 분열**: md(768) vs lg(1024) 혼재 → tablet(768~1023) 에서 화면마다 동작이 다름(어떤 곳은 고정 사이드바, 어떤 곳은 본문 margin).
3. **본문 가림 패턴**: KCos admin 의 `lg:ml-64` 고정 margin 은 breakpoint 경계/모바일에서 본문 가림·잘림 위험. flex 기반(#1/#3/#4)이 안전.
4. **desktop sticky vs fixed 혼재**: #1 sticky / #2#3 fixed / #4 hybrid → 짧은 콘텐츠에서 sticky 가 따라 내려가는 시각 차이.
5. **메뉴 계층 스타일 drift**: operator·KPA admin 은 정돈(domain heading/border/pl-12), KCos admin 은 미정돈(ml-8, heading 없음). KPA operator 글자/간격이 "정리 안 됨"이라는 최초 지적의 실체 = 표준 부재.
6. **복제 코드**: KCos `DashboardLayout` 은 거의 동일한 복제본 2개(공통화 안 됨).
...
2. **깨진 영역부터 수리**(회귀 최소·효과 최대): `store-hub`(KCos) → mobile drawer 도입(KPA `PharmacyHubLayout` 패턴 이식), `supplier`(Neture) → mobile drawer 도입.
3. **본문 margin 제거**: KCos admin `DashboardLayout` 의 `lg:ml-64` → flex 레이아웃으로 교체(또는 #1 패턴 흡수). 복제본 2개 공통화 검토.
...
| 순위 | 대상 | 사유 |
|:---:|------|------|
| **P0** | store-hub(KCos) mobile drawer | 모바일 실파손 |
| **P0** | supplier(Neture) mobile drawer | 모바일 실파손 |
| **P1** | KCos admin `lg:ml-64` 본문 가림 제거 + 복제본 공통화 | 본문 가림·중복 |
