...
| GP status 탭 2개 | ❌ (suspended/withdrawn) | ✅ 4개(승인/반려/정지/탈퇴) | WO-O4O-OPERATOR-MEMBERS-LIST-COMMONIZATION-V1 |
| KCOS status 탭 2개 | ❌ (suspended/withdrawn) | ✅ 5개(가입 신청/활성/거절/정지/탈퇴) | WO-O4O-KCOSMETICS-MEMBERSHIP-APPROVAL-FLOW-STABILIZATION-V1 |
| GP/KCOS/Neture EditUserModal 별도 | ⚠ 서비스-로컬 구현 | ✅ CommonEditUserModal 공통 적용 완료 | WO-O4O-OPERATOR-EDITUSER-MODAL-PHASE1-NETURE-GP-KCOS-V1 |
...
| 회원 유형/운영 권한 컬럼 분리 | ⚠ 미적용 | ✅ GP/KCOS/Neture 모두 extraColumns로 분리 | WO-O4O-GLYCOPHARM-KCOS-MEMBER-LIST-MODAL-PERMISSION-DISPLAY-ALIGNMENT-V1 |
...
| Gap | 우선도 | 내용 | 권장 WO |
|-----|:-----:|------|--------|
| **GP admin roleTabs 불일치** | 🔴 | operator: `glycopharm:pharmacist/store_owner` ↔ admin: `pharmacist/pharmacy`, `pharmacy_owner` 불일치 | WO-O4O-MEMBER-MANAGEMENT-ADMIN-ROLETAB-STATUSTAB-ALIGNMENT-V1 |
...
| **GP admin statusTabs/extraColumns 없음** | 🟡 | operator에 있는 상태탭 4개 + 운영권한 컬럼이 admin에 없음 | WO-O4O-MEMBER-MANAGEMENT-ADMIN-ROLETAB-STATUSTAB-ALIGNMENT-V1 |
...
| GP EditUserModal `membershipRoleOptions` 어휘 불일치 | 🟡 | `pharmacy`/`supplier` bare → `glycopharm:pharmacist`/`glycopharm:store_owner` canonical 전환 검토 필요 | IR (데이터 확인) → WO |
