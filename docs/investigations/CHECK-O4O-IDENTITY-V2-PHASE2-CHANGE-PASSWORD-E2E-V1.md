...
- Phase 1 검증 (04:00 UTC) 과 Phase 2 검증 (05:43 UTC) 사이의 시간 차에서 외부 actor 또는 다른 인터랙션이 있었을 가능성.
- 이후 누적 5회 invalid 로 ACCOUNT_LOCKED 발생 → Account #3 fresh 로 핵심 검증 이전.

**판정:** Phase 2 본질 검증(KPA 변경 작동 + 격리) 은 PASS.
...
- B-2 V2Check2Y 성공 → credential.passwordHash 가 V2Check2Y 로 갱신됨 직접 증명
...
**판정:** ✅ PASS — credential upsert 가 정확히  만 갱신.
...
**검증 방법:** Account #3 (KPA 만 가입, neture 미가입) 에서 serviceKey=neture 로 change-password 시도.
...
| F-4 | Login V2Check2Y | success | success | — |
...
| 회귀 항목 | 결과 | 근거 |
|---|---|---|
| Login 흐름 (KPA / serviceKey 없음) | ✅ 정상 | 모든 login 시도 정상 응답 |
...
- 본 Phase 2 WO 의 결함은 아니나, 운영 데이터 일관성 관찰에 참고. 구체적 원인 추적은 본 CHECK 범위 외.
...
- (선택) PasswordChangeModal 에 `serviceName` prop 추가 — UX 명시화
