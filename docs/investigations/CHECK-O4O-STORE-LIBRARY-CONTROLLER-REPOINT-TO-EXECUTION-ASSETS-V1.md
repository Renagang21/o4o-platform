...
> **결과: PASS — `createStoreLibraryController` repository 를 `StoreLibraryItem` → `StoreExecutionAsset` repoint. 레거시 `StoreLibraryItem` 엔티티 제거 + connection 등록 제거. KCos `/pharmacy/library`(rename 이후 부재 테이블 조회 → broken/degraded) 정합 복구. api-server typecheck 0 errors.**
...
선행 점검 결과: `createStoreLibraryController`(KCos `/pharmacy/library` CRUD, 라이브 페이지 소비)가 `getRepository(StoreLibraryItem)`(`@Entity('store_library_items')`)를 사용. 그러나 `store_library_items` 는 `store_execution_assets` 로 **hard rename**(migration `20260421010000`, back-compat 없음) → 부재 테이블 조회 = broken/degraded. 본 WO 가 복구.
...
- **browser/DB smoke:** 미수행 — dev 서버·인증 guard. **배포 후 권장:** ① KCos `/pharmacy/library` GET/POST 정상(StoreLibraryResourcesPage/SelectorModal/TabletDisplays 자료 표시) ② (read-only) `store_execution_assets` 존재·`store_library_items` 부재 DB 확인(gcloud).

## 6. 완료 판정

**PASS.** `/pharmacy/library` 컨트롤러를 현행 `store_execution_assets`(StoreExecutionAsset)로 repoint, 레거시 `StoreLibraryItem` 제거. rename 이후 부재 테이블을 가리키던 KCos 자료실 경로를 정합 복구. 컬럼 호환·typecheck 0·HTTP 계약/DB/migration/UI 무변경.

## 7. 후속

1. **(배포 후)** KCos `/pharmacy/library` 동작 smoke(자료 목록/생성/수정/삭제) — 복구 확인.
2. (선택) `WO-O4O-STORE-PRODUCTION-MATERIAL-LIST-QUERY-CLEANUP-V1` — ProductionMaterials 조회 공통화 + KCos QR/direct 소스 완성.
