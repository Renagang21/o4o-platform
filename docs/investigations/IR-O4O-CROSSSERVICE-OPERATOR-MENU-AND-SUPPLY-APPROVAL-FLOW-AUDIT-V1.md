...
- 이벤트 오퍼 관리 페이지: `EventOfferManagePage.tsx`(KPA), `EventOfferApprovalsPage.tsx`(KCos)
...
| Menu Domain | KPA-Society | K-Cosmetics | Neture |
| --- | :---: | :---: | :---: |
| 대시보드 | route✓ | route✓ | route✓ |
| 회원 관리 | route✓ | route✓ | route✓ |
| **Approvals(승인)** | route✓ | route✓ | route✓ |
| **상품 관리** | **숨김(WO)** | route✓ | route✓ |
| **주문 관리** | **숨김/미정의** | route✓ | route✓ |
| Stores(매장 HUB) | route✓ | route✓ | route✓(매장 관리만) |
| Content | route✓ | route✓ | route✓ |
| 자료실 관리 | route✓ | route✓ | 미정의 |
| LMS | route✓ | route✓ | 미정의 |
| Signage | route✓ | route✓ | 미정의 |
| Forum | route✓ | route✓ | route✓ |
| Analytics | route✓ | route✓ | route✓ |
| System | route✓ | 미정의 | route✓ |
...
> **핵심 관찰:** `공급 상품 신청 승인`(→`/operator/product-applications`)과 `이벤트 오퍼 승인`(→`/operator/event-offers`)은 **KPA/KCos 2서비스에서 동일 path로 이미 정합**되어 있다. 즉 사용자가 우려한 "공급자 상품/이벤트 오퍼 승인을 어디서 처리하는지 불명확"은 **코드 레벨에선 이미 Approvals 그룹으로 통일**되어 있다. 진짜 불일치는 §3.
...
대조 `products`(45) `상품 관리`→`/operator/products`, `orders`(59) `주문 관리`→`/operator/orders` 정의됨.
...
- 승인 = 매장 승인 + 이벤트 오퍼 + 공급 상품 신청.
- 승인 첫 항목이 `신청 관리`(통합 신청). System 그룹 미정의.
...
**비대칭 요약:** 메커니즘(sidebar/layout)은 공통화되었으나, **메뉴 노출 정책이 서비스별로 갈림**. Neture는 설계상 별도 축이라 직접 비교 대상이 아니다.
...
| 매장 공급 상품 신청 승인 | 서비스 운영자 | `/operator/product-applications` (ProductApplicationManagementConsole) | **Approvals > 공급 상품 신청 승인** | kpa / k-cosmetics |

- 백엔드는 **KPA 컨트롤러를 serviceKey 파라미터화**하여 KCos 재사용 (`createOperatorProductApplicationsController(..., {scope, serviceKey})`, kpa.routes 246 / gp 537 / kcos 139). 승인 SQL은 `approval.service_key` 기반이라 서비스 격리됨.
...
- Event Offer는 플랫폼 공통 도메인(EVENT-OFFER-COMMON-DOMAIN-V1). KPA/KCos는 operator 승인 일관 배치, **Neture만 미연결**(공급자/시장 도메인 특성).
- 승인 위치는 KPA/KCos 모두 **Approvals 그룹**으로 이미 통일. 라벨도 "이벤트 오퍼 승인"으로 동일.
...
4. 매장 공급 상품 신청 승인 — 서비스 편입 (KPA/KCos, ProductApproval→OPL)
5. 이벤트 오퍼 승인 (KPA/KCos)
...
| 승인 업무 | KPA | K-Cosmetics | Neture |
|---|---|---|---|
| 매장/가입 승인 | (회원/협업) | 신청 관리 | 가입 승인 |
| 공급자 가입·활성화 | — | — | 공급자 활성화/서비스 승인 |
| 공급 상품 신청 승인 | Approvals✓ | Approvals✓ | (Neture=공급자 Offer 측) |
| 이벤트 오퍼 승인 | Approvals✓ | Approvals✓ | 미노출 |
| 협업/기타 | 협업 문의 | — | 유통참여형 펀딩 |

→ **공급 상품 신청 승인·이벤트 오퍼 승인은 KPA/KCos 모두 Approvals 그룹에 동일 배치(정합).** 라벨 차이(매장 승인 vs 신청 관리 vs 가입 승인)와 Neture 이벤트오퍼 미연결만 비정합.
...
표준 8 Group(Dashboard + A자료등록 / B AI작업 / C 큐레이션 / D 매장지원 / E 운영수익 / F 검수·승인 + System)을 도메인 IA로 매핑. **KPA/KCos = 동일 3-domain(커뮤니티/매장 HUB/운영 공통) 공유, Neture = 별도 4-domain.**

### KPA / K-Cosmetics (공통 축)
```
대시보드
...
|---|---|---|---|---|---|
| 1 | **WO-O4O-CROSSSERVICE-OPERATOR-APPROVAL-GROUP-LABEL-ALIGN-V1** | 검수·승인 그룹 라벨/순서/항목 2서비스 정합 (매장 승인/신청 관리/가입 승인 표현 통일) | KPA/KCos | 하 | 본 IR |
| 2 | **WO-O4O-KPA-OPERATOR-PRODUCT-ORDER-MENU-PARITY-DECISION-V1** | KPA 상품 관리/주문 관리(view-only) 노출 정책 재결정·정렬 (숨김 유지 or 해제) | KPA(↔KCos parity) | 중 | 본 IR, Shared Module Protocol |
...
| 5 | **WO-O4O-OPERATOR-MENU-DOMAIN-IA-COMMONIZE-V1** | sidebar 공통화 후속 — KPA/KCos 도메인 IA 메타데이터 단일화 | 2서비스 | 중 | sidebar 공통화 IR |
...
1. **승인 흐름 위치는 "이미 정합"이며 불명확하지 않다.** 공급 상품 신청 승인·이벤트 오퍼 승인은 KPA/KCos 모두 Approvals 그룹의 동일 path(`/operator/product-applications`, `/operator/event-offers`)에 배치됨. → "승인 흐름이 불명확해 별도 콘솔 필요" 판정은 **기각**. 단, 라벨/구성 통일 여지는 있음(WO1).

2. **메뉴 불일치(상품 관리/주문 관리)는 "정상 정책 차이"이자 "과거 작업 잔재"의 경계 사례다.** KPA의 숨김은 `WO-KPA-OPERATOR-STORE-RELATED-MENU-HIDE-V1`/`-STORES-MENU-HIDE-V1`에 의한 의도적 결정(=정책 차이)이나, 이후 KCos가 view-only 상품/주문 메뉴를 노출하는 방향으로 정렬되며 cross-service parity가 깨짐(=잔재). → **capability/route 누락은 아님(가설 b,c 기각).** 재정렬 WO(WO2)가 필요.
...
| 서비스 parity | — | sidebar 공통화 IR: "도메인 IA 공통화 가능" | ⚠️ 정합 WO 필요 |
...
- serviceKey 파라미터화 컨트롤러: `apps/api-server/src/routes/kpa/controllers/operator-product-applications.controller.ts:33-75` (mount kpa.routes:246.routes:537 / cosmetics.routes:139)
