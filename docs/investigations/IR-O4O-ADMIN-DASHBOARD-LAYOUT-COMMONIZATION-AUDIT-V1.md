...
로컬 AdminBlock 컴포넌트 중복 (서비스별 복사본).
...
| ------ | ----- | ------------ | -------- |
| admin guard | `AdminAuthGuard` | `ProtectedRoute(cosmetics:admin)` | `AdminRoute` |
| admin 진입 경로 | `/admin/kpa-dashboard` | `/admin` | `/admin` |
| admin sidebar | `AdminLayout` (전용) | `DashboardLayout role="admin"` | 전용 레이아웃 |
| operator 링크 from admin | `/operator` 빠른 이동 ✅ | ❌ 없음 | ❌ 없음 |
| admin pages 수 | 2 | 2 | 22 |
| admin/operator 분리 | 완전 분리 (AdminRoutes) | App.tsx 라우트 분리 | 완전 분리 |
...
| 서비스 | 가능성 | 필요 작업 | 위험도 |
|--------|:------:|---------|:-----:|
| ✅ 높음 | `AdminDashboardLayout` wrapper 씌우기 + 로컬 AdminBlock을 config 주입으로 전환 | 낮음 |
| KCOS | ✅ 높음 | — | 낮음 |
...
이 컴포넌트는 제목/설명/링크 목록을 받아 카드 형태로 렌더링하는 범용 패턴이다. → `admin-ux-core`에 공통 컴포넌트로 추출 가능.
...
| **KPA** | `AdminAuthGuard` guard 패턴 | KPA 독자 admin 인증 로직. KCOS의 RoleGuard와 다름 |
...
| 위험 | 영향 서비스 | 설명 |
|------|-----------|------|
| KPA AdminAuthGuard 변경 | KPA | KCOS와 guard 패턴 통일 시 KPA admin 보안 로직 변경 필요 |
| Neture 22 pages 강제 통합 | Neture | 대시보드 허브 외 나머지는 Neture 특수 콘솔. 강제 공통화 금지 |
| KCOS Phase 2 AdminBlock 공통화 | KCOS | 링크 경로(정산/인보이스/약국 vs 매장/거버넌스) 다름. 블록 구조만 공통화하고 config는 서비스별 |
| KCOS Operator API 재매핑 의존 | KCOS | 전용 admin API 없이 operator API 필터링 사용 — 장기적으로 backend contract 개선 필요 (별도 IR) |
...
| 순서 | WO | 설명 |
|:---:|-----|------|
| 1 | — | `AdminDashboardLayout` wrapper 적용. Phase 2 AdminBlock을 `StructureActionBlock` config 주입으로 전환 또는 AdminBlock을 admin-ux-core 공통 컴포넌트로 추출 후 사용. |
| 2 | `WO-O4O-KCOS-ADMIN-DASHBOARD-LAYOUT-WRAPPER-V1` | KCOS 서비스 링크만 다름. |
| 3 | `WO-O4O-ADMIN-UX-CORE-ADMIN-BLOCK-EXTRACTION-V1` | KCOS 로컬 AdminBlock → `packages/admin-ux-core` 공통 컴포넌트 추출. 중복 해소. |
...
| 항목 | 설명 |
|------|------|
| KCOS admin 전용 backend API | Operator API 재매핑 의존 해소 — backend contract IR 필요 |
...
| **Admin dashboard 공통화 기반** | `@o4o/admin-ux-core` 4-Block이 이미 존재. Neture는 완전 적용. KCOS는 블록은 사용하나 wrapper 미적용. KPA는 독자. **신규 개발 불필요, 정합만 필요**. | ⚠️ 정합 가능 |
| **서비스별 차이 = 도메인 차이 vs 구현 편차** | KCOS 로컬 AdminBlock 중복 → **구현 편차** (정합 대상). KPA 독자 구현 → **부분 편차** (API 차이 존재). Neture 22 pages → **도메인 차이** (정당 분리). | ✅ 명확히 구분됨 |
| **동일 조작 질서** | 4-Block A→B→C→D 순서가 표준. KCOS는 블록 사용하나 wrapper 없어 C(GovernanceAlerts) 누락. → 적용하면 완성. | ⚠️ KCOS 정합 권장 |
| **Neture admin 특수성** | `AdminDashboardPage` 허브만 공통, 나머지 22 pages는 Neture 도메인 특수 콘솔. **의도된 분리이며 공통화 대상 아님**. | ✅ |
| **1인 개발 생산성** | KCOS 로컬 AdminBlock 중복이 유지보수 비용 증가. admin-ux-core 공통 추출로 단일 소스 유지 가능. KPA 독자 구현 유지보수 비용 있음. | ⚠️ 정합 권장 |

**결론**:
1. `@o4o/admin-ux-core`가 이미 존재하여 신규 공통 레이아웃 개발 불필요.
2. KCOS: `AdminDashboardLayout` wrapper 적용 + 로컬 AdminBlock 공통 추출 → 즉시 실행 가능.
...
| `services/web-k-cosmetics/src/pages/admin/KCosmeticsAdminDashboard.tsx` | — |
