...
- **공통 헬퍼 additive 확장:** `operatorOrderQuery.ts` `serviceKey: string` → **`string | string[]`**.
  ...
  - WHERE `co.metadata->>'serviceKey' = ANY($1::text[])` (단일 string 도 1-요소 배열로 동일 동작 → **KCos 무영향**).
...
- query: `page, limit, status, paymentStatus, search, dateFrom, dateTo` (KCos 와 동일).
...
```
→ KCos `/operator/orders` 와 **동일 shape**. 공통 frontend `OperatorOrderStatusPage` wrapper(`{orders, stats, total}`) 가 `data.orders / data.stats / data.pagination.total` 로 매핑 가능.
...
- `apps/api-server` `tsc --noEmit`: **error 0** (전체). 공통 헬퍼 변경 후에도 KCos 소비처 포함 0 → backward-compatible 확인.
...
## 11. KCos/KPA store 주문 화면 회귀 없음 확인

- KCos `/operator/orders`: 헬퍼 string 인자 경로 불변(ANY(1-요소)) → 동작/shape 동일. api-server 전체 typecheck 0 으로 컴파일 회귀 없음.
...
| 항목 | 결과 |
|---|---|
| 사전 git 상태 | 동시 세션 WIP 존재 — 미접촉 |
| endpoint | `GET /api/v1/kpa/operator/orders` |
| auth | `authenticate` + `requireKpaScope('kpa:operator')` |
| response shape | KCos 동일(`{success, data:{orders,stats,pagination}}`) |
| serviceKey scope | `IN ('kpa-society','kpa')` (ANY 배열) — KPA 격리 |
| view-only / mutation | 유지 / 추가 0 |
| DB / migration | 무변경 |
| TypeScript | api-server error 0 |
| KCos 회귀 | 없음(additive, typecheck 0) |

*Date: 2026-06-16 · KPA operator orders backend enable · GET /kpa/operator/orders (view-only, kpa:operator) · 공통 operatorOrderQuery 헬퍼 serviceKey 배열 지원 additive(KCos 불변) · serviceKey IN (kpa-society,kpa) · DB/mutation 무변경 · typecheck PASS · 후속: frontend wiring WO.*
