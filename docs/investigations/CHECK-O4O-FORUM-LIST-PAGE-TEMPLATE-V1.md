...
> **유형:** forum list page 공통화 — shared `ForumListTemplate` 생성 + KCos/Neture 적용
> **결과: PASS — `ForumListTemplate`(presentational) 신설, KCos/Neture 2서비스 list 적용(서비스 전이므로 Neture inline→Tailwind 시각 정렬 허용). KPA 미수정(BaseTable 보존). 기능/route/basePath/fetch/filter 무변경.**
...
KCos/Neture forum list 의 반복 table/loading/empty/error + pagination JSX 를 단일 presentational `ForumListTemplate` 으로 공통화. KPA 는 BaseTable 고유라 제외. backend/route/fetch 무변경.
...
**모두 forum ForumPage.tsx 외 파일** — path-specific 격리.
...
- **props:** posts · pinnedPosts? · currentPage/totalPages/onPageChange · onPostClick(post→page navigate post.routeTo) · loading/error/onRetry · renderEmpty?(slot) · showPostType · renderTypeBadge?(slot) · renderTitleBadge?(slot HOT) · showLikeCount/showCommentCount · pinnedLabel · accentColor.
...
- <HOT> null}` 로 hot 섹션·본 목록 모두 표시. hotPosts 를 pinnedPosts 슬롯으로 전달(amber 강조 동일).
...
- **정적:**
  - KCos/Neture 2서비스 `ForumListTemplate` 사용 확인. inline pagination/table JSX 제거.
  - KPA `ForumListPage.tsx` 미변경 확인.
  - **Neture `routeTo: ${basePath}/post/${slug}` 유지**(line 44) → supplier/partner/user basePath 경계 보존.
  - postType 표시(KCos/Neture true false)·viewCount(테이블 미표시, 정책 유지)·like/comment 컬럼 유지.
  - KCos/Neture server page · type filter 그대로).
- **browser smoke:** **미수행** — dev 서버 미기동 + 인증/서비스 guard. **시각 변경 동반**(특히 Neture inline→Tailwind, 버튼/테이블 골격이 KCos 와 동일 Tailwind 로 정렬) → 배포 후 또는 후속 smoke 권장(CHECK §9). 동작/route/필드는 tsc+정적 보존 확인.
- **무변경:** backend/API/DB/migration/route/menu ✅ · ForumListItem shape ✅ · Forum Detail/Write/Request ✅.

## 8. 완료 판정

**PASS.** `ForumListTemplate` 신설 + KCos/Neture 적용(Neture 시각 정렬 허용), KPA 미수정. fetch/filter/route/basePath/backend 무변경, typecheck(5) 통과. forum list page 공통화 본체 1차 완료.

## 9. 후속 / 잔여

1. **browser smoke**(KCos/Neture `/forum/posts`) — 시각 변경(Neture 정렬·버튼 32px 등) 렌더 확인. 배포 후 권장.
...
3. (정책) KCos/Neture tag 인라인 표시 parity fetch 전략(limit 50→server) — 별도.
4. forum 사용자-facing page 공통화는 KPA 억지 흡수 대신 **write postType 정책 IR** 또는 **detail shared parts** 로 이동 권장.

---

*Date: 2026-06-12 · WO-O4O-FORUM-LIST-PAGE-TEMPLATE-V1 · ForumListTemplate + KCos/Neture 적용 PASS(Neture 시각 정렬 허용). KPA 미수정. backend/route/fetch 무변경. 시각 변경은 후속 smoke 권장.*
