...
- 수정: 커뮤니티 wrapper 7개 (KCos ForumWritePage / KPA·Neture CommunityManagementPage / KPA OperatorContentHubPage)
...
(직전 census 대비 변화: KPA FC +2 / CO −1 / VD −1 · KCos FC +1 / VD −1 FC +3 / VD −3 · Neture FC +1 / VD −1)
...
- 프로덕션 확인: KPA `/operator/community` `/operator/community`, Neture `/admin/community-admin` 모두 공통 콘솔 렌더 (§12).
...
KCos wrapper 는 `POST_TYPES` · `SHELL_LABELS` · fetch/submit 핸들러만 보유하고 셸/폼은 공통 컴포넌트
...
| B · 서비스 성격상 선택적 미채택 | 76 | Neture LMS 전 축(E1~E13, 공급자·파트너 협업 플랫폼에 교육 과정 없음) · KCos 의 폐쇄형 커뮤니티 축 · G축 활동/신청 대다수. |
| C · 실제 누락·결함 의심 | **0** | 없음. |
...
- 회귀 테스트로 실증: `lms-crossservice-read-write-boundary.spec.ts` · `community-forum-orphan-write-guard.spec.ts` · `community-crossservice-my-posts-contract.spec.ts` 전부 PASS.
- 프로덕션 교차 확인: KPA `/operator/docs` = 6건 `/operator/docs` = 0건, KPA `/lms` = 5건 vs KCos `/lms` = 0건 → **cross-service mixing 0**.
...
| 4서비스 frontend typecheck (`@o4o/web-kpa-society` · `@o4o/web-k-cosmetics` · `pharmacy-hub-web` · `@o4o/web-neture`) | ✅ error 0 |
...
- B21: KPA `CORE_ONLY → FULLY_COMMON`, Neture `VIEW_DUPLICATED → FULLY_COMMON`
- C7: KPA `VIEW_DUPLICATED → FULLY_COMMON`
- B8: KCos `VIEW_DUPLICATED → FULLY_COMMON`
