...
O4O 3개 서비스(KPA-Society, K-Cosmetics, Neture)의 **매장 허브(Store Hub)** 영역 UI-UX 표준화 현황을 조사한다. Operator/Admin·My Store 영역은 본 IR 범위 외(별도 채팅방/IR).

**핵심 결론**: Store Hub는 이미 **Cycle 1 정렬 완료**로 기능·copy API·사용자 표현이 통일됨. 남은 표준화 여지는 **Hub Layout 프레임 복제**(K-Cos) 1건이며, 우선순위는 낮다.
...
| 계층 | 역할 | 표준화 도구 | 상태 |
|------|------|-----------|:---:|
| **L1 Hub Layout** | 사이드바/메뉴 프레임 + Outlet | (서비스별 자체) | ⚠️ K-Cos 복제 |
...
| POP | ✅ | ✅ |
| QR | ✅ "QR-code" | ✅ "QR 코드" |
| 이벤트/특가 | ✅ "이벤트/특가" | ✅ "캠페인·이벤트" |

**발견**:
- 라벨 일부(이벤트/특가 vs 캠페인·이벤트)만 다름 — **구현 편차(복제)**.
- KPA는 `MENU[]` config array 방식 + sidebar 구조가 달라 별도 유지 — **부분 도메인 차이**.
- 공통 `StoreHubLayout` 컴포넌트는 **존재하지 않음** — K-Cos가 NavLink/Outlet 프레임을 각자 복제.
...
**유일한 미공통 영역**: L1 Hub Layout (NavLink/Outlet 프레임). K-Cos가 거의 동일 코드를 각자 보유.
...
| 항목 | 현황 | 가능성 | 우선순위 |
|------|------|:---:|:---:|
| L1 Hub Layout 공통화 (K-Cos) | NavLink/Outlet 프레임 ~99% 복제 | ✅ config-driven 추출 가능 | 낮음 |
...
| 항목 | 위험 이유 |
|------|---------|
| KPA PharmacyHubLayout 통합 | MENU config array + sidebar 구조가 K-Cos의 NavLink 프레임과 다름. 강제 통합 시 KPA 회귀 위험 |
...
| 낮음 | L1 Hub Layout 공통화 (K-Cos) | 선택적 — 유지보수 빈도 높아지면 진행 |
...
| WO | 범위 | 위험도 | 우선순위 |
|----|------|:---:|:---:|
| `WO-O4O-STORE-HUB-LAYOUT-TEMPLATE-V1` | K-Cos HubLayout → 공통 StoreHubLayout 추출 | 낮음 | 낮음 (선택) |
...
| **서비스별 차이 = 도메인 차이 vs 구현 편차** | KPA Layout sidebar 구조 = **부분 도메인 차이**. Neture 워크스페이스 = **도메인 차이**(제외 정당). | ✅ 명확히 구분 |
...
| **공통화 필요 영역 vs 서비스별 예외** | 공통화 필요: L1 Layout(K-Cos, 선택적). 예외 유지: KPA Layout(sidebar 차이), Neture(워크스페이스), 서비스별 hero 문구/색상. | ✅ |
...
2. 남은 표준화 여지는 L1 Hub Layout 복제(K-Cos) 1건 — **구현 편차이나 우선순위 낮음**.
...
| `services/web-k-cosmetics/src/components/layouts/KCosmeticsHubLayout.tsx` | — |
