...
> **판정:** **PASS** (Phase 1 — 회원 작성 form shell 추출. KCos 무변경. list/detail/search 는 Phase 2 분리)

---

## 1. 작업 개요

KPA `/content` 회원 작성형 콘텐츠 구조 중 **가장 핵심이자 가장 self-contained 한 작성 폼**을 service-neutral 순수 form shell `CommunityContentWriteShell` 로 추출(`@o4o/shared-space-ui`). **KPA 만 적용, KCos route/화면 무변경.** `InstructorCourseFormShell`(LMS) 선례와 동일 패턴(API client 미 import, onSubmit 주입, 서비스 고유=slot).
...
**무변경:** backend / DB / migration / API / route / menu / KCos / Neture / package.json / pnpm-lock / Dockerfile / 기타 KPA 페이지.
...
| KCos/Neture | shell 미 import → 영향 없음(추가-only export) |
...
| `WO-O4O-KCOS-CONTENT-STANDARD-ROUTE-ALIGNMENT-V1` | KCos 에 표준 모듈 적용(제품 go 후) — 본 shell 위에 |
...
| 다음 | Phase 2(list/detail/search) → route 역할 재정의 → KCos 적용 |
