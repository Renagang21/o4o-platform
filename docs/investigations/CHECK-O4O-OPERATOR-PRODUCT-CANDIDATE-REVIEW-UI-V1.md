...
### 10.1 Live smoke (2026-06-06, 재시도 완료)

브라우저 lock(이전 세션 MCP 자동화 chrome) 해제 후 sohae2100 로그인 → 라이브 수행. (계정: sohae2100 = platform:super_admin)

| 항목 | 결과 |
|---|---|
| CI 배포 (Deploy Web Services, Phase 5 `dbb624184` + 활용연결 `443a3ee68`) | ✅ completed/success |
| 로그인 → `/operator/product-candidates` 진입 | ✅ |
| 사이드바 메뉴 `Products > 상품 후보 검토` 노출 | ✅ |
| 페이지 렌더 (헤더 + GuideBlock 4단계 + 통계카드 + DataTable) | ✅ |
| 필터 탭 (전체/대기/검토중/매칭됨/**활용연결**/반려/보관) + 매칭 select + 검색 | ✅ ('linked' 탭 노출) |
| empty state "검토할 상품 후보가 없습니다" | ✅ |
| API guard: `GET …/product-candidates` (무scope, platform admin) | ✅ HTTP 400 `PLATFORM_ADMIN_SCOPE_REQUIRED` — **F6 Boundary Policy 설계대로**(서비스 operator 는 service-scoped 200). 페이지는 graceful empty 처리 |
| API scope: `GET …?all=true` (cross-service opt-in) | ✅ HTTP 200 `{items:[],total:0}` — end-to-end 정상 |
| API 액션 end-to-end: POST create → 201, POST `/:id/archive` → 200(status=archived), `?status=archived` 필터 total=1 | ✅ (disposable [SMOKE] 후보 1건 생성→archive 정리) |
| 상세 모달 / "활용 상품으로 추가" 액션 **시각** 검증 | ⏸ 미수행 — 사용 가능 테스트 계정이 전부 platform:super_admin 이라 page list(무scope)가 400→빈목록 → 행 클릭 불가. service-scoped `neture:operator` 계정 부재(문서 gotcha). 모달/액션은 tsc 통과 + 동일 패턴(기존 페이지) + API 액션 검증으로 갈음 |

> **판정: PASS** (렌더·메뉴·필터·empty·API guard/scope/액션 end-to-end 확인). 상세 모달 육안만 계정 제약으로 보류.
>
> 발견(follow-up): platform:super_admin 으로 페이지 진입 시 list 가 무scope 400 → silent empty. service operator 는 정상. 필요 시 페이지에서 platform admin 에 한해 `all=true` 또는 service 선택 UI 노출 검토(ProductConsoleController 와 동일 UX 정합). [[project_sohae2100_platform_super_admin_smoke_gotcha]]
