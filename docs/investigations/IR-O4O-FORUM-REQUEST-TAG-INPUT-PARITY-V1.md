...
> **목적:** 포럼 개설 신청의 `tags` 입력/전송 정책이 3서비스(KPA/KCos/Neture)에 일관되게 적용되는지 확인하고, KCos/Neture 신청 실패 가능성을 확정 + 후속 정책 방향 권고.
...
**핵심 결론:** **CONFIRMED — KCos/Neture 포럼 개설 신청은 현재 `400 TAGS_REQUIRED`로 거부된다(live 기능 결함).** canonical backend `ForumRequestService.create`는 `tags`를 **모든 serviceCode에 hard-required**로 강제하나, **tag 입력 UI·payload·client type을 구현한 서비스는 KPA뿐**이다. KCos/Neture 신청 폼은 `{name, description, reason}`만 전송하고 3개 route 모두 live이므로, 사용자가 신청을 제출하면 반드시 실패한다.
...
**다른 세션 WIP(미접촉):** Footer/legal 작업, CHECK-...-ORDER-VIEW-LOOP M, untracked 다수. 본 IR 은 신규 문서 1건만 생성.
...
- **확정(정적):** KCos/Neture 신청 폼 submit → `{name,description,reason}` (tags 부재) → route handler 가 body.tags(undefined)를 service.create 에 전달 → `!tags` 참 → **`400 TAGS_REQUIRED`**. route layer 에 tags 기본 주입 없음.
- **3개 route 모두 live**(App.tsx 마운트 확인). 따라서 비-KPA 사용자가 포럼 개설 신청을 시도하면 **항상 실패**.
- **E(타입 불일치):** KCos/Neture client type 이 tags 를 아예 누락 → 컴파일도 막지 못함(backend 계약과 frontend 타입 drift).
...
| 옵션 | 내용 | 장점 | 단점 |
|------|------|------|------|
| **A** | tags 필수 유지 + KCos/Neture frontend 에 tag input 추가(KPA 패턴 재사용) | 3서비스 정책 일관·tag 중심 canonical 유지·검색/분류 품질·KPA parity | KCos/Neture UI 작업 3건·Neture 강제감 |
| **B** | backend tags optional 화(create 검증 완화) | 실패 즉시 제거·UI 변경 최소·Neture 축소 유리 | tag 중심 정책 약화·검색/분류 품질 저하·서비스 경험 편차 고착 |
| **C** | service 별 정책(예: KPA/KCos 필수, Neture optional) | 서비스 정체성 반영 | backend validation serviceCode 분기 복잡·1인 유지보수성 저하 |

## 9. 권장안

**기본 권장: Option A (tags 필수 유지 + KCos/Neture frontend tag input 추가).**
...
**즉시성 주의:** 2서비스 신청이 실제 거부 상태이므로, A 채택 시에도 **KCos 우선 unbreak**(tag input 추가) → Neture 는 forum 범위 결정과 연동.

## 10. 즉시 WO 가능한 후보

1. **`WO-O4O-FORUM-REQUEST-TAG-INPUT-PARITY-V1`** (Option A) — KCos(+조건부 Neture) RequestCategoryPage 에 tag input UI 추가 + client type/payload 에 `tags` 추가(KPA forumMembership/forum.ts 패턴 이식). frontend-only(+client type), backend 무변경. 2개 서비스 신청 unbreak.
...
| 점검 | 결과 |
|------|------|
| forum 신청이 "포럼+태그" 기준으로 이해되는가 | ⚠️ KPA 만 그렇고 KCos/Neture 는 tag 누락 → 정책-구현 불일치 |
...
| KCos 에 분류 품질 위해 tag 필요한가 | ✅ community 서비스 — 검색/분류에 유효 |
| Neture 는 forum/tag 축소가 적절한가 | ⚠️ supplier 정체성상 가능 → §9 별도 결정(D) |
| backend validation 이 사용자 화면과 일치하는가 | ❌ 불일치 — KCos/Neture 화면엔 tag 입력 없는데 backend 는 필수 |
| 없는 입력을 요구해 신청을 실패시키지 않는가 | ❌ 현재 실패시킴 — 본 IR 의 핵심 결함 |
| 공통화가 1인 유지보수성을 높이는가 | ✅ A(공통 tag input 패턴 이식) 또는 B(단일 정책) 모두 일관화. C(분기)만 저해 |

**종합:** backend 는 tags 필수(canonical), frontend 는 KPA 만 반영 → **KCos/Neture 포럼 신청 live 실패 확정**. 권장은 **Option A**(tag 중심 정책 유지 + 2서비스 tag input 정합), Neture 는 forum 범위 정책과 연동. backend 완화(B)는 정책 약화로 비권장.
...
- **tag 입력 UI:** KPA ✅ / KCos·Neture ❌
- **tags payload 전송:** KPA ✅ / KCos·Neture ❌ (client type 에도 tags 부재 — drift)
- **runtime 신청 실패:** **확정** — KCos/Neture(3개 route live) 신청 시 `400 TAGS_REQUIRED`
- **권장 정책:** **Option A** (tags 필수 유지 + KCos frontend tag input 추가, Neture 는 forum 범위 결정과 연동). backend optional 화(B)는 비권장.
