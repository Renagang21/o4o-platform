...
**PASS** (구현·정적검증·배포 후 라이브 보안 curl 완료 — 무인증 5개 엔드포인트 전부 401 차단 확인).
...
### 배포 후 결과 (라이브 curl, 2026-06-06 / rev 27053777495 success)
무인증 요청 → 전부 **401** (baseline 대비 차단 확인):
- `GET /api/trial-shipping/<uuid>` → **401** (이전 404)
- `POST /api/trial-shipping/<uuid>` → **401**
- `GET /api/trial-fulfillment/<uuid>` → **401** (이전 404)
- `POST /api/trial-fulfillment/<uuid>/create-order` → **401**
- `GET /api/trial-fulfillment/stats` → **401** (이전 **200 노출**)
- 잘못된 토큰(`Bearer invalid…`) → **401**

→ **무인증 PII/이행/통계 노출 차단 확인 (핵심 취약점 해소).**

소유권/운영자 분기(403/200)는 동일 `requireOwnerOrOperator`/operator-scope 미들웨어 로직 + tsc 검증으로 보증(인증 토큰+실 participation 필요한 라이브 케이스는 미수행):
- 인증 O · 소유자/운영자 X → operator-scope 가 403
- 본인 participation(`participantId===user.id`) → 통과(200)
- 운영자 스코프 보유 → 운영 엔드포인트 200, 비운영자 → 403
...
*상태: 완료 (구현·정적검증·라이브 보안 curl PASS)*
