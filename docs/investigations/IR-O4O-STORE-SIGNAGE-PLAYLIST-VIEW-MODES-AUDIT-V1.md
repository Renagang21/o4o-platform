...
> **목적:** KPA/KCos "내 매장" 디지털사이니지 플레이리스트의 **보기 모드**(일반 관리 보기 vs 실제 매장 송출 보기) 분리 현황을 확정한다.
> **작성:** 2026-06-13
...
> **2-view 구조는 KPA/KCos 2서비스에 이미 존재한다.** "관리 보기 → player 선택 → chrome-free 전체화면 송출(새 탭)" 3단 패턴이 동일하게 구현되어 있다.
>
> 따라서 후속 WO 는 **"2-view 신규 구축"이 아니라 "격리 방식 정규화 + 공개 송출 경로 정리 + 깨진 CTA cleanup + 용어 정렬"** 로 좁혀야 한다. 본 IR 은 그 차이(drift)를 확정한다.

## 1. 조사 개요

각 서비스 "내 매장" signage 의 보기 모드를 (A) 일반 관리 보기(layout chrome 포함) vs (B) 실제 송출 보기(헤더/사이드바/버튼 없는 전체화면) 기준으로 조사. 3개 read-only 탐색(KPA / KCos).
...
> 관리/송출 route 골격은 **2서비스 동일**. 차이: 공개(무인증) 송출은 **KPA만 보유** 는 추가 `/library`·`/preview`(stub) 보유.
...
> KPA/KCos 의 `/store/.../play` 는 layout wrapper **안**에 있고 `fixed inset-0 z-50` 로 덮어 chrome 을 가림 — 시각적으로는 동일하나 store layout 이 뒤에 mount 됨(불필요 마운트). KPA 는 추가로 `/public/signage`·`/signage/play/*` 를 route-격리로 보유.
...
| KPA SignageFullscreenPlayerPage (`/signage/play/*`) | 커뮤니티 전체화면 재생 |
 SignagePreviewPage (`/preview`) | **stub("준비 중")** — dead/미완 |
...
| **실제 송출 보기** | `/store/marketing/signage/play/:playlistId` | — | full-screen canvas + 최초 전체화면 안내 overlay만. header/sidebar/footer/탭/관리버튼 금지 |
| **공개 송출(선택)** | `/public/signage?playlist=:id` | 무인증, layout 밖 | 키오스크/TV URL 재생 |

단 기능상 이미 chrome-free 이므로 **우선순위는 낮음**.
...
| stub | `/preview` 준비중 | — preview 완성/제거 결정 |
...
2. **`WO-O4O-DIGITAL-SIGNAGE-CROSSSERVICE-APPLY-V1`** (KCos 확산) — baseline 용어 + KCos 깨진 route cleanup + (선택) 송출 route 격리.
3. **(선택) `IR-O4O-STORE-SIGNAGE-PUBLIC-DISPLAY-POLICY-V1`** — 실제 TV 무로그인 송출(공개 URL) 을 KCos 에도 둘지(KPA `/public/signage` 패턴) 정책 결정.
...
| 실제 매장 TV 무로그인 송출? | ⚠️ KPA만(`/public/signage`). KCos 는 store 인증 필요 — 물리 TV 시나리오 정책 결정 필요 |
| device/screen 엔티티? | ❌ 없음 — "화면"=브라우저 전체화면. device 관리 미구현(과장 금지) |
| KPA 고유 강제? | 주의 — 공개 송출/커뮤니티 재생은 KPA 고유, KCos 강제 전 정책 확인 |
| 1인 유지보수성 | — |
