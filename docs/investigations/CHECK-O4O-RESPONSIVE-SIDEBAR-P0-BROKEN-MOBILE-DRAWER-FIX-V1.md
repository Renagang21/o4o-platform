...
> **목적**: 모바일/태블릿에서 실제로 깨지는 왼쪽 사이드바 영역(store-hub KCos / Neture supplier)을 우선 정상화. 전체 표준화는 후속 WO.
> **작성일**: 2026-06-17
...
| 영역 | desktop(>=lg) | <lg | 비고 |
|------|---------------|-----|------|
| KCos store-hub | `lg:sticky lg:top-20 lg:w-52` (기존 sticky 유지) | drawer w-72 | 동일 aside 가 mobile=fixed drawer / desktop=sticky 로 전환 (lg:* 오버라이드) |
...
> KCos: `-translate-x-full` 기본 + `lg:translate-x-0`(미디어쿼리 우선) 으로 desktop 항상 표시. 본문 `flex-1 min-w-0` — 고정 margin 없음 → 모바일 본문 가림/가로스크롤 없음.
...
| 항목 | 결과 |
|------|------|
 typecheck (`tsc -p tsconfig.app.json`) | ✅ PASS (에러 0) |
...
- store-hub , /store-hub/content : <1024 hamburger→drawer open, 메뉴선택 close, >=1024 사이드바 고정
...
- KCos admin DashboardLayout lg:ml-64 본문 margin 제거 + 복제본 공통화
