...
**완료 인계**: 모IR의 핵심 권고(K-Cos bulk parity)는 이미 구현·검증 완료 — `WO-...FORUM-DELETE-REQUEST-BULK-PARITY-V1`(`16a76fb6e`), `WO-...FORUM-REQUEST-BULK-PARITY-V1`(`bb52b6819`), smoke PASS `CHECK-...FORUM-BULK-PARITY-SMOKE-V1`(`f2c772945`)

---

## 0. 이 IR이 답하는 질문

모IR(`...AUDIT-V1`)은 "K-Cos에 bulk action이 누락됨"을 지적했고, 그 후속 WO가 실행되어 **bulk parity는 해소**되었다. 모IR §7은 그다음 단계로 **공통 wrapper 추출 타당성**을 별도 IR로 보류했다.
...
모IR 작성 시점 K-Cos는 "단건 처리(selectable/ActionBar/bulk 없음)"였다. **현재는 보강 완료**:

| 서비스 | 화면 | DataTable | selectable | ActionBar | BulkResultModal | bulk 제외 |
|--------|------|:---:|:---:|:---:|:---:|------|
| ForumRequestsPage | ✅ | ✅ | ✅ | ✅ | 보완(revision) bulk 제외 |
...
| 서비스 | 신청/삭제요청 bulk |
|--------|-------------------|
| K-Cos | **fan-out** `Promise.allSettled(ids.map(single))` (backend batch endpoint 없음) |
...
| 단계 | 범위 | 타당성 | 위험 |
|:---:|------|:---:|:---:|
| 1 | **ForumDeleteRequestsConsole (K-Cos)** | 높음 (95%+ 동일, 승인/거절만 — 위험 action 없음) | **낮음** |
| 2 | **ForumRequestsConsole (K-Cos)** | 높음 (95%+ 동일) | 낮음~중간 (보완 bulk 제외 정확 계승 필요) |
...
| 편차 | 현황 | canonical 제안 |
|------|------|----------------|
| 상태필터 UI 신청=select / 삭제요청=탭그룹, K-Cos=개별 버튼 | 탭그룹으로 통일 |
| error 상태 UI=AlertCircle+retry, **K-Cos 누락** | 콘솔에 표준 내장 |
| status badge / header=`StatusBadge`/`PageHeader`, K-Cos=인라인 | 공통 컴포넌트 사용 |
| detail 진입 | K-Cos/KPA=Drawer, Neture=Modal | `BaseDetailDrawer` canonical |
...
| WO/IR | 범위 | 위험 | 우선 |
|-------|------|:---:|:---:|
| `WO-O4O-OPERATOR-FORUM-DELETE-REQUESTS-CONSOLE-COMMONIZATION-V1` | `@o4o/operator-core-ui`에 `ForumDeleteRequestsConsolePage` 추출 + K-Cos thin wrapper + 주입형 client. error UI·탭그룹 canonical 포함. backend 변경 없음 | 낮음 | 🥇 |
...
- **서비스 차이가 도메인 차이인가 구현 편차인가?** 도메인 차이(보존): KPA 2탭 결합·카테고리 관리·Neture batch endpoint·detail Modal. 구현 편차(공통화 대상): K-Cos status badge·header·error UI·상태필터 형태.
...
**결론**: 철학 충돌 없음. bulk parity로 이미 수렴된 K-Cos를 **삭제요청 콘솔부터 단계적으로 추출**하는 것이 위험 대비 실익이 가장 크다. Neture·KPA는 별도 IR로 신중 진행.
