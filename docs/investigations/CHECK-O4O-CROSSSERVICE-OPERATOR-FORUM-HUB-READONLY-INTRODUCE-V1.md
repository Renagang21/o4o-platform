...
> KPA Forum 운영 hub 를 공통 모듈로 추출, K-Cosmetics 에 **read-only** hub 도입.
...
- KCos `getPosts` = `fetchForumPosts({limit}).then(r => ({ data: r.data }))` (community `/forum/posts` read, optionalAuth).
- KCos `client.deletePost` 미주입 → 공통 모듈이 `canDelete=false` 로 액션/선택/일괄삭제 전부 비노출.

---

## 4. KCos read-only 제한 내용
...
- KCos: `enablePostActions` 미설정(false) + `deletePost` adapter 미주입 → 단건/일괄 삭제·수정 액션 코드 경로 비활성.
- 공통 모듈 `canDelete = enablePostActions && typeof client.deletePost === 'function'` → KCos 양쪽 false.
- KPA 만 true (기존 동작 보존).
...
- KCos `/operator/forum` → 실제 page 렌더(빌드 PASS).
- KPA hard-nav 그대로(forum-management 는 KPA redirect 존재).
- `/operator/forum-management` 등 KPA-only 경로를 KCos 에 남기지 않음(KCos 는 forum-requests 주입).
...
- mutation 추가 없음(KCos). Neture 미접촉. 다른 세션 WIP 미접촉.
...
- 배포 후 권장: KPA `/operator/forum`(기존 동작·삭제 유지), KCos `/operator/forum`(hub 렌더, 삭제 액션 없음), 2서비스 Forum 메뉴 펼침 → 포럼 운영/신청 관리/삭제 요청/분석 표시, console/pageerror/4xx 없음.

---

## 14. 후속 WO 후보

1. **WO-...-FORUM-CATEGORIES-INTRODUCE-V1** — 포럼 목록 관리(비활성/영구삭제) KCos 도입. **영구삭제 정책 명시 승인 선행** (API 는 이미 ready — IR feasibility §4).
2. (옵션) KCos hub 의 게시글 moderation 을 operator-safe 경로(`/forum/operator/*`)로 제공할지 정책 검토.
...
- ✅ KCos `/operator/forum` read-only hub 도입 (redirect → 실 hub 승격)
- ✅ KCos 삭제/일괄삭제/수정 액션 미노출
- ✅ KCos Forum 메뉴 `포럼 운영` 추가 (canonical 4항목)
