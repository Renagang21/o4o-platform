...
| **의약품 제품 허가정보(완제의약품 허가상세)** | ✅ **원료약품 및 그 분량**(MATERIAL_NAME) = 유효성분 + **첨가제** |

> **아스파탐·대두유·유당·색소는 「원료약품 및 그 분량」에 성분명으로 명시**된다(예: 아스파탐, 대두유, 유당수화물, 황색4호·5호). 이것이 유일한 공식 첨가제 원천이다.
...
| 허가사항 문서 | **EE_DOC**(효능효과) · **UD_DOC**(용법용량) · **NB_DOC**(사용상주의사항) — 전문 원문(임상 표기 "10 mL/min **미만**" 등, `<` 기호 회피) |
| 첨가제 | **MATERIAL_NAME**(원료약품 및 그 분량) |
