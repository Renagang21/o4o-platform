...
> **결론(요약): 판정 B(부분 정합) + D(서비스 drift) 주.** QR 은 POP 과 달리 **이미 영속 재편집 엔티티**(`store_qr_codes`, PUT 수정 가능)이며 운영자 QR 템플릿(`operator_qr_templates`) → HUB → import(`/stores:slug/qr/staff/import` → store_qr_codes 사본) 흐름, production-materials 표시(getStoreQrCodes), 이미지(PNG/SVG)·인쇄 PDF·전단 PDF·스캔 통계까지 backend 2서비스 마운트 — **"단순 QR 목록(C)" 아님**. 그러나 **프론트가 KPA 편중**: KPA 는 생성/편집/AI/통계/POP-QR selector/상품 prefill/일괄인쇄 보유, **KCos 는 생성·삭제·import·production-materials 표시만**(편집 폼 ✗ · POP-QR selector ✗ · AI/통계 ✗) → **D drift**. cross-content 연결은 **POP embed(qrId, KPA 한정)** 뿐 — blog/signage/snapshot·direct origin 연결 없음(B).
...
| 컨트롤러 | 엔드포인트 | mount |
|----------|-----------|-------|
| `store-qr-landing.controller.ts` | `/pharmacy/qr`(CRUD) ·`/source/products` ·`/:id/image | analytics | flyer` ·`/print` ·공개 `/qr/public/:slug` | kpa:402:404 · cosmetics:168 |
| `operator-qr.controller.ts` | `/operator/qr/templates`(CRUD+publish/archive) | kpa:461:455 · cosmetics:197 |
| `qr.controller.ts`(staff) | `/stores/:slug/qr/staff/import`(operator template→store_qr_codes) | kpa:471:473 · cosmetics:206 |

### 3.3 frontend (서비스별 차이 큼)
| 화면 | route | KPA / KCos |
|------|-------|------|
| 매장 QR | `/store/marketing/qr` | — |
| HUB QR import | `/store-hub/qr` | HubQrLibraryPage (3서비스) |
| client | qrStaff.ts / storeQr.ts(KPA) · qrFetch inline(KCos) | — |
...
| **5.3 편집 단계** | store_qr_codes title/description/landingType/landingTargetId/slug **편집 가능(PUT)**. **KPA 편집 폼 O, KCos 편집 폼 ✗**(생성·삭제만). KPA AI 제목/설명 보조 O |
| **5.4 출력/저장** | `store_qr_codes` 영속(재편집) + production-materials 표시 ✅. 출력: 이미지/인쇄PDF/전단PDF(KPA rich, KCos PNG 다운로드). QR 은 **POP PDF 와 달리 영속 엔티티라 "콘텐츠 저장" 내재 충족** |
| **5.5 parity** | §5 표 — KPA full, KCos thin(D) |
...
> backend 는 2서비스 완비. **프론트 격차(편집·POP-QR·AI·통계)가 KPA↔KCos drift(D)**. canonical(MENU-TREE §2.1): QR "기존 구현 KPA 단독 90%, 후속 W5" — 일치.
...
| **D** 서비스 drift | **주** | KPA full vs KCos thin(편집·POP-QR·AI·통계 미흡) |
...
- **QR: store_qr_codes 자체가 영속·재편집 엔티티 → "콘텐츠 저장" 내재.** 따라서 QR 의 핵심 gap 은 "저장"이 아니라 **KCos 편집·연결 parity(D)** 와 **cross-content 연결(B)**.

## 8. 후속 WO 후보

1. **`WO-O4O-QR-EDITOR-GP-KCOS-PARITY-V1`**(D 핵심) — KCos StoreQrPage 에 **편집 폼**(title/description/landingType/landingTargetId) 추가. backend PUT `/pharmacy/qr:id` 이미 존재 → frontend only. (POP staff parity WO 동형.)
2. **`WO-O4O-POP-QR-SELECTOR-GP-KCOS-PARITY-V1`**(D) — KCos StorePopPage 에 QR selector(selectedQrId → generate qrId) 추가. KPA 동형, backend 무변경.
...
4. (선택) `WO-O4O-QR-AI-ANALYTICS-GP-KCOS-PARITY-V1` — KCos AI 제목·설명 + 스캔 통계 parity.
5. (선택) `IR-O4O-QR-ADMIN-DASHBOARD-LEGACY-AUDIT-V1` — `QR-CANONICAL-AND-LEGACY-AUDIT` 의 admin-dashboard parallel QR UI dead 의심 확인.

> **권장 순서**: ① KCos QR 편집 폼(D, frontend) → ② KCos POP-QR selector(D) → ③ cross-content 연결(B) / AI·통계 parity.

## 9. 검증 (본 조사)

- **코드/DB/route/UI 변경 0** (read-only). 산출물 = IR 문서 1건. 동시 세션 WIP 미접촉.
- store_qr_codes 편집 가능(PUT) · operator_qr_templates+import 2서비스 · production-materials qr 포함 · POP embed qrId(KPA) · KCos 편집/POP-QR/AI 부재 확인(file:line은 §3·§5).
...
- 핵심 gap 은 **(D) KCos 프론트 parity**(편집 폼·POP-QR selector·AI/통계 부재)와 **(B) cross-content 연결**(POP embed 외 blog/signage/origin 연결 없음).
- **권고**: 저장 구조 변경 불요. ① KCos QR 편집 폼 parity(frontend, backend PUT 기존) → ② POP-QR selector parity → ③ cross-content/AI·통계. 본 조사 범위는 확인까지 — 코드 무변경.

---

*Date: 2026-06-15 · read-only IR · 코드/DB 무변경 · QR = 영속 재편집 엔티티+템플릿 import+production-materials+POP embed (C 아님). 판정 B(cross-content 약함)+D(KCos 편집·POP-QR·AI/통계 parity 미흡). 후속: KCos QR 편집 폼 parity 우선.*
