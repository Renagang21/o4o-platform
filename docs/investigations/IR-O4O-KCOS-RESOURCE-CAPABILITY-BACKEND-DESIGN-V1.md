...
>
> K-Cosmetics 에 KPA 과 동일한 **Operator Resource Capability** 를 도입하기 위한 backend 구조 설계. **Capability 먼저, Wrapper 는 그 다음** 원칙.
...
| 항목 | 권고 |
|---|---|
| 테이블 이름 | `cosmetics_contents` |
| Entity | — |
| Migration | 단일 migration |
| Controller | — |
...
| 의도 | **Generic CMS content** (KPA/K-Cos resource 와 다른 paradigm — slot / pending approval 흐름) |

→ **다른 도메인.** KPA resources 는 `draft/published/private` (3-state, 가시성 중심); CMS 는 `draft/pending/published/archived` (4-state, 승인 흐름 중심). **재사용 비추천**.
...
| Operator scope | `kpa-society:operator` | — | (필요: `cosmetics:operator`) | 권한 정의 이미 존재 |
| Frontend `resourcesApi` | ✓ | ✓ | ❌ | 신규 생성 필요 |
...
| 차원 | A. | B. `cms_contents` 재사용 | C. `hub-content` 활용 | D. 기존 cosmetics 테이블 재사용 |
|---|:---:|:---:|:---:|:---:|
| KPA parity (status 모델 draft/published/private) | ✅ 정확 일치 | ❌ 4-state CMS workflow 다름 | ❌ 상위 layer | ❌ store/order 와 무관 |
| `usage_type` (READ/LINK/DOWNLOAD/COPY) | ✅ 가능 | ❌ cms 에 없음 | ❌ | ❌ |
| `source_type` (manual/upload/external) | ✅ 가능 | ❌ | ❌ | ❌ |
| `reusable_policy` (restricted/platform) | — | ❌ | ❌ | ❌ |
| Operator 관리 (CRUD + 상태 + 삭제) | — | △ CmsContent 에 transition matrix 다름 | ❌ | ❌ |
| Public 조회 (`sub_type=resource` 필터) | — | △ slot 구조 다름 | ❌ | ❌ |
| 향후 AI / Store Execution 확장성 | ✅ blocks(jsonb) + sub_type 으로 확장 가능 | △ slot 구조 의도 다름 | △ HUB publishing layer | ❌ |
| Migration 비용 | — | ❌ 큰 정책 정합 비용 | ❌ 큰 정합 비용 | ❌ 의미 없음 |
| 재작업 위험 | — | ⭐⭐ 높음 (transition 모델 충돌) | ⭐⭐ 매우 높음 | ⭐⭐⭐ 매우 높음 |
...
- KPA 와 paradigm 다름 → frontend commonization 불가
...
| Route | Permission | 비고 |
|---|---|---|
| `GET /api/v1/cosmetics/contents` (public) | `optionalAuth` | — |
| `GET /api/v1/cosmetics/operator/resources` | `authenticate + requireCosmeticsScope('cosmetics:operator')` | — |
...
KPA `ResourceItem` 신규 K-Cos `KCosResourceItem` 이 모두 동일 shape 이어야 `OperatorResourcesPage` wrapper 가 generic 으로 작동.
...
interface ResourceItem {
  ...
  like_count: number; / 만 (KPA 는 0 으로 default 가능)
...
**KPA 차이:** KPA 는 `like_count` / `view_count` / `author_name` / `updated_by` 컬럼 부재. 해결 옵션:
- (b) KPA backend 도 같은 컬럼 추가 (선행 정합 WO) → 본 IR 범위 외

(b) 는 별건 KPA 정합 WO 의 영역.
...
| 파일 | 역할 | 줄 수 추정 |
|---|---|---:|
| `apps/api-server/src/database/migrations/{ts}-CreateCosmeticsContentsTables.ts` | — | ~60 |
| `apps/api-server/src/routes/cosmetics/entities/cosmetics-content.entity.ts` | — | ~100 |
| `apps/api-server/src/routes/cosmetics/controllers/resources.controller.ts` | — | ~340 |
| `apps/api-server/src/routes/cosmetics/cosmetics.routes.ts` (수정) | — | 5 |
...
| 패턴 `resources.controller.ts` mirror |
...
| 항목 | 내용 |
|---|---|
| 범위 | (a) K-Cos `kCosResourcesApi` + `OperatorResourcesPage.tsx` 신설 + (b) KPA + K-Cos 2 service 가 동일 `OperatorResourcesConsolePage` wrapper 호출 |
| 패턴 | — |
| Backend 변경 | 0 (선행 WO 가 완료 전제) |
| Frontend 변경 | 신규 wrapper + 3 service thin wrapper |
| 회귀 위험 | 낮음 (KPA 는 wrapper 통합만, 동작 동일) |
...
| 서비스별 독립 도메인 | ✅ KPA/K-Cos 각자 contents 테이블 보유 (의도된 분리) | 없음 |
| "같은 capability = 같은 backend 구조" | — | 없음 |
...
| 사업 철학 (KPA = Community Canonical, K-Cos 동등 축) | ✅ 본 IR 의 동일 capability 부여가 사용자 원칙 정합 | 없음 |
...
| 후속 WO 의 전제 조건 충족 | ✅ Frontend commonization WO (Tier 1.3) 의 K-Cos 차단 해소 |
 패턴 = Canonical template 공식 인정 | ✅ 본 IR 명문화 |
...
*Status: 조사 완료 — K-Cos backend gap 확정 canonical template 채택 + 후속 WO 순서 고정.*
