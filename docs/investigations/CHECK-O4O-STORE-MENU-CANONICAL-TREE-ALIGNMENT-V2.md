...
| 항목 | KPA | K-Cosmetics |
| ------ | ----- | ------------- |
| 공급자/거래 상품 라우트 | `/commerce/products` ✅ (PharmacyB2BPage) | ❌ 없음 |
| 거래 신청 라우트 | ❌ (`/requests`=상담요청만) | ❌ 없음 |
| 상품 설명 | `/marketing/product-descriptions` ✅ | `/library/product-descriptions` ✅ |
| 매출/정산 | ❌ 없음 | `/commerce/billing` ✅ |
| 분석 상품성과/콘텐츠·채널성과 | ❌ 없음 | ❌ 없음 |
| 노출 설정 | ❌ 없음 | ❌ 없음 |
| 실기능 보존 대상 | 태블릿·상담요청 | 태블릿 |
...
- 서비스별 실제 노출 차이는 각 config 의 items(라우트 존재분)로 유지. KPA=상품·주문 관리=상품(/management/b2b)·거래 신청·주문 관리, KC=주문 관리.
