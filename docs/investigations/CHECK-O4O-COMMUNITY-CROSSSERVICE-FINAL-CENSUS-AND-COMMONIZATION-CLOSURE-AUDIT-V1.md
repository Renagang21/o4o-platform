...
- **작업일**: 2026-08-20
- **시작 commit**: `d05fd8a28`
- **대상 서비스**: KPA-Society · K-Cosmetics · PharmacyHub · Neture (5)
...
| 축 | 실제 수집물 |
|---|---|
| Frontend route | 각 서비스 `App.tsx`/`routes/*` 의 `path=` 전수 (KPA 225 · KCos 182 238 · PH 53 · Neture 291) |
...
| ID | 기능 단위 | KPA | KCos | PH | Neture | 근거 |
|---|---|---|---|---|---|---|---|
...
| B13 | 내 개설 요청 현황 | FC | NI | FC | NI | NI | KPA 만 화면 보유 |
...
| B21 | 운영자 커뮤니티 관리 콘솔 | CO | NI | VD | NI | VD↔Neture 유사도 0.92(454~489L, 공통 모듈 없음) |
...
| E11 | 강사 강의 관리 | CO | FC | CO | NI | NI | KPA 921L 571L 자체 편집기 |
| E12 | 강사 수강생 관리 | CO | NI | CO | NI | NI | KPA 716L 278L |
...
| H1 | 비즈니스 포럼 | NI | NI | SS | NI | NI | business/forum |
| H2 | 피드백 포럼 | NI | NI | SS | NI | NI | forum/feedback |
...
|---|---|---|---|---|
| B8 포럼 글쓰기/수정 화면 셸 | 유사도 0.84 | 중복 셸 약 250L × 2 | **낮음** — 차이는 라벨 언어 · 상세 route(`/forum/post/:id` vs `/forum/posts/:id`) · api import 경로뿐. `ForumWriteForm` 을 감싸는 `ForumWritePageTemplate` 로 승격 가능 | 약 250L 제거 · 글쓰기 후 이동 경로 결함 재발 차단 |
| B21 운영자 커뮤니티 관리 콘솔 | 유사도 0.92 | 중복 약 420L × 2 | **중간** — 공통 모듈 위임이 전혀 없다. `@o4o/operator-core-ui/modules/*` 선례(members · forum-delete-requests)와 동일한 client adapter 패턴으로 승격 가능 | 약 420L 제거 · KPA(629L, 유사도 0.44~0.46)까지 3서비스 수렴 여지 |
| C7 운영자 콘텐츠 허브 콘솔 | 유사도 0.81 | 중복 약 480L × 2 | **중간** — 공통은 `@o4o/operator-ux-core` DataTable primitive 뿐이고 목록·필터·편집 셸 전체가 복제됐다 | 약 480L 제거 |
...
| B8 글쓰기 셸 (KPA·Neture) | KPA 291L · Neture 569L | 예 — 상호 유사도 0.20. Neture 는 공급자·service-update 대상 선택이 추가된다 | `ACCEPTED_CORE_ONLY` (KCos 승격 시 재검토) |
| B21 운영자 커뮤니티 콘솔 (KPA) | KPA 629L | 아니오 — Neture 공통화 시 함께 수렴 대상 | `MUST_FIX_BEFORE_CLOSE` (D-1 ②에 흡수) |
| C1·C2·C3·C4 콘텐츠 (KPA) | KPA 761L 목록 · 244L 상세 · 178L 작성 | 예 — KPA 콘텐츠는 참여·설문·문서 축이 함께 붙어 KCos 의 단순 목록과 업무가 다르다 | `ACCEPTED_CORE_ONLY` |
...
| E11·E12 강사 강의·수강생 (KPA) | KPA 921L/716L 571L/278L | 예 — 상호 유사도 0.65 미만. KPA 는 퀴즈·과제·AI 구조 생성까지 포함 | `ACCEPTED_CORE_ONLY` |
| F1 MembershipGate (5) | KPA 179L · KCos 101L 101L · Neture 83L · PH 95L | 예 — **명문화된 예외**. `packages/auth-utils/src/membershipGate.ts` 헤더가 "미포함: SERVICE_KEY 상수 / React 컴포넌트(서비스별 UI 차이 유지) / redirect·onboarding CTA(서비스별 정책 유지)" 로 제외 범위를 고정 | `ACCEPTED_CORE_ONLY` |
...
| **B. 서비스 성격상 선택적 미채택** | 74 | Neture LMS 전 축(E1~E13) · PH 커뮤니티 콘텐츠/자료실(C1~C5·D1~D3) · KCos·Neture 퀴즈/과제/수료증 검증(E8~E10) · 타 서비스의 통합 검색 화면(B25) · 최신활동 전체보기(A3) |
| **C. 실제 누락·결함 가능성** | 19 | **G1 포럼 알림 · G2 포럼 추천 · G3 포럼 AI (각 4서비스 = 15)** — backend 는 살아 있는데 프런트 소비가 0 (D-4 DEAD_BACKEND 와 동일 대상) · KCos 내 개설요청 현황(B13) · KCos·Neture·PH 인기 태그(G4 중 4) |
...
| `services/web-k-cosmetics | `saveProductAiContent` | 서비스 내 소비 0 |
...
| `/api/v1/lms/*` | 서비스 사용자 대면 | `resolveLmsServiceScope(req)` — service context 또는 명시적 `serviceKey`. KCos·PH 프런트는 공통 factory `createLmsLearnerClient(http, { serviceKey })` 로 canonical 값을 client 계층에서 주입하고, 서버가 raw 값을 canonical 로 덮어쓴다 | 결함 없음 |
...
| service-prefixed frontend 가 generic unscoped backend 호출 | 해당 없음. KCos·PH 의 `/lms/*` 호출은 generic mount 를 쓰지만 canonical `serviceKey` 를 항상 주입한다 |
...
| 4서비스 frontend tsc | KPA 0 · KCos 0 0 · PH 0 · Neture 0 |
...
- **미구현 route 부재 확인**: PH `/contents`, Neture `/contents`, KCos `/contents` 는 앱 내 404 화면으로 처리된다(HTTP 200 + SPA NotFound). 커뮤니티 콘텐츠 canonical route 는 KCos 에서 `/content` 이며 정상 동작한다. PH 는 커뮤니티 콘텐츠 route 자체가 없음을 확인했다(census C1~C5 = NOT_IMPLEMENTED 와 일치).
...
- 대상: B21 운영자 커뮤니티 관리 콘솔(KPA 629L 489L · Neture 454L) + C7 운영자 콘텐츠 허브 콘솔(KPA 707L 590L)
