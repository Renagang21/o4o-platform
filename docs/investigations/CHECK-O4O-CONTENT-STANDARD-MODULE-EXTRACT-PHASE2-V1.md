...
> **판정:** **PASS** (Phase 2 — 상세 표시 shell + 검색 primitive 추출. list-page shell 은 의도적 보류. KCos 무변경)

---

## 1. 작업 개요

KPA `/content` 의 **상세 표시 구조 + 검색 입력**을 service-neutral 공통 부품으로 추출(`@o4o/shared-space-ui/community/`). Phase 1(write shell)에 이어 detail/search 정렬. **KPA 만 적용, KCos route/화면 무변경, 제품 비종속 유지.**
...
**무변경:** backend / DB / migration / API / route / menu / KCos / Neture / package.json / pnpm-lock / Dockerfile.
...
| 항목 | 처리 | 이유 |
|------|------|------|
| **list-page shell**(ContentDocumentsPage 전체) | **보류**(Phase 3 / KCos 적용 시) | `@o4o/ui` `BaseTable`(이미 공유) + columns(copy-to-store/소유권/restricted) + `BaseDetailDrawer` + bulk ActionBar + mobile 카드 + pagination 강결합. 전체 shell 화는 prop 과다·회귀 리스크 큼, 이득 적음(테이블 공통성은 BaseTable 이 이미 제공). WO §9.2 "무리한 추출보다 회귀 리스크 최소화 우선" 적용 |
...
| KCos/Neture | 부품 미 import → 영향 없음(추가-only export) |
...
| 후보 | 내용 |
|------|------|
| `WO-O4O-CONTENT-STANDARD-MODULE-EXTRACT-PHASE3-LIST-V1` | list-page shell(BaseTable columns/bulk/drawer/mobile/pagination 을 config·slot 으로) — 또는 KCos 적용 시 함께 |
| `IR-O4O-CONTENT-ROUTE-ROLE-REDEFINE-V1` | — |
| `WO-O4O-CONTENT-BROWSE-ROUTE-CLEANUP-V1` `/library/content` alias 제거 + browse 이동 |
| `WO-O4O-KCOS-CONTENT-STANDARD-ROUTE-ALIGNMENT-V1` | KCos 표준 모듈 적용(제품 go 후) — write/detail/search shell 위에 |
...
| 다음 | (선택)Phase 3 list shell → route 역할 재정의 → KCos 적용 |
