...
> **유형:** 페이지 컴포넌트 추출 — KCos `/store/library/production-materials` near-identical UI 공통화. backend/API/DB/route **무변경**.
> **결과: PASS — `@o4o/store-ui-core StoreProductionMaterialsView`(header/CTA/list/AssetRow/empty/derivation modal/styles) 추출. KCos wrapper 는 fetch + service-specific derivations 경로만 보유. 기존 4소스(execution/blog/QR/direct) 표시·정렬·badge·원본보기 조건 유지. typecheck(store-ui-core + KCos + KPA) 0 errors. KCos 각 −194 줄.**
...
선행 작업 후 KCos `StoreProductionMaterialsPage` 는 **doc 2줄 + `asset-derivations` API 경로만 다르고 나머지 byte-identical**(header/CTA/list/AssetRow/styles/modal/fetchAll). → 공통 컴포넌트 추출 적격.
...
- **TypeScript 0 errors:** `store-ui-core` · `web-k-cosmetics` · `web-kpa-society`(store-ui-core 소비처 — additive export, 무영향) **각 0**.
- **정적:**
  - KCos wrapper = fetch + fetchDerivations + `<StoreProductionMaterialsView>` 만. 4소스 merge(`mergeProductionMaterials`) 인자 불변.
...
- **browser smoke:** 미수행 — dev 서버·인증 guard. 마크업/로직 이동(behavior-preserving) + typecheck(4) 로 검증. **배포 후 권장:** KCos `/store/library/production-materials` 렌더(header/CTA/목록/badge/원본보기/empty) + 4소스 표시 + 정렬 유지 확인.

## 4. 중단 기준 점검 (§7)

| 기준 | 해당 | 조치 |
|------|:---:|------|
| KCos 차이 큼 | 아니오 | doc+derivations 경로만 차이 |
...
**PASS.** KCos production-materials UI 를 `StoreProductionMaterialsView`(store-ui-core)로 추출, wrapper 는 fetch+derivations 경로만 보유. 기존 4소스 표시·동작 보존, KPA·backend·DB·route 무변경, typecheck(4) 통과. dedup −388줄.

## 6. 후속

1. (배포 후) KCos `/store/library/production-materials` smoke — 렌더·4소스·정렬·원본보기 확인.
2. (선택) **기능별 제작 정비(POP / QR-code / 블로그 / 상품 설명)** 축 진입 — 기반 정리 완료.

---

*Date: 2026-06-15 · 페이지 컴포넌트 추출 PASS · StoreProductionMaterialsView(store-ui-core) 추출 + KCos thin wrapper(−388줄). 4소스 표시·동작 보존. KPA/backend/DB/route 무변경. typecheck(4) 0. 배포 후 smoke 권장.*
