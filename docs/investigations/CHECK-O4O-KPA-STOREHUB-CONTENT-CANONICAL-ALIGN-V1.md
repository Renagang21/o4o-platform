...
- **browser smoke 미수행** — 배포 후 권장: `/store-hub/content` 제목/설명/6-필터/검색/빈상태/복사 버튼·안내문구 확인, `/store-hub/{blog,pop,qr,signage}` 무영향, console error 0, GP·KCos 무변경.

## 7. 완료 판정

**PASS (typecheck).** KPA `/store-hub/content` 프레이밍·6-필터·용어를 GP·KCos canonical 로 정렬. POP/QR/블로그/사이니지 탭 미추가, backend/route/copy/링크대상/타서비스/package 무변경. browser smoke 는 배포 후.
