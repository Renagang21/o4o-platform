...
| **H. Password Reset → credential 갱신** | ✅ **PASS** | — |
| **★ I. 서비스별 다른 password 실증** | ✅ **PASS (4/4)** | KPA+A:✅ / KPA+B:❌ / B:✅ / A:❌ |
...
**결과:**
- KPA + A: HTTP 200, `accessToken` cookie 발급
- A: HTTP 200, `accessToken` cookie 발급 (동일 user, 동일 memberships)
...
**검증 방법:** Account #2 — KPA 는 password A 은 password B 인 상태에서 4가지 cross-credential 시도
...
| I-3 | B | success | `success: true` | — |
| I-4 | A | INVALID_CREDENTIALS | `success: false, code: INVALID_CREDENTIALS` | — |
...
**결과:**
- 정확한 password: `{"success":false,"code":"ACCOUNT_NOT_ACTIVE","error":"Account is not active"}` — credential 조회 skip → users.password (V1 path) 검증 통과 → status check 에서 정지
...
- 모든 서비스 (KPA / Neture / GlucoseView / K-Cosmetics) 에 대해 production 도메인 검증
