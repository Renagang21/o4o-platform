...
> KPA 전용 operator **포럼 목록 관리(categories)** 를 공통 모듈로 추출, K-Cosmetics 에 도입.
>
...
KCos client 는 KPA `forumOperatorApi` 의 categories subset 과 **동일 구현**(동일 axios `authClient.api`, base `/api/v1`, 동일 path/메서드, serviceCode 만 상이) → 응답/에러(409) shape 동일.
...
## 6. KCos hub shortcut 보정 내용

`forum-hub` 모듈 `ForumHubNav` 에 optional `categories?` 추가 + 조건부 ShortcutCard(`포럼 목록 관리`, List 아이콘).
2서비스 hub wrapper(KPA/KCos)에 `categories: '/operator/forum-categories'` 주입 → hub 에서 목록 관리 진입 가능.
...
- 공통 콘솔: `err?.response?.data?.data?.blockedReasons` 표시(`삭제 불가: ...`). KCos client 가 동일 axios 라 에러 shape 동일 → 메시지 정상.
...
- KCos hub 의 posts delete 도 비활성 그대로(이전 WO read-only 유지).
...
- KCos 는 기존 공통 endpoint 를 serviceCode 만 바꿔 호출(프론트 client 추가). Neture 미접촉. 다른 세션 WIP 미접촉.
...
- 배포 후 권장: KPA/KCos `/operator/forum-categories` 목록/empty 정상, Forum 메뉴 canonical 5항목, hub 에서 포럼 목록 관리 shortcut, 비활성/활성/영구삭제 버튼 표시. **운영 데이터에서 실제 destructive action 실행 금지(버튼 표시만 확인)**, console/4xx 없음.
...
- ✅ KCos `/operator/forum-categories` route/page 도입 (serviceCode client adapter, backend 무변경)
- ✅ KCos Forum 메뉴 canonical 5항목 완성, hub 에 포럼 목록 관리 shortcut
