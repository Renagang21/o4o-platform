...
- **이미 공통화된 1건:** **AppreciationPanel**(`@o4o/shared-space-ui`)을 3서비스 모두 사용(테마만 상이: KPA blue emerald / KCos pink / Neture는 contact 중심). → detail은 이미 공통 부품 1개를 가짐.
- **두 부류로 갈린다:**
  - **KPA·Neture = 풀 인터랙션 detail** (comment 작성·like·edit/delete·ownership 가드).
  - **KCos = read-only detail** (comment **list만**, 작성/like/edit/delete API 자체가 없음 — 의도된 최소 구현).
...
| 항목 | KPA | Neture | K-Cosmetics |
| ------ | ----- | -------- | ------------- |
| detail route | `/forum/post/:id` | `/forum/post/:slug` (+ supplier/partner/workspace basePath) | `/forum/post/:postId` |
| key | **id** | **slug** | id |
| basePath | 없음 | **있음**(`/forum` 기본, supplier/partner 변형) | 없음 |
| edit CTA | `/forum/edit/:id` | `${basePath}/write?edit=${id}` | **없음** |
| back | `/forum` | `/forum`(or basePath) | `/forum` |
| delete 후 redirect | `/forum` | `navigate(basePath)` | **없음** |
...
| API | KPA | Neture | K-Cosmetics |
| ----- | ----- | -------- | ------------- |
| get detail | `getPost(id)` | `fetchForumPostBySlug(slug)` | `fetchForumPostById(postId)` |
| comment **list** | `getComments` | `fetchForumComments` | `fetchForumComments` |
...
> **E 분류(API contract 차이로 보류):** comment **작성/수정/삭제·like·delete**는 KPA·Neture만 존재, KCos는 **API 자체가 없음**. comment **list 렌더**는 3서비스 공통(presentational 가능). 따라서 comment **section은 "list 표시"만 공통화 후보**, **작성/CRUD는 보류(E)**. endpoint는 모두 `/forum/*` 동일 패턴이나 함수·response shape 어댑터 차이 존재.

---

## 6. 렌더 영역 매트릭스

| 영역 | KPA | Neture | KCos | 공통화 분류 |
|------|:--:|:--:|:--:|:--:|:--:|
| title | ✅ | ✅ | ✅ | ✅ | **A** |
...
- **분기(작성/CRUD):**
  ...
  - KCos: **read-only**(작성/수정/삭제 API·폼 없음).
- **판단:** comment **write/CRUD 공통화는 E(보류)**. KCos에 comment 작성을 부여하는 것은 **F(정책 결정)**. 1차에서는 comment **list 표시만** 공통 후보로 두되, 작성 폼/액션은 각 서비스 보존.
...
- KCos: **edit/delete 없음**(read-only, ownership 가드 없음).
- **판단:** ownership 가드 로직(KPA·Neture)이 **거의 동일** → 공통 helper/hook 후보(B). 단 **actions UI 자체는 KCos 부재**라 공통 부품에서는 **슬롯/옵션(C)**, KCos에 강제 금지(F 정책).
...
| `extractTextContent` + `post.body` | `<ContentRenderer html>` |
...
- **comment 공통화:** KCos API 부재(E) + 작성 권한 정책(F) 선결 필요.
...
| 3 | `IR-O4O-FORUM-COMMENT-CRUD-CONTRACT-V1` | E/F | KPA·Neture comment CRUD 계약 + KCos 작성 부여 정책 결정 |
...
| KPA 고유 기능을 다른 서비스에 강제하지 않는가 | **충족.** ClosedForumBlocker·tags·appreciation은 D/슬롯 — KCos 강제 없음 |
...
- **조사 기준 commit:** `d1d1d8405`
- **detail route 매트릭스:** KPA `/forum/post:id` · Neture `/forum/post:slug`(+basePath) `/forum/posts:id` · KCos `/forum/post:postId`
...
- **comment 비교:** KPA(create+delete)·Neture(full CRUD)·KCos(read-only list) → 작성/CRUD는 E 보류
- **action/edit/delete:** KPA·Neture ownership 가드(거의 동일, helper 후보) / KCos 부재(F 정책)
