...
| 활용하기 드롭다운 전체 항목 표시(잘림 없음) | ✅ PASS |
| 테이블 border radius / 헤더 / 행 구분선 시각 회귀 없음 | ✅ PASS |
| 하단(테이블 박스 밖)으로 메뉴 펼침 표시 | ✅ PASS |

### 브라우저 smoke (배포본 `f21d5bff7`, kpa-society.co.kr, Sohae 약국 매장, 2026-06-25)
- 검증용 임시 material 제작자료 1건 생성("드롭다운 검증용 임시 제작자료") → 행에 "활용하기" 드롭다운 확보
- "활용하기" 클릭 → 메뉴 4항목(POP 만들기 / QR-code 만들기 / 블로그 글쓰기 / 사이니지에 추가)이
  테이블 박스(2행) 아래로 **완전히 펼쳐져 표시 — 잘림 없음** (스크린샷 prod-materials-dropdown-fixed.png)
- 테이블 라운드 코너·헤더·행 구분선 시각 회귀 없음
- 검증 후 임시 제작자료 삭제 → 목록 원복(블로그 1행) 완료
- 블로그(kind='blog') 행은 "활용하기"가 아닌 "원본 보기"(모달) → 본 수정 영향 없음
