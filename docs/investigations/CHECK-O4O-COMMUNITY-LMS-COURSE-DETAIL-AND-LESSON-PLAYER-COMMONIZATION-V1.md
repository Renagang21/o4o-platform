...
| --- | --- | --- |
| 강의 상세 route | `/lms/course/:id` | `/lms/course/:id` |
| 강의 상세 page | `pages/lms/LmsCourseDetailPage.tsx` (622L) | `pages/lms/LmsCourseDetailPage.tsx` (314L) |
| 레슨 플레이어 route | `/lms/course/:courseId/lesson/:lessonId` | 동일 |
| 레슨 플레이어 page | `pages/lms/LmsLessonPage.tsx` (1,219L) | `pages/lms/LmsLessonPage.tsx` (841L) |
| course/lesson API client | `api/lms.ts` → `@o4o/lms-client` `createLmsLearnerClient` | 동일 |
| loading / error / empty | 자체 구현 | 자체 구현 |
| 진도 표시 | `CourseProgressBar`(공통 presentational) | 동일 |
| 이전/다음 레슨 | 자체 구현 | 자체 구현 |
| 콘텐츠 타입 렌더 | video/article/quiz/assignment + AI | 동일 |
| 수강 권한 | members-only + 로그인 리다이렉트(state.from) | members-only |
| 완료 처리 | `updateProgress(+metrics)` | `updateProgress` (metrics 없음) |
| 서비스별 CTA/문구/theme | "안내 흐름/단계" 어휘 · KPA blue | 기본 어휘 · pink `#db2777` |
| shared package 소비 | `CourseProgressBar` / `LessonList` 만 (= CORE_ONLY) | 동일 |
...
- **실제 차이(보존 대상)**: 어휘(KPA), accent, 수료증 표면(KPA=별도 화면 `/mypage/certificates`=상세 사이드바 PDF 다운로드 · KCos=미제공), 비로그인 처리(KPA=로그인 페이지 + `state.from`=LoginModal · KCos=로그인 페이지), 감사 포인트 패널 theme/에러 파싱 패턴(KPA `err.message` vs KCos `err.response.data.error`) 만 인라인 플레이어를 상세에 갖고 있던 IA 차이.
- **결함으로 판정한 차이**: KCos `updateProgress` 가 완료 메트릭을 보내지 않아 백엔드 완료 규칙(`WO-O4O-LMS-LESSON-TYPE-COMPLETION-RULES-V1`)에 거부될 수 있었다 → 공통화 과정에서 인자 추가로 정합.
...
| `pages/education/lmsViewAdapter.ts` (199L) | `#16a34a` | 기본 + "수강신청" | 사이드바 PDF 다운로드 카드 `CourseCertificateCard.tsx` (89L) | LoginModal | AppreciationPanel(emerald) + `renderSidebarExtra` |
...
| 5 강의 상세 | VIEW_DUPLICATED | **FULLY_COMMON** |
| 6 레슨 플레이어 | VIEW_DUPLICATED | **FULLY_COMMON** |
