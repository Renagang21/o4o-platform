...
| 항목 | K-Cosmetics | 판정 |
| --- | --- | --- |
| analytics summary (`forumAnalyticsApi`) | ✅ 보유 | OK |
| community 포럼 posts **read** (`/api/v1/{svc}/forum/posts`) | ✅ 존재 (`services/forumApi.ts`) | 읽기 가능 |
| operator 관점 posts **delete** client | ❌ 없음 | 미지원 + 권한 미확정 |
| forum **categories** operator client (getCategories/updateCategory/deactivate/hardDelete) | ❌ 없음 | 프론트 미보유 |
| forum categories operator backend (공통 namespace) | 추정 존재 — 미검증 | 정책 미확정 |

> KCos 의 forum operator client 는 requests/delete-requests/analytics 콘솔의 adapter 형태로만 존재. posts/categories 용 client 는 부재.

---

## 4. 중단 기준 해당 항목

- **#1 (KPA 전용 결합)**: hub 의 최근글/삭제는 community-scoped `forumApi`(KPA `/api/v1/kpa/forum/posts`)에 결합. KCos 프론트 client 부재.
- **#3 (mutation 중심 + 정책 확인 필요)**: 포럼 목록 관리가 **영구 삭제(hardDelete)/비활성** 중심. KCos 에 destructive operator surface 도입은 사업/정책 결정 사항.
- **#4 (route 추가 시 dead link/권한 오류)**: hub 가 `/operator/forum-management`(KPA-only) · `/operator/community`(KCos 부재)로 hard-nav → 그대로 이식 시 dead link. posts delete 는 KCos operator 권한 미확정 → 403 위험.
...
1. hub 의 posts read/delete 가 community-scoped `forumApi` 에 결합 — KCos 프론트 client 부재 + operator delete 권한 미확정(403 위험).
2. categories 가 영구삭제(hardDelete)/비활성 중심 destructive surface — KCos 도입은 정책 결정 선행 필요.
3. hub hard-nav 타깃(`/operator/forum-management`, `/operator/community`)이 KCos 에 부재 — 그대로 이식 시 dead link.
4. backend(공통 categories operator endpoint)의 KCos serviceCode 지원은 **미검증** — 검증 없이 route 노출 시 403/빈화면 위험.
...
1. **WO-...-FORUM-HUB-API-FEASIBILITY-VERIFY-V1** — KCos 의 (a) operator posts delete 권한, (b) 공통 `/api/v1/forum/operator/categories` serviceCode 지원, (c) operator 가 community posts 삭제 가능 여부를 backend/API 레벨로 검증. read-only 검증.
2. **WO-...-FORUM-HUB-READONLY-INTRODUCE-V1** (feasibility PASS 시) — KPA hub 를 공통 모듈화하되 **삭제 mutation 제거 + nav 타깃 parametrize**(forum-management→forum-requests, KCos 는 community shortcut 제외)한 read-only 운영 hub 로 KCos 도입. `/operator/forum` redirect → 실 hub 승격.
3. **WO-...-FORUM-CATEGORIES-INTRODUCE-V1** — 영구삭제/비활성 정책을 KCos 에 대해 명시 승인 후 categories 도입 (별도 사업 결정 선행).

> hub 의 삭제 기능을 KCos 에 그대로 줄지, read-only 로 줄지는 **사업 정책 결정**. 본 WO 범위에서 단독 결정하지 않음.
