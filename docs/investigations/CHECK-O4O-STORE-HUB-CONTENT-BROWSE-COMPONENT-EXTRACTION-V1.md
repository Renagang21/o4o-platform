...
> **유형:** wrapper dedup — KCos `/store-hub/content` near-identical 카드 그리드 공통 추출. 데이터/복사/route 무변경.
> KPA 는 구조 상이로 제외(근거 §4). typecheck(5) 0 errors.
...
`/store-hub/content` browse 는 2서비스 모두 `ContentHubTemplate` 기반. 잔여 중복은 KCos 의 **near-identical 카드 그리드 wrapper**. 데이터 구조/복사 정책/route 변경 없이 이 중복만 제거.
...
| **빌드 baseline** | KCos/KPA `tsc --noEmit` **각 0 errors** (직전 IR-...-INTERRUPTED-BUILD 의 LMS/product-applications in-flight 오류는 소관 세션이 해소함 — 깨끗한 검증 가능) |
...
| item mapping | `cmsToItem`(badge color/isNew/isPinned) | `apiItemToContentHubItem` | — |
| 렌더 | **DefaultTableView**(기본 테이블) | **카드 그리드** | **카드 그리드**(KCosCardGrid) |
| 카드 컴포넌트 | — | — | KCosContentCard |
...
- accent 는 **정적 literal 클래스 맵**(`bg-primary-50`…/`bg-pink-50`…) — 동적 `bg-${x}` 금지. (Tailwind JIT 안전)
...
- **TypeScript 0 errors:** `shared-space-ui` · `web-k-cosmetics` · `web-kpa-society`(미변경) · `web-neture`(shared 소비처) **각 0** (= baseline 0, 신규 오류 0).
- **정적:**
  - KCos `KCosCardGrid`/카드 컴포넌트 **제거**, `contentHubCardGrid('primary'|'pink')` 사용.
  - KCos 잔존 lucide / `ContentHubItemContext` import **0**(카드 외 미사용 확인 후 제거).
...
- **browser smoke:** 미수행 — dev 서버 미기동·인증 guard. 카드는 동일 마크업/클래스 이동(시각 무변경 기대) + tailwind content(shared-space-ui src) 포함 확인.

## 7. 중단 기준 점검 (§13)

| 기준 | 해당 | 조치 |
|------|:---:|------|
| props 과도 | KPA 통합 시 해당 | KPA 제외, KCos 카드 그리드만 추출 |
...
→ KCos 카드 그리드 추출은 저위험 — 진행. KPA fold-in 은 **보류**(구조 상이, 별도 판단).

## 7-보충. 잔여 (선택)

- KCos `apiItemToContentHubItem` adapter(~15줄)도 동일하나 서비스별 `HubContentItemResponse` 타입(다른 import 경로)에 의존 → 공통화 시 응답 타입 공유 필요. 본 WO 범위 밖(소폭, 결합도↑). 후속 판단.

## 8. 완료 판정

**PASS.** KCos `/store-hub/content` near-identical 카드 그리드를 `contentHubCardGrid(accent)` 로 공통화. 서비스 용어/API/복사 정책/accent 보존, KPA·ContentHubTemplate·route·API·DB 무변경, typecheck(5) 통과. wrapper 중복 −182줄.
...
4. (선택) KCos adapter 공통화(응답 타입 공유 전제) / KPA fold-in 재평가.
5. (선택) browser smoke — KCos(pink) 카드 렌더.

---

*Date: 2026-06-15 · WO-O4O-STORE-HUB-CONTENT-BROWSE-COMPONENT-EXTRACTION-V1 · KCos 카드 그리드 → contentHubCardGrid(accent) 공통화 PASS. 용어(약국/매장)·API·복사·accent 보존. KPA 제외(구조 상이). typecheck(5) 0. dedup −182줄.*
