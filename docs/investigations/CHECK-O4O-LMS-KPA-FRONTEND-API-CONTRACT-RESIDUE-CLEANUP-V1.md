...
|---|------|----------------|-------------------|
| 1 | — | `/api/v1/lms/certificates/course/{courseId}` | 없음 → `GET /api/v1/lms/certificates?courseId=` 재사용 |
...
- appreciation 은 설계상 **service-neutral 공용 도메인**이다(`AppreciationService` 주석: `service_point_budgets 완전 무관`, serviceKey 미전달). K-Cosmetics 는 이미 동일한 generic base 를 쓰고 있었고, 이번 변경은 KPA 를 **기존 canonical 계약에 맞춘 것**이라 backend 경계를 새로 넓히지 않는다.
...
배포: `4a0cdecb4` → `Deploy Web Services (Cloud Run)` run `32106480722` **success** (`deploy-kpa-society`). 실제 브라우저(Playwright, 운영 도메인) 관측.
...
- **certificate 는 운영 `lms_certificates` 0건**이므로 발급하지 않고(§11 지침) 배포 산출물의 요청 URL 계약으로 검증했다: KPA 번들 내 `certificates …/download` 문자열 **0**, `kpa/appreciation` **0** 번들 내 `/lms/certificates/course/` **0** · canonical `/lms/certificates` 존재.
- 배포 후 KPA 화면 runtime JS 오류 **0**.
...
1. 감사 목록이 항상 빈 배열로 보인다. 본 WO 범위(3건) 밖이므로 수정하지 않고 보고한다. → 별도 WO 제안.
