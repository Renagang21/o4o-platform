...
### 브라우저 smoke (배포본 `21697fdaa`, kpa-society.co.kr, Sohae 약국 매장 계정, 2026-06-25)
| 항목 | 결과 |
|---|---|
| QR 모달에 "내 매장 자료 / 운영자 콘텐츠 / 블로그" 탭 노출 | ✅ |
| 블로그 탭에 기존 블로그 글 "테스트"(초안) + 상태 배지 노출 | ✅ |
| 블로그 선택 → 생성폼 prefill: 연결 유형=외부 링크, 연결 URL=`https://kpa-society.co.kr/store/sohae-약국/blog/test1` (절대 URL) | ✅ |
| QR 저장 → 목록에 "테스트 · 외부 링크 · /qr/qr-…" 추가 | ✅ |
| 공개 `/qr/:slug` 렌더 → 제목 "테스트" + "바로가기"(link) 버튼 | ✅ |
| 테스트 QR 삭제 → 목록 빈 상태 복귀 (데이터 정리) | ✅ |
| 사이니지 자산 선택 모달 정상(탭 미노출) | ✅ 정적 확인 (opt-in 미전달) |

> 운영자 콘텐츠 탭/자료 탭의 실제 항목 노출은 해당 계정에 데이터가 있을 때 추가 확인 권장(코드 경로·권한은 확인됨).
> 블로그가 draft 라 연결 대상(블로그 본문)은 발행 후 표시 — QR 자체는 정상 생성·렌더됨.
