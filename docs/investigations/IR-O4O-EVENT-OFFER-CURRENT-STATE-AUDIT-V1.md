...
이벤트 오퍼는 **신규로 만들 기능이 아니다.** O4O 공통 도메인으로 이미 구현되어 3개 서비스(KPA·K-Cosmetics·Neture)에 라우트·컨트롤러·엔티티·마이그레이션·화면이 mount 되어 있다. 더 중요한 것은 **새 WO 정책의 핵심 조항 대부분이 이미 코드에 반영**되어 있다는 점이다.

**판정: B에 가깝되 C 요소가 많음 — "상당 부분 구현 완료, 소수 정책 항목만 정렬/완성 필요".** 신규 Foundation 구현(WO §10-A) 불필요. 기존 구현을 살리고 최소 정렬만 한다.

**이미 정책과 일치(추가 작업 불필요)**:
1. Neture는 "공급자 제안 생성 기지(허브)" 역할만 한다. → WO §2.2 일치.
2. **승인 주체 = 대상 서비스 운영자** — 승인 큐는 `status='pending' AND service_key=$1` 로 서비스별 격리, 각 서비스 운영자가 자기 서비스 대상 제안만 승인(KPA `requireKpaScope('kpa:operator')` K-Cos 동형). 중앙/Neture 일괄 승인 없음. → WO §3 일치.
...
2. KPA는 주문 흐름 구조 존재. → WO §5(바로 주문) 완성 필요.
...
| 컬럼 | 타입 | 의미 |
|---|---|---|
| `service_key` | varchar(50) | `kpa-groupbuy` / `k-cosmetics-event-offer` / `neture-event-offer`(dormant) |
...
| **§8.2** Neture가 대상 서비스인가? | ❌ 아니다. 공급자 제안 생성 기지로만. | 정책 일치 ✅ |
...
| **§8.4** 바로 주문 가능? | participate()=즉시 createOrder(). | 백엔드 ✅ / 프론트 미완 ⚠️ |
...
| K-Cos 매장 "추가/주문" 버튼 | **미완(disabled)** | "준비 중" — WO §5 바로주문 완성 필요 |
| K-Cos 공급자 생성 화면 | **부재(의도적)** | 공급자는 Neture 경유 제안 → 부재가 설계와 정합 |
...
1. K-Cos 매장 이벤트 오퍼 "바로 주문" 버튼 활성화(participate 연결) — 백엔드 route는 이미 존재.
...
2. commit `f999b53cd`, CHECK 문서 작성.
