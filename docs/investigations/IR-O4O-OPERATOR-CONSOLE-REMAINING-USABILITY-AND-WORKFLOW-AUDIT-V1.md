...
### 3-3. 🔴 **대시보드 KPI 숫자와 이동 목록 건수 불일치** (브라우저 실측, 2026-07-26 추가)

`/operator` 대시보드의 **"전체 회원 6"** 을 클릭하면 `/operator/members` 로 이동하는데,
그 화면은 **"전체 5"** 를 표시한다. 화면·API 양쪽에서 확인했다.

| 소스 | 값 | 근거 |
|------|:---:|------|
| 대시보드 KPI `total-members` | **6** | `GET /api/v1/kpa/operator/dashboard` |
| 회원 목록 | **5** (전부 `active`) | `GET /api/v1/kpa/members` — `total: 5` |
| 화면 표시 | KPI 6 → 목록 "전체 5" · `tbody tr` 5행 | 브라우저 실측 |

**원인:** 대시보드 KPI 는 `SELECT COUNT(*) FROM kpa_members` 로 **상태 무관 전수**를 세는 반면
(`operator-dashboard.service.ts` 의 admin 분기), 목록 API 는 **active 만** 반환한다.
따라서 KPI 가 가리키는 6번째 회원은 **그 목록에서 영원히 볼 수 없다.**

**업무 영향:** 운영자가 "6명"을 보고 목록에 갔는데 5명만 있으면 데이터 유실로 오인한다.
KPI→목록 연결은 운영자 콘솔의 기본 계약인데 그것이 깨져 있다.

**수정 방향(택 1, 정책 아님 — 구현 판단):** ① KPI 도 active 기준으로 세거나
② 목록에 상태 필터를 추가해 비활성 회원까지 볼 수 있게 하거나 ③ KPI 라벨을 "전체 회원(비활성 포함)"
으로 명확히 한다. ①이 가장 단순하고 기대와 일치한다.

### 3-4. 🟡 `MemberManagementPage` 혼합 구현
...
- **KPA 서비스 콘솔은 브라우저 실측을 수행했다**(자동화 프로필 확보 후 추가 검증):

| 검증 항목 | 결과 |
|-----------|------|
| 대시보드 블록 구성 | Overview · Quick Actions · Recent Activity · 역할 안내 — Action Queue/AI 는 데이터 0 이라 미렌더(빈 상태 정상 동작) |
| KPI → 라우트 연결 | 8개 KPI 전부 `/operator/*` 실재 경로로 연결 — **깨진 링크 0** |
| **KPI ↔ 목록 건수 일치** | ❌ **불일치 1건 발견** — §3-3 |
| `/operator/members` 표준 요소 | 검색 ✅ · 전체선택 체크박스 ✅(1) · 행 체크박스 ✅(5) · 페이지네이션 ❌(5행이라 미표시일 수 있음) |
| 메뉴명 ↔ 화면 제목 | KPI "회원 승인 대기" → 화면 "회원 관리" — 명칭 차이는 있으나 오해 소지는 낮아 **후순위(cosmetic)** |

- **`admin.neture.co.kr` 화면 실측은 여전히 미수행** — cross-site httpOnly 쿠키(`SameSite=None`)라
  자동화 프로필에서 세션이 유지되지 않는다. admin-dashboard 의 raw `<table>` 71건에 대한 시각 확인은
  구현 WO 에서 사용자 브라우저로 수행해야 한다.
