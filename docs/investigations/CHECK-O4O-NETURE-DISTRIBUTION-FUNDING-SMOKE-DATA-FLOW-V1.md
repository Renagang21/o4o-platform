...
운영 루프(생성→제출→승인 전 차단→승인→공개→참여→입금 확인→정리) **전 단계 정상 동작**. 단 CSV export에서 **기존 버그(한글 제목 Content-Disposition 500) 발견 → 즉시 수정 커밋**(`627bb62dc`), 배포 후 재확인 필요.
...
| CSV export | ❌→🔧 500(한글 제목 Content-Disposition `ERR_INVALID_CHAR`) **발견 → 수정 커밋 `627bb62dc`** (ascii filename + RFC5987 filename*), 배포 후 재확인 |
...
*상태: CONDITIONAL PASS — CSV 수정 배포 후 export 재확인 시 PASS*
