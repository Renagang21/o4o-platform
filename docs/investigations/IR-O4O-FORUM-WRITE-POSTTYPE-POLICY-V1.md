...
forum 사용자-facing list 축(hub/request/list primitives/shape/template)은 KCos/Neture 공통화 완료, KPA list 는 BaseTable 고유로 제외. 다음은 **write page** 이나, postType·editor·edit route 편차가 있어 정책 선결이 필요하다. 본 IR 은 write 공통화 전 기준을 확정한다(코드 무변경).

**핵심 결론:**
1. **`postType` 은 drift 아니라 실제 backend 필드** — `ForumPost.type`(enum PostType, default DISCUSSION). create/update controller 가 `type` 을 수용. KPA/Neture 는 write 에서 미노출(→backend default DISCUSSION), KCos 는 5-option select 노출. **"잘못된 필드"가 아니라 UX 노출 차이.**
3. **권장 = Option B(postType optional)** — shared write form `showPostType?` 로 KCos opt-in, KPA/Neture opt-out. list template 의 `showPostType` 패턴과 일치, KCos 배지 의미 보존.
4. **editor drift(C, 선행)**: **KCos 만 plain `<textarea>`**, KPA/Neture 는 RichTextEditor(@o4o/content-editor). write 공통화 전 **KCos editor parity** 필요.
6. **edit route 불일치(D)**: KPA `/forum/edit:id`(param) · Neture `?edit=`(searchParam) · KCos 없음 → **1차는 create-only 공통화**, edit 후속.
...
| ------ | ----- | ------------ | -------- |
| title | ✅ | ✅ | ✅ |
| content editor | **RichTextEditor**(HTML→blocks) | **textarea**(text→blocks) | **RichTextEditor**(HTML→blocks) |
| postType select | ❌ | ✅(5종, 영문 label) | ❌ |
| postType in payload | ❌(→default) | ✅ `type` | ❌ |
| tags | ❌ | ❌ | ❌ |
| forum/category 선택 | ❌(slug from route) | ❌ | categorySlug |
| edit mode | ✅ `/forum/edit/:id`(param) | ❌ create-only | ✅ `?edit=`(searchParam) |
| 고유 | `showStoreSave`(role) | — | `showContactOnPost`+contactSettings |
| success redirect | `/forum/post/:id`·`/forum/:slug` | `/forum/post/:id` | `${base}/post/${slug}` |
...
- → KCos 가 `type` 전송은 **정상 계약 사용**. KPA/Neture 미전송은 default DISCUSSION. **postType 은 유효 필드이며 invalid drift 아님.**
...
| 옵션 | 내용 | 장 | 단 | 판정 |
|------|------|----|----|:---:|
| A 제거 | KCos postType select 제거(전 서비스 무입력, backend default) | write 단순·tag/category 정책과 충돌 감소 | **KCos list type 배지 무의미화**(모두 DISCUSSION) — KCos 입력↔표시 단절 | — |
| **B optional** | shared form `showPostType?` — KCos opt-in, KPA/Neture opt-out | KCos 배지 보존·list template `showPostType` 와 일치·기존 UX 보존 | 공통 form 분기 1개(저복잡) | **권장** |
| C 전 서비스 표준화 | 4서비스 postType 도입 | list 배지·write 정합 | KPA/Neture UX 부담·현 단순화 역행 | 비권장 |

## 13. 권장 정책

1. **postType = Option B(optional)** — list template 의 `showPostType` 와 대칭. backend type 무변경(default DISCUSSION).
...
- (또는 editor parity WO 에 포함.)

**본체:**
- `WO-O4O-FORUM-WRITE-FORM-COMMONIZATION-V1`(H, create-only) — shared `ForumWriteForm`(title + RichTextEditor + optional `showPostType` + 확장 slot `renderExtra`(Neture contact)/editor extra prop(KPA store save)). KCos/Neture 우선, KPA 는 showStoreSave slot 으로 포함 가능 또는 별도. edit 제외.
...
| 점검 | 결과 |
|------|------|
| forum write 가 단순·명확한가 | ⚠️ KCos textarea postType-but-no-badge 등 비대칭 — Option B + editor parity 로 정렬 |
...
| KCos postType 실가치 확인 | ✅ KCos=list 배지 정합(가치 O)=표시 단절(정렬 필요) |
| Neture supplier/partner 맥락 유지 | ✅ contactSection/basePath/categorySlug slot 으로 보존 |
| editor 형식이 detail 렌더와 정합 | — |
| edit/create 무리한 동시 통합 회피 | ✅ 1차 create-only, edit 후속(D) |
| 공통화가 1인 유지보수성 향상 | ✅ editor 단일·content 단일·write form 공통 → 유지보수 단순화 |

권장은 **Option B(optional showPostType)** — list template `showPostType` 와 대칭·KCos 배지 보존. KPA showStoreSave·Neture contactSection 은 확장 slot(E/F).
...
- **write 필드 비교:** §4 매트릭스(KPA/Neture=postType 무입력+RichTextEditor=postType+RichTextEditor(HTML string) / KCos=postType+textarea)
...
- **edit route parity:** KPA param·Neture searchParam·KCos 없음 → 1차 create-only
- **권장 정책:** **Option B(postType optional)** + KCos editor parity content blocks 정렬, edit 후속
- **1차 WO:** (선행) editor parity content align → (본체) write form 공통화 create-only
- **후속 WO:** edit route parity postType 최종 / detail shared parts
