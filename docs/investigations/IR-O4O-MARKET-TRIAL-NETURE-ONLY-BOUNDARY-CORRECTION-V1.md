...
> **정책 정정**: 유통참여형 펀딩은 **Neture 전용**(Neture 공급자가 Neture 안에서 진행).
...
## 1. 목적
유통참여형 펀딩(=Market Trial)을 **Neture-only** 기능으로 확정하고, KPA/KCos Store 측에 남은 연결 흔적(route/카드/배너/메뉴/리다이렉트/backend tie)을 전수 조사해 cleanup WO 후보로 분리한다.

## 2. 정책 정정 기준 (확정)
```
소속      : Neture (Neture 공급자가 Neture 내부에서 진행)
비연결 대상 : KPA/KCos 의 운영자 · 매장 허브 · 내 매장 · O4O 주문 가능 상품 · 이벤트 오퍼
...
- "Store 서비스(KPA/KCos)는 Neture 리다이렉트 유지"
...
## 4. 조사 범위
frontend: services/web-{kpa-society,k-cosmetics}, web-neture; packages/{store-ui-core,store-products-ui,shared-space-ui}. backend: apps/api-server/src/{controllers/market-trial,routes/market-trial*,database/migrations}. (read-only, file:line 근거.)
...
| `api/public.ts:79,87,108` | `/store/market-trial` 링크 ×2 + "[안내] 유통참여형 펀딩 참여 가이드" | 데이터/안내 | ✅ |
| `pages/business/{BusinessHubPage,BusinessForumPage,BusinessPreparationPage,BusinessProductsPage,BloodCareBusinessStatusPage}` | "유통참여형 펀딩 기반 제품 개발" 텍스트(사업 아이디어, "실행은 Neture" 주기) | 텍스트 언급(링크 아님) | △(저위험, Neture 귀속 명시됨 — 표현 정리 선택) |
...
| store-ui-core | `config/storeMenuConfig.ts:33,161` | "market-trial 미마운트 제거" 주석 | 이미 메뉴 제거됨 | ▢(이미 정리) |

> store guide copy(`kpa.ts`/`k-cosmetics.ts`)에는 유통참여형/펀딩/market-trial **언급 없음**(grep 0건) — Store 가이드 측은 깨끗.
...
| KPA/KCos service/store guide | (유통참여형 언급 없음) | 정정 불필요 |
...
- `WO-O4O-MARKET-TRIAL-STORE-REDIRECT-AND-CARD-REMOVAL-V1` — KPA `/market-trial` 라우트 3개 + MarketTrialSection 홈 카드 + ServiceBanner 배너 제거; KCos homeStaticData 카드 2개 제거; (서비스별 path-specific)
...
- Store 서비스(KPA/KCos)에 route/카드/배너/메뉴/리다이렉트/참여이력/주문전환 연결 없음.
...
*Date: 2026-06-11 · read-only IR · 코드 무변경 · 정책: 유통참여형 펀딩 Neture-only, Store 연결 흔적 제거 후보 분리(KPA 활성/KCos 잔재). 선행 IR supersede.*
