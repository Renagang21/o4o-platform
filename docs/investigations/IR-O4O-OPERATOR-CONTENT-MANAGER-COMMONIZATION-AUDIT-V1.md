...
Neture는 공급자/파트너 도메인 특성상 공지/뉴스 운영자 콘텐츠 관리 화면 자체가 없다(의도된 차이).
...
news API `/api/v1/cosmetics/news/*`.
...
| 항목 | CmsContentManager (KPA/K-Cos 공통) |
|------|:---:|
| DataTable | ✅ |
...
| 서비스 | 결과 |
|--------|:---:|
| KPA/K-Cos | CmsContentManager config-driven 공통화 완료 |
...
| WO | 필요성 |
|----|:---:|
| K-Cos OPERATOR-CONTENT-MANAGER-ALIGNMENT | ❌ 불필요 (이미 적용) |
...
| **KPA 기준을 K-Cos에 적용해도 O4O 철학과 맞는가** | 이미 2서비스가 동일 CmsContentManager 사용. KPA assetCopy만 config 차이 — 서비스별 Hub→Store 정책 반영. 철학 일치. | ✅ |
...
**결론**:
1. Operator 콘텐츠 관리는 `CmsContentManager` config-driven으로 KPA/K-Cos 이미 공통화 완료.
