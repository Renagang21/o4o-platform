...
> 공통 모듈 `storeMenuConfig.ts` 는 3개 서비스가 공유한다. 후속 WO에서 이 파일을 변경할 경우 `O4O-SHARED-MODULE-CHANGE-PROTOCOL-V1` 에 따라 KPA/KC/Neture 4개 소비처 + admin/operator/store 영향을 모두 재검증해야 한다.
...
- **도메인 탭 (DOMAIN_TABS, 라인 42-48):** 전체 / 일반 B2B(`kpa`) / Event Offer(`kpa-groupbuy`) / 혈당관리 / 화장품(`cosmetics`) — 5개
...
- **"판매 신청"이 거래 신청과 같은 흐름?** → 다른 흐름. 판매 신청은 b2c 자사 상품 등록 측.
...
- **주문 관리만으로 충분?** → 운영 관점에서 불충분. 약국/매장이 거래할 상품을 "내 매장" 안에서 볼 화면이 없고, HUB로 나가야 함.
...
| 상품 | `pharmacyApi.getProducts` (`api/pharmacy.ts:416`) | — | — | **NO** | 일반 상품 API 테스트. 검증용 |
| 거래 신청 | (B2BOrderPage) | — | general` (`pharmacy.controller.ts:221`) | — | 부분 | 코드 존재, 레거시 스키마 |
...
> 핵심 격차: **KPA/KC는 공통 `SupplierProductOffer`** 위에 있고 위에 있다.

---

## 10. Cross-Service Gaps

1. **화면 성숙도 격차:** KPA(FUNCTIONAL) / KC(MISSING). 같은 "상품" 메뉴인데 본문이 운영/검증/부재로 3원화.
3. **IA 불일치:** KC는 거래 상품을 "내 매장" 안에서 못 보고 Store HUB로 나가야 함.
4. 공통 도메인 정렬 미완.
5. **거래 신청 흐름 비대칭:** KPA="판매 신청" 탭(b2c)="거래 신청" 별도 페이지(`/b2b-order`) / KC=없음. 용어·위치·흐름 모두 다름.
6. **컬럼/표시 비대칭:** KPA(재고·상태 미표시). 표준 컬럼 세트 부재.
...
| 3 | **WO-O4O-STORE-COMMERCE-PRODUCT-PAGE-CROSSSERVICE-PARITY-V1** | KPA 기준 구조로 KC 공통 정렬 (컬럼/탭/배지/empty 표준) | 2서비스 | KPA 기준 | 공통화 추출 | 중 | WO 1, 2 |
...
WO 4·5는 parity 확정 후 정리 단계.

---

## 14. Out of Scope

본 IR은 다음을 하지 않는다: 코드 수정 / 라우트 수정 / 메뉴 수정 / API 구현 / DB migration / 상품 화면 리팩터링 검증 문구 제거 / KC 상품 화면 신설 / KPA 화면 변경 / 배포.
...
- 공통 메뉴: `packages/store-ui-core/src/config/storeMenuConfig.ts` (KPA 236-287/248 164-224/174-180, KC 97-152/106-109, 정책 주석 30-31)
...
- 엔티티: `ProductMaster.entity.ts`, `SupplierProductOffer.entity.ts:36`, `ProductApproval.ts:33`, `organization-product-listing.entity.ts:27`, `store-local-product.entity.ts:25`
