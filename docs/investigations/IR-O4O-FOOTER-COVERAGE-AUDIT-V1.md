...
| 가장 큰 누락은? | **store-facing 화면.** ① store-hub: KCos는 top-level hub layout이라 footer 없음(KPA는 공개 Layout 중첩이라 있음 — 서비스별 불일치). ② 내 매장/내 약국 owner dashboard: 2서비스 공통 `StoreDashboardLayout`(@o4o/store-ui-core)에 footer 없음(공통 1곳). ③ KPA 공개 storefront `/store:slug` bare(footer 없음) 공개 storefront는 custom inline footer(공통 legal block 미사용). |
...
**핵심:** Footer 정비의 마지막 남은 조각은 "어디에 붙일까"가 아니라 **"사용자-facing인데 footer 없는 layout이 어디인가"** 이며, 그 답은 ①per-service store-hub layout(KCos) ②공통 store owner dashboard layout ③KPA 공개 storefront 세 군데로 수렴한다.
...
- 전면 FooterCore 비권장, KCos 한정 FooterCore 선택 과제
...
| **매장 HUB** | `/store-hub`,`/store-hub/*`(9) | `PharmacyHubLayout` **(공개 `Layout` 중첩)** | ✅ 상속 | **OK — 단 KCos와 구조 불일치(§6)** |
...
| 대상 | 서비스 | layout | 비고 |
|------|--------|--------|------|
| store-hub | KCos | `KCosmeticsHubLayout` | per-service. KPA는 공개 Layout 중첩이라 해당 없음 |
| **내 매장/약국 dashboard** | KCos, KPA | **공통 `StoreDashboardLayout`(@o4o/store-ui-core)** | **1곳 수정으로 2서비스 동시 해결. 단 F3 Store Layer Freeze → WO 필요** |
...
| 대상 | 내용 |
|------|------|
 공개 storefront | `StoreLayout` custom inline footer가 공통 `PublicLegalFooterInfo` 미사용 → 공개 매장 페이지 법정정보 표기가 footer 표준과 분리됨 |
| store-hub 구조 불일치 | KPA는 공개 Layout 중첩(footer 상속), KCos는 top-level hub layout(footer 없음) → **같은 store-hub인데 서비스별 결과 다름** |
...
| KPA 공개 storefront | `/store/:slug`,`/products/:id`,`/checkout`,`/payment/*` bare 렌더 |
 storefront blog | `/store:slug/blog*` bare |
...
> KPA 패턴(공개 layout 중첩으로 footer 상속)이 "layout 단위 보장" 원칙에 부합. KCos hub layout에 (compact) footer를 직접 추가하거나, 동일하게 공개 layout 중첩 구조로 정렬하는 두 방향 중 택일. **page-level 추가는 비권장.**
...
| 우선 | WO 후보 | 대상 | 범위 | 비고 |
|:---:|--------|------|------|------|
| **1** | `WO-O4O-STORE-FACING-FOOTER-COVERAGE-V1` | KCos·KPA | **내 매장/약국 dashboard(공통 `StoreDashboardLayout`) + KCos store-hub layout**에 footer coverage 보장. layout 단위. admin/operator 제외. Neture 제외 | **F3 Store Layer Freeze → 명시적 WO 필수.** 공통 layout 1곳이 2서비스 동시 영향 → Shared Module Change Protocol 적용(전 소비처 확인) |
| **2** | `WO-O4O-STORE-FACING-COMPACT-FOOTER-V1` | store-facing | Full 대신 compact footer 변형 도입(법정/약관/개인정보/문의/copyright 유지) | #1과 묶어 진행 가능 |
| 3 | `WO-O4O-PUBLIC-PAGE-FOOTER-COVERAGE-FIX-V1` | 3서비스 공개 | KPA 공개 storefront `/store:slug*` storefront blog `StoreLayout` custom footer→공통 legal block 정렬 + 404 | 공개 매장/법정 표기 일관성 |
...
2. **공통 `StoreDashboardLayout` 수정은 Shared Module Change Protocol** — KPA/KCos(+ 잠재 소비처) 전부 영향 확인. 단일 서비스 기준 완료 판단 금지. F3 Freeze WO 필요.
...
- store-hub를 KPA식 공개 layout 중첩으로 정렬할 경우 KCos route wrapper 재배치 → guard/sidebar 누락 주의.
