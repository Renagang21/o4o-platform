...
| 단계 | 동작 | 근거 |
|------|------|------|
| HUB import | `importOperatorPop(slug, id)` → toast "내 매장 POP(초안)에 추가" — **navigate 없음**(hub 페이지 잔류) | — |
...
| PDF builder | `StorePopPage` generate payload = `supplierItemIds`/production router state(library/direct/snapshot) — **store_pops 미사용** `StorePopPage.tsx:180-188` |
...
> **선행 POP IR 의 "staff list/edit 2서비스 parity" 는 backend 한정.** 프론트 사본 관리 페이지는 **KPA only** — KCos import 는 목록/수정 UI 없는 dead-end.
...
- KCos: 사본 관리 페이지(PharmacyPopPage) 부재 + popStaff list/edit client 부재 → import dead-end
...
2. `WO-O4O-POP-STAFF-PAGE-GP-KCOS-PARITY-V1`(parity) — KCos 에 `PharmacyPopPage` 동등 페이지 + popStaff list/update/delete client 추가(KPA 기준). 현재 KCos import dead-end 해소.
...
- import 후 navigate 없음(HubPopLibraryPage) · KPA PharmacyPopPage builder 진입은 generic nav · KCos PharmacyPopPage·staff client 부재 확인.
...
- KPA 는 사본 목록/수정(`PharmacyPopPage`)은 되지만 "그 POP 으로 제작" 연결은 없음(분리 인지). **KCos 는 사본 관리 UI 자체가 없어 import 가 dead-end.**
- **권고**: ① product 결정(연결 vs 독립 트랙) → ② 연결 시 `WO-O4O-POP-IMPORT-TO-BUILDER-LINK-V1`(DB 불요) → ③ KCos staff 페이지 parity(`WO-O4O-POP-STAFF-PAGE-GP-KCOS-PARITY-V1`). 본 조사 범위는 연결 여부 확인까지 — 코드 무변경.

---

*Date: 2026-06-15 · read-only IR · 코드/DB 무변경 · 판정 C(미연결) — POP PDF builder 가 store_pops 미수용, 가져온 POP→제작 동선 끊김. KPA PharmacyPopPage 사본 관리만, KCos 사본 페이지 부재(dead-end). 연결·parity 후속 제안(DB 불요).*
