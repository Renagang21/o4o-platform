...
> **대상:** KPA Society / K-Cosmetics 2서비스의 "내 매장·내 약국 콘텐츠" + 매장 허브(store-hub) 콘텐츠 화면
...
3. **KPA `/store-hub/content` 만 outlier 다.** K-Cosmetics 는 공통 `ContentHubTemplate` + "콘텐츠 자료실" 프레이밍 + CMS 타입 필터 `전체/공지/가이드/지식/프로모션/뉴스` 를 이미 사용. KPA 는 독자 `HubContentLibraryPage` + "플랫폼 콘텐츠 / 본부·공급사 CMS 콘텐츠" 프레이밍 + `전체/공지·소식/가이드/지식자료/혜택·이벤트` 5탭.

4. **가설 검증 결과: "매장 허브 콘텐츠 = 내 매장 콘텐츠의 가져오기/공급/탐색 화면" 은 참.** 3서비스 모두 복사/가져오기 → `내 자료함`(자산) 또는 `내 매장 블로그/POP/QR` 사본으로 귀착하며, 원본/사본 분리 문구도 동일.

5. 탭을 콘텐츠 페이지에 욱여넣지 말 것. 후속 WO 후보는 §8·§10.
...
| store-hub 하위 | sourceDomain | KPA 컴포넌트 || K-Cosmetics 컴포넌트 | 가져오기 → 귀착 |
|----------------|--------------|-------------|---------------------|----------------------|-----------------|
...
| ------ | ------------- | ------------- |
| 컴포넌트 | `HubContentLibraryPage` (독자) | `HubContentPage` (ContentHubTemplate 기반) |
| 화면 제목 | **"플랫폼 콘텐츠"** | **"매장에서 바로 쓰는 콘텐츠"** |
| 설명 문구 | "본부/공급사가 제공하는 CMS 콘텐츠를 탐색하고 내 매장에 복사합니다" | "K-Cosmetics 매장을 위한 콘텐츠 자료실" |
| 탭/필터 | `전체 / 공지·소식 / 가이드 / 지식자료 / 혜택·이벤트` (5) | `전체 / 공지 / 가이드 / 지식 / 프로모션 / 뉴스` (6) |
| 리스트 형태 | 카드 | 카드 그리드 |
| 검색 | 제목/요약 | 콘텐츠 검색 |
| 페이지네이션 | 20/page | 12/page |
| 복사 버튼 | "내 매장에 복사" | "내 매장에 복사" |
| 빈 상태 | "현재 제공되는 콘텐츠가 없습니다" | (템플릿 공통) |
| 가져오기 후 안내 | (원본/사본 분리 안내) | "복사된 콘텐츠는 내 매장 > 자산 관리에 별도 사본으로 저장됩니다 …" |
| API | `cmsApi.getContents` + `assetSnapshotApi.copy` | `GET /hub/contents?serviceKey=k-cosmetics&sourceDomain=cms` + `POST /cosmetics/assets/copy` |

> **핵심:** K-Cosmetics 는 **동일 ContentHubTemplate + 동일 6-필터 + 동일 "콘텐츠 자료실" 언어**로 이미 정렬됨. KPA 만 "플랫폼 콘텐츠 / CMS 탐색" 이라는 **CMS-운영자 관점 프레이밍 + 5탭** 으로 남아 있는 소수.
...
| 표면 | 경로 | KPA | K-Cosmetics |
| ------ | ------ | ----- | ------------- |
| 자료함-콘텐츠 | `/store/library/contents` | `StoreLibraryContentsPage` | `StoreLibraryContentsPage` |
| 자료함-자료 | `/store/library/resources` | ✓ | ✓ |
...
| 블로그 | `/store/content/blog` | `PharmacyBlogPage` | `StoreBlogManagePage` |
| POP | `/store/marketing/pop` (+ `/pop/library` 사본) | `StorePopPage` | `StorePopPage` |
| QR | `/store/marketing/qr` | `StoreQRPage` | `StoreQrPage` |
| 사이니지 | `/store/marketing/signage/playlist` | `StoreSignagePage` | `StoreSignagePage` |
| 상품별 마케팅 | `/store/commerce/products/:id/marketing` · `/pop` | (StoreQRPage 내 product landing) | `ProductMarketingPage`·`ProductPopBuilderPage` |
...
| 콘텐츠 유형 | 내부 키(sourceDomain/type) | KPA | K-Cosmetics | 공통화 상태 |
| ------------ | --------------------------- | ----- | ------------- | ------------ |
...
| 안내문 | — | **별도 유형 없음** | **별도 유형 없음** | 미존재 (POP/CMS 내 포섭) |
| 이미지/배너/동영상 | signage media type(image/video) | 사이니지 내 | 사이니지 내 | 별도 표면 없음 |
...
| 항목 | KPA | K-Cosmetics | 판단 |
| ------ | ----- | ------------- | ------ |
| 직접 생성(블로그/POP/QR) | 매장측 직접 작성 | 매장측 직접 작성 | 공통 |
| AI 생성/보조 | 일부 | POP/QR/상품설명 AI | KCos 가 AI 통합 더 진전 |
| 템플릿 | 일부 | POP 템플릿(모던/소프트/전문형) | KCos 가 명시적 |
| 허브에서 가져오기 | ✓ (5 도메인) | ✓ | 공통 |
| 가져온 콘텐츠 사본화 | ✓ snapshot/import | ✓ | 공통 |
| 원본/사본 분리 | ✓ (문구 명시) | ✓ (문구 명시) | 공통 — 거의 동일 카피 |
| 중복 가져오기 | 허용(새 사본) | 허용 | 공통 (DB unique 제거됨) |
| 수정/삭제 권한 | 사본 매장 소유, draft | 동일 | 공통 |
| 사이니지 활용 | snapshot → playlist | 동일 | 공통 |
| 상품 연결 | QR landing=product | product marketing graph | KCos 가 더 구조화 |
...
- **HUB 목록**: `GET /api/v1/hub/contents` 단일 service-neutral 엔드포인트(`serviceKey` + `sourceDomain` 파라미터). `HubContentQueryService`. → KPA 프론트는 일부 `cmsApi.getContents` 사용(legacy), KCos 는 `hubContentApi.list` 사용 — **프론트 client 차이일 뿐 backend 동일**.
...
- **divergence(backend)**: ① blog.controller ② asset copy 허용 role 이 KPA 만 명시(4개), Cos 미명시, ③ QR resolver Phase1 placeholder(`null`) 일부. → **본 IR 의 frontend 정비와 독립**. 별도 정리 후보로만 기록.
...
| 제목·문구 적절성 | **부적절(소수 일탈).** "플랫폼 콘텐츠 / 본부·공급사 CMS 콘텐츠 탐색" 은 운영자/플랫폼 관점. KCos 의 "매장에서 바로 쓰는 콘텐츠 / 콘텐츠 자료실" 이 매장 사용자 관점에 부합하며 이미 2/3 표준 |
| 탭 적절성 | "혜택·이벤트" 1개를 제외하면 KCos 의 `공지/가이드/지식/프로모션/뉴스` 와 사실상 동형. KPA 만 라벨/구성이 어긋남 |

**결론: KPA `/store-hub/content` 는 기능적으로는 정상이나 IA·문구·탭이 KCos canonical 에서 이탈한 outlier. 유지보다 수렴이 적절.**
...
- **Q1. 2서비스 구조가 같은가/다른가** → **백엔드·데이터·내 매장 흐름은 같다(공통).** 다른 곳은 `/store-hub/content` **프론트 화면의 문구·탭·컴포넌트뿐**이며, KCos 가 표준, KPA 가 outlier.
...
- **Q4. KPA `공지·소식/가이드/지식자료/혜택·이벤트` 탭 적절성** → CMS 분류로는 타당하나 **KCos canonical(`공지/가이드/지식/프로모션/뉴스`)과 어긋남**. "혜택·이벤트" 는 event-offer 중복 소지. → 주 탭으로 유지하되 canonical 라벨로 정렬 권장.
...
1. **(권장) `WO-O4O-KPA-STOREHUB-CONTENT-CANONICAL-ALIGN-V1`** — KPA `HubContentLibraryPage` 를 KCos `ContentHubTemplate` 패턴으로 수렴(제목/설명/6-필터/안내문구). frontend-only, backend 무변경. Shared Module Change Protocol 적용(소비처 = KPA store-hub 단일). "혜택·이벤트" → "프로모션" 통합 여부 결정 포함.
...
*read-only investigation · 백엔드·데이터·내 매장 흐름은 2서비스 공통 · divergence 는 KPA `/store-hub/content` 프론트(문구·탭·컴포넌트) outlier · POP/QR/블로그/사이니지는 이미 store-hub 형제 페이지(탭 아님) · 가설("매장 허브 콘텐츠 = 내 매장 콘텐츠 가져오기/공급 허브") 참 · 권장 = KPA 를 KCos canonical 로 수렴, backend 무변경.*
