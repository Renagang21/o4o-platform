...
- 대부분 이미 `= $n` parameter binding. checkout 1곳만 single literal(저위험 미세 정리 후보, 비필수).
...
| 원칙 | 현황 | 판정 |
|------|------|:---:|
| **OPL serviceKey drift가 매장 상품 노출 안정성을 해치는가** | KPA는 흐름별 단일 serviceKey로 명확 필터, K-Cos는 OPL 미사용. 노출 안정성 위협 없음. | ✅ 안정 |
| **serviceKey constants가 O4O 공통 운영 안정성에 도움** | KPA/K-Cos는 다중 키 gate 부재 → 상수 도입이 오히려 불필요 추상화. | ⚠️ 서비스별 상이 |
...
- `apps/api-server/src/constants/service-keys.ts`
