...
> **검증 보고서 (Verification Report)** — Production 배포 + 2 service 통합 AI 보존 확인.
>
> `WO-O4O-OPERATOR-RESOURCES-CANONICAL-COMMONIZATION-V1` (commit `181d0ec14`) 적용 후 KPA / K-Cos 의 OperatorResourcesPage 가 단일 wrapper 로 정합되는지 검증.
...
| D. WO scope 준수 (Backend 0, 신규 페이지 분리 0) | ✅ Backend 변경 0 page 분리 회피 (aiSlot 흡수) |
| F. K-Cos Capability 도입 | ✅ /operator/resources route + menu + 페이지 + API 모두 신설 |
| G. KPA thin wrapper 변환 | ✅ KPA 725→19 lines 747→50 lines |
...
| 3 | `packages/operator-core-ui/src/modules/resources/OperatorResourcesConsolePage.tsx` (신규) | 732 | **공통 wrapper 핵심** (KPA 의 95% 동일 코드 추출 + aiSlot) |
| 4 | `packages/operator-core-ui/src/modules/resources/index.ts` (신규) | +19 | exports |
| 5 | `services/web-k-cosmetics/src/api/resources.ts` (신규) | 108 | `kCosResourcesApi` |
...
| 10 | — | 50/-747 | thin wrapper + aiSlot (AiContentModal 보존) |
...
| KPA thin | OperatorResourcesPage.tsx | 19 |
 thin (aiSlot 포함) | OperatorResourcesPage.tsx | 50 |
...
interface OperatorResourcesConsolePageProps {
  ...
  aiSlot?: ResourcesConsoleAiSlot; / 만 전달
...
| 항목 | 신규 위치 |
|---|---|
| API client | `services/web-k-cosmetics/src/api/resources.ts` (kCosResourcesApi mirror, path `/cosmetics`) |
...
| KPA `OperatorResourcesPage` UI 패턴 | ✅ 보존 | wrapper 가 KPA 의 columns / filters / drawer / bulk 패턴 그대로 추출 |
 `OperatorResourcesPage` UI 패턴 | ✅ 보존 | 동일 |
| Backend `cosmetics_contents` | ✅ 무영향 | 선행 WO 의 신규 테이블, 본 WO 는 frontend only |
| 다른 service (Neture) | ✅ 무영향 | 본 WO scope 외 |

**1 가지 작은 변경:** Wrapper 의 `header description` 이 "자료실 자료 운영 관리 — 총 X개" 로 통일됨. → **`policyBanner` prop 으로 정책 안내 문구는 service-별 보존됨** (KPA 모두 자체 문구 전달).
...
| 작업 원칙 | 준수 |
|---|:---:|
| KPA Canonical 기준 (Wrapper 가 KPA 의 공통 코드 추출) | ✅ |
| 같은 Capability → 같은 UI/UX → 같은 Wrapper | ✅ 3 service 100% 동일 wrapper |
 AI Modal Slot 흡수 (page 분리 금지) | ✅ aiSlot prop 으로 흡수 |
| 신규 Backend 금지 | ✅ Backend 변경 0 |
| Service별 독립 UI 금지 | ✅ wrapper 단일 |
| K-Cos 예외 처리 금지 | ✅ KPA 와 동일 thin wrapper 구조 |
...
| 2 `/operator/resources` | 동일 + **"AI 콘텐츠 생성" 버튼 보임 + 모달 열림 + AI 생성 → 자료실 저장 → 목록 refresh** |
| 3 | K-Cos `/operator/resources` | 동일 (신규 페이지, AI 버튼 없음) — 자료 등록은 가능하지만 backend POST 흐름 검증 |

추가: KPA / K-Cos 의 운영자 메뉴에서 "자료실 관리" 진입 링크 확인.
