...
- 대상 서비스 5개: KPA-Society(KPA) · K-Cosmetics(KCos) · Neture(NET) · Pharmacy-Hub(PH)
...
| 4 | 백엔드 mount | `apps/api-server/src/bootstrap/register-routes.ts` 전체 `app.use()` · `routes/forum/**` · `controllers/forum/**` |
...
| F07 | 게시글 수정 | FC | NI | FC | NI | NI | KPA `/forum/edit:id`, NET `ForumWritePage` edit 모드. KCos/PH 는 작성만 |
...
| # | 기능 단위 | KPA | KCos | NET | PH | 근거 |
|---|---|:--:|:--:|:--:|:--:|:--:|---|
| F10 | 콘텐츠 목록 | VD | VD | SS | VD | NI↔KCos `ContentListPage` **diff 2줄**(146줄 중). KPA 는 별도 761줄 구현. NET `/content` 는 운영자 발행 HUB 라이브러리 — 회원 작성형과 다른 축 |
...
| F20 | 강사 강의 개설·관리 | CO | CO | NI | CO | NI | 백엔드 `/lms/instructor/*` 공통 + `@o4o/lms-client` `createLmsInstructorClient`. 화면 KPA 7 route 4 / KCos 2 로 범위 자체가 다름 |
...
| F22 | 회원 자료 등록·수정 | SS | NI | NI | NI | NI | KPA `ResourceWritePage` 781줄. KCos 는 운영자 등록만(`/operator/resources`) |
...
| F25 | 제작자료 안내 (`/guide/features/production-materials`) | FC | FC | NI | FC | NI | `GuideFeatureManualPage` 공통 (KPA 는 전용 wrapper, KCos 는 props 주입) |
...
| F31 | 내 포럼 대시보드 | VD | VD | VD | VD | NI | 285 / 581 / 576 / 572줄. |
...
| F34 | 포럼 회원 관리 (소유자) | VD | VD | NI | VD | NI | 381 / 354 / 354줄. |
...
| # | 기능 단위 | 소유 | 판정 | 근거 · 사유 |
|---|---|:--:|:--:|---|
| F44 | 설문 · 참여 (`/surveys`,`/participation`,`/content/surveys`) | KPA | SS | 회원 대상 설문 응답·결과. KCos 는 운영자 설문(`/operator/surveys`)만 |
...
| F50 | 사업 영역 허브 (`/business/*` 6 route) | SS | BloodCare·사업 준비·사업 포럼 |
| F51 | 피드백 포럼 (`/forum/feedback`) | SS | — |
| F52 | 참여 신청 (`/apply/*` 3 route) | SS | 약국·약사 참여 신청 및 내 신청 |
...
KCos 는 채택 폭이 거의 같고 VD 6 도 동일하다 — **서비스가 서로의 복제본**이라는 §2 관찰과 일치한다.
...
| `AppreciationPanel` | 14 | 3 | KPA·KCos |
...
| `contentHubCardGrid` | 2 | 2 | KCos |
| `SignageHubTemplate` | 1 | 1 만 |
...
| `CommunityContentSearchBar` 외 목록 템플릿 부재 | KCos `ContentListPage` | 146줄 × 2 (diff 2줄) |
| `MyRequestsInbox` `pages/forum/MyRequestsPage` (285줄, legacy 잔존) | `pages/mypage/MyRequestsPage`(58줄)는 공통 채택 완료 |

### 4-3. 그 밖의 공통 패키지

| 패키지 | 커뮤니티 축 소비 | 상태 |
|---|---|---|
| `@o4o/account-ui` | KPA 19 · KCos 14 13 · NET 12 · PH 4 파일 | mypage 축은 사실상 공통화 완료 |
...
| 콘텐츠 · 자료실 | `/api/v1/kpa/contents` (kpa.routes.ts 인라인 ~550줄) | `/api/v1/cosmetics/contents` | `/api/v1/neture/content` (cms 축) | — | — | — | **물리 테이블 분리** `kpa_contents` / `cosmetics_contents` / cms_contents |
...
| 홈 · 최신 활동 | `/api/v1/kpa/home/*` (8 endpoint) | `/api/v1/cosmetics/home/latest` (1) | — | — | — | 각 `*.routes.ts` 인라인 핸들러 3벌 | 서비스 prefix |
...
- **포럼**: 컨트롤러는 이미 단일 세트. 남은 문제는 **mount 축이 4갈래**(공통 무필터 / KPA prefix prefix / `createServiceForumRouter`)라는 점이다.
...
`F16`·`F17` 이 KPA 1,841줄 1,492줄 · KCos 1,155줄로 각자 구현돼 있다. 커뮤니티 축 최대 복제 덩어리다.
...
`F31`·`F34`. NET 은 supplier 축에 576줄 사본을 하나 더 갖고 있다.
...
**선행**: **테이블 통합 여부 결정이 먼저다.** `kpa_contents` / `cosmetics_contents` 물리 분리를 유지한 채 라우터만 파라미터화할지, 단일 테이블+`service_key` 로 갈지는 §14 Frozen Baseline(F4/F5 Content Stable) 판단이 필요하다.
...
**S1 결함 정렬이 본체**다. 이어서 KPA `ForumListPage`(758줄) → `ForumListTemplate`, NET `ForumPostPage`(1,033줄) → `ForumPostHeader`+`ForumDetailStates`+`forumContentToHtml` 로 수렴한다.
...
| U5 | LMS 강사 영역 (F20) | `CORE_ONLY` | KPA 7 route(강의 운영·과제 채점 포함) 4 vs KCos 2 로 **기능 범위 자체가 다르다**. "같은 기능 단위" 로 묶을지 KPA 확장분을 별도 SS 로 뺄지 경계가 애매. 백엔드가 공통이라 CO 로 통일 |
...
| 3 | F11 콘텐츠 상세 × FC | `App.tsx:623` → `pages/contents/ContentDetailPage.tsx`(130줄) → `CommunityContentDetailView`. KCos 동일 파일과 diff 4줄 | ✔ |
...
| 7 | F42 좋아요 × NI | `pages/forum/ForumPostDetailPage.tsx` 에 `handleLike`·`toggleLike` grep 0건 | ✔ |
