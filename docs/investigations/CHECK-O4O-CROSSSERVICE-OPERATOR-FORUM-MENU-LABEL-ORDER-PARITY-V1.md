...
KCos는 canonical 상대 순서(신청 관리 → 삭제 요청 → 분석) 유지. KPA 전용 `포럼 운영`/`포럼 목록 관리`는 해당 route/page가 KPA에만 존재하므로 그대로 유지(KCos 미추가).
...
- KCos에 `포럼 운영`(`/operator/forum`)·`포럼 목록 관리`(`/operator/forum-categories`) 메뉴 **추가하지 않음** (해당 route/page 부재).
...
- WO-O4O-CROSSSERVICE-OPERATOR-FORUM-ROUTE-ALIAS-PARITY-V1 — KCos `/operator/forum` base redirect alias, 진입 통일.
- WO-O4O-CROSSSERVICE-OPERATOR-FORUM-ANALYTICS-UI-COMMONIZATION-V1 — forum-analytics 공통 컴포넌트 추출 (복제 3벌 → 1).
- WO-O4O-CROSSSERVICE-OPERATOR-FORUM-DASHBOARD-UI-PARITY-V1 / -CATEGORIES-PARITY-V1 — 운영 허브·목록 관리 KCos 이식 (사업 필요성 판단 선행).
