...
> K-Cosmetics operator **Signage** 하위 메뉴를 동일한 canonical 구조로 정렬.
...
| # | path | K-Cosmetics (before) | path |
| :-: | --- | --- | --- |
| 1 | `/operator/signage/hq-media` | **사이니지 콘텐츠** | `/operator/signage/content` |
| 2 | `/operator/signage/hq-playlists` | HQ 미디어 | `/operator/signage/hq-media` |
| 3 | `/operator/signage/templates` | HQ 플레이리스트 | `/operator/signage/hq-playlists` |
| 4 | `/operator/signage/content` | 템플릿 | `/operator/signage/templates` |
| 5 | `/operator/signage/library` | 강제 콘텐츠 | `/operator/signage/forced-content` |
| 6 | `/operator/signage/forced-content` | — | — |
...
- 본 작업 git diff 는 K-Cosmetics 2개 config 파일만 포함.
...
- 의도된 보존(기능 삭제 아님, 메뉴 정합만). route 정리는 별도 WO 범위.
