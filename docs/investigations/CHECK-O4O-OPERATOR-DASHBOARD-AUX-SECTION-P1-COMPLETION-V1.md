...
- 배포 후 권장 확인: KPA `/operator`(Guide+Axis+5block) `/operator`(header 부재·Alerts+Axis+5block) / KCos `/operator`(notice 유지·Axis+5block) / Neture `/operator`(새 Axis 표시·5block) / console error 0.
...
→ 운영자 대시보드 부가 섹션 **P1 완료로 고정**. 다음은 이후 P2(above-block slot) 진행 권고.
...
| 부가 섹션이 운영 판단을 방해하지 않는 위치인가 | ⚠ KPA GuideCard 최상단은 여전히 무거움 → P2 보류 |
