...
> **결론(요약): 판정 B (stale 아님 — 마운트된 컨트롤러가 사용 중) + ⚠️ latent/live 버그 발견.** `StoreLibraryItem`(`@Entity('store_library_items')`)은 `createStoreLibraryController`(Cosmetics `/pharmacy/library` CRUD)가 `getRepository(StoreLibraryItem)` 로 사용하며, 이 컨트롤러는 KCos 라이브 페이지가 호출한다. 그러나 물리 테이블 `store_library_items` 는 `store_execution_assets` 로 **hard rename**(migration `20260421010000`, back-compat view 없음) 되어 **존재하지 않는다** → 해당 컨트롤러는 부재 테이블을 조회. **A(완전 stale·즉시 삭제) 아님** — 삭제 시 컨트롤러 import 깨짐. **즉시 코드 변경 없이** 별도 repoint WO 로 분리.
...
- **KPA 미마운트**(KPA `storeLibrary.ts:13` 주석 "KPA 백엔드 미등록").
- **엔드포인트:** `GET/POST/PUT/DELETE /pharmacy/library` (자료실 CRUD).
- **프론트 소비(라이브):** KCos 페이지가 `getStoreLibraryItems`(`@/api/storeLibrary`) 호출:
...
> **운영 실제 거동(500 vs 빈 목록)은 후속 WO 에서 DB/런타임 확인 권장.**
...
- 일부 사용(B): 예 — KCos /pharmacy/library 컨트롤러 + 라이브 페이지 소비
...
3. 배포 후 KCos `/pharmacy/library`(StoreLibraryResourcesPage/SelectorModal/TabletDisplays) 정상 동작 확인(현재 broken 가능 → 복구).
- **본 WO 에서 즉시 수정하지 않은 이유:** KCos **라이브 엔드포인트 거동 변경**(복구이지만 변경)이며, B안 지침(문서화+별도 WO) + 운영 거동 확인 동반이 안전. browser smoke 불가 환경.
