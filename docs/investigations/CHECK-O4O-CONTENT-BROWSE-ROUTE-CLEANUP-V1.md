...
> **작성:** 2026-06-14
> **판정:** **PASS** (KCos 운영자 발행 browse → `/store-hub/content` 수렴, `/content` 해방. KPA 무변경. 회원 `/content` 미적용)

---

## 1. 작업 개요

`/content` 표준(회원 작성형) 적용 전 단계로, KCos 의 운영자 발행 콘텐츠 browse 를 canonical `/store-hub/content` 로 수렴(redirect)하고 home "콘텐츠" 링크를 repoint. **회원 작성형 `/content` 는 이번 WO 에서 만들지 않음.**
...
| 서비스 | 위치 | 이전 | 이후 |
|--------|------|------|------|
| CommunityMainPage 최신탭 + 서비스카드 | `/content` | `/store-hub/content` |
...
| redirect route 정의 | — |
| browse canonical `/store-hub/content` | — |
| browser smoke | — |
...
| 후보 | 내용 |
|------|------|
| `WO-O4O-GP-CONTENT-STANDARD-ROUTE-ALIGNMENT-V1` | redirect 해제 |
...
| 항목 | 결과 |
|------|------|
| `/content`·`/library/content` → `/store-hub/content` redirect, home 링크 repoint |
