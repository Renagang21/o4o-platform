...
> **목적**: K-Cosmetics operator 에 문의 관리(처리) 기능을 제공.
> **작성일**: 2026-06-16
...
- **backend 가드 1라인 조정 필요** 문의 **관리** 컨트롤러 가드를 `admin` → `operator` 레벨로 변경.
- 문의 **설정** 컨트롤러는 손대지 않음 → admin 전용 유지.
- DB/모델/설정 구조 변경 없음.

---

## 2. K-Cosmetics scopeRoleMapping 조사 결과 과의 차이

`packages/security-core/src/service-configs.ts`

| 서비스 | scopeRoleMapping | 문의 가드 `requireServiceLegalScope('admin')` 시 operator 접근 |
|--------|------------------|--------------------------------------------------------------|
| **K-Cosmetics** | `'cosmetics:admin': ['cosmetics:admin']` / `'cosmetics:operator': ['cosmetics:operator','cosmetics:admin']` | — |

→ 가설(KCos operator 403) **코드로 확정**.
...
### Shared Module 영향 분석 (이 컨트롤러는 KCos 공통)
- 이 컨트롤러 소비처: `register-routes.ts` 단일 마운트 1곳. serviceKey 화이트리스트 = `k-cosmetics` 만.
...
- **메뉴 위치**: `content` 그룹(커뮤니티 운영 도메인) — 공지사항/뉴스·설문조사 관리 다음.
- **route**: `/operator/contacts`
...
> ⚠️ 본 WO 의 핵심 리스크는 backend 가드 변경의 **cross-service 영향**(KCos 공통)이다.
...
*backend 1라인 가드 조정(KCos 공통, 무회귀) + frontend operator 이관. DB/모델 무변경. app·api typecheck PASS.*
