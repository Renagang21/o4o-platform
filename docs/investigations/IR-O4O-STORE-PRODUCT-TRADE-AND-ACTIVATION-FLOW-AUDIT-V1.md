...
- **상품·거래는 서비스별 성숙도 격차가 큼**: KPA 상품·주문 FUNCTIONAL 상품 화면은 **PLACEHOLDER(백엔드 미연결)**, K-Cosmetics 는 상품/거래신청 자체가 MISSING(주문만).
...
- 가장 가치 있는 후속: **KPA에서 안정화된 활성화/자료함 흐름을 KC로 공통화**(신규가 아니라 확장).

---

## 2. Scope

- **대상:** KPA-Society / K-Cosmetics 의 `/store` 영역
...
| 상품 | store/management/b2b | PharmacyB2BProducts | YES | **PLACEHOLDER (B2B API 미연결)** |
| 거래 신청 | store/b2b-order | B2BOrderPage | YES | FUNCTIONAL |
| 주문 관리 | store/commerce/orders | PharmacyOrders | YES | FUNCTIONAL |
...
| 내 약국 제품 | store/my-products | StoreProductsManagerPage | YES | FUNCTIONAL |
| 상품 설명 | store/library/product-descriptions | StoreProductDescriptionsPage | YES | FUNCTIONAL |
| POP | store/marketing/pop | StorePopPage | YES | FUNCTIONAL (supplier items 중심) |
| QR-code | store/marketing/qr | StoreQrPage | YES | FUNCTIONAL (기본 CRUD, AI 없음) |
| 블로그 | store/content/blog | PharmacyBlogPage | YES | FUNCTIONAL |
...
| 콘텐츠/자료/제작 자료 | store/library/* | (동명 컴포넌트) | YES | FUNCTIONAL(콘텐츠) / PARTIAL(자료·제작자료, Phase 2-B/2-D) |
...
| 동영상/플레이리스트/스케줄/TV재생 | StoreSignageMainPage | FULL (3탭 + 강제콘텐츠 경고) |
...
- `거래 신청`(/b2b-order)은 프랜차이즈/일반 탭+취급요청까지 FUNCTIONAL. 주문 관리 FUNCTIONAL(채널 표시 포함).
...
- **구조:** 경로(`/library/contents|resources|production-materials`)는 공통이나 **구현은 서비스별 개별 + Phase 격차**.
  ...
  - KC: `자료`는 store_library_items read-only(Phase 2-B), `제작 자료`는 generated assets 만(Phase 2-D) — QR/블로그 미통합, derivation 없음.
- **store_asset_derivations(원본→파생 추적):** **백엔드 FULL**(테이블+서비스+`GET /api/v1/kpa/store/asset-derivations`, 다형관계·org/service 격리), **프론트는 KPA만**(`storeAssetDerivations.ts` + StoreProductionMaterialsPage 모달). **KC 프론트 MISSING이나 백엔드 인프라는 재사용 가능**(최근 커밋 966062aa1/9632554a0).
- **출처/용도 필터:** KPA 자료/제작자료에 출처·용도 표시. KC 는 category/usageType 라벨 정도.
...
| 갭 | KPA | KC | 비고 |
|---|---|---|---|---|
| 상품(거래 대상) 화면 | FUNCTIONAL | PLACEHOLDER | MISSING 백엔드 미연결, KC 부재 |
| 거래 신청 전용 화면 | 우회(작업대) | FUNCTIONAL | MISSING | 서비스별 흐름 상이 |
| 자료함 `자료` 등록/삭제 | FULL | PARTIAL | PARTIAL | KC Phase 2-B 미완 |
| 자료함 `제작 자료` 결과물 통합(QR/블로그) | FULL | MISSING | MISSING | KC Phase 2-D 미완 |
| derivation viewer(프론트) | FULL | MISSING | MISSING | 백엔드는 공통 가능 |
| 사이니지 동영상/스케줄 탭 | FULL | FULL | MISSING(화면) | KC 탭 미구현 |
| 제품 row action 콘텐츠 진입 | 실증 FULL | 미실증 | 미실증 | KC 검증 필요 |
...
- **store_asset_derivations 백엔드** — KPA 전용이 아니라 org/service 격리 다형 구조라 KC 프론트만 붙이면 됨.
- **KPA `StoreLibraryResourcesPage`(2-source 통합) / `StoreProductionMaterialsPage`(결과물 통합)** — KC Phase 2-B/2-D 완성 시 KPA 구현을 공통 패키지로 승격 가능.
...
| R2 | 사용자가 빈/임시 데이터를 봄 | MED | — |
| R3 | 자료함 KC Phase 2-B/2-D 미완 — 제작 결과물(QR/블로그)이 자료함에서 조회 안 됨(파편화) | MED | StoreProductionMaterialsPage(KC) generated only |
| R4 | 분석 `상품 성과`/`콘텐츠·채널 성과`, `노출 설정` 라우트 부재 — 트리 기대치와 실제 갭 | LOW | storeMenuConfig 미포함, route 없음 |
| R5 | 제품 row action(콘텐츠 진입)이 KPA만 실증 — KC 미검증 | LOW | StoreLocalProductsPage 행 액션(KPA) |
...
| 3 | `WO-O4O-STORE-LIBRARY-CROSSSERVICE-PHASE2-COMPLETE-V1` | 자료함 KC 자료 등록/삭제 + 제작 결과물 통합(QR/블로그) | KC | KPA 재사용 | MED | KPA 구현 승격 |
| 4 | `WO-O4O-STORE-ACTIVATION-PRODUCT-ROW-ACTIONS-V1` | 제품 row action(상품설명/POP/QR/블로그/활용자료) KC 검증·통일 | KC | KPA 재사용 | LOW | — |
| 5 | `WO-O4O-STORE-ASSET-DERIVATION-FRONTEND-CROSSSERVICE-V1` | derivation viewer KC 확장(백엔드 공통) | KC | 백엔드 재사용 | LOW | Phase2 완료 |
| 6 | `WO-O4O-COSMETICS-SIGNAGE-VIDEO-SCHEDULE-TABS-V1` | KC 사이니지 동영상/스케줄 탭 구현(KPA 정합) | KC | KPA 재사용 | MED | R1 정리 후 |
| 7 | `WO-O4O-STORE-CREATION-ENTRY-UX-CONSISTENCY-V1` | 진입 UX 통일(KC QR 생성 버튼 등 정합) | KC(주) | 정리 | LOW | — |
| 8 | `WO-O4O-STORE-TRADE-APPLICATION-FLOW-CONSOLIDATION-V1` | 거래 신청 흐름 서비스 간 정합(KPA 우회 전용) | KPA/정리 | MED | — |
...
- 상품·거래: `web-kpa-society/.../PharmacyB2BPage.tsx`, `StoreOrdersPage.tsx` · `web-k-cosmetics/.../store/StoreOrdersPage.tsx`
...
- 사이니지: `digital-signage-core/.../SignageMedia|SignagePlaylist|SignagePlaylistItem.entity.ts`; KPA `StoreSignagePage.tsx` `signage/StoreSignageMainPage.tsx`, KC `store/StoreSignagePage.tsx`; **RISK** `dropshipping-cosmetics/.../signage-playlist.entity.ts:11-14`, `campaign.entity.ts:33-37`
