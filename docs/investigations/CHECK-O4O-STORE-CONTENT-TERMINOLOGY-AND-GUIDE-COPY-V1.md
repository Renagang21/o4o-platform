...
> **결과: PASS — store-hub 복사 액션 7개 페이지 Download→Copy 아이콘 교체 + content 3서비스 decoupling 안내(infoTextAfter) 추가. typecheck(5) 0 errors.**
...
| 서비스 | 파일 | 위치 |
|--------|------|------|
| `pages/hub/HubContentListPage.tsx` | — |
| `pages/hub/HubBlogLibraryPage.tsx` | ActionBar bulk-import |
| `pages/hub/HubPopLibraryPage.tsx` | ActionBar bulk-import |
...
| `에 별도 사본으로 저장됩니다. 원본이 수정·삭제되어도 내 약국 사본은 영향받지 않습니다.` |
...
- `HubBlogLibraryPage`(아이콘) · `HubPopLibraryPage`(아이콘).
...
- **TypeScript 0 errors:** `shared-space-ui` · `web-k-cosmetics` · `web-kpa-society` · `web-neture`(shared 소비처) **각각 0**.
- **정적:**
...
**PASS.** 복사 액션 아이콘(Download→Copy) 7페이지 + content decoupling 안내 3서비스 정렬. API/DB/route/복사 구조/dedup 무변경, typecheck(5) 통과.

## 9. 후속

1. `WO-O4O-STORE-HUB-CONTENT-BROWSE-COMPONENT-EXTRACTION-V1` — KPA `HubContentLibraryPage` vs KCos near-identical browse wrapper 공통 추출.
...
*Date: 2026-06-15 · WO-O4O-STORE-CONTENT-TERMINOLOGY-AND-GUIDE-COPY-V1 · 복사 아이콘(Download→Copy) 7페이지 + content decoupling 안내 2서비스 PASS. 라벨/용어 공식 공간명 유지. API/DB/구조/dedup 무변경. typecheck(5) 0.*
