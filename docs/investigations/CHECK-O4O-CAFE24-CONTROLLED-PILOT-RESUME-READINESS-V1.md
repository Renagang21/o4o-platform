...
> **판정**: **RE_AUTH_REQUIRED · Pilot 표본·등록데이터 준비 완료**
> **일자**: 2026-09-04
...
| Phase B 연결 복구 | **코드로는 불가** — Cafe24 재승인 필요 (사용자 행동 1건) |
| Phase C products/variants 계약 재확인 | **보류** — access token 을 얻지 못해 실호출 불가 |
| Phase D ProductMaster 표본 선정 | 완료 — **30건 / 7개 실험군** |
| Phase E Cafe24 등록용 데이터 준비 | 완료 — `C:/tmp/cafe24-pilot/` 4파일 (repo 밖) |
| 후속 WO 자동 실행 | 없음 — 사용자 지시 대기 |

**DB write 0.** refresh 시도가 실패했으므로 `cafe24_connections` 갱신도 발생하지 않았다.
`product_masters` / `product_identifiers` 는 SELECT 만 했다. Cafe24 상품 원장 복제 0.
secret/token 실제 값은 이 문서·산출물·로그 어디에도 없다.

---

## 1. Phase A — 현재 연결 상태 (실측)
...
## 2. Phase B — 복구 시도 결과 = WO §4 경우 2 (재승인 필요)
...
> refresh token 수명은 14일이다 (발급 8/20 → 만료 9/3). 앞으로도 **2주 넘게 방치하면 매번 재승인**이 필요하다.
> 자동 갱신 잡을 둘지는 이번 WO 범위 밖이며 별도 판단이다 (§7).

---

## 3. Phase C — 보류 (측정하지 못한 것을 측정했다고 적지 않는다)

`products/count` · `products` · `products/{product_no}/variants` 실호출은 **하지 못했다.** access token 이 없기 때문이다.
따라서 다음 4가지는 이번에 **확인되지 않았다**:

1. 현재 API 버전(`2026-03-01`)의 `products` 응답 key 집합이 선행 census(84개)와 같은지
2. `barcode` key 부재가 지금도 그대로인지 (**선행 census 의 관측이며 이번 세션에서 재확인하지 못했다**)
3. `mall.read_product` 만으로 `variants` 가 조회되는지 — **WO §11 중지 조건 후보이나 아직 판정 불가**
4. `custom_variant_code` 가 variants 응답에 실리는지

재승인 직후 이 4가지가 Pilot 의 첫 측정 항목이다.
...
| `cafe24-pilot-upload-paste-v1.csv` | Cafe24 엑셀 양식에 **붙여넣기용** 30행 (상품명 / 자체상품코드 / 제조사 / 브랜드 / 판매가 / 진열·판매상태 / 옵션명 / 옵션값 / 품목자체코드 / PILOT_GROUP) |
...
## 6. 남은 사용자 행동 (2건 · 둘 다 Cafe24/O4O 화면)

1. **재승인** — §2-1. **필수**. 이것이 없으면 Pilot 자체가 진행 불가.
2. **상품 30건 등록** — 엑셀 양식을 전달해 주시면 붙여넣기 완성본을 만들어 드린다.
   양식 없이 진행하려면 `cafe24-pilot-upload-paste-v1.csv` 를 참고해 관리자에서 올린다.
...
`WO-O4O-CAFE24-PRODUCT-MATCHING-CONTROLLED-PILOT-V1` 은 **재개 가능하다.**
코드 · 자격정보 · 라우트 · census 러너는 모두 살아 있고, 막힌 것은 만료된 token 하나뿐이다.
그 WO 의 §2-1(사용자 사전작업)에 필요한 표본이 이번에 준비됐다.

재승인 + 상품 등록이 끝나면 그 WO 를 **별도 지시로** 실행한다 (이번 WO 는 자동 실행하지 않는다 — WO §13). 첫 측정 순서:

1. `products` 응답 key 재census (`barcode` 부재 재확인)
2. `variants` 조회 가능 여부 — **scope 확대 금지 · 불가하면 중지·보고**
3. A~G 군별 사다리 정확도
4. 권장 매칭키 확정
...
- 코드 변경 **0**. 이번 커밋은 본 CHECK 문서 1개뿐이다. migration · schema · dependency 변경 없음.
- 임시 probe 스크립트(`tmp-cafe24-resume-probe.ts`)는 실행 후 삭제 확인. 산출물은 전부 repo 밖(`C:/tmp/cafe24-pilot/`).
...
- **문서 정합**: 발견 1건 / SUPERSEDED 표기 0건 / 링크 수정 0건 / 별도 WO 제안 1건
  - 발견: 선행 CHECK-…-CENSUS-RESUME-V1 §1-1 의 "승인 대기 2건"(CI env 정본화 · 임시 secret 은퇴)은 이미 처리 완료 상태다(§1). 기록물이므로 원문은 수정하지 않았다 (CLAUDE.md §16-1).
