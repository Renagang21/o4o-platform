...
| Phase I 2차 반려 원인 확정 · v4 | **완료** — 원인 `유효기간 사용여부` 공백 불가. 30건 전부 `N` 명시한 v4 생성. §11 |
| Phase J 3차 반려 원인 확정 · v5 | **완료** — 원인 `배송정보=T`(개별설정)가 배송비 선결제 설정을 필수화. 몰 기본 배송정책 `F` 로 전환한 v5 생성. §12 |
...
## 11. Phase I — 2차 반려 원인 확정 · v4 생성 (2026-09-04)

### 11-1. 원인

v3 업로드 결과 **30/30 반려**. 오류 메시지:

```text
유효기간 사용여부에 올바르지 않은 값을 입력하였거나 입력형식을 벗어났습니다.
```

원본 양식 샘플 실측: `유효기간 사용여부` 는 `Y` / `N` 값을 요구하며, 공백은 허용되지 않는다.
`Y` 인 행은 `유효기간` 을 `2015-06-08~2017-03-11` 형식으로 함께 입력한다.

v3 는 이 열을 공백으로 두었다. **Cafe24 신규등록 CSV 는 "공백처럼 보여도 기본값을 명시해야 하는 열"이 존재한다**는
것이 `공급가`(§10)에 이어 두 번째로 확인됐다.

### 11-2. 조치

이번 Pilot 은 유효기간 기능을 쓰지 않으므로 30건 전부 `N` 으로 명시한다.

| 열 | v3 | v4 |
|---|---|---|
| 유효기간 사용여부 (58) | (공백) | `N` × 30 |
| 유효기간 (59) | (공백) | (공백 유지) |

러너: `C:/tmp/cafe24-pilot/build-upload-v4.py` · 산출: `cafe24-pilot-upload-v4.csv`

### 11-3. 업로드 전 validation (`validation-v4.txt`)

| 항목 | 결과 |
|---|---|
| 헤더 라인 바이트 일치 · BOM · CR | 일치 · 유지 · 0 |
| 열 개수 / 90열 위반 행 | 90 = 90 / 0 |
| 데이터 행 | 30 |
| 유효기간 사용여부 == `N` | **30/30** |
| 유효기간 blank | **30/30** |
| 공급가 blank / 판매가 blank | 0 / 0 (§10 유지, 8000 / 10000) |
| **v3 대비 변경된 열** | **`유효기간 사용여부` 30행뿐 — 그 외 0** |
| 상품코드(00) 공백 / 자체 상품코드 | 30/30 / 13건 중복 0 |
| 옵션 3행 문법 | 유지 |
| 모델명 / 상품명(관리용) | 18/30 / 12/30 |
| A~G 분포 | A5 B5 C5 D5 E4 F3 G3 = 30 |

여전히 값 없음: 상품분류 번호 · 자체분류 코드 · 제조일자 · 출시일자 · 상품가 (몰별 마스터 값이라 추측 입력하지 않는다).

### 11-4. 남은 사용자 행동

`C:/tmp/cafe24-pilot/cafe24-pilot-upload-v4.csv` 업로드 → **성공/실패 숫자만**.
또 전량 반려되면 **첫 오류 메시지 1개만** 받아 그 열 하나만 닫는 방식(v5)을 반복한다.

---

## 12. Phase J — 3차 반려 원인 확정 · v5 생성 (2026-09-04)
...
```text
배송정보가 개별설정(T)인 경우, 배송비 선결제 설정은 필수입력 항목 입니다.
```

v2 설계 때 `배송정보`(66)에 `T`(개별설정)를 넣은 것이 원인이다. 개별설정을 택하면
`배송비 선결제 설정`·`배송비 구분`·`배송비입력` 등 배송정책 열이 연쇄적으로 필수가 된다.
이번 Pilot 은 배송정책이 아니라 ProductMaster 매칭이 목적이므로 개별설정 자체를 쓰지 않는다.

### 12-2. 조치 — 몰 기본 배송정책 사용

원본 양식 **샘플 2행**이 정확히 이 패턴이다: `배송정보=F` + 개별 배송 열 전부 공백.
...
| 배송방법 (67) | (공백) | (공백) | (공백 유지) |
| 배송지역 (69) | (공백) | (공백) | (공백 유지) |
| 배송비 선결제 설정 (70) | (공백) | (공백) | (공백 유지) |
| 배송기간 (71) | (공백) | (공백) | (공백 유지) |
| 배송비 구분 (72) | (공백) | (공백) | (공백 유지) |
| 배송비입력 (73) | (공백) | (공백) | (공백 유지) |
| 국내/해외배송 (68) | `A` | `A` | `A` (유지) |

러너 `build-upload-v5.py` 는 개별 배송 열이 실제로 공백인지 `assert` 로 확인한 뒤 66번 열만 바꾼다.
**배송비 값을 새로 설계하지 않는다.**

### 12-3. 업로드 전 validation (`validation-v5.txt`)

| 항목 | 결과 |
|---|---|
| 헤더 라인 바이트 일치 · BOM · CR | 일치 · 유지 · 0 |
| 열 개수 / 90열 위반 행 / 데이터 행 | 90 = 90 / 0 / 30 |
| 배송정보 == `F` | **30/30** |
| 배송방법·배송지역·배송비 선결제 설정·배송기간·배송비 구분·배송비입력 blank | 각 **30/30** |
| **v4 대비 변경된 열** | **`배송정보` 30행뿐 — 그 외 0** |
| 공급가 / 판매가 (§10 유지) | blank 0 / 0 · 8000 / 10000 |
| 유효기간 사용여부 `N` / 유효기간 blank (§11 유지) | 30/30 / 30/30 |
| 상품코드(00) 공백 / 자체 상품코드 | 30/30 / 13건 중복 0 |
| 옵션 3행 문법 · 모델명 · 상품명(관리용) | 유지 · 18/30 · 12/30 |
| A~G 분포 | A5 B5 C5 D5 E4 F3 G3 = 30 |
...
| 1 | 공급가 | 숫자 (판매가 이하) | v3 |
| 2 | 유효기간 사용여부 | `Y` / `N` | v4 |
| 3 | 배송정보 | `F`(공통) / `T`(개별, 후속 열 필수) | v5 |

여전히 값 없음: 상품분류 번호 · 자체분류 코드 · 제조일자 · 출시일자 · 상품가.

### 12-5. 남은 사용자 행동

`C:/tmp/cafe24-pilot/cafe24-pilot-upload-v5.csv` 업로드 → **성공/실패 숫자만**.
전량 반려면 **첫 오류 메시지 1개만** 받아 그 열 하나만 닫는 방식을 반복한다.
