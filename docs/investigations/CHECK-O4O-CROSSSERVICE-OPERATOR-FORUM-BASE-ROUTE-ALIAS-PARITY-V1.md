...
> KPA / K-Cosmetics operator **Forum 기본 진입(`/operator/forum`)** 동작 통일 검증.
>
> 선행:
...
- K-Cos 는 분석(`forum-analytics`)이 아니라 **포럼 신청 관리(forum-requests)** = 첫 actionable 화면으로 redirect (WO 권장 기준 충족).
...
| redirect 대상 | K-Cosmetics |
| --- | --- |
| `/operator/forum-requests` | ✅ `App.tsx:724` — `<Route path="forum-requests" element={<ForumRequestsPage />} />` |
...
1. `forum-analytics` 공통화 — ForumAnalyticsPage(KCos) · ForumAnalyticsDashboard(KPA) 공통 컴포넌트 추출.
2. 포럼 운영(hub) / 포럼 목록 관리(forum-categories) K-Cosmetics 이식 — route/page 신설 동반, 사업 필요성 판단 선행. 이식 완료 시 K-Cos 의 `/operator/forum` redirect 를 실 hub 로 승격.
...
- ✅ K-Cos redirect 대상 = 포럼 신청 관리(forum-requests), 분석 화면 미사용
