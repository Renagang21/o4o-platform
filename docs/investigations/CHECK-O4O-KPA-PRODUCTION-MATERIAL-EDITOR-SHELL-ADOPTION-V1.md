...
> **판정: PASS (by investigation — adoption 보류).** KPA `ProductionMaterialEditorPage` 는 공통 `ProductionMaterialEditorShell`(KCos)와 **(1) 제작 유형 catalog 4종 vs POP/QR 2종, (2) 테마 색상(primary #2563EB vs #3b82f6) 차이** 로 인해, 무손실 adoption 시 shell 에 `productionTypes` + `colors` 2개 prop 확장 + 전 style 테마화(갓 배포된 KCos pixel-parity 위험)가 필요 → WO §6.3/§12 대로 **코드 변경 없이 후속 분리.** 어댑터(createAsset/getAccessToken/findTemplate/notify)는 호환 확인됨.
...
1. `productionTypes` prop 주입 + `selectedType` key 타입을 `'pop'|'qr'` → 제네릭(string/ProductionTarget) 일반화. (KPA 4종 / KCos 2종)
2. `colors` 테마 prop 주입 + shell 의 모든 inline hex(styles ~15곳 + JSX 2곳)를 colors 파라미터화. 기본값을 현 hex 와 **정확히 일치**시켜 KCos pixel-parity 유지(방금 배포된 코드 — 회귀 위험).

→ WO §6.3("production type이 크게 다름") + §10.3(테마/도메인 스타일 shell 주입) 해당. prop 2개 추가 자체는 임계(15) 미만이나, **전 style 테마화 리팩터 + 갓 배포된 KCos pixel-parity 재검증 위험**이 "가장 작은 안전 변경" 범위를 넘음. WO 종료 지침: "다르면 '다르다'는 결론만 남기는 것도 정상 PASS."
...
- 코드 변경 **0**. 본 CHECK 문서만.
- store-ui-core / KCos / KPA editor **미변경**. shell props **미변경**(zero-@o4o 유지).
...
- 코드 변경 없음 → typecheck 비대상. 직전 commit(`64fdb911e`)에서 store-ui-core/KCos EXIT 0 확인됨(KCos 회귀 영향 없음 — 본 WO 무변경).
...
- 신규 dependency 0, API/DB/schema/route 무변경, KCos 회귀 0, store-ui-core zero-@o4o 유지.

## 9. 후속 WO 후보

1. **WO-O4O-PRODUCTION-MATERIAL-EDITOR-SHELL-THEME-AND-CATALOG-PROPS-V1** — shell 에 `productionTypes`(제네릭 key) + `colors`(테마 토큰, 기본=현 hex) prop 확장 후 KPA wrapper 전환. KCos 기본값 pixel-parity 검증 포함. (KPA convergence 를 원할 때)
...
*Date: 2026-06-16 · KPA editor shell adoption · PASS(by investigation, 보류) · KPA = catalog 4종(pop/qr/blog/product-description) + 테마 primary #2563EB vs shell POP/QR 2종 + #3b82f6 → 무손실 adoption 에 productionTypes+colors prop 확장+테마 리팩터 필요(KCos pixel-parity 위험) → 후속 분리 · 코드/dep/DB 무변경 · KCos 회귀 0.*
