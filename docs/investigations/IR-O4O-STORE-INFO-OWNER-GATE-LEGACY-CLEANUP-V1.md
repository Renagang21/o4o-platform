...
> **service-specific 회원 테이블의 `subRole`** 로 인가하는데, 이는
> `/store/info` 라우트 가드가 쓰는 **canonical role** 와 다른 소스다.
...
| business-info GET/PATCH | — | ❌ 403 (해당 row/subRole 부재 시) |
...
**`WO-O4O-STORE-INFO-OWNER-GATE-CANONICAL-ROLE-ALIGN-BACKEND-V1`** (KCos):

business-info GET/PATCH 인가를 **라우트 가드와 동일한 canonical 소스**로 정렬한다.
- 방안 A(권장): `req.user.roles` 에 admin / super_admin 포함 여부로 인가 — route guard 와 동일 기준.
- 방안 B: canonical `service_memberships`/`role_assignments` 로 store_owner 해석.
- ❌ 비권장: `cosmetics_members` 에 subRole **DB backfill** — CLAUDE.md §1(UI 정책을 DB backfill 로 해결 금지) 위반, legacy 소스 영속화.
...
검증: api-server tsc + KCos `/store/info` 승인 계정 통과 + 권한 없는 계정 차단 + console/4xx(business-info 200) smoke.
...
- `cosmetics_members` 테이블 자체의 용도(회원 등록/staff 구분)는 본 IR 범위 외 — business-info 인가만 canonical 로 교정.
