...
| # | 절차 | 기대 |
|---|------|------|
| 1 | `/content` 검색 입력 노출 | 목록 상단 검색창 |
| 2 | 제목 검색 | 문서형 섹션 결과 갱신, `?search=` 반영 |
| 3 | 태그 검색 | 태그 매칭 결과 표시(서버 `tags::text ILIKE`) |
| 4 | 작성자 검색 | 결과 표시 |
| 5 | 결과 없는 검색어 | `검색 결과가 없습니다` (전체 목록과 구분) |
| 6 | 검색어 삭제 | 전체 목록 복귀, `?search=` 제거 |
| 7 | "검색 결과 전체 보기" | `/content/documents?search=X` 진입 + 검색 적용 상태 |
| 8 | 설문 인계 | `/content/surveys?search=X` 진입 + 검색 적용 |
| 9 | 회귀 | 상세 진입 · 내 자료함 가져가기 · restricted 표시 · 자료실/포럼 검색 정상 |
| 10 | 콘솔 | 오류 0 |
