...
- **`order_type` enum**(계약 §3): `GENERIC`(기본)/`DROPSHIPPING`/`COSMETICS`/`TOURISM`. `checkout_orders.order_type` 컬럼은 마이그레이션(`1736950000000-AddOrderTypeToCheckoutOrders`)으로 존재.
...
| 차원 | checkoutService(dropshipping) | K-Cosmetics retail | LMS payment |
| ------ | ------------------------------ | -------------------- | ------------- |
| 생성 경로 | `checkoutService.createOrder` (canonical) | off-contract `EcommerceOrder` 직접(POST `/cosmetics/orders`) | `EcommerceOrder` 직접 |
| 당사자 | buyer + **supplierId(필수)** + seller + partner | buyer + **sellerId(=store)**, supplier 미지정 | buyer(수강생) |
| 서비스 식별 | order_type/metadata | `metadata.serviceKey='cosmetics'` (+ order_type=COSMETICS 기대) | lms |
| channel | 없음 | **local/travel** (metadata) | 없음 |
| shippingFee/discount | **강제 0** (createOrder line 120-121) | **보존 필요** (DTO 입력) | N/A |
| items | jsonb `{productId,productName,quantity,unitPrice,subtotal}` | sku/discount/options/productSnapshot 포함 | N/A |
| status | CheckoutOrderStatus(created/pending_payment/paid/refunded/cancelled) | EcommerceOrder OrderStatus(+CONFIRMED) | 동 |
| 결제 | `checkoutService.completePayment/refundOrder` | `cosmetics-payment.controller`+`KCosmeticsPaymentEventHandler`(EcommerceOrder) | LmsPaymentEventHandler(EcommerceOrder) |
...
- 진입은 의미별 분리: `createOrder()`(dropshipping, 기존 계약 불변) / `createRetailOrder()`.
