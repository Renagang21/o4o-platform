...
> **목표:** K-Cosmetics 에 KPA 와 동일한 **회원 작성 콘텐츠**(`/content`) 풀세트를 백엔드 → 프론트 순으로 정렬한다.
> 현재 KCos 백엔드에는 회원 write API 가 없고 operator 자료실(`/operator/resources`) CRUD + read-only `GET /contents?sub_type=resource` browse 만 존재한다.

---

## 0. 배경 / 타당성 (조사 확정)

| 항목 | KPA (reference) | K-Cosmetics |
| ------ | ----------------- | ------------- |
| 회원 목록 `GET /contents` | ✅ ([kpa.routes.ts:1410](../../apps/api-server/src/routes/kpa/kpa.routes.ts#L1410)) | read-only only |
...
| 추천 `POST /contents/:id/recommend` | ✅ (L1731) | ❌ (table 없음) |
...
- `cosmetics_contents` 는 `blocks`(jsonb) 만 있고 **`body` / `content_type` 컬럼이 없다.**
...
### A-1. KCos contents 라우터 팩토리에 회원 핸들러 미러링
...
- 감사로그(`writeAuditLog`)는 KPA 고유 — KCos 팩토리에서는 생략.
...
*Created: 2026-06-14 · Status: Phase A 완료(배포·마이그레이션·smoke PASS) · Phase B KCos 구현 완료(documents-only)*
