...
- **최소 API 조합 (문서 기준 · 라이브 필드 검증은 Secret 등록 후로 분리):**
  ...
  2. **식약처 묶음의약품정보서비스** (data.go.kr `15063908`) — 체인의 마지막 고리를 **단일 API로** 제공.
     기본 제공 항목: `대표 품목기준코드 · 대표 제품명 · 대표 주성분 · 대표 함량 · 대표 업소명 · 대표 심평원 주성분코드 ·
     대표 심평원 제품코드 · 대표 ATC코드` + 묶음 구성원(`품목기준코드 · 제품명 · 업소명 · 심평원 제품코드`).
     = **주성분코드 + 성분 + 함량 + 동일성분 제품군 목록**을 한 번에 주고 식약처↔심평원 코드를 잇는다.
...
- **health.kr 의존 제거 가능성(예비 판정):** health.kr 4개 intent(약품검색·동일성분·낱알식별·상세)가 모두 공공 API로 대응된다.
  특히 robots Disallow 였던 동일성분 페이지(`result_sunb`)는 묶음의약품정보서비스가 정면으로 대체한다.
