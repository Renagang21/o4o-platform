...
현재 `opsFailed` 는 주문·재고·정산 3개만 묶어 하나의 문구로 안내한다.
이번 작업에서 **어느 영역이 실패했는지 구분**되도록 정비한다.

```text
처리 필요 카드가 실패한 영역의 수치를 0 으로 표시하지 않을 것
실패 영역은 수치 대신 불러오지 못했음을 표시하거나 카드에서 제외
핵심 운영 KPI 카드도 동일 기준 적용
```
...
변경 전 각 API 의 실패 반환값
변경 후 실패 계약과 상수명
상세 조회(getOrderById 등) 처리 방침과 근거
각 소비처의 4상태 분리 결과
대시보드 실패 영역 구분 방식
공유 컴포넌트 2경로 검증 결과
...
부분 반영 시에도 "API 만 바꾸고 소비처를 두는" 상태로 종료하지 않는다.
