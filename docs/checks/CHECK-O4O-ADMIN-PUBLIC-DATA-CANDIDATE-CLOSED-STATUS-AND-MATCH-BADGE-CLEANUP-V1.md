...
### 4.1 실브라우저 smoke (배포 후 수행)
admin 은 로그인 게이트 화면이므로 push→CI 배포 완료 후 수행. 확인 항목:
1. `/admin/o4o-product-db/candidates` 진입 시 기본이 **등록 전** 목록.
2. 등록 전 후보 `기본상품 매칭` = "기존 기본상품 없음"/"식별자 충돌 검토", **`unmatched` 원문 미노출**.
3. `등록 완료`/`제외` 필터로 종료 후보 조회 시 `기본상품 매칭` = `—`.
4. 식별자 있는 후보(의약품) 상세 드로어 = 식별자/충돌 섹션 표시. 식별자 없는 후보(의료기기) = 해당 섹션 숨김.
5. 일반 후보 상세 제목 = "후보 상세", 충돌 후보 = "후보 상세 · 충돌 검토".
