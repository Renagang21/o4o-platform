...
- 판정 요약: **backend 는 이미 완전 공통(단일 EventOfferService × service_key)**, KCos 탐색 화면도 이미 공통 컴포넌트 사용 중.
...
- **단일 서비스**: `apps/api-server/src/routes/kpa/services/event-offer.service.ts` (1,601L) 를 KPA·KCos·Neture 컨트롤러가 **service_key 인자만 바꿔** 호출한다. 코드 복사 없음 → backend 는 이미 공통화 완료.
...
- **서비스 키 2체계**: platform key(`kpa-society`/`k-cosmetics`) ↔ event offer key(`kpa-groupbuy`/`k-cosmetics-event-offer`).
  변환은 `constants/event-offer-service-mapping.ts` 의 `TARGET_TO_EVENT_OFFER_KEY` 단일 지점.
- **엔드포인트**: KPA `/api/v1/kpa/groupbuy/*` · KCos `/api/v1/cosmetics/event-offers/*` · Neture `/api/v1/neture/event-offers/*`.
...
| 후보 | 판정 |
|---|---|
| 매장 탐색 목록 화면(KCos) | **이미 공통** — `@o4o/store-ui-core` `EventOffersHubList`. 재작업 안 함 |
| KPA 탐색 화면 vs KCos/**near-identical 아님** — KPA 는 4탭 상태 필터 · 운영자 stats · 검색/공급업체 필터 · 공급업체 묶음 담기 패널을 가진 상위 집합. 얇은 목록으로 축소하면 기능 손실이므로 통합하지 않음(§4 "거대한 조건문 컴포넌트 금지") |
| `src/utils/eventOfferCart.ts` (KPA/KCos) | **실중복** — uuid 검증 + payload 조립이 동일. 서비스 고유는 serviceKey·가격 우선순위뿐 → 공통화함 |
...
| 항목 | 결과 |
|---|---|
| `npx tsc -b` KPA / K-Cosmetics / PharmacyHub | 4/4 PASS |
| `npx vite build` KPA / K-Cosmetics / PharmacyHub | 4/4 PASS |
...
2. **KPA 탐색 화면 971L** — 상태 탭·필터·선택·묶음 담기 상태기계는 향후 `useSupplyProductList` 계열 headless Core 로 흡수할 여지가 있으나, 이번엔 KCos 와 near-identical 이 아니라 보류했다.
3. **`EnrichedEventOffer` 타입이 backend 보다 좁다** — KCos 의 status union 에 `upcoming`/`sold_out`/`rejected` 가 빠져 있다(런타임엔 올 수 있음). 타입만의 문제라 이번 범위 밖.
