...
KPA(`kpa.routes.ts`) 와 동일하게 `'/stores'` 로 정렬. 로직 변경 없음.
...
### D3. 운영자 회원 콘솔 모바일 가로 스크롤 (KPA · K-Cosmetics 공통)
...
(① Neture AI/알림 엔드포인트 서비스 운영자 스코프 정책 ② PharmacyHub 승인 E2E 용 테스트 가입 신청 데이터 시드)
