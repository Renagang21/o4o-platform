...
> K-Cosmetics·**Neture** 공개 푸터를 정적 하드코딩 → serviceKey 기반 동적 법정정보(public
...
> **결과: CODE PASS** (tsc KCos/Neture 0 + build 0). 배포 후 브라우저 smoke 예정. — 2026-06-12
...
KCos `/terms`·`/privacy` + Neture 기존 CMS `/terms`·`/privacy`. 모든 선행 WO 기반 위에 푸터만 연동.
...
## 4. 서비스별 적용 여부
- **KCos**: 동적 법정정보 블록 추가(기존 법정정보 없음 — 순수 추가). + 푸터에 `/service-guide`·`/terms`·`/privacy` 링크 추가.
...
- 색상 `inherit` 으로 각 푸터(밝은 Neture / 어두운 KCos)에 맞춰 상속.

## 9. 링크 연결 결과
- KCos 푸터: `/service-guide`(서비스 안내) + `/terms`(이용약관) + `/privacy`(개인정보처리방침) + 기존 `/contact` 유지.
...
*Date: 2026-06-12 · Status: CODE PASS. KCos/Neture 동적 전환(Neture 하드코딩 제거), KPA 제외(후속 IR).*
