...
대상 5개 서비스: `web-neture` · `web-kpa-society` · `web-k-cosmetics` · `web-pharmacy-hub`
...
| 축 | neture | kpa-society | k-cosmetics | pharmacy-hub |
| --- | --- | --- | --- | --- |
| 1. 정적 죽은 링크 | 0 (HOLD orphan 파일 내부 3건 제외) | 0 (splat `/admin`·`/operator`·`/tablet` 오탐) | 0 | 0 |
| 1'. 동적(템플릿) 링크 | 28 prefix / 실결함 0 | 42 / 0 | 16 / 0 | 6 / 0 |
| 2. header·footer 링크 | 정상 | 정상 | **[글쓰기]→`/forum` 2건** | 정상 |
| 3. NotFound UX | **canonical** | 비표준 (App.tsx inline·`<a href>`) | 비표준 | **canonical** |
| 4. guard fallback | 안내 화면 | 안내 카드(🔒 접근 권한이 없습니다) | `/login` redirect | MembershipGate 안내 |
| 5. API 실패 위장 | 10 | 14 | 8 | 0 |
| 6. orphan 잔여 | 0 (선행 WO 로 정리) | 0 | 0 | 0 |
| 7. 공통 레이아웃 | 서비스별 자체 | 서비스별 자체 | 서비스별 자체 | 서비스별 자체 |
...
| H3 | — | **서비스 정책 판단 필요.** `RoleGuard` 가 `deniedRedirect: '/'` 로 고정되어 있어(코드 주석: "안내 카드 계약이 없다") 권한 없는 사용자는 아무 안내 없이 홈으로 튕긴다. 안내 화면 렌더로 바꾸면 해당 서비스의 **모든** guarded route 동작이 바뀌므로 문구 통일 범위를 넘는다 |
...
| C2 | **guard 거부 화면(AccessDenied) 공통 컴포넌트** | 판정 순서는 이미 `@o4o/auth-react` `createRouteGuard` 로 공통화되어 있고 서비스별 차이는 `renderDenied` / `deniedRedirect` 주입뿐이다. |
| C3 | **포럼 링크 규약** | 게시글 상세 경로가 서비스마다 `/forum/post:id`(neture·kpa·kcos) vs `/forum/posts:id` 로 갈린다. 이번엔 각 서비스 내부 정합만 맞췄다. 규약 통일은 route 변경이라 별도 판단 필요 |
...
| run | commit | 배포 대상 |
|---|---|---|
| `31454971988` | `1ce5e3c68` | k-cosmetics · kpa-society (neture · pharmacy-hub skipped) |
| `31455699000` | `2869cbc37` | k-cosmetics |
...
| 정상 route | — | 강의 목록 3건 · 상세 정상 렌더 (운영자 행 클릭 수정의 도착지 확인) |
| 정상 route | — | 목록 3건 · 상세 정상 렌더 (글 작성 후 이동 경로의 도착지 확인) |
| footer·header 링크 | — | 전부 실 route (`/forum` `/lms` `/business` `/service-guide` `/contact` `/terms` `/privacy`) |
| 내부 링크 | — | 배포 후 `/forum/write` 로 확인 (기존 `/forum`) |
...
| commit | 내용 |
|---|---|
| `1ce5e3c68` | 404 UX 표준화 2서비스 동적 죽은 링크 2건 |
| `17118a1a8` | 운영자 강의 목록 행 클릭을 canonical `/lms/course/:id` 로 정렬 |
| `2869cbc37` | 포럼 [글쓰기] 버튼 → `/forum/write` (k-cosmetics 2) |
| (본 문서) | CHECK 문서 |
