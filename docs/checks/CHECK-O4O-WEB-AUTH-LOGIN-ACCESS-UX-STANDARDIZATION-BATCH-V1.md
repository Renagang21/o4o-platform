...
> 대상: `services/web-neture` · `web-kpa-society` · `web-k-cosmetics` · `web-pharmacy-hub`
> 작성: 2026-08-11
...
| 미인증 → `Navigate(fallback, state.from)` (Core · `StoreOwnerGuard` · `HubGuard`) | `VALID_LOGIN_REDIRECT` | 복귀 경로 보존이 이미 공통 계약 |
| kpa `HubGuard` operator → `/operator` operator → `/operator`, neture `redirectMap` | `VALID_LOGIN_REDIRECT` | 거부가 아니라 **역할별 정상 분기**다. 안내 화면으로 바꾸면 오히려 회귀 |
...
| # | 대상 | 라벨 | 이유 | 다음 방법 |
|---|---|---|---|---|
| H1 | `@o4o/store-ui-core` `StoreOwnerGuard` — `denialFallback = '/'` | `HOLD_AUTH_CONTRACT` | KPA·K-Cosmetics·Pharmacy-Hub 3서비스 공유 guard 이고 `renderDenied` 같은 주입점이 없다. 안내 화면을 넣으려면 공통 패키지 계약 변경이 필요 — WO §5 "다른 서비스 전체 guard 동작을 바꾸는 고위험 변경" | optional `renderDenied` prop 추가 + 소비처 4서비스 전수 확인(CLAUDE.md Shared Module Change Rule) 별도 WO |
...
| H3 | — | `HOLD_POLICY` | 미인증에게 기능 소개를 보여주는 것이 의도된 마케팅 동선. 로그인 안내로 대체하면 정책 변경 | 서비스 소유자 판단 필요 |
...
| R2 | 5개 서비스 홈 | 정상 렌더 | **PASS** — neture / kpa-society / k-cosmetics / pharmacy-hub 전부 정상 |
| R3 | 없는 route | 404 안내 | **PASS** — `404 / 요청하신 페이지를 찾을 수 없습니다.` (직전 catch-all WO 회귀 유지) |
| R4 | — | 역할 분기 정상 | **PASS** — 내 약국 홈 정상 렌더 (역할 분기 redirect 는 거부가 아니므로 미변경) |
