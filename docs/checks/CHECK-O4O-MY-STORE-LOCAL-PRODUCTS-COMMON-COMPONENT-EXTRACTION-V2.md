...
> **성격**: KCos 의 99% 동일 StoreLocalProductsPage 전체를 공통 `StoreLocalProductsManager`로 통합. backend/DB/checkout/cart 무변경, KPA 미변경.
> **결과: PASS — StoreLocalProductsManager(@o4o/store-ui-core) 추출 + KCos thin wrapper 전환. store-ui-core+KCos+KPA typecheck 0.**
> **작성일**: 2026-06-12

---

## 1. 목적
KCos 매장 취급 상품 CRUD 페이지(거의 동일 660줄 × 2)를 service-neutral `StoreLocalProductsManager`로 통합. api client + 문맥 라벨만 주입. StoreLocalProduct = non-order 의미 유지.
...
| 항목 | K-Cosmetics | 처리 |
| ------ | ------------- | ------ |
| api import 경로 | `@/services/localProductApi` | wrapper 에서 주입 |
| 카테고리 placeholder | `예: 스킨케어, 메이크업` | `labels.categoryPlaceholder` prop |
...
| 서비스 | 변경 |
|--------|------|
 `pages/store-management/StoreLocalProductsPage.tsx` | **660줄 → ~25줄 wrapper**. `<StoreLocalProductsManager api={{fetch/create/update/delete}} labels={{categoryPlaceholder:'예: 건강기능식품, 의약품'}}/>` |
...
KPA `StoreLocalProductsPage.tsx`(695줄)는 **BaseTable(@o4o/ui) 기반**으로 list 렌더 구조가 KCos(커스텀 table)와 상이 → 본 V2(Tailwind 커스텀 table manager) 대상 아님. **무변경**(V1 배지 공통만 적용된 상태 유지). KPA 통합은 table primitive/slot 기반 manager 필요 → V3 후보.
...
- **정적**: `StoreLocalProductsManager` index export 확인. KCos 페이지 = thin wrapper(api+labels 주입) 확인. UI/CRUD/모달 동작 보존(manager 로 이동). 주문/장바구니 미혼입.
- **smoke**: 미수행(배포 전) — 동일 코드 이동 + 동일 Tailwind라 시각/동작 동일, tsc 가 api prop 타입 가드. 배포 후 KCos local-products 등록/수정/삭제/배지 렌더 확인 권장.

## 10. 완료 판정
**PASS** — KCos StoreLocalProductsPage(660줄×2) → 공통 `StoreLocalProductsManager` + thin wrapper(~25줄×2). 중복 ~1300줄 → manager 1개로 통합. StoreLocalProduct non-order 유지, 주문/cart 미혼입, backend/DB·KPA 무변경. typecheck(4) 통과.
...
*Date: 2026-06-12 · WO-O4O-MY-STORE-LOCAL-PRODUCTS-COMMON-COMPONENT-EXTRACTION-V2 · StoreLocalProductsManager 통합 + KCos thin wrapper PASS. 660줄×2 → manager 1개. KPA(BaseTable) 미변경(V3). non-order 유지, backend/cart 무변경.*
