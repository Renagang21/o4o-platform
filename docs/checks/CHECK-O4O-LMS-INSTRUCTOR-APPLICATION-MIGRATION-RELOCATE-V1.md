...
> **결과: PASS(코드/적용) — api-server tsc 0, git mv relocate, enum ALTER 제거. 배포 migration job 적용 + public-profile probe 재확인 §6.**
...
## 6. Live 검증 (배포 migration job — 적용 후 기록)
- 배포 Cloud Run Job `o4o-api-migrations` 적용 → typeorm_migrations 등록 기대: `[X] N AddInstructorRoleSupport1739500000000`.
- public-profile probe 재실행 기대: `GET /api/v1/kpa/lms/instructors/{uuid}/public-profile` → **42P01/500 해소** → 404 INSTRUCTOR_NOT_FOUND(더미 UUID) 또는 200.
  - **[적용 후 기록]** ← 배포 완료 시 갱신.

## 7. 완료 기준 체크 (WO §10)
1(원본 확인) ✅. 2(enum ALTER live 불일치 확인) ✅. 3(스캔 dir 에 enum ALTER 제거 버전 배치) ✅. 4(orphaned 원본 제거 — git mv) ✅. 5(CREATE TABLE IF NOT EXISTS 안전) ✅. 6(entity↔컬럼 정합) ✅. 7(api-server tsc 0) ✅. 8(migration job 성공) ◐ 배포후. 9(prod 테이블 생성) ◐ 배포후. 10(probe 42P01 해소) ◐ 배포후. 11(entity/getPublicProfile/route 보존) ✅. 12(CHECK) ✅. 13(path-specific) ✅. 14(다른 세션 무접촉) ✅.
...
*Date: 2026-06-12 · Status: PASS(코드/적용). AddInstructorRoleSupport 스캔 dir 이전(enum ALTER step 제거, table/index 보존), tsc 0. live 강사 public-profile 500(42P01) 해소 목적. migration job 적용·probe 재확인 §6 배포후.*
