...
> **커밋:** 구현 `f951ad841` (backend + frontend 동일 커밋 · §3 #1 요건) · CHECK(본 문서) 후속
> **판정:** **구현 COMPLETE** — 결제 개시부터 공급자 처리까지 연결. 인증 E2E 결제 smoke 는 `AUTHENTICATED_SUPPLIER_SMOKE = PENDING`(§10)
> **중지 조건:** A~H **전부 미발동**(§9)
...
- 이번 WO 는 **새 queue/framework 를 만들지 않았다**(WO §2-F 기본안).
- 현재 상태: ① handler 내 idempotent 재시도(bridge 자체가 `metadata.checkoutOrderId` dedup) ② Pharmacy-Hub operator recovery route(기존) ③ 신규 축은 **같은 bridge 를 타므로 recovery route 를 일반화하면 즉시 적용 가능**한 구조.
- **일반화 route 는 이번에 추가하지 않았다** — 실데이터 `paid` 0건이라 운영 필요가 없고, 추가 시 운영자 권한·감사 계약을 새로 정해야 한다(범위 확대). §11 후속 제안.
...
## 11. 후속 제안 (이번 WO 에서 하지 않은 것)

1. **canonical 문서 정정 WO** — `O4O-B2B-SUPPLIER-TO-STORE-ORDER-CONTRACT-V1` §5-1 · 118행 SUPERSEDE(Q15).
2. bridge recovery route 일반화(Pharmacy-Hub 전용 → 전 producer) — 실결제 발생 후 필요성 재평가.
3. `SupplierUnifiedOrderService` 축소/은퇴 — bridge 안정성 실증 + 프런트 소비처 전환 후.
4. `glycopharm-event-offer` 4건의 출처·소유 서비스 확정.
5. `neture_suppliers.user_id` 복구(Identity data repair · 사용자 승인 필요) — 이것 없이는 어떤 공급자 E2E 도 불가.

## 12. 문서 정합 · Git

`발견 1건 / SUPERSEDED 표기 2건 / 링크 수정 0건 / 별도 WO 제안 5건`
...
- 구현 커밋 `f951ad841`(CI·배포 전부 success 확인 후 CHECK 커밋) · rebase 1회(타 세션 문서 커밋 위로) · reset/force 0.
