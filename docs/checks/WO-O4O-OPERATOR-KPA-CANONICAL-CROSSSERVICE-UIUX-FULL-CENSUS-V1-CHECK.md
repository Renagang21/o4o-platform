...
- **비교 대상**: Neture · K-Cosmetics · PharmacyHub
...
| 축 | 산출 경로 | 결과 |
|---|---|---|
| route | `services/web-kpa-society/src/routes/OperatorRoutes.tsx`, 나머지 4개는 `src/App.tsx` 의 `/operator` 서브트리 | KPA 65 64 · K-Cosmetics 49 · Neture 34 · PharmacyHub 12 |
| menu | `services/*/src/config/operatorMenuGroups.ts` | KPA 37 · Neture 68(admin 혼재) 36 · K-Cosmetics 30 · PharmacyHub 10 |
| page | `services/*/src/pages/operator//*.tsx` | KPA 55 · KCos 45 · Neture 32 · PH 12) |
...
### 2-3. 사이니지 HQ 미디어 — canonical

| | KPA / K-Cosmetics |
|---|---|
| 컬럼 | `☐ · 액션 · 이름 · 타입 · 소스 · 상태 · 생성일` |
| checkbox | 6 / 1 |
| RowActionMenu | ✅ ⋮ |
| 검색 | 공통 SearchBar |
| StatusBadge | `활성` + `HUB 노출 중` 2단 |
| empty state | — (데이터 5건) |
| 코드 | `@o4o/operator-core-ui/modules/signage-hq` thin wrapper 23줄 |
...
blog/POP/QR · store-detail · store-channels · surveys 를
...
| A5 | 가입 신청 상세 | NA | **VD** | NA | **VD** | FC | Neture 1249줄 391줄 자체 구현 |
...
| D3 | 이벤트 오퍼 승인 | **UD** | NA | **VD** | **VD** | NA | KPA 1082줄 / KCos 275줄 221줄 — 3개 전부 별개 구현 |
| D4 | 판매자 모집 노출 승인 | FC | NA | FC | **UD** | NA 만 ux-core Console 직접 호출(91줄) |
| D5 | 매장 가입/승인 | NA | NA | NA | **VD** | NA `StoreApprovals` 421+838줄 자체 구현 |
...
| D8 | 강사 승인(자격) | FC | NA | NA | FC | NA | KPA 529줄 497줄 — **canonical 전체 체인 보유 화면** |

### E. 매장 (4 업무)

| # | 업무 | KPA | Neture | K-Cos | PH | 비고 |
|:--:|---|:--:|:--:|:--:|:--:|:--:|---|
| E1 | 매장 관리 목록 | FC | **VD** | FC | **UD** | **SS** 는 core-ui+ux-core 혼용(192줄). PH 재판정 → §8-A |
| E2 | 매장 상세 | FC | NA | FC | **VD** | NA 419줄 자체 구현 |
| E3 | 채널 관리 | FC | NA | FC | **VD** | NA 408줄 자체 구현 |
...
| F2 | 상품 상세 | NA | NA | **VD** | **VD** | NA | KCos 309줄 387줄, 공통 primitive 0 |
...
| # | 업무 | KPA | Neture | K-Cos | PH | 비고 |
|:--:|---|:--:|:--:|:--:|:--:|:--:|---|
| G1 | HQ 미디어 | FC | NA | FC | **VD** | NA 571+228줄. 브라우저 실측 §2-3 |
| G2 | HQ 플레이리스트 | FC | NA | FC | **VD** | NA 310+101+286줄 |
| G3 | 템플릿 | FC | NA | FC | **VD** | NA 119+187줄 |
| G4 | 강제 콘텐츠 | FC | NA | FC | **VD** | NA 431줄 |

### H. QR / POP / 블로그 / 동영상 / 태블릿 / 다국어 (6 업무)

| # | 업무 | KPA | Neture | K-Cos | PH | 비고 |
|:--:|---|:--:|:--:|:--:|:--:|:--:|---|
| H1 | 매장 HUB 블로그 | FC | NA | FC | **VD** | NA 450+252줄. **컬럼은 실측상 KPA 와 동일** — 시각 수렴·코드 중복 |
| H2 | 매장 HUB POP | FC | NA | FC | **VD** | NA 442+254줄 |
| H3 | 매장 HUB QR | FC | NA | FC | **VD** | NA 476+365줄 |
...
| # | 업무 | KPA | Neture | K-Cos | PH | 비고 |
|:--:|---|:--:|:--:|:--:|:--:|:--:|---|
| I1 | 강의 관리 | FC | NA | FC | **VD** | NI `LmsCoursesPage` 226줄 + 공통 wrapper 27줄 **이중 존재** |
| I2 | 설문조사 관리 | FC | NA | FC | **VD** | NI 224+270줄 자체 구현 |
...
| # | 업무 | KPA | Neture | K-Cos | PH | 비고 |
|:--:|---|:--:|:--:|:--:|:--:|:--:|---|
| J1 | Operator 대시보드 | FC | FC | FC | FC | FC | 5개 전부 `OperatorDashboardLayout`. nav 실측 KPA 10 9 / KCos 10 / Neture 10 / PH 5 |
| J2 | AI 리포트 | FC | FC | FC | FC | **NI** | 4개는 `@o4o/ui` 공통 + `aiReportConfig` 주입 |
| J3 | 운영 분석 | FC | FC | **NI** | **VD** | FC | KCos 는 route·page 자체가 없음. |
| J4 | Action Queue | NA | **SS** | NA | NA | NA | Neture 225줄 |
| J5 | AI 사용량 / 정산 | NA | **SS** | NA | **SS** | NA 394+378+353+891줄 · Neture ai-operations/asset-quality |
...
| K4 | 서비스 설정 | NA | FC | **VD** | **VD** | NA | KCos 161줄 513줄 자체 구현 |
...
| **D 승인** | `ProductApplicationManagementConsole` + `recruitment-exposure` | 이벤트오퍼 2서비스 별개 매장승인 자체 축 · Neture 승인 3종 고유 | **상** | 승인 도메인이 서비스마다 상이. F12 Product Resource 정합 필요 | ❌ |
| **E 매장** | `modules/stores` · `store-detail` · `store-channels` | Neture 자체 · PH 미구현 | **하** | 없음 (KPA·KCos 동일 backend) | — |
| **F 상품/주문** | `OperatorProductStatusPage` · `OperatorOrderStatusPage` | Neture 주문 자체 · 상품상세 공통 모듈 부재 | **중** | Neture=유통 원장 vs 나머지=서비스 상품 현황 | 부분 |
| **G 사이니지** | `modules/signage-hq` ×18 thin wrapper (23~25줄) | — | **하** | — | — |
| **H QR/POP/블로그** | `hub-content-list/write` · `qr-template-write` | KPA 동영상/다국어/태블릿은 KPA 전용 | **하** | **없음** — 실측 컬럼까지 KPA 와 동일 | — |
| **I LMS/설문/문의** | `lms-courses` · `surveys` · `contact-inquiry` | Neture 문의 자체 · KPA 문의 콘솔 없음 | **하** | 없음 | — |
| **J 분석/AI** | `OperatorDashboardLayout` 5-Block · `operator-analytics` · `@o4o/ui` AI 리포트 | KCos 운영분석 **부재** 자체 구현 · Neture AI 고유 축 | **하** | AI 정산·Action Queue(Neture)는 고유 — 공통화 대상 아님 | — |
| **K 역할/감사/법정** | `@o4o/ui` RoleManagement wrapper · `service-legal` | **KPA 만 법정정보 자체 구현** · 감사로그 KPA 전용 · 설정 KCos 자체 | **중** | 감사 로그는 KPA 외 backend 미확인 | 부분 |
...
| 서비스 | 건수 | 업무 |
|---|:--:|---|
| KPA | 2 | I3 문의 관리(설정만 있고 운영자 콘솔 없음 — KCos 는 보유) · L4 mobile |
...
| # | 항목 | 차이 | 필요 선행 작업 |
|:--:|---|---|---|
| 1 | 가입 신청 승인 (A4/A5) | Neture=`registration_requests` · KCos=서비스별 application · PH=`service_memberships` | 승인 대상 엔티티 축 통일 판단. **PH 의 `consoleMode='approval'` 이 유일한 공통 구현체** |
| 2 | 회원 목록 컬럼 (A1) | KPA=`유형/활동 유형/추가 권한` · KCos=`회원 유형/운영 권한` · Neture=`회사명/공급자 프로필` | column config 주입 계약 확장. RBAC role→label 파생 규칙이 서비스마다 다름 |
| 3 | 매장 승인 (D5) 만 `store_approvals` 별도 축 | 공통 승인 모듈 신설 여부 결정 |
| 4 | 상품 (F1/F2) | Neture=유통 원장(ProductMaster/Listing) · KPA/KCos=서비스 상품 현황 | **F12 Product Resource Baseline** 과 정합 필요. 단순 합침 금지 |
| 5 | 이벤트 오퍼 (D3) | KPA 1082줄이 오퍼 생성까지 포함 · KCos 는 승인만 | `EVENT-OFFER-COMMON-DOMAIN-V1` 기준 재정렬 선행 |
| 6 | AI 정산 (J5) 만 `ai_usage`/`invoices`/`settlements` 보유 | 서비스 고유 — 공통화 대상 아님 |
| 7 | signage (G1~G4) | — | **선행 조건 없음. 즉시 교체 가능** |
...
- H1 H2 H3 (블로그/POP/QR) · E2 E3 (매장상세/채널) · I1 I2 (LMS/설문) · J3 (운영분석)
