...
## 8. 사용자 결정 필요 (선행 조사 §8 승계)

| # | 질문 | 선택지 |
|:-:|------|--------|
| **A** | 관리자 비밀번호 재설정(§5-1)의 의미 | (가) 현행 유지 + **무효임을 응답·UI 에 명시** / (나) 재설정 시 해당 사용자 **credential 전부 삭제** → 다음 로그인은 `users.password` fallback / (다) 관리자가 **serviceKey 지정**해 특정 서비스만 재설정 |
| **B** | 서비스별 비밀번호 분리 유지 여부 | 유지(→ A 의 가/다) / 폐기(→ Identity V2 재설계, 대형 WO) |
| **C** | §5-2 감사 로그 결함 | 별도 WO 발주 여부 |
| **D** | 테스트 계정 문서(§6) | 서비스별 비밀번호 컬럼 구조로 개편 여부 |

> A-(나)는 credential 삭제 = 운영 데이터 write → dry-run·모집단 확정·rollback 계약 선행 필수.

**권장:** B=유지, A=(가), D=개편, C=별도 WO.
근거 — 분리는 의도된 보안 경계이므로 되돌릴 이유가 없고, 실제 사용자 피해는 §5-1 의 **사일런트 무효**
하나뿐이며 그것은 "무효임을 알리는" 것만으로 해소된다(정책 변경 불필요).
...
*결과: `HOLD` · P0 아님(설계된 동작) · WO §4 수정안은 보안 경계 제거이므로 적용 금지 · 오류 코드 분리는 이미 구현됨 · 결정 4건(A–D) 대기*
