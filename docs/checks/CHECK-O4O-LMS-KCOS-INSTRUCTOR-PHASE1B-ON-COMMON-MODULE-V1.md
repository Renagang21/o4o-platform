...
> **결과: PASS** — KCos bespoke read-only 테이블(105줄)을 공통 `InstructorCoursesManager`(@o4o/operator-core-ui) 소비로 전환(wrapper **38줄**). **2서비스(KPA/KCos) 강사 목록이 모두 동일 manager** 위에 정렬. manager 에 backward-compatible 옵션(read-only: create/edit/manage/delete 선택화, category/lessonCount/thumbnail 컬럼 토글) 추가 → KPA 무영향. operator-core-ui 모듈 + 3서비스 typecheck 0. KPA wrapper·backend·package.json/Dockerfile·Neture 무변경.
> **선행:** `WO-O4O-LMS-INSTRUCTOR-COURSE-LIST-MANAGER-EXTRACTION-V1`(manager 추출, KPA 적용 / KCos 보류)
...
직전 WO 에서 추출된 공통 `InstructorCoursesManager`(KPA 적용, KCos 보류) 위에 KCos 강사 목록을 올린다. bespoke 신규 구축이 아니라 공통 모듈 build-on. 강사 목록 화면만 대상.
...
**무변경:** KPA 강사 목록 wrapper(manager 옵션 추가가 backward-compatible), backend, DB/migration, Neture, package.json/pnpm-lock/Dockerfile, `@o4o/lms-ui`, 강사 editor/lesson/quiz/assignment.
...
read-only 서비스 지원 위해 `InstructorCoursesManager` 에 추가(전부 optional → 기존 KPA 동작 불변):
...
- **KPA wrapper 미변경**(create/edit/manage/delete 주입 그대로 → 기존 렌더 동일). typecheck 0 으로 무회귀 확인.

## 6. KPA / Neture 미수정 확인

- KPA `CourseListPage` `InstructorCoursesPage` wrapper **미변경**(git status — manager + KCos 만). manager 옵션 추가는 backward-compatible(KPA typecheck 0).
- Neture 미수정, `operator-core-ui`→`@o4o/lms-ui` import 0(Neture transitive 없음). serviceKey hardcode 0.

## 7. 검증 결과

- **TypeScript:** `@o4o/operator-core-ui` instructor-courses 모듈 **0**(패키지 잔존 1건은 `@o4o/error-handling` dep 사전 에러, 무관).
- **정적:** KCos wrapper(38줄)가 `InstructorCoursesManager` 소비, bespoke `<table>` 제거. KPA wrapper 미변경. manager serviceKey/lms client import 0. operator-core-ui→lms-ui 0.
...
**PASS.** KCos 강사 강의 목록을 공통 `InstructorCoursesManager` 위에 build-on(read-only config, 38줄). **2서비스 강사 목록 축이 단일 manager 로 정렬 완료.** manager 옵션은 backward-compatible(KPA 무영향, typecheck 0). serviceKey hardcode·lms-ui 의존·Neture 소비 없음, backend/package/Dockerfile 무변경. KCos Phase 1-B(생성/편집/삭제)는 route·API 추가 시 config 확장만으로 manager 활성화.
