...
배포 완료 후(Web + API Cloud Run deploy ✅), KPA `kpa-society-web` 프로덕션에서 **약국 경영자 체험 계정**(테스트 약국 매장)으로 실제 브라우저 검증 수행.

| # | 항목 | 결과 |
|---|---|---|
| 1 | 관리 화면 상담 토글 라벨 | ✅ `상담 요청 버튼 (후속 기능 · 권장하지 않음)` 로 표시 |
| 2 | 관리 화면 상담 토글 설명 | ✅ "태블릿 V1은 공용 안내 화면으로… 후속 모바일 앱/PWA 기능에서 다룹니다. 기본은 꺼짐…" 로 표시 |
| 3 | 콘텐츠 미연결 폴백 안내 | ✅ 진열 항목에 "이 제품에 연결된 설명 콘텐츠가 없습니다. 태블릿에는 기본 상품 설명이 표시됩니다. 상품별 설명을 별도로 보여주려면…" 표시 |
| 4 | 미리보기 kiosk 마운트 | ✅ 상품 0건/1건 모두 런타임 오류 없이 정상 렌더(변경한 reducer/state 안전) |
| 5 | 상세 뷰 개인정보 input | ✅ 이름/요청사항 input **없음** |
| 6 | 상세 뷰 최종 행동 문구 | ✅ (자체 상품) "이 제품은 매장에서 직접 안내하는 상품입니다. 이 화면을 직원에게 보여주세요." |
| 7 | 상세 뷰 하단 action bar | ✅ "돌아가기"만 노출, "직원에게 안내 요청" 버튼 없음 |

> **검증 데이터 한계**: 접근 가능한 테스트 매장(테스트 약국 매장 / 네뚜레-약국)에 **공급(취급 O4O) 상품 진열 데이터가 없어**, 공급 상품 상세의 supplier CTA("이 제품이 궁금하신가요? / 이 화면을 직원에게 보여주세요")와 상담 버튼 opt-in 게이팅(`=== true`)은 **라이브 진열로 재현하지 못했다**. 자체(local) 상품 상세는 라이브 확인 완료(위 #5~7). 공급 상품 경로는 동일 배포 컴포넌트이며 3개 소비처 typecheck + 코드 리뷰로 커버됨. 검증용으로 생성한 자체 제품은 **진열 제거 + 비활성화로 정리**함.
>
> 공개 API `GET /stores/네뚜레-약국/tablet/settings` 는 기존 설정 row 를 반영(showConsultationButton=true — 기존 row 유지 케이스, WO §6.3). 신규 기본값(false)은 settings row 가 없는 매장에 적용된다.
