...
> KPA 에서 검증한 canonical Store Cart 흐름을 K-Cosmetics buyer 화면으로 확장.
> 브라우저 route·sidebar·empty 렌더 전부 통과.
...
| 항목 | K-Cosmetics |
| ------ | ------------- |
| buyer 화면 | `services/web-k-cosmetics/src/pages/hub/HubEventOffersPage.tsx` |
| 기존 action | 동일 |
| event API client | `cosmeticsEventOfferApi` (`api`, /api/v1) |
| event item 타입 | 동일 |
| 기존 canonical cart client/route/사이드바 | 없음 |
| cart serviceKey | `k-cosmetics` |
| event-offer OPL service_key | `k-cosmetics-event-offer` |
...
- **Backend (1)**: `apps/api-server/src/services/cart/event-offer-cart-checkout.service.ts` —
  `CART_TO_EVENT_OFFER_SERVICE_KEY` 에 `k-cosmetics→k-cosmetics-event-offer` 추가.
- **KCos (6)** 각:
...
### 3.1 TypeScript
- `web-k-cosmetics` tsc 0 · `apps/api-server` tsc 0 ✅
...
### 3.4 live 브라우저 smoke (배포 k-cosmetics-web)
| 항목 | KCos |
| ------ | :----: |
| `/store-hub/cart` 라우트 렌더 | ✅ (operator/admin 허용) |
