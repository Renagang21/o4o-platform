...
> **유형:** KCos `/lms` hub 공개/회원제 visibility 배지 표시 — frontend mapper 보강 (frontend-only)
> **결과: PASS** — KCos `mapCourse` 가 응답의 `visibility`(+requiresApproval/isPaid)를 `LmsHubCourse` 로 매핑 → `LmsHubTemplate` 유형 컬럼이 공개/회원제 배지 표시. backend/API/DB 무변경(데이터 이미 응답에 존재). KPA/Neture/backend 무변경. KCos typecheck 0.
...
선행 IR(판정 A) 대로 KCos `/lms` hub 에서 공개/회원제 배지가 표시되도록 frontend adapter 만 보강. backend 응답엔 visibility 가 이미 있고(service-neutral, KPA 가 사용 중) KCos `mapCourse` 가 버리던 것을 매핑.
...
- 잔존은 타 세션 forum), `web-k-cosmetics` **0**.
- **grep:** KCos `mapCourse` 에 `visibility: normalizeVisibility(c.visibility)` 확인. payment/checkout/rewardPolicy/youtube/iframe 재도입 0.
...
- **browser smoke:** 미수행 — 배포 후 KCos `/lms` 목록에서 공개/회원제 배지 표시·강의 상세 이동 확인 권장.
...
**PASS.** KCos `/lms` hub 가 공개/회원제 visibility 배지를 표시(mapper 매핑) — `/lms` hub 의 **구조·색상·visibility 표시까지 2서비스 정렬 완료**. backend/API/DB·KPA·Neture·LmsHubTemplate 구조 무변경, typecheck 0.
