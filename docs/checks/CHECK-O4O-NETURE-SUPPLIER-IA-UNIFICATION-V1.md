...
- 시도함
- 로컬 dev 서버에서 공급자 로그인 시도를 수행했으나, 인증 API 호출이 CORS 차단으로 실패해 실제 공급자 공간 진입은 확인하지 못함
- 원인 예시: `https://api.neture.co.kr/api/v1/auth/login` 에 대한 preflight 응답에 `Access-Control-Allow-Origin` 없음
- 따라서 데스크톱/모바일 사이드바 시각적 확인은 로그인 후 재시도 필요
