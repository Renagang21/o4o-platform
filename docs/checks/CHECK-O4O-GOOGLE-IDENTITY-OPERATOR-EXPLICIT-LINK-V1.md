...
- `users.password` 와 `service_credentials` 해시는 모두 `$2a# CHECK — WO-O4O-GOOGLE-IDENTITY-OPERATOR-EXPLICIT-LINK-V1
...
(bcrypt) 라 hashPrefix 로는 어느 층을 비교했는지 구분 불가 — 계약(serviceKey 유무)으로 판정.
...
## 3. 운영 Smoke (§10 · WO §8-A 정정판 · 사용자 브라우저 + read-only count) — PENDING (§3-C 정정 배포 후 재개)
...
| (본 커밋) | §3-C lockout 계약 정정(stale lock 정상화) + 계약 테스트 6(WO §8-B) |
