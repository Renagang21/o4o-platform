...
## 7. 검증 결과 (배포 후 프로덕션 read-only)

| 화면 | 메뉴 노출 | href 일치 | 클릭 이동 | route | 조회 API | 렌더 | 콘솔 | active | 쓰기 미실행 | 최종 |
|---|:--:|:--:|:--:|:--:|---|---|:--:|:--:|:--:|:--:|
| 플랫폼 HUB | ✅ | ✅ | ✅ | `/admin/platform/hub` | **200** ×1 | `Global Risk Overview`·`승인율`·위험 지표 | **0** | ✅ | ✅ | **PASS** |
| 매장 네트워크 | ✅ | ✅ | ✅ | `/admin/store-network` | **200** ×3 | `Total Stores 3` 실데이터 | **0** | ✅ | ✅ | **PASS** |
| 오프라인 매장 | ✅ | ✅ | ✅ | `/admin/physical-stores` | **200** ×1 | 정상 빈 상태 안내 | **0** | ✅ | ✅ | **PASS** |

- **4xx·5xx 0건**
- **금지 메뉴 노출 0건** — `서비스 현황` / `/admin/services/overview` / `판매자 정산` / `공급자 정산` /
  `Service Monitoring` 을 사이드바 전체 링크에서 검색해 **미검출**
- 선택 메뉴의 **active 표시 정상**

### 7-1. 새로고침 후 route 유지 — **기존 결함(회귀 아님)**

새로고침 시 `/login` 으로 이동한다. 이는 **본 변경과 무관한 기존 admin 딥링크·새로고침 auth bootstrap 문제**다.
선행 `WO-O4O-ADMIN-FORUM-BASE-FEATURE-GUARD-ALIGNMENT-V1` smoke 에서 이미 확인됐고,
당시 대조군으로 `/home` 하드 이동도 동일하게 `/login` 으로 튕기는 것을 확인해
**특정 화면이 아니라 admin 전반의 문제**임을 검증했다. 이번 메뉴 연결이 만든 증상이 아니다.
...
| 항목 | 결과 |
|---|---|
| `npx tsc --noEmit -p tsconfig.json` (admin-dashboard) | **0 error** |
| 전체 monorepo build | **미실행** (WO 지시 — 변경이 메뉴 정의 1파일로 한정) |
| 공용 패키지 변경 | **없음** → 광범위 build 불필요 |
...
| 항목 | 값 |
|---|---:|
| 쓰기 endpoint 실행 | **0** (`Sync Stores`·`새로고침`·`Refresh` 등 버튼 미클릭) |
| 운영 데이터 변경 | **0** |
| 권한 변경 | **0** (약화·제거·신규 역할 없음) |
| 다른 세션 작업물 접촉 | **0** — 작업 중 트리에 있던 `neture-settlement.service.ts` 등 병렬 세션 파일 미접촉 |
| `pnpm-lock.yaml` | 미포함 |
...
- **좁은 화면(모바일) 메뉴 처리 미검증** — 데스크톱 뷰포트(1500×1100)로만 확인했다.
- **비관리자 계정에서의 메뉴 미노출 미검증** — super_admin 1개 계정으로만 확인했다.
  (다만 세 route 의 가드가 기존 `Ops Metrics` 와 동일하므로 노출 경계가 달라질 근거는 없다.)
- **새로고침 route 유지**는 기존 결함이라 이번 범위에서 다루지 않았다(§7-1).
- 범위 밖 추가 발견 **없음**.
...
|---|---|
| 세 화면 메뉴 연결 완료 | ✅ **완료** |
| 관리자 메뉴에서 실제 사용 가능 | ✅ 메뉴 클릭 → 정상 route → 조회 200 → 데이터 렌더 |
| HOLD 화면 미연결 | ✅ 서비스 현황·판매자 정산·공급자 정산·Multi-Site Builder 계열 **전부 미연결** |
