...
- **결정(2026-06-12): 현 상태 보류** — prepare 까지 검증으로 충분, Phase 4~8 은 **Toss sandbox/client/secret key 준비 후 V4** 로 닫음. **결제 bypass 는 도입하지 않는다**("B2B 는 반드시 결제 후 공급자에게 보인다" 기준 유지 — bypass 는 결제 검증이 아니라 결제 우회라 기준을 흐림).
- 후속 선행: `IR/WO-O4O-TOSS-PAYMENT-PROVIDER-CONFIGURATION-V1`(Toss 키 설정) → `CHECK-O4O-NETURE-B2B-CANONICAL-END-TO-END-POSITIVE-SMOKE-V4`(키 설정 후 Phase 4~8).
...
## 8. 후속
- `IR/WO-O4O-TOSS-PAYMENT-PROVIDER-CONFIGURATION-V1` — Toss sandbox client/secret key 설정(시크릿/운영 영역). **bypass 미도입 확정.**
- → `CHECK-O4O-NETURE-B2B-CANONICAL-END-TO-END-POSITIVE-SMOKE-V4` — 키 설정 후 Phase 4~8(Toss 결제→confirm→paid→bridge→supplier).
