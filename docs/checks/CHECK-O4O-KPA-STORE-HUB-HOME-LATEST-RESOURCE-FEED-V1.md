...
> 결론: **PASS** — KPA `/store-hub` 홈이 정적 안내 카드에서 최신 자원 피드(상품/콘텐츠/디지털)로 전환됨. KCos 기본 5-block 무회귀(라이브 확인).

---

## 1. 변경 요약
- **StoreHubTemplate**(공용): `renderMainSections` + `showHeroCta` optional 슬롯 additive. 미전달 시 기존 5-block 그대로 → KCos 무영향.
...
| 워크플로 | 결과 |
|---|---|
| Deploy Web Services (Cloud Run) | success — detect-changes 전 서비스 배포(shared-space-ui 변경 감지). deploy-kpa-society success. |
...
- **KPA 피드 미노출**(`새로 공급 가능한 상품`/`새로운 디지털 자료` 부재). console error 0. → additive 슬롯이 KPA opt-in 으로만 동작함을 라이브 확인.
...
- KCos `/store-hub` 라이브 미실행(typecheck 라이브 동형으로 대체).
- `/store-hub/*` 하위 라우트는 본 WO 미변경(사이드바 링크 정상 노출 확인).

---
**작성:** 2026-06-29 · 배포 `b2deb2012` · KPA 피드 PASS 기본 5-block 무회귀 PASS.
