...
보호 route 대상: KPA·KCos `/store-hub` · PharmacyHub `/store-owner` · Neture `/mypage`.
...
| serviceKey | HTTP |
|---|:---:|
| `kpa-society` · `k-cosmetics` · `neture` | **200** |
| `pharmacy-hub` | **404** |
...
- **Neture 는 `.smoke_creds` 에 자격이 없다**(키: KPA·KCos·PH 만) → 공식 3서비스 중 Neture 만 미검증.
...
→ KPA·K-Cosmetics 의 로그아웃 결과를 **실패로 보고하지 않는다.** 측정이 유효하지 않았을 뿐이다.
...
| 2 | KPA·KCos **UI 로그아웃** 재검증 | §10-5 측정 결함 |
| 3 | — | §10-7. PostLoginRedirect ↔ LoginModal 레이스 정리 |
