...
KPA 에만 있던 `product_approvals` 운영자 승인 surface 를 K-Cosmetics 운영자에게 확장.
KPA 컨트롤러를 serviceKey/scope 로 일반화 → KCos 마운트 + 공통 콘솔 + thin wrapper + 메뉴.
...
## 4. Phase 2 — KCos backend route mount (`face32609`)
...
- KCos 는 audit parity 위해 `new ActionLogService(dataSource)` 주입.
...
- AI 요약 버튼은 `api.aiSummarize` 제공 시에만 노출(KCos 미제공 → 숨김).
...
## 6. Phase 4 — KCos operator pages 추가 (`3500d1215`)
...
- KCos: 제목 '공급 상품 신청 승인', `toError` 친절 메시지, res.data 언랩 어댑터.

## 7. Phase 5 — KCos route/menu 노출 (`3500d1215`)

- KCos `App.tsx`: `<Route path="product-applications" ... />` (operator lazy import).
- KCos `operatorMenuGroups` `approvals` 그룹에 **'공급 상품 신청 승인'** → `/operator/product-applications` (매장 승인 / 이벤트 오퍼 승인과 **별도 항목**).
...
- product-applications surface(콘솔/KCos wrapper) 내 `판매자 모집` / `B2B 승인` / `B2C` → **0건**
- product-applications mount/route/menu 참조 10건 확인.

**Browser smoke (권장, 배포 후):**
- KCos `/operator/product-applications` 진입 + 메뉴 '공급 상품 신청 승인' 노출 + 목록/empty/승인·거절 버튼 렌더.
- KPA regression: 기존 화면 정상 렌더.
- pending 0 시 write smoke 생략. production write 는 명시 승인 없이 미수행.

## 10. 완료 판정

✅ **완료.** KCos backend route mount + scope 격리 + 공통 콘솔 + wrapper + route/menu. approve=activateListing:true(per-store 단건). KPA 무회귀, 메뉴 노출 별도 WO 유지. DB/migration 0, storefront/checkout 0. 5개 tsc PASS. Browser smoke 만 배포 후 권장.
...
4. (선택) KCos operator AI 요약 endpoint 도입 시 wrapper 에 `aiSummarize` 주입 → 버튼 자동 노출.
