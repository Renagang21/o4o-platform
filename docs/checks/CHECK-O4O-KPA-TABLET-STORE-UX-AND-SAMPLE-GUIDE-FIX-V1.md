...
코너 현황판, 1동작 화면 교체, 미리보기 통합, 태블릿·QR 상품 정합은 구현·배포·검증 완료했다.

다만 보호 샘플 2종의 상품 이미지가 아직 외부 placeholder이므로, 실사용 수준의 샘플·안내 사례 완성 조건은 충족하지 못했다.

사용자가 제공하는 실제 상품 사진 6개를 적용하고 태블릿·상품 상세·QR 모바일에서 재검증한 뒤 최종 PASS로 전환한다.
...
| §3 미리보기 통합 | 완료 |
| §5 고객 화면 정합 | 완료 |
| §4 실사용 샘플 | 미완료(사진 수령 대기) |
| **전체 WO** | **PARTIAL** |

근무자 관점 핵심 달성: **모든 코너를 한 화면에서 보고(현황판), 한 번에 바꾸고(1동작 교체), 한 종류 미리보기로 확인.** 내부 용어 0. §4 문안은 이미 실사용 수준(코너 설명·콘텐츠 카드 자연스러움 실측 확인).

배포 커밋: `5159917cc`(§1-§3·§5) → `15f3e6a4b`(board 기본화면) → `3626629c5`(되돌리기) → `ac3cc128c`(휴대전화 390px). 웹·API 배포 success. **§4 적용 후 본 CHECK를 최종 갱신(같은 문서)한다.**
...
**결정**: 지어내거나 무관한 약 사진을 붙이면 교보재 신뢰가 깨지므로, **사용자가 실제 제품 사진을 제공**하기로 함. §4 문안(코너 설명·콘텐츠 카드)은 실측상 이미 자연스러워 재작성 불필요.

**이미지 제공 이후 할 일(새 기능 없음 · 이 CHECK를 최종 갱신):**
1. 제공 사진을 **O4O 관리 미디어 저장소에 업로드**해 **안정적인 내부 URL** 확보. ⚠️ **외부 임시 URL 직접 연결 금지** — 샘플은 안내 매뉴얼이라 외부 이미지 서비스의 만료·차단·리퍼러 의존은 부적합.
2. 6개 `store_local_products.thumbnail_url` 을 그 내부 URL 로 교체. 필요 시 `images[0]` 도 동일 값으로 정합.
3. 기존 `placehold.co` URL(및 `%0A` 깨짐) 제거.
4. **4화면 재검증**: 태블릿 카드 · 상품 상세 · 관리자 미리보기 · QR 모바일.
5. 재검증 후 §0 결론을 **PARTIAL → 최종 PASS** 로 전환.
