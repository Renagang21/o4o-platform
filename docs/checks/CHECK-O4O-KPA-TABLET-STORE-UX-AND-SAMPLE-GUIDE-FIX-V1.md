...
코너 현황판, 1동작 화면 교체, 미리보기 통합, 태블릿·QR 상품 정합 구현·배포·검증 완료. 보호 샘플 2종(피부·구강)의 상품 이미지를 외부 placeholder → **O4O 내부 미디어(`o4o-media-library`, 안정 GCS)의 실제 제품 사진**으로 교체하고 **태블릿 카드·상품 상세·관리자 미리보기·QR 모바일 4화면**에서 실사진 렌더 재검증 완료.
...
근무자 관점 핵심 달성: **모든 코너를 한 화면에서 보고(현황판), 한 번에 바꾸고(1동작 교체), 한 종류 미리보기로 확인.** 내부 용어 0. 샘플은 코너 구조(여러 제품 + 코너 설명 + 제품별 클릭 상세)가 실사진으로 채워져 근무자가 **배치·작업 방식·활용 방식**을 참고할 수 있는 상태.

배포 커밋: `5159917cc`(§1-§3·§5) → `15f3e6a4b`(board 기본화면) → `3626629c5`(되돌리기) → `ac3cc128c`(휴대전화 390px). 웹·API 배포 success. §4 이미지 = 런타임 데이터(DB), 배포 무관.
...
**방향 정정(사용자)**: 샘플은 **참고용** — 근무자가 보는 것은 **배치·작업 방식·활용 방식**이지 이미지가 코너/제품과 정확히 일치할 필요는 없다. "상관없는 이미지라도 넣어 화면이 실제처럼 채워지게 하라." 단, 앞서 락인한 **외부 임시 URL 금지 → O4O 내부 미디어** 원칙은 유지.

**적용 완료**:
- 소스 = **O4O 미디어 저장소 `o4o-media-library`(GCS, 안정 내부 URL)의 실제 제품 사진** — placehold.co 같은 외부 임시 서비스 아님. 공개 접근 확인(HTTP 200 image/jpeg). 별도 업로드 불필요(이미 O4O 관리 미디어).
- 6개 `store_local_products` 의 `thumbnail_url` + `images[0]` 를 서로 다른 실사진 6장으로 교체. `placehold.co`·`%0A` 완전 제거(실측 잔존 0). **ProductMaster·상품 ID·코너 구조 무변경**, 백업 `sample-backup-*.json`(원복 가능).
- 문안(코너 설명·콘텐츠 카드·제품별 개별 설명)은 이미 실사용 수준이라 재작성 불필요. 제품별 설명 존재 확인(예: 후시딘 "상처 부위 2차 감염 예방…") → 제품 클릭 시 상세 표시.

**4화면 실사진 렌더 재검증(실측)**:
| 화면 | 결과 |
|------|:----:|
| 태블릿 카드(공개 `/tablet`) | ✅ 실사진 3제품 + 코너 콘텐츠 4장 (media-library 200×3 / placehold 0) `w1-tablet-cards.png` |
| 상품 상세(제품 클릭) | ✅ 실사진 대형 + 개별 설명 + QR `w2-product-detail.png` |
| 관리자 미리보기 | ✅ 동일 kiosk 실사진 `v3-preview-*.png` |
| QR 모바일(390px) | ✅ 실사진 3제품, idle 없음 `v5-qr-skin.png` |

> 대상 6개(백업 `sample-backup-*.json`): 피부 f8b78a16(후시딘연고 `cd3a2b29` / 비판텐연고 `5e7344c5` / 마데카솔겔 `1fcce2f6`) · 구강 c86863d8(케어가글액 박하 `868341a6` / 사과 `91cfdb0d` / 유칼립투스 `099eee4f`).
...
| 5 | 피부·구강 샘플 실제 상품 이미지 | ✅ (O4O 내부 미디어 실사진 6장 적용) |
| 6 | placehold·색상영문·%0A 잔존 없음 | ✅ (실측 잔존 0) |
| 7 | 샘플 설명·상품 구성 자연스러움 | ✅ (문안 실사용 수준) |
| 8 | 태블릿·QR 상품 집합·이미지 일치 | ✅ (양쪽 동일 3제품·동일 실사진) |
...
*근무자 UX = 코너 현황판(한눈에) + 1동작 교체(연결·용어 없음, 되돌리기) + 미리보기 1종(태블릿/휴대전화 390px). QR = 태블릿과 상품 동일(진열 3제품) + 대기영상 first 제거. §4 = O4O 내부 미디어(o4o-media-library) 실사진 6장으로 샘플 채움(placehold·%0A 제거), 4화면 실사진 렌더 검증. 코너 구조=여러 제품+코너 설명+제품별 클릭 상세(참고용 배치·작업·활용 시연). 스키마·ProductMaster·상품 ID·보호샘플 구조 무변경.*
