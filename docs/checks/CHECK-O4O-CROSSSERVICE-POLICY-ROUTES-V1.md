...
> K-Cosmetics 에 공개 `/terms`·`/privacy` 를 신규 `service_policy_documents` public API 기반으로
...
> **결과: CODE PASS** (tsc KCos 0 + build KCos 0). 배포 후 브라우저 smoke 예정. — 2026-06-12
...
- Neture/KCos Admin 에서 정책 문서 입력·게시 가능(선행 UI WO). KPA 는 별도 `kpa_legal_documents` 구조.
...
| URL | 결과 |
|-----|------|
| terms | "이용약관" + "현재 공개된 문서가 없습니다." (placeholder 없음 레이아웃 정상). |
| privacy | "개인정보처리방침" + empty state ✅ |
...
*Date: 2026-06-12 · Status: CODE PASS. KCos 신규 추가, Neture 현행 유지, KPA 제외(후속 IR).*
