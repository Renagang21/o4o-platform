...
### 2-3. 대본 1차 축약(V2) → TTS 재생성 · 재실측 (2026-09-15, EP01 narration concise pass V2)

- 규칙: 목표 길이 초과 시 **음성 속도를 바꾸지 않고 대본을 1회 축약** → 재측정. V1 원문·음성은 `C:\tmp\minerock600-pilot\ep01\narration\v1\` 보존, V2 가 canonical `ep01-cut{1,2,3}.{wav,mp3}`.
- V2 문장: [`narration-ko.txt`](../../scripts/media/minerock600-pilot/ep01/narration-ko.txt) (사용자 제시안 그대로). 동일 `gemini-3.1-flash-tts-preview` · `Kore` · 동일 style · 각 1회 · HTTP 200 ×3.

| CUT | V1 실측 | **V2 실측** | V2 권장 CUT(+0.4/0.8) | 기획 |
|---|---|---|---|---|
| 1 | 8.88s | **6.44s** | **7.64s** | 7s |
| 2 | 12.28s | **10.00s** | **11.20s** | 10s |
| 3 | 13.44s | **9.56s** | **10.76s** | 10s |
| 합계 | 34.60s | **26.00s** | **29.60s** | 27s (목표 25~30s) → **범위 내 ✅** |

- 발음: 기계 판정 불가. 입력이 전부 한글 숫자 표기("천오십 미터"·"경도는 육백")라 오독 요인 없음 — 청취 확인은 사용자.
- 자막(`subtitles-ko.srt` · `assemble_ep01.py SUBS`)은 요약형이라 V2 와 내용 일치, 타이밍(기획 7/10/10)은 합성 시 실측으로 재산출 → 이번엔 불변.
- 미실행: 영상 생성 · FFmpeg 합성. **다음 = Higgsfield/Seedance CUT 3개 생성 (권장 길이 7.64 / 11.20 / 10.76s)**.
