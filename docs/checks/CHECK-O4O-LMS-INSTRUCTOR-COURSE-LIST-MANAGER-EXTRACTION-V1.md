...
> **결과: PASS** — KPA `CourseListPage`(BaseTable+RowActionMenu+bulk)를 canonical 로 config-driven `InstructorCoursesManager`(@o4o/operator-core-ui) 추출. KPA 기능 보존(검색·bulk·완료율·수강자 action 은 config 토글). operator-core-ui 신규 모듈 + KPA typecheck 0. backend/package.json/Dockerfile/Neture/KCos 무변경.
...
강사 축 첫 단계 — 강의 목록만 작게 공통화. 운영자와 달리 2서비스 구현이 분기(KPA BaseTable+bulk raw table+검색+완료율+수강자 / KCos read-only)하므로 **KPA 를 canonical 로 config-driven manager 추출** 가 자기 옵션으로 채택, KCos 는 미성숙이라 보류(Phase 1-B 에서 공통 모듈 위에 build).
...
```
- `InstructorCourse` view type(id/title/status/thumbnail/description/enrollmentCount/completionRate/createdAt). **envelope·필드 매핑은 wrapper 의 `api.list` 책임**(KPA `currentEnrollments`·`res.data.data` `enrolledCount`·`res.data`).

## 5. KPA 적용 결과

| | KPA |
| --- | --- |
| api.list | `lmsInstructorApi.myCourses` → map |
| api.delete | `lmsInstructorApi.deleteCourse` |
| accent | `#4f46e5` |
| search | off |
| bulkDelete | **on** |
| rowActions | edit, delete |
| columns | description, createdAt |
| edit route | `/instructor/courses/:id/edit` |

→ KPA: bulk 선택삭제·생성일·설명 보존. **기능 손실 없이 config 로 차이 흡수.**
...
- **경미 정규화:** 상태 라벨 통일(published '공개', archived '보관됨' — KPA 기존 '발행됨'/'보관됨' '공개'/'종료' → 통일). 셀렉트 체크박스 색 accent 화. 기능 영향 없음.
...
- **TypeScript:** `@o4o/operator-core-ui` 신규 모듈 **0**(패키지 잔존 1건은 `@o4o/error-handling` dep 사전 에러, 무관). forum 타 세션 무관).
- **정적:** KPA(44줄) wrapper 가 `InstructorCoursesManager` 소비. manager serviceKey/lms client 직접 import 0. operator-core-ui→lms-ui import 0. KCos 미수정.
- **무변경:** backend, package.json/pnpm-lock, Dockerfile, Neture.
- **browser smoke:** 미수행 — 배포 후 KPA `/instructor/courses` 목록·검색·row action·bulk(KPA)·수강자 렌더 확인 권장(삭제는 production 미실행).
...
**PASS.** KPA 강사 강의 목록을 canonical 로 공통 `InstructorCoursesManager`(config-driven) 추출, KPA 적용(wrapper 44/45줄, 기능 config 흡수로 무손실), KCos 보류(Phase 1-B build-on). serviceKey hardcode·lms-ui 의존·Neture 소비 없음, backend/package/Dockerfile 무변경, typecheck 0. 강사 축 "첫 단추" — editor/lesson/quiz/assignment 는 후속 IR 로 분리.
