...
| 특수문자 `%` | ✅ 200 · total=40 — **SQL 오류 없음** (단, LIKE 와일드카드로 동작 — §9-5 참조) |
...
### 9-4. 미실행 회귀 4건 — 재실행 완료

차단은 **재배포 없이 자연 해제**되었다. 스모크(13시대) 이후 병행 세션의 api-server 커밋으로
리비전 `03063`(14:58) · `03064`(15:05) · `03065`(15:13) 가 배포되며 인스턴스가 교체되어
in-memory `blockedIPs` 가 사라졌다. 따라서 **차단 해제 목적의 재배포는 수행하지 않았다**
(승인은 받았으나 불필요해짐 — 불필요한 프로덕션 배포 회피).

리비전 `o4o-core-api-03065-h5k` · traffic 100% 에서 재실행:

| # | 그룹 | 결과 |
|:-:|------|------|
| ① | 검색 + 필터 조합 | ✅ 전부 200 — `search=e&status=active` total=16 · `+role=platform:super_admin` total=1 · `status=all&role=all` total=38 (필터 미적용 분기 정상) |
| ② | 페이지네이션 | ✅ 전부 200 — `page=1/2` 각 rows=5 · `page=99` **rows=0 · total=40 유지**(오류 없음) · 검색+페이지 `total=38` 유지 |
| ③ | 단독 필터 | ✅ 전부 200 — `status=active` 16 · `status=pending` 1 · `role=platform:super_admin` 2 · `role=all` 40 · `sortBy=email&sortOrder=ASC` 정상 |
| ④ | 단건 조회 | ✅ 존재 200 · 미존재 **404 `User not found`** |
| ⑤ | 일반 특수문자 검색 | ✅ `%` `_` `.` `@` `-` 전부 200 · **500 · SQL 오류 0** (injection 문자열 미사용) |

검색 없는 목록의 `total=40` 이 수정 전후 동일하다 — 검색 수정이 기본 목록 결과를 바꾸지 않았다.

### 9-5. 정정 — `%` · `_` 는 LIKE 와일드카드로 동작한다

§9-2 초안에 "`%` 가 리터럴로 매칭된다"고 적었으나 **오기였다.** 재실행 실측:

```
search=%  → total=40 (전체)      search=_  → total=40 (전체)
search=.  → total=40             search=@  → total=40        search=-  → total=18
```

검색어를 `%${term}%` 로 감싸기만 하고 `%`·`_` 를 이스케이프하지 않으므로 이 두 문자는
LIKE 와일드카드로 해석된다. **오류·보안 문제는 없고**(파라미터 바인딩 유지) §6 의
"특수문자 포함 검색 → SQL 오류 없음" 요건도 충족하지만, 리터럴 `%`·`_` 검색은 전체를 반환한다.
계약 확대를 피하려고 이번 WO 에서는 동작을 바꾸지 않았다 — 필요 시 `ESCAPE` 절 도입을 별도 판단한다.
...
| 항목 | 사유 |
|------|------|
| ~~검색+필터 조합 · 페이지네이션 · 단건 조회 회귀~~ | ✅ **§9-4 에서 재실행 완료** (차단 자연 해제 후) |
| 리터럴 `%`·`_` 검색 (ESCAPE 절) | §9-5 — 오류·보안 문제 없음. 계약 확대를 피해 미변경, 별도 판단 사항 |
