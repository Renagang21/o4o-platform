...
> `WO-O4O-CONTACT-DELIVERY-AND-NOTIFICATION-V1` 결과 (범위 정정: KCos 우선).
> K-Cosmetics 의 공개 문의를 **신규 cross-service `ContactInquiry`** 로 접수하고 운영자 **in-app 알림**
> (`contact.new`)으로 연결. 기존 contact 가 있던 Neture/KPA 는 **무변경**. 이메일·Admin 화면은 후속.
> **결과: CODE PASS** (tsc api-server/KCos 0 + build 0). 배포 후 migration 자동 + 브라우저 smoke 예정. — 2026-06-12

---

## 1. 작업 목적
KCos 의 공개 문의가 화면 제출(mailto/정적)에서 끝나지 않고 backend 저장 + 운영자 in-app 알림으로 연결되게 한다.
...
## 4. 알림 수신자 결정 기준
- `role_assignments` 에서 `{prefix}:operator` + `{prefix}:admin` DISTINCT user 조회.
...
## 7. serviceKey 확인 결과
- backend 화이트리스트 + role prefix 매핑 일치.
...
## 13. 테스트 문의 생성 여부
- smoke 로 **KCos 각 1건의 [SMOKE] 테스트 문의**(email `smoke-test@example.com`, 제목 `[SMOKE] … 문의 접수+알림 검증`) 생성됨.
...
## 후속 작업
1. `WO-O4O-CONTACT-INQUIRY-ADMIN-MANAGEMENT-V1` — KCos 문의 조회·상태 처리 Admin 화면 + 알림 targetUrl.
...
*Date: 2026-06-12 · Status: CODE PASS. KCos 공개 문의 접수+운영자 in-app 알림 신규. Neture/KPA 무변경, 이메일/Admin 후속.*
