...
직전 `WO-...-COURSE-FORM-SHELL-V1` 에서 강의 **기본정보 form**(title/desc/visibility/approval/reusablePolicy/tags)을 순수 `InstructorCourseFormShell` 로 추출(KPA create edit). 본 WO 는 그 다음 축 = **레슨 목록**. (강사 목록 → `InstructorCoursesManager`, 기본정보 → form shell, **레슨 목록 → 본 manager** 순서로 editor 축이 작게 쌓임.)
...
- LessonModal 은 `renderEditor` 로만 진입 → KPA(quiz/assignment 포함) 차이를 manager 가 알지 못함.
...
- **정적:** KPA 레슨 목록 기능(목록/순번/drag/추가/편집/삭제) 보존. manager API client/serviceKey/lms-ui import **0**(grep 확인). LessonModal 은 renderEditor 슬롯·wrapper 소유. quiz/assignment/AI/reward 미혼입.
- **무변경:** backend, package.json/pnpm-lock, Dockerfile(operator-core-ui 이미 COPY·dep — 신규 의존 0), Neture, K-Cosmetics.
- **browser smoke:** 미수행 — 배포 후 KPA `/instructor/courses:id`(레슨 목록·drag·추가/편집 modal·AI 구조 버튼) `/instructor/courses:id`(레슨 목록·drag·video/article modal) 렌더 확인 권장(production write action 미실행).
...
KCos 미적용(editor 부재). serviceKey hardcode·lms-ui 의존·Neture 소비 없음, backend/package/Dockerfile 무변경, typecheck 0. editor 축 "기본정보 form + 레슨 목록 shell" 정리 완료 — 퀴즈/과제/AI 는 후속 별도 설계.
