...
> **결과: PASS** — `LmsHubConfig.accent?` 추가(기본 `#2563eb`). 템플릿의 강의명 링크 / 기본 수강 CTA / 선택 체크박스에 accent 적용. KPA `#2563EB` `#16a34a` / K-Cosmetics `#db2777` 명시 주입. 기능·구조 불변, backward-compatible. 2서비스 typecheck 0. shared-space-ui→lms-ui import 0(Neture guard 유지).
...
2서비스(KPA/KCos) `/lms` 강의 목록 hub 가 `LmsHubTemplate` 으로 수렴됐으나, 템플릿 내부에 KPA blue(`#2563eb`)가 hardcoded 되어 KCos 브랜드색과 어긋났다. accent 를 service config 기반으로 정리해 각 서비스 브랜드색으로 렌더한다. 기능 변경 아님 — 색상 config 마무리 정리.

## 2. 선행 hub 수렴 상태

KPA/KCos `/lms` 모두 `LmsHubTemplate`(shared-space-ui, BaseTable 테이블) 소비. LessonList(row)·CourseProgressBar 도 2서비스 적용 완료. Neture LMS 제외 유지.
...
- **config 계약:** `LmsHubConfig.accent?: string` (optional). 미지정 시 `'#2563eb'` fallback → **backward-compatible**(기존 KCos 가 명시 안 했어도 깨지지 않음; 본 WO 에서 2서비스 모두 명시 주입).
...
- accent 외 구조/기능 **불변**. KCos `EducationPage` 는 config 에 accent 한 줄만 추가(fetchCourses/courseDetailPath/hero 동일). KPA renderCta/renderRowActions/visibility 매핑 유지.
...
- shared-space-ui 는 source-direct(소비처 컴파일 검증). `accentColor` 는 유효 CSSProperties.
...
- **browser smoke:** 미수행 — 배포 후 KPA(blue)/KCos(pink) `/lms` 강의명 링크·CTA·체크박스 accent 색, 목록/검색/페이지네이션/상세 이동, Neture LMS 메뉴 없음 확인 권장.
