...
### 4-1-1. 디자인 보존 최종 검증 (2026-06-26, PASS)

배경 박스가 있는 테스트 콘텐츠(`POST /store-contents`, html=청록 배경 카드 + 민트 테두리 카드)를 생성해
end-to-end 확인. **사용자 확인 완료.**

| 단계 | 결과 |
|---|---|
| 백엔드 저장이 inline style strip 하는가 | ✅ 안 함 — 저장 html 에 `background`/`color`/`border`/`border-radius` 전부 보존(API 직접 확인). `content_json` 을 그대로 저장(`store-content.controller`) |
| PDF 생성 HTML 에 배경/색/테두리/radius 보존 | ✅ 전수 확인(가로채기 캡처) |
| PDF 미리보기 시각 — 배경 박스·색·여백 렌더 | ✅ 청록 배경 + 흰 글자 + 민트 카드 + 테두리 정상(스크린샷, 사용자 확인) |

**결론**: PDF·백엔드 모두 디자인을 보존한다. 실제 운영에서 색이 사라진다면 원인은 **프론트 편집기의
저장 직렬화(HTML 직접입력→저장 시 inline style 유실)** 로 한정 — 본 WO 범위 밖, 후속 IR/WO 후보:
`KPA 콘텐츠 HTML 직접 입력 저장값 렌더링/이스케이프 케이스 점검`(콘텐츠 상세·QR 랜딩·PDF 동일 렌더 정책 일관성 포함).
