...
> **결과: PASS — 공통 `LocalProductBadge`+`LOCAL_PRODUCT_BADGE_OPTIONS`(@o4o/store-ui-core) 추출 + KPA/KCos 적용. store-ui-core + 2 web typecheck 0.**
> **작성일**: 2026-06-12

---

## 1. 목적
KPA/KCos 의 동일한 매장 취급 상품 배지(BADGE_OPTIONS + 목록 배지 render) 중복을 공통 컴포넌트로 추출. StoreLocalProduct = O4O 주문 무관(non-order) 의미 유지.
...
| `pages/store-management/StoreLocalProductsPage.tsx`(660) | `api/localProducts` | 동일 | 커스텀 table + Tailwind |
| KCos | `pages/store/StoreLocalProductsPage.tsx`(660) | `services/localProductApi` | 동일 | 커스텀 table + Tailwind |

> `BADGE_OPTIONS` const 는 **3서비스 byte-identical**(none/new/recommend/event, 동일 Tailwind class). 3서비스 모두 **Tailwind 기반**(주문 배지의 inline-theme 분기 없음) → 배지 공유 안전.
...
| empty/loading/error · KPI · table column · form/modal | ✖ V2 | KPA 는 BaseTable(@o4o/ui) / KCos 는 커스텀 table 로 list 구조 상이. 폼/모달은 660줄 풀 CRUD — 전체 page shell 추출은 V2(WO "전체 page shell V1 금지") |
| **전체 페이지(KCos)** | ✖ V2(권고)≡KCos 99% 동일 → `StoreLocalProductsManager`(apiClient/labels prop) 로 서비스 통합이 최대 dedup. KPA(BaseTable, 35줄 차이)는 별도. **디자인 시스템 충돌 없음(둘 다 Tailwind)이라 V2 안전 — 별도 WO 권장** |
...
| KPA `StoreLocalProductsPage.tsx` | 로컬 `BADGE_OPTIONS` 제거 → `LOCAL_PRODUCT_BADGE_OPTIONS as BADGE_OPTIONS`(폼 옵션 호환) import. BaseTable status 컬럼 badge render(10줄) → `<LocalProductBadge>` |
 `StoreLocalProductsPage.tsx` | 동상. 목록 td badge render(9줄) → `<LocalProductBadge>` |
...
## 10. 검증 결과
- **TypeScript**: `@o4o/store-ui-core` 0 · `web-kpa-society` 0 · `web-k-cosmetics` 0.
...
## 11. 완료 판정
**PASS** — `LocalProductBadge` 공통 추출 + KPA/KCos 적용(중복 BADGE_OPTIONS·배지 render 통합). StoreLocalProduct non-order 의미 유지, 주문/cart 미혼입. backend/DB 무변경. typecheck(store-ui-core + 2 web) 통과.

## 12. 후속 작업
1. KPA(BaseTable)는 별도/확장 슬롯. **최대 dedup 지점.**
...
*Date: 2026-06-12 · WO-O4O-MY-STORE-LOCAL-PRODUCTS-COMMON-COMPONENT-EXTRACTION-V1 · LocalProductBadge 공통 추출 + 3서비스 적용 PASS. StoreLocalProduct non-order 유지, backend/cart 무변경.*
