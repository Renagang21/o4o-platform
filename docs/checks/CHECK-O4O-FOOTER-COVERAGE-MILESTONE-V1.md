...
1. KCos/KPA 내 매장/내 약국 owner dashboard = coverage 대상 · 2. 공통 `StoreDashboardLayout`에 **additive `footer?` prop**(미주입 시 동작 불변) · 3. footer는 service wrapper가 주입(공통 `StoreFacingFooter`@shared-space-ui) · 4. `store-ui-core`에 서비스/footer 의존 추가 없음(레이어 방향 보존) · 5. KCos hub layout(top-level)에 footer 추가 · 6. KPA store-hub는 공개 Layout 상속 → 무수정 · 7. Neture 내 매장 없음 → 대상 아님.
...
1. storefront는 무수정(§3) · 2. 404는 minimal 복귀 네비 표준 유지 · 3.
- 근거: [PUBLIC-PAGE-FOOTER-COVERAGE-FIX](CHECK-O4O-PUBLIC-PAGE-FOOTER-COVERAGE-FIX-V1.md) — `f1a82803e`(+`51a3ca274`).
...
| KPA `/store/:slug` = bare, footer 없음 | self-contained 테마형 block-engine storefront, **자체 footer 보유** | IR Type-C **오분류 정정**, 무수정 |
 storefront custom footer → 공통 정렬 | **판매자(약국) 법정정보를 store 엔티티에서 동적 표시** | 플랫폼 footer 교체 = **법적 회귀**, 무수정 |
| 404 = Footer 추가 권장 | 선행 `WO-SERVICE-PAGE-FOOTER-COVERAGE-AUDIT-AND-FIX`가 minimal-nav로 표준화 404만 네비 정합, **footer 미추가** |
...
- **store-facing:** KCos/KPA footer 보강, `StoreDashboardLayout` additive, Shared Module Change Protocol(소비처 3개) 수행, 3서비스 tsc 0, 4서비스 deploy success, root 200.
- **브라우저 시각 smoke:** Playwright 점유로 보류(각 CHECK에 정적/배포 갈음 근거 기록). 가용 시 store-facing footer 404 1회 확인 권장.
...
| `WO-O4O-PUBLIC-FOOTER-CORE-GP-KCOS-V1` | KCos 공개 Footer 구조 추가 축소 희망 시. 운영 필수 아님. 전면 3서비스 FooterCore는 여전히 비권장 |

## 7. 검증 (이 마일스톤 자체)
- [x] 문서 1개만 생성 (코드/UI/API/DB/route 변경 0)
- [x] coverage 기준 고정 (§1) + 포함/제외 명시 (§1.1·§1.2)
- [x] 3단계 완료 상태 + commit 근거 (§2)
- [x] storefront 오분류 정정 플랫폼 footer 교체 비대상 + KPA 자체 footer 기록 (§3)
