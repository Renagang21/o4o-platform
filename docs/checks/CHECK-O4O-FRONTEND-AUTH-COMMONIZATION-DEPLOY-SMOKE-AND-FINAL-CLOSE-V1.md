...
| 워크플로 | 결과 |
|---|---|
| Deploy Web Services (Cloud Run) — run `31552647939` | success (detect-changes + neture / pharmacy-hub / kpa-society / k-cosmetics 5 job 전부 success) |
...
| # | 확인 항목 | KPA | Neture | K-Cos | PH |
| --- | --- | :--: | :--: | :--: | :--: |
| ① | 정상 로그인·로그아웃 | PASS | PASS | PASS | PASS |
| ② | 새로고침(전체 로드) 후 세션 복원 | PASS | PASS | PASS | PASS |
| ③ | 비로그인 보호 화면 접근 처리 | PASS `/login` | PASS 로그인 모달 | PASS `/login` | PASS 동일 URL 안내 |
| ④ | 로그인 사용자의 허용 화면 진입 | PASS | PASS | PASS | PASS |
| ⑤ | 역할 선택·전환 기존 동작 | 해당 없음 | 해당 없음 | 해당 없음 | 해당 없음 |
| ⑥ | `logoutAll` 이후 세션 처리 | PASS 현재 세션 종료 | 계약 동일 | 계약 동일 | 해당 없음 |
| ⑦ | 표시명·공통 UI 노출 | PASS | PASS | PASS | PASS |
| ⑧ | `MembershipGate` 동일 URL 안내 UX | — | — | — | PASS |
| ⑨ | 관리자·운영자·일반 회원 접근 경계 | PASS | PASS | PASS | PASS |
...
| 항목 | 근거 | 성격 |
|---|---|---|
| `E2E — Auth Runtime Regression` 3건 실패 (`[K-Cosmetics] 새로고침 시 /auth/me 중복 호출 없음`, `[K-Cosmetics] lazy session checkSession 트리거`, `[Neture] loading freeze 없음`) | 2026-08-10 `66520eda6` 부터 연속 실패 | 기존 실패. 이번 커밋으로 4→3건 감소 |
