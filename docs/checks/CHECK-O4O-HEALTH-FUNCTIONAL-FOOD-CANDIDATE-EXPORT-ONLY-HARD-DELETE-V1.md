...
## 8. 후속 실행 — B 원료형태 그룹 90건 hard delete (2026-07-05)

B(원료형태) 97건 전수 read-only 검토 → 소비자성 마커(캅셀/로얄/스틱) 3건 + 브랜드·소비자문구(듀오락×2/키즈·보장/땡큐베리) 4건 = **7건 보류**, 나머지 **90건**(…추출분말/농축분말/원말/혼합균주 벌크 원료)만 삭제.

| 항목 | 값 |
|---|---:|
| cleanup_key | `hff_rawmaterial_hard_delete_20260705` |
| 제외 마커 | 캅셀/캡슐/타블렛/스틱/젤리/환/골드/프리미엄/로얄/명품/데일리/플러스 + 듀오락/땡큐베리/키즈/보장 |
| snapshot / delete | 90 / 90 (원자적 assertion) |
| HFF candidate | 41,351 → **41,261** |
| 복구 | 동 audit 테이블(full_snapshot jsonb) |

## 9. 범위 밖 / 남은 후속

- **B 보류 7건**: 완제품 가능성(제형/브랜드) → 유지.
...
- **매장 설명(store description) 생성: 당분간 보류(사용자 지시 2026-07-05).** BULK-APPLY-RUN 착수 안 함.
- 잔여 HFF **41,261** 은 표본 기준 다수가 실판매 후보 → 전량삭제 금지 유지.

### 누적 삭제 요약
| 그룹 | cleanup_key | 건수 |
|---|---|---:|
| 전량수출용 | hff_export_only_hard_delete_20260705 | 2,835 |
| 균주 학명 | hff_strain_hard_delete_20260705 | 699 |
| 원료형태 | hff_rawmaterial_hard_delete_20260705 | 90 |
| **합계** | | **3,624** |

HFF candidate: 44,885 → **41,261** (−3,624, 전량 복구 가능).
