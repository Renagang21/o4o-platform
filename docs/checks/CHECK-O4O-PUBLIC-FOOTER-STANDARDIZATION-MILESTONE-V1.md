...
> **작성일:** 2026-06-14
> **상태:** ✅ **마일스톤 — Footer 정비 핵심 영역 완료.** IR Phase 1(Option B) 전부 PASS. 남은 건 선택 과제(KCos 한정 FooterCore)뿐.

## 0. 한 줄 결론
3서비스 공개 Footer의 **법정정보 drift·dead link·loader 중복**이라는 핵심 위험 3종이 모두 닫혔다. 전면 FooterCore(Option D)는 비권장 유지, KCos 한정 FooterCore(Option C 부분)만 선택 과제로 남는다.
...
| dead link / route drift | ✅ 해결 | LINK-GUARD | — |
...
| KPA loader transport | **`fetch` 유지** (KCos/Neture는 axios) | LOADER-CONSOLIDATION §5 — 공통 factory 위에서 어댑터만 다름, 로직 동일 |
...
| WO 후보 | 매핑 | 내용 | 판단 |
|--------|:---:|------|------|
| `WO-O4O-PUBLIC-FOOTER-CORE-GP-KCOS-V1` | C(부분) | KCos 쌍 한정 `PublicFooterCore`+config 추출 (구조 동일: dark·컬럼형·brand+3컬럼) | **선택** — 내부 정리이지 운영 필수 아님. KPA/Neture는 shape 차이로 제외 |
| `WO-O4O-PUBLIC-FOOTER-DESIGN-ALIGNMENT-V1` | — | footer 디자인/모바일 정렬 | **보류** — 전면 디자인 통합은 비권장 유지 |

> **전면 FooterCore(Option D)는 후보 아님.** KCos FooterCore도 "하면 좋은 정리"일 뿐, 착수 여부는 별도 판단.
