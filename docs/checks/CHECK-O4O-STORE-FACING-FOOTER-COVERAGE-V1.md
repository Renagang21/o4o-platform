...
## 1. 목적
KCos·KPA의 store-facing 사용자 화면(매장 HUB / 내 매장·내 약국 owner dashboard)에 Footer coverage 보장. **layout 단위 주입**(page별 `<Footer/>` 패치 금지). 공개 footer 표준(법정정보·링크·loader)은 기존 유지.

## 2. 선행 IR 반영
IR-FOOTER-COVERAGE-AUDIT §5 누락:
- 내 매장/약국 owner dashboard → 공통 `StoreDashboardLayout`(@o4o/store-ui-core) footer 없음 (Type A, 1곳).
- KCos store-hub layout top-level → footer 없음 (Type A). KPA store-hub는 공개 `Layout` 중첩 → 상속(해당 없음).
...
| 4 | — | StoreLayoutWrapper footer 주입 |
| 5 | — | hub footer + root flex-col |
...
- KCos `/terms`·`/privacy`·`/contact`, KPA `/policy`·`/privacy`·`/contact` — 전부 기존 공개 footer와 동일 route(이전 LINK-GUARD 결과 유지). `/education`·`/about` 등 dead link 미사용.
...
| workflow_dispatch service=all | `27489802289` | — |
| liveness (4 root) | — | k-cosmetics.site / kpa-society.co.kr / neture.co.kr **전부 200** |
...
## 17. 후속
1. `WO-O4O-PUBLIC-PAGE-FOOTER-COVERAGE-FIX-V1` — KPA `/store:slug` bare storefront custom inline footer→공통 정렬 + 404.
