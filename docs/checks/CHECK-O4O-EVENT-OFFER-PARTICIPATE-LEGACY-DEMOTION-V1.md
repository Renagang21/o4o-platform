...
| 영역 | 결과 |
|------|------|
| frontend api client `participate:` 정의 | 3곳 — legacy 표시 대상 |
| backend `service.participate(` 호출 | 컨트롤러 route 핸들러뿐 |
| backend `async participate(` 정의 | `routes/kpa/services/event-offer.service.ts:604` |
...
- **tsc**: api-server 0 · web-kpa-society 0 · web-k-cosmetics 0 ✅
- **회귀**: participate route/service/에러코드 무변경(주석만), checkout-confirm·StoreCartPage·cart add 무변경 ✅

## 5. 잔존 legacy/internal 호출처 (격하 표시됨, 미삭제)
- **Neture event-offer 컨트롤러**(`routes/neture/controllers/event-offer.controller.ts`)에도 participate route 존재 — 본 WO 3서비스 범위 밖이라 주석 미적용. Neture event-offer 는 STORE_SERVICE_KEY_MAP 미적용(지원 허브)으로 별도 취급. 후속 정리 시 함께 검토.
