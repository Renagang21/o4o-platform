...
```text
D3 정책 결론:
공급자 품목군은 사전 허가 심사 항목이 아니라 선택 정보로 재정의한다.
상품 등록과 승인요청은 초기 서비스 진입을 위해 hard block을 최소화한다.
규제 품목의 운영 리스크는 상품 승인, 운영자 확인, 판매 가능 전 보완, suspended 상태로 통제한다.
기존 supplier category 번호/PDF/status 데이터는 삭제하지 않고 보존한다.
```

D1~D3 한 줄 요약:
> Neture 공급자 온보딩은 "사전 차단형"에서 "선진입·후보완형"으로 전환되었고, 공급자 가입·승인·상품 등록의 진입 장벽이 크게 낮아졌다.

## 6-B. 후속 보류 항목 (의도적 미수행 — 보존)

| 보류 항목 | 이유 |
|----------|------|
| 상품 레벨 PDF 증빙 | 초기 상품 등록 복잡도 증가 |
| 규제 품목별 workflow | 현재 단계에서는 과함 |
| 자동 정산 gate 연결 | payout 로드맵 확정 후 |
| 법무/규제 세부 룰 | 실제 판매 품목·운영 사례 축적 후 |
| 공급자-품목군 승인제 부활 | 필요 시 suspended / 상품승인 reject로 대체 가능 |
