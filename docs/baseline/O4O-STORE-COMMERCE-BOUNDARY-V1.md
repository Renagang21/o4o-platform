...
### 13-1. 현재 계약 상태 (2026-08-25 확정)

```text
PLATFORM_DIRECT_SALE_BUSINESS_CONTRACT = NONE
```

현재 O4O에는 **플랫폼 자체 판매 사업 계약이 없다.**
매장 경영자의 소비자 판매뿐 아니라, **O4O 자체가 소비자 전자상거래를 운영하는 방향도 채택하지 않았다.**
실제 판매는 오프라인에서는 외부 POS(§3), 온라인에서는 네이버·쿠팡 등 외부 판매채널(§5)이 담당한다.

따라서 `platform-seller` / platform checkout / platform refund / consumer payment 코드는
**존재한다는 사실만으로 플랫폼 직접판매 사업의 근거가 되지 않으며**, §8 판정 절차의 대상이다.

> **역추론 금지.** "코드에 `platform-seller` 가 있으니 플랫폼 판매를 유지한다" 는 판단은 허용하지 않는다.
