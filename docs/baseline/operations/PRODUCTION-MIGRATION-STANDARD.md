...
> baseline rolled over 2026-09-18 (WO-O4O-RETIRED-SERVICE-MIGRATION-HISTORY-SQUASH-AND-BASELINE-FINAL-CLOSURE-V1).
> Background: production `typeorm_migrations` is an immutable audit history (684 rows, 681 distinct names,
> max id 685 at the 2026-09-18 capture) that is never replayed: replaying the repository history on an empty
> database fails (IR-O4O-MIGRATION-TIMESTAMP-ORDERING-HISTORY-GAP-AND-FRESH-DATABASE-REPLAY-CENSUS-V1) and
> the repository no longer holds a source file for every historical name.
> Empty databases are therefore built from a canonical schema snapshot, not from history replay.
>
> **Historical source files are not runtime provenance. Legacy production history provenance is verified
> by an ordered history fingerprint. Historical source replay remains ZERO. Fresh databases use the
> canonical baseline.**
...
| Snapshot meta | `.../bootstrap/canonical-schema-baseline.meta.ts` | baseline version `2026-09-18-id685` (supersedes `2026-09-15-id678`, 7 incremental migrations absorbed), expected fingerprint + line count, retired object patterns the snapshot must not contain. No historical migration name |
...
| Bootstrap marker | table `o4o_schema_baselines` (`.../bootstrap/baseline-marker.ts`) | exactly one row per bootstrap (`baseline_version`, fingerprint, `supersedes_baseline_version`); **not** a bulk INSERT of historical names into `typeorm_migrations`. Production carries no marker and never receives one |
...
| Historical freeze | `.../incremental/historical-migrations.manifest.json` | identity freeze `{ file, className, declaredName, name }` of the **retained** historical source files only (544 after the 2026-09-18 rollover) — `name` is the TypeORM runtime name (`declaredName ?? className`). Never loaded, never replayed, never runtime provenance; corrected only by `--write-historical --maintenance`, shrinks only through `--baseline-rollover` (guard C23) |
| Legacy history baseline | `.../incremental/legacy-history-baseline.ts` | `{ rowCount, distinctNameCount, orderedNameSequenceSha256, capturedThroughId, capturedAt }` of the production `typeorm_migrations` rows in id order, captured read-only. Contains **no migration name**. Guard C21 |
| Ordered history fingerprint | `.../bootstrap/legacy-history-fingerprint.ts` | the single hash implementation (`sha256` of each UTF-8 name followed by `\n`, lowercase hex) and `verifyLegacyHistoryPrefix` — pure functions shared by the classifier, the harness and the specs |
| Migration identity parser | `scripts/db/migration-identity.mjs` | single TypeScript-AST parser (file · class · declared `name` · runtime name) shared by the guard, the manifest writer and the tests. Only a class-level `name` with a static string literal counts; SQL text, local variables, object literals and helper classes are ignored; a dynamic `name` fails (`DYNAMIC_NAME`). Fixture + real-repo test `scripts/db/__tests__/migration-identity.test.mjs` |
...
| History rules | `.../bootstrap/incremental-history.ts` | contiguous-prefix rule (gap / skip / reversal / duplicate → invalid) — pure function |
...
| Contract guard | `scripts/db/check-migration-contract.mjs` (CI `ci-pipeline.yml`) | C01–C25: naming · ordering · manifest · entry-point · no-glob · no API-startup migration · legacy baseline / states / retained-manifest lockstep · no plaintext legacy name file · no retired object in the snapshot · no connection-detail logging · manifest identity == AST parser (C25). `--write-historical` is verify-only by default; `--maintenance` writes (refused under `CI`); `--baseline-rollover` additionally allows the retained set to shrink, only when the new baseline meta / states / manifest are consistent and no removed source is imported by runtime |
...
| Isolated-PostgreSQL harness | `apps/api-server/src/__tests__/helpers/isolated-pg-classifier-harness.ts` (spec `database-state-classifier-schema-drift-and-connection-log-hardening.spec.ts`) | builds 23 real database states (synthetic legacy history + injected baseline / manifest / states, incl. renamed · missing · reordered · inserted · truncated · third-duplicate histories) and checks the classifier verdicts; runs when `O4O_ISOLATED_PG_URL` points at a local PostgreSQL, otherwise prints an explicit SKIP. The production-equivalent legacy `--status` scenario additionally needs `O4O_LEGACY_HISTORY_FILE` (untracked local capture) |
...
| `BOOTSTRAPPED` | exactly one marker matching the code baseline, history does not reproduce the legacy fingerprint, history = contiguous incremental prefix only, core tables present, **live fingerprint == expected state for that prefix** | bootstrap SKIPPED, incremental only |
| `LEGACY_ESTABLISHED` | no marker, `typeorm_migrations[0 .. rowCount-1]` in id order reproduces `LEGACY_HISTORY_BASELINE` exactly (row count, distinct count, ordered sha256), rows after that prefix = contiguous incremental prefix, core tables present, **live fingerprint == expected state for that prefix** (production) | bootstrap SKIPPED, incremental only |
| `UNKNOWN_PARTIAL` | anything else (partial schema, drift of any column/constraint/table, marker mismatch, marker + legacy fingerprint coexisting, a legacy row renamed / missing / reordered / inserted / truncated / duplicated, history gap `[M2]` / skip `[M1, M3]` / reversal `[M3, M2]` / duplicate / foreign name after the prefix, unregistered expected state…) | **fail-fast, exit 1** — no repair, no fallback, no DDL |

Since v2.1 the schema fingerprint is compared for **every** established database (both states, with or
without incremental migrations applied): marker or ordered history fingerprint proves provenance, the
schema fingerprint proves the schema. Comparison = sha256 **and** line count against
`EXPECTED_SCHEMA_STATES[prefixLength]`. Since v2.3 no plaintext copy of the legacy history names exists
in the repository; the classifier reports `LEGACY_HISTORY_FINGERPRINT = MATCH | MISMATCH | NOT_APPLICABLE`.
...
2. **Never rename, renumber, edit or re-run an applied migration** (file name, class name, `name`). The retained historical sources are frozen in `historical-migrations.manifest.json`; a retained source is removed only by a baseline rollover (`--baseline-rollover`), never edited.
3. **Never modify `typeorm_migrations` rows** (no manual INSERT / UPDATE / DELETE / rename / reorder, no synthetic "historical" bulk INSERT). The production history is an immutable audit record; the legacy history baseline is captured from it read-only and is never used to rewrite it.
...
7. **Snapshot regeneration (baseline rollover)** requires a new baseline version (`YYYY-MM-DD-id<N>`, never reused), a new expected fingerprint and a WO; it is built with `scripts/db/build-canonical-schema-baseline.mjs` from an isolated database constructed from the previous baseline + all incremental migrations, never from production, and is accepted only when isolated fingerprint == production live fingerprint == final expected state. After the rollover the incremental manifest is empty, `EXPECTED_SCHEMA_STATES` holds the baseline entry only, and production receives no new marker.
...
10. **History prefix rule.** The rows after the legacy prefix in `typeorm_migrations` must be exactly `manifest[0..k-1]` in id order; gaps, skips, reversals and duplicates are `UNKNOWN_PARTIAL`. The legacy prefix itself is verified as a whole by the ordered history fingerprint — any renamed, missing, reordered or inserted legacy row is `UNKNOWN_PARTIAL` (fail-closed).
11. **No connection details in logs** — anywhere in the migration path (job, CLI, deploy workflow, error dumps).
12. **Migration identity is parsed, never grepped.** The historical manifest `name` is the runtime name from `scripts/db/migration-identity.mjs`; a manifest entry that disagrees with the parser is a guard failure (C25), not something to patch by hand. The manifest is corrected only by `--write-historical --maintenance` outside CI, without adding, removing or absorbing entries; the retained set shrinks only under `--baseline-rollover`, and either change is recorded in a CHECK.
14. **Historical source files are not runtime provenance.** No runtime module imports a historical migration source (jest + guard C21); a historical source may be deleted once the legacy history fingerprint covers its runtime identity and a fresh database builds from the canonical baseline alone.
13. **Local DB setup never prints credentials** — `scripts/setup-local-db.sh` reports `Database password: SET|MISSING|INVALID` only, passes the password to psql via stdin / a per-process `PGPASSWORD`, and writes it to the 0600 `.env` only. Regression: `scripts/db/__tests__/setup-local-db-credential-log.test.mjs` (canary secret, success + failure paths, CI).

### Change Log

| Date | Change | Basis |
|---|---|---|
| 2026-09-18 | v2.3 — baseline rollover `2026-09-15-id678` → `2026-09-18-id685` (7 incremental migrations absorbed, isolated fingerprint == production live fingerprint) · legacy production history verified by an **ordered history fingerprint** (`legacy-history-baseline.ts` + `legacy-history-fingerprint.ts`) instead of a plaintext name list · `historical-migration-names.ts` / `legacy-history.facts.ts` deleted · historical manifest = identity freeze of the retained sources only (544) · `--write-historical --maintenance --baseline-rollover` gate · classifier reports `LEGACY_HISTORY_FINGERPRINT` · harness 23 scenarios incl. history negatives · production `typeorm_migrations` untouched | WO-O4O-RETIRED-SERVICE-MIGRATION-HISTORY-SQUASH-AND-BASELINE-FINAL-CLOSURE-V1 (CHECK: `docs/checks/CHECK-O4O-MIGRATION-HISTORY-BASELINE-FINAL-CLOSURE-V1.md`) |
| 2026-09-15 | v2.2 — historical manifest identity: 3 entries whose `name` the regex parser had taken from SQL text (`'pharmacy'`, `'seller'`, `'seller'`) corrected to their runtime class names · `declaredName` field added to every entry · AST identity parser (`scripts/db/migration-identity.mjs`) shared by guard / writer / tests · `HISTORICAL_MIGRATION_NAMES` = runtime names only (657→644, production history still fully covered) · guard C25 · `--write-historical` verify-only + `--maintenance` · local DB setup credential non-exposure + canary test | WO-O4O-MIGRATION-HISTORICAL-MANIFEST-IDENTITY-AND-LOCAL-DB-CREDENTIAL-LOG-FINAL-CLOSURE-V1 (CHECK: `docs/checks/CHECK-O4O-MIGRATION-HISTORICAL-MANIFEST-IDENTITY-AND-LOCAL-DB-CREDENTIAL-LOG-FINAL-CLOSURE-V1.md`) |
