...
| Historical freeze | `.../incremental/historical-migrations.manifest.json` | 644 frozen entries `{ file, className, declaredName, name }` — `name` is the TypeORM runtime name (`declaredName ?? className`). Never loaded, never renamed; corrected only by `--write-historical --maintenance` under an explicit WO |
| Historical names (generated) | `.../incremental/historical-migration-names.ts` | one runtime name per frozen entry (644); used only to validate `LEGACY_ESTABLISHED` history names. Regenerated by `--write-historical --maintenance`, lockstep guard C21 |
...
| Contract guard | `scripts/db/check-migration-contract.mjs` (CI `ci-pipeline.yml`) | C01–C25: naming · ordering · manifest · entry-point · no-glob · no API-startup migration · names/states/facts lockstep · no connection-detail logging · manifest identity == AST parser (C25). `--write-historical` is verify-only by default; `--maintenance` writes and is refused under `CI` |
