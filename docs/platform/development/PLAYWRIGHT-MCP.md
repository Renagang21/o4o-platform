...
> **`.mcp.json`은 저장소에서 추적하지 않는다** (`.gitignore`).
> 따라서 신규 clone이나 다른 PC에는 이 파일이 **자동으로 존재하지 않으며**, 각 작업환경에서 위 내용을 직접 생성해야 한다.
> 설정 여부는 로컬에서 파일 존재를 직접 확인한다. 자격증명·머신별 절대경로는 이 파일에 기록하지 않는다 (§5).
