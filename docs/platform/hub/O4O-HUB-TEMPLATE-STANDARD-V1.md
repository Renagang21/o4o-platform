...
|----------|:-----------:|:----------:|:-----------:|
| **ContentHub** | `pharmacy/HubContentLibraryPage.tsx` | `hub/HubContentListPage.tsx` | — |
| **LmsHub** | `lms/EducationPage.tsx` | `education/EducationPage.tsx` | — |
| **ForumHub** | `forum/ForumHomePage.tsx` | `forum/ForumHubPage.tsx` | — |
| **StoreHub** | `pharmacy/StoreHubPage.tsx` | `hub/StoreHubPage.tsx` | — |
| **ResourcesHub** | `resources/ResourcesHubPage.tsx` | `resources/ResourcesPage.tsx` | — |
| **SignageHub** | **직접 구현** ※ | `store-management/signage/ContentLibraryPage.tsx` | — |

> ※ KPA Signage는 `WO-KPA-SIGNAGE-VIDEO-PLAYLIST-STRUCTURE-REFORM-V2`에 의해 SignageHubTemplate에서
> 동영상/플레이리스트 탭 기반 직접 구현으로 전환됨 (2026-04-23). Override 사유: § 8 참조.
...
| 서비스 | 도메인 | WO | 사유 |
|--------|--------|-----|------|
| KPA | Signage | `WO-KPA-SIGNAGE-VIDEO-PLAYLIST-STRUCTURE-REFORM-V2` | 동영상/플레이리스트 탭 분리, 복합 모달, 다중 데이터 소스 |
...
## 12. K-Cosmetics Placeholder Status

### 12.1 현재 상태

K-Cosmetics는 현재 HUB 템플릿을 직접 사용하지 않는다.
`CommunityHubPage`에서 `@o4o/shared-space-ui`의 타입과 섹션 컴포넌트를 import하지만,
6개 HUB 템플릿 중 어느 것도 사용하지 않는 상태이다.

### 12.2 향후 적용 계획

K-Cosmetics에 Forum, LMS, Signage 등의 HUB가 필요해지면:

1. KPA 구현을 Reference로 참조한다 (§ 9)
2. 동일한 템플릿 + Config 패턴을 적용한다
3. `serviceKey: 'k-cosmetics'`로 데이터를 격리한다
4. 별도의 WO를 통해 진행한다
