...
2. **외부 참조 정리 가능** — 참조는 **경로 참조**와 **이름 참조**를 구분해 판단한다.
   - **경로 참조**(마크다운 링크 등 경로를 포함한 참조): 이동과 함께 새 경로로 수정한다. 수정 없이 이동하면 안 된다.
   - **이름 참조**(코드 주석·문서 본문에서 파일명만 언급): `git mv` 후에도 유효하므로 **이동 차단 사유가 아니다.**
   - 다만 이름 참조라도 그 문서가 **현재 정책·실행의 근거로 인용되고 있으면** ACTIVE 여부를 별도로 판단한다.
