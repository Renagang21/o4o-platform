...
- `archive/work-orders/` — 완료된 일회성 WO 이력
- `archive/obsolete/` — 폐기된 기능 문서 (Care, GlucoseView, Point Policy 등)

> archive 폴더의 문서는 현재 기준 문서가 아닙니다.
> 현재 기준은 `baseline/`, `architecture/`, `rbac/` 폴더를 참조하세요.

### 실험/트라이얼
- `market-trial/` — Neture Market Trial canonical baseline (Extension App, Active)

### 서비스별 조사 기록
- `kpa/` — KPA-Society 서비스 IR·구조 분석 기록 (README 참조)
- `neture/` — Neture 서비스 IR·도메인 구조 분석 기록 (README 참조)
