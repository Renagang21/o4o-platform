...
> **원칙**: 서비스 선택은 "가입"이 아니라 "진입" 개념이다.
> 사용자는 플랫폼 공통 계정을 만들고, 이후 서비스 핸드오프 구조로 서비스를 전환한다.

```
1. User 생성
   └── 이메일·비밀번호·이름·전화번호 입력
   └── 진입 경로 기록 (어느 서비스에서 가입했는가)
...
6. 로그인 → 서비스 진입
   ...
   └── service_memberships 생성 (진입 서비스 기준)
   └── 이후 ServiceSwitcher + Handoff로 타 서비스 전환
...
> **원칙**: 로그인 ≠ 강의 이용 가능. 접근은 반드시 **Membership Group 또는 Qualification 기반**으로 판단한다.

| 항목 | 기준 |
|------|------|
| **강의 이용자** | 로그인 사용자 중 **강의 접근 권한이 부여된 회원군** 소속자만 가능 |
| **강사** | `instructor` 자격 보유자만 강의 개설 가능 — 기본 role로 부여 금지 |
| **강의 접근 제어** | "로그인 여부" 기반 아님 → **Membership Group + Qualification 복합 판단** |
| **약사/약대생** | `pharmacist_member`, `pharmacy_student_member` → 약사 전용 강의 접근 허용 |
| **외부 전문가** | `external_expert` → 전문가용 강의 접근 허용 + `instructor` 자격 시 강의 개설 가능 |
| **소비자** | `consumer` → 공개 강의만 접근 허용 (별도 정의 전까지 기본 제한) |
| **공급자 스태프** | `supplier_staff` → 기본적으로 강의 접근 없음. 명시적 권한 부여 필요 |
