...
- platform admin + `all=true` query → cross-service 허용 (감사 로그 자동 기록)
- platform admin + 둘 다 미지정 → **HTTP 400** (`PLATFORM_ADMIN_SCOPE_REQUIRED`)

**silent exemption 금지.** Rule 3 의 "예외 없이" 조항은 platform admin 에도 적용되며, opt-in 없는 무필터 조회는 정책 위반이다.

> 본 조항은 `IR-O4O-BOUNDARY-POLICY-PLATFORM-ADMIN-EXEMPTION-V1` Option B 채택 결과로 도입되었다. `WO-O4O-BOUNDARY-POLICY-PLATFORM-ADMIN-EXEMPTION-FIX-V1` 가 Operator Console 5+ endpoint 에 적용 (`/operator/members`, `/operator/members/stats`, `/operator/stores`, `/operator/products`, `/operator/analytics/{summary,actions,insight}`). 헬퍼: [`resolveOperatorScope`](../../apps/api-server/src/utils/serviceScope.ts).
...
| **WO-O4O-NETURE-ADMIN-USERS-SCOPE-FIX-V1** | Neture `/admin/users` | frontend `serviceKey=neture` 강제 + backend `getStats` `serviceKey` query 수용 + `COUNT(DISTINCT user_id)` |
| **WO-O4O-BOUNDARY-POLICY-PLATFORM-ADMIN-EXEMPTION-FIX-V1** | Operator Console 5+ endpoint | Option B 일괄 적용 — silent platform admin exemption 제거, 명시 opt-in (`serviceKey` / `all=true`) 강제. Rule 3 §4 보강 동반 |
