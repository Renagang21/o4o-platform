...
### 1. nginx 설정 충돌 문제 (신규 추가 - 2025-06-20)

#### **🎯 증상**: 사이트 전체 404 에러, "conflicting server name" 경고
```bash
# 문제 확인
sudo nginx -t
# 출력 예시: nginx: [warn] conflicting server name "neture.co.kr" on 0.0.0.0:80, ignored

curl -I https://도메인명/
# 출력 예시: HTTP/2 404 또는 502 Bad Gateway
```

#### **⚡ 즉시 해결법** (실전 검증 완료 - 12분 내 해결)
```bash
# 1. 중복 설정 파일 찾기
grep -r "도메인명" /etc/nginx/

# 2. 중복 파일 백업 처리 (예시)
sudo mv /etc/nginx/sites-available/www.도메인명 /etc/nginx/sites-available/www.도메인명.disabled

# 3. nginx 설정 검증
sudo nginx -t  # "conflicting server name" 경고 사라져야 함

# 4. nginx 재시작
sudo systemctl reload nginx

# 5. 즉시 테스트
curl -I https://도메인명/  # HTTP/2 200 OK 확인
```

#### **📋 상세 해결 가이드**
- **완전 해결 사례**: [neture-co-kr-404-error-resolution.md](neture-co-kr-404-error-resolution.md)
- **예방법**: 새 도메인 설정 전 기존 설정 파일 확인
- **모니터링**: 일일 `nginx -t` 실행으로 충돌 조기 발견
...
### **완전 해결된 주요 사례들**
- **[neture.co.kr 404 에러 해결](neture-co-kr-404-error-resolution.md)** - nginx 설정 충돌 해결 (2025-06-20)
- **[웹서버 Sparse-Checkout 성공](webserver-sparse-checkout-success.md)** - Git 동기화 최적화
- **[해결된 이슈 목록](known-issues.md)** - 모든 해결된 문제들의 요약
...
3. 팀원들과 해결책 공유
4. 정기 점검 항목에 추가 고려
5. **상세 해결 사례 문서 작성** (12분 이상 소요된 문제)
