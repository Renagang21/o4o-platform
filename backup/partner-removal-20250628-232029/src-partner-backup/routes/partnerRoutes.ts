...
import { partnerService, PartnerApplication, PartnerProfile, PayoutRequest } from '../services/PartnerService';
...
router.post('/apply', async (req, res) => {
  try {
    ...
    const application = await partnerService.createApplication({
      customer_id,
      customer_email,
      customer_name,
      application_reason,
...
router.get('/application/:customerId', async (req, res) => {
  try {
    ...
    const applications = await partnerService.getApplications();
...
router.get('/profile/customer/:customerId', async (req, res) => {
  try {
    ...
    const partner = await partnerService.getPartnerByCustomerId(customerId);
...
router.post('/generate-link', async (req, res) => {
  try {
    ...
    // Partner 존재 확인
    const partner = await partnerService.getPartnerByCode(partner_code);
    if (!partner) {
      return res.status(404).json({
        error: 'Partner not found',
        message: '존재하지 않는 Partner 코드입니다.'
    ...
    const link = partnerService.generatePartnerLink(
      partner_code, 
      product_id, 
...
router.post('/track-click', async (req, res) => {
  try {
    ...
    const click = await partnerService.recordClick({
...
router.post('/payout-request', async (req, res) => {
  try {
    ...
    const payoutRequest = await partnerService.createPayoutRequest({
      partner_id,
...
router.get('/admin/applications', async (req, res) => {
  try {
    ...
    const applications = await partnerService.getApplications(status as string);
...
router.post('/admin/applications/:id/reject', async (req, res) => {
  try {
    ...
    const application = await partnerService.rejectApplication(
      parseInt(id), 
      reviewer_id || 'admin', 
      reason
    );
...
router.get('/admin/partners', async (req, res) => {
  try {
    const { status } = req.query;
    const partners = await partnerService.getPartners(status as string);
...
router.get('/admin/payout-requests', async (req, res) => {
  try {
    const { status } = req.query;
    const requests = await partnerService.getPayoutRequests(status as string);
