...
SELECT 'caution_section' AS k, count(*) FROM shared_product_descriptions d
  JOIN product_masters pm ON pm.id = d.master_id
 WHERE pm.regulatory_type = 'COSMETIC' AND d.language = 'ko' AND d.description_type = 'STORE'
   AND d.status = 'canonical' AND d.content LIKE '%<h3>주의사항</h3>%';
SELECT 'mfds_usage_batch_tagged' AS k, count(*) FROM product_masters
 WHERE regulatory_type = 'COSMETIC' AND tags->>'mfdsUsageBatch' IS NOT NULL;
