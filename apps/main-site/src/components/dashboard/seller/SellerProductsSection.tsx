      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        {products.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-500">상품 목록 요약 (Phase 3에서 구현)</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">상품 목록</h2>
          <p className="text-sm text-gray-600 mt-1">
            판매 중인 드랍쉬핑 상품을 관리합니다.
          </p>
        </div>
      ...
      {products.length === 0 ? (
        <EmptyState
          ...
          title="등록된 상품이 없습니다"
          description="공급업체의 제품을 선택하고 마진을 설정하여 판매를 시작하세요."
          action={
        ...
        <div>
          {/* TODO: Full product list with filters, margin settings, status */}
          <p className="text-gray-500">상품 목록 및 관리 기능 (Phase 3에서 구현)</p>
        </div>
      )}
    </div>
  );
