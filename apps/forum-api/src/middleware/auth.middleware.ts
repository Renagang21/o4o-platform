...
// Core API verify response
interface CoreAPIVerifyResponse {
  success: boolean;
  data?: {
    user: UserContext;
  };
...
export async function verifyWithCoreAPI(token: string): Promise<{
  ...
  try {
    const response = await axios.get<CoreAPIVerifyResponse>(
      `${env.CORE_API_URL}/api/v1/auth/verify`,
      {
        headers: {
