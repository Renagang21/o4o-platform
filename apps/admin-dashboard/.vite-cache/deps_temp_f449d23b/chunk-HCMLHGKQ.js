...
var AuthClient = class {
  constructor(baseURL) {
    ...
    this.api.interceptors.request.use((config) => {
      var _a, _b;
      let token = localStorage.getItem("accessToken") || localStorage.getItem("token") || localStorage.getItem("authToken");
      if (!token) {
        const authStorage = localStorage.getItem("admin-auth-storage");
        if (authStorage) {
          try {
            const parsed = JSON.parse(authStorage);
            if (((_a = parsed.state) == null ? void 0 : _a.accessToken) || ((_b = parsed.state) == null ? void 0 : _b.token)) {
    ...
    this.api.interceptors.response.use((response) => response, async (error) => {
      ...
      if (((_a = error.response) == null ? void 0 : _a.status) === 401 && !originalRequest._retry) {
        ...
        try {
          ...
          if (refreshToken) {
            const refreshUrl = this.baseURL.includes("api.neture.co.kr") ? "https://api.neture.co.kr/api/auth/refresh" : `${this.baseURL.replace("/v1", "")}/auth/refresh`;
          ...
          if (typeof window !== "undefined") {
            const errorData = (_b = refreshError == null ? void 0 : refreshError.response) == null ? void 0 : _b.data;
            if ((errorData == null ? void 0 : errorData.code) === "TOKEN_EXPIRED") {
...
var getApiUrl = () => {
  var _a, _b;
  if (typeof window !== "undefined") {
    const envApiUrl = ((_a = window.__ENV__) == null ? void 0 : _a.VITE_API_URL) || typeof import.meta !== "undefined" && ((_b = import.meta.env) == null ? void 0 : _b.VITE_API_URL);
    if (envApiUrl) {
      return envApiUrl.endsWith("/api/v1") ? envApiUrl : envApiUrl.endsWith("/api") ? `${envApiUrl}/v1` : `${envApiUrl}/api/v1`;
    }
...
//# sourceMappingURL=chunk-HCMLHGKQ.js.map
