...
const PayoutRequests: React.FC = () => {
  ...
  const fetchPayoutData = async () => {
    try {
      ...
      const [summaryRes, accountsRes, requestsRes, historyRes] = await Promise.all([
        fetch('/api/v1/dropshipping/partner/payout/summary', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch('/api/v1/dropshipping/partner/payout/accounts', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch('/api/v1/dropshipping/partner/payout/requests', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch('/api/v1/dropshipping/partner/payout/history', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        })
      ]);
  ...
  const handleRequestPayout = async () => {
    ...
    try {
      const response = await fetch('/api/v1/dropshipping/partner/payout/request', {
        method: 'POST',
        headers: {
  ...
  const handleAddAccount = async () => {
    try {
      const response = await fetch('/api/v1/dropshipping/partner/payout/accounts', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleCancelRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/dropshipping/partner/payout/requests/${requestId}/cancel`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
