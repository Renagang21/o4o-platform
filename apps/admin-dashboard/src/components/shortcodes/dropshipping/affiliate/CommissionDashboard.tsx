...
const CommissionDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [summaryRes, commissionsRes, performanceRes, productsRes, tierRes] = await Promise.all([
        fetch(`/api/v1/dropshipping/affiliate/commission/summary?range=${dateRange}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch(`/api/v1/dropshipping/affiliate/commissions?range=${dateRange}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch(`/api/v1/dropshipping/affiliate/performance?range=${dateRange}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch(`/api/v1/dropshipping/affiliate/top-products?range=${dateRange}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        }),
        fetch('/api/v1/dropshipping/affiliate/tier-info', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        })
      ]);
  ...
  const handleExportData = async () => {
    try {
      const response = await fetch(`/api/v1/dropshipping/affiliate/commission/export?range=${dateRange}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
