...
const FolderManager: FC<FolderManagerProps> = ({ _isOpen, onClose, onFolderSelect }) => {
  ...
  const { data: folders = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await authClient.api.get('/v1/content/media/folders')
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CreateMediaFolderDto) => {
      return authClient.api.post('/v1/content/media/folders', data)
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: UpdateMediaFolderDto) => {
      return authClient.api.put(`/v1/content/media/folders/${data.id}`, data)
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return authClient.api.delete(`/v1/content/media/folders/${id}`)
    },
    onSuccess: () => {
