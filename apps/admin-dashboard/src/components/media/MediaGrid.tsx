...
// Updated MediaItem interface to match current usage
interface MediaItem {
  id: string;
  url: string;
  type: 'image' | 'video';
        ...
        {items.map((item) => {
          ...
          return (
            <li
              ...
              aria-label={`${item.title || item.filename}, ${item.mediaType}, ${formatFileSize(item.size || item.fileSize || 0)}`}
            ...
            >
              ...
              >
                ...
                >
                  <div>{item.filename || item.title}</div>
                </div>
              </div>
