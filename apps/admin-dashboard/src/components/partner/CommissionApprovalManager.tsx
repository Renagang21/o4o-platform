...
import { getCommissionHistory, processCommissions } from '@/api/partner';
import { formatPrice, formatRate } from '@/utils/vendorUtils';
import toast from 'react-hot-toast';
import type { PartnerCommission, ProcessCommissionRequest } from '@o4o/types';
...
export const CommissionApprovalManager: FC = () => {
  ...
  const filteredCommissions = commissions.filter((commission: any) => {
    const matchesSearch = searchQuery === '' ||
      ...
      commission.partnerUserId.toLowerCase().includes(searchQuery.toLowerCase());
    return matchesSearch;
  });

  const totalAmount = filteredCommissions.reduce((sum: number, c: PartnerCommission) => sum + c.commissionAmount, 0);
                  ...
                  filteredCommissions.map((commission: any) => {
                    ...
                    return (
                      <tr key={commission.id} className="border-b border-modern-border-primary hover:bg-modern-bg-tertiary">
                        ...
                        <td className="p-4">
                          <p className="text-sm font-medium">추천인 #{commission.partnerUserId.slice(0, 8)}</p>
                        </td>
                        <td className="p-4">
                          <div>
