...
const Conversions: React.FC = () => {
  ...
  useEffect(() => { fetchConversions(); }, [filter, dateRange]);

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'confirmed':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs"><CheckCircle className="w-3 h-3" />확정</span>;
      case 'pending':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded text-xs"><Clock className="w-3 h-3" />대기중</span>;
      case 'cancelled':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-red-100 text-red-700 rounded text-xs"><XCircle className="w-3 h-3" />취소됨</span>;
      case 'refunded':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs"><XCircle className="w-3 h-3" />환불됨</span>;
      default:
        return null;
    }
  };

  const columns: O4OColumn<Conversion>[] = [
    {
      key: 'orderNumber',
      header: '주문번호',
      render: (_, row) => <span className="font-medium">{row.orderNumber || row.orderId}</span>,
    },
    {
      key: 'orderAmount',
      header: '주문금액',
      align: 'right',
      sortable: true,
      sortAccessor: (row) => row.orderAmount,
      render: (_, row) => <span>{row.orderAmount.toLocaleString()}원</span>,
    },
    {
      ...
      render: (_, row) => <span className="font-medium text-blue-600">{row.commissionAmount.toLocaleString()}원</span>,
    },
    {
      key: 'status',
      header: '상태',
    ...
    },
    {
      ...
      render: (_, row) => <span className="text-sm text-gray-600">{new Date(row.createdAt).toLocaleDateString()}</span>,
    },
    {
      key: 'confirmedAt',
      header: '확정일',
      render: (_, row) => <span className="text-sm text-gray-600">{row.confirmedAt ? new Date(row.confirmedAt).toLocaleDateString() : '-'}</span>,
    },
  ];
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">총 전환</p><p className="text-2xl font-bold">{summary?.totalConversions}</p></div>
            <TrendingUp className="w-8 h-8 text-green-600 opacity-50" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">총 주문금액</p><p className="text-2xl font-bold">{summary?.totalAmount.toLocaleString()}원</p></div>
            <DollarSign className="w-8 h-8 text-blue-600 opacity-50" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">누적 커미션</p><p className="text-2xl font-bold text-blue-600">{summary?.totalCommission.toLocaleString()}원</p></div>
            <DollarSign className="w-8 h-8 text-blue-600 opacity-50" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm space-y-1">
            <div className="flex justify-between"><span className="text-gray-600">대기중</span><span className="font-medium text-yellow-600">{summary?.pendingCount}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">확정</span><span className="font-medium text-green-600">{summary?.confirmedCount}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">취소됨</span><span className="font-medium text-red-600">{summary?.cancelledCount}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">환불됨</span><span className="font-medium text-gray-600">{summary?.refundedCount}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          ...
          <BaseTable<Conversion>
            columns={columns}
            data={conversions}
            rowKey={(row) => row.id}
            emptyMessage="전환 내역이 없습니다."
            tableId="partnerops-conversions"
            columnVisibility
            persistState
          />
        )}
