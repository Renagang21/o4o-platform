...
const CosmeticsPartnerRoutines: React.FC = () => {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<{ type: 'step' | 'routine'; index?: number; routineId?: string } | null>(null);
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null);
  const [draggedStep, setDraggedStep] = useState<number | null>(null);
  const [showPreview, setShowPreview] = useState(false);
  const [routineFilter, setRoutineFilter] = useState<'all' | 'published' | 'draft'>('all');
  const [editingRoutine, setEditingRoutine] = useState<PartnerRoutine | null>(null);
  ...
  const handleRemoveStep = (index: number) => {
    // Phase 6-E: Show confirmation modal instead of direct delete
  ...
  // Phase 6-E: Move step up/down (alternative to drag)
  ...
  // Phase 6-E: Enhanced filtering with status filter
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          ...
          <div className="text-sm text-gray-500">전체 루틴</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          <div className="text-2xl font-bold text-green-600">{stats.published}</div>
          <div className="text-sm text-gray-500">공개 루틴</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          <div className="text-2xl font-bold text-gray-600">{stats.draft}</div>
          <div className="text-sm text-gray-500">비공개 루틴</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm border">
          <div className="text-2xl font-bold text-pink-600">{stats.totalViews.toLocaleString()}</div>
      ...
      <div className="space-y-4">
        {filteredRoutines.map((routine) => (
          <div key={routine.id} className="bg-white rounded-lg shadow-sm border overflow-hidden">
            <div
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full flex items-center gap-1 ${routineTypeColors[routine.routineType]}`}>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto py-8">
          <div className="bg-white rounded-xl p-6 w-full max-w-3xl mx-4 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900">새 루틴 생성</h2>
                <p className="text-sm text-gray-500 mt-1">스킨케어 루틴을 만들어 공유하세요</p>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="flex gap-6">
              {/* Form Section */}
              <div className={`space-y-5 overflow-y-auto max-h-[60vh] ${showPreview ? 'flex-1' : 'w-full'}`}>
                {/* Basic Info */}
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(['morning', 'evening', 'weekly', 'special'] as const).map((type) => (
                        ...
                        >
                          <div className={`w-8 h-8 rounded-full mx-auto mb-1 flex items-center justify-center ${routineTypeColors[type]}`}>
                ...
                <div>
                    ...
                    {createForm.steps.map((step, index) => (
                      ...
                      >
                        ...
                        <input
                          ...
                          placeholder="제품명"
                          className="flex-1 px-3 py-1.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm"
                        />
                        <input
                          type="text"
                  ...
                  <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm text-pink-600 hover:bg-pink-50 rounded-lg border-2 border-dashed border-pink-200 hover:border-pink-300 transition-colors"
                  >
                    <Plus className="w-4 h-4" />
                    단계 추가
                  </button>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`px-2 py-0.5 text-xs rounded-full flex items-center gap-1 ${routineTypeColors[createForm.routineType]}`}>
                    ...
                    {/* Preview Steps */}
                    <div className="space-y-2">
                      {createForm.steps.filter(s => s.productName).map((step) => (
                        <div key={step.order} className="flex items-center gap-2 p-2 bg-white rounded-lg">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{step.productName}</p>
                            {step.description && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-gray-900">
                  {showDeleteConfirm.type === 'step' ? '단계 삭제' : '루틴 삭제'}
                </h3>
            ...
            <p className="text-sm text-gray-600 mb-6">
              {showDeleteConfirm.type === 'step'
                ? '삭제된 단계는 복구할 수 없습니다. 나머지 단계의 순서가 자동으로 조정됩니다.'
                : '삭제된 루틴은 복구할 수 없으며, 관련된 모든 데이터가 영구적으로 삭제됩니다.'}
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={showDeleteConfirm.type === 'step' ? confirmRemoveStep : confirmDeleteRoutine}
