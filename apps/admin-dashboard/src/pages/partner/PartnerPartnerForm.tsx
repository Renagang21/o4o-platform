...
interface PartnerPartnerFormData {
  name: string;
  email: string;
  website?: string;
...
const PartnerPartnerForm: FC = () => {
  ...
  const { data: partnerData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await authClient.api.get(`/partner/partners/${id}`);
  ...
  const createMutation = useMutation({
    mutationFn: async (data: PartnerPartnerFormData) => {
      const response = await authClient.api.post('/partner/partners', data);
      return response.data;
    },
    onSuccess: () => {
      toast.success('파트너사가 추가되었습니다');
      queryClient.invalidateQueries({ queryKey: ['partner-partners'] });
      navigate('/partner/partners');
    },
    onError: () => {
      toast.error('파트너사 추가에 실패했습니다');
    }
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: PartnerPartnerFormData) => {
      const response = await authClient.api.put(`/partner/partners/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (isEditMode) {
      updateMutation.mutate(formData as PartnerPartnerFormData);
    } else {
      createMutation.mutate(formData as PartnerPartnerFormData);
    }
  };

  const handleChange = (field: keyof PartnerPartnerFormData, value: any) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label htmlFor="type">파트너사 유형 *</Label>
                <Select
                  value={formData.type}
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              <Textarea
                ...
                placeholder="파트너사에 대한 메모나 특이사항을 입력하세요..."
                rows={4}
              />
            </CardContent>
          </Card>
