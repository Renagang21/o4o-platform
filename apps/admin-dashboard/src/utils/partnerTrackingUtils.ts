...
export async function trackReferralClick(
  ...
  try {
    ...
    // API 호출로 클릭 기록
    await fetch('/api/v1/partner/track-click', {
      method: 'POST',
...
export async function trackReferralConversion(
  ...
  try {
    const response = await fetch('/api/v1/partner/track-conversion', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        userId,
...
export async function validateSingleTierReferral(
  ...
  try {
    // API를 통해 순환 추천 체크
    const response = await fetch('/api/v1/partner/validate-referral', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function establishReferralRelationship(
  ...
  try {
    const response = await fetch('/api/v1/partner/establish-relationship', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        referralCode,
...
export class ReferralRealtimeTracker {
  ...
  connect(_partnerUserId: string): void {
