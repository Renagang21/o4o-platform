...
export class UserApi {
  ...
  static async updateUser(userId: string, userData: Partial<UserFormData>): Promise<ApiResponse<User>> {
    const response = await unifiedApi.raw.put(`/api/v1/users/${userId}`, userData)
    return response.data
  }

  static async approveUser(userId: string, notes?: string): Promise<ApiResponse<User>> {
    const response = await unifiedApi.raw.post(`/api/v1/users/${userId}/approve`, { notes })
    return response.data
  }

  static async rejectUser(userId: string, reason: string): Promise<ApiResponse<User>> {
    const response = await unifiedApi.raw.post(`/api/v1/users/${userId}/reject`, { notes: reason })
    return response.data
  }
  ...
  static async deleteUser(userId: string): Promise<ApiResponse<void>> {
    const response = await unifiedApi.raw.delete(`/api/v1/users/${userId}`)
    return response.data
  }

  static async bulkAction(action: UserBulkAction): Promise<ApiResponse<void>> {
    const endpoint = action.action === 'approve' ? '/api/v1/users/bulk-approve' : '/api/v1/users/bulk-reject'
  ...
  static async getUserStats(): Promise<ApiResponse<UserStats>> {
    const response = await unifiedApi.raw.get('/api/v1/users/statistics')
    return response.data
  }
  ...
  static async exportUsers(filters: UserFilters = {}): Promise<Blob> {
    ...
    const response = await unifiedApi.raw.get(`/api/v1/users/export/csv?${params}`, {
      responseType: 'blob'
    })
    return response.data
  }
  ...
  static async getUserActivity(userId: string): Promise<ApiResponse<UserActivityLog[]>> {
    const response = await unifiedApi.raw.get(`/api/v1/users/${userId}/approval-history`)
    return response.data
  }
