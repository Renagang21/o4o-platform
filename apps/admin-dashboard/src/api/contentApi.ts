...
export class ContentApi {
  ...
  static async savePostDraft(id: string, content: JSONContent): Promise<ApiResponse<void>> {
  ...
  static async savePageDraft(id: string, content: JSONContent): Promise<ApiResponse<void>> {
  ...
  static async exportFieldGroups(ids?: string[]): Promise<ApiResponse<{ data: FieldGroup[]; filename: string }>> {
    const response = await api.post('/admin/custom-fields/export', { ids })
    return response.data
  }

  static async importFieldGroups(data: { groups: FieldGroup[] }): Promise<ApiResponse<FieldGroup[]>> {
