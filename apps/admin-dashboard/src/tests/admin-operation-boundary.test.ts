 ...
 *
 * 1. **플랫폼 전역 관리자 경계 상수**
 *    `PLATFORM_ADMIN_ROLES = ['platform:super_admin']` 는 서비스 경계가 없는 플랫폼 전역
 *    화면의 유일한 route 경계다. `requiredPermissions` 만 선언하면 백엔드가
 *    `user.permissions` 를 공급하지 않으므로 실효 경계가 "관리자급이면 통과"
 *    (서비스 접두 `kpa:admin` 포함) 로 넓어진다
 ...
 *
 * 3. **canonical 마운트 접두**
 *    약사회 전용 membership 마운트는 제거되어 0 이다(재등장 방지).
 *
...
describe('플랫폼 전역 관리자 경계 상수', () => {
  // WO-O4O-LEGACY-YAKSA-ADMIN-AND-DOMAIN-FEATURES-FULL-REMOVAL-V1
  //   `/admin/membership/*` · `/admin/yaksa/*` route 와 routes/yaksa.routes.tsx 는
  //   약사회 전용 기능 제거와 함께 삭제되어 대조 대상이 없다. 경계 상수 자체는 다른
  //   플랫폼 전역 화면(core-users 등)이 계속 참조하므로 그대로 고정한다.
...
describe('canonical 마운트 접두', () => {
  // WO-O4O-LEGACY-YAKSA-ADMIN-AND-DOMAIN-FEATURES-FULL-REMOVAL-V1
  //   @o4o/membership-yaksa 제거 후 membership 마운트는 0 이어야 한다(재등장 방지).
  it('membership 은 어디에도 마운트되지 않는다', () => {
