...
// 현재 이메일 설정 가져오기
router.get('/email', authMiddleware_1.authMiddleware.verifyToken, authMiddleware_1.authMiddleware.requireAdmin, async (req, res) => {
    try {
        ...
        logger_1.default.error('Failed to get email settings:', error);
        res.status(500).json({
...
// 이메일 설정 업데이트
router.put('/email', authMiddleware_1.authMiddleware.verifyToken, authMiddleware_1.authMiddleware.requireAdmin, async (req, res) => {
    try {
        ...
        logger_1.default.info('Email settings updated successfully');
        res.json({
        ...
        logger_1.default.error('Failed to update email settings:', error);
        res.status(500).json({
...
// 테스트 이메일 발송
router.post('/email/test', authMiddleware_1.authMiddleware.verifyToken, authMiddleware_1.authMiddleware.requireAdmin, async (req, res) => {
    try {
      ...
      `,
        ...
        if (result.success) {
            logger_1.default.info(`Test email sent successfully to ${testEmail}`);
            res.json({
                success: true,
            ...
            logger_1.default.error(`Failed to send test email to ${testEmail}:`, result.error);
            res.status(500).json({
                success: false,
        ...
        logger_1.default.error('Test email error:', error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=email-settings.routes.js.map