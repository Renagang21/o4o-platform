...
router.get('/kakao/callback', passport_1.default.authenticate('kakao', { session: false }), async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.redirect(getRedirectUrls().failure);
...
router.get('/naver/callback', passport_1.default.authenticate('naver', { session: false }), async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            return res.redirect(getRedirectUrls().failure);
...
router.post('/link/:provider', auth_v2_1.authenticateCookie, async (req, res) => {
    try {
...
router.delete('/unlink', auth_v2_1.authenticateCookie, async (req, res) => {
    try {
...
// Get linked accounts
router.get('/linked-accounts', auth_v2_1.authenticateCookie, async (req, res) => {
    try {
