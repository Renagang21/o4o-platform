...
router.get('/products', async (req, res) => {
    try {
        const { page = 1, limit = 20, category, search, featured, status = 'active' } = req.query;
...
router.get('/cart', auth_1.authenticateToken, async (req, res) => {
    res.json({
        success: true,
        data: {
            items: [],
...
router.post('/cart/items', auth_1.authenticateToken, async (req, res) => {
    res.json({
        success: true,
