...
class CustomFieldsController {
    constructor() {
        this.fieldGroupRepository = connection_1.AppDataSource.getRepository(CustomField_1.FieldGroup);
        this.customFieldRepository = connection_1.AppDataSource.getRepository(CustomField_1.CustomField);
    ...
    async createFieldGroup(req, res) {
        try {
            const { title, description, location, rules, options, active = true, order = 0, placement = 'normal', fields = [] } = req.body;
            if (!title || !title.trim()) {
            ...
            if (fields.length > 0) {
                const fieldEntities = fields.map((field, index) => this.customFieldRepository.create({
                    ...field,
    ...
    async updateFieldGroup(req, res) {
        try {
            const { id } = req.params;
            const { title, description, location, rules, options, active, order, placement, fields } = req.body;
            const fieldGroup = await this.fieldGroupRepository.findOne({
                where: { id },
                relations: ['fields']
            });
    ...
    async deleteFieldGroup(req, res) {
        try {
            ...
            if (fieldIds.length > 0) {
                await this.customFieldValueRepository.delete({ fieldId: (0, typeorm_1.In)(fieldIds) });
            }
    ...
    async createCustomField(req, res) {
        try {
            const { name, label, type, description, required = false, defaultValue, placeholder, validation, conditionalLogic, options, min, max, step, maxLength, minLength, pattern, multiple = false, order = 0, groupId } = req.body;
            if (!name || !name.trim()) {
                return res.status(400).json({
                    success: false,
    ...
    async saveCustomFieldValues(req, res) {
        try {
    ...
    async exportFieldGroups(req, res) {
        try {
            const { ids } = req.body;
            const queryBuilder = this.fieldGroupRepository
                .createQueryBuilder('fieldGroup')
    ...
    async importFieldGroups(req, res) {
        try {
            const { data } = req.body;
            ...
            for (const groupData of data.fieldGroups) {
                try {
                    ...
                    if (groupData.fields && Array.isArray(groupData.fields)) {
                        const fieldEntities = groupData.fields.map((fieldData) => this.customFieldRepository.create({
                            ...fieldData,
                            groupId: savedFieldGroup.id
                        }));
                        for (const fieldEntity of fieldEntities) {
