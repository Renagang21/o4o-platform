...
class ProductVariationController {
    constructor() {
        ...
        this.attributeRepository = connection_1.AppDataSource.getRepository(ProductAttribute_1.ProductAttribute);
        this.attributeValueRepository = connection_1.AppDataSource.getRepository(ProductAttributeValue_1.ProductAttributeValue);
        this.variationRepository = connection_1.AppDataSource.getRepository(ProductVariation_1.ProductVariation);
        /**
         * 상품 속성 추가
         */
        this.addProductAttribute = async (req, res) => {
            try {
                ...
                if (values && Array.isArray(values)) {
                    const attributeValues = values.map((value, index) => this.attributeValueRepository.create({
         ...
         * 상품 변형 생성
         */
        this.createProductVariation = async (req, res) => {
            var _a;
            try {
                const { productId } = req.params;
                ...
                for (const varData of variations) {
                    const { attributes, sku, price, stock = 0, images, weight, dimensions } = varData;
                    // 속성 조합 문자열 생성
                    ...
                    const variationSku = sku || `${product.sku}-${Object.values(attributes)
                        .map((attr) => attr.slug || attr.value.toLowerCase())
                        .join('-')}`;
                    // 변형 생성
                    const variation = this.variationRepository.create({
                        ...
                        attributes,
                        attributeString,
                        retailPrice: (price === null || price === void 0 ? void 0 : price.retail) || product.retailPrice,
                        salePrice: price === null || price === void 0 ? void 0 : price.sale,
                        wholesalePrice: (price === null || price === void 0 ? void 0 : price.wholesale) || product.wholesalePrice,
                        affiliatePrice: (price === null || price === void 0 ? void 0 : price.affiliate) || product.affiliatePrice,
                        stockQuantity: stock,
                        stockStatus: stock > 0 ? 'in_stock' : 'out_of_stock',
                        manageStock: true,
                        images,
                        imageUrl: (_a = images === null || images === void 0 ? void 0 : images[0]) === null || _a === void 0 ? void 0 : _a.url,
                        weight,
                        dimensions,
                        status: 'active',
                        enabled: true
         ...
         * 변형 자동 생성 (모든 속성 조합)
         */
        this.generateVariations = async (req, res) => {
            try {
                const { productId } = req.params;
                ...
                for (const combination of combinations) {
                    ...
                    for (const [attrSlug, valueSlug] of Object.entries(combination.slugMap)) {
                        const attrAdjustments = priceAdjustments[attrSlug];
                        if (attrAdjustments && typeof attrAdjustments === 'object') {
                            const adjustment = attrAdjustments[valueSlug];
         ...
         * 상품 변형 목록 조회
         */
        this.getProductVariations = async (req, res) => {
            try {
                const { productId } = req.params;
         ...
         * 변형 재고 업데이트
         */
        this.updateVariationStock = async (req, res) => {
            try {
         ...
         * 변형 가격 일괄 업데이트
         */
        this.bulkUpdateVariationPrices = async (req, res) => {
            try {
                const { productId } = req.params;
...
//# sourceMappingURL=productVariationController.js.map