...
const wordpress_transformer_1 = require("../utils/wordpress-transformer");
class CPTController {
    // ============= Custom Post Type Management =============
    // Get all CPTs
    static async getAllCPTs(req, res) {
        try {
            const cptRepo = connection_1.AppDataSource.getRepository(CustomPostType_1.CustomPostType);
    ...
    static async createCPT(req, res) {
        var _a;
        try {
            const { slug, name, singularName, description, icon, fieldGroups, settings } = req.body;
            const cptRepo = connection_1.AppDataSource.getRepository(CustomPostType_1.CustomPostType);
            ...
            const cpt = new CustomPostType_1.CustomPostType();
            cpt.slug = slug;
            cpt.name = name;
            ...
            cpt.createdBy = ((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) || '';
    ...
    static async getPostsByCPT(req, res) {
        try {
            const { slug } = req.params;
            const { page = 1, per_page = 10, // WordPress-style parameter
            status, search, orderby = 'date', // WordPress-style parameter
            order = 'desc', // WordPress-style parameter
            _embed = false // WordPress-style parameter
             } = req.query;
            const postRepo = connection_1.AppDataSource.getRepository(CustomPost_1.CustomPost);
            ...
            if (search) {
                queryBuilder.andWhere('(post.title ILIKE :search OR post.content ILIKE :search OR post.fields::text ILIKE :search)', { search: `%${search}%` });
            }
            ...
            const wpPosts = wordpress_transformer_1.WordPressTransformer.transformCustomPosts(posts, {
                includeContent: true,
    ...
    static async createPost(req, res) {
        ...
        try {
            ...
            post.status = status || CustomPost_1.PostStatus.DRAFT;
            post.meta = meta;
            post.authorId = ((_a = req.user) === null || _a === void 0 ? void 0 : _a.id) || '';
            if (status === CustomPost_1.PostStatus.PUBLISHED) {
    ...
    // ============= Helper Methods =============
    static validateFieldGroups(fieldGroups) {
    ...
    static validatePostFields(fields, fieldGroups) {
        const errors = [];
        let valid = true;
        for (const group of fieldGroups) {
    ...
    static async getPostById(req, res) {
        try {
            ...
            // Transform to WordPress format
            const wpPost = wordpress_transformer_1.WordPressTransformer.transformCustomPost(post, {
                includeContent: true,
                includeEmbedded: String(_embed) === 'true'
            });
            res.json(wpPost);
    ...
    static async getPublicPosts(req, res) {
        try {
            const { slug } = req.params;
            const { per_page = 10, page = 1, orderby = 'date', order = 'desc', _embed = false, search } = req.query;
            const postRepo = connection_1.AppDataSource.getRepository(CustomPost_1.CustomPost);
            const queryBuilder = postRepo.createQueryBuilder('post')
                .leftJoinAndSelect('post.postType', 'postType')
                .leftJoinAndSelect('post.author', 'author')
...
//# sourceMappingURL=cptController.js.map