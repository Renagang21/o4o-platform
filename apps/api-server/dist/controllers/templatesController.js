...
class TemplatesController {
    constructor() {
        this.templateRepository = connection_1.AppDataSource.getRepository(Template_1.Template);
    ...
    async getTemplates(req, res) {
        try {
            ...
            const systemTemplates = await this.getSystemTemplates(type);
            // Combine and return
            const allTemplates = [
                ...systemTemplates,
                ...userTemplates
    ...
    async getSystemTemplate(req, res) {
        try {
            const { name } = req.params;
            const templatesDir = path_1.default.join(process.cwd(), 'templates', 'system');
            const templatePath = path_1.default.join(templatesDir, `${name}.json`);
            try {
                const templateData = await promises_1.default.readFile(templatePath, 'utf-8');
    ...
    async createTemplate(req, res) {
        ...
        try {
            ...
            const { name, description, type = 'page', content, settings, customFields, preview, featured = false, tags, version = '1.0.0', compatibility } = req.body;
            if (!name || !name.trim()) {
    ...
    async updateTemplate(req, res) {
        ...
        try {
            ...
            const { name, description, type, content, settings, customFields, preview, active, featured, tags, version, compatibility } = req.body;
            await this.templateRepository.update(id, {
                name,
                description,
                type,
    ...
    async exportTemplate(req, res) {
        ...
        try {
            ...
            const exportData = {
                ...
                exportedBy: ((_a = template.author) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown'
            };
    ...
    async getSystemTemplates(type) {
        try {
            ...
            try {
                ...
                await promises_1.default.mkdir(templatesDir, { recursive: true });
                await this.createDefaultSystemTemplates(templatesDir);
            }
            const files = await promises_1.default.readdir(templatesDir);
            const templateFiles = files.filter((file) => file.endsWith('.json'));
            const systemTemplates = await Promise.all(templateFiles.map(async (file) => {
                try {
    ...
    async createDefaultSystemTemplates(templatesDir) {
        ...
        await Promise.all(defaultTemplates.map(async (template) => {
            const templatePath = path_1.default.join(templatesDir, template.filename);
            await promises_1.default.writeFile(templatePath, JSON.stringify(template.data, null, 2));
        }));
    }
}
