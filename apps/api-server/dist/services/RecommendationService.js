...
const ProductReview_2 = require("../entities/ProductReview");
...
class RecommendationService {
    constructor() {
        ...
        this.wishlistRepository = connection_1.AppDataSource.getRepository(ProductReview_2.Wishlist);
    ...
    async getPersonalizedRecommendations(userId, limit = 10) {
        try {
            ...
            simpleLogger_1.default.error('Failed to get personalized recommendations:', error);
     ...
     * 연관 상품 추천
     */
    async getRelatedProducts(productId, type, limit = 6) {
    ...
    async analyzeUserPreferences(userId) {
        ...
        orders.forEach(order => {
            var _a;
            (_a = order.items) === null || _a === void 0 ? void 0 : _a.forEach(item => {
                if (item.product) {
                    // 카테고리 선호도
                    const categoryId = item.product.categoryId;
                    if (categoryId) {
                        preference.categories.set(categoryId, (preference.categories.get(categoryId) || 0) + 1);
                    }
                    // 가격대
                    ...
                    if (brand) {
                        preference.brands.set(brand, (preference.brands.get(brand) || 0) + 1);
                    }
                }
            });
        });
     ...
     * 브라우징 이력 기반 추천
     */
    async getBrowsingBasedRecommendations(userId) {
        // Redis에서 최근 본 상품 조회
     ...
     * 위시리스트 기반 추천
     */
    async getWishlistBasedRecommendations(userId) {
     ...
     * 점수 병합
     */
    mergeScores(scores, newScores, weight) {
    ...
    async cacheRecommendations(userId, recommendations) {
        ...
        await this.redis.setex(cacheKey, 3600, // 1시간
    ...
    async getFallbackRecommendations(limit) {
        const bestSellers = await this.productRepository.find({
            where: { status: Product_1.ProductStatus.ACTIVE },
     ...
     * 사용자 행동 기록
     */
    async trackUserBehavior(userId, event, productId) {
     ...
     * A/B 테스트용 추천
     */
    async getABTestRecommendations(userId, variant, limit = 10) {
        if (variant === 'A') {
            // 기존 알고리즘
