...
class AutoScalingService {
    constructor() {
        ...
        this.performanceService = new PerformanceOptimizationService_1.PerformanceOptimizationService();
    ...
    async collectScalingMetrics() {
        try {
            ...
            // 메트릭 저장
            await this.redis.hset('scaling_metrics', 'current', JSON.stringify({
                ...metrics,
                timestamp: new Date().toISOString()
            }));
            // 메트릭 히스토리 저장
            await this.redis.lpush('scaling_history', JSON.stringify(metrics));
    ...
    async gatherSystemMetrics() {
        ...
        // 현재 활성 연결 수
        const activeConnections = connection_1.AppDataSource.isInitialized ?
    ...
    async getRequestRate() {
        try {
            ...
            const recentRequests = requests.filter((timestamp) => parseInt(timestamp) > oneMinuteAgo);
     ...
     * 메트릭 트렌드 분석
     */
    async analyzeMetricTrends(currentMetrics) {
        try {
            // 최근 5분간 메트릭 가져오기
            ...
            // 트렌드 정보 저장
            await this.redis.hset('scaling_trends', 'current', JSON.stringify(trends));
            // 예측 기반 스케일링 결정
    ...
    calculateTrends(history, current) {
        ...
        return {
            cpuTrend: this.calculateTrendDirection(older.map((m) => m.cpuUsage), recent.map((m) => m.cpuUsage)),
            memoryTrend: this.calculateTrendDirection(older.map((m) => m.memoryUsage), recent.map((m) => m.memoryUsage)),
            requestTrend: this.calculateTrendDirection(older.map((m) => m.requestRate), recent.map((m) => m.requestRate)),
            responseTrend: this.calculateTrendDirection(older.map((m) => m.avgResponseTime), recent.map((m) => m.avgResponseTime)),
     ...
     * 트렌드 방향 계산
     */
    calculateTrendDirection(older, recent) {
     ...
     * 예측 기반 스케일링 결정
     */
    async makePredictiveScalingDecision(trends) {
    ...
    async executeScalingAction(action) {
        try {
            ...
            await this.recordScalingError(action, error);
        }
    }
    /**
     * 스케일 업 실행
     */
    async scaleUp(step) {
        const targetInstances = Math.min(this.currentInstances.size + step, this.maxInstances);
     ...
     * 스케일 다운 실행
     */
    async scaleDown(step) {
        const targetInstances = Math.max(this.currentInstances.size - step, this.minInstances);
     ...
     * 로드 밸런서에 인스턴스 등록
     */
    async registerInstanceToLoadBalancer(instance) {
    ...
    async prepareForScaleDown() {
        ...
        if (instanceToRemove) {
            await this.redis.setex(`draining_${instanceToRemove.id}`, 300, '1');
        }
    }
    /**
     * 제거할 인스턴스 찾기
    ...
    findInstanceToRemove() {
        ...
        if (instances.length <= this.minInstances)
            return null;
        // 가장 적은 부하의 인스턴스 선택
        return instances.reduce((min, instance) => instance.currentLoad < min.currentLoad ? instance : min);
    }
    /**
     * 인스턴스 상태 모니터링
     */
     ...
     * 인스턴스 실패 횟수 조회
     */
    async getInstanceFailureCount(instanceId) {
        try {
     ...
     * 최적 가중치 계산
     */
    calculateOptimalWeight(instance) {
        // 부하가 낮을수록 높은 가중치
     ...
     * 인스턴스 가중치 업데이트
     */
    async updateInstanceWeight(instanceId, weight) {
     ...
     * 로드 밸런서 가중치 업데이트
     */
    async updateLoadBalancerWeights(instances) {
        const weights = instances.map((instance) => ({
            instanceId: instance.id,
            weight: this.calculateOptimalWeight(instance)
        }));
        await this.redis.hset('load_balancer_config', 'weights', JSON.stringify(weights));
    }
    /**
     * 스케일링 이벤트 기록
     */
    async recordScalingEvent(action) {
        const event = {
            ...action,
     ...
     * 스케일링 에러 기록
     */
    async recordScalingError(action, error) {
        const errorEvent = {
            ...action,
            error: {
     ...
     * 스케일링 알림 전송
     */
    async sendScalingNotification(action) {
        // 웹훅, 이메일, 슬랙 등으로 알림 전송
        const notification = {
            type: 'scaling_event',
            action: action.type,
     ...
     * 스케일링 설정 업데이트
     */
    async updateScalingConfiguration(config) {
