...
exports.crowdfundingService = exports.CrowdfundingService = void 0;
...
class CrowdfundingService extends events_1.EventEmitter {
    constructor() {
        super(...arguments);
        ...
        this.updateRepository = connection_1.AppDataSource.getRepository(FundingUpdate_1.FundingUpdate);
     ...
     * 프로젝트 생성
     */
    async createProject(creatorId, data) {
        ...
        simpleLogger_1.default.info(`Crowdfunding project created: ${project.id}`);
        // 관리자 알림
     ...
     * 리워드 생성
     */
    async createReward(projectId, data) {
        ...
        simpleLogger_1.default.info(`Reward created for project ${projectId}: ${reward.id}`);
        return reward;
    }
    /**
     * 프로젝트 승인
     */
    async approveProject(projectId, adminId) {
        const project = await this.projectRepository.findOne({ where: { id: projectId } });
        if (!project) {
            throw new Error('Project not found');
        }
        ...
        simpleLogger_1.default.info(`Project approved: ${projectId}`);
        return project;
    }
    /**
     * 프로젝트 거절
    ...
    async createBacking(data) {
        ...
        await this.projectRepository.save(project);
        simpleLogger_1.default.info(`Backing created: ${backing.id} for project ${data.projectId}`);
        // 목표 달성 확인
     ...
     * 결제 확인
     */
    async confirmPayment(backingId, paymentId) {
        ...
        backing.paymentStatus = 'completed';
        ...
        simpleLogger_1.default.info(`Payment confirmed for backing: ${backingId}`);
        return backing;
    }
    /**
     * 프로젝트 업데이트 작성
     */
    async createProjectUpdate(projectId, creatorId, data) {
        const project = await this.projectRepository.findOne({
        ...
        simpleLogger_1.default.info(`Project update created: ${update.id} for project ${projectId}`);
        return update;
    }
    /**
     * 펀딩 종료 처리
     */
    async endFunding(projectId) {
        const project = await this.projectRepository.findOne({
            where: { id: projectId }
        });
        if (!project) {
        ...
        await this.projectRepository.save(project);
        simpleLogger_1.default.info(`Funding ended for project ${projectId}: ${project.status}`);
        return project;
    }
    /**
     * 펀딩 성공 시 일반 상품으로 전환
     */
    async convertToProduct(project) {
        ...
        simpleLogger_1.default.info(`Project ${project.id} converted to product ${product.id}`);
        // 전환 알림
     ...
     * 환불 처리
     */
    async processRefunds(projectId) {
        ...
        for (const backing of backings) {
            try {
                ...
                if (backing.paymentId) {
                    await TossPaymentsService_1.tossPaymentsService.cancelPayment(backing.paymentId, '펀딩 실패로 인한 환불', backing.amount);
                ...
                simpleLogger_1.default.error(`Failed to refund backing ${backing.id}:`, error);
            }
        }
    }
    /**
     ...
     * 후원자 대시보드 데이터
     */
    async getBackerDashboard(backerId) {
        const backings = await this.backingRepository.find({
            where: { backerId },
        ...
        const stats = {
            ...
            successfulProjects: backings.filter(b => { var _a; return ((_a = b.project) === null || _a === void 0 ? void 0 : _a.status) === 'successful'; }).length
        };
        return {
            backings,
            stats
    ...
    async getProjects(filter = {}) {
        ...
        if (filter.search) {
            query.andWhere('(project.title LIKE :search OR project.description LIKE :search)', { search: `%${filter.search}%` });
        }
        // 정렬
    ...
    async updateProjectStatuses() {
        ...
        await this.projectRepository
            .createQueryBuilder()
            .update(FundingProject_1.FundingProject)
            .set({ status: 'ongoing' })
...
// 싱글톤 인스턴스
exports.crowdfundingService = new CrowdfundingService();
//# sourceMappingURL=CrowdfundingService.js.map