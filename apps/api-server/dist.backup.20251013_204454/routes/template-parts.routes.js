...
router.get('/area/:area/active', async (req, res) => {
    try {
        ...
        templateParts = templateParts.filter(part => {
            ...
            if (typeof conditions === 'string') {
                try {
                    ...
                    logger_1.default.warn(`Template part ${part.id} has invalid JSON conditions:`, parseError);
                    conditions = null;
                }
            }
            if (!conditions) {
            ...
            try {
                ...
                if (conditions.pages && contextData.pageId) {
                    if (!conditions.pages.includes(contextData.pageId))
                        return false;
                }
                // Check post type conditions
                if (conditions.postTypes && contextData.postType) {
                    if (!conditions.postTypes.includes(contextData.postType))
                        return false;
                }
                // Check category conditions
                if (conditions.categories && contextData.categories) {
                    const hasCategory = conditions.categories.some(cat => contextData.categories.includes(cat));
                    if (!hasCategory)
                        return false;
                }
                // Check user role conditions
                if (conditions.userRoles && contextData.userRole) {
                    if (!conditions.userRoles.includes(contextData.userRole))
                        return false;
                }
                return true;
            }
            catch (conditionError) {
                logger_1.default.warn(`Error evaluating conditions for template part ${part.id}:`, conditionError);
            ...
            try {
                ...
                if (typeof templatePart.content === 'string') {
                    try {
                        ...
                        logger_1.default.warn(`Template part ${templatePart.id} has invalid JSON content:`, parseError);
                        sanitizedContent = [];
                    }
                }
                ...
                if (!Array.isArray(sanitizedContent)) {
                    logger_1.default.warn(`Template part ${templatePart.id} content is not an array, converting to empty array`);
                    sanitizedContent = [];
                }
                return {
                    ...templatePart,
        ...
        logger_1.default.error('Template Parts API Error:', {
...
router.get('/:identifier', auth_middleware_1.authenticate, async (req, res) => {
    try {
...
router.put('/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.info('[Template Parts] PUT request received:', {
            id,
        ...
        if (validatedData.slug && validatedData.slug !== templatePart.slug) {
            const existing = await templatePartRepository.findOne({
                where: {
                    ...
                    id: (0, typeorm_1.Not)(id) // Exclude current record from uniqueness check
                }
            });
            if (existing) {
                return res.status(400).json({ error: 'Slug already exists' });
        ...
        if (error instanceof zod_1.z.ZodError) {
            logger_1.default.error('[Template Parts] Validation error:', error.issues);
        ...
        logger_1.default.error('[Template Parts] Error updating template part:', error);
        res.status(500).json({
