 ...
 */
router.put('/:id/toggle', auth_middleware_1.authenticate, permission_middleware_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!isActive) {
            const dependentPlugins = Array.from(pluginStates.values())
                .filter(p => { var _a; return ((_a = p.dependencies) === null || _a === void 0 ? void 0 : _a.includes(id)) && p.status === 'active'; });
 ...
 * @access  Private (Admin only)
 */
router.post('/:id/settings', auth_middleware_1.authenticate, permission_middleware_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/:id', auth_middleware_1.authenticate, permission_middleware_1.requireAdmin, async (req, res) => {
    try {
        ...
        // 이 플러그인에 의존하는 다른 플러그인 체크
        const dependentPlugins = Array.from(pluginStates.values())
            .filter(p => { var _a; return (_a = p.dependencies) === null || _a === void 0 ? void 0 : _a.includes(id); });
        if (dependentPlugins.length > 0) {
            return res.status(400).json({
                success: false,
 ...
 * @access  Private (Admin only)
 */
router.post('/install', auth_middleware_1.authenticate, permission_middleware_1.requireAdmin, async (req, res) => {
    try {
