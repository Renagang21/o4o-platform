...
class MediaController {
    constructor() {
        ...
        this.getMedia = async (req, res) => {
            try {
                const { page = 1, limit = 24, per_page = 24, // Support WordPress-style per_page parameter
                type, folderId, search, sortBy = 'uploadedAt', sortOrder = 'DESC' } = req.query;
                const pageNum = Math.max(1, parseInt(page) || 1);
                // Use per_page if provided (WordPress compatibility), otherwise use limit
                const perPageValue = per_page || limit;
                const limitNum = Math.min(1000, Math.max(1, parseInt(perPageValue) || 24));
                ...
                if (search) {
                    queryBuilder.andWhere('(media.filename ILIKE :search OR media.originalName ILIKE :search OR media.altText ILIKE :search)', { search: `%${search}%` });
                }
                // Apply sorting
        ...
        this.replaceMedia = [
            ...
            async (req, res) => {
                ...
                try {
                    ...
                    try {
                        const oldFilePath = path_1.default.join(process.cwd(), media.path);
                    ...
                    const uploadDir = path_1.default.join(process.cwd(), 'public', 'uploads', 'images');
                    await promises_1.default.mkdir(uploadDir, { recursive: true });
                    const newFilePath = path_1.default.join(uploadDir, media.filename);
                    await promises_1.default.writeFile(newFilePath, req.file.buffer);
                    // Update metadata
                    let metadata = {
                        size: req.file.size,
                        originalName: req.file.originalname,
    ...
    get folderRepository() {
        return connection_1.AppDataSource.getRepository(MediaFolder_1.MediaFolder);
    }
}
