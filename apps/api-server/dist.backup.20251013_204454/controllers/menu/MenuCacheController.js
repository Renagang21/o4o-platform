...
exports.MenuCacheController = void 0;
...
class MenuCacheController {
    ...
    async createMenuCache(req, res) {
        try {
            ...
            logger_1.default.error('Error creating menu cache:', error);
            res.status(500).json({
                success: false,
    ...
    async deleteMenuCache(req, res) {
        try {
            ...
            logger_1.default.error('Error deleting menu cache:', error);
            res.status(500).json({
                success: false,
    ...
    async getCacheStatus(req, res) {
        try {
            ...
            const oldestCache = cacheEntries.length > 0
                ? cacheEntries.reduce((oldest, entry) => entry.createdAt < oldest ? entry.createdAt : oldest, cacheEntries[0].createdAt)
                : null;
            const newestCache = cacheEntries.length > 0
                ? cacheEntries.reduce((newest, entry) => entry.createdAt > newest ? entry.createdAt : newest, cacheEntries[0].createdAt)
                : null;
            ...
            logger_1.default.error('Error getting cache status:', error);
            res.status(500).json({
                success: false,
    ...
    // HELPER METHODS
    // ============================================================================
    generateCacheKey(menuId) {
        return `menu:${menuId}:${Date.now()}`;
    }
    invalidateOldCacheEntries(menuId, currentKey) {
        for (const [key, entry] of this.cache.entries()) {
    ...
    async prerenderMenuHTML(menuTree) {
...
exports.MenuCacheController = MenuCacheController;
//# sourceMappingURL=MenuCacheController.js.map