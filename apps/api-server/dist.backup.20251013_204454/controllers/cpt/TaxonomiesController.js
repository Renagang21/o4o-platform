...
const Taxonomy_1 = require("../../entities/Taxonomy");
class TaxonomiesController {
    constructor() {
        this.taxonomyRepo = connection_1.AppDataSource.getRepository(Taxonomy_1.Taxonomy);
        this.termRepo = connection_1.AppDataSource.getTreeRepository(Taxonomy_1.Term);
        this.termRelationshipRepo = connection_1.AppDataSource.getRepository(Taxonomy_1.TermRelationship);
    }
    ...
    async getAllTaxonomies(req, res) {
        try {
            ...
            if (search) {
                queryBuilder.andWhere('(taxonomy.label ILIKE :search OR taxonomy.name ILIKE :search OR taxonomy.description ILIKE :search)', { search: `%${search}%` });
            }
            if (objectType) {
    ...
    async createTaxonomy(req, res) {
        try {
            const { name, label, description, objectTypes, labels, settings, hierarchical = false, public: isPublic = true, showUI = true, showInMenu = true, showInNavMenus = true, showTagcloud = true, showInQuickEdit = true, showAdminColumn = false } = req.body;
            const user = req.user;
    ...
    async getTermsByTaxonomy(req, res) {
        try {
            ...
            if (hierarchical === 'true' && taxonomy.hierarchical) {
                ...
                if (search) {
                    queryBuilder.andWhere('(term.name ILIKE :search OR term.description ILIKE :search)', { search: `%${search}%` });
                }
                if (parentId && parentId !== 'null') {
    ...
    // ============= Term Relationships =============
    async assignTermsToObject(req, res) {
        try {
            ...
            if (termIds && termIds.length > 0) {
                const relationships = termIds.map((termId, index) => this.termRelationshipRepo.create({
                    objectId,
                    objectType,
                    termId,
                    termOrder: index
...
//# sourceMappingURL=TaxonomiesController.js.map