...
exports.FieldGroupsController = void 0;
...
class FieldGroupsController {
    constructor() {
        ...
        this.customFieldRepo = connection_1.AppDataSource.getRepository(CustomField_1.CustomField);
        this.fieldValueRepo = connection_1.AppDataSource.getRepository(CustomField_1.CustomFieldValue);
    }
    async getAllFieldGroups(req, res) {
        try {
            ...
            if (search) {
                queryBuilder.andWhere('(fieldGroup.title ILIKE :search OR fieldGroup.description ILIKE :search)', { search: `%${search}%` });
            }
            if (active !== undefined) {
    ...
    async createFieldGroup(req, res) {
        try {
            ...
            if (fields && fields.length > 0) {
                const customFields = fields.map((field, index) => this.customFieldRepo.create({
                    ...field,
    ...
    async duplicateFieldGroup(req, res) {
        try {
            const { id } = req.params;
            ...
            if (originalFieldGroup.fields && originalFieldGroup.fields.length > 0) {
                const duplicateFields = originalFieldGroup.fields.map(field => this.customFieldRepo.create({
                    ...field,
                    id: undefined, // Let TypeORM generate new ID
                    groupId: savedFieldGroup.id
                }));
    ...
    async toggleFieldGroupStatus(req, res) {
        try {
            const { id } = req.params;
            const user = req.user;
            if (!user || user.role !== 'admin') {
    ...
    async reorderFieldGroups(req, res) {
        try {
    ...
    async getFieldGroupsByLocation(req, res) {
        try {
            ...
            queryBuilder.andWhere("fieldGroup.location::text LIKE :postType", { postType: `%"value":"${postType}"%` });
...
exports.FieldGroupsController = FieldGroupsController;
//# sourceMappingURL=FieldGroupsController.js.map