...
class BusinessInfoController {
    ...
    static async createBusinessInfo(req, res) {
        try {
            const { id: userId } = req.params;
            ...
            const errors = await (0, class_validator_1.validate)(businessInfo);
            if (errors.length > 0) {
                res.status(400).json({
                    success: false,
            ...
            // Log the activity
            const activityData = UserActivityLog_1.UserActivityLog.createProfileUpdateActivity(userId, ['business_info_created'], currentUser.id, { businessName: savedBusinessInfo.businessName });
        ...
        try {
            ...
            updatableFields.forEach(field => {
                if (req.body[field] !== undefined) {
                    const oldValue = businessInfo[field];
                    const newValue = req.body[field];
                    if (JSON.stringify(oldValue) !== JSON.stringify(newValue)) {
                        changedFields.push(field);
                        businessInfo[field] = newValue;
                    }
                }
            });
            ...
            const savedBusinessInfo = await BusinessInfoController.businessInfoRepository.save(businessInfo);
            // Log the activity
            const activityData = UserActivityLog_1.UserActivityLog.createProfileUpdateActivity(userId, changedFields.map(field => `business_${field}`), currentUser.id, {
                businessName: savedBusinessInfo.businessName,
                changedFields
            });
            const activity = BusinessInfoController.activityRepository.create(activityData);
    ...
    static async deleteBusinessInfo(req, res) {
        try {
            ...
            // Log the activity
            const activityData = UserActivityLog_1.UserActivityLog.createProfileUpdateActivity(userId, ['business_info_deleted'], currentUser.id, { deletedBusinessName: businessName });
            const activity = BusinessInfoController.activityRepository.create(activityData);
            await BusinessInfoController.activityRepository.save(activity);
            res.status(200).json({
                success: true,
    ...
    static async verifyBusinessInfo(req, res) {
        try {
            const { id: userId } = req.params;
            ...
            // Log the activity
            const activityData = UserActivityLog_1.UserActivityLog.createAdminApprovalActivity(userId, currentUser.id, `Business information verified: ${businessInfo.businessName}`);
            const activity = BusinessInfoController.activityRepository.create(activityData);
            await BusinessInfoController.activityRepository.save(activity);
            res.status(200).json({
                success: true,
    ...
    static async getBusinessTypes(req, res) {
        try {
            const businessTypes = Object.values(BusinessInfo_1.BusinessType).map(type => ({
                value: type,
    ...
    static async getBusinessSizes(req, res) {
        try {
            const businessSizes = Object.values(BusinessInfo_1.BusinessSize).map(size => ({
                value: size,
                label: size.charAt(0).toUpperCase() + size.slice(1) +
                    (size === BusinessInfo_1.BusinessSize.MICRO ? ' (1-9 employees)' :
                        size === BusinessInfo_1.BusinessSize.SMALL ? ' (10-49 employees)' :
                            size === BusinessInfo_1.BusinessSize.MEDIUM ? ' (50-249 employees)' :
                                size === BusinessInfo_1.BusinessSize.LARGE ? ' (250-999 employees)' :
                                    size === BusinessInfo_1.BusinessSize.ENTERPRISE ? ' (1000+ employees)' : '')
            }));
            res.status(200).json({
                success: true,
                data: businessSizes
    ...
    static async getIndustries(req, res) {
        try {
            const industries = Object.values(BusinessInfo_1.Industry).map(industry => ({
                value: industry,
                label: industry === BusinessInfo_1.Industry.FOOD_BEVERAGE ? 'Food & Beverage' :
                    industry.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
            }));
            res.status(200).json({
                success: true,
                data: industries
    ...
    static async getBusinessStatistics(req, res) {
        try {
            ...
            const businessTypeStats = await Promise.all(Object.values(BusinessInfo_1.BusinessType).map(async (type) => ({
                type,
                count: await BusinessInfoController.businessInfoRepository.count({ where: { businessType: type } }),
                label: type.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
            })));
            // Distribution by industry
            const industryStats = await Promise.all(Object.values(BusinessInfo_1.Industry).map(async (industry) => ({
                industry,
            ...
            const businessSizeStats = await Promise.all(Object.values(BusinessInfo_1.BusinessSize).map(async (size) => ({
                size,
...
BusinessInfoController.businessInfoRepository = data_source_1.default.getRepository(BusinessInfo_1.BusinessInfo);
BusinessInfoController.userRepository = data_source_1.default.getRepository(User_1.User);
BusinessInfoController.activityRepository = data_source_1.default.getRepository(UserActivityLog_1.UserActivityLog);
//# sourceMappingURL=businessInfo.controller.js.map