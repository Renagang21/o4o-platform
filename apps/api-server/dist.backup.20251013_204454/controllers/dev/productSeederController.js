...
exports.ProductSeederController = void 0;
 ...
 */
class ProductSeederController {
    constructor() {
        this.wordpressApiUrl = process.env.WORDPRESS_API_URL || 'https://neture.co.kr/wp-json/wp/v2';
        this.wordpressUser = process.env.WORDPRESS_USER || 'admin';
        this.wordpressPassword = process.env.WORDPRESS_APPLICATION_PASSWORD || '';
    }
    /**
     * 샘플 상품 데이터 생성기
     */
...
주요 특징: 최신 기술 적용, 프리미엄 소재 사용, A/S 보증 포함`,
     ...
     */
    async seedSingleProduct(req, res) {
        try {
            // 개발 환경 체크
     ...
     */
    async seedMultipleProducts(req, res) {
        try {
            // 개발 환경 체크
            if (process.env.NODE_ENV === 'production') {
                return res.status(403).json({
            ...
            for (let i = 0; i < Math.min(count, products.length); i++) {
                try {
                    logger_1.default.info(`Creating product ${i + 1}/${count}: ${products[i].title}`);
     ...
     */
    async createWordPressProduct(productData) {
        const auth = Buffer.from(`${this.wordpressUser}:${this.wordpressPassword}`).toString('base64');
        const response = await axios_1.default.post(`${this.wordpressApiUrl}/ds_product`, {
     ...
     */
    async checkProducts(req, res) {
        try {
            const auth = Buffer.from(`${this.wordpressUser}:${this.wordpressPassword}`).toString('base64');
            const response = await axios_1.default.get(`${this.wordpressApiUrl}/ds_product?per_page=100`, {
                headers: {
                    'Authorization': `Basic ${auth}`
                }
            });
    ...
    async clearProducts(req, res) {
        ...
        try {
            ...
            for (const product of products) {
                if (((_a = product.acf) === null || _a === void 0 ? void 0 : _a.created_by) === 'seeder') {
                    try {
                        await axios_1.default.delete(`${this.wordpressApiUrl}/ds_product/${product.id}`, {
                            headers: {
                                'Authorization': `Basic ${auth}`
                            }
                        });
...
exports.ProductSeederController = ProductSeederController;
exports.default = new ProductSeederController();
//# sourceMappingURL=productSeederController.js.map