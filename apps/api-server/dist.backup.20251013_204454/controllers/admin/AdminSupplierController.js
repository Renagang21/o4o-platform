...
exports.AdminSupplierController = void 0;
    ...
    constructor() {
        ...
        this.getSuppliers = async (req, res) => {
            try {
                ...
                // Apply search filter
                if (search) {
                    queryBuilder.where('(supplier.contactPerson ILIKE :search OR supplier.contactEmail ILIKE :search OR user.email ILIKE :search)', { search: `%${search}%` });
                }
                // Apply status filter
        ...
        this.getSupplier = async (req, res) => {
            try {
                const { id } = req.params;
                const supplierRepo = (0, typeorm_1.getRepository)(Supplier_1.Supplier);
        ...
        this.createSupplier = async (req, res) => {
            try {
                ...
                const { userId, contactPerson, contactEmail, contactPhone, companyDescription, specialties, certifications, website, isActive = true, status = Supplier_1.SupplierStatus.PENDING, tier = 'basic', taxId, bankName, bankAccount, accountHolder } = req.body;
        ...
        this.updateSupplier = async (req, res) => {
            try {
                ...
                const { userId, contactPerson, contactEmail, contactPhone, companyDescription, specialties, certifications, website, isActive, status, tier, taxId, bankName, bankAccount, accountHolder, defaultPartnerCommissionRate } = req.body;
                // Update fields
                if (userId !== undefined)
                ...
                if (accountHolder !== undefined)
                    supplier.accountHolder = accountHolder;
                if (defaultPartnerCommissionRate !== undefined)
                    supplier.defaultPartnerCommissionRate = defaultPartnerCommissionRate;
        ...
        this.updateSupplierStatus = async (req, res) => {
            try {
                const { id } = req.params;
        ...
        // Approve supplier
        this.approveSupplier = async (req, res) => {
            var _a;
            try {
                const { id } = req.params;
        ...
        this.getSupplierStatistics = async (req, res) => {
            try {
                const supplierRepo = (0, typeorm_1.getRepository)(Supplier_1.Supplier);
                const [totalSuppliers, activeSuppliers, approvedSuppliers, suppliersByStatus, topSuppliers] = await Promise.all([
                    supplierRepo.count(),
                    supplierRepo.count({ where: { isActive: true } }),
                    supplierRepo.count({ where: { status: Supplier_1.SupplierStatus.APPROVED } }),
                    supplierRepo
...
exports.AdminSupplierController = AdminSupplierController;
//# sourceMappingURL=AdminSupplierController.js.map