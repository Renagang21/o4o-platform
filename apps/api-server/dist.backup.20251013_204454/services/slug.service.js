...
class SlugService {
     ...
     */
    async ensureUniquePostSlug(input, excludeId, isSlug = false) {
        let baseSlug;
        if (isSlug) {
     ...
     */
    async ensureUniquePageSlug(slugOrTitle, excludeId) {
     ...
     */
    async ensureUniqueTagSlug(slugOrName, excludeId) {
     ...
     */
    async ensureUniqueCategorySlug(slugOrName, excludeId) {
        // If it looks like a slug already, use it directly, otherwise generate from name
        let baseSlug = slugOrName.includes(' ') || /[A-Z가-힣]/.test(slugOrName)
            ? this.generateSlug(slugOrName)
            : slugOrName;
     ...
     */
    getSuggestedSlugs(title, count = 5) {
    ...
    async isPostSlugTaken(slug, excludeId) {
    ...
    async isTagSlugTaken(slug, excludeId) {
    ...
    async isCategorySlugTaken(slug, excludeId) {
