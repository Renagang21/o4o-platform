...
exports.APPROVAL_WORKFLOW_SCRIPT = exports.PRICING_CALCULATION_SCRIPT = exports.registerDropshippingACFFields = exports.DROPSHIPPING_ACF_GROUPS = void 0;
const CustomField_1 = require("../../entities/CustomField");
const connection_1 = require("../../database/connection");
const logger_1 = __importDefault(require("../../utils/logger"));
exports.DROPSHIPPING_ACF_GROUPS = [
    {
...
async function registerDropshippingACFFields() {
    const fieldGroupRepo = connection_1.AppDataSource.getRepository(CustomField_1.FieldGroup);
    const customFieldRepo = connection_1.AppDataSource.getRepository(CustomField_1.CustomField);
    for (const groupDef of exports.DROPSHIPPING_ACF_GROUPS) {
        ...
        if (!fieldGroup) {
            fieldGroup = fieldGroupRepo.create({
                ...
                options: {
                    ...
                    style: groupDef.style,
                    labelPlacement: (groupDef.label_placement || 'top'),
                    instructionPlacement: (groupDef.instruction_placement || 'label')
                },
                placement: groupDef.position,
                active: true,
                order: 0
            });
            await fieldGroupRepo.save(fieldGroup);
            logger_1.default.info(`Created ACF field group: ${groupDef.title}`);
        }
        ...
        for (let i = 0; i < groupDef.fields.length; i++) {
            ...
            if (!existingField) {
                ...
                if (fieldDef.type === 'true_false')
                    fieldType = 'toggle';
                if (fieldDef.type === 'date_picker')
                    fieldType = 'date';
                const field = customFieldRepo.create({
                    ...
                    groupId: fieldGroup.id,
                    order: i,
                    description: fieldDef.instructions || '',
                    required: fieldDef.required || false,
                    defaultValue: String(fieldDef.default_value || ''),
                    placeholder: fieldDef.placeholder || '',
                    min: fieldDef.min,
                    max: fieldDef.max,
                    step: fieldDef.step ? Number(fieldDef.step) : undefined,
                    options: fieldDef.choices ? Object.entries(fieldDef.choices).map(([value, label]) => ({ value, label: String(label) })) : undefined,
                    multiple: fieldDef.max && fieldDef.max > 1 ? true : false,
                    validation: fieldDef.readonly ? { custom: 'readonly' } : undefined
                });
                await customFieldRepo.save(field);
                logger_1.default.info(`Created ACF field: ${fieldDef.name}`);
            }
        }
    }
}
exports.registerDropshippingACFFields = registerDropshippingACFFields;
// Enhanced pricing calculation JavaScript (법률 준수 및 자율 가격)
exports.PRICING_CALCULATION_SCRIPT = `
<script>
(function($) {
  'use strict';
...
exports.APPROVAL_WORKFLOW_SCRIPT = `
<script>
(function($) {
  'use strict';
...
`;
//# sourceMappingURL=dropshipping-fields.js.map