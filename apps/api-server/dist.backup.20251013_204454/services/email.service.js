...
class EmailService {
    constructor() {
        ...
        logger_1.default.info(`EMAIL_SERVICE_ENABLED=${envValue}`);
        this.isEnabled = envValue !== 'false';
        if (this.isEnabled) {
            logger_1.default.info('Email service is enabled, creating transporter...');
            this.transporter = this.createTransport();
        }
        else {
            logger_1.default.info('Email service is disabled via EMAIL_SERVICE_ENABLED environment variable');
        }
    }
    createTransport() {
        const { SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASS, NODE_ENV, EMAIL_SERVICE_ENABLED } = process.env;
        // Check if email service is explicitly disabled
        if (EMAIL_SERVICE_ENABLED === 'false') {
            logger_1.default.info('Email service disabled by configuration');
            return null;
        }
        // Development mode: Use ethereal email for testing
        if (NODE_ENV === 'development' && !SMTP_HOST) {
            logger_1.default.info('Email service running in development mode with console output');
        ...
        if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
            logger_1.default.warn('SMTP configuration incomplete. Email service will be disabled.');
            logger_1.default.warn('Required: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS');
        ...
        try {
            ...
            logger_1.default.info('Email transporter created with SMTP settings');
            return transporter;
        }
        catch (error) {
            logger_1.default.error('Failed to create email transporter:', error);
            return null;
        }
    }
    async initialize() {
        ...
        if (!this.isEnabled) {
            logger_1.default.info('Email service initialization skipped (service disabled)');
            return;
        }
        // Skip if transporter wasn't created
        if (!this.transporter) {
            logger_1.default.warn('Email service initialization skipped (no transporter available)');
            this.isInitialized = false;
            return;
        }
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Email service verification timeout')), 10000));
            await Promise.race([verifyPromise, timeoutPromise]);
            this.isInitialized = true;
            logger_1.default.info('Email service initialized and verified successfully');
        }
        catch (error) {
    ...
    async sendEmail(options) {
        ...
        if (!this.isEnabled) {
            logger_1.default.debug('Email service is disabled, skipping email send', {
                to: options.to,
        ...
        if (!this.transporter) {
            logger_1.default.warn('Email transporter not available, skipping email send', {
                to: options.to,
                subject: options.subject
            });
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development' && !process.env.SMTP_HOST) {
                logger_1.default.info('Development email sent:', {
                    to,
                    subject,
                ...
                if (info.message) {
                    logger_1.default.debug('Email content preview available');
                }
            }
            else {
...
<body>
  <div class="container">
    ...
    async sendUserApprovalEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/userApproved.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            ...
            logger_1.default.error('Failed to send approval email:', error);
            throw error;
        }
    }
    // Send user rejection email
    async sendUserRejectionEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/userRejected.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            // Replace placeholders
            htmlTemplate = htmlTemplate
            ...
            logger_1.default.error('Failed to send rejection email:', error);
            throw error;
        }
    }
    // Send account suspension email
    async sendAccountSuspensionEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/accountSuspended.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            // Replace placeholders
            htmlTemplate = htmlTemplate
            ...
            logger_1.default.error('Failed to send suspension email:', error);
            throw error;
        }
    }
    // Send account reactivation email
    async sendAccountReactivationEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/accountReactivated.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            // Replace placeholders
            htmlTemplate = htmlTemplate
            ...
            logger_1.default.error('Failed to send reactivation email:', error);
            throw error;
        }
    }
    // Send commission calculated email
    async sendCommissionCalculatedEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/commissionCalculated.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            // Replace placeholders
            htmlTemplate = htmlTemplate
            ...
            logger_1.default.error('Failed to send commission calculated email:', error);
            throw error;
        }
    }
    // Send settlement request email
    async sendSettlementRequestEmail(to, data) {
        const templatePath = path_1.default.join(__dirname, '../templates/email/settlementRequest.html');
        try {
            let htmlTemplate = await promises_1.default.readFile(templatePath, 'utf8');
            // Replace placeholders
            htmlTemplate = htmlTemplate
            ...
            logger_1.default.error('Failed to send settlement request email:', error);
            throw error;
        }
    }
