...
class AIJobWorker {
    constructor() {
        ...
        this.CONCURRENCY = 10; // Sprint 4: Increased from 5 to 10 for better throughput
        ...
        // Create worker
        this.worker = new bullmq_1.Worker(this.QUEUE_NAME, async (job) => {
        ...
        this.worker.on('active', (job) => {
            logger_1.default.info('AI job started', {
                jobId: job.id,
        ...
        this.worker.on('completed', (job, result) => {
            logger_1.default.info('AI job completed', {
                jobId: job.id,
                userId: job.data.userId,
                provider: job.data.provider,
                model: job.data.model,
            ...
            prometheus_metrics_service_1.prometheusMetrics.recordJobCompletion(job.data.provider, job.data.model, result.success ? 'success' : 'failed', result.duration, result.retryCount, result.usage ? {
        ...
        this.worker.on('failed', async (job, error) => {
            if (job) {
                logger_1.default.error('AI job failed', {
                    jobId: job.id,
                    userId: job.data.userId,
                    provider: job.data.provider,
                    model: job.data.model,
                ...
                if (job.attemptsMade >= maxAttempts) {
                    await ai_dlq_service_1.aiDLQ.moveToDLQ(job, error.message, job.attemptsMade);
                }
            }
            else {
                logger_1.default.error('AI job failed (no job info)', { error: error.message });
            }
        });
        this.worker.on('error', (error) => {
            logger_1.default.error('AI worker error', { error: error.message });
        });
        logger_1.default.info('AI job worker started', {
    ...
    async cleanup() {
        ...
        logger_1.default.info('AI job worker cleaned up');
    }
}
// Start worker (singleton)
exports.aiJobWorker = AIJobWorker.getInstance();
//# sourceMappingURL=ai-job.worker.js.map