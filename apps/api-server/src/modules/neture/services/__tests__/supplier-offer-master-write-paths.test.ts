 ...
 *
 * offer.service 의 write 경로가 실제로 ProductMaster 를 건드리는지 확인한다.
 *   1) (은퇴) resolveProductMetadata — CUTOVER-AND-LEGACY-MASTER-RESOLUTION-RETIREMENT-V1 로 메서드 제거
...
describe('(은퇴) resolveProductMetadata — 공급자 Offer 경로의 Master 재추론/write 경로 자체가 없다', () => {
  /**
   * WO-O4O-SUPPLIER-PRODUCT-REGISTRATION-AI-FIRST-CUTOVER-AND-LEGACY-MASTER-RESOLUTION-RETIREMENT-V1 §2.3
   * 기존 기대("created=false 면 UPDATE 0 · created=true 면 updateProductMaster 1회")는 메서드 제거로 더 이상 유효하지 않다.
   * 새 계약: 공급자 Offer 경로에서 resolveOrCreateMaster · updateProductMaster 호출 0 (신규 Master 도 만들지 않는다).
   */
  it('NetureOfferService 에 resolveProductMetadata / createSupplierOffer 가 없다', () => {
    const { svc } = buildService(true);
    const anySvc = svc as unknown as Record<string, unknown>;
    expect(anySvc.resolveProductMetadata).toBeUndefined();
    expect(anySvc.createSupplierOffer).toBeUndefined();
  });

  it('offer.service 소스(주석 제외)에 resolveOrCreateMaster( · updateProductMaster( · resolveBrandId · ProductImportCommonService 참조 0', () => {
    const raw = fs.readFileSync(path.join(__dirname, '..', 'offer.service.ts'), 'utf-8');
    const src = raw.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^[ \t]*\/\/.*$/gm, '');
    expect(src).not.toContain('resolveOrCreateMaster(');
    expect(src).not.toContain('updateProductMaster(');
    expect(src).not.toContain('resolveBrandId');
    expect(src).not.toContain('ProductImportCommonService');
    expect(src).not.toContain('resolveProductMetadata');
    // 공급자 Offer 생성 primitive 는 1개 · 호출자는 from-master 1개
    expect(src.match(/private async persistOfferForResolvedMaster\(/g)).toHaveLength(1);
    expect(src.match(/this\.persistOfferForResolvedMaster\(/g)).toHaveLength(1);
  });
});
