/**
 * Quasi-Drug Permit Candidate Import Service — 의약외품 raw JSONL → ProductCandidate 후보 적재
 *
 ...
 *  - EE/UD/NB XML 은 파싱하지 않고 rawPayload 원문 보존.
 *
 * 중복 기준 (WO):
 *   sourceType=external_api + identifierType=MFDS_CODE + normalizedIdentifierValue=ITEM_SEQ
 *   + rawPayload->>'sourceKind'='quasi_drug_permit' + deleted_at IS NULL
...
import {
  ...
  type ParsedQuasiDrugRow,
} from './quasi-drug-permit-jsonl.parser.js';
import {
  mapQuasiDrugRow,
  QUASI_DRUG_SOURCE_LABEL,
  QUASI_DRUG_SOURCE_KIND,
  type MappedQuasiDrugCandidate,
  type QuasiDrugReviewFlag,
} from './quasi-drug-permit-candidate.mapper.js';

export interface QuasiDrugImportOptions {
...
export interface QuasiDrugImportReport {
  mode: 'dry-run' | 'apply';
  sourceFileName: string;
  sourceLabel: string;
  totalRows: number;
  ...
  /** candidate_name 255자 초과로 truncate 된 건수 */
  candidateNameTruncatedCount: number;
  officialTextPresentCount: number;
  officialTextMissingCount: number;
  reviewFlagCounts: Record<QuasiDrugReviewFlag, number>;
...
const EMPTY_FLAG_COUNTS = (): Record<QuasiDrugReviewFlag, number> => ({
  ITEM_SEQ_MISSING: 0,
  ITEM_NAME_MISSING: 0,
...
export class QuasiDrugPermitCandidateImportService {
  async run(opts: QuasiDrugImportOptions): Promise<QuasiDrugImportReport> {
    const mode: 'dry-run' | 'apply' = opts.apply ? 'apply' : 'dry-run';
    const notes: string[] = [];

    const parsed = parseQuasiDrugPermitJsonl(opts.text);

    const report: QuasiDrugImportReport = {
      mode,
    ...
    // 1) 매핑
    const mapped: MappedQuasiDrugCandidate[] = [];
    for (const r of rows) {
      try {
        const m = mapQuasiDrugRow(r, { serviceKey: opts.serviceKey ?? null });
        mapped.push(m);
        ...
        if (m.nameTruncated) report.candidateNameTruncatedCount += 1;
    ...
    report.sampleMappedRows = mapped.slice(0, 3).map((m) => ({
      ...
      candidateNameLength: m.candidateInput.candidateName?.length ?? 0,
      ...
      nameTruncated: m.nameTruncated,
      reviewFlags: m.reviewFlags,
      officialRegulatoryTextKeys: Object.keys(
        (m.candidateInput.rawPayload.officialRegulatoryText as Record<string, unknown>) ?? {},
      ),
    }));
  ...
  /** dedup 키: ITEM_SEQ 없으면 식별 불가(null) → skip 분류 */
  private dedupKeyOf(m: MappedQuasiDrugCandidate): string | null {
  ...
  private predictOffline(mapped: MappedQuasiDrugCandidate[], report: QuasiDrugImportReport): void {
    const seen = new Set<string>();
    for (const m of mapped) {
      const key = this.dedupKeyOf(m);
      if (key == null) {
        report.counts.skipped += 1; // ITEM_SEQ 결측 → 식별 불가
        continue;
      }
      if (seen.has(key)) {
  ...
  private async predictWithDb(
    ds: DataSource,
    mapped: MappedQuasiDrugCandidate[],
    report: QuasiDrugImportReport,
  ): Promise<void> {
    const seen = new Set<string>();
    for (const m of mapped) {
      const key = this.dedupKeyOf(m);
   ...
   *   + normalized_identifier_value=ITEM_SEQ + raw_payload->>'sourceKind'='quasi_drug_permit'
   *   + deleted_at IS NULL
   */
  private async findExisting(
    ds: DataSource,
    m: MappedQuasiDrugCandidate,
   ...
   *    candidate_name 은 mapper 에서 이미 255자 truncate 됨.
   */
