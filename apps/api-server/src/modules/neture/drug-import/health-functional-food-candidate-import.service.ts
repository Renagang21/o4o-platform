/**
 * Health Functional Food Candidate Import Service — 건강기능식품 raw JSONL → ProductCandidate 후보 적재
 *
 * WO-O4O-HEALTH-FUNCTIONAL-FOOD-PUBLIC-CANDIDATE-IMPORT-MAPPER-AND-SERVICE-V1
 * 선행 dry-run: WO-O4O-HEALTH-FUNCTIONAL-FOOD-PUBLIC-FULL-RAW-FETCH-AND-FULL-DRYRUN-V1
 *
 * 안전 경계 (이 WO 고유):
 *  - dry-run 이 기본. --apply 명시 시에만 DB write. (이 WO 에서 --apply 는 프로덕션 미실행.)
 ...
 *
 * 중복 기준 (WO):
 *   sourceType=external_api + identifierType=MFDS_STTEMNT_NO + normalizedIdentifierValue=STTEMNT_NO
 *   + rawPayload->>'sourceKind'='health_functional_food' + deleted_at IS NULL
...
import {
  ...
  type ParsedHealthFunctionalFoodRow,
} from './health-functional-food-jsonl.parser.js';
import {
  mapHealthFunctionalFoodRow,
  HFF_SOURCE_LABEL,
  HFF_SOURCE_KIND,
  HFF_IDENTIFIER_TYPE,
  type MappedHealthFunctionalFoodCandidate,
  type HealthFunctionalFoodReviewFlag,
} from './health-functional-food-candidate.mapper.js';

export interface HealthFunctionalFoodImportOptions {
...
export interface HealthFunctionalFoodImportReport {
  mode: 'dry-run' | 'apply';
  sourceFileName: string;
  sourceLabel: string;
  totalRows: number;
  ...
  /** candidate_name 방어적 truncate 발생 건수 */
  nameTruncatedCount: number;
  reviewFlagCounts: Record<HealthFunctionalFoodReviewFlag, number>;
...
const EMPTY_FLAG_COUNTS = (): Record<HealthFunctionalFoodReviewFlag, number> => ({
  STTEMNT_NO_MISSING: 0,
  PRDUCT_MISSING: 0,
  MANUFACTURER_MISSING: 0,
  MAIN_FUNCTION_MISSING: 0,
  PRESERVATION_MISSING: 0,
  INTAKE_HINT_MISSING: 0,
  CANDIDATE_NAME_OVERLENGTH: 0,
  SKU_IDENTIFIER_MISSING: 0,
});

export class HealthFunctionalFoodCandidateImportService {
  async run(opts: HealthFunctionalFoodImportOptions): Promise<HealthFunctionalFoodImportReport> {
    const mode: 'dry-run' | 'apply' = opts.apply ? 'apply' : 'dry-run';
    const notes: string[] = [];

    const parsed = parseHealthFunctionalFoodJsonl(opts.text);

    const report: HealthFunctionalFoodImportReport = {
      mode,
    ...
    // 1) 매핑
    const mapped: MappedHealthFunctionalFoodCandidate[] = [];
    for (const r of rows) {
      try {
        const m = mapHealthFunctionalFoodRow(r, { serviceKey: opts.serviceKey ?? null });
        mapped.push(m);
    ...
    report.sampleMappedRows = mapped.slice(0, 3).map((m) => ({
      ...
      mainFunction: m.candidateInput.rawPayload.mainFunction ?? null,
    }));
  ...
  /** dedup 키: STTEMNT_NO 없으면 식별 불가(null) → skip 분류 */
  private dedupKeyOf(m: MappedHealthFunctionalFoodCandidate): string | null {
  ...
  private predictOffline(
    mapped: MappedHealthFunctionalFoodCandidate[],
    report: HealthFunctionalFoodImportReport,
  ): void {
    const seen = new Set<string>();
    for (const m of mapped) {
      const key = this.dedupKeyOf(m);
      if (key == null) {
        report.counts.skipped += 1; // STTEMNT_NO 결측 → 식별 불가
        continue;
      }
      if (seen.has(key)) {
   ...
   *   source_type='external_api' + identifier_type='MFDS_STTEMNT_NO'
   *   + normalized_identifier_value=STTEMNT_NO + raw_payload->>'sourceKind'='health_functional_food'
   *   + deleted_at IS NULL
   */
  private async findExisting(
    ds: DataSource,
    m: MappedHealthFunctionalFoodCandidate,
    ...
    const rows: Array<{ id: string }> = await ds.query(
      `SELECT id FROM product_candidates
        WHERE source_type = 'external_api'
          AND identifier_type = $1
          AND normalized_identifier_value = $2
          AND raw_payload->>'sourceKind' = $3
          AND deleted_at IS NULL
        LIMIT 1`,
      [HFF_IDENTIFIER_TYPE, norm, HFF_SOURCE_KIND],
    );
    return rows[0] ?? null;
  }
