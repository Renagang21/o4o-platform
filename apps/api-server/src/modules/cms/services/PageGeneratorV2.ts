...
import { ViewType, type ViewSchema } from '../entities/View.js';
...
export class PageGeneratorV2 {
  ...
  async generatePage(options: GeneratePageOptions): Promise<Page> {
    ...
    if (!view.isCompatibleWithViewRenderer()) {
      throw new Error(`View '${options.viewSlug}' is not V2 compatible. Schema version must be 2.0.`);
    }
  ...
  async generatePagesFromCPT(options: GeneratePagesFromCPTOptions): Promise<Page[]> {
    ...
    if (!view.isCompatibleWithViewRenderer()) {
      throw new Error(`View '${options.viewSlug}' is not V2 compatible`);
    }
  ...
  async updatePageFromView(pageId: string): Promise<Page> {
    ...
    // 3. Update Page content with new View schema
   ...
   */
  async previewPage(options: GeneratePageOptions): Promise<{ viewSchema: any; content: any }> {
    const view = await this.viewService.getViewBySlug(options.viewSlug);
    if (!view) {
      throw new Error(`View not found: ${options.viewSlug}`);
    }
  ...
  async generateLandingPage(
    ...
    // Create inline View schema for landing page
    ...
    if (!view) {
      view = await this.viewService.createView({
        ...
        type: ViewType.PAGE,
        schema: viewSchema,
        tags: ['landing-page', 'auto-generated']
      });
