...
export class SessionService {
   ...
   */
  async getActiveSessions(affiliateUserId?: string): Promise<number> {
    const pattern = affiliateUserId ? 
  ...
  async getSessionMetrics(
    ...
    const query = this.sessionRepo.createQueryBuilder('session')
      .where('session.affiliateUserId = :affiliateUserId', { affiliateUserId })
    ...
    const activeSessions = await this.getActiveSessions(affiliateUserId);
  ...
  private async storeSession(session: SessionData): Promise<void> {
    ...
    const indexKey = `session:${session.sessionId}:${session.affiliateUserId}`;
    ...
    // Store index for affiliate lookup
  ...
  async getRealtimeSessionFeed(limit: number = 10): Promise<any[]> {
    ...
    return sessions
      ...
      .map(s => ({
        ...
        affiliateUserId: s.affiliateUserId,
        device: s.device,
        browser: s.browser,
        country: s.country,
