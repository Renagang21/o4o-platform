...
export const createPartnerLinkSchema = Joi.object({
  partnerUserId: Joi.string().uuid().required(),
...
export const getPartnerStatsSchema = Joi.object({
  partnerUserId: Joi.string().uuid().optional(),
  startDate: Joi.date().iso().optional(),
  endDate: Joi.date().iso().optional(),
  groupBy: Joi.string().valid('day', 'week', 'month').optional().default('day')
});

export const getPartnerUserSchema = Joi.object({
