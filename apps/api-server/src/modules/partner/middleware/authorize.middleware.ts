...
/**
 * Middleware to check if user is admin or the partner owner
 */
export const requireAdminOrPartner = (req: AuthorizedRequest, res: Response, next: NextFunction): void => {
  if (!req.user) {
    res.status(401).json({
      success: false,
      error: 'Authentication required',
  ...
  // Check if user is accessing their own partner data
  const partnerUserId = req.params.partnerUserId || 
                         req.query.partnerUserId || 
                         req.body?.partnerUserId;

  if (req.user.role === 'partner' && req.user.partnerId === partnerUserId) {
    next();
    return;
  }
  ...
  res.status(403).json({
    success: false,
    error: 'You can only access your own partner data',
    timestamp: new Date().toISOString()
  });
};
