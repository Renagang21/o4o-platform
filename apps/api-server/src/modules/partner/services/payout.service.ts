...
export class PayoutService {
   ...
   */
  async createPayout(data: CreatePayoutDto, adminId: string): Promise<PartnerPayout> {
    const { partnerUserId, commissionIds, paymentMethod, bankAccount, paymentDetails, notes } = data;

    // Validate all commissions belong to the partner and are approved
    ...
    for (const commission of commissions) {
      if (commission.partnerUserId !== partnerUserId) {
        invalidCommissions.push({
          id: commission.id,
          reason: 'Commission does not belong to this partner'
        });
    ...
    // Clear cache
    await this.clearCache(partnerUserId);
  ...
  async processPayout(
    ...
    // Clear cache
    await this.clearCache(payout.partnerUserId);
  ...
  async getPayouts(params: GetPayoutsDto): Promise<{
    ...
    const formattedItems = items.map(payout => ({
      ...
      partnerName: payout.partnerUser?.user?.name,
      partnerEmail: payout.partnerUser?.user?.email,
  ...
  /**
   * Calculate payout summary for an partner
   */
  async calculatePayoutSummary(partnerUserId: string): Promise<{
    ...
    if (pendingPayouts.length > 0) {
      canCreatePayout = false;
      reason = 'There is already a pending payout for this partner';
    }
  ...
  private async clearCache(partnerUserId: string): Promise<void> {
    const keys = [
      `partner:payouts:${partnerUserId}`,
      `partner:commissions:${partnerUserId}`,
      `partner:stats:${partnerUserId}`
    ];
