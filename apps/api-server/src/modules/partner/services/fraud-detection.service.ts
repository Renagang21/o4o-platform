...
export class FraudDetectionService {
  ...
  async analyzeClick(clickData: {
    ...
    const clickFloodScore = await this.checkClickFlooding(clickData.partnerUserId, clickData.ipAddress);
    ...
    // Check IP patterns
    const ipPatternScore = await this.checkIPPatterns(clickData.ipAddress, clickData.partnerUserId);
    ...
    if (clickData.country) {
      const geoScore = await this.checkGeoAnomalies(clickData.partnerUserId, clickData.country);
      if (geoScore) {
    ...
    const result: FraudAnalysisResult = {
      partnerUserId: clickData.partnerUserId,
    ...
    if (recommendation === 'block') {
      await this.blockPartner(clickData.partnerUserId, result);
    } else if (recommendation === 'review') {
      await this.flagForReview(clickData.partnerUserId, result);
    }
  ...
  /**
   * Perform comprehensive fraud analysis for an partner
   */
  async performComprehensiveAnalysis(partnerUserId: string): Promise<{
    ...
    const patterns = {
      ...
      velocityAnalysis: await this.analyzeVelocity(partnerUserId),
      anomalyScore: await this.calculateAnomalyScore(partnerUserId)
    };
  ...
  private async checkClickFlooding(partnerUserId: string, ipAddress: string): Promise<FraudIndicator> {
    const key = `fraud:clicks:${partnerUserId}:${ipAddress}`;
  ...
  private async checkIPPatterns(ipAddress: string, partnerUserId: string): Promise<FraudIndicator> {
    ...
    // Check for multiple partners using same IP
    const partnerKey = `fraud:ip:partners:${ipAddress}`;
    await this.redisService.sadd(partnerKey, partnerUserId);
    await this.redisService.expire(partnerKey, 86400);

    const partnerCount = await this.redisService.scard(partnerKey);
    if (partnerCount > 3) {
      return {
        type: 'ip_pattern',
        severity: 'high',
        score: 35,
        details: { sharedIP: true, partnerCount }
      };
    }
  ...
  private async checkGeoAnomalies(partnerUserId: string, country: string): Promise<FraudIndicator> {
    ...
    const geoKey = `fraud:geo:${partnerUserId}`;
  ...
  private async checkConversionPatterns(partnerUserId: string, amount: number): Promise<FraudIndicator> {
  ...
  private async checkIPConversions(ipAddress: string, partnerUserId: string): Promise<FraudIndicator> {
  ...
  private async storeAnalysisResult(result: FraudAnalysisResult): Promise<void> {
    const key = `fraud:analysis:${result.partnerUserId}`;
    await this.redisService.set(key, JSON.stringify(result), 3600);

    // Store in history
    const historyKey = `fraud:history:${result.partnerUserId}`;
  ...
  private async blockPartner(partnerUserId: string, result: FraudAnalysisResult): Promise<void> {
    // Add to blocked list
    await this.redisService.sadd('fraud:blocked:partners', partnerUserId);

    // Send notification to admin
    await this.notificationService.createNotification({
      partnerUserId: 'admin', // Send to admin
      type: 'alert',
      title: 'Partner Blocked for Fraud',
      message: `Partner ${partnerUserId} has been automatically blocked due to fraud detection`,
      data: result,
      priority: 'urgent'
    });
  ...
  private async flagForReview(partnerUserId: string, result: FraudAnalysisResult): Promise<void> {
    // Add to review queue
    await this.redisService.sadd('fraud:review:queue', partnerUserId);

    // Send notification to admin
    await this.notificationService.createNotification({
      partnerUserId: 'admin',
      type: 'alert',
      title: 'Partner Flagged for Review',
      message: `Partner ${partnerUserId} has been flagged for manual review`,
      data: result,
      priority: 'high'
    });
  }
  ...
  private analyzeClickPatterns(clicks: PartnerClick[]): any {
    const patterns = {
  ...
  private analyzeConversionPatterns(conversions: PartnerConversion[]): any {
    const patterns = {
  ...
  private analyzeSessionPatterns(sessions: PartnerSession[]): any {
    const patterns = {
  ...
  private async analyzeVelocity(partnerUserId: string): Promise<any> {
    const intervals = [60, 300, 3600, 86400]; // 1 min, 5 min, 1 hour, 1 day
    const velocity: any = {};

    for (const interval of intervals) {
      const key = `velocity:${partnerUserId}:${interval}`;
  ...
  private async calculateAnomalyScore(partnerUserId: string): Promise<number> {
    // Simple anomaly scoring based on deviation from normal patterns
    // This is a simplified version - in production, use ML models
    
    const metrics = await this.getPartnerMetrics(partnerUserId);
    let score = 0;
  ...
  private async getPartnerMetrics(partnerUserId: string): Promise<any> {
    ...
    const [clicks, conversions] = await Promise.all([
      this.clickRepo.find({
        where: { partnerUserId, createdAt: Between(dayAgo, now) }
      }),
      this.conversionRepo.find({
        where: { partnerUserId, createdAt: Between(dayAgo, now) }
      })
  ...
  private generateRecommendations(risk: FraudAnalysisResult, patterns: any): string[] {
    ...
    if (risk.riskLevel === 'critical') {
      ...
      recommendations.push('Place partner under enhanced monitoring');
  ...
  /**
   * Check if an partner is blocked
   */
  async isPartnerBlocked(partnerUserId: string): Promise<boolean> {
    return await (this.redisService as any).sismember('fraud:blocked:partners', partnerUserId);
  }

  /**
   * Get fraud history for an partner
   */
  async getFraudHistory(partnerUserId: string): Promise<FraudAnalysisResult[]> {
    const historyKey = `fraud:history:${partnerUserId}`;
