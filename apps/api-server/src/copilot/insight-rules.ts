...
function checkOrderSpike(
  ...
  if (growth > 50) {
    items.push({
      id: 'order-spike',
      message: `주문이 전주 대비 ${growth}% 급증했습니다. 재고, 주문 처리 상태, 고객 안내 준비 상태를 확인하세요.`,
      level: 'warning',
      link: getLink(service, 'orders'),
    });
  }
...
function checkInactivity(
  ...
  // WO-O4O-NETURE-OPERATOR-DASHBOARD-AI-SUMMARY-INACTIVITY-RULE-REFINE-V1 (Neture):
  // WO-O4O-CROSSSERVICE-INSIGHT-RULES-VAGUE-MESSAGE-REFINE-V1 (Cosmetics 정렬):
  //   Neture / Cosmetics 모두 zero-orders 단독 조건만으로 warning 을 띄우지 않는다.
  ...
  if (service === 'neture' && monthlyOrders === 0) {
    ...
    items.push({
      id: 'no-orders',
      message: '최근 주문 데이터가 아직 충분하지 않습니다. 상품 노출·콘텐츠·프로모션 흐름을 먼저 확인하세요.',
      level: 'info',
    });
  }
