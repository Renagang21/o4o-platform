...
export function createGlycopharmRoutes(dataSource: DataSource): Router {
  ...
  router.use('/', createStoreExecutionAssetsController(dataSource, coreRequireAuth as any, 'glycopharm'));
  ...
  router.use('/store-playlists', createStorePlaylistController(dataSource, coreRequireAuth as any, undefined, 'glycopharm'));
  ...
  // ============================================================================
  // ============================================================================
  // Home Routes — /api/v1/glycopharm/home/*
      ...
      tasks.push((async () => {
        const rows: any[] = await dataSource.query(
          `SELECT c.id, c.title, c.created_at,
           ...
           FROM glycopharm_contents c LEFT JOIN users u ON c.created_by = u.id
