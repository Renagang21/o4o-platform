/**
 * Operator POP Controller — Operator HUB POP Write API
 *
 * WO-O4O-KPA-POP-PUBLISHING-PHASE2-BACKEND-V1 (2026-05-24)
 *
 * 운영자가 매장 HUB 에 게시할 POP 콘텐츠를 작성/수정/게시하는 backend write API.
 * 매장 직접 작성 (store_owner) 경로는 본 controller 대상이 아니며 (현재는 PDF 생성 흐름의
 * createStorePopController 만 존재) author_role 로 같은 store_pops 테이블을 분리한다.
 *
 * 본 controller 는 operator-blog.controller.ts (WO-O4O-OPERATOR-BLOG-PUBLISHING-WRITE-API-V1)
 * 패턴을 그대로 mirror — store_blog_posts / store_pops 두 테이블이 동일 형태를 갖기 때문.
 *
 * 권한 (3 개 서비스 공통):
 *   - {service}:operator
 *   - {service}:admin
 ...
 *
 * 라우트 (router 내부, 외부 mount: /api/v1/{serviceKey}/operator/pop):
 *   - GET    /posts             — 운영자 게시 POP 목록 (draft + published + archived)
 *   - GET    /posts/:id         — 단일 조회
 *   - POST   /posts             — 생성 (draft 로)
 *   - PUT    /posts/:id         — 수정
 *   - PATCH  /posts/:id/publish — 발행 (queryPop HUB 노출 시작)
 ...
 *   - apps/api-server/src/routes/o4o-store/controllers/operator-blog.controller.ts (mirror 원본)
 */
...
import type {
  StorePopStatus,
  StorePopAuthorRole,
} from '../entities/store-pop.entity.js';
 ...
 * operator-blog.controller.ts 와 동일 알고리즘 (mirror).
 */
...
export function createOperatorPopController(
   ...
   * operator-blog.controller.ts 와 동일 패턴.
   */
  ...
  async function isOperatorSlugTaken(
    ...
    let sql = `SELECT id FROM store_pops
               WHERE service_key = $1
  ...
  router.get('/posts/:id', requireAuth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!post) {
        res.status(404).json({
          success: false,
          error: { code: 'POST_NOT_FOUND', message: 'Operator POP post not found' },
        });
        return;
      }
    ...
    try {
      ...
      if (await isOperatorSlugTaken(finalSlug)) {
        res.status(409).json({
          ...
          error: {
            code: 'SLUG_CONFLICT',
            message: 'An operator POP post with this slug already exists in this service',
          },
        });
        return;
      }
