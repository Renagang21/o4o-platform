...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card { background: #1e1e2e; border: 1px solid #333; border-radius: 6px; padding: 12px; margin: 8px 0; }
...
<pre id="userResult">Enter userId</pre>

<h2>3. Approve Membership</h2>
<div class="row">
  <input id="membershipId" placeholder="membershipId (UUID)">
  <button class="danger" onclick="doApprove()">Approve</button>
</div>
<pre id="approveResult">Enter membershipId from step 1</pre>
...
async function loadPending() {
  ...
  show('pendingResult', await api('/pending' + q));
}
async function checkUser() {
  const uid = document.getElementById('userId').value.trim();
  if (!uid) return alert('userId required');
  show('userResult', await api('/user/' + uid));
}
async function doApprove() {
  const mid = document.getElementById('membershipId').value.trim();
  if (!mid) return alert('membershipId required');
  if (!confirm('Approve ' + mid + '?')) return;
