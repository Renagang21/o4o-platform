/**
 * Neture Community Hub Controller
 *
 * WO-O4O-NETURE-COMMUNITY-OPERATOR-MANAGEMENT-V1
 *
 * Reuses CommunityHubService from KPA with SERVICE_CODE = 'neture'.
...
const SERVICE_CODE = 'neture';

export function createNetureCommunityHubController(
  dataSource: DataSource,
  requireAuth: AuthMiddleware,
  requireNetureScope: ScopeMiddleware,
): Router {
  const router = Router();
  ...
  router.get(
    ...
    requireAuth as any,
    requireNetureScope('neture:operator') as any,
    asyncHandler(async (req: Request, res: Response): Promise<void> => {
