 ...
 *
 * WO-O4O-COSMETICS-PAYMENTCORE-INTEGRATION-V1
 *
 * K-Cosmetics 결제 컨트롤러 — PaymentCore 통합 버전.
 * Legacy Toss 직접 호출 제거, 플랫폼 단일 결제 엔진 사용.
 *
 ...
 *
 * 주문 상태 변경은 KCosmeticsPaymentEventHandler 책임 (변경 없음).
 */
...
export function createCosmeticsPaymentController(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const payment = await paymentService.prepare({
          ...
          successUrl,
          failUrl,
          sourceService: 'cosmetics',
        });

        logger.info('[Cosmetics Payment] Payment prepared', {
   ...
   *
   * PaymentCore가 내부에서 수행:
   * - payment.amount 기반 금액 검증 (프론트엔드 amount 불사용)
   * - CREATED → CONFIRMING → PAID 상태 전이
   * - Toss API Confirm 호출
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const err = error as Error;
        logger.error('[Cosmetics Payment] Get payment info error:', err);
