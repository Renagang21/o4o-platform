/**
 * KPA Payment Controller
 *
 * WO-O4O-KPA-CUSTOMER-COMMERCE-LOOP-V1
 *
 * PaymentCoreService를 통한 결제 흐름.
 * GlycoPharm glycopharm-payment.controller.ts 1:1 복사 + serviceKey 치환.
 *
...
export function createKpaPaymentController(
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        opsMetrics.inc(OPS.PAYMENT_PREPARE, { service: 'kpa' });
        ...
        logger.info('[KPA Payment] Payment prepared', {
        ...
        opsMetrics.inc(OPS.PAYMENT_PREPARE_ERROR, { service: 'kpa' });
        const err = error as Error;
        logger.error('[KPA Payment] Prepare error:', err);
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        opsMetrics.inc(OPS.PAYMENT_CONFIRM_SUCCESS, { service: 'kpa' });

        logger.info('[KPA Payment] Payment confirmed', {
          paymentId: payment.id,
          orderId: order.id,
          orderNumber: order.orderNumber,
        ...
        opsMetrics.inc(OPS.PAYMENT_CONFIRM_FAILED, { service: 'kpa' });
        const err = error as Error;
        logger.error('[KPA Payment] Confirm error:', err);
        ...
        // paymentKey UNIQUE violation → idempotent 처리
        ...
        if (pgError.code === '23505' && pgError.detail?.includes('paymentKey')) {
          ...
          opsMetrics.inc(OPS.PAYMENT_DUPLICATE_BLOCKED, { service: 'kpa' });
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const err = error as Error;
        logger.error('[KPA Payment] Get payment info error:', err);
