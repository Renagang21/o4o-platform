...
export class RestockNotificationService extends EventEmitter {
  private redis: Redis;
  ...
  // 알림 저장소 (실제로는 DB)
  private notifications: Map<string, RestockNotification> = new Map();
  private notificationQueues: Map<string, NotificationQueue> = new Map();
  ...
  async subscribeToRestock(
    ...
    logger.info(`Restock notification subscribed: ${notification.id} for product ${productId}`);
  ...
  async unsubscribeFromRestock(
    ...
    logger.info(`Restock notification cancelled: ${notificationId}`);
  }

  /**
   * 재고 업데이트 처리
   */
  private async processStockUpdate(data: {
    productId: string;
    variationId?: string;
  ...
  async sendRestockNotifications(
        ...
        batch.map(async (notification) => {
          if (notification.status === 'active') {
            try {
              ...
              logger.error(`Failed to send restock notification ${notification.id}:`, error);
            }
          }
        })
      );
    ...
    logger.info(`Sent ${processedCount} restock notifications for product ${productId}`);
    ...
    switch (notification.notificationType) {
      ...
      case 'all':
        await this.sendEmailNotification(recipientEmail, {
          ...
          attributes: notification.metadata?.attributes
        });
        break;
  ...
  private async sendSmsNotification(
    ...
    logger.info(`SMS notification would be sent to ${phone}: ${message}`);
  }
   ...
   * 큐에 추가
   */
  private addToQueue(notification: RestockNotification): void {
   ...
   * 큐에서 제거
   */
  private removeFromQueue(notification: RestockNotification): void {
    const queueKey = `${notification.productId}:${notification.variationId || 'default'}`;
    const queue = this.notificationQueues.get(queueKey);
   ...
   * 큐 정리
   */
  private cleanupQueue(queueKey: string): void {
    const queue = this.notificationQueues.get(queueKey);
   ...
   * 상품별 알림 신청 수 조회
   */
  async getProductNotificationCount(productId: string): Promise<number> {
  ...
  async cleanupExpiredNotifications(): Promise<void> {
    ...
    if (cleanedCount > 0) {
      logger.info(`Cleaned up ${cleanedCount} expired restock notifications`);
    }
  }
  ...
  async getNotificationStatistics(): Promise<{
    ...
    topRequestedProducts: Array<{ productId: string; count: number }>;
    conversionRate: number;
  }> {
    const stats = {
      totalActive: 0,
      totalNotified: 0,
      topRequestedProducts: [] as Array<{ productId: string; count: number }>,
      conversionRate: 0
    };
...
// 싱글톤 인스턴스
export const restockNotificationService = new RestockNotificationService();