 ...
 *
 * Returns stale cached data while fetching fresh data in the background.
 *
 * Usage:
 * ```typescript
 * router.get('/policies', staleWhileRevalidate(300), policyController.list);
 * ```
 *
 * @param ttlSeconds - Time to live in seconds
 * @param namespace - Cache namespace
 */
export function staleWhileRevalidate(ttlSeconds: number = 300, namespace?: string) {
  return async (req: Request, res: Response, next: NextFunction) {
    if (req.method !== 'GET') {
      return next();
    }
    ...
    try {
      ...
      res.json = function (body: any) {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          cacheService.set(cacheKey, body, namespace, {
            ttl: ttlSeconds,
            staleWhileRevalidate: true
          }).catch(err => {
            console.error('Cache set error:', err);
