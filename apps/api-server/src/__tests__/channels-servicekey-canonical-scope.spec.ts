...
describe('channels 영역 static contract', () => {
  ...
  it('admin-dashboard channel 화면은 canonical value 만 보낸다', () => {
    // WO-O4O-CMS-CHANNELS-COMPLETED-BRANCHES-MAIN-INTEGRATION-V1 §8·§12:
    //   canonical value 목록의 위치는 CMS 공통 카탈로그(cmsServiceCatalog.ts) 한 벌이다.
    //   channel 화면은 그 카탈로그를 소비만 하고 자기 배열을 다시 갖지 않는다
    //   (화면별 복제가 살아나면 축이 다시 섞인다). 카탈로그 값 자체의 canonical 여부는
    //   cms-servicekey-alias-ssot-closure.spec.ts 가 검사한다.
    const catalog = stripComments(
      fs.readFileSync(
        path.resolve(API_SRC, '../../admin-dashboard/src/pages/cms/cmsServiceCatalog.ts'),
        'utf-8',
      ),
    );
    expect(catalog).toMatch(/value:\s*'kpa-society'/);
    expect(catalog).not.toMatch(/value:\s*'kpa'/);
    expect(catalog).not.toMatch(/value:\s*'cosmetics'/);
    ...
    for (const rel of ['ChannelFormModal.tsx', 'ChannelList.tsx']) {
      ...
      expect(code).toMatch(/cmsServiceOptionsWithAll\(/);
      expect(code).toMatch(/from '\.\.\/cmsServiceCatalog'/);
      // 로컬 서비스 목록 복제 / role prefix 재등장 금지
      expect(code).not.toMatch(/value:\s*'kpa'/);
      expect(code).not.toMatch(/value:\s*'cosmetics'/);
      expect(code).not.toMatch(/value:\s*'kpa-society'/);
    }
  });
});
