 ...
 *   ③ corner_legacy_all   코너 확정 + 진열 없음 (compatibility)
 *   ④ none                코너 미확정
 *
...
describe('resolveCornerProducts — 코너 상품 집합·순서', () => {
  it('supplier 먼저, 그다음 local (kiosk 병합 순서와 동일)', async () => {
    ...
    const out = await resolveCornerProducts(ds, CTX, TABLET, true);
  ...
  it('configured=true 는 진열로 집합을 제한한다', async () => {
    const ds = makeDataSource({ locals: LOCAL_ROWS });
    await resolveCornerProducts(ds, CTX, TABLET, true);
  ...
  it('configured=false 는 그 코너의 legacy 집합(진열 제한 없음)', async () => {
    const ds = makeDataSource({ locals: LOCAL_ROWS });
    await resolveCornerProducts(ds, CTX, TABLET, false);
    const localSql = ds.query.mock.calls.map((c: any[]) => c[0]).find((s: string) => s.includes('store_local_products'));
    expect(localSql).not.toContain('AND disp.id IS NOT NULL');
    expect(utils.queryTabletVisibleProducts).toHaveBeenCalledWith(
  ...
  it('supplier 가시성 게이트를 우회하지 않는다 — 게이트가 0건이면 0건', async () => {
    ...
    await expect(resolveCornerProducts(ds, CTX, TABLET, true)).resolves.toEqual([]);
  });
...
describe('소비 계약 — selectionMode', () => {
  it('서버가 확정한 3단은 모두 server-resolved 로 판정한다', () => {
    expect(isServerResolvedProductList('selected')).toBe(true);
    expect(isServerResolvedProductList('corner_display')).toBe(true);
    expect(isServerResolvedProductList('corner_legacy_all')).toBe(true);
  });

  it("코너 미확정('none')과 표식 없음은 자체 조회로 남긴다 (legacy 회귀 0)", () => {
    expect(isServerResolvedProductList('none')).toBe(false);
