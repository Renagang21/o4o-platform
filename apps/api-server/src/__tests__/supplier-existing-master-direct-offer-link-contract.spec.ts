 ...
 *   2. persistence primitive `persistOfferForResolvedMaster` 하나만 존재하고 from-master 가 호출한다(복사 0)
 *   3. (은퇴) 레거시 POST /products · createSupplierOffer · validateCreateInput · MASTER_ID_DIRECT_INJECTION_NOT_ALLOWED 는 사라졌다
 ...
 *   6. DRUG gate 완화 0(guard 파일 무접촉) · DDL/migration 0
 *
 * WO-O4O-SUPPLIER-PRODUCT-REGISTRATION-AI-FIRST-CUTOVER-AND-LEGACY-MASTER-RESOLUTION-RETIREMENT-V1 §2.3 · §2.4 로
 *   "두 경로 공유" · "레거시 불변" · "web-neture 미호출" 기대는 더 이상 유효하지 않다 — 이 파일은 그 자리에서 은퇴 계약으로 뒤집었다.
 *   web-neture cutover · 레거시 라우트 제거 · Offer 경로 Master write 0 의 최종 상태는
 *   supplier-product-registration-cutover-contract.spec.ts 가 증명한다.
 */
...
describe('② persistence primitive 공유 — 복사 0', () => {
  it('persistOfferForResolvedMaster 는 offer.service 에 1회 정의되고 호출자는 from-master 하나뿐', () => {
  ...
  it('Offer 유일성 계약은 primitive 안에만: findDuplicateOffer 선검사 → offerRepo.save → 23505 fallback', () => {
    ...
    // 진입 경로 본문에는 중복 검사·save 가 없다(primitive 로만 도달)
    for (const body of [fromMasterBody]) {
...
describe('③ (은퇴) 레거시 createSupplierOffer / validateCreateInput / resolveProductMetadata / POST /products', () => {
  it('offer.service 에 레거시 메서드 3종이 없다', () => {
    expect(offerSrc).not.toMatch(/\n {2}async createSupplierOffer\(/);
    expect(offerSrc).not.toMatch(/\n {2}private async validateCreateInput\(/);
    expect(offerSrc).not.toMatch(/\n {2}private async resolveProductMetadata\(/);
  });

  it('MASTER_ID_DIRECT_INJECTION_NOT_ALLOWED 는 enum 과 서비스에서 사라졌다 (masterId 를 받는 경로 = from-master 뿐)', () => {
    expect(stripComments(read(ERROR_CODES))).not.toContain('MASTER_ID_DIRECT_INJECTION_NOT_ALLOWED');
    expect(offerSrc).not.toContain('MASTER_ID_DIRECT_INJECTION_NOT_ALLOWED');
  });

  it("컨트롤러에 router.post('/products', ...) 레거시 핸들러가 없고 netureService.createSupplierOffer( 호출 0", () => {
    expect(controllerSrc).not.toMatch(/router\.post\('\/products',/);
    expect(controllerSrc).not.toContain('netureService.createSupplierOffer(');
...
describe('⑤ 무접촉 — DRUG guard · DDL/migration', () => {
