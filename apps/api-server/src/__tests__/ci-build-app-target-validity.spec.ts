 ...
 *
 *   `main-site` case 를 남겨둔 선례(WO-O4O-MAIN-SITE-CI-BUILD-CONTRACT-CENSUS-AND-
 *   DISPOSITION-V1)는 "CI 가 호출하지 않을 뿐 **수동 호출 경로로는 유효하다**" 가 근거였다.
 *   `apps/main-site` 는 실재하므로 그 근거가 성립하지만, `@o4o/storefront` 는
 *   수동으로도 실행될 수 없어 보존 근거가 적용되지 않는다. 따라서 case 를 제거했다.
 *
 * 고정하는 계약
 * ---------------------------------------------------------------
