/**
 * WO-O4O-OTC-EASY-DRUG-READY-TOPICAL-UNIT1-CONTENT-FP-V3-FINAL-PRODUCTION-V1 — topical-unit-1 LIVE apply 선행 게이트(na)
 *
 * apply 러너와 별개 코드경로. **read-only(SELECT) 전용 · DB write 0.**
 * 이 게이트가 전부 PASS 여야만 LIVE apply 를 시작한다(중지 조건 = fail 1건 이상).
 *
 *  G1  oral route COMPLETE      : oral 540 master 전건 KO/EN authored canonical · easy deprecated 540 · easy 잔존 0 · audit 540
 *  G2  topical PRE_APPLY READY  : preapply-ready-topical-unit-1.json status=PRE_APPLY_READY · 327m/55fp · writePlan 1962
 *  G3  readiness 산출물 불변     : build/en-build/preapply-ready md5 == readiness commit(bb2e79a22) blob md5
 *  G4  ledger/승인 SSOT 불변     : unit-ledger·재승인 SSOT md5 == 승인 commit(00851d237) blob md5 + topical 대상 ledger 일치
 *  G5  대상 easy anchor         : topical 327 master KO canonical == mfds_easy_drug 327
 *  G6  기존 authored canonical 0 : KO/EN 모두 0 (다른 세션 LIVE write 0 의 직접 증거)
 *  G7  V3 sourceRef 충돌 0      : topical 55 sourceRef 를 쓰는 LIVE 행 0
 *  G8  canonicalDup 0           : 대상 master KO/EN canonical 중복 0
 *  G9  audit 0                  : 대상 master 에 이 트랙 WO audit 0
 *  G10 타 route 미착수          : ophthalmic 253 · oromucosal 14 authored canonical 0 · easy canonical 전건
 *  G11 전문용 혼입 0            : 대상 master 가 전부 일반의약품(OTC) — 전문의약품(ETC) 0
 *
 * Usage(apps/api-server): ../../node_modules/.bin/tsx src/scripts/otc-v3-topical-preflight-gate.na.ts [--port 5471]
 */
...
const port = (): number => { const a = arg('--port'); return a ? parseInt(a, 10) : 5471; };
...
const THIS_WO = 'WO-O4O-OTC-EASY-DRUG-READY-TOPICAL-UNIT1-CONTENT-FP-V3-FINAL-PRODUCTION-V1';
const READINESS_WO = 'WO-O4O-OTC-EASY-DRUG-READY-TOPICAL-OROMUCOSAL-CONTENT-FP-V3-FINAL-READINESS-V1';

// git blob(커밋 시점 원본) 과 현재 워킹트리 파일의 md5 대조 — 산출물/SSOT 불변 증명
...
function fileMd5(abs: string): string { return md5(readFileSync(abs)); }
...
async function main(): Promise<void> {
  ...
  const ready = rd(path.join(NA, 'preapply-ready-topical-unit-1.json'));
  const ko = rd(path.join(NA, 'build-topical-unit-1.json'));
  const en = rd(path.join(NA, 'en-build-topical-unit-1.json'));
  ...
  const topical = unitsById['topical-unit-1'];
  ...
  if (ready.masterCount !== 327) fails.push(`G2: masterCount=${ready.masterCount}!=327`);
  if (ready.fpCount !== 55) fails.push(`G2: fpCount=${ready.fpCount}!=55`);
  if (ready.writePlan.koTotal !== 1308 || ready.writePlan.enTotal !== 654 || ready.writePlan.grandTotal !== 1962) fails.push(`G2: writePlan ${JSON.stringify(ready.writePlan)}`);

  const G3_FILES = ['otc-ready-na-v3/build-topical-unit-1.json', 'otc-ready-na-v3/en-build-topical-unit-1.json',
    'otc-ready-na-v3/preapply-ready-topical-unit-1.json', 'otc-ready-na-v3/preapply-verify-v1.json',
  ...
  // 대상 집합 = ledger(승인 SSOT) ∩ build 산출물 — 두 축이 정확히 일치해야 한다
  ...
  const ledgerIds: string[] = topical.masterIds;
  ...
  notes.g4.targetSet = { buildMasters: buildIds.length, ledgerMasters: ledgerIds.length,
    buildRefs: buildRefs.length, ledgerRefs: topical.sourceRefs.length,
    masterSetEqual: sameSet(buildIds, ledgerIds), refSetEqual: sameSet(buildRefs, topical.sourceRefs) };
  ...
  if (new Set(buildIds).size !== 327) fails.push(`G4: distinct master=${new Set(buildIds).size}!=327`);
  // EN payload 는 KO fp 와 1:1
  ...
  const oroIds = unitsById['oromucosal-unit-1'].masterIds as string[];
  ...
  await ds.initialize();
  try {
    // ── G1 oral route COMPLETE ──
    const o = (await ds.query(`SELECT
      ...
      count(*) FILTER (WHERE kocanon>1)::int "koDup", count(*) FILTER (WHERE encanon>1)::int "enDup"
      FROM (SELECT mid,
        ...
        FROM unnest($1::uuid[]) mid) t`, [oralIds]))[0];
    const oAudit = (await ds.query(`SELECT count(*)::int n, count(DISTINCT master_id)::int m FROM shared_product_description_audit_logs WHERE (metadata->>'productionWo')=ANY($1)`, [ORAL_WO]))[0];
    notes.g1 = { oralMasters: oralIds.length, ...o, auditRows: oAudit.n, auditMasters: oAudit.m, contractWrite: oralIds.length * 6 };
    if (o.koAuthoredCanonical !== 540) fails.push(`G1: oral koAuthoredCanonical=${o.koAuthoredCanonical}!=540`);
    if (o.enAuthoredCanonical !== 540) fails.push(`G1: oral enAuthoredCanonical=${o.enAuthoredCanonical}!=540`);
    if (o.easyDeprecated !== 540) fails.push(`G1: oral easyDeprecated=${o.easyDeprecated}!=540`);
    if (o.easyStillCanonical !== 0) fails.push(`G1: oral easyStillCanonical=${o.easyStillCanonical}`);
    if (o.koDup !== 0 || o.enDup !== 0) fails.push(`G1: oral canonicalDup ko=${o.koDup} en=${o.enDup}`);
    if (oAudit.m !== 540) fails.push(`G1: oral audit masters=${oAudit.m}!=540`);

    // ── G5~G9 topical 대상 현 상태 ──
    const t = (await ds.query(`SELECT
      ...
      FROM (SELECT mid,
        ...
        EXISTS(SELECT 1 FROM shared_product_descriptions s WHERE s.master_id=mid AND s.status='canonical' AND s.description_type='STORE' AND s.language='en' AND s.source_type IN ('mfds_drug_otc','nutrition_combo') AND s.deleted_at IS NULL) en_authored,
        EXISTS(SELECT 1 FROM shared_product_descriptions s WHERE s.master_id=mid AND s.status='deprecated' AND s.source_type='mfds_easy_drug' AND s.description_type='STORE' AND s.deleted_at IS NULL) dep_easy
        FROM unnest($1::uuid[]) mid) t`, [ledgerIds]))[0];
    ...
    notes.topicalBefore = { masters: ledgerIds.length, ...t, v3SourceRefRows: refRows.n, v3SourceRefOutside: refRows.outside, trackAudit: tAudit.n };
    if (t.easyCanonical !== 327) fails.push(`G5: easyCanonical=${t.easyCanonical}!=327`);
    ...
    // ── G10 타 route(ophthalmic/oromucosal) 미착수 ──
    for (const [label, ids, n] of [['ophthalmic', ophIds, 253], ['oromucosal', oroIds, 14]] as const) {
      ...
      notes[`g10_${label}`] = { masters: ids.length, ...r };
      if (ids.length !== n) fails.push(`G10: ${label} ledger masters=${ids.length}!=${n}`);
      if (r.koAuthoredCanonical !== 0 || r.enAuthoredCanonical !== 0) fails.push(`G10: ${label} authored canonical ko=${r.koAuthoredCanonical} en=${r.enAuthoredCanonical} (미착수 전제 위반)`);
      if (r.easyDeprecated !== 0) fails.push(`G10: ${label} easyDeprecated=${r.easyDeprecated}`);
      if (r.easyCanonical !== ids.length) fails.push(`G10: ${label} easyCanonical=${r.easyCanonical}!=${ids.length}`);
    }

    // ── G11 전문용(전문의약품) 혼입 0 ──
    // 축1(구조): product_drug_extensions.drug_category — build 셀렉션 축(reproduce-build VERBATIM) 과 동일.
    //            대상 327 master 전건이 'otc' 여야 하고 'otc' 아닌 행이 0 이어야 한다.
    // 축2(내용): 공식 원문(officialSections) 에 전문가 전용 표지 문구 — census.la 의 PROFESSIONAL_RE VERBATIM.
    ...
    const nonOtc = cls.filter((c: any) => c.cat !== 'otc').reduce((t: number, c: any) => t + c.n, 0);
    const PROFESSIONAL_RE = /전문의약품|의료전문가\s*전용|병원\s*전용|조제\s*전용/;
    const profFps = ko.fingerprints.filter((f: any) => Object.values(f.officialSections || {}).some((v: any) => PROFESSIONAL_RE.test(String(v || ''))))
      .map((f: any) => f.fp);
    notes.g11 = { drugCategoryDistribution: cls, otcCoveredMasters: otcCovered, nonOtcMasters: nonOtc,
      professionalUseFps: profFps.length, professionalUseFpList: profFps };
    if (otcCovered !== ledgerIds.length) fails.push(`G11: drug_category='otc' 미충족 master=${ledgerIds.length - otcCovered}`);
    if (nonOtc !== 0) fails.push(`G11: 비-otc drug_category master=${nonOtc} (전문용 혼입 의심)`);
    if (profFps.length !== 0) fails.push(`G11: 원문 전문가전용 표지 fp=${profFps.length} (${profFps.join(',')})`);
  } finally { await ds.destroy(); }

  const out = { ...notes, pass: fails.length === 0, failCount: fails.length, fails };
  console.log(JSON.stringify(out, null, 2));
  console.log(`\n=== TOPICAL-UNIT-1 PREFLIGHT GATE · PASS=${out.pass} · fails=${out.failCount} ===`);
