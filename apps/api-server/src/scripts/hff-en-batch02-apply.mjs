 ...
 *   node ... --apply  +  HFF_EN_B02_APPLY_CONFIRM=YES
 */
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-en-batch02-render-audit-v1.json`, 'utf8'));
const APPLY = process.argv.includes('--apply') && process.env.HFF_EN_B02_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-en-batch02-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-en-batch02-apply-results-v1.json`, JSON.stringify(out, null, 1));
fs.writeFileSync(`${D}/hff-en-batch02-safe-targets-v1.json`, JSON.stringify({ builtAt: new Date().toISOString(), count: SAFE.length, targets: SAFE }, null, 1));
