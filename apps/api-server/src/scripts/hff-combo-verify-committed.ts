...
// 기존 복합형 baseline 은 DB 에서 동적 조회(모든 batch:single-nutrient-combo-* 태그, 현재 slug 제외).
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 기존 복합형 baseline 동적 조회(모든 combo 태그, 현재 slug 제외) + 전체 누적
    const allComboTags: Array<{ tag: string; c: number }> = await q(
      `SELECT t AS tag, count(*)::int c FROM product_masters m, jsonb_array_elements_text(m.tags::jsonb) t
       WHERE m.tags::jsonb @> '["import:mfds-hff"]'::jsonb AND t LIKE 'batch:single-nutrient-combo-%' GROUP BY t ORDER BY c DESC`);
