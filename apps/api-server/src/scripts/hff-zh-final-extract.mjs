 ...
 *
 * 최종 잔여 모집단의 KO canonical 슬롯 문구 추출 (read-only).
 ...
 *
 * 산출: data/hff-zh-final-phrases-v1.json (저장소) · <CACHE>/hff-zh-final-docslots.jsonl (캐시)
 */
...
fs.writeFileSync(`${CACHE}/hff-zh-final-docslots.jsonl`, lines.join('\n'));

const byKind = {};
for (const p of phrases) byKind[p.kind] = (byKind[p.kind] ?? 0) + 1;
const sorted = [...phrases].sort((a, b) => b.docFreq - a.docFreq);
fs.writeFileSync(`${D}/hff-zh-final-phrases-v1.json`, JSON.stringify({
