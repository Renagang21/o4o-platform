 ...
 *
 * DB 접근 0. prep 산출물(otc-v4-pilot-500-prep/source)만 입력으로 쓴다.
...
import { WO_500, PILOT_500_LEDGER } from './otc-v4-pilot-500-contract.ga.js';

const PREP = path.join(DATA_DIR, 'otc-v4-pilot-500-prep.ga.json');
const SOURCE = path.join(DATA_DIR, 'otc-v4-pilot-500-source.ga.json');
...
const OUT_KO = path.join(DATA_DIR, 'otc-v4-pilot-500-ko-payload.ga.json');
const OUT_EN = path.join(DATA_DIR, 'otc-v4-pilot-500-en-payload.ga.json');
const OUT_REPORT = path.join(DATA_DIR, 'otc-v4-pilot-500-author-report.ga.json');
