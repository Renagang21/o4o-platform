/**
 * HFF 유산균 Batch 002 200건 — STORE 설명서 canonical 적재 (dry-run 기본 · --apply env 이중게이트)
 *
 * 대상: 생산 batch-002 (prod-b-cp01..10 작성분 200건). HOLD 0.
 ...
 *   apply  : HFF_B2_CANONICAL_APPLY_CONFIRM=YES PROXY_PORT=5435 npx tsx ... --apply
 */
...
const CONFIRM = process.env.HFF_B2_CANONICAL_APPLY_CONFIRM === 'YES';
...
function loadTargets(): GuardProductInput[] {
  ...
  for (let i = 1; i <= 10; i++) {
    out.push(...JSON.parse(fs.readFileSync(`${DATA}/hff-probiotics-prod-b-cp${String(i).padStart(2, '0')}.json`, 'utf8')));
  }
  return out;
}

async function main(): Promise<void> {
  if (APPLY && !CONFIRM) throw new Error('APPLY_BLOCKED: --apply 는 HFF_B2_CANONICAL_APPLY_CONFIRM=YES 필요');

  const items = loadTargets();
  if (items.length !== 200) throw new Error(`대상 수 불일치: ${items.length} (기대 200)`);
  ...
  console.log(`[가드A] 파일 200: BLOCKED ${blocked} · REVIEW ${review} · draft결손 ${missDraft} · grounding결손 ${missGround}`);

  const stmts = items.map((it) => String((it as any).statementNo).trim());
  if (new Set(stmts).size !== 200) throw new Error(`신고번호 중복(파일): 유일 ${new Set(stmts).size}`);
  ...
  try {
    ...
    const report: any = { mode: APPLY ? 'apply' : 'dry-run', target: 200, guard: { blocked, review, missDraft, missGround, candMatch: cands.length, masterDup: dup.length }, planned: { ...planned, totalWrites: planned.masters + planned.candidateLinks + planned.spdKo + planned.spdEn }, sampleSanitize: sample };
    ...
    if (v.masters !== 200 || v.spdKo !== 200 || v.spdEn !== 200 || v.canonicalDup !== 0 || v.candidatesLinked !== 200) {
      await qr.rollbackTransaction();
