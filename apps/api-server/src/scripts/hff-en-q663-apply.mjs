/**
 * WO-O4O-HFF-EN-INTEGRATED-ISSUE-QUEUE-663-FULL-CLEANUP-V1  §8
 *
 * 문제 큐 663 해소분 EN canonical INSERT.
 *   - INSERT 전용. KO canonical / ProductMaster 는 절대 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_EN_Q663_APPLY_CONFIRM=YES.
 *   - 렌더 감사 verdict 가 PASS 가 아니면 실행하지 않는다.
 *   - rollback manifest 는 apply 전에 기록한다.
 *   - master 별 EN 중복 가드 + KO hash 재확인(낙관적 잠금)을 트랜잭션 안에서 수행한다.
 */
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-en-q663-render-audit-v1.json`, 'utf8'));
...
const APPLY = process.argv.includes('--apply') && process.env.HFF_EN_Q663_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-en-q663-rollback-v1.json`, JSON.stringify({
  wo: RENDER.wo, preparedAt: new Date().toISOString(),
  rollback: "INSERT 전용이므로 되돌리기는 삽입된 EN canonical 행의 soft delete 이다: UPDATE shared_product_descriptions SET deleted_at=now() WHERE id = ANY(insertedIds) AND language='en' AND source_type='o4o_hff_generated'.",
    ...
    for (const t of chunk) {
      ...
      const dup = await c.query(`
        SELECT 1 FROM shared_product_descriptions
         WHERE master_id=$1 AND description_type='STORE' AND status='canonical' AND language='en' AND deleted_at IS NULL`,
      [t.productMasterId]);
      if (dup.rows.length) { skipped.push({ m: t.productMasterId, why: 'EN_EXISTS' }); continue; }
      const r = await c.query(`
        ...
        VALUES ($1,'STORE','canonical','en','o4o_hff_generated',$2, now(), now()) RETURNING id`,
...
const out = {
  wo: RENDER.wo, appliedAt: new Date().toISOString(),
...
fs.writeFileSync(`${D}/hff-en-q663-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
