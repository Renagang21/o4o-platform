 ...
 *
 * DB 접근 0. prep 산출물(otc-v4-route535-prep/source)만 입력으로 쓴다.
...
import { WO_500, PILOT_500_LEDGER } from './otc-v4-route535-contract.ga.js';

const PREP = path.join(DATA_DIR, 'otc-v4-route535-prep.ga.json');
const SOURCE = path.join(DATA_DIR, 'otc-v4-route535-source.ga.json');
const TM = path.join(DATA_DIR, 'otc-v4-route535-tm.ga.json');
...
/** seed 우선순위: finalall(최신) → next2000(최대) → pilot 500 → pilot 100 → oral V3. 전부 검증 통과본. */
const SEED_TMS = [
...
const OUT_KO = path.join(DATA_DIR, 'otc-v4-route535-ko-payload.ga.json');
const OUT_EN = path.join(DATA_DIR, 'otc-v4-route535-en-payload.ga.json');
const OUT_REPORT = path.join(DATA_DIR, 'otc-v4-route535-author-report.ga.json');
