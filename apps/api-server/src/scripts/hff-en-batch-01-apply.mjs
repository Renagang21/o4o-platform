 ...
 *   node ... --apply  +  HFF_EN_B01_APPLY_CONFIRM=YES
 */
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-en-batch-01-render-audit-v1.json`, 'utf8'));
const APPLY = process.argv.includes('--apply') && process.env.HFF_EN_B01_APPLY_CONFIRM === 'YES';
    ...
    for (const t of part) {
      if (t.op === 'UPDATE') {
        const r = await c.query(`
          ...
          [t.newContent, t.enCanonicalId, t.masterId, t.oldContentHash]);
        ...
        const dup = await c.query(`SELECT 1 FROM shared_product_descriptions
          WHERE master_id=$1 AND description_type='STORE' AND status='canonical' AND language='en' AND deleted_at IS NULL`, [t.masterId]);
        if (dup.rowCount) { skipped.push({ id: t.masterId, why: 'EN_ALREADY_EXISTS' }); continue; }
        const r = await c.query(`
          ...
          VALUES ($1,'STORE','canonical','en','o4o_hff_generated',$2, now(), now()) RETURNING id`, [t.masterId, t.newContent]);
        if (r.rowCount === 1) { ins++; t.createdCanonicalId = r.rows[0].id; } else skipped.push({ id: t.masterId, why: 'INSERT_FAILED' });
      }
    }
...
fs.writeFileSync(`${D}/hff-en-batch-01-apply-results-v1.json`, JSON.stringify(out, null, 1));
