...
const APPLY = process.argv.includes('--apply') && process.env.OTC_EN_OMISSION_FIX6 === 'CONFIRM';
...
const R: Array<{ id: string; re: RegExp; en: (m: RegExpMatchArray) => string }> = [
  { id: 'CONTRA_AGE_PREG_V6', re: /이 약에 과민증 환자,\s*만\s*(\d+)\s*세 미만 소아,\s*임부 또는 임신하고 있을 가능성이 있는 여성 및 수유부는 이 약을 사용하지 마십시오/,
...
async function main(): Promise<void> {
  const src: any[] = JSON.parse(fs.readFileSync(P('otc-en-omission-remaining13.ga.json'), 'utf8')).rows;
  ...
  fs.writeFileSync(P(`otc-en-omission-restore-v6-${APPLY ? 'apply' : 'dryrun'}.ga.json`),
