 ...
 *
 * DB 접근 0. TM 파일(otc-v4-pilot-500-tm.ga.json)만 입출력한다.
 *
...
const shardFile = (i: number): string => path.join(DATA_DIR, `otc-v4-pilot-500-tm-shard${i}.ga.json`);
const filledFile = (i: number): string => path.join(DATA_DIR, `otc-v4-pilot-500-tm-shard${i}-filled.ga.json`);
const OUT_MERGE = path.join(DATA_DIR, 'otc-v4-pilot-500-tm-merge-report.ga.json');
...
const NUMBER_WORD: Record<string, RegExp> = {
  ...
  2: /\b(two|twice|second|secondary|2nd|both)\b/i,
...
function exemptedNums(ko: string, en: string): string[] {
  ...
  const stripped = ko.replace(IDIOM_POS, ' ');
...
function runRekey(): void {
  ...
  const ctrlFree = (s: string): string => s.normalize('NFC').replace(/[ ---]/g, '');
  ...
  const after = Object.values(tm.entries).filter(Boolean).length;
  fs.writeFileSync(TM, JSON.stringify({ ...tm, filled: after }, null, 2) + '\n', 'utf8');
  fs.writeFileSync(path.join(DATA_DIR, 'otc-v4-pilot-500-tm-rekey-report.ga.json'), JSON.stringify({
