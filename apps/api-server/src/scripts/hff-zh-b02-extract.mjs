 ...
 *
 * Batch 02 모집단의 KO canonical 슬롯 문구 추출 (read-only).
 ...
 *
 * 산출: data/hff-zh-b02-phrases-v1.json (저장소) · <CACHE>/hff-zh-b02-docslots.jsonl (캐시)
 */
...
for (const q of rdl('hff-zh-deferred-issue-queue-through-batch01-v1.jsonl')) if (q.productMasterId) excluded.add(q.productMasterId);
...
fs.writeFileSync(`${CACHE}/hff-zh-b02-docslots.jsonl`, lines.join('\n'));

const byKind = {};
for (const p of phrases) byKind[p.kind] = (byKind[p.kind] ?? 0) + 1;
const sorted = [...phrases].sort((a, b) => b.docFreq - a.docFreq);
fs.writeFileSync(`${D}/hff-zh-b02-phrases-v1.json`, JSON.stringify({
