// READ-ONLY atc-combo fingerprint-group audit — GA-V5 pool regeneration (agent GA). DB write 0.
...
const ENV = 'C:/Users/home/coding/o4o-platform/apps/api-server/.env';
const OUT = process.env.POOL_OUT || 'C:/Users/home/AppData/Local/Temp/claude/c--Users-home-coding-o4o-platform/988204c9-cd33-461a-806d-cd120b60847f/scratchpad/atc-combo-candidates-bysize.v5.json';
const pw = readFileSync(ENV, 'utf8').match(/^DB_PASSWORD=(.*)$/m)[1].trim();
const PORT = parseInt(process.env.AUDIT_DB_PORT || '5442', 10);
...
try {
  ...
  writeFileSync(OUT, JSON.stringify({ summary, candidates_count: candidates.length, total_pending_masters: candidates.reduce((s,c)=>s+c.size,0), candidates: candidates.slice(0,300) }, null, 1), 'utf8');
