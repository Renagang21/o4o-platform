/**
 * HFF 유산균 192건 — STORE 설명서 canonical 적재 (dry-run 기본 · --apply env 이중게이트)
 *
 * 대상: 생산 batch-001 (prod-a-cp01..10 작성분 192건). HOLD 8 · 비타민C 제외.
...
const CONFIRM = process.env.HFF_STORE_CANONICAL_APPLY_CONFIRM === 'YES';
...
function loadTargets(): GuardProductInput[] {
  ...
  for (let i = 1; i <= 10; i++) {
    out.push(...JSON.parse(fs.readFileSync(`${DATA}/hff-probiotics-prod-a-cp${String(i).padStart(2, '0')}.json`, 'utf8')));
  }
  return out;
}

async function main(): Promise<void> {
  if (APPLY && !CONFIRM) throw new Error('APPLY_BLOCKED: --apply 는 HFF_STORE_CANONICAL_APPLY_CONFIRM=YES 필요');

  const items = loadTargets();
  if (items.length !== 192) throw new Error(`대상 수 불일치: ${items.length} (기대 192)`);
  ...
  console.log(`[가드A] 파일 192: BLOCKED ${blocked} · REVIEW ${review} · draft결손 ${missDraft} · grounding결손 ${missGround}`);

  const stmts = items.map((it) => String((it as any).statementNo).trim());
  if (new Set(stmts).size !== 192) throw new Error(`신고번호 중복(파일): 유일 ${new Set(stmts).size}`);
  ...
  try {
    ...
    const report: any = { mode: APPLY ? 'apply' : 'dry-run', target: 192, guard: { blocked, review, missDraft, missGround, candMatch: cands.length, masterDup: dup.length }, planned: { ...planned, totalWrites: planned.masters + planned.candidateLinks + planned.spdKo + planned.spdEn }, sampleSanitize: sample };
    ...
    if (v.masters !== 192 || v.spdKo !== 192 || v.spdEn !== 192 || v.canonicalDup !== 0 || v.candidatesLinked !== 192) {
      await qr.rollbackTransaction();
