...
const APPLY = process.argv.includes('--apply') && process.env.OTC_EN_OMISSION_FIX4 === 'CONFIRM';
...
async function main(): Promise<void> {
  const src: any[] = JSON.parse(fs.readFileSync(P('otc-en-omission-remaining46.ga.json'), 'utf8')).rows;
  ...
  fs.writeFileSync(P(`otc-en-omission-restore-v4-${APPLY ? 'apply' : 'dryrun'}.ga.json`),
