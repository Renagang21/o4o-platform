/**
 * WO-O4O-OTC-SAFETY-MISMATCH-FRESH-RESTART-NA-V5 — 커밋 정본(HEAD otc-safety-subgroup-apply.ts) 동일 로직, ENV_PATH 만 리포 기준. 파라미터화 안전 subgroup 매장설명서 러너(KO+EN).
...
const ENV_PATH = path.resolve(process.cwd(), '.env'); // V5: 리포 기준 해석(러너는 apps/api-server 에서 실행)
...
const port = (): number => { const a = arg('--port'); if (a) return parseInt(a, 10); try { return parseInt(readFileSync(PORT_FILE, 'utf8').trim(), 10) || 5447; } catch { return 5447; } }; // V5: --port 우선(공유 port 파일 미변경)
