// WO-O4O-OTC-REMAINING-READY-SHARD-NA-V2-LIVE-APPLY-V1 — EN config 완전성 게이트 (에이전트 나).
//
// KO apply 착수 전에 "나 shard 전체 EN 저작 페이로드가 준비됐는가"를 파일 기준으로 판정한다.
// read-only · DB 미접속 · write 0 (판정 결과만 stdout + --out JSON).
//
// 검사:
//   1) eligible fp/master = dry-run manifest(공용 러너 산출) 기준 240 fp / 839 master
//   2) EN config 들의 fp 합집합 == eligible fp (누락 0 · 초과 0)
//   3) config 간 fp 중복 0 · master 중복 0
//   4) HOLD fp/master 포함 0
//   5) 각 fp 가 V2 SSOT(na) 소속인지 확인
//   6) 필수 필드(title/efficacy/usage/caution/summaryTable) 채움 · 한글 잔존 0
//   7) usageLabel 을 config 가 들고 있지 않을 것(러너가 route 로 주입)
//   8) 예상 EN write == master × 2 == 1,678T
//
// Usage(apps/api-server):
//   node src/scripts/otc-v2-en-config-coverage.na.mjs [config...]
import fs from 'node:fs';
import path from 'node:path';

const DATA = 'src/scripts/data';
const MANIFEST = path.join(DATA, 'otc-v2-dryrun-manifest.na.json');
...
const explicit = files.length ? files
  : fs.readdirSync(DATA).filter((f) => /^otc-v2-en-config-na-p\d+\.json$/.test(f)).sort().map((f) => path.join(DATA, f));

const manifest = JSON.parse(fs.readFileSync(MANIFEST, 'utf8'));
const ssot = JSON.parse(fs.readFileSync(SSOT, 'utf8'));
const census = JSON.parse(fs.readFileSync(CENSUS, 'utf8'));
const naFp = new Set(ssot.shards.na.fingerprintList);
const sizeByFp = new Map(census.readyGroups.filter((g) => naFp.has(g.fp)).map((g) => [g.fp, g.size]));
const mastersByFp = new Map(census.readyGroups.filter((g) => naFp.has(g.fp)).map((g) => [g.fp, g.masterIds]));
...
for (const f of explicit) {
  ...
  for (const e of groups) {
    ...
    if (!naFp.has(e.fp)) fail.push(`${where}: na shard 밖 fp`);
...
const result = {
  wo: 'WO-O4O-OTC-REMAINING-READY-SHARD-NA-V2-LIVE-APPLY-V1',
  shard: 'na', readOnly: true, dbWrite: 0,
...
console.log(`EN-CONFIG-COVERAGE na — ${result.verdict}`);
