/**
 * O4O 화장품 상품명 정규화 코어 (V0 규칙 SSOT)
 *
 * 기준문서: docs/cosmetics/O4O-COSMETICS-PRODUCT-GUIDE-PRODUCTION-STANDARD-V0.md §3~§4
 * 파일럿(WO-…-GUIDE-PILOT-V0)과 소매 census(WO-…-RETAIL-PRODUCT-CENSUS-V1)가 **같은 규칙**을
 * 쓰도록 여기 한 곳에만 둔다. 규칙을 고칠 때는 반드시 기준문서도 함께 고친다.
 */
/**
 ...
 */
const STRIP_RULES = [
  { id: 'R01', desc: '판매채널 전용/단독 표기', re: /\[(only ?화해|올영단독|단독|본사직영|공식|공식판매처)\]|\((OY ?단독|올영단독|온|역|단독|공식)\)/gi },
  ...
  // 천단위 쉼표(`1,000ml`)와 증량 표기(`60+10매`)가 실제로 자주 나온다 — 앞쪽 숫자까지 함께 제거한다.
  { id: 'R05', desc: '용량 (영문 단위)', re: /(?:\d[\d,]*(?:\.\d+)?\s*[+＋]\s*)?\d[\d,]*(?:\.\d+)?\s*(?:ml|mL|ML|g|kg|L|ea|EA|pcs)\b/g },
  { id: 'R05K', desc: '수량 (한글 단위)', re: /(?:\d+\s*[+＋]\s*)?\d+\s*(?:매입|개입|매|개|포|정|팩|회분|캡슐|장|박스|줄|족)(?![가-힣])/g },
  ...
  // 소매 census 에서 반복 확인된 사례 (V1)
  { id: 'R15', desc: '총량 표기 괄호', re: /[\[（(][^\])）]*총\s*\d*[^\])）]*[\])）]/g },
  { id: 'R16', desc: '택N·교차선택 옵션 표기', re: /[中중]?\s*택\s*\d+|교차\s*(?:선택|가능|증정)/g },
  { id: 'R17', desc: '팩/세트 대괄호', re: /\[\s*(?:set|세트|듀오|\d+\s*(?:pack|팩|개)|리필팩)\s*\]/gi },
  { id: 'R08', desc: '잔여 기호 정리', re: /[*#＊]+|[+＋·、,]+\s*$|^\s*[-–+]+|\(\s*\)|\[\s*\]/g },
  // R18 은 앞 규칙들이 숫자를 지운 뒤 남는 찌꺼기를 정리하므로 반드시 마지막이다.
  // 문장 중간의 X 는 제품명일 수 있으므로(`소스 X`) **끝에 매달린 경우만** 지운다.
  { id: 'R18', desc: '수량 제거 후 남은 잔재', re: /\s[xX×]\s*$|[\[（(]\s*[/／,·]?\s*[\])）]/g },
];
...
const CATEGORY_TYPE = {
  ...
  // 소매 census(V1) 에서 실제로 등장한 판매처 3depth 카테고리명.
  '스킨/토너/패드': '토너', '미스트/페이셜오일': '미스트', '에센스/세럼/앰플': '세럼',
  '로션/에멀젼/올인원': '로션', '크림/아이크림': '크림', 스팟케어: '트러블케어', '스킨케어 세트': '스킨케어세트',
  '파운데이션/콤팩트': '메이크업', '메이크업 픽서': '메이크업', '메이크업베이스/프라이머': '메이크업',
  '셰이딩/컨투어링': '메이크업', 아이섀도우: '메이크업', 마스카라: '메이크업', 아이브로우: '메이크업',
  아이라이너: '메이크업', 립라이너: '립', 립밤: '립',
  '클렌징폼/젤/비누': '클렌징폼', '클렌징티슈/패드': '클렌징워터', '필링/스크럽': '스크럽',
  '클렌징오일/워터': '클렌징오일', '클렌징로션/크림': '클렌징워터',
  워시오프팩: '팩', 필오프팩: '팩', '부분마스크/코팩/패치': '패치', '시트마스크(시트팩)': '마스크',
  '모델링/고무팩': '팩', '태닝/애프터선': '선케어', '선스틱/선스프레이': '선케어', 선케어세트: '선케어',
  '선쿠션/파우더': '선케어', 선크림: '선케어',
  핸드케어: '핸드크림', '샤워/입욕': '바디워시', 바디미스트: '바디미스트', '바디로션/크림/오일': '바디로션',
  '린스/컨디셔너': '컨디셔너', '헤어케어 세트': '헤어케어', '헤어컬러/펌': '염모제',
  '헤어오일/에센스': '헤어에센스', 탈모케어: '탈모케어',
  '일반 네일 폴리시': '네일', '젤 네일 폴리시': '네일', 네일케어: '네일',
  '기타 향수': '향수', 드레스퍼퓸: '향수', '제모/왁싱': '제모',
};
/**
...
export { STRIP_RULES, COLOR_RULES, TYPES, CATEGORY_TYPE, CATEGORY_SKIP, typeFromCategory, detectType, normalize, keyOf };
