/**
 * WO-O4O-OTC-BATCH-01B-ALFACALCIDOL-KO-EN-COMPLETE-V1 — STEP 2 (en 번역 persist)
 *
 * 알파칼시돌 en 번역 1건을 ko canonical 이 있는 master 에 en needs_review 전개 (INSERT only).
 * dry-run 기본 / apply 는 이중 게이트(`--apply` + DRUG_OTC_BATCH01B_ALFA_EN_CONFIRM=YES).
 *
 * 대상 master = STEP 1 로 만든 ko canonical(source_type=mfds_drug_otc, source_ref_id=candidate 0436f0d8).
 *   → ko↔en master_id·source_ref_id 정합. apply 직전 재열거 → EXPECTED(21) 불일치 ABORT.
 *
 * 안전: buildDrugOtcEnConsumerHtml(구조화 번역만·bodyMarkdown/translatorNote 미삽입) ·
 *   한글/빈/<table>/주석/이중escape/sd-warn ABORT · summary NULL(en) ·
 *   INSERT WHERE NOT EXISTS(en STORE needs_review|canonical) → 충돌0+멱등 · 단일 TX · 사후 중복0 ROLLBACK.
 * 접속: Cloud SQL Auth Proxy(:5442 → production). Usage(apps/api-server): NODE_ENV= ../../node_modules/.bin/tsx src/scripts/drug-otc-batch-01b-alfacalcidol-en-persist.ts [--apply]
 */
...
const EN_PATH = path.resolve(process.cwd(), 'src/scripts/data/otc-en-translations-batch-01b-alfacalcidol-v1.json');
const SOURCE_TYPE = 'mfds_drug_otc';
const GROUP_KEY = '알파칼시돌|0.5마이크로그램|연질캡슐';
const EXPECTED_KO = 21;

async function main(): Promise<void> {
  const apply = process.argv.slice(2).includes('--apply') && process.env.DRUG_OTC_BATCH01B_ALFA_EN_CONFIRM === 'YES';
  ...
  try {
    ...
    if (masterIds.length !== EXPECTED_KO) report.anomalies.push(`ko canonical ${masterIds.length} !== EXPECTED ${EXPECTED_KO} (STEP 1 선행 필요/재열거 불일치)`);
    ...
    if (apply && report.newInsert > 0) {
      ...
      try {
        ...
        // 사후: ko↔en 정합(같은 master·source_ref) = EXPECTED
  ...
  else console.log('  (dry-run — write 없음. apply: --apply + DRUG_OTC_BATCH01B_ALFA_EN_CONFIRM=YES)');
}
