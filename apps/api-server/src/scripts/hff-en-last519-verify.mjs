...
const SEL = JSON.parse(fs.readFileSync(`${D}/hff-en-last519-selection-v1.json`, 'utf8'));
const B01 = JSON.parse(fs.readFileSync(`${D}/hff-en-bulk-production-completed-through-batch-01-v1.json`, 'utf8'));
const sha = (s) => crypto.createHash('sha256').update(s ?? '').digest('hex');
const HANGUL = /[가-힣]/;
const norm = (s) => (s ?? '').replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();

const c = new pg.Client({ host: '127.0.0.1', port: parseInt(process.env.PROXY_PORT ?? '5519', 10), user: process.env.PGUSER, password: process.env.PGPW, database: 'o4o_platform', ssl: false });
await c.connect();
...
fs.writeFileSync(`${D}/hff-en-last519-final-hold-v1.jsonl`, queue.map((r) => JSON.stringify(r)).join('\n') + '\n');
fs.writeFileSync(`${D}/hff-en-last519-final-hold-summary-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-en-bulk-production-completed-through-last519-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-en-bulk-production-remaining-after-last519-v1.json`, JSON.stringify({
...
const out = {
  ...
  globals: g, expectedEnDelta: 1282, actualEnDelta: g.en_canon - 15498,
  statusSum: CLS.results.length, sum4209: CLS.results.length === 4209, byStatus: st,
  expectedUnlockFromSelection: SEL.expectedUnlock,
...
out.globalsOk = g.ko_canon === 40918 && g.pm_hff === 40948 && g.en_dup === 0 && g.en_canon === 16780;
...
fs.writeFileSync(`${D}/hff-en-last519-independent-verification-v1.json`, JSON.stringify(out, null, 1));
