 ...
 *
 * ⚠️ 독립성 계약: 실행기(otc-v4-carryover72-executor.ga.ts)의 어떤 함수도 import 하지 않는다.
 ...
 *
 * 실행:
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-carryover72-independent-verify.ga.ts --baseline --port 5502
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-carryover72-independent-verify.ga.ts --verify   --port 5502
 */
...
const LEDGER_500 = P('otc-v4-carryover72-selection-ledger.ga.json');
...
const RESULT = P('otc-v4-carryover72-result-ledger.ga.json');
const SOURCE = P('otc-v4-carryover72-source.ga.json');
const OUT_BASELINE = P('otc-v4-carryover72-verify-baseline.ga.json');
const OUT_VERIFY = P('otc-v4-carryover72-independent-verification.ga.json');
...
async function verify(pool: Pool): Promise<void> {
  ...
  const residueAudit = excIds.length ? (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-carryover72'`, [excIds])).rows[0].n : 0;
  ...
  const outsideAudit = (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE (metadata->>'batchId')='otc-v4-carryover72' AND master_id <> ALL($1::uuid[])`, [ledgerIds])).rows[0].n;
  ...
  const auditPerGreen = greenIds.length ? (await pool.query(
    `SELECT master_id::text mid, count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-carryover72'
  ...
  if (base) {
    ...
    const nowRows = base.pilot100GreenIds ? (await pool.query(
      `SELECT master_id::text mid, COALESCE(language,'ko') lang, md5(content) h
         FROM shared_product_descriptions
        WHERE master_id = ANY($1::uuid[]) AND description_type='STORE' AND status='canonical' AND deleted_at IS NULL
        ORDER BY 1,2`, [[...readJson(GREEN_100).rows.map((r: any) => r.masterId), ...readJson(GREEN_500).rows.map((r: any) => r.masterId), ...readJson(GREEN_N2K).rows.map((r: any) => r.masterId)]])).rows : [];
  ...
  G('IV-23', '선행 배치(pilot100 + pilot500 + next2000) 교집합 0', '0', String(inter.length), inter.length === 0);
  ...
  const out = {
    ...
    kind: 'independent-verification', batchId: 'otc-v4-carryover72', verifiedAt: new Date().toISOString(),
