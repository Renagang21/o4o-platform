/**
 * WO-O4O-HFF-ZH-ALL-REMAINING-10414-DIRECT-BULK-PRODUCTION-AND-TRACK-CLOSURE-V1  §3 / §7
 *
 ...
 *
 * 산출: data/hff-zh-final-safe-targets-v1.json · data/hff-zh-final-render-audit-v1.json
 *       data/hff-zh-deferred-issue-queue-through-final-v1.jsonl (+ -summary-v1.json)
 */
...
const WO = 'WO-O4O-HFF-ZH-ALL-REMAINING-10414-DIRECT-BULK-PRODUCTION-AND-TRACK-CLOSURE-V1';
/* 최종 마감: 목표는 정상 잔여 전량. 고정 목표치 없음. */
...
/* 2차 패스(ZH_QUEUE_RETRY=1): §7 "정상 생산 가능 잔여 0" 을 만족시키기 위해, 문제 큐로 제외했던 문서도
   현재 자산으로 다시 build() 해 본다. 이때 승계 큐는 직전 패스가 만든 통합 큐를 기준으로 한다. */
const QUEUE_RETRY = process.env.ZH_QUEUE_RETRY === '1';
const PRIOR_QUEUE = rdl(process.env.ZH_PRIOR_QUEUE ?? 'hff-zh-deferred-issue-queue-through-batch03-v1.jsonl');
const RETRY = new Set(PRIOR_QUEUE.filter((q) => QUEUE_RETRY || q.issueType === 'RENDER_FAILURE').map((q) => q.productMasterId));
...
for (const row of pool) {
  if (!row.content || !/<h2|<li/.test(row.content)) {
    issues.push({ batch: 'final', productMasterId: row.master_id, koCanonicalId: row.id, productName: row.product_name,
  ...
  if (b.misses.length || b.hangul) {
    ...
    /* 번역이 확정되지 않은 문서는 예외 없이 큐에 남긴다 — 수치 구조 문제(NUMBER_DRIFT)로 이미 기록한
       경우만 제외한다. 이 분기를 `!b.misses.length` 로 두면 대응 문구가 없는 miss 문서가 큐에서
       통째로 누락된다(§7 문제 큐 누락 0). */
    if (!drift) {
      const left = b.misses.length
        ? b.misses.slice(0, 4).map((m) => m.text).join(' / ')
        : (stripKeep(b.html.replace(/<h1[^>]*>[\s\S]*?(?=<small|<\/h1>)/g, ' ').replace(/<[^>]+>/g, ' ')).match(/[^\s]*[가-힣][^\s]*/g) ?? []).slice(0, 6).join(' ');
      issues.push({ batch: 'final', productMasterId: row.master_id, koCanonicalId: row.id, productName: row.product_name,
...
fs.writeFileSync(`${CACHE}/hff-zh-final-render-blockers.json`,
  JSON.stringify([...missTally.entries()].sort((a, b) => b[1] - a[1]).slice(0, 400).map(([k, n]) => ({ n, k })), null, 1));
/* 최종 마감 WO: 목표는 "정상 잔여 전량"이므로 고정 10,000 게이트와 절단을 폐기한다.
   생산 가능한 것은 전부 대상으로 삼고, 남는 것은 문제 큐로만 빠진다(§1·§3). */
...
/* 이번 패스에서 다시 판정한 문서의 기존 큐 항목은 이번 결과로 대체한다(중복 방지). */
const ATTEMPTED = new Set(pool.map((r) => r.master_id));
pool.length = 0;
clearMemo();
global.gc?.();
...
const TASKS = `${CACHE}/hff-zh-final-render-tasks.jsonl`;
...
const runChunk = (i) => new Promise((resolve, reject) => {
  const out = `${CACHE}/hff-zh-final-render-part-${i}.json`;
...
for (const i of chunks) fs.rmSync(`${CACHE}/hff-zh-final-render-part-${i}.json`, { force: true });
...
const carried = PRIOR_QUEUE.filter((q) => !resolved.has(q.productMasterId) && !ATTEMPTED.has(q.productMasterId));
const mergedQueue = [...(QUEUE_RETRY ? [] : carried), ...issues];
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-final-v1.jsonl`, mergedQueue.map((x) => JSON.stringify(x)).join('\n') + (mergedQueue.length ? '\n' : ''));
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-final-summary-v1.json`, JSON.stringify({
  wo: WO, generatedAt: audit.renderedAt, total: mergedQueue.length, carriedForward: mergedQueue.length - issues.length,
  resolvedFromPriorQueue: resolved.size, addedByThisPass: issues.length,
