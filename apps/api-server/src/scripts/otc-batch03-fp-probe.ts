/**
 * read-only FP PROBE (WO-O4O-OTC-NIZATIDINE-LEVOCARNITINE-SOBREROL-KO-EN-COMPLETE-GA-V1, Agent 가). DB write 0.
 * 3그룹(니자티딘 75정 / 엘카르니틴 330정 / 소브레롤 200캡슐)을 runner 와 동일 fingerprint 산식으로 재고정.
...
const GROUPS = [
  { key: 'nizatidine-75mg-jeong', ing: '니자티딘', dose: '75밀리그램', form: '정', expTarget: 18, expExclude: 3 },
  { key: 'levocarnitine-330mg-jeong', ing: '엘카르니틴', dose: '330밀리그램', form: '정', expTarget: 16, expExclude: 26 },
  { key: 'sobrerol-200mg-capsule', ing: '소브레롤', dose: '200밀리그램', form: '캡슐', expTarget: 15, expExclude: 0 },
];
    ...
    for (const g of GROUPS) {
      const drafts = await ds.query(
        `SELECT candidate_id::text, source_identifier_value gk, title
         ...
         WHERE source_label='MFDS_DRUG_OTC' AND deleted_at IS NULL AND source_identifier_value LIKE $1`, [`${g.ing}|${g.dose}|%`]);
      ...
      const dist: Record<string, { total: number; easy: number; authored: number; other: number; route: string; form: string }> = {};
      for (const r of withFp) {
        const d = dist[r.fp] || (dist[r.fp] = { total: 0, easy: 0, authored: 0, other: 0, route: r.route, form: r.form });
        d.total++;
        if (r.ko_canon_src === 'mfds_easy_drug') d.easy++;
        else if (r.ko_canon_src === 'mfds_drug_otc' || r.ko_canon_src === 'nutrition_combo') d.authored++;
        else d.other++;
      }
      const easySorted = Object.entries(dist).sort((a, b) => b[1].easy - a[1].easy);
      ...
      results.push({
        key: g.key, ing: g.ing, dose: g.dose, form: g.form,
        expTarget: g.expTarget, expExclude: g.expExclude,
        drafts, coarseTotal: coarse.length,
        dominantFp: domFp, dominantForm: domFp ? dist[domFp].form : null, dominantRoute: domFp ? dist[domFp].route : null,
        target_total: target.length, target_easy: targetEasy.length, target_authored_conflict: targetAuthored.length,
        nonOralInTarget: target.filter((r: any) => r.route !== 'oral').map((r: any) => r.name),
        excludeFps, excludeCount,
        matchTarget: targetEasy.length === g.expTarget, matchExclude: excludeCount === g.expExclude,
        fpDistribution: Object.entries(dist).sort((a, b) => b[1].total - a[1].total).map(([fp, d]) => ({ fp, ...d })),
