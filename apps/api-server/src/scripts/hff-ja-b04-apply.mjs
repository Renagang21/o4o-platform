 ...
 *
 * Batch 04 JA canonical INSERT.
 *   - INSERT 전용. KO canonical / EN / ZH / ProductMaster / candidate 는 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_JA_B04_APPLY_CONFIRM=YES.
...
fs.writeFileSync(`${D}/hff-ja-b04-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-ja-b04-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
