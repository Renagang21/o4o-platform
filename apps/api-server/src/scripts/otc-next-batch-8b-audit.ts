/**
 * WO-O4O-OTC-GROUNDED-UPGRADE-NEXT-BATCH-8B-AUDIT-NA-V1 — Agent 나. READ-ONLY, DB write 0.
 *
 * batch-8 감사(otc-next-batch-8-audit.ts)의 machinery 를 재사용해, bridge SSOT 의 authored그대로확장 fp-entry 를
 * bridge_n desc 로 walk 하며 (완료/진행 groupKey + pilot + 민감약효군 + 비경구 + empty-ingredient 제외)
 * 동일 gate pipeline 으로 다음 최대 8개의 clean Track A READY 후보를 선정한다.
 *
 * 제외 groupKey 23 = 기존 완료 15 + 현재 가·다 실행 중 batch-8 전량 8. + pilot fp 4b4e162690065e8e.
 *   + SENSITIVE_RE. + route!=oral.
 * empty-ingredient(다성분 콤보, ingredientOf()='' → LIKE '%()' 부적용)는 이 coarse-by-(ingredient) 파이프라인
 *   대상 아님 → skip(사유 기록).
 *
 * gate (NEXT-5/batch-8 VERBATIM): fp 재현(target===bridge_n) · easy STORE ko canonical 정확히1/master==target ·
 ...
 *
 * write 산식(정정본 고정): T=target master 수 · ko=4T · en=2T · 총=6T. (en=T 오산 금지.)
 *
 * 산출: src/scripts/data/otc-next-batch-8b-audit-v1.json (deterministic: master id asc, fp n desc→fp asc,
 *   후보 target desc→groupKey asc). 콘솔 요약. 2회 실행 byte-compare 외부에서.
 * Usage(apps/api-server): npx tsx src/scripts/otc-next-batch-8b-audit.ts
 */
...
// 완료/진행 groupKey (ingredient|strength|form) — 제안 금지. 총 23 = 완료 15 + 실행중 batch-8 8.
const EXCLUDE_DONE = [
  '에르도스테인|300밀리그램|정',
...
// 현재 에이전트 가·다 실행 중 (batch-8) — 중복 배정 금지
const EXCLUDE_INFLIGHT = [
  '락토바실루스아시도필루스균|300밀리그램|캡슐',
  '트리메부틴말레산염|200밀리그램|정',
  '알파칼시돌|0.5마이크로그램|연질캡슐',
  '메코발라민|500마이크로그램|캡슐',
  '덱스판테놀|100밀리그램|정',
  '아세틸시스테인|100밀리그램|캡슐',
  '폴산|1밀리그램|정',
...
const EXCLUDE = new Set<string>([...EXCLUDE_DONE, ...EXCLUDE_INFLIGHT]);
...
async function main(): Promise<void> {
  ...
  for (const r of forBundling) {
    const item = { groupKey: r.groupKey, target: r.target_master, ko: r.expected_ko_write.ko_total, en: r.expected_en_write.en_total, total: r.total_write, exclude: r.exclude_nonTarget, zeroExclude: r.exclude_nonTarget === 0 };
  ...
  // 묶음 내 실행 순서: zero-exclude 선두 → carve-out 적은(저위험) 순 → target desc → groupKey asc
  const sortBundle = (arr: any[]) => arr.sort((a, b) =>
    (a.zeroExclude === b.zeroExclude ? 0 : a.zeroExclude ? -1 : 1) || a.exclude - b.exclude || b.target - a.target || (a.groupKey < b.groupKey ? -1 : 1));
  sortBundle(bundleGa); sortBundle(bundleDa);

  const out = {
    wo: 'WO-O4O-OTC-GROUNDED-UPGRADE-NEXT-BATCH-8B-AUDIT-NA-V1', agent: '나', readOnly: true, dbWrite: 0,
  ...
  writeFileSync(path.join(OUT_DIR, 'otc-next-batch-8b-audit-v1.json'), JSON.stringify(out, null, 2), 'utf8');
