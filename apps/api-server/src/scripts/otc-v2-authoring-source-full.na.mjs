// WO-O4O-OTC-REMAINING-READY-SHARD-NA-V2-LIVE-APPLY-V1 — EN 저작용 **무절단** 공식 원문 덤프 (에이전트 나).
//
// 배경: 공용 러너의 `--emit-sample` 은 주의 축을 `slice(0, 1200)` 으로 절단한다.
//       절단본으로 EN 을 저작하면 공식 주의 정보층이 소실되므로, 저작 기준 원문은 무절단으로 따로 뽑는다.
//       공용 러너는 수정하지 않는다(본 스크립트는 별도 read-only 산출).
//
// 계약: 섹션 추출·정규화는 census-v2 VERBATIM. 대상은 V2 SSOT(na) ∩ dry-run manifest eligible.
//       가 세션 산출물(otc-v2-authoring-source-full.ga.mjs) 과 동일 방식 — 파일 미접촉.
// read-only · DB write 0 · 자격증명은 process.env 로만 사용(값 출력 0).
//
// Usage(apps/api-server):
//   npx tsx src/scripts/otc-v2-authoring-source-full.na.mjs
...
const OUT = process.env.OUT || path.join(DATA, 'otc-v2-authoring-source-full.na.json');
const EXPECT_FP = 240;
...
const groups = census.readyGroups.filter((g) => naFp.has(g.fp) && !holdFp.has(g.fp));
if (groups.length !== EXPECT_FP) throw new Error(`eligible ${groups.length} !== ${EXPECT_FP}`);
...
fs.writeFileSync(OUT, JSON.stringify({
  wo: 'WO-O4O-OTC-REMAINING-READY-SHARD-NA-V2-LIVE-APPLY-V1', shard: 'na', readOnly: true, dbWrite: 0,
  note: '무절단 공식 원문(EN 저작 기준). 절단·요약 없음.', groups: out,
}, null, 1) + '\n', 'utf8');
const totalCau = out.reduce((a, x) => a + x.lens.cau, 0);
console.log(`FULL-SOURCE na — ${out.length} fp / ${out.reduce((a, x) => a + x.size, 0)} master → ${OUT}`);
