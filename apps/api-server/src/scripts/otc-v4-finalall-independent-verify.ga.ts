 ...
 *
 * ⚠️ 독립성 계약: 실행기(otc-v4-finalall-executor.ga.ts)의 어떤 함수도 import 하지 않는다.
 ...
 *
 * 실행:
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-finalall-independent-verify.ga.ts --baseline --port 5502
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-finalall-independent-verify.ga.ts --verify   --port 5502
 */
...
const LEDGER_500 = P('otc-v4-finalall-selection-ledger.ga.json');
...
const RESULT = P('otc-v4-finalall-result-ledger.ga.json');
const SOURCE = P('otc-v4-finalall-source.ga.json');
const OUT_BASELINE = P('otc-v4-finalall-verify-baseline.ga.json');
const OUT_VERIFY = P('otc-v4-finalall-independent-verification.ga.json');
...
async function verify(pool: Pool): Promise<void> {
  ...
  const residueAudit = excIds.length ? (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-finalall'`, [excIds])).rows[0].n : 0;
  ...
  const outsideAudit = (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE (metadata->>'batchId')='otc-v4-finalall' AND master_id <> ALL($1::uuid[])`, [ledgerIds])).rows[0].n;
  ...
  const auditPerGreen = greenIds.length ? (await pool.query(
    `SELECT master_id::text mid, count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-finalall'
  ...
  const out = {
    ...
    kind: 'independent-verification', batchId: 'otc-v4-finalall', verifiedAt: new Date().toISOString(),
