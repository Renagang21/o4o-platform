/**
 * WO-O4O-OTC-FAMOTIDINE-10MG-PILOT-DRYRUN-GA-V1 (펙소페나딘 전환) — STEP 3 (en canonical 전환)
 *
 * 펙소페나딘 en 설명서(needs_review)를 canonical 로 상태 전환(내용 불변).
 * dry-run 기본 / apply 는 이중 게이트(`--apply` + DRUG_OTC_FEXO_EN_PROMOTE_CONFIRM=YES).
 * 원칙: 상태만 canonical + updated_at. content·summary 지문(길이+md5) 전후 동일. ko 불변. INSERT/DELETE 0. 단일 TX.
 * 대상=en STORE(mfds_drug_otc)·candidate 049c2a1c. EXPECTED=14. 멱등(needs_review만 flip).
 * Usage(apps/api-server): NODE_ENV= ../../node_modules/.bin/tsx src/scripts/drug-otc-fexofenadine-en-canonical-promotion.ts [--apply]
 */
...
async function main(): Promise<void> {
  const apply = process.argv.slice(2).includes('--apply') && process.env.DRUG_OTC_FEXO_EN_PROMOTE_CONFIRM === 'YES';
  ...
  try {
    ...
    const en: Array<{ id: string; master_id: string; status: string; content: string; summary: string | null }> = await ds.query(
      `SELECT id::text, master_id::text, status, content, summary FROM shared_product_descriptions
  ...
  else console.log(`  toFlip ${report.needsReview} (dry-run. apply: --apply + DRUG_OTC_FEXO_EN_PROMOTE_CONFIRM=YES)`);
}
