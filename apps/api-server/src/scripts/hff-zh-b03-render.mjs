/**
 * WO-O4O-HFF-ZH-BATCH-03-10000-DIRECT-BULK-PRODUCTION-V1  §3 / §7
 *
 ...
 *
 * 산출: data/hff-zh-b03-safe-targets-v1.json · data/hff-zh-b03-render-audit-v1.json
 *       data/hff-zh-deferred-issue-queue-through-batch03-v1.jsonl (+ -summary-v1.json)
 */
...
const WO = 'WO-O4O-HFF-ZH-BATCH-03-10000-DIRECT-BULK-PRODUCTION-V1';
const TARGET = 10000;
...
const PRIOR_QUEUE = rdl('hff-zh-deferred-issue-queue-through-batch02-v1.jsonl');
...
fs.writeFileSync(`${CACHE}/hff-zh-b03-pool.jsonl`,
...
for (const row of pool) {
  if (!row.content || !/<h2|<li/.test(row.content)) {
    issues.push({ batch: 'batch03', productMasterId: row.master_id, koCanonicalId: row.id, productName: row.product_name,
...
fs.writeFileSync(`${CACHE}/hff-zh-b03-render-blockers.json`,
...
const TASKS = `${CACHE}/hff-zh-b03-render-tasks.jsonl`;
...
const runChunk = (i) => new Promise((resolve, reject) => {
  const out = `${CACHE}/hff-zh-b03-render-part-${i}.json`;
...
for (const i of chunks) fs.rmSync(`${CACHE}/hff-zh-b03-render-part-${i}.json`, { force: true });
...
fs.writeFileSync(`${D}/hff-zh-b03-safe-targets-v1.json`, JSON.stringify({ wo: WO, generatedAt: audit.renderedAt, count: targets.length, targets }, null, 1));
fs.writeFileSync(`${D}/hff-zh-b03-render-audit-v1.json`, JSON.stringify(audit, null, 1));
const mergedQueue =[...PRIOR_QUEUE.filter((q) => !resolved.has(q.productMasterId)), ...issues];
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-batch03-v1.jsonl`, mergedQueue.map((x) => JSON.stringify(x)).join('\n') + (mergedQueue.length ? '\n' : ''));
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-batch03-summary-v1.json`, JSON.stringify({
