 ...
 *
 * DB 접근 0. TM 파일(otc-v4-route535-tm.ga.json)만 입출력한다.
 *
...
import { WO_500 } from './otc-v4-route535-contract.ga.js';

const TM = path.join(DATA_DIR, 'otc-v4-route535-tm.ga.json');
const shardFile = (i: number): string => path.join(DATA_DIR, `otc-v4-route535-tm-shard${i}.ga.json`);
const filledFile = (i: number): string => path.join(DATA_DIR, `otc-v4-route535-tm-shard${i}-filled.ga.json`);
const OUT_MERGE = path.join(DATA_DIR, 'otc-v4-route535-tm-merge-report.ga.json');
...
function runRekey(): void {
  ...
  const after = Object.values(tm.entries).filter(Boolean).length;
  fs.writeFileSync(TM, JSON.stringify({ ...tm, filled: after }, null, 2) + '\n', 'utf8');
  fs.writeFileSync(path.join(DATA_DIR, 'otc-v4-route535-tm-rekey-report.ga.json'), JSON.stringify({
