...
const APPLY = process.argv.includes('--apply') && process.env.HFF_ZH_B01_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-zh-b01-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-zh-b01-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
