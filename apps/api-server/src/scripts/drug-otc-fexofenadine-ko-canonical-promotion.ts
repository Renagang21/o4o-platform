/**
 * WO-O4O-OTC-FAMOTIDINE-10MG-PILOT-DRYRUN-GA-V1 (펙소페나딘 전환) — STEP 1 (ko canonical promotion)
 *
 * 펙소페나딘염산염 60mg 정 draft(candidate 049c2a1c)를 ko STORE canonical 로 승격 (INSERT only).
 * draft 는 완비(보정 없음) — content_json 그대로 buildDrugOtcConsumerHtml 렌더.
 * dry-run 기본 / apply 는 이중 게이트(`--apply` + DRUG_OTC_FEXO_KO_CONFIRM=YES).
 *
 * 대상: 그룹(펙소페나딘염산염|60밀리그램|정) master 중 canonical STORE 없고 rx 아닌 경구 master. EXPECTED=14.
 * apply 직전 재열거 → EXPECTED(14) 불일치 ABORT.
 *
 ...
 * 접속: Cloud SQL Auth Proxy(:5442 → production). Usage(apps/api-server): NODE_ENV= ../../node_modules/.bin/tsx src/scripts/drug-otc-fexofenadine-ko-canonical-promotion.ts [--apply]
 */
...
const LANGUAGE = 'ko';
const GROUP = { key: '펙소페나딘염산염|60밀리그램|정', ingredient: '펙소페나딘염산염', dose: '60밀리그램', formKeyword: '정' };
const EXPECTED_PROMOTE = 14;

async function main(): Promise<void> {
  const apply = process.argv.slice(2).includes('--apply') && process.env.DRUG_OTC_FEXO_KO_CONFIRM === 'YES';
  ...
  try {
    ...
    const grp: Array<{ id: string }> = await ds.query(`SELECT pm.id::text FROM product_masters pm WHERE ${grpBase} AND pm.drug_category ILIKE 'otc'`, P);
    const rxRows: Array<{ id: string }> = await ds.query(
      `SELECT DISTINCT pm.id::text FROM product_masters pm JOIN product_drug_extensions e ON e.product_master_id=pm.id AND e.deleted_at IS NULL AND e.drug_category='rx' WHERE ${grpBase} AND pm.drug_category ILIKE 'otc'`, P);
    const gr: Array<{ n: string }> = await ds.query(
      `SELECT count(*)::text n FROM product_masters pm WHERE ${grpBase} AND pm.drug_category ILIKE 'otc'
    ...
    const pt: Array<{ id: string }> = await ds.query(
      `SELECT pm.id::text FROM product_masters pm
        WHERE ${grpBase} AND pm.drug_category ILIKE 'otc'
  ...
  else console.log('  (dry-run — write 없음. apply: --apply + DRUG_OTC_FEXO_KO_CONFIRM=YES)');
}
