// WO-O4O-OTC-REMAINING-READY-SHARD-DA-V2-LIVE-APPLY-V1 — EN 저작용 **무절단** 공식 원문 덤프 (에이전트 다).
//
...
//       가 세션 동명 .ga.mjs 는 접촉하지 않는다 — 본 파일은 da 전용 사본이다.
//
// 계약: 섹션 추출·정규화는 census-v2 VERBATIM. 대상은 V2 SSOT(da) ∩ dry-run manifest eligible.
// read-only · DB write 0 · 자격증명은 process.env 로만 사용(값 출력 0).
//
// Usage(apps/api-server):
//   npx tsx src/scripts/otc-v2-authoring-source-full.da.mjs
...
const OUT = process.env.OUT || path.join(DATA, 'otc-v2-authoring-source-full.da.json');
...
const groups = census.readyGroups.filter((g) => daFp.has(g.fp) && !holdFp.has(g.fp));
...
fs.writeFileSync(OUT, JSON.stringify({
  wo: 'WO-O4O-OTC-REMAINING-READY-SHARD-DA-V2-LIVE-APPLY-V1', shard: 'da', readOnly: true, dbWrite: 0,
  note: '무절단 공식 원문(EN 저작 기준). 절단·요약 없음.', groups: out,
}, null, 1) + '\n', 'utf8');
const totalCau = out.reduce((a, x) => a + x.lens.cau, 0);
console.log(`FULL-SOURCE da — ${out.length} fp / ${out.reduce((a, x) => a + x.size, 0)} master → ${OUT}`);
