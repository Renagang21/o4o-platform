/**
 * READ-ONLY — Agent B 소유 단일 기능성 원료 selector (공용 hff-sf-select 로직 재사용, B_INGREDIENTS 주입). DB write 0.
 *   PROXY_PORT=5442 npx tsx src/scripts/hff-sf-b-select.ts --ingredient 차전자피식이섬유 --out <dir>
 *
 * 공용 hff-sf-select.ts 는 Agent C 미커밋 WIP → 편집 금지. 본 파일은 동일 로직을 B 소유 신규 파일로 두고
 * 원료 config 만 hff-sf-b-registry(B_INGREDIENTS)에서 주입. resolveFunctions/classify/parsers 는 read-only import.
 * 대상: pure-single([원료] 브래킷 1종 & ing.labelRe) · 고형 · 미승격 · exclude-taken.
 * 산출: <dir>/<slug>-ready.json · <dir>/<slug>-shard-<0|1|2>.json · <dir>/<slug>-review-later.json
 */
  ...
  try {
    ...
    for (;;) {
      ...
      for (const r of rows) {
        ...
        if (classify(label) && !ing.allowClassified) continue; // 기존 registry 등재 원료면 제외(교차 귀속 방지). allowClassified 원료는 pure-single 소유이므로 우회.
        funnel.pureSingle++;
