/**
 * HFF 유산균 Batch 003 226건 — STORE 설명서 canonical 적재 (dry-run 기본 · --apply env 이중게이트)
 *
 * 대상: 생산 batch-003 (prod-c-cp01..12 작성분 226건). HOLD 8(별도 -hold.json, 적재 대상 아님).
 * 계약: hff-b2-store-canonical-apply.ts(Batch 002=200) 그대로 — 입력·건수 226·env게이트명만 변경.
 ...
 *
 * Usage:
 *   dry-run: PROXY_PORT=5460 npx tsx src/scripts/hff-b3-store-canonical-apply.ts
 *   apply  : HFF_B3_CANONICAL_APPLY_CONFIRM=YES PROXY_PORT=5460 npx tsx ... --apply
 */
...
const PROXY_PORT = parseInt(process.env.PROXY_PORT ?? '5460', 10);
...
function loadTargets(): GuardProductInput[] {
  ...
  for (let i = 1; i <= 12; i++) {
    out.push(...JSON.parse(fs.readFileSync(`${DATA}/hff-probiotics-prod-c-cp${String(i).padStart(2, '0')}.json`, 'utf8')));
  }
  return out;
}
...
async function main(): Promise<void> {
  ...
  console.log(`[가드A] 파일 ${TARGET}: BLOCKED ${blocked} · REVIEW ${review} · draft결손 ${missDraft} · grounding결손 ${missGround} · REVIEW rule ${JSON.stringify(reviewRules)}`);
  ...
  try {
    ...
    if (!APPLY) {
      fs.writeFileSync(`${SP}/hff-b3-apply-dryrun-plan.json`, JSON.stringify({ planned, sample, outcomes: outcomes.slice(0, 5) }, null, 2));
    ...
    const tags = JSON.stringify(['import:mfds-hff', 'batch:probiotics-prod-003', 'wo:hff-store-canonical']);
    await qr.query(
    ...
    fs.writeFileSync(`${SP}/hff-b3-apply-rollback-manifest.json`, JSON.stringify({ createdMasters: ids, createdSpd: spdIds, candIds, snapshot, outcomes }, null, 2));
    await qr.commitTransaction();
    console.log('COMMIT 완료 · 롤백 매니페스트 → scratchpad/hff-b3-apply-rollback-manifest.json');
    console.log('JSON_REPORT_BEGIN'); console.log(JSON.stringify(report, null, 2)); console.log('JSON_REPORT_END');
  } catch (e) {
...
main().catch((e) => { console.error('[hff-b3-store-canonical-apply] FAILED:', e instanceof Error ? e.message : e); process.exit(1); });
