/**
 * WO-O4O-HFF-ZH-BATCH-04-10000-DIRECT-BULK-PRODUCTION-V1  §4
 * 실제 render 와 **같은 build()** 로 측정한다 (offline, DB 미접근).
 * 입력: .cache/hff-zh-b04-pool.jsonl (render 가 read-only 로 덤프한 KO 모집단)
 */
...
const pool = fs.readFileSync(`${CACHE}/hff-zh-b04-pool.jsonl`, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
...
fs.writeFileSync(`${CACHE}/hff-zh-b04-atoms.json`, JSON.stringify(atoms, null, 1));
/* 같은 문구가 clause·spec 등 여러 슬롯으로 중복 집계된다. 저작은 kind 무관(any)으로 하므로
   본문 기준으로 합산해 라운드 작업 목록을 만든다. */
const byText = new Map();
for (const [k, n] of tally) {
  const t = k.split('|').slice(2).join('|');
  byText.set(t, (byText.get(t) ?? 0) + n);
}
const next = [...byText.entries()].sort((a, b) => b[1] - a[1]).slice(0, 1200).map(([k, n]) => ({ n, k }));
fs.writeFileSync(`${CACHE}/hff-zh-b04-render-blockers.json`, JSON.stringify(next, null, 1));
