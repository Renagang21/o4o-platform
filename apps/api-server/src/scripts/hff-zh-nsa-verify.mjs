/**
 * WO-O4O-HFF-ZH-NUMBER-STRUCTURE-AMBIGUOUS-101-REPAIR-AND-APPLY-V1  §8 독립검증 (read-only)
 *
 * Apply 산출물이 아니라 **DB 현재 상태**만 읽어 재판정한다.
 *   - 대상 합계 101 / KO 해시 drift 0 / ProductMaster 변경 0 / ZH 증가량 일치
 *   - canonical 중복 0 / 수치·단위 drift 0 / 슬롯 한국어 0 / raw HTML·marker 0
 *   - Batch 밖 write 0 (이번 패스 삽입 id 가 전부 대상 master 에 속하는지)
 *   - 큐 누락·중복 0 (직전 큐 420 = 이번 해소 101 + 잔여 319)
 */
...
const TARGETS = JSON.parse(fs.readFileSync(`${D}/hff-zh-nsa-safe-targets-v1.json`, 'utf8')).targets;
const APPLY = JSON.parse(fs.readFileSync(`${D}/hff-zh-nsa-apply-result-v1.json`, 'utf8'));
const IDS = new Set(TARGETS.map((t) => t.productMasterId));
const BASE_QUEUE = rdl('hff-zh-deferred-issue-queue-through-nsa-base-v1.jsonl');
const NOW_QUEUE = rdl('hff-zh-deferred-issue-queue-through-final-v1.jsonl');
...
/* Apply 기록과 같은 정의로 센다 — HFF ProductMaster 총수(설명서 유무와 무관). */
...
/* Batch 밖 write 0 — 이번 삽입 id 는 전부 대상 master 에 속해야 한다. */
...
out.verdict = (out.zhRowsFound === 101 && !contract && !koDrift && !hangulDoc && !numDrift && !rawHtml
  && !markerLeft && !emptySection && !dup && !outside && out.zhDelta === 101 && out.koUnchanged
  && out.enUnchanged && out.productMasterUnchanged && out.queueAccounting && !out.queueDup
  && !out.queueStillHasTarget) ? 'PASS' : 'FAIL';
fs.writeFileSync(`${D}/hff-zh-nsa-verify-v1.json`, JSON.stringify(out, null, 1));
