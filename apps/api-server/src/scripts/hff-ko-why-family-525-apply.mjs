 ...
 * 이중 게이트: --apply + HFF_WF525_APPLY_CONFIRM=YES
 */
...
const CONFIRM = process.env.HFF_WF525_APPLY_CONFIRM === 'YES';
const D = 'apps/api-server/src/scripts/data';
const RB = `${D}/hff-ko-why-family-525-rollback-manifest-v1.json`;
const OUT = `${D}/hff-ko-why-family-525-apply-results-v1.json`;
const sha = (s) => crypto.createHash('sha256').update(s ?? '').digest('hex');
const HAS_FN = `content ~ '<h2>[^<]*기능성[^<]*</h2>'`;

const rb = JSON.parse(fs.readFileSync(RB, 'utf8'));
const targets = rb.targets ?? [];
const render = JSON.parse(fs.readFileSync(`${D}/hff-ko-why-family-525-render-audit-v1.json`, 'utf8'));
...
if (!CONFIRM) throw new Error('APPLY_BLOCKED: HFF_WF525_APPLY_CONFIRM=YES 필요');
