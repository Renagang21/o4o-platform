/**
 * WO-O4O-HFF-JA-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §7
 *
 * Batch 02 JA canonical INSERT.
 *   - INSERT 전용. KO canonical / EN / ZH / ProductMaster / candidate 는 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_JA_B02_APPLY_CONFIRM=YES.
...
const APPLY = process.argv.includes('--apply') && process.env.HFF_JA_B02_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-ja-b02-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-ja-b02-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
