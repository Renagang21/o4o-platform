...
/* 조사 겹침 — 서로 다른 규칙이 같은 자리에 조사를 붙였다는 신호.
   `がを` 는 뺐다: `けがを`(けが=負傷) 처럼 **が 로 끝나는 명사 + を** 가 정상 일본어에 흔하다.
   실측된 진짜 사고는 `をが`(`内容液をが…`) 와 `はを`(`またはを水…`) 이므로 그 짝은 남긴다. */
export const PARTICLE_DUP = /をが|をを|がが|をは|はを|にを/;
/* 같은 한자 즉시 반복 — 다만 **수식어 계열에 한정**한다.
   일반 한자의 중첩은 정상 일본어에 흔하다: `血中中性脂質`(中中) · `構成成分`(成成) · `直接接触`(接接).
   실측된 진짜 사고는 색·정도 수식어가 두 번 붙는 형태였다(`濃濃褐色` `黄黄緑色`). */
export const KANJI_DUP = /([濃淡暗明濁微鮮黄白赤黒緑青紫褐灰桃橙])\1/;
/** 종결·조사 중복. */
export const END_DUP = /。。|のの/;
/* 기능성 경계 소실 — 구분 기호 없이 `役立つ` 가 두 번 이어지면 서로 다른 기능성이 붙은 것이다.
   열거 번호(`2.` `(3)` `3)`)도 경계다 — 원문이 번호로 항목을 나눈 것을 사고로 보면 안 된다. */
export const CLAIM_RUN = /役立つ(?:(?!\d\s*[.)]|[。、・：:])[\s\S])*役立つ/;
