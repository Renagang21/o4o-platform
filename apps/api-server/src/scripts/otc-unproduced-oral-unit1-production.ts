/**
 * WO-O4O-OTC-UNPRODUCED-ORAL-UNIT1-FINAL-PRODUCTION-V1 — 경구 미생산 Unit 1 생산 실행기
 *
 * 단일 DB write-owner: **agent-da**. Unit 1 전체(373 fp / 1,850 master)를 하나의 승인 생산 단위로 실행한다.
 *
 * ── 입력 계약 ────────────────────────────────────────────────────────────────────
 * 생산 입력은 **Unit 1 승인 SSOT 하나뿐**이다 (`8328047ac`).
 * Unit 2 SSOT 는 읽지 않는다 — 파일 경로 자체를 참조하지 않으며, Unit 2 master 혼입 여부는
 * 실행 순서 원장의 선언 수치로만 교차 확인한다.
 *
 * ── 지문 계약 ────────────────────────────────────────────────────────────────────
 ...
 *   tsx src/scripts/otc-unproduced-oral-unit1-production.ts --mode=dry-run
 *   tsx ... --rollback-test                              # 강제 실패 주입 → 전량 rollback 확인
 *   OTC_ORAL_U1_KO_CONFIRM=YES tsx ... --mode=apply --lang=ko --apply
 *   OTC_ORAL_U1_EN_CONFIRM=YES tsx ... --mode=apply --lang=en --apply
 *   tsx ... --verify
 */
...
const EN_PATH = path.join(DATA_DIR, 'otc-unproduced-oral-unit1-en.json');
const RUN_LEDGER = path.join(DATA_DIR, 'otc-unproduced-oral-unit1-apply-order.json');
const WO = 'WO-O4O-OTC-UNPRODUCED-ORAL-UNIT1-FINAL-PRODUCTION-V1';
const UNIT = 'oral-unit-1';
const AUTHORED_SOURCE = 'mfds_drug_otc';
const AUTHORED_SOURCES = ['mfds_drug_otc', 'mfds_drug_otc_nutrition_combo', 'nutrition_combo'];
const EXPECTED = { fp: 373, master: 1850, ko: 7400, en: 3700, write: 11100 };
...
function gatesOf(states: GState[], allIds: string[], canonicalDup: number, extra: any, stage: 'ko' | 'en'): Record<string, boolean> {
  ...
  return {
    'G1 SSOT status=APPROVED_FOR_PRODUCTION': j.status === 'APPROVED_FOR_PRODUCTION',
    'G2 총계 373fp/1,850master': states.length === EXPECTED.fp && allIds.length === EXPECTED.master,
    'G3 fp 재현 100%': bad === 0 && fpOk === EXPECTED.master,
    'G4 master 누락·중복 0': allIds.length === states.reduce((t, x) => t + x.g.size, 0),
    ...
    'G10 Unit 2 대상 혼입 0': u2 ? (allIds.length === EXPECTED.master && states.length === EXPECTED.fp) : false,
    'G11 canonicalDup 0': canonicalDup === 0,
    'G12 예상 write': stage === 'ko'
      ? (eligM === EXPECTED.master && eligM * 4 === EXPECTED.ko)
      : (eligM === EXPECTED.master && eligM * 2 === EXPECTED.en),
    'G13 이상 그룹 0': !states.some((x) => x.anomalies.length),
  };
}
...
async function runApply(): Promise<void> {
  ...
  const confirmEnv = lang === 'ko' ? 'OTC_ORAL_U1_KO_CONFIRM' : 'OTC_ORAL_U1_EN_CONFIRM';
  const ds = await connect();
  ...
  const runPath = path.join(DATA_DIR, `otc-unproduced-oral-unit1-apply-run.${lang}.json`);
...
async function verify(): Promise<void> {
  ...
  const checks: Record<string, boolean> = {
    '대상 master 1,850': v.targetMasters === EXPECTED.master,
    'KO authored canonical 1,850': v.koAuthoredCanonical === EXPECTED.master,
    'easy_drug deprecated 1,850': v.easyDeprecated === EXPECTED.master,
    'easy_drug canonical 잔존 0': v.easyStillCanonical === 0,
    'audit 1,850': v.auditKo === EXPECTED.master,
    'needs_review 0': v.needsReviewLeft === 0,
    'canonicalDup 0': v.canonicalDup === 0,
    'sourceRef leak 0': v.sourceRefLeak === 0,
    'EN 한글 0': v.enHangul === 0,
    'EN canonical 1,850': enDone,
  };
  console.log(`UNIT1 INDEPENDENT VERIFY — ${JSON.stringify(v)}`);
  for (const [k, ok] of Object.entries(checks)) console.log(`  ${ok ? 'PASS' : '*** FAIL ***'}  ${k}`);
  const outPath = path.join(DATA_DIR, 'otc-unproduced-oral-unit1-verify.json');
