/**
 * WO-O4O-HFF-JA-BATCH-01-10000-DIRECT-BULK-PRODUCTION-V1  §8
 *
 * Batch 01 JA canonical INSERT (10,000).
 *   - INSERT 전용. KO canonical / EN / ZH / ProductMaster / candidate 는 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_JA_B01_APPLY_CONFIRM=YES.
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-ja-b01-render-audit-v1.json`, 'utf8'));
const TARGETS = JSON.parse(fs.readFileSync(`${D}/hff-ja-b01-safe-targets-v1.json`, 'utf8')).targets;
...
const APPLY = process.argv.includes('--apply') && process.env.HFF_JA_B01_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-ja-b01-rollback-v1.json`, JSON.stringify({
...
const out = {
  ...
  expectedEqualsActual: inserted.length === TARGETS.length && (after.ja_canon - before.ja_canon) === inserted.length,
  skippedSample: skipped.slice(0, 20), failed,
};
fs.writeFileSync(`${D}/hff-ja-b01-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
