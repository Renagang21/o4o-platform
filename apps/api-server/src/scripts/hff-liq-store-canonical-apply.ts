/**
 * HFF 유산균 **액상** 6건 — STORE 설명서 canonical 적재 (dry-run 기본 · --apply env 이중게이트)
 *
 * WO-O4O-HFF-PROBIOTICS-LIQUID-LOAD-PREP-V1 런북 §3.2 액상 델타 반영.
 * 계약: hff-b3-store-canonical-apply.ts(고형 226) 그대로 재사용 — 아래 델타만 변경.
 *   TARGET 6 · loadTargets=hff-probiotics-liq-cp01.json · env게이트=HFF_LIQ_CANONICAL_APPLY_CONFIRM
 *   tags=batch:probiotics-liquid-pilot · 롤백=hff-liq-apply-rollback-manifest.json
 *   가드A grounding 완전성 = **liquidGrounding**(cfu/serving/cfuBasis PARSED) — solid declaredAmount 아님.
 *
 ...
 *   regulatory_type='건강기능식품' · barcode NULL(무바코드) · mfds_permit_number=STTEMNT_NO
 *
 * 배치 쿼리: preload 2 SELECT + apply 3 bulk(unnest). 단일 트랜잭션.
 * 접속: 로컬은 Cloud SQL Auth Proxy(127.0.0.1:PROXY_PORT)만.
 *
 * Usage:
 *   dry-run: PROXY_PORT=5442 npx tsx src/scripts/hff-liq-store-canonical-apply.ts
 *   apply  : HFF_LIQ_CANONICAL_APPLY_CONFIRM=YES PROXY_PORT=5442 npx tsx ... --apply
 */
...
const CONFIRM = process.env.HFF_LIQ_CANONICAL_APPLY_CONFIRM === 'YES';
...
function loadTargets(): GuardProductInput[] {
  return JSON.parse(fs.readFileSync(path.join(DATA, DATA_FILE), 'utf8'));
}

async function main(): Promise<void> {
  if (APPLY && !CONFIRM) throw new Error('APPLY_BLOCKED: --apply 는 HFF_LIQ_CANONICAL_APPLY_CONFIRM=YES 필요');
  fs.mkdirSync(SP, { recursive: true });

  const items = loadTargets();
  if (items.length !== TARGET) throw new Error(`대상 수 불일치: ${items.length} (기대 ${TARGET})`);

  // ── 가드 A: 파일 레벨 재검증 (최신 Guard 1.2.0 — liquidGrounding → 자동 액상 경로) ──
  ...
  for (const it of items) {
    ...
    // ★ 액상 델타: solid declaredCfu/serving/declaredAmount 가 아니라 liquidGrounding 완전성으로 판정
    const lg = it.liquidGrounding;
    if (!lg || lg.cfu?.state !== 'PARSED' || !lg.serving || lg.cfuBasis?.state !== 'PARSED') missGround++;
  }
  if (blocked || missDraft || missGround) throw new Error(`GUARD_FAIL blocked=${blocked} draft=${missDraft} ground=${missGround}`);
  console.log(`[가드A] 파일 ${TARGET}: BLOCKED ${blocked} · REVIEW ${review} · draft결손 ${missDraft} · liquidGrounding결손 ${missGround} · REVIEW rule ${JSON.stringify(reviewRules)}`);
  ...
  try {
    ...
    const report: any = { mode: APPLY ? 'apply' : 'dry-run', target: TARGET, guard: { blocked, review, reviewRules, missDraft, missGround, candMatch: cands.length, masterDup: dup.length }, planned: { ...planned, totalWrites: planned.masters + planned.candidateLinks + planned.spdKo + planned.spdEn }, sampleSanitize: sample };

    if (!APPLY) {
      fs.writeFileSync(`${SP}/hff-liq-apply-dryrun-plan.json`, JSON.stringify({ planned, sample, outcomes }, null, 2));
    ...
    const tags = JSON.stringify(['import:mfds-hff', 'batch:probiotics-liquid-pilot', 'wo:hff-store-canonical']);
    await qr.query(
    ...
    fs.writeFileSync(`${SP}/hff-liq-apply-rollback-manifest.json`, JSON.stringify({ createdMasters: ids, createdSpd: spdIds, candIds, snapshot, outcomes }, null, 2));
    await qr.commitTransaction();
    console.log('COMMIT 완료 · 롤백 매니페스트 → scratchpad/hff-liq-apply-rollback-manifest.json');
    console.log('JSON_REPORT_BEGIN'); console.log(JSON.stringify(report, null, 2)); console.log('JSON_REPORT_END');
  } catch (e) {
...
main().catch((e) => { console.error('[hff-liq-store-canonical-apply] FAILED:', e instanceof Error ? e.message : e); process.exit(1); });
