/**
 * WO-O4O-HFF-ZH-BATCH-04-10000-DIRECT-BULK-PRODUCTION-V1  §7
 *
 * Batch 04 ZH canonical INSERT (10,000).
 *   - INSERT 전용. KO canonical / EN / 다른 언어 / ProductMaster / candidate 는 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_ZH_B04_APPLY_CONFIRM=YES.
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-zh-b04-render-audit-v1.json`, 'utf8'));
const TARGETS = JSON.parse(fs.readFileSync(`${D}/hff-zh-b04-safe-targets-v1.json`, 'utf8')).targets;
...
const APPLY = process.argv.includes('--apply') && process.env.HFF_ZH_B04_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-zh-b04-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-zh-b04-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
