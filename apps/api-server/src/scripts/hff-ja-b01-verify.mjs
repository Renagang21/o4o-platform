/**
 * WO-O4O-HFF-JA-BATCH-01-10000-DIRECT-BULK-PRODUCTION-V1  §8 독립검증
 *
...
const APPLIED = JSON.parse(fs.readFileSync(`${D}/hff-ja-b01-apply-result-v1.json`, 'utf8'));
const QUEUE = fs.existsSync(`${D}/hff-ja-deferred-issue-queue-v1.jsonl`)
  ? fs.readFileSync(`${D}/hff-ja-deferred-issue-queue-v1.jsonl`, 'utf8').split('\n').filter(Boolean).map((l) => JSON.parse(l))
  : [];
...
for (const r of rows) {
  ...
  /* ZH 를 경유하지 않았다는 것을 산출물로 재확인한다 — 일본어 본문에 중국어 간체 전용 자형이 있으면 안 된다(§1). */
...
out.verdict = (out.batchTotalIs10000 && !contract && !contentMismatch && !hangulDoc && !simplifiedDoc && !numDrift && !notInBatch
  && !koDrift && out.koUnchanged && out.enUnchanged && out.zhUnchanged && out.pmUnchanged && out.jaDeltaMatchesInserted
  && !dup && !outside && !out.issueQueueDup && !out.issueQueueMissingField && !out.issueQueueBadType) ? 'PASS' : 'FAIL';
fs.writeFileSync(`${D}/hff-ja-b01-verify-v1.json`, JSON.stringify(out, null, 1));
