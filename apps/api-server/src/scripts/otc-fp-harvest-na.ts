...
function bucket(sec: Record<string, string>) { let ind = '', dos = '', cau = ''; for (const [t, b] of Object.entries(sec)) { if (/효능|효과|적응|용도/.test(t)) ind += b; else if (/용법|용량|복용|투여\s*방법|사용\s*방법|사용법/.test(t)) dos += b; else if (/상호\s*작용|병용/.test(t)) { } else if (/주의|경고|금기|부작용|이상\s*반응|임부|임신|수유/.test(t)) cau += b; } return { ind, dos, cau }; }
