/**
 * WO-O4O-HFF-ZH-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §7
 *
 * Batch 01 ZH canonical INSERT (10,000).
 *   - INSERT 전용. KO canonical / EN / 다른 언어 / ProductMaster / candidate 는 쓰지 않는다.
 *   - 이중 게이트: `--apply` + HFF_ZH_B02_APPLY_CONFIRM=YES.
 *   - 렌더 감사 verdict 가 PASS 가 아니면 실행하지 않는다.
 *   - rollback manifest 는 apply 전에 기록한다.
 *   - master 별 ZH 중복 가드 + KO hash 재확인(낙관적 잠금)을 트랜잭션 안에서 수행한다.
 */
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-zh-b02-render-audit-v1.json`, 'utf8'));
const TARGETS = JSON.parse(fs.readFileSync(`${D}/hff-zh-b02-safe-targets-v1.json`, 'utf8')).targets;
if (RENDER.verdict !== 'PASS') { console.error('RENDER_NOT_PASS'); process.exit(1); }
if (TARGETS.length !== 10000) { console.error(`TARGETS_NOT_10000:${TARGETS.length}`); process.exit(1); }

const APPLY = process.argv.includes('--apply') && process.env.HFF_ZH_B02_APPLY_CONFIRM === 'YES';
const c = new pg.Client({ host: '127.0.0.1', port: parseInt(process.env.PROXY_PORT ?? '5641', 10), user: 'o4o_api', password: process.env.PGPW, database: 'o4o_platform', statement_timeout: 900000 });
await c.connect();
...
fs.writeFileSync(`${D}/hff-zh-b02-rollback-v1.json`, JSON.stringify({
...
/* Batch 밖 write 검사 기준 시각 — 삽입 시작 전이어야 이 작업 구간 전체를 덮는다(§8). */
    ...
    for (const t of chunk) {
      ...
      const dup = await c.query(`
        SELECT 1 FROM shared_product_descriptions
         WHERE master_id=$1 AND description_type='STORE' AND status='canonical' AND language='zh' AND deleted_at IS NULL`,
      [t.productMasterId]);
      if (dup.rows.length) { skipped.push({ m: t.productMasterId, why: 'ZH_EXISTS' }); continue; }
      const r = await c.query(`
        ...
        VALUES ($1,'STORE','canonical','zh','o4o_hff_generated',$2, now(), now()) RETURNING id`,
...
const out = {
  wo: RENDER.wo, startedAt, appliedAt: new Date().toISOString(),
  expectedInsert: TARGETS.length,
  targets: TARGETS.length, inserted: inserted.length, skipped: skipped.length, failedShards: failed.length,
  before, after,
  ...
  zhDelta: after.zh_canon - before.zh_canon,
...
fs.writeFileSync(`${D}/hff-zh-b02-apply-result-v1.json`, JSON.stringify({ ...out, insertedIds: inserted.map((x) => x.id) }, null, 1));
