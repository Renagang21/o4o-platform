/**
 * WO-O4O-HFF-JA-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §7 공용 엔진·자산 변경 회귀검증
 *
 * Batch 02 를 위해 엔진·사전을 손대면 **이미 저장된 JA 10,000 건**이 영향을 받을 수 있다.
 * DB read-only 로 다음 셋만 본다.
 *   - 저장된 기존 canonical 수정 0  : 저장 본문 해시 == Batch 01 렌더 통과 본문 해시
...
const B01 = JSON.parse(fs.readFileSync(`${D}/hff-ja-b01-safe-targets-v1.json`, 'utf8')).targets;
...
const out = {
  ...
  scope: 'batch01 stored JA canonical', documents: koRows.length, storedFound: stored.size,
...
fs.writeFileSync(`${D}/hff-ja-b02-regression-v1.json`, JSON.stringify(out, null, 1));
