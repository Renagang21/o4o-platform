// GA-V6 re-plumb of otc-combo-ko-compose.ga.mjs — THIS-MACHINE paths only. Medical/compose logic byte-identical.
...
//   ENV: PROD=<prodscreen json> OUTCFG=<config path> FPS=<csv> AUDIT_DB_PORT=5442
import { readFileSync, writeFileSync } from 'node:fs';
const pw = readFileSync('C:/Users/home/coding/o4o-platform/apps/api-server/.env','utf8').match(/^DB_PASSWORD=(.*)$/m)[1].trim();
const PROD = process.env.PROD || 'C:/Users/home/AppData/Local/Temp/claude/c--Users-home-coding-o4o-platform/2b3a47a5-e7ba-4a0a-a5c1-fabefbef425a/scratchpad/prodscreen-v6.json';
const OUTCFG = process.env.OUTCFG || 'src/scripts/data/otc-oral-combo-leaflet-config-batch12.ga.json';
...
const ds = new DataSource({ type:'postgres', host:'127.0.0.1', port:parseInt(process.env.AUDIT_DB_PORT||'5442',10), username:'o4o_api', password:pw, database:'o4o_platform', entities:[], synchronize:false, logging:['error'] });
await ds.initialize();
...
try { for (const r of picks) {
  ...
  if (!cj) { skip++; console.error('SKIP(no eff/use/cau)', r.fp, r.name); continue; }
...
writeFileSync(OUTCFG, JSON.stringify({ _doc:'batch12 (GA-V6) deterministic KO compose (faithful source restructure). EN authored separately.', groups }, null, 1),'utf8');
