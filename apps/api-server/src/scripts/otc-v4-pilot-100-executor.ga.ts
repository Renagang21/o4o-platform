 ...
 *   — pilot 100 제품별(master 단위) 독립 생산 실행기 (에이전트 가 · 단독 write owner)
 *
 ...
 *   --apply --confirm     : LIVE. 추가로 env OTC_V4_APPLY_PILOT_100=CONFIRM 필수(2중 게이트).
 *
 * 재실행 멱등: 이미 KO·EN authored canonical + V4 sourceRef 가 성립한 master 는 SKIP_ALREADY_COMPLETE (write 0).
 * 체크포인트: 10건마다 원장 flush — proxy 만료 시 마지막 체크포인트부터 재개(완료분 자동 skip).
 *
 * Usage(apps/api-server):
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-pilot-100-executor.ga.ts [--port 5495] [--rollback-test] [--apply --confirm] [--limit N] [--only <masterId>]
 */
import fs from 'node:fs';
import path from 'node:path';
import { Pool, PoolClient } from 'pg';
import { WO, BATCH_ID, DATA_DIR, md5, masterRefV4 } from './otc-v4-master-leaflet-contract.ga.js';
...
const KO_PAYLOAD = path.join(DATA_DIR, 'otc-v4-pilot-100-ko-payload.ga.json');
const EN_PAYLOAD = path.join(DATA_DIR, 'otc-v4-pilot-100-en-payload.ga.json');
const OUT_RESULT = path.join(DATA_DIR, 'otc-v4-pilot-100-result-ledger.ga.json');
const OUT_GREEN = path.join(DATA_DIR, 'otc-v4-pilot-100-green-ledger.ga.json');
const OUT_EXC = path.join(DATA_DIR, 'otc-v4-pilot-100-exception-handoff-na.ga.json');
const OUT_CKPT = path.join(DATA_DIR, 'otc-v4-pilot-100-checkpoint-ledger.ga.json');
...
function reproCmd(masterId: string): string {
  return `../../node_modules/.bin/tsx src/scripts/otc-v4-pilot-100-executor.ga.ts --only ${masterId} --rollback-test --port ${port()}`;
}
...
const RECOMMEND: Record<string, string> = {
  ...
  IDENTITY_CONFLICT: 'canonical 일반명코드 확정 후 재투입(품목기준코드 단일이면 per-master 생산 가능)',
...
async function runOne(pool: Pool, u: Unit): Promise<any> {
  ...
  // 1) 사전 예외(prep 재현) — 동일 실행기에 투입되나 DB write 0 으로 격리
  ...
  // 2) 재실행 멱등 — 이미 완료된 제품은 write 0 으로 skip
  const st = await liveState(pool, u);
  ...
  // 3) preflight — 기존 canonical / sourceRef 점유
  ...
  try {
    ...
    try {
      ...
      rep.exception = excRecord(u, classify(msg), msg.startsWith('EN') ? 'persist' : 'persist', msg);
    }
  } catch (e) {
  ...
  // 5) 제품별 독립 사후 확인(TX 밖 · 별도 커넥션) — GREEN 은 실제 반영, 실패는 residue 0
...
async function main(): Promise<void> {
  if (MODE === 'APPLY' && process.env.OTC_V4_APPLY_PILOT_100 !== 'CONFIRM') {
    console.error('LOCKED: confirm env(OTC_V4_APPLY_PILOT_100=CONFIRM) 미설정 — LIVE apply 금지'); process.exit(3);
  }
  ...
  try {
    for (let i = 0; i < units.length; i++) {
      ...
      if ((i + 1) % CHECKPOINT_EVERY === 0 || i === units.length - 1) {
        ...
        checkpoints.push({ checkpoint: checkpoints.length + 1, processed: done, lastMasterId: r.masterId,
...
function flush(results: any[], checkpoints: any[], target: number, started: string): any {
  ...
  fs.writeFileSync(OUT_RESULT, JSON.stringify({ wo: WO, kind: 'per-master-result-ledger', summary, results }, null, 2) + '\n', 'utf8');
  fs.writeFileSync(OUT_GREEN, JSON.stringify({
  ...
  fs.writeFileSync(OUT_EXC, JSON.stringify({
  ...
  fs.writeFileSync(OUT_CKPT, JSON.stringify({ wo: WO, kind: 'checkpoint-ledger', every: CHECKPOINT_EVERY, checkpoints }, null, 2) + '\n', 'utf8');
  return { summary };
}
