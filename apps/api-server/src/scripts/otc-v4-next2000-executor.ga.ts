 ...
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-next2000-executor.ga.ts [--port 5502] [--rollback-test] [--apply --confirm] [--limit N] [--only <masterId>]
 */
import fs from 'node:fs';
import path from 'node:path';
import { Pool, PoolClient } from 'pg';
import { DATA_DIR, md5, masterRefV4 } from './otc-v4-master-leaflet-contract.ga.js';
import { WO_500, BATCH_ID_500 } from './otc-v4-next2000-contract.ga.js';
...
const CONFIRM_ENV = 'OTC_V4_APPLY_NEXT2000';
...
const KO_PAYLOAD = path.join(DATA_DIR, 'otc-v4-next2000-ko-payload.ga.json');
const EN_PAYLOAD = path.join(DATA_DIR, 'otc-v4-next2000-en-payload.ga.json');
const OUT_RESULT = path.join(DATA_DIR, 'otc-v4-next2000-result-ledger.ga.json');
const OUT_GREEN = path.join(DATA_DIR, 'otc-v4-next2000-green-ledger.ga.json');
const OUT_EXC = path.join(DATA_DIR, 'otc-v4-next2000-exception-handoff-na.ga.json');
const OUT_CKPT = path.join(DATA_DIR, 'otc-v4-next2000-checkpoint-ledger.ga.json');
...
function reproCmd(masterId: string): string {
  return `../../node_modules/.bin/tsx src/scripts/otc-v4-next2000-executor.ga.ts --only ${masterId} --rollback-test --port ${port()}`;
}
