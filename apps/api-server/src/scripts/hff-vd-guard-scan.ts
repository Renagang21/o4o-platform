/**
 * 비타민 D 파일럿 20건 — 최신 Guard 전수 검사 (read-only)
 *   npx tsx src/scripts/hff-vd-guard-scan.ts
 * 물 규칙(G-WATER)·연령(F-KIDS)·기능성 강화(H) 포함 최신 가드로 판정. DB write 0.
 */
...
const F = 'C:/Users/sohae/o4o-platform/docs/checks/data/product-description-guard/hff-vitamin-d-20.json';
...
for (const it of items) {
  ...
  if (risk.length) {
    flagged.push(`${it.candidateId} | ${String(it.productName).slice(0, 24)} | ${r.overallStatus}`);
    for (const f of risk.slice(0, 5)) flagged.push(`     ${f.status} ${f.ruleId} [${f.language ?? '-'}] "${String(f.matchedText ?? '').slice(0, 44)}"`);
  }
}
console.log(`비타민 D 20건 최신 Guard 검사\n`);
