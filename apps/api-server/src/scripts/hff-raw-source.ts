...
export async function* dbCandidateSource(port: number, username?: string, password?: string, database?: string): AsyncGenerator<HffRawItem> {
  const ds = new DataSource({ type: 'postgres', host: '127.0.0.1', port, username, password, database, entities: [], synchronize: false, logging: ['error'], ssl: false, extra: { max: 2 } });
  await ds.initialize();
  try {
    // 스트리밍: keyset 페이지네이션(id 순) — 대량 안전
    let after = '00000000-0000-0000-0000-000000000000';
    for (;;) {
      const rows: Array<{ id: string; src: HffRawItem }> = await ds.query(
 ...
 */
export function resolveSource(argv: string[], env: NodeJS.ProcessEnv): { kind: 'db' | 'file'; gen: AsyncGenerator<HffRawItem>; label: string } {
  ...
  if (kind === 'db') {
    ...
    return { kind, gen: dbCandidateSource(port, env.DB_USERNAME, env.DB_PASSWORD, env.DB_NAME), label: `db:raw_payload@${port}` };
  }
