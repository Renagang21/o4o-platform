 ...
 *   node ... --apply  +  HFF_EN_B05_APPLY_CONFIRM=YES
 */
import fs from 'node:fs';
import pg from 'pg';

const D = 'apps/api-server/src/scripts/data';
const SAFE = JSON.parse(fs.readFileSync(`${D}/hff-en-batch05-safe-targets-v1.json`, 'utf8')).targets;
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-en-batch05-render-audit-v1.json`, 'utf8'));
const APPLY = process.argv.includes('--apply') && process.env.HFF_EN_B05_APPLY_CONFIRM === 'YES';
if (RENDER.verdict !== 'PASS') { console.error('RENDER_NOT_PASS'); process.exit(1); }

const c = new pg.Client({ host: '127.0.0.1', port: parseInt(process.env.PROXY_PORT ?? '5621', 10), user: process.env.PGUSER, password: process.env.PGPW, database: 'o4o_platform', ssl: false });
await c.connect();
...
fs.writeFileSync(`${D}/hff-en-batch05-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-en-batch05-apply-results-v1.json`, JSON.stringify(out, null, 1));
fs.writeFileSync(`${D}/hff-en-batch05-safe-targets-v1.json`, JSON.stringify({ builtAt: new Date().toISOString(), count: SAFE.length, targets: SAFE }, null, 1));
