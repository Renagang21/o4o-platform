...
function resolveAtomRaw(t0, depth) {
  ...
  /* KO 저작기가 만든 인트로 템플릿. 원문이 기계 생성한 **고정 문형**이라 대응도 고정이다.
     제품명은 원문 표기를 유지하고(제품명 계약) 수치는 원문 토큰을 그대로 옮긴다.
     범용 문장 분해가 아니라 문형 전체를 한 덩어리로 확정하는 규칙이다. */
  {
    const gi = GEN_INTRO_RG.exec(t0);
    if (gi) {
      const amount = /[가-힣]/.test(gi[3]) ? resolveAtom(gi[3].trim(), depth + 1) : gi[3].trim();
      if (amount) return `${gi[1].trim()}は、紅参の指標成分であるジンセノサイド（Rg1・Rb1・Rg3）を${amount}配合し、以下の食品医薬品安全処が認定した機能性で毎日の健康管理を助けます。`;
    }
  }
...
/* KO 저작기 인트로 템플릿(홍삼). 제품명·수치만 다르고 나머지는 고정 문형이다. */
const GEN_INTRO_RG = /^(.*?)(?:은|는)\s*홍삼의 지표성분 진세노사이드\(Rg1[·․]Rb1[·․]Rg3\)를\s*(.*?)\s*(.+?)\s*담아,\s*아래 식약처 인정 기능성으로 매일의 건강관리를 돕습니다\.?$/;
...
function specValueCore(t) {
  ...
  /* 최종 제품 표시량 X의 N~M%  — 괄호 없이 수치·단위가 이어지는 표기.
     위 문형들과 같은 계열이며 X 에 한글이 없을 때만 처리한다. */
  if ((m = /^최종\s*제품\s*표시량\s*([^가-힣]+?)\s*의\s*([\d.,]+)\s*(%?)\s*[~〜～]\s*([\d.,]+)\s*%$/.exec(t)))
    return `最終製品表示量${m[1]}の${m[2]}${m[3]}〜${m[4]}%`;
  /* 표시량 X의 N~M%  — 괄호 없는 형태 */
  if ((m = /^표시량\s*([^가-힣()（）]+?)\s*의\s*([\d.,]+)\s*(%?)\s*[~〜～]\s*([\d.,]+)\s*%$/.exec(t)))
    return `表示量${m[1]}の${m[2]}${m[3]}〜${m[4]}%`;
...
function tailNorm(s) {
  return s
    ...
    .replace(/のの(?=[^の])/g, 'の')
    /* 색 어근이 겹쳐 붙는 경우(`노란 연두색` → `黄`+`黄緑` = `黄黄緑`). 일본어에 `黄黄緑` 표기는 없다.
       수식(`노란`)을 버리지 않고 `黄みがかった黄緑` 로 편다 — 색 이름을 바꾸지 않는 표기 정리다.
       `のの` 축약과 같은 계열의 마무리 규칙이며, 겹치지 않은 색은 건드리지 않는다. */
    .replace(/黄黄緑/g, '黄みがかった黄緑')
    .replace(/([濃淡暗明濁微鮮])\1/g, '$1');
}
