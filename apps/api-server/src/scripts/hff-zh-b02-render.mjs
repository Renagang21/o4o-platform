/**
 * WO-O4O-HFF-ZH-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §3 / §7
 *
 ...
 *
 * 산출: data/hff-zh-b02-safe-targets-v1.json · data/hff-zh-b02-render-audit-v1.json
 *       data/hff-zh-deferred-issue-queue-through-batch02-v1.jsonl (+ -summary-v1.json)
 */
...
const WO = 'WO-O4O-HFF-ZH-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1';
const TARGET = 10000;
const sha = (s) => crypto.createHash('sha256').update(s ?? '').digest('hex');

/* ── KO canonical 모집단 (read-only) ──────────────────────────── */
const c = new pg.Client({ host: '127.0.0.1', port: parseInt(process.env.PROXY_PORT ?? '5631', 10), user: 'o4o_api', password: process.env.PGPW, database: 'o4o_platform', statement_timeout: 900000 });
await c.connect();
...
/* Batch 01 문제 큐와 KO 영구 HOLD 는 이번 모집단에서 제외한다(§3). */
const PRIOR_QUEUE = rdl('hff-zh-deferred-issue-queue-through-batch01-v1.jsonl');
const EXCLUDE = new Set(PRIOR_QUEUE.map((q) => q.productMasterId).filter(Boolean));
...
for (const row of pool) {
  if (!row.content || !/<h2|<li/.test(row.content)) {
    issues.push({ batch: 'batch02', productMasterId: row.master_id, koCanonicalId: row.id, productName: row.product_name,
...
fs.writeFileSync(`${CACHE}/hff-zh-b02-render-blockers.json`,
...
const TASKS = `${CACHE}/hff-zh-b02-render-tasks.jsonl`;
...
const runChunk = (i) => new Promise((resolve, reject) => {
  const out = `${CACHE}/hff-zh-b02-render-part-${i}.json`;
...
for (const i of chunks) fs.rmSync(`${CACHE}/hff-zh-b02-render-part-${i}.json`, { force: true });
...
fs.writeFileSync(`${D}/hff-zh-b02-safe-targets-v1.json`, JSON.stringify({ wo: WO, generatedAt: audit.renderedAt, count: targets.length, targets }, null, 1));
fs.writeFileSync(`${D}/hff-zh-b02-render-audit-v1.json`, JSON.stringify(audit, null, 1));
const mergedQueue = [...PRIOR_QUEUE, ...issues];
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-batch02-v1.jsonl`, mergedQueue.map((x) => JSON.stringify(x)).join('\n') + (mergedQueue.length ? '\n' : ''));
fs.writeFileSync(`${D}/hff-zh-deferred-issue-queue-through-batch02-summary-v1.json`, JSON.stringify({
  wo: WO, generatedAt: audit.renderedAt, total: mergedQueue.length, carriedForward: PRIOR_QUEUE.length, addedByBatch02: issues.length,
