/**
 * WO-O4O-HFF-ZH-BATCH-04-10000-DIRECT-BULK-PRODUCTION-V1  §8 독립검증
 *
 * apply 산출물을 신뢰하지 않고 **DB 현재 상태만** 읽어 재계산한다. read-only.
 *   - Batch 04 상태 합계 10,000
...
const APPLIED = JSON.parse(fs.readFileSync(`${D}/hff-zh-b04-apply-result-v1.json`, 'utf8'));
const QUEUE = fs.readFileSync(`${D}/hff-zh-deferred-issue-queue-through-batch04-v1.jsonl`, 'utf8')
...
fs.writeFileSync(`${D}/hff-zh-b04-verify-v1.json`, JSON.stringify(out, null, 1));
