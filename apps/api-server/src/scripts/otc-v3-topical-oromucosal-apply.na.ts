 ...
 *   --apply --confirm    : LIVE(commit). 추가로 per-route confirm env 필수 — 이 WO 에서는 미설정 → LOCKED. EN 은 KO canonical 선행.
 *
...
async function main(): Promise<void> {
  ...
  // ── LIVE apply 이중 게이트 + per-route confirm env (이 WO 에서는 LOCKED) ──
  if (mode === 'APPLY') {
