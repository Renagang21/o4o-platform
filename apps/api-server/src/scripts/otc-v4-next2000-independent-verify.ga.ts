 ...
 *
 * ⚠️ 독립성 계약: 실행기(otc-v4-next2000-executor.ga.ts)의 어떤 함수도 import 하지 않는다.
 ...
 *
 * 실행:
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-next2000-independent-verify.ga.ts --baseline --port 5502
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-next2000-independent-verify.ga.ts --verify   --port 5502
 */
...
const LEDGER_500 = P('otc-v4-next2000-selection-ledger.ga.json');
...
const RESULT = P('otc-v4-next2000-result-ledger.ga.json');
const SOURCE = P('otc-v4-next2000-source.ga.json');
const OUT_BASELINE = P('otc-v4-next2000-verify-baseline.ga.json');
const OUT_VERIFY = P('otc-v4-next2000-independent-verification.ga.json');
...
async function snapshotBaseline(pool: Pool): Promise<void> {
  ...
  // 2단계는 pilot 100 GREEN 80 **과 pilot 500 GREEN 416** 을 함께 불변 대상으로 잡는다.
...
async function verify(pool: Pool): Promise<void> {
  ...
  G('IV-01', '모집단 ↔ 결과 원장 일치', `${ledgerIds.length}건 동일 집합`, `결과 ${resIds.length}건`,
    ledgerIds.length === resIds.length && ledgerIds.every((v, i) => v === resIds[i]));
  ...
  G('IV-03', 'GREEN+EXCEPTION+SKIP = 입력 수', String(ledgerIds.length), `${green.length}+${exc.length}+${skip.length}=${green.length + exc.length + skip.length}`,
    green.length + exc.length + skip.length === ledgerIds.length);
  ...
  const residueAudit = excIds.length ? (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-next2000'`, [excIds])).rows[0].n : 0;
  ...
  const outsideAudit = (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE (metadata->>'batchId')='otc-v4-next2000' AND master_id <> ALL($1::uuid[])`, [ledgerIds])).rows[0].n;
  ...
  const auditPerGreen = greenIds.length ? (await pool.query(
    `SELECT master_id::text mid, count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-next2000'
  ...
  if (base) {
    ...
    const nowRows = base.pilot100GreenIds ? (await pool.query(
      `SELECT master_id::text mid, COALESCE(language,'ko') lang, md5(content) h
         FROM shared_product_descriptions
        WHERE master_id = ANY($1::uuid[]) AND description_type='STORE' AND status='canonical' AND deleted_at IS NULL
        ORDER BY 1,2`, [[...readJson(GREEN_100).rows.map((r: any) => r.masterId), ...readJson(GREEN_500).rows.map((r: any) => r.masterId)]])).rows : [];
    ...
    G('IV-20', '선행 GREEN(pilot100 80 + pilot500 416) 불변', '변경 0', `변경 ${changed.length} / 소실 ${removed.length}`,
    ...
    G('IV-20', '선행 GREEN(pilot100 80 + pilot500 416) 불변', 'baseline 필요', 'baseline 파일 없음', false);
    G('IV-21', '대상 밖 LIVE canonical 총량 불변', 'baseline 필요', 'baseline 파일 없음', false);
  }

  // I. pilot 100 예외 20 write 0
  const exc100 = fs.existsSync(EXC_100) ? readJson(EXC_100).rows.map((r: any) => r.masterId) : [];
  const exc100Write = exc100.length ? (await pool.query(
    `SELECT count(*)::int n FROM shared_product_descriptions
      WHERE master_id = ANY($1::uuid[]) AND description_type='STORE' AND source_type=$2 AND deleted_at IS NULL`,
    [exc100, AUTHORED_V4])).rows[0].n : 0;
  G('IV-22', 'pilot 100 예외 20 write 0', '0', String(exc100Write), exc100Write === 0);

  // J. 선행 배치 교집합 0 — 2단계는 pilot 100 과 pilot 500 **양쪽** 모두와 겹치지 않아야 한다.
  const prior = new Set<string>([
    ...readJson(LEDGER_100).masters.map((m: any) => m.masterId),
    ...readJson(LEDGER_P500).masters.map((m: any) => m.masterId),
  ]);
  const inter = ledgerIds.filter((id) => prior.has(id));
  G('IV-23', '선행 배치(pilot100 100 + pilot500 500) 교집합 0', '0', String(inter.length), inter.length === 0);
  ...
  const out = {
    ...
    kind: 'independent-verification', batchId: 'otc-v4-next2000', verifiedAt: new Date().toISOString(),
