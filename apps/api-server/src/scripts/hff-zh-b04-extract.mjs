/**
 * WO-O4O-HFF-ZH-BATCH-04-10000-DIRECT-BULK-PRODUCTION-V1  §3
 *
 * Batch 04 모집단의 KO canonical 슬롯 문구 추출 (read-only).
 ...
 *
 * 산출: data/hff-zh-b04-phrases-v1.json (저장소) · <CACHE>/hff-zh-b04-docslots.jsonl (캐시)
 */
...
const priorQueue = rdl('hff-zh-deferred-issue-queue-through-batch03-v1.jsonl');
...
fs.writeFileSync(`${CACHE}/hff-zh-b04-docslots.jsonl`, lines.join('\n'));

const byKind = {};
for (const p of phrases) byKind[p.kind] = (byKind[p.kind] ?? 0) + 1;
const sorted = [...phrases].sort((a, b) => b.docFreq - a.docFreq);
fs.writeFileSync(`${D}/hff-zh-b04-phrases-v1.json`, JSON.stringify({
