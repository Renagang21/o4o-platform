/**
 * WO-O4O-HFF-ZH-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §4
 * 실제 render 와 **같은 build()** 로 측정한다 (offline, DB 미접근).
 * 입력: .cache/hff-zh-b02-pool.jsonl (render 가 read-only 로 덤프한 KO 모집단)
 */
...
fs.writeFileSync(`${CACHE}/hff-zh-b02-atoms.json`, JSON.stringify(atoms, null, 1));
const next = [...tally.entries()].sort((a, b) => b[1] - a[1]).slice(0, 400).map(([k, n]) => ({ n, k }));
fs.writeFileSync(`${CACHE}/hff-zh-b02-render-blockers.json`, JSON.stringify(next, null, 1));
