/**
 * WO-O4O-HFF-JA-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1  §3 / §4 / §6 / §7
 *
 * HFF KO canonical → JA canonical 생성(메모리) + 렌더 검증 + Batch 02 모집단 고정.
 *   - DB 는 read-only. write 0.
 *   - KO canonical HTML 을 템플릿으로 두고 **텍스트 슬롯만** 치환한다(구조 불변 → renderer family 승계).
 *   - 모집단은 §3 그대로다: JA 부재 · KO 영구 HOLD 제외 · 현재 자산으로 build 가능 · master_id ASC 선두 10,000.
 *     원시 선두를 먼저 동결한 뒤 미생산분을 대기 HOLD 로 남기는 방식은 쓰지 않는다(§4 금지 사유).
 *   - 생산 가능 후보가 10,000 미만이면 임의 HOLD 를 만들지 않고 **정상 후보 전량**으로 전환한다(§3).
 *
 * 산출: data/hff-ja-b02-safe-targets-v1.json · data/hff-ja-b02-render-audit-v1.json · data/hff-ja-b02-ids-v1.json
 *       data/hff-ja-deferred-issue-queue-v1.jsonl (batch01 승계 + batch02 누적)
 */
...
const WO = 'WO-O4O-HFF-JA-BATCH-02-10000-DIRECT-BULK-PRODUCTION-V1';
const BATCH = 'batch02';
const TARGET = 10000;
...
fs.writeFileSync(`${CACHE}/hff-ja-b02-render-blockers.json`,
  JSON.stringify([...missTally.entries()].sort((a, b) => b[1] - a[1]).slice(0, 400).map(([k, n]) => ({ n, k })), null, 1));

const produced = targets.length;
/* §3: 후보가 10,000 미만이면 임의 HOLD 를 만들지 않고 정상 후보 전량으로 전환한다. */
const fullFallback = produced < TARGET;
const batchSize = Math.min(produced, TARGET);
...
const TASKS = `${CACHE}/hff-ja-b02-render-tasks.jsonl`;
...
const runChunk = (i) => new Promise((resolve, reject) => {
  const out = `${CACHE}/hff-ja-b02-render-part-${i}.json`;
...
for (const i of chunks) fs.rmSync(`${CACHE}/hff-ja-b02-render-part-${i}.json`, { force: true });
...
const audit = {
  ...
  targets: targets.length, batchSize, fullFallback, canonicalDup: dup,
  ...
  remainingProducibleAfterBatch: produced - batchSize,
...
fs.writeFileSync(`${D}/hff-ja-b02-safe-targets-v1.json`, JSON.stringify({ wo: WO, generatedAt: audit.renderedAt, count: targets.length, targets }, null, 1));
fs.writeFileSync(`${D}/hff-ja-b02-render-audit-v1.json`, JSON.stringify(audit, null, 1));
/* 같은 KO·자산·규칙이면 같은 배치가 재현돼야 한다(§3) — 재현 확인용 원장. */
fs.writeFileSync(`${D}/hff-ja-b02-ids-v1.json`, JSON.stringify({
