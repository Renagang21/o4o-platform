...
const rb = JSON.parse(fs.readFileSync(`${D}/hff-ko-why-family-525-rollback-manifest-v1.json`, 'utf8'));
const cls = JSON.parse(fs.readFileSync(`${D}/hff-ko-why-family-525-classification-v1.json`, 'utf8'));
const OUT_V = `${D}/hff-ko-sd-func-and-525-independent-verification-v1.json`;
const OUT_C = `${D}/hff-ko-sd-func-and-525-post-corpus-audit-v1.json`;
const OUT_DELTA = `${D}/hff-ko-why-family-525-review-queue-delta-v1.jsonl`;
...
for (const t of targets) {
  ...
  /* sd-func 다원료 블록은 <li><b>라벨</b><ul class="sd-why"><li>절</li></ul></li> 로 중첩된다.
     단순 <li>…</li> 는 라벨+절을 한 문자열로 붙여 허위 not-verbatim 을 만든다 → 최말단 <li> 만 취한다. */
  const innerLis = (html) => { const out = []; const re = /<li[^>]*>([\s\S]*?)(?=<li[^>]*>|<\/li>)/g; let m;
    while ((m = re.exec(html)) !== null) { const x = unesc(m[1].replace(/<[^>]+>/g, '')).trim(); if (x) out.push(x); } return out; };
  const clauses = innerLis(t.insertedBlock);
  clausesExpected += clauses.length;
  const items = innerLis(cur).map(dense);
...
if (g.without_fn !== 14) fail.push(`withoutFnExpected14got${g.without_fn}`);
...
fs.writeFileSync(OUT_C, JSON.stringify({ auditedAt: new Date().toISOString(), globals: g,
  expected: { without_fn: 14, hff_canonical: 40913 }, agent9HoldQueue: agent9,
  functionSectionAbsent: { before: 525, after: g.without_fn, restored: 525 - g.without_fn },
  verdict: (g.without_fn === 525 && g.hff_canonical === 40913 && nonDrift === 0) ? 'PASS' : 'FAIL' }, null, 1));

console.log(JSON.stringify({ verification: v, corpus: { globals: g, agent9, absentBefore: 525, absentAfter: g.without_fn }, deltaRows: delta.length }, null, 2));
