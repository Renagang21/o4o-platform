 ...
 *
 * DB 접근 0. prep 산출물(otc-v4-carryover72-prep/source)만 입력으로 쓴다.
...
import { MULTI_ROUTE_KEY, composeKoMulti, renderEnMulti } from './otc-v4-carryover72-multi-profile.ga.js';
/** multi-nonoral 은 배치 로컬 profile 로 저작한다(공유 composer 미수정). */
const isMulti = (route: string): boolean => route === MULTI_ROUTE_KEY;
import { toPlain, type EnV3Payload } from './otc-v3-content-leaflet-composer.na.js';
import { WO_500, PILOT_500_LEDGER } from './otc-v4-carryover72-contract.ga.js';

const PREP = path.join(DATA_DIR, 'otc-v4-carryover72-prep.ga.json');
const SOURCE = path.join(DATA_DIR, 'otc-v4-carryover72-source.ga.json');
const TM = path.join(DATA_DIR, 'otc-v4-carryover72-tm.ga.json');
...
/** seed 우선순위: next2000(최대·최신) → pilot 500 → pilot 100 → oral V3. 전부 검증 통과본. */
const SEED_TMS = [
  path.join(DATA_DIR, 'otc-v4-finalall-tm.ga.json'),
  path.join(DATA_DIR, 'otc-v4-next2000-tm.ga.json'),
...
const OUT_KO = path.join(DATA_DIR, 'otc-v4-carryover72-ko-payload.ga.json');
const OUT_EN = path.join(DATA_DIR, 'otc-v4-carryover72-en-payload.ga.json');
const OUT_REPORT = path.join(DATA_DIR, 'otc-v4-carryover72-author-report.ga.json');
...
function runKo(): void {
  ...
  for (const r of targets) {
    ...
    const res = isMulti(m.route) ? composeKoMulti(sec, m) : composeKoV4(sec, m);
...
function runEn(): void {
  ...
  for (const r of rows.filter((x) => x.producible && koBy.has(x.masterId))) {
    ...
    const res = isMulti(m.route) ? renderEnMulti(payload, m.route, sec['용법·용량'] || '') : renderEnV4(payload, m.route, sec['용법·용량'] || '');
