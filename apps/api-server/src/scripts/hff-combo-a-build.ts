...
import { FN, A_INGREDIENTS, NONA_FUNC, LIQUID } from './hff-combo-a-unregistered-registry.js';
import type { Assigned } from './hff-combo-a-classify.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // 3) 원료별 기능성 귀속 (classify seam: SINGLE / TIER-NAME 이름앵커 세그먼트 / TIER-UNIQUE 폴백)
      const attr = attributeFunctions(present, mf);
      if (attr.ok === false) {
        const reason = attr.reason;
        if (reason === 'UNLABELED_MULTI') funnel.unlabeledMulti++;
        else if (reason.startsWith('SKIN_AMBIGUOUS') || reason === 'NAME_SEG_NO_FUNC') funnel.unresolvedLabel++;
        else funnel.noFuncMatch++;
        hold.push({ stmt: r.stmt, name: r.name.trim(), reason, present: present.map((p) => p.key) }); continue;
      }
      const assigned: Assigned[] = attr.assigned;
      ...
      target.push(gi); pool.push({ stmt, name: seed.productName, sig, tier: attr.tier, funcs: assigned.map((a) => `${a.ing.key}:${a.funcs.join(',')}`) });
      funnel.target++;
    }
