/**
 * WO-O4O-HFF-JA-REMAINING-16162-PHRASE-AUTHORING-STRUCTURED-COMPRESSION-AND-CONTINUOUS-SHARDS-V1  §7 독립검증
 *
 * apply 산출물을 신뢰하지 않고 **DB 현재 상태만** 읽어 재계산한다. read-only.
 *   - Batch 03 상태 합계 = 배치 크기
...
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-ja-b04-render-audit-v1.json`, 'utf8'));
const TARGETS = JSON.parse(fs.readFileSync(`${D}/hff-ja-b04-safe-targets-v1.json`, 'utf8')).targets;
const APPLIED = JSON.parse(fs.readFileSync(`${D}/hff-ja-b04-apply-result-v1.json`, 'utf8'));
...
fs.writeFileSync(`${D}/hff-ja-b04-verify-v1.json`, JSON.stringify(out, null, 1));
