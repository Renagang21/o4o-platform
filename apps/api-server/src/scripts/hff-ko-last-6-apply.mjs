 ...
 *   node ... --apply   +   HFF_LAST6_APPLY_CONFIRM=YES
...
const SCAN = JSON.parse(fs.readFileSync(`${D}/hff-ko-last-6-scan-v1.json`, 'utf8'));
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-ko-last-6-render-audit-v1.json`, 'utf8'));
const APPLY = process.argv.includes('--apply') && process.env.HFF_LAST6_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-ko-last-6-apply-results-v1.json`, JSON.stringify(out, null, 1));
