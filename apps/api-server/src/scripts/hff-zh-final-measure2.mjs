/**
 * WO-O4O-HFF-ZH-ALL-REMAINING-10414-DIRECT-BULK-PRODUCTION-AND-TRACK-CLOSURE-V1  §4
 * 실제 render 와 **같은 build()** 로 측정한다 (offline, DB 미접근).
 * 입력: .cache/hff-zh-final-pool.jsonl (render 가 read-only 로 덤프한 KO 모집단)
 */
...
fs.writeFileSync(`${CACHE}/hff-zh-final-atoms.json`, JSON.stringify(atoms, null, 1));
...
fs.writeFileSync(`${CACHE}/hff-zh-final-render-blockers.json`, JSON.stringify(next, null, 1));
