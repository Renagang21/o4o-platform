...
const TARGET = 10000;
const P = JSON.parse(fs.readFileSync(`${D}/hff-zh-b02-phrases-v1.json`, 'utf8'));
const docs = fs.readFileSync(`${CACHE}/hff-zh-b02-docslots.jsonl`, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
...
fs.writeFileSync(`${CACHE}/hff-zh-b02-next-phrases.json`, JSON.stringify(next.slice(0, 4000), null, 1));
