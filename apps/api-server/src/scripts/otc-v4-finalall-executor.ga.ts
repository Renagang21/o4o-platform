 ...
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-finalall-executor.ga.ts [--port 5502] [--rollback-test] [--apply --confirm] [--limit N] [--only <masterId>]
 */
...
const PREP = path.join(DATA_DIR, 'otc-v4-finalall-prep.ga.json');
const KO_PAYLOAD = path.join(DATA_DIR, 'otc-v4-finalall-ko-payload.ga.json');
const EN_PAYLOAD = path.join(DATA_DIR, 'otc-v4-finalall-en-payload.ga.json');
const OUT_RESULT = path.join(DATA_DIR, 'otc-v4-finalall-result-ledger.ga.json');
const OUT_GREEN = path.join(DATA_DIR, 'otc-v4-finalall-green-ledger.ga.json');
const OUT_EXC = path.join(DATA_DIR, 'otc-v4-finalall-exception-handoff-na.ga.json');
const OUT_CKPT = path.join(DATA_DIR, 'otc-v4-finalall-checkpoint-ledger.ga.json');
...
function reproCmd(masterId: string): string {
  return `../../node_modules/.bin/tsx src/scripts/otc-v4-finalall-executor.ga.ts --only ${masterId} --rollback-test --port ${port()}`;
}
