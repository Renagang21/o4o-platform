/**
 * WO-O4O-HFF-EN-BATCH-04-5000-DIRECT-BULK-PRODUCTION-AND-DEFERRED-ISSUE-QUEUE-V1 §3
 * Batch 04 모집단 5,000건 선정 (read-only).
 *
...
const PORT = parseInt(process.env.PROXY_PORT ?? '5601', 10);
...
for (const f of ['hff-en-batch-01-population-v1.json', 'hff-en-batch02-population-5000-v1.json',
  'hff-en-batch03-population-5000-v1.json']) {
...
// §3: 기존 통합 문제 큐(Batch 01~03, 417건)는 모집단에서 제외한다.
const queued = new Set();
for (const l of fs.readFileSync(`${D}/hff-en-deferred-issue-queue-through-batch03-v1.jsonl`, 'utf8').split('\n').filter(Boolean)) {
...
fs.writeFileSync(`${D}/hff-en-batch04-population-5000-v1.json`, JSON.stringify({
  wo: 'WO-O4O-HFF-EN-BATCH-04-5000-DIRECT-BULK-PRODUCTION-AND-DEFERRED-ISSUE-QUEUE-V1',
  batch: 4, builtAt: new Date().toISOString(), readOnly: true, dbWrites: 0,
