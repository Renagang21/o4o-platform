/**
 * WO-O4O-HFF-UNREGISTERED-INGREDIENT-BULK-PRODUCTION-A-V1 — registry 미등록 실재 원료 A 전용 additive 빌더.
 *
 * 직전 [[다원료]] WO 가 `LABEL_UNMAPPED` 로 HOLD 한 원인은 **기능성 문장이 아니라 원료 라벨**이다.
 * 원문에는 원료명·기능성이 명확한데 공용 registry(SF/FUNCTIONAL/NUTRIENT META)에 해당 원료가 없어
 * `classify()` 가 실패했을 뿐이다. 본 빌더는 **공용 registry 를 수정하지 않고** A 전용 additive mapping
 * (`UI_MAP`)으로 라벨만 해석해 제품 단위로 생산한다.
 *
 * 확장 범위 (직전 빌더 대비):
 *   ① `UI_MAP` — registry 미등록 실재 기능성 원료 라벨 → key/표시명(KO·EN). 개별인정형 고시번호·®·
 *      "제품"·공백변이 접미는 정규화 후 매칭. 원료로 특정되지 않는 조각(추출물/유지/내용/제품/B1…)은 매핑 금지.
 *   ② **단일 원료 브래킷 경로** — `[원료] 기능성` 이 1개인 제품도 원문 귀속이 명확하므로 생산한다
 *      (colon/numbered 는 산문에서 오검출 위험이 있어 1건일 때 사용하지 않는다).
 *   ③ **규격 근거 게이트** — 매핑된 원료가 `BASE_STANDARD`(표시량·규격 기재)에 실재하는지 확인.
 *      없으면 `SPEC_BASIS_MISSING` HOLD (원료명·표시량 공식 근거 요구).
 *
 * 유지되는 계약(직전 WO 와 동일):
 ...
 *   - 공용 parser·registry·composer·apply 무수정.
 *
 *   PROXY_PORT=5442 npx tsx src/scripts/hff-ui-a-build.ts --out <dir> [--chunk N] [--census]
 * DB write 0. 산출: <dir>/ui-a-target-<i>.json (hff-sf-apply.ts 입력) · ui-a-{pool,hold,selfcheck}.json
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const funnel = { scanned: 0, shard0: 0, skipped: 0, noStructure: 0, preambleResidue: 0, labelUnmapped: 0, emptyBlock: 0, dupKey: 0, fiberPending: 0, specBasisMissing: 0, enPending: 0, atomUnrendered: 0, composeHold: 0, guardFail: 0, guardBlock: 0, guardReview: 0, noUiIng: 0, target: 0 };
    ...
    if (!CENSUS) for (let i = 0, b = 0; i < target.length; i += CHUNK, b++) {
      fs.writeFileSync(path.join(OUTDIR, `ui-a-target-${b}.json`), JSON.stringify(target.slice(i, i + CHUNK), null, 1));
      chunks.push(Math.min(CHUNK, target.length - i));
    }
    const w = (nm: string, d: unknown): void => fs.writeFileSync(path.join(OUTDIR, `ui-a-${nm}.json`), JSON.stringify(d, null, 1));
