// WO-O4O-OTC-REMAINING-READY-SHARD-GA-V2-LIVE-APPLY-V1 — EN config 저작 검증기 (에이전트 가).
//
...
//   npx tsx src/scripts/otc-v2-en-config-verify.ga.mjs src/scripts/data/otc-v2-en-config-ga-p01.json
import fs from 'node:fs';
import path from 'node:path';
import { renderEn, missingNumericsEn } from './otc-v2-store-leaflet-runner.shared.ts';

const DATA = 'src/scripts/data';
const FULL = path.join(DATA, 'otc-v2-authoring-source-full.ga.json');
...
console.log(`EN-CONFIG-VERIFY ga — entries ${entries} · ${failures.length === 0 ? 'PASS' : `FAIL ${failures.length}`}`);
