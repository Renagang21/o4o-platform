 ...
 * 입력: .cache/hff-zh-b03-pool.jsonl (render 가 read-only 로 덤프한 KO 모집단)
 */
...
const pool = fs.readFileSync(`${CACHE}/hff-zh-b03-pool.jsonl`, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
...
fs.writeFileSync(`${CACHE}/hff-zh-b03-atoms.json`, JSON.stringify(atoms, null, 1));
...
fs.writeFileSync(`${CACHE}/hff-zh-b03-render-blockers.json`, JSON.stringify(next, null, 1));
