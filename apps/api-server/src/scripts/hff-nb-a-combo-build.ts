/**
 * WO-O4O-HFF-NOBRACKET-BULK-PRODUCTION-A-V1 — noBracket **복합형(명시 귀속)** A 전용 additive 빌더.
 *
 * `hff-nb-a-build.ts` 가 단일 원료만 생산하고 HOLD 한 MULTI_INGREDIENT 중,
 * 원문 MAIN_FNCTN 이 **원료별로 기능성을 명시 귀속**한 경우만 회수한다.
 *   - `parseFnAttribution` mode 가 `colon` 또는 `numbered` (원문 라벨 구조 · 추정 귀속 아님)
 *     ※ `inline` / `none` 은 registry 추정 폴백이므로 제외한다(귀속 모호 → HOLD).
 *   - **fullCover 만 생산**: 규격에서 식별된 모든 원료가 귀속을 받고, 미귀속 문장 0 · 미해석 라벨 0.
 *     하나라도 남으면 기능성 누락·오귀속 위험 → HOLD.
 *   - `식이섬유` 는 상이 원료가 한 키로 접히므로 포함 시 제품 전체 HOLD(PENDING_SHARED).
 *   - EN 은 공용 매퍼(`mapFunctionEnC ?? mapFunctionEn`)로만 만든다. 하나라도 미매핑이면 HOLD(임의 영문 0).
 * 공식 기능성 문장은 원문 그대로 렌더링하며 삭제·순화·추가를 하지 않는다. 전문가 상담 footer 유지.
 *
 * 자기완결(A 전용): 공용 composer 는 표시량 기반이라 본 경로에 부적합하므로 `hff-combo-a-build.ts` 와
 * 동일 계약의 함수기반 카드(mg 미기재)를 로컬 구성한다. 공용 parser·registry·apply 는 수정하지 않는다.
 *
 *   PROXY_PORT=5442 npx tsx src/scripts/hff-nb-a-combo-build.ts --out <dir> [--chunk N]
 * DB write 0. 산출: <dir>/nbc-a-target-<i>.json (hff-sf-apply.ts 입력) · nbc-a-{pool,hold,selfcheck}.json
 */
...
import { parseSpecs, classify, NONFUNC, parseFnAttribution } from './hff-source-parse.js';
...
interface NbIng { key: string; displayKo: string; displayEn: string; fnKo: string[]; fnEn: string[] }
...
interface NbSeed {
  statementNo: string; candidateId: string; productName: string; manufacturer: string; ings: NbIng[];
...
/** 함수기반 복합형 카드(표시량 미기재) — `hff-combo-a-build.ts` composeComboA 와 동일 레이아웃 계약. */
function composeNbCombo(seed: NbSeed): { ko: string; en: string } | { error: string } {
...
/** 로컬 combo guard: 원료 카드 수 ko=en=N · 원료별 기능성 문구 draft 포함 · 원료 키 중복 0. */
function nbComboGuard(seed: NbSeed, ko: string, en: string): string[] {
  ...
  if (koCards !== n || enCards !== n) out.push(`G-NB-CARD-COUNT: n${n}/ko${koCards}/en${enCards}`);
  for (const a of seed.ings) {
    if (!a.fnKo.length || a.fnKo.length !== a.fnEn.length) out.push(`G-NB-FUNC-COUNT:${a.key}`);
    for (const f of a.fnKo) if (!ko.includes(esc(f))) out.push(`G-NB-KO-MISSING:${a.key}`);
    for (const f of a.fnEn) if (!en.includes(esc(f))) out.push(`G-NB-EN-MISSING:${a.key}`);
  }
  const keys = seed.ings.map((a) => a.key);
  if (new Set(keys).size !== keys.length) out.push(`G-NB-DUP:${keys.join(',')}`);
  return out;
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const funnel = { scanned: 0, shard0: 0, noBracket: 0, skipped: 0, unresolvedLabel: 0, notMulti: 0, fiberPending: 0, noStructure: 0, notFullCover: 0, noMeta: 0, enPending: 0, composeHold: 0, comboGuardFail: 0, guardBlock: 0, guardReview: 0, target: 0 };
    const target: unknown[] = []; const pool: unknown[] = []; const hold: Array<Record<string, unknown>> = [];
    const seen = new Set<string>(); const holdReason: Record<string, number> = {}; const distCombo: Record<string, number> = {};
    const H = (stmt: string, name: string, reason: string, extra: Record<string, unknown> = {}): void => {
      ...
      if (hold.length < 3000) hold.push({ stmt, name, reason, ...extra });
    };
    ...
    for (;;) {
      ...
      for (const r of rows) {
        ...
        const mf = r.fn || ''; if (/\[[^\]]{1,24}\]/.test(mf)) continue; funnel.noBracket++;
        if (seen.has(stmt)) continue; seen.add(stmt);
        if (r.mid != null || taken.has(stmt) || hasMaster.has(stmt)) { funnel.skipped++; continue; }

        const sp = parseSpecs(r.base || ''); const keys = new Set<string>(sp.byKey.keys()); let bad = false;
        for (const lb of sp.unknownLabels) {
          const n = lb.replace(/^\s*(?:\((?:\d+|[가-힣])\)|[①②③④⑤⑥⑦⑧⑨⑩]|\d+[).])\s*/, '').trim();
          if (NONFUNC.test(n)) continue;
          const sf = Object.values(SF_INGREDIENTS).find((i) => i.labelRe.test(n) || (i.indicatorRe?.test(n) ?? false));
          if (sf) { keys.add(sf.key); continue; }
          const c = classify(n); if (c) keys.add(c); else { bad = true; break; }
        }
        if (bad) { funnel.unresolvedLabel++; continue; }        // 단일 빌더와 동일 사유 — 중복 집계 회피
        if (keys.size < 2) { funnel.notMulti++; continue; }     // 단일은 hff-nb-a-build.ts 소관
        if (keys.has('식이섬유')) { funnel.fiberPending++; H(stmt, r.name.trim(), 'PENDING_SHARED_FIBER'); continue; }

        const fp = parseFnAttribution(mf);
        if (fp.mode !== 'colon' && fp.mode !== 'numbered') { funnel.noStructure++; H(stmt, r.name.trim(), `NO_EXPLICIT_STRUCTURE:${fp.mode}`); continue; }
        if (fp.unattributed.length || fp.unknownLabels.length) { funnel.notFullCover++; H(stmt, r.name.trim(), 'RESIDUE_UNATTRIBUTED', { mode: fp.mode }); continue; }
        const ks = [...keys];
        if (ks.some((k) => (fp.byKey.get(k)?.length ?? 0) === 0)) { funnel.notFullCover++; H(stmt, r.name.trim(), 'PARTIAL_COVER', { mode: fp.mode, miss: ks.filter((k) => !fp.byKey.get(k)?.length) }); continue; }
        // 규격에 없는 원료로 귀속된 기능성이 있으면(원문 라벨 ⊄ 규격) 귀속 신뢰 불가 → HOLD.
        if ([...fp.byKey.keys()].some((k) => !keys.has(k))) { funnel.notFullCover++; H(stmt, r.name.trim(), 'FN_KEY_NOT_IN_SPEC', { mode: fp.mode }); continue; }

        const ings: NbIng[] = []; let metaMiss = '', enMiss = '';
        for (const k of ks) {
          const meta = metaOf(k); if (!meta) { metaMiss = k; break; }
          const fnKo = fp.byKey.get(k) ?? []; const fnEn: string[] = [];
          for (const f of fnKo) { const e = mapFunctionEnC(f) ?? mapFunctionEn(f); if (!e) { enMiss = f; break; } fnEn.push(e); }
          if (enMiss) break;
          ings.push({ key: k, displayKo: meta.displayKo, displayEn: meta.displayEn, fnKo, fnEn });
        }
        if (metaMiss) { funnel.noMeta++; H(stmt, r.name.trim(), `NO_META:${metaMiss}`); continue; }
        if (enMiss) { funnel.enPending++; H(stmt, r.name.trim(), 'GROUNDING_PENDING_EN', { miss: enMiss.slice(0, 50) }); continue; }

        const seed: NbSeed = { statementNo: stmt, candidateId: r.id, productName: r.name.trim(), manufacturer: r.maker.trim(), ings,
        ...
        const c = composeNbCombo(seed);
        if ('error' in c) { funnel.composeHold++; H(stmt, seed.productName, `COMPOSE_${c.error}`); continue; }
        const cg = nbComboGuard(seed, c.ko, c.en);
        if (cg.length) { funnel.comboGuardFail++; H(stmt, seed.productName, `COMBO_GUARD:${cg[0]}`); continue; }
        ...
        if (blocked.length) { funnel.guardBlock++; H(stmt, seed.productName, `GUARD_BLOCKED:${blocked.map((f) => f.ruleId).join(',')}`); continue; }
        if (g.overallStatus === 'REVIEW_REQUIRED') { funnel.guardReview++; H(stmt, seed.productName, `GUARD_REVIEW:${g.findings.filter((f) => f.status === 'REVIEW_REQUIRED').map((f) => f.ruleId).join(',')}`); continue; }

        const combo = ks.slice().sort().join('+'); distCombo[combo] = (distCombo[combo] ?? 0) + 1;
        target.push(gi); pool.push({ stmt, name: seed.productName, keys: ks, mode: fp.mode });
        funnel.target++;
      }
    }
    ...
    for (let i = 0, b = 0; i < target.length; i += CHUNK, b++) {
      fs.writeFileSync(path.join(OUTDIR, `nbc-a-target-${b}.json`), JSON.stringify(target.slice(i, i + CHUNK), null, 1));
      chunks.push(Math.min(CHUNK, target.length - i));
    }
    const w = (nm: string, d: unknown): void => fs.writeFileSync(path.join(OUTDIR, `nbc-a-${nm}.json`), JSON.stringify(d, null, 1));
    w('pool', pool); w('hold', hold); w('selfcheck', { funnel, distCombo, holdReason, chunks });
    console.log('JSON_NBC_A_BEGIN');
    console.log(JSON.stringify({ funnel, distCombo, holdReason, targetTotal: target.length, chunks }, null, 2));
    console.log('JSON_NBC_A_END');
