/**
 * WO-O4O-HFF-EN-BATCH-03-5000-DIRECT-BULK-PRODUCTION-AND-DEFERRED-ISSUE-QUEUE-V1 §8·§10
 * Batch 03 종료 집계 + 통합 deferred issue queue + 독립 검증 (read-only, DB write 없음).
 */
...
const PORT = parseInt(process.env.PROXY_PORT ?? '5591', 10);
...
// ── 1. Batch 03 상태 집계 ──────────────────────────────────────────────────
const pop = rd('hff-en-batch03-population-5000-v1.json').rows;
for (const r of pop) r.productMasterId ??= r.masterId;
const cls = rd('hff-en-batch03-classification-v1.json').results;
const holdBy = new Map(cls.filter((r) => r.holdReason).map((r) => [r.productMasterId, r]));
const applied = rdIf('hff-en-batch03-apply-results-v1.json');
...
for (const r of pop) {
  ...
  holds.push({
    batch: 3, productMasterId: r.productMasterId, koCanonicalId: r.koCanonicalId,
...
for (const r of holds) {
  queue.push({ batch: 3, productMasterId: r.productMasterId, koCanonicalId: r.koCanonicalId,
...
fs.writeFileSync(`${D}/hff-en-deferred-issue-queue-through-batch03-v1.jsonl`,
  queue.map((q) => JSON.stringify(q)).join('\n') + '\n', 'utf8');
wr('hff-en-deferred-issue-queue-through-batch03-summary-v1.json', {
...
// ── 4. 종료 매니페스트 ────────────────────────────────────────────────────
const closure = {
  wo: 'WO-O4O-HFF-EN-BATCH-03-5000-DIRECT-BULK-PRODUCTION-AND-DEFERRED-ISSUE-QUEUE-V1',
  closedAt: new Date().toISOString(), batch: 3,
...
wr('hff-en-batch03-closure-v1.json', closure);
wr('hff-en-batch03-completed-v1.json', { builtAt: closure.closedAt, count: completed.length, rows: completed });
fs.writeFileSync(`${D}/hff-en-batch03-final-hold-v1.jsonl`, holds.map((h) => JSON.stringify(h)).join('\n') + '\n', 'utf8');
...
wr('hff-en-batch03-independent-verification-v1.json', verification);

const b01 = rdIf('hff-en-batch01-closure-v1.json') ?? {};
const b02 = rdIf('hff-en-batch02-closure-v1.json') ?? {};
wr('hff-en-production-completed-through-batch03-v1.json', {
  ...
  batch03: { population: pop.length, completed: completed.length },
  hffEnCanonicalTotalLive: enTotal,
});
wr('hff-en-production-remaining-after-batch03-v1.json', {
