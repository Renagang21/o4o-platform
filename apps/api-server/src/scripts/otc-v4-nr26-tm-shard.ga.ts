 ...
 *   — nasal14·rectal12 EN 번역메모리 미충족 문장 샤딩 / 병합 (에이전트 가)
 *
 * DB 접근 0. TM 파일(otc-v4-nr26-tm.ga.json)만 입출력한다.
 *
...
import { WO_NR } from './otc-v4-nr26-contract.ga.js';
import { FAMILY, EVIDENCE_KO, INVERSION_FAMILIES, type RouteFamily } from './otc-v4-nr26-profile.ga.js';

const TM = path.join(DATA_DIR, 'otc-v4-nr26-tm.ga.json');
const shardFile = (i: number): string => path.join(DATA_DIR, `otc-v4-nr26-tm-shard${i}.ga.json`);
const filledFile = (i: number): string => path.join(DATA_DIR, `otc-v4-nr26-tm-shard${i}-filled.ga.json`);
const OUT_MERGE = path.join(DATA_DIR, 'otc-v4-nr26-tm-merge-report.ga.json');
 ...
 * 문장 단위 **경로 표현 역전** 게이트.
 *
 * route535 에서는 EN 경구동사 오도입을 생산 직전 전수검사에서야 잡았다(22건). 여기서는
 * TM 병합 시점에 막는다. 문장이 쓰이는 경로(annotations.routes)의 §6·§7 금지 계열 마커가
 * EN 에 나타났는데 **KO 원문에 그 계열 근거가 전혀 없으면** 반려한다.
 * (KO 가 "눈에 들어가지 않도록" 처럼 근거를 가진 경우는 EVIDENCE_KO 로 면제 — 오탐 방지.)
 */
export function inversionInSentence(ko: string, en: string, routes: string[]): string[] {
  const hits: string[] = [];
  const fams = new Set<RouteFamily>();
  for (const r of routes) for (const f of (INVERSION_FAMILIES[r] || [])) fams.add(f);
  for (const fam of fams) {
    const mk = FAMILY[fam];
    if (!mk.en.test(en)) continue;
    if (mk.ko.test(ko) || EVIDENCE_KO[fam].test(ko)) continue;   // KO 원문 근거 있음 → 역전 아님
    hits.push(`${fam}(${mk.label})`);
  }
  return hits;
}
...
function runSplit(): void {
  ...
  const shards: Array<Array<{ ko: string; section: string; routes: string[]; masters: number; numbers: string[]; forbiddenRouteWords: string[]; previousRejection?: string }>> = [];
  for (let i = 0; i < n; i++) shards.push([]);
  pending.forEach((ko, idx) => {
    const a = tm.annotations[ko];
    const rej = prevReject.get(ko);
    // KO 원문에 근거가 없는 계열만 실제 금지 대상이다(문장별로 다르다).
    const fams = new Set<RouteFamily>();
    for (const r of a.routes) for (const f of (INVERSION_FAMILIES[r] || [])) fams.add(f);
    const forbiddenRouteWords = [...fams]
      .filter((f) => !FAMILY[f].ko.test(ko) && !EVIDENCE_KO[f].test(ko))
      .map((f) => FAMILY[f].label);
    shards[idx % n].push({ ko, section: a.section, routes: a.routes, masters: a.masters, numbers: numTokens(ko), forbiddenRouteWords, ...(rej ? { previousRejection: rej } : {}) });
  });
  for (let i = 0; i < n; i++) {
    fs.writeFileSync(shardFile(i), JSON.stringify({
      wo: WO_NR, kind: 'tm-shard', shard: i, shards: n, total: shards[i].length,
      instruction: 'sentences[].ko 를 EN 으로 1:1 번역해 filled 파일의 entries{ko:en} 에 채운다. 의료사실 추가·삭제 금지. 수치·연령·기간 보존. 한글 잔존 금지. KO 원문에 없는 투여 경로 표현(경구 복용·점안·피부 도포·질내 삽입·흡입 등) 도입 금지 — sentences[].forbiddenRouteWords 참고.',
      sentences: shards[i],
    }, null, 2) + '\n', 'utf8');
  }
...
function runMerge(): void {
  ...
  for (let i = 0; ; i++) {
    ...
    for (const [ko, en] of Object.entries(ent)) {
      ...
      // 경로 표현 역전 — route535 에서 생산 직전에야 잡힌 유형을 문장 단위로 앞당겨 막는다.
      const inv = inversionInSentence(ko, t, tm.annotations[ko]?.routes || []);
      if (inv.length) { rejected.push({ shard: i, ko, reason: `경로 표현 도입(원문 근거 없음): ${inv.join(',')}` }); continue; }
  ...
  fs.writeFileSync(OUT_MERGE, JSON.stringify({
    wo: WO_NR, mode: 'merge', before, applied, skippedExisting, after,
...
function runRekey(): void {
  ...
  const after = Object.values(tm.entries).filter(Boolean).length;
  fs.writeFileSync(TM, JSON.stringify({ ...tm, filled: after }, null, 2) + '\n', 'utf8');
  fs.writeFileSync(path.join(DATA_DIR, 'otc-v4-nr26-tm-rekey-report.ga.json'), JSON.stringify({
    wo: WO_NR, mode: 'rekey',
...
// NR26_TM_LIB=1 이면 라이브러리로만 로드한다(게이트 negative control 테스트용, 부수효과 0).
if (process.env.NR26_TM_LIB === '1') { /* no-op */ }
else if (has('split')) runSplit();
