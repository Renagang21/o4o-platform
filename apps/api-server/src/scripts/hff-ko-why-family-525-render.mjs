...
const OUT = `${D}/hff-ko-why-family-525-render-audit-v1.json`;
...
for (const t of targets) {
  /* sd-func 다원료 블록은 <li><b>라벨</b><ul class="sd-why"><li>절</li></ul></li> 로 중첩된다.
     단순 <li>…</li> 는 라벨+절을 하나로 붙여 innerText 에 없는 문자열을 만든다 → 최말단 <li> 만 취한다. */
  const clauses = [];
  { const re = /<li[^>]*>([\s\S]*?)(?=<li[^>]*>|<\/li>)/g; let m;
    while ((m = re.exec(t.insertedBlock)) !== null) { const x = m[1].replace(/<[^>]+>/g, '').trim(); if (x) clauses.push(x); } }
  ...
  for (const w of [430, 820, 1280]) {
    ...
    const r = await page.evaluate((cls) => {
      ...
      const iw = hs.indexOf('왜 이 제품인가'), ifn = hs.findIndex((h) => /기능성$/.test(h) && h !== '왜 이 제품인가'), ii = hs.findIndex((h) => h.startsWith('섭취방법'));
