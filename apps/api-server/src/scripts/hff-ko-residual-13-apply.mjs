/** Phase 2-C — 기능성 복구 11건 적용. 단일 트랜잭션, 낙관적 잠금, 이중 게이트. */
import fs from 'node:fs';
import crypto from 'node:crypto';
import pg from 'pg';

const APPLY = process.argv.includes('--apply');
const CONFIRM = process.env.HFF_RESIDUAL13_APPLY_CONFIRM === 'YES';
const D = 'apps/api-server/src/scripts/data';
const OUT = `${D}/hff-ko-residual-13-apply-results-v1.json`;
...
const rbFile = JSON.parse(fs.readFileSync(`${D}/hff-ko-residual-13-rollback-v1.json`, 'utf8'));
const render = JSON.parse(fs.readFileSync(`${D}/hff-ko-residual-13-render-audit-v1.json`, 'utf8'));
...
const contents = new Map(JSON.parse(fs.readFileSync(`${D}/tmp-hff-residual13-newcontent.json`, 'utf8')).map((x) => [x.canonicalId, x]));
...
if (!CONFIRM) throw new Error('APPLY_BLOCKED: HFF_RESIDUAL13_APPLY_CONFIRM=YES 필요');
...
const snap = async () => (await c.query(`
  SELECT
    ...
    (SELECT count(*)::int FROM shared_product_descriptions WHERE ${KO} AND content NOT LIKE '%매장 내 약사 등 전문가%') AS no_expert`)).rows[0];
...
try {
  ...
  const got = new Map();
  for (const r of (await c.query(`SELECT id, content FROM shared_product_descriptions WHERE id = ANY($1)`, [targets.map((t) => t.canonicalId)])).rows) got.set(r.id, r.content);
  ...
  if (mid.no_fn !== before.no_fn - targets.length) { failReason = `NO_FN_DELTA_UNEXPECTED ${before.no_fn} -> ${mid.no_fn}`; throw new Error(failReason); }
  if (mid.audience !== before.audience || mid.no_expert !== before.no_expert) { failReason = 'POLICY_COUNTS_CHANGED'; throw new Error(failReason); }
...
const out = { ranAt: new Date().toISOString(), wo: rbFile.wo, phase: rbFile.phase,
  ...
  effects: idx.effects, holdRemaining: idx.holdRows };
