 ...
 *
 * ⚠️ 독립성 계약: 실행기(otc-v4-pilot-500-executor.ga.ts)의 어떤 함수도 import 하지 않는다.
 ...
 *
 * 실행:
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-pilot-500-independent-verify.ga.ts --baseline --port 5502
 *   ../../node_modules/.bin/tsx src/scripts/otc-v4-pilot-500-independent-verify.ga.ts --verify   --port 5502
 */
...
const LEDGER_500 = P('otc-easy-drug-remaining-pilot-500-ledger-v1.json');
...
const RESULT = P('otc-v4-pilot-500-result-ledger.ga.json');
const SOURCE = P('otc-v4-pilot-500-source.ga.json');
const OUT_BASELINE = P('otc-v4-pilot-500-verify-baseline.ga.json');
const OUT_VERIFY = P('otc-v4-pilot-500-independent-verification.ga.json');
...
async function snapshotBaseline(pool: Pool): Promise<void> {
  ...
  const p100 = readJson(GREEN_100);
  const greenIds: string[] = (p100.rows || []).map((r: any) => r.masterId);
...
async function verify(pool: Pool): Promise<void> {
  ...
  const residueAudit = excIds.length ? (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-pilot-500'`, [excIds])).rows[0].n : 0;
  ...
  const outsideAudit = (await pool.query(
    `SELECT count(*)::int n FROM shared_product_description_audit_logs
      WHERE (metadata->>'batchId')='otc-v4-pilot-500' AND master_id <> ALL($1::uuid[])`, [ledgerIds])).rows[0].n;
  ...
  const auditPerGreen = greenIds.length ? (await pool.query(
    `SELECT master_id::text mid, count(*)::int n FROM shared_product_description_audit_logs
      WHERE master_id = ANY($1::uuid[]) AND (metadata->>'batchId')='otc-v4-pilot-500'
  ...
  if (base) {
    ...
    const nowRows = base.pilot100GreenIds ? (await pool.query(
      `SELECT master_id::text mid, COALESCE(language,'ko') lang, md5(content) h
         FROM shared_product_descriptions
        WHERE master_id = ANY($1::uuid[]) AND description_type='STORE' AND status='canonical' AND deleted_at IS NULL
        ORDER BY 1,2`, [readJson(GREEN_100).rows.map((r: any) => r.masterId)])).rows : [];
    ...
    G('IV-20', 'pilot 100 GREEN 80 불변', '변경 0', `변경 ${changed.length} / 소실 ${removed.length}`,
    ...
    G('IV-20', 'pilot 100 GREEN 80 불변', 'baseline 필요', 'baseline 파일 없음', false);
  ...
  // J. pilot 100 교집합 0
  const ids100 = readJson(LEDGER_100).masters.map((m: any) => m.masterId);
  const inter = ledgerIds.filter((id) => ids100.includes(id));
  G('IV-23', 'pilot 100 교집합 0', '0', String(inter.length), inter.length === 0);
  ...
  const out = {
    ...
    kind: 'independent-verification', batchId: 'otc-v4-pilot-500', verifiedAt: new Date().toISOString(),
