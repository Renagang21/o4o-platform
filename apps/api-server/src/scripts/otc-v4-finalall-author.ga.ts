 ...
 *
 * DB 접근 0. prep 산출물(otc-v4-finalall-prep/source)만 입력으로 쓴다.
...
import { WO_500, PILOT_500_LEDGER } from './otc-v4-finalall-contract.ga.js';

const PREP = path.join(DATA_DIR, 'otc-v4-finalall-prep.ga.json');
const SOURCE = path.join(DATA_DIR, 'otc-v4-finalall-source.ga.json');
...
const OUT_KO = path.join(DATA_DIR, 'otc-v4-finalall-ko-payload.ga.json');
const OUT_EN = path.join(DATA_DIR, 'otc-v4-finalall-en-payload.ga.json');
const OUT_REPORT = path.join(DATA_DIR, 'otc-v4-finalall-author-report.ga.json');
