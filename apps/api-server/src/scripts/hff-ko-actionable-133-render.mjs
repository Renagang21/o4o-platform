...
const OUT = `${D}/hff-ko-actionable-hold-133-render-audit-v1.json`;
const safe = JSON.parse(fs.readFileSync(`${D}/hff-ko-actionable-hold-133-safe-targets-v1.json`, 'utf8'));
const newA = new Map(JSON.parse(fs.readFileSync(`${D}/tmp-hff-133-newcontent.json`, 'utf8')).map((x) => [x.canonicalId, x.newContent]));
...
const docs = [
  ...safe.targets.map((t) => ({ kind: 'A', name: t.productName, html: newA.get(t.canonicalId) })),
...
for (const d of docs) {
  ...
  for (const w of [430, 820, 1280]) {
    ...
    const r = await page.evaluate(() => {
      ...
      // sd-func 구조에서 바깥 li 는 <b>원료명</b>(영문 포함 가능)을 담는다.
      // 기능성 절 검사는 **중첩 ul 이 없는 최말단 li** 로 한정해야 라벨을 절로 오판하지 않는다.
      const items = [...document.querySelectorAll('.sd-why li, .sd-core li, .sd-func li, .sd-item li')]
        .filter((n) => !n.querySelector('ul')).map((n) => n.textContent.trim());
      return {
