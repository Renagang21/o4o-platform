/**
 * Quasi-Drug Permit Candidate Import CLI — 의약외품 raw JSONL → ProductCandidate 후보 적재
 *
...
import { QuasiDrugPermitCandidateImportService } from '../modules/neture/drug-import/quasi-drug-permit-candidate-import.service.js';
...
function parseArgs(argv: string[]): CliArgs {
  ...
  const file = get('file');
  if (!file) throw new Error('--file 필수 (의약외품 raw JSONL 경로)');
...
async function main(): Promise<void> {
  ...
  if (args.apply && process.env.QUASI_DRUG_IMPORT_ALLOW_APPLY !== 'I_UNDERSTAND') {
    throw new Error(
      'APPLY_BLOCKED: --apply 는 WO-O4O-QUASI-DRUG-PUBLIC-CANDIDATE-IMPORT-MAPPER-AND-SERVICE-V1 안전 경계에 의해 차단됨. ' +
        '데이터 변경은 사용자 승인 + 백업 확인 후 별도 Gate A apply WO 로 진행. (해제: QUASI_DRUG_IMPORT_ALLOW_APPLY=I_UNDERSTAND)',
    );
  }
  ...
  const service = new QuasiDrugPermitCandidateImportService();
  ...
  console.log('───────────────────────────────────────────────');
  console.log('의약외품(제품 허가정보) candidate import 결과');
  console.log('───────────────────────────────────────────────');
  ...
  console.log(
    `classification      : active=${report.classification.active} cancelled=${report.classification.cancelled}`,
  );
  console.log(`candidateNameTrunc  : ${report.candidateNameTruncatedCount}`);
  console.log(
...
main().catch((e) => {
  console.error('[quasi-drug-permit-candidate-import] FAILED:', e instanceof Error ? e.message : e);
  process.exit(1);
});
