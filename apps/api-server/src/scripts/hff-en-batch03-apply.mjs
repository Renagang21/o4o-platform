 ...
 *   node ... --apply  +  HFF_EN_B03_APPLY_CONFIRM=YES
 */
import fs from 'node:fs';
import pg from 'pg';

const D = 'apps/api-server/src/scripts/data';
const SAFE = JSON.parse(fs.readFileSync(`${D}/hff-en-batch03-safe-targets-v1.json`, 'utf8')).targets;
const RENDER = JSON.parse(fs.readFileSync(`${D}/hff-en-batch03-render-audit-v1.json`, 'utf8'));
const APPLY = process.argv.includes('--apply') && process.env.HFF_EN_B03_APPLY_CONFIRM === 'YES';
...
fs.writeFileSync(`${D}/hff-en-batch03-rollback-v1.json`, JSON.stringify({
...
fs.writeFileSync(`${D}/hff-en-batch03-apply-results-v1.json`, JSON.stringify(out, null, 1));
fs.writeFileSync(`${D}/hff-en-batch03-safe-targets-v1.json`, JSON.stringify({ builtAt: new Date().toISOString(), count: SAFE.length, targets: SAFE }, null, 1));
