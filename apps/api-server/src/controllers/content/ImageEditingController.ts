...
export class ImageEditingController {
  ...
  cropImage = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await imageProcessingService.cropImage(
        mediaId, 
        cropX, 
        cropY, 
        cropWidth, 
      ...
      logger.error('Error in cropImage:', error);
      res.status(500).json({
        success: false,
  ...
  rotateImage = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await imageProcessingService.rotateImage(
        mediaId, 
        rotationAngle,
        {
      ...
      logger.error('Error in rotateImage:', error);
      res.status(500).json({
        success: false,
  ...
  addWatermark = async (req: Request, res: Response): Promise<void> => {
    try {
      const { 
        mediaId, 
        text, 
  ...
  optimizeImage = async (req: Request, res: Response): Promise<void> => {
    try {
      const { 
        mediaId, 
        format, 
      ...
      logger.error('Error in optimizeImage:', error);
      res.status(500).json({
        success: false,
  ...
  getImageAnalysis = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      ...
      const result = await imageProcessingService.getImageAnalysis(id);
      ...
      logger.error('Error in getImageAnalysis:', error);
      res.status(500).json({
        success: false,
  ...
  batchOptimizeImages = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      for (const mediaId of mediaIds) {
        try {
          const result = await imageProcessingService.optimizeImage(mediaId, {
      ...
      logger.error('Error in batchOptimizeImages:', error);
      res.status(500).json({
        success: false,
  ...
  generateResponsiveImages = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.error('Error in generateResponsiveImages:', error);
      res.status(500).json({
        success: false,
