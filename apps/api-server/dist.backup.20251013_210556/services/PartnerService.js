...
class PartnerService {
    constructor() {
        ...
        this.commissionRepository = connection_1.AppDataSource.getRepository(PartnerCommission_1.PartnerCommission);
        ...
        this.sellerRepository = connection_1.AppDataSource.getRepository(Seller_1.Seller);
    }
    // 파트너 등록 신청
    async applyAsPartner(data) {
        try {
            ...
            const partner = this.partnerRepository.create({
                ...
                tier: Partner_1.PartnerTier.BRONZE,
                totalEarnings: 0,
                totalClicks: 0,
                totalOrders: 0,
                conversionRate: 0,
            ...
            logger_1.default.info(`Partner application submitted: ${savedPartner.id} by user ${data.userId}`);
            return savedPartner;
        }
        catch (error) {
            logger_1.default.error('Error creating partner application:', error);
            throw error;
        }
    }
    // 파트너 승인/거절
    async approvePartner(partnerId, approved, adminNotes) {
        try {
            ...
            if (partner.status !== Partner_1.PartnerStatus.PENDING) {
                throw new Error('Partner is not in pending status');
            }
            partner.status = approved ? Partner_1.PartnerStatus.ACTIVE : Partner_1.PartnerStatus.REJECTED;
            ...
            logger_1.default.info(`Partner ${approved ? 'approved' : 'rejected'}: ${partnerId}`);
            return updatedPartner;
        }
        catch (error) {
            logger_1.default.error('Error approving partner:', error);
            throw error;
        }
    }
    // 파트너 조회
    ...
    async getPartners(filters = {}) {
        try {
            const { status, tier, isActive, minCommissionEarned, maxCommissionEarned, registeredAfter, registeredBefore, search, sortBy = 'registeredAt', sortOrder = 'desc', page = 1, limit = 20 } = filters;
            ...
            if (search) {
                queryBuilder.andWhere('(partner.businessName ILIKE :search OR partner.referralCode ILIKE :search OR user.name ILIKE :search OR user.email ILIKE :search)', { search: `%${search}%` });
            }
            ...
            logger_1.default.error('Error fetching partners:', error);
            throw error;
        }
    }
    // 파트너 업데이트
    async updatePartner(id, data) {
        try {
            ...
            logger_1.default.info(`Partner updated: ${id}`);
            return updatedPartner;
        }
        catch (error) {
            logger_1.default.error('Error updating partner:', error);
            throw error;
        }
    }
    // 추천 링크 생성
    async generateReferralLink(params) {
        try {
            const partner = await this.partnerRepository.findOne({
                where: { id: params.partnerId, isActive: true, status: Partner_1.PartnerStatus.ACTIVE }
            });
            if (!partner) {
            ...
            logger_1.default.error('Error generating referral link:', error);
            throw error;
        }
    }
    // 클릭 추적
    async trackClick(referralCode, trackingData) {
        try {
            ...
            if (!partner) {
                logger_1.default.warn(`Click tracking failed: Invalid referral code ${referralCode}`);
                return;
            }
            partner.totalClicks = (partner.totalClicks || 0) + 1;
            partner.lastActiveAt = new Date();
            await this.partnerRepository.save(partner);
            logger_1.default.info(`Click tracked for partner ${partner.id}: ${referralCode}`);
        }
        catch (error) {
            logger_1.default.error('Error tracking click:', error);
        }
    }
    // 커미션 내역 조회
    async getCommissions(filters = {}) {
        try {
            const { partnerId, status, productId, sellerId, dateFrom, dateTo, minAmount, maxAmount, referralCode, sortBy = 'createdAt', sortOrder = 'desc', page = 1, limit = 20 } = filters;
            ...
            logger_1.default.error('Error fetching commissions:', error);
            throw error;
        }
    }
    // 파트너 성과 통계
    async getPartnerStats(partnerId, period) {
        try {
            const partner = await this.partnerRepository.findOne({
                where: { id: partnerId }
            });
            ...
            const stats = await this.commissionRepository
                ...
                .setParameters({
                pending: PartnerCommission_1.CommissionStatus.PENDING,
                confirmed: PartnerCommission_1.CommissionStatus.CONFIRMED,
                paid: PartnerCommission_1.CommissionStatus.PAID
            })
                .getRawOne();
            ...
            logger_1.default.error('Error fetching partner stats:', error);
            throw error;
        }
    }
    // 파트너 티어 업데이트 (월별 자동 실행)
    async updatePartnerTiers() {
        try {
            const partners = await this.partnerRepository.find({
                where: { isActive: true, status: Partner_1.PartnerStatus.ACTIVE }
            });
            ...
            for (const partner of partners) {
                const monthlyStats = await this.commissionRepository
                    ...
                    .andWhere('commission.status = :status', { status: PartnerCommission_1.CommissionStatus.PAID })
                ...
                let newTier = Partner_1.PartnerTier.BRONZE;
                if (monthlyCommissions >= 1000000) { // 100만원 이상
                    newTier = Partner_1.PartnerTier.PLATINUM;
                }
                else if (monthlyCommissions >= 500000) { // 50만원 이상
                    newTier = Partner_1.PartnerTier.GOLD;
                }
                else if (monthlyCommissions >= 200000) { // 20만원 이상
                    newTier = Partner_1.PartnerTier.SILVER;
                }
                if (partner.tier !== newTier) {
                    partner.tier = newTier;
                    await this.partnerRepository.save(partner);
                    logger_1.default.info(`Partner tier updated: ${partner.id} -> ${newTier}`);
                }
            }
        }
        catch (error) {
            logger_1.default.error('Error updating partner tiers:', error);
            throw error;
        }
    }
    // 파트너 통계 요약 (관리자용)
    async getOverallStats() {
        try {
            const partnerStats = await this.partnerRepository
                ...
                .setParameters({
                active: Partner_1.PartnerStatus.ACTIVE,
                pending: Partner_1.PartnerStatus.PENDING,
                bronze: Partner_1.PartnerTier.BRONZE,
                silver: Partner_1.PartnerTier.SILVER,
                gold: Partner_1.PartnerTier.GOLD,
                platinum: Partner_1.PartnerTier.PLATINUM
            })
                .getRawOne();
            ...
            logger_1.default.error('Error fetching overall stats:', error);
            throw error;
        }
    }
    // 추천 코드 고유성 확인 및 생성
