...
class ForecastingService {
        ...
        try {
            ...
            const result = {
                ...
                metadata: {
                    ...
                    trainingPeriod: {
                        start: ((_a = historicalData[0]) === null || _a === void 0 ? void 0 : _a.date) || '',
                        end: ((_b = historicalData[historicalData.length - 1]) === null || _b === void 0 ? void 0 : _b.date) || '',
                    },
            ...
            // Cache for 1 hour
            await cache_service_1.cacheService.set(cacheKey, result, { ttl: 3600 });
            return result;
        }
        catch (error) {
            logger_1.default.error('Error in sales forecasting:', error);
            throw error;
        }
    }
    // Demand Forecasting
        ...
        try {
            ...
            const result = {
                ...
                metadata: {
                    ...
                    trainingPeriod: {
                        start: ((_a = adjustedData[0]) === null || _a === void 0 ? void 0 : _a.date) || '',
                        end: ((_b = adjustedData[adjustedData.length - 1]) === null || _b === void 0 ? void 0 : _b.date) || '',
                    },
                    generatedAt: new Date().toISOString(),
                    filters: options.filters,
                },
            ...
            logger_1.default.error('Error in demand forecasting:', error);
            throw error;
        }
    }
    // Inventory Level Forecasting
    async forecastInventory(options) {
        var _a, _b;
        ...
        try {
            ...
            logger_1.default.error('Error in inventory forecasting:', error);
            throw error;
        }
    }
    // Revenue Forecasting
    async forecastRevenue(options) {
        ...
        try {
            ...
            logger_1.default.error('Error in revenue forecasting:', error);
            throw error;
        }
    }
    // Forecasting Algorithm Implementations
    linearTrendForecasting(data, periods) {
        const n = data.length;
        if (n < 2)
            throw new Error('Insufficient data for linear trend');
    ...
    seasonalForecasting(data, periods, seasonality) {
    ...
    // Helper Methods
    calculateSeasonalIndices(data, seasonality) {
        ...
        const seasonalAverages = seasonalSums.map((sum, i) => seasonalCounts[i] > 0 ? sum / seasonalCounts[i] : 1);
        const overallAverage = seasonalAverages.reduce((a, b) => a + b) / seasonality;
        return seasonalAverages.map(avg => avg / overallAverage);
    }
    addConfidenceIntervals(forecasts, historicalData, confidence) {
    ...
    calculateSafetyStock(demandData, serviceLevel, leadTime) {
    ...
    addSeasonalityAdjustment(forecasts, historicalData) {
    ...
    getSeasonalMultiplier(data, periodIndex) {
    ...
    async getSalesHistoricalData(filters) {
        try {
            // TODO: replace with actual database queries for sales historical data
            return [];
        }
        catch (error) {
            logger_1.default.error('Error fetching sales historical data:', error);
            return [];
        }
    }
    async getDemandHistoricalData(filters) {
        try {
            // TODO: replace with actual order/demand data from database
            return [];
        }
        catch (error) {
            logger_1.default.error('Error fetching demand historical data:', error);
            return [];
        }
    }
    async getInventoryHistoricalData(filters) {
        try {
            ...
            // Apply filters
            if (filters === null || filters === void 0 ? void 0 : filters.supplierIds) {
                query = query.andWhere('inventory.supplierId IN (:...supplierIds)', { supplierIds: filters.supplierIds });
            }
            if (filters === null || filters === void 0 ? void 0 : filters.categories) {
            ...
            if (movements.length === 0) {
                ...
                const inventoryItems = await inventoryRepository.find({
                    where: (filters === null || filters === void 0 ? void 0 : filters.supplierIds) ? { supplierId: filters.supplierIds[0] } : {},
                    take: 100
                });
            ...
            movements.forEach(movement => {
                ...
                month.totalValue += movement.afterQuantity * (((_a = movement.inventory) === null || _a === void 0 ? void 0 : _a.unitCost) || 0);
                month.count += 1;
            });
            ...
            logger_1.default.error('Error fetching inventory historical data:', error);
            // Return empty data on error
            return [];
        }
    }
    async getRevenueHistoricalData(filters) {
        try {
            // TODO: replace with actual revenue data from database
            return [];
        }
        catch (error) {
            logger_1.default.error('Error fetching revenue historical data:', error);
            return [];
        }
    }
}
