...
class SellerProductService {
    constructor() {
        this.sellerProductRepository = connection_1.AppDataSource.getRepository(SellerProduct_1.SellerProduct);
    ...
    async addProductToSeller(data) {
        try {
            ...
            if (data.sellerPrice < product.recommendedPrice * 0.8) {
                logger_1.default.warn(`Seller price significantly below recommended: ${data.sellerPrice} vs ${product.recommendedPrice}`);
            }
            const sellerProduct = this.sellerProductRepository.create({
                ...data,
                status: SellerProduct_1.SellerProductStatus.ACTIVE,
            ...
            logger_1.default.info(`Product added to seller: ${data.productId} -> ${data.sellerId}`);
            return savedSellerProduct;
        }
        catch (error) {
            logger_1.default.error('Error adding product to seller:', error);
            throw error;
        }
    }
    // 판매자 제품 정보 수정
    async updateSellerProduct(sellerProductId, data) {
        try {
            ...
            logger_1.default.info(`Seller product updated: ${sellerProductId}`);
            return updatedSellerProduct;
        }
        catch (error) {
            logger_1.default.error('Error updating seller product:', error);
            throw error;
        }
    }
    // 판매자 제품 제거 (소프트 삭제)
    async removeProductFromSeller(sellerProductId) {
        try {
            const result = await this.sellerProductRepository.update(sellerProductId, {
                isActive: false,
                status: SellerProduct_1.SellerProductStatus.DISCONTINUED
            });
            if (result.affected === 0) {
                throw new Error('Seller product not found');
            }
            logger_1.default.info(`Product removed from seller: ${sellerProductId}`);
            return true;
        }
        catch (error) {
            logger_1.default.error('Error removing product from seller:', error);
            throw error;
        }
    }
    // 판매자 제품 목록 조회
    async getSellerProducts(filters = {}) {
        try {
            const { sellerId, productId, supplierId, categoryId, status, isActive, inStock, priceMin, priceMax, search, tags, sortBy = 'addedAt', sortOrder = 'desc', page = 1, limit = 20 } = filters;
            ...
            if (search) {
                queryBuilder.andWhere('(product.name ILIKE :search OR product.description ILIKE :search OR sp.customTitle ILIKE :search)', { search: `%${search}%` });
            }
            ...
            logger_1.default.error('Error fetching seller products:', error);
            throw error;
        }
    }
    // 판매자가 추가 가능한 제품 목록 조회
    async getAvailableProducts(sellerId, filters = {}) {
        try {
            const seller = await this.sellerRepository.findOne({
                where: { id: sellerId }
            });
            ...
            const queryBuilder = this.productRepository
                ...
                .leftJoinAndSelect('product.supplier', 'supplier')
                .leftJoinAndSelect('product.category', 'category')
                .where('product.status = :status', { status: Product_1.ProductStatus.ACTIVE })
            ...
            logger_1.default.error('Error fetching available products:', error);
            throw error;
        }
    }
    // 대량 제품 추가
    async bulkAddProducts(data) {
        try {
            const seller = await this.sellerRepository.findOne({
                where: { id: data.sellerId, isActive: true }
            });
            ...
            const existingSellerProducts = await this.sellerProductRepository.find({
                where: { sellerId: data.sellerId, productId: (0, typeorm_1.In)(productIds) }
            });
            ...
            logger_1.default.info(`Bulk products added to seller: ${savedSellerProducts.length} products -> ${data.sellerId}`);
            return savedSellerProducts;
        }
        catch (error) {
            logger_1.default.error('Error bulk adding products:', error);
            throw error;
        }
    }
    // 판매자 제품 수익성 분석
    async analyzeProfitability(sellerProductId) {
        try {
            const sellerProduct = await this.sellerProductRepository.findOne({
                where: { id: sellerProductId },
                relations: ['product', 'seller']
            ...
            logger_1.default.error('Error analyzing profitability:', error);
            throw error;
        }
    }
    // 판매자 제품 재고 동기화 (공급자 재고와 동기화)
    async syncInventory(sellerId) {
        try {
            ...
            for (const sellerProduct of sellerProducts) {
                ...
                if ((sellerProduct.sellerInventory || 0) !== availableInventory) {
                    ...
                    if (availableInventory === 0) {
                        sellerProduct.status = SellerProduct_1.SellerProductStatus.OUT_OF_STOCK;
                        outOfStock++;
                    }
                    else if (sellerProduct.status === SellerProduct_1.SellerProductStatus.OUT_OF_STOCK) {
                        sellerProduct.status = SellerProduct_1.SellerProductStatus.ACTIVE;
                    }
                    await this.sellerProductRepository.save(sellerProduct);
                    updated++;
                }
            }
            logger_1.default.info(`Inventory sync completed for seller ${sellerId}: ${updated} updated, ${outOfStock} out of stock`);
            return { updated, outOfStock };
        }
        catch (error) {
            logger_1.default.error('Error syncing inventory:', error);
            throw error;
        }
    }
    // 판매자 제품 통계
    async getSellerProductStats(sellerId) {
        try {
            const stats = await this.sellerProductRepository
                ...
                .setParameters({
                active: SellerProduct_1.SellerProductStatus.ACTIVE,
                inactive: SellerProduct_1.SellerProductStatus.INACTIVE,
                outOfStock: SellerProduct_1.SellerProductStatus.OUT_OF_STOCK
            })
                .getRawOne();
            return {
            ...
            logger_1.default.error('Error fetching seller product stats:', error);
            throw error;
        }
    }
    // 판매자 제품 성과 분석 (베스트셀러, 수익성 등)
    async getSellerProductPerformance(sellerId, limit = 10) {
        try {
            // 베스트셀러 제품
            ...
            logger_1.default.error('Error fetching seller product performance:', error);
            throw error;
        }
    }
}
exports.SellerProductService = SellerProductService;
exports.default = SellerProductService;
//# sourceMappingURL=SellerProductService.js.map