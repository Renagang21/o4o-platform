...
class AIUsageReportService {
    constructor() {
        this.queue = ai_job_queue_service_1.aiJobQueue.getQueue();
    ...
    async generateReport(startDate, endDate, options) {
        try {
            ...
            const filteredJobs = completedJobs.filter(job => {
                if (!job.finishedOn)
                    return false;
                if (job.finishedOn < startTime || job.finishedOn > endTime)
                    return false;
                // Apply optional filters
                if ((options === null || options === void 0 ? void 0 : options.userId) && job.data.userId !== options.userId)
                    return false;
                if ((options === null || options === void 0 ? void 0 : options.provider) && job.data.provider !== options.provider)
                    return false;
                if ((options === null || options === void 0 ? void 0 : options.model) && job.data.model !== options.model)
                    return false;
                return true;
            });
            // Extract usage data
            const usageEntries = filteredJobs
                .map(job => {
                ...
                const estimatedCost = this.estimateCost(job.data.provider, job.data.model, promptTokens, completionTokens);
                return {
            ...
            if (options === null || options === void 0 ? void 0 : options.topUsersLimit) {
            ...
            logger_1.default.info('Generated usage report', {
            ...
            logger_1.default.error('Failed to generate usage report', { error: error.message });
            throw error;
        }
    }
    /**
    ...
    estimateCost(provider, model, promptTokens, completionTokens) {
        var _a;
        const pricing = (_a = TOKEN_PRICING[provider]) === null || _a === void 0 ? void 0 : _a[model];
        if (!pricing) {
            logger_1.default.warn('No pricing data for provider/model', { provider, model });
            return 0;
        }
        // Cost per 1K tokens
    ...
    async cleanup() {
        ...
        logger_1.default.info('AI usage report service cleaned up');
    }
}
exports.aiUsageReport = AIUsageReportService.getInstance();
//# sourceMappingURL=ai-usage-report.service.js.map