...
exports.permalinkService = exports.PermalinkService = void 0;
...
class PermalinkService {
    constructor() {
        ...
        this.customPostTypeRepository = connection_1.AppDataSource.getRepository(CustomPostType_1.CustomPostType);
        // URL 구조 패턴 정의 (보완된 버전)
        this.patterns = {
            '%year%': '(\\d{4})',
            '%monthnum%': '(\\d{1,2})',
     ...
     * Permalink 설정 저장
     */
    async savePermalinkSettings(settings) {
        try {
            // 설정 검증
            ...
            logger_1.default.info('Permalink settings saved successfully', {
            ...
            logger_1.default.error('Failed to save permalink settings:', error);
     ...
     * URL 생성 (향상된 버전)
     */
    async generatePostUrl(post, structure) {
        var _a, _b;
        ...
        url = url.replace(/%author%/g, ((_b = (_a = post.author) === null || _a === void 0 ? void 0 : _a.name) === null || _b === void 0 ? void 0 : _b.toLowerCase().replace(/\s+/g, '-')) || 'author');
        // 카테고리 처리
        ...
        if (url.length > settings.maxUrlLength) {
            logger_1.default.warn(`Generated URL exceeds max length: ${url} (${url.length} > ${settings.maxUrlLength})`);
        }
    ...
    async parseUrl(path, structure) {
        ...
        try {
            ...
            logger_1.default.error('Failed to parse URL:', error);
            return null;
        }
    }
    /**
     ...
     */
    async generateUrlPreviews(structure) {
        const previews = [];
        // 게시글 미리보기
     ...
     * 설정 검증 (보완된 버전)
     */
    async validatePermalinkSettings(settings) {
        const errors = [];
        // 구조 검증
     ...
     * 리다이렉트 규칙 생성
     */
    async generateRedirectRules(oldStructure, newStructure) {
        const rules = [];
        try {
            // 게시글들의 리다이렉트 규칙 생성
            ...
            logger_1.default.info(`Generated ${rules.length} redirect rules for structure change`);
            return rules;
        }
        catch (error) {
            logger_1.default.error('Failed to generate redirect rules:', error);
            return [];
        }
    }
    // Private helper methods
    getDefaultPermalinkSettings() {
        return {
            structure: '/%postname%/',
    ...
    async generatePostPreview(structure) {
        // 샘플 포스트로 미리보기 생성
        const samplePost = {
    ...
    generateCategoryPreview(structure) {
    ...
    calculateSeoScore(structure, exampleUrl) {
        let score = 100;
        // 날짜 포함 시 점수 감점
    ...
    generateSeoWarnings(structure, exampleUrl) {
        const warnings = [];
        if (structure.includes('%year%') || structure.includes('%monthnum%') || structure.includes('%day%')) {
    ...
    async checkCustomPostTypeConflicts(settings) {
        var _a;
        const conflicts = [];
        try {
            ...
            for (const cpt of customPostTypes) {
                // rewrite slug 충돌 검사
                const rewriteSlug = ((_a = cpt.rewrite) === null || _a === void 0 ? void 0 : _a.slug) || cpt.slug;
                if (rewriteSlug) {
            ...
            logger_1.default.error('Failed to check custom post type conflicts:', error);
        }
        return conflicts;
    }
}
exports.PermalinkService = PermalinkService;
exports.permalinkService = new PermalinkService();
//# sourceMappingURL=permalink.service.js.map