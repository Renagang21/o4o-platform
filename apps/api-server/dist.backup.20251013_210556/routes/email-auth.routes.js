...
router.post('/register', registerValidation, validateDto_1.validateDto, async (req, res, next) => {
    try {
        ...
        const user = userRepository.create({
            ...
            name,
            status: auth_1.UserStatus.PENDING,
            businessInfo: null
        });
...
router.post('/login', loginValidation, validateDto_1.validateDto, async (req, res, next) => {
    try {
        ...
        const lockStatus = await refreshToken_service_1.refreshTokenService.checkAccountLock(email);
        if (lockStatus.locked) {
            await refreshToken_service_1.refreshTokenService.trackLoginAttempt(email, false, ipAddress, userAgent, deviceId, 'Account locked');
        ...
        if (!isPasswordValid) {
            ...
            await refreshToken_service_1.refreshTokenService.trackLoginAttempt(email, false, ipAddress, userAgent, deviceId, 'Invalid password');
            return res.status(401).json({
                success: false,
                message: '이메일 또는 비밀번호가 올바르지 않습니다',
                error: {
        ...
        if (user.status === auth_1.UserStatus.PENDING) {
        ...
        if (user.status === auth_1.UserStatus.REJECTED || user.status === auth_1.UserStatus.INACTIVE) {
            return res.status(403).json({
                success: false,
        ...
        await refreshToken_service_1.refreshTokenService.trackLoginAttempt(email, true, ipAddress, userAgent, deviceId);
        // Generate tokens
        const jwtPayload = {
            userId: user.id,
            email: user.email,
            role: user.role || auth_1.UserRole.CUSTOMER,
            emailVerified: true
        };
        const accessToken = (0, auth_utils_1.generateAccessToken)(jwtPayload);
        const refreshToken = rememberMe ?
            await refreshToken_service_1.refreshTokenService.generateRefreshToken(user, deviceId, userAgent, ipAddress) :
            undefined;
        // Update last login
...
router.post('/verify-email', (0, express_validator_1.body)('token').notEmpty().withMessage('토큰이 필요합니다'), validateDto_1.validateDto, async (req, res, next) => {
    try {
        ...
        user.status = auth_1.UserStatus.APPROVED;
        await userRepository.save(user);
        // Mark token as used
...
router.post('/refresh', (0, express_validator_1.body)('refreshToken').notEmpty().withMessage('리프레시 토큰이 필요합니다'), validateDto_1.validateDto, async (req, res, next) => {
    try {
        ...
        const result = await refreshToken_service_1.refreshTokenService.refreshAccessToken(refreshToken);
        if (result.error) {
            return res.status(401).json({
                success: false,
...
router.post('/logout', auth_middleware_1.authenticate, async (req, res, next) => {
    ...
    try {
        ...
        if (refreshToken) {
            ...
            await refreshToken_service_1.refreshTokenService.revokeToken(refreshToken, 'User logout');
        }
        else if (userId) {
            // Revoke all user tokens if no specific token provided
            await refreshToken_service_1.refreshTokenService.revokeAllUserTokens(userId, 'User logout');
        }
        res.json({
            success: true,
            message: '로그아웃되었습니다'
