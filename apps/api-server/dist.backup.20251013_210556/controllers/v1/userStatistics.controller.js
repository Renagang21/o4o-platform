...
class UserStatisticsController {
    static async getUserStatistics(req, res) {
        try {
            ...
            // User counts by role
            const usersByRole = await Promise.all(Object.values(User_1.UserRole).map(async (role) => ({
                role,
            ...
            const usersByStatus = await Promise.all(Object.values(User_1.UserStatus).map(async (status) => ({
                status,
                count: await UserStatisticsController.userRepository.count({ where: { status } }),
                label: status.charAt(0).toUpperCase() + status.slice(1).replace('_', ' ')
            })));
            // Beta user statistics
            const betaUsersByStatus = await Promise.all(Object.values(BetaUser_1.BetaUserStatus).map(async (status) => ({
                status,
    ...
    static async getUserRegistrationStats(days) {
        const startDate = new Date();
    ...
    static async getActivityStatistics(days) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        // Activities by category
        const activitiesByCategory = await Promise.all(Object.values(UserActivityLog_1.ActivityCategory).map(async (category) => ({
            category,
        ...
        return {
            ...
            topTypes: topActivityTypes.map(t => ({
                ...
                label: t.type.split('_').map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
            }))
        };
    }
    static async getGeographicDistribution(days) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
    ...
    static async getTopActiveUsers(days, limit) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
    ...
    static async getSecurityStatistics(days) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        ...
        const failedLogins = await UserStatisticsController.activityRepository
            .createQueryBuilder('activity')
            .where('activity.activityType = :type', { type: UserActivityLog_1.ActivityType.LOGIN })
    ...
    static async getUserGrowthTrend(req, res) {
        try {
    ...
    static async getRetentionStatistics(req, res) {
        try {
            const { cohortMonths = '6' } = req.query;
            const monthsNum = parseInt(cohortMonths, 10);
            ...
            for (let i = 0; i < monthsNum; i++) {
                ...
                const cohortUsers = await UserStatisticsController.userRepository.count({
                    where: {
                        createdAt: (0, typeorm_1.Between)(cohortStart, cohortEnd)
                    }
                });
...
UserStatisticsController.userRepository = data_source_1.default.getRepository(User_1.User);
UserStatisticsController.activityRepository = data_source_1.default.getRepository(UserActivityLog_1.UserActivityLog);
UserStatisticsController.betaUserRepository = data_source_1.default.getRepository(BetaUser_1.BetaUser);
//# sourceMappingURL=userStatistics.controller.js.map