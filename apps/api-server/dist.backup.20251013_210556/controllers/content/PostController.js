...
class PostController {
    constructor() {
        ...
        this.getPosts = async (req, res) => {
            try {
                const { page = 1, limit = 10, status, search, authorId, categoryId, tag, orderBy = 'createdAt', order = 'DESC', includeDrafts = false } = req.query;
        ...
        this.getPost = async (req, res) => {
            ...
            try {
                ...
                if (includeRevisions === 'true') {
                    revisions = await revision_service_1.revisionService.getPostRevisions(id, 10);
                }
                ...
                if (process.env.NODE_ENV === 'development') {
                    logger_1.default.info('API Response Structure:', {
                        hasSuccess: 'success' in responseData,
                        hasData: 'data' in responseData,
                        hasPost: responseData.data && 'post' in responseData.data,
                        slugValue: (_b = (_a = responseData.data) === null || _a === void 0 ? void 0 : _a.post) === null || _b === void 0 ? void 0 : _b.slug
                    });
                }
                res.json(responseData);
            }
            catch (error) {
                logger_1.default.error('Error getting post:', error);
                res.status(500).json({
                    success: false,
        ...
        this.createPost = async (req, res) => {
            var _a;
            try {
                const { title, slug, content, excerpt, status = 'draft', type = 'post', format = 'standard', template, categoryIds = [], tagNames = [], seo = {}, customFields = {}, postMeta = {}, publishedAt, scheduledAt, password, allowComments = true, featured = false, sticky = false, featuredImage } = req.body;
                const authorId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id; // From auth middleware
                if (!authorId) {
                    res.status(401).json({
                        success: false,
                ...
                if (categoryIds.length > 0) {
                    categories = await this.categoryRepository.findBy({ id: (0, typeorm_1.In)(categoryIds) });
                }
                // Handle tags
                let tags = [];
                ...
                await revision_service_1.revisionService.createPostRevision(savedPost, {
        ...
        this.updatePost = async (req, res) => {
            ...
            try {
                ...
                await revision_service_1.revisionService.createPostRevision(post, {
                    entityId: id,
                    entityType: 'post',
                    authorId: userId,
                    revisionType: 'manual',
                ...
                if (updates.categoryIds) {
                    const categories = await this.categoryRepository.findBy({ id: (0, typeorm_1.In)(updates.categoryIds) });
        ...
        this.restorePost = async (req, res) => {
            var _a;
            try {
                const { id } = req.params;
                const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                ...
                logger_1.default.error('Error restoring post:', error);
                res.status(500).json({
                    success: false,
        ...
        this.bulkAction = async (req, res) => {
            ...
            try {
                ...
                const posts = await this.postRepository.findBy({ id: (0, typeorm_1.In)(postIds) });
                if (posts.length === 0) {
                    res.status(404).json({
                        success: false,
                ...
                logger_1.default.error('Error in bulk action:', error);
                res.status(500).json({
                    success: false,
        ...
        this.getRevisions = async (req, res) => {
            try {
                const { id } = req.params;
                ...
                const revisions = await revision_service_1.revisionService.getPostRevisions(id, Number(limit));
                res.json({
                    success: true,
                    data: { revisions }
                });
                ...
                logger_1.default.error('Error getting revisions:', error);
                res.status(500).json({
                    success: false,
        ...
        this.restoreRevision = async (req, res) => {
            ...
            try {
                ...
                const restoredPost = await revision_service_1.revisionService.restorePostRevision(id, revisionId, userId);
                // Load complete post with relations
                const completePost = await this.postRepository
                    .createQueryBuilder('post')
                    .leftJoinAndSelect('post.author', 'author')
                ...
                logger_1.default.error('Error restoring revision:', error);
                res.status(500).json({
                    success: false,
        ...
        this.autoSave = async (req, res) => {
            ...
            try {
                ...
                const result = await revision_service_1.revisionService.autoSaveContent(id, 'post', content, userId);
                res.json({
                ...
                logger_1.default.error('Error auto-saving:', error);
                res.status(500).json({
                    success: false,
        ...
        this.getPreview = async (req, res) => {
            try {
                ...
                if (revisionId) {
                    ...
                    const revisionRepository = connection_1.AppDataSource.getRepository('PostRevision');
                ...
                logger_1.default.error('Error getting preview:', error);
                res.status(500).json({
                    success: false,
        ...
        this.duplicatePost = async (req, res) => {
            var _a;
            try {
                const { id } = req.params;
                ...
                const slug = await slug_service_1.slugService.ensureUniquePostSlug(duplicateTitle);
                ...
                logger_1.default.error('Error duplicating post:', error);
                res.status(500).json({
                    success: false,
    ...
    formatPostResponse(post) {
        ...
        return {
            ...
            categories: ((_a = post.categories) === null || _a === void 0 ? void 0 : _a.map((cat) => ({
                id: cat.id,
                name: cat.name,
                slug: cat.slug,
                description: cat.description
            }))) || [],
            tags: ((_b = post.tags) === null || _b === void 0 ? void 0 : _b.map((tag) => ({
                id: tag.id,
                name: tag.name,
                slug: tag.slug,
                color: tag.color
    ...
    async getOrCreateTags(tagNames) {
        ...
        for (const tagName of tagNames) {
            ...
            if (!tag) {
                const slug = await slug_service_1.slugService.ensureUniqueTagSlug(tagName);
