...
class MigrationController {
    ...
    async initializeDropshippingSystem(req, res) {
        const queryRunner = connection_1.AppDataSource.createQueryRunner();
        try {
            await queryRunner.connect();
            await queryRunner.startTransaction();
            const postTypeRepo = queryRunner.manager.getRepository(CustomPostType_1.CustomPostType);
            const fieldGroupRepo = queryRunner.manager.getRepository(CustomField_1.FieldGroup);
            const customFieldRepo = queryRunner.manager.getRepository(CustomField_1.CustomField);
            // 1. Create CPTs
            ...
            for (const cptDef of cptDefinitions) {
                ...
                if (!postType) {
                    ...
                    logger_1.default.info(`Created CPT: ${cptDef.slug}`);
                }
            }
            ...
            for (const groupDef of fieldGroups) {
                ...
                if (!fieldGroup) {
                    fieldGroup = fieldGroupRepo.create({
                        ...
                        placement: groupDef.placement,
                        active: true,
                        order: 0
                    });
                    await fieldGroupRepo.save(fieldGroup);
                    logger_1.default.info(`Created field group: ${groupDef.title}`);
                    // Create fields
                    for (let i = 0; i < groupDef.fields.length; i++) {
                        ...
                        logger_1.default.info(`Created field: ${fieldDef.name}`);
                    }
                }
            }
    ...
    async createSampleData(req, res) {
        try {
            ...
            const adminUser = await userRepo.findOne({ where: { role: auth_1.UserRole.ADMIN } });
            if (!adminUser) {
    ...
    async verifySystemStatus(req, res) {
        try {
            const postTypeRepo = connection_1.AppDataSource.getRepository(CustomPostType_1.CustomPostType);
            const customPostRepo = connection_1.AppDataSource.getRepository(CustomPost_1.CustomPost);
            const fieldGroupRepo = connection_1.AppDataSource.getRepository(CustomField_1.FieldGroup);
