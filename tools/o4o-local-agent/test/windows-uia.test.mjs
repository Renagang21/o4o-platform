...
test('경계(소스): uia.mjs 는 fs/child_process/네트워크 0 · 스크립트는 Start-Process/Invoke/-Command 0 · pid 는 등재 process 이름과 대조 · 키 4 · 텍스트 500', () => {
  ...
  // execFile 지점은 여전히 windows-window-control 하나 · 스크립트 7개
  const control = codeOnly(read('windows-window-control.mjs'));
  const scripts = [...control.matchAll(/'([\w-]+\.ps1)'/g)].map((m) => m[1]).sort();
  assert.deepEqual(scripts, ['windows-app-launch.ps1', 'windows-browser-open.ps1', 'windows-computer-input.ps1', 'windows-computer-inspect.ps1', 'windows-uia.ps1', 'windows-window-activate.ps1', 'windows-window-census.ps1']);
});
