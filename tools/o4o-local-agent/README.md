...
| 클립보드 읽기 · 쓰기 | 해당 모듈 없음 |
| 브라우저 안 제어 (탭 · DOM · 클릭 · 입력) | agent 자체에는 여전히 없습니다. WO-O4O-BROWSER-DOM-CONTROL-V0 부터 **등재 site 탭에 한해** Chrome 확장(content script)이 수행하고, agent 는 인자 재검증 · 전달 · 결과 자르기만 합니다(`browser-dom-limits.mjs` · `bridge-relay.mjs`). selector · JS · URL 을 만들거나 실행하는 코드는 agent 에 없습니다 |
