  ...
  try {
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      if (isPaid) {
        ...
        // Add payment amount (should match order total)
        metaData.payment_amount = metaData.total;
      }
    ...
    const samples = await dataSource.query(`
      SELECT 
        ...
        meta_data->>'total' as total_amount
      FROM custom_posts 
      WHERE cpt_slug = 'ds_order'
      ORDER BY id DESC
      LIMIT 5
    ...
    console.log('\n=== Sample Orders (Recent) ===');
    samples.forEach(order => {
      console.log(`Order #${order.id}:`);
      console.log(`  Status: ${order.payment_status}`);
      console.log(`  Method: ${order.payment_method}`);
      console.log(`  Amount: ₩${parseInt(order.total_amount).toLocaleString('ko-KR')}`);
      if (order.payment_date) {
        console.log(`  Paid at: ${new Date(order.payment_date).toLocaleString('ko-KR')}`);
      }
