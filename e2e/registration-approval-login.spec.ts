...
const services: ServiceConfig[] = [
  ...
  {
    name: 'GlucoseView',
    url: 'https://glucoseview.co.kr',
    registerPath: '/register',
    loginPath: '/login',
    operatorEmail: 'admin-glucoseview@o4o.com',
    operatorPassword: 'O4oTestPass',
    approvalPath: '/operator/glucoseview/users',
    fillRegister: async (page: Page) => {
      // GlucoseView uses placeholder-based inputs (no name attributes)
      await fillField(page, 'input[placeholder*="12345"]', '88888');
      await fillField(page, 'input[placeholder*="홍길동"]', '테스트사용자');
      await fillField(page, 'input[placeholder*="사이트에서"]', '테스트약사GV');
      await fillField(page, 'input[placeholder*="하이픈"], input[placeholder*="숫자만 입력"]', '01012345678');
      await fillField(page, 'input[placeholder*="example@email"]', TEST_USER.email);
      // Password fields - by placeholder
      await fillField(page, 'input[placeholder*="영문, 숫자, 특수문자"]', TEST_USER.password);
      await fillField(page, 'input[placeholder*="비밀번호 재입력"]', TEST_USER.password);
      // Branch/Chapter selection
      const selects = page.locator('select');
      const selectCount = await selects.count();
      if (selectCount >= 1) {
        const branchSelect = selects.nth(0);
      ...
      if (selectCount >= 2) {
        const chapterSelect = selects.nth(1);
        await page.waitForTimeout(1000);
        const options2 = await chapterSelect.locator('option').all();
        if (options2.length > 1) {
          await chapterSelect.selectOption({ index: 1 });
        }
      }
      await fillField(page, 'input[placeholder*="약국 이름"]', '테스트약국E2E');
      // Check terms of service checkboxes
      const checkboxes = page.locator('input[type="checkbox"]');
      const cbCount = await checkboxes.count();
      for (let i = 0; i < cbCount; i++) {
        if (!(await checkboxes.nth(i).isChecked())) {
    ...
    navigateToApproval: async (page: Page) => {
      await page.goto('https://glucoseview.co.kr/operator/glucoseview/users', { waitUntil: 'networkidle', timeout: 30000 });
      await page.waitForTimeout(3000);
      const pendingTab = page.locator('button:has-text("가입 신청")');
      if (await pendingTab.isVisible({ timeout: 5000 }).catch(() => false)) {
        await pendingTab.click();
