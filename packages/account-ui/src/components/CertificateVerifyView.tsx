/**
 * CertificateVerifyView — 수료증 공개 검증 공통 View
 *
 * 원본: KPA `CertificateVerifyPage` (WO-O4O-LMS-CERTIFICATE-VERIFICATION-V1)
 * 추출: WO-O4O-PHARMACYHUB-LMS-LEARNER-FULL-ADOPTION-V1 §10·§20
 *
 * 수료증 PDF 에 인쇄되는 검증 URL 은 `{서비스 도메인}/certificate/verify/{id}` 이며
 * (`CertificateController.downloadPdf`), 이 화면은 모든 서비스에서 동일하다.
 * 서비스별 복제 대신 공통 View 로 두고, 서비스는 route + API base 만 넘긴다.
 *
 * 공개 화면이다 — 인증을 요구하지 않으며 백엔드 `GET /lms/certificates/:id/verify`
 * 가 최소 필드만 반환한다(PII 비노출 계약은 백엔드가 보장).
 */
...
export interface CertificateVerifyViewProps {
  /** route param 에서 넘긴 수료증 id */
  certificateId?: string;
  /** 서비스 API base (예: `import.meta.env.VITE_API_BASE_URL`) */
  apiBaseUrl: string;
}

export function CertificateVerifyView({ certificateId, apiBaseUrl }: CertificateVerifyViewProps) {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        const res = await fetch(`${apiBaseUrl}/api/v1/lms/certificates/${certificateId}/verify`);
