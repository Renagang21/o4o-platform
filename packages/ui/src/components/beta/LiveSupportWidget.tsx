...
export const LiveSupportWidget: React.FC<LiveSupportWidgetProps> = ({
  ...
  useEffect(() => {
    ...
    socketInstance.on('error', (error: any) => {
      ...
      onShowToast('연결 오류가 발생했습니다.', 'error');
    });
    ...
    socketInstance.on('feedback:live_support_started', (data: { feedbackId: string, conversationId: string }) => {
      ...
      onShowToast('실시간 지원이 시작되었습니다. 곧 상담원이 연결됩니다.', 'success');
  ...
  }, [betaUserEmail, betaUserId, onShowToast]);

  const startLiveSupport = async () => {
    if (!feedbackId) {
      onShowToast('피드백 ID가 필요합니다.', 'error');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setIsLiveSupport(true);
        onShowToast('실시간 지원을 요청했습니다. 상담원 연결을 기다려주세요.', 'success');
      } else {
        onShowToast(data.error?.message || '실시간 지원 요청에 실패했습니다.', 'error');
      }
    } catch (error) {
...
export const EnhancedBetaFeedbackWidget: React.FC<EnhancedBetaFeedbackWidgetProps> = ({
  ...
  const handleFeedbackSubmitted = (submittedFeedbackId: string) => {
    ...
    onShowToast('피드백이 제출되었습니다. 실시간 지원을 요청할 수 있습니다.', 'success');
  };
  ...
  const handleBetaRegistered = (userId: string, email: string) => {
    ...
    onShowToast('베타 등록이 완료되었습니다!', 'success');
  };
            ...
            <div className="p-4">
              ...
              <button
                onClick={() => {
                  ...
                  onShowToast('피드백 모달을 구현해주세요', 'info');
                }}
