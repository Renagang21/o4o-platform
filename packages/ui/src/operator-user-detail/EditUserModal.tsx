...
export default function EditUserModal({ userId, apiAdapter, theme, onClose, onSuccess }: EditUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiAdapter.put(`/operator/members/${userId}`, payload);
      onSuccess();
      onClose();
    } catch (err: any) {
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          <div className="text-center py-8">
            <Loader2 className={`w-6 h-6 ${loaderColor} animate-spin mx-auto mb-2`} />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" name="lastName" value={form.lastName} onChange={handleChange}
                      className={`w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 ${ringColor}`} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" name="taxEmail" value={form.taxEmail} onChange={handleChange}
                        className={`w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 ${ringColor}`}
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button type="submit" disabled={saving}
                className={`flex-1 py-2 text-sm ${btnBg} text-white rounded-lg disabled:opacity-50`}>
                {saving ? '저장 중...' : '저장'}
              </button>
            </div>
          </form>
