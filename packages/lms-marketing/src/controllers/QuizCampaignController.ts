 ...
 *
 * QuizCampaign CRUD 및 캠페인 관리 API
...
export class QuizCampaignController {
  private service: QuizCampaignService;

  constructor(dataSource: DataSource) {
    this.service = new QuizCampaignService(dataSource);
  }
  ...
  async findById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!entity) {
        res.status(404).json({ success: false, error: 'QuizCampaign not found' });
        return;
      }
  ...
  async recordParticipation(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      await this.service.recordParticipation(id);
  ...
  async recordCompletion(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { score } = req.body;
      await this.service.recordCompletion(id, score || 0);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ success: false, error: error.message });
    }
