/**
 * Cosmetics Partner Extension Lifecycle Hooks
 *
...
export async function install(context: InstallContext): Promise<void> {
  const { appId } = context;
  console.log(`[${appId}] cosmetics-partner-extension installed.`);
}
...
export async function activate(context: ActivateContext): Promise<ActivateResult> {
  const { appId } = context;
  console.log(`[${appId}] cosmetics-partner-extension activated.`);
...
export async function deactivate(context: DeactivateContext): Promise<void> {
  const { appId } = context;
  console.log(`[${appId}] cosmetics-partner-extension deactivated.`);
}
...
export async function uninstall(context: UninstallContext): Promise<void> {
  const { appId } = context;
  console.log(`[${appId}] cosmetics-partner-extension uninstalled.`);
}
