/**
 * StoreQrConsoleView — 내 매장(약국) QR 콘솔 (공통 화면 본체)
 ...
 * QR URL(`/qr/{slug}`) · 슬러그 생성 규칙 · landing type 집합 · 다운로드 포맷은 동일하므로
 * 공통 본체가 그대로 보존한다(identifier/URL 의미 변경 금지).
 *
 * ⚠️ store-ui-core 에 새 dependency 를 만들지 않는다.
 *    getAccessToken(@o4o/auth-client) · API_BASE_URL · ProductionTemplate 은 전부 주입/구조적 타입이다.
 */
...
export interface StoreQrItem {
  ...
  landingType: StoreQrLandingType;
...
export interface StoreQrCreateInput {
  title: string;
  description?: string;
  landingType: StoreQrLandingType;
...
export interface StoreQrConsoleApi {
  ...
  create: (input: StoreQrCreateInput) => Promise<void>;
  update: (id: string, input: StoreQrCreateInput) => Promise<void>;
  remove: (id: string) => Promise<void>;
  /** PNG 512 이미지 blob (format=png&size=512) */
  downloadImage: (id: string) => Promise<Blob>;
}

/** 서비스 productionTemplates registry 의 구조적 부분집합 */
export interface StoreQrTemplate {
  name: string;
}

/** 서비스 accent — 원본 값을 그대로 주입한다(공통 기본값으로 치환하지 않는다) */
export interface StoreQrConsoleTheme {
  /** 아이콘·주요 버튼 배경·선택된 유형 테두리 */
  accent: string;
  /** 카드 아이콘 배경·선택된 유형 배경 */
  accentSoft: string;
}

export interface StoreQrConsoleLabels {
  /** 헤더 부제 — '매장 QR 코드 생성 및 관리' */
  subtitlePrefix: string;
  /** 비어있을 때 안내 — '새 QR 코드를 만들어 매장 자료, 홈페이지, ...' */
  emptyHint: string;
}

export interface StoreQrConsoleViewProps {
  api: StoreQrConsoleApi;
  theme: StoreQrConsoleTheme;
  labels: StoreQrConsoleLabels;
  /** 제작 시작에서 넘어온 templateId 해석 (서비스별 registry) */
  findTemplate?: (templateId: string) => StoreQrTemplate | null | undefined;
}
...
export function StoreQrConsoleView({ api, theme, labels, findTemplate }: StoreQrConsoleViewProps) {
  const navigate = useNavigate();
  const location = useLocation();

  const [items, setItems] = useState<StoreQrItem[]>([]);
  ...
  const [selectedTemplate] = useState<StoreQrTemplate | null>(() => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const input: StoreQrCreateInput = {
  ...
  const handleDelete = async (qr: StoreQrItem) => {
  ...
  const handleDownload = async (qr: StoreQrItem) => {
    setDownloadingId(qr.id);
    try {
      const blob = await api.downloadImage(qr.id);
  ...
  const handleCopy = async (qr: StoreQrItem) => {
  ...
  const createBtnStyle: React.CSSProperties = { ...createBtnBaseStyle, backgroundColor: theme.accent };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>
            {labels.subtitlePrefix} — 총 {items.length}개
          </p>
        </div>
          ...
          {items.map((qr) => {
            ...
            return (
              <div key={qr.id} style={qrCardStyle}>
                <div style={{ width: 48, height: 48, borderRadius: 10, backgroundColor: theme.accentSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <QrCode size={22} color={theme.accent} />
                </div>
          ...
          <div style={modalStyle}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                ...
                {selectedTemplate && (
                  <p style={{ fontSize: 12, color: theme.accent, margin: '2px 0 0' }}>{selectedTemplate.name}</p>
                )}
              </div>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div style={fieldGroupStyle}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {(Object.keys(LANDING_TYPE_CONFIG) as StoreQrLandingType[]).map((type) => (
                    <button
                      key={type}
                      type="button"
                      ...
                      style={{
                        padding: '6px 14px',
                        borderRadius: 8,
                        border: `2px solid ${form.landingType === type ? theme.accent : '#e2e8f0'}`,
                        backgroundColor: form.landingType === type ? theme.accentSoft : '#fff',
                        color: form.landingType === type ? theme.accent : '#64748b',
...
/* ─── Styles (원본 값 그대로. accent 만 theme 주입) ───────────── */
...
const createBtnBaseStyle: React.CSSProperties = {
