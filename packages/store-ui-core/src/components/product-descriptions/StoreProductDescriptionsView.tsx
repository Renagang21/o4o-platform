/**
 * StoreProductDescriptionsView — 내 매장(약국) 상품 상세설명 관리 (공통 화면 본체)
 * WO-O4O-MY-STORE-CROSSSERVICE-FINAL-COMMONIZATION-AUDIT-AND-CLEANUP-V1
 *
 * 원본 계약 유지:
 *   WO-O4O-STORE-PRODUCT-DESCRIPTION-OWNERSHIP-ALIGNMENT-V1 (소유=매장 자체 상품 · 실패/0건 구분 · 저장 실패 시 내용 보존)
...
export interface StoreDescriptionTemplate {
  name: string;
  style?: string | null;
...
export interface StoreProductDescriptionsApi {
  fetchLocalProducts: (params: {
    page?: number;
    limit?: number;
    activeOnly?: string;
  }) => Promise<{ items: StoreDescriptionProduct[] }>;
  updateLocalProduct: (
    id: string,
...
export function StoreProductDescriptionsView({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<StoreDescriptionTemplate | null>(null);
   ...
   */
  const readDetailHtml = (p: StoreDescriptionProduct | null | undefined): string =>
  ...
  useEffect(() => {
    ...
    // findTemplate 은 서비스 모듈 상수라 재생성되지 않는다(원본 동작 유지).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.state]);
                ...
                <div style={styles.editorMeta}>
                  {hasExisting ? (
                    <span style={styles.metaText}>
                      저장: {new Date(selectedProduct.updated_at ?? Date.now()).toLocaleString('ko-KR')}
                    </span>
                  ) : (
