/**
 * ProductMarketingView — 상품 마케팅 자산 그래프 (공통 화면 본체)
...
export interface ProductMarketingViewProps {
  /** 서비스 API adapter — endpoint 는 서비스 소유 */
  getProductMarketing: (productId: string) => Promise<{ success: boolean; data: ProductMarketingData }>;
  unlinkProductMarketingAsset: (productId: string, linkId: string) => Promise<{ success: boolean }>;
}
...
export function ProductMarketingView({
  getProductMarketing,
  unlinkProductMarketingAsset,
}: ProductMarketingViewProps) {
  ...
  if (!data) {
    return (
      <div style={styles.container}>
        <div style={styles.loadingState}>
          <Link2 size={48} style={{ color: c.neutral300 }} />
          <p style={{ color: c.neutral500, fontSize: '14px', marginTop: '12px' }}>데이터를 불러올 수 없습니다</p>
        </div>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
            <Link to="/store/commerce/products" style={{ color: c.neutral400, fontSize: '13px', textDecoration: 'none' }}>
              상품 관리
            </Link>
            <span style={{ color: c.neutral300 }}>/</span>
            <span style={{ color: c.neutral600, fontSize: '13px' }}>마케팅 자산</span>
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={() => navigate(-1)} style={styles.actionBtn}>
            <ArrowLeft size={14} />
            돌아가기
          </button>
      ...
      <div style={styles.kpiGrid}>
        <div style={styles.kpiCard}>
          <Link2 size={20} style={{ color: c.primary, marginBottom: '8px' }} />
        ...
        {data.qrAssets.length === 0 ? (
          ...
          <div style={styles.assetList}>
            {data.qrAssets.map((qr: ProductQrAsset) => {
              const link = data.links.find((l: { assetType: string; assetId: string; id: string }) => l.assetType === 'qr' && l.assetId === qr.id);
              return (
        ...
        {data.libraryAssets.length === 0 ? (
          ...
          <div style={styles.assetList}>
            {data.libraryAssets.map((lib: ProductLibraryAsset) => {
              const link = data.links.find((l: { assetType: string; assetId: string; id: string }) => l.assetType === 'library' && l.assetId === lib.id);
              return (
