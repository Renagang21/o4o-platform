/**
 * MediaPickerModal — 공용 미디어 선택기/업로더 (store-ui-core canonical)
 *
 * WO-O4O-CONTENT-ASSET-MEDIA-LIBRARY-STANDARDIZATION-V1
 *
 * 기존 서비스별 사본(web-neture / web-kpa-society 의 components/common/MediaPickerModal)을
 * 이 단일 공용 컴포넌트로 통합한다. 서비스별 UI 사본을 만들지 않는다.
 *
 * 설계(§7.1): dumb-ish 컴포넌트. UI/뷰 상태는 내부에서 관리하되, 모든 IO는
 * 주입된 `api`(list/upload/moveToFolder/deleteAsset) 로 위임하고, 권한 판정은
 * `isOperator` prop 으로 받는다. store-ui-core 는 auth/서비스 apiClient 에 의존하지 않는다.
 *
 * - 새 이미지 업로드 (동의 필수, 폴더 선택)
 * - 라이브러리에서 기존 이미지 선택 (폴더 필터, 그리드/리스트 보기)
 * - 운영자: 폴더 이동 / 삭제
 * - 단일 선택, 이미지 중심
 */
...
/**
 * 미디어 라이브러리 IO 어댑터 — 각 서비스의 mediaApi 를 주입한다.
 * (서비스별 mediaApi 는 이미 이 형태와 구조적으로 호환)
 */
export interface MediaPickerApi {
  list(options?: { page?: number; limit?: number; assetType?: string; folder?: string }): Promise<{
    success: boolean; data?: MediaAssetItem[]; total?: number; error?: string;
  }>;
  upload(file: File, consent: boolean, serviceKey?: string, folder?: string): Promise<{
    success: boolean; data?: MediaAssetItem; error?: string;
  }>;
  moveToFolder(assetId: string, folder: string): Promise<{ success: boolean; error?: string }>;
...
export interface MediaPickerModalProps {
  ...
  /** 미디어 라이브러리 IO 어댑터 (서비스 mediaApi 주입) */
  api: MediaPickerApi;
  /** 운영자 권한 여부 (폴더 이동/삭제 노출) */
  isOperator?: boolean;
  title?: string;
...
export function MediaPickerModal({
  ...
  const handleUpload = async () => {
    ...
    const res = await api.upload(file, true, undefined, uploadFolder);
    if (res.success && res.data) {
  ...
  const handleMoveToFolder = async (assetId: string, folder: string) => {
    setMovingId(assetId);
    const res = await api.moveToFolder(assetId, folder);
    if (res.success) {
      setPage(1);
