/**
 * StoreProductsManagerPage — 내 매장 상품 관리
 *
 ...
 *
 * Header:
 *   기본 헤더는 패키지 내부에서 제공한다. (neutral Tailwind)
 *   서비스 별 헤더가 필요하면 `headerSlot` props로 주입한다.
 */
...
// ── 기본 헤더 (admin-dashboard PageHeader 미사용 — 공통화) ────────────────────
    ...
    <div className="mb-6 flex items-start justify-between gap-4">
      ...
      <div className="flex gap-2 flex-shrink-0">
        {actions.map((a) => {
          ...
          const cls = isPrimary
            ? 'inline-flex items-center gap-2 rounded-md border border-blue-600 bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 hover:border-blue-700'
            : 'inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50';
          return (
            <button key={a.id} onClick={a.onClick} className={cls}>
              {a.icon}
              {a.label}
            </button>
          );
...
export interface StoreProductsManagerPageProps {
   ...
   *
   * `register`/`refresh` 액션은 슬롯 내부에서 직접 트리거할 수 있도록
   * 콜백을 인자로 받는다.
   */
  headerSlot?: (args: {
    onRegister: () => void;
...
export default function StoreProductsManagerPage({
  ...
  const handleRegister = useCallback(() => openModal('register'), [openModal]);
