/**
 * OperatorForumCategoriesPage — 포럼 목록 관리 (공통 콘솔)
 *
 * WO-O4O-CROSSSERVICE-OPERATOR-FORUM-CATEGORIES-GP-KCOS-INTRODUCE-V1
 *
 * KPA reference(ForumCategoriesManagementPage)를 공통 모듈로 추출. 서비스 차이는 client(serviceCode 바인딩) + tableId.
 * 활성/비활성(soft·가역) / 태그 수정 / 영구삭제(hardDelete) + 일괄 처리.
 * hardDelete 는 delete-check 사전 확인 + 409 차단(blockedReasons) + 확인 문구를 그대로 보존 — backend 안전장치 준수.
 * 게시글 삭제 기능은 포함하지 않음(카테고리 삭제와 구분).
 */
...
import type {
  ForumCategoryData as CategoryData,
  ForumCategoryDeleteCheck as DeleteCheckData,
...
const forumCategoryPolicy = defineActionPolicy<CategoryData>('forum:categories', {
  rules: [
    { key: 'editTags', label: '태그 수정' },
    { key: 'activate', label: '활성화', visible: (row) => !row.isActive },
    { key: 'deactivate', label: '비활성화', variant: 'warning', visible: (row) => row.isActive },
    { key: 'hardDelete', label: '완전 삭제', variant: 'danger', visible: (row) => !row.isActive },
  ],
});
...
export function OperatorForumCategoriesPage({
  ...
  const handleActivate = async (cat: CategoryData) => {
    ...
    try {
      const result = await client.activate(cat.id);
      if (result.success) {
  ...
  const openHardDeleteModal = async (cat: CategoryData) => {
    ...
    try {
      const result = await client.getDeleteCheck(cat.id);
  ...
  const handleHardDelete = async () => {
    ...
    try {
      const result = await client.hardDelete(hardDeleteTarget.id, { reason: hardDeleteReason.trim() });
      if (result.success) {
  ...
  const handleTagEditSave = async () => {
    ...
    setIsTagSaving(true);
    try {
      const result = await client.updateCategory(tagEditTarget.id, { tags: tagEditTags });
      if (result.success) {
  ...
  const handleBulkSoftDelete = async () => {
    ...
    const result = await batch.executeBatch(async (ids) => {
      const settled = await Promise.allSettled(
        ids.map((id) => client.directDeactivate(id, { reason: reasonText })),
      );
      return {
        data: {
  ...
  const handleBulkHardDelete = async () => {
    ...
    const result = await batch.executeBatch(async (ids) => {
      ...
      for (const id of ids) {
        try {
          const check = await client.getDeleteCheck(id);
          ...
          const r = await client.hardDelete(id, { reason: reasonText });
          results.push({
            id,
