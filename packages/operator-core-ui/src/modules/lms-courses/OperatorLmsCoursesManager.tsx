/**
 * OperatorLmsCoursesManager — 운영자 강의 관리 공통 모듈
 *
 ...
 *       + bulk(비공개/종료/완전삭제) + 반려 사유 modal + 상세 Drawer.
 * archived 강의만 완전 삭제 가능(cascade).
 *
 * OPERATOR-DATATABLE-POLICY-V1: @o4o/operator-ux-core DataTable 만 사용.
 */
...
export interface OperatorLmsCourse {
  id: string;
  title?: string;
...
export interface OperatorLmsCoursesListParams {
  search?: string;
  status?: string;
  page: number;
  limit: number;
...
/** 서비스별 운영자 강의 API 어댑터(공통 모듈은 직접 client 를 고정하지 않는다). */
export interface OperatorLmsCoursesApi {
  list: (params: OperatorLmsCoursesListParams) => Promise<unknown>;
...
export interface OperatorLmsCoursesConfig {
  api: OperatorLmsCoursesApi;
  /** 상세/편집 이동 경로. 기본 `/lms/course/${id}`. */
  detailPath?: (id: string) => string;
  /** Drawer 내 상세 링크 라벨. 기본 '강의 페이지 이동'. */
  detailLinkLabel?: string;
}

export interface OperatorLmsCoursesManagerProps {
  config: OperatorLmsCoursesConfig;
}
...
const courseActionPolicy = defineActionPolicy<OperatorLmsCourse>('lms:operator:courses', {
  rules: [
    ...
    { key: 'reject', label: '반려', variant: 'danger', visible: (row) => row.status === 'pending_review' },
    { key: 'unpublish', label: '비공개 처리', visible: (row) => row.status === 'published' },
    {
      key: 'archive',
      label: '강의 종료',
      variant: 'danger',
...
export function OperatorLmsCoursesManager({ config }: OperatorLmsCoursesManagerProps) {
  const navigate = useNavigate();
  const api = config.api;
  const detailPath = config.detailPath ?? ((id: string) => `/lms/course/${id}`);
  const detailLinkLabel = config.detailLinkLabel ?? '강의 페이지 이동';

  const [courses, setCourses] = useState<OperatorLmsCourse[]>([]);
  ...
  const [rejectModal, setRejectModal] = useState<{ open: boolean; course: OperatorLmsCourse | null; reason: string; submitting: boolean }>(
    { open: false, course: null, reason: '', submitting: false },
  );
  const batch = useBatchAction();
  const [selectedCourse, setSelectedCourse] = useState<OperatorLmsCourse | null>(null);
  const PAGE_SIZE = 20;
  ...
  const handleUnpublish = async (course: OperatorLmsCourse) => {
    try {
  ...
  const handleArchive = async (course: OperatorLmsCourse) => {
    try {
  ...
  const handleHardDelete = async (course: OperatorLmsCourse) => {
    try {
      await api.hardDelete(course.id);
  ...
  const handleApprove = async (course: OperatorLmsCourse) => {
    try {
      await api.approve(course.id);
  ...
  const openRejectModal = (course: OperatorLmsCourse) => {
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      await api.reject(rejectModal.course.id, reason);
      ...
      async (ids) => {
        ...
        for (const id of ids) {
          try {
            ...
            const reason = (err as any)?.data?.error || err?.message || failLabel;
  ...
  const handleBulkUnpublish = () =>
    runBatch([...selectedIds].filter((id) => courses.find((c) => c.id === id)?.status === 'published'), api.unpublish, '처리 실패');
  const handleBulkArchive = () =>
    runBatch([...selectedIds].filter((id) => { const c = courses.find((co) => co.id === id); return c != null && c.status !== 'archived'; }), api.archive, '처리 실패');
  const handleBulkHardDelete = () =>
    runBatch([...selectedIds].filter((id) => courses.find((c) => c.id === id)?.status === 'archived'), api.hardDelete, '삭제 실패');
  ...
  const columns: ListColumnDef<OperatorLmsCourse>[] = [
    {
      key: 'title',
      header: '강의명',
      render: (v) => (
    ...
    },
    {
      ...
      width: '120px',
      render: (_v, row) => (
        <span className="text-sm text-slate-600">{row.instructor?.name || row.instructorName || '-'}</span>
      ),
    },
    {
      key: 'category',
      ...
      width: '100px',
      render: (v) => (
        <span className="inline-block px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-xs font-medium">{v || '-'}</span>
      ),
    },
    {
      key: 'lessonCount',
      ...
      render: (_v, row) => (
        <RowActionMenu
          actions={buildRowActions(courseActionPolicy, row, {
            view: () => navigate(detailPath(row.id)),
        ...
        {selectedCourse && (
          <div style={{ fontSize: 14, color: '#374151' }}>
            {[
              ...
              { label: '수정일', value: formatDate(selectedCourse.updatedAt) },
            ].map((item) => (
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #e2e8f0' }}>
              <a href={detailPath(selectedCourse.id)} style={{ fontSize: 13, color: '#2563eb', textDecoration: 'none' }}>
                {detailLinkLabel} →
              </a>
            </div>
          </div>
...
const searchBtnStyle: React.CSSProperties = {
  ...
  borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', flexShrink: 0,
};
const clearBtnStyle: React.CSSProperties = {
  padding: '8px 14px', backgroundColor: '#f1f5f9', color: '#475569',
  border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 13, cursor: 'pointer', flexShrink: 0,
};

export default OperatorLmsCoursesManager;
