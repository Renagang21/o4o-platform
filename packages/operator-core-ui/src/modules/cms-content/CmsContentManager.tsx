/**
 * CmsContentManager — Shared operator CMS content management panel
 *
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <ContentList
        ...
        apiFetch={apiFetch}
        assetCopyEnabled={assetCopyEnabled}
        storeContentPath={storeContentPath}
        assetCopyFn={assetCopyFn}
      />
...
function ContentList({
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<{ data: CmsContent[]; total: number; totalPages: number }>(
        `${apiBase}/news/admin/list?${params}`,
      );
      setItems(res.data);
  ...
  }, [type, page, statusFilter, searchQuery, pickedOnly, refreshKey, apiBase, apiFetch]);
  ...
  async function handleStatusToggle(item: CmsContent) {
    ...
    try {
      await apiFetch(`${apiBase}/news/${item.id}`, {
        method: 'PUT',
  ...
  async function handleCopyToStore(item: CmsContent) {
    if (!assetCopyFn) return;
    setActionLoading(item.id);
    try {
      await assetCopyFn(serviceKey, item.id);
      onCopyToast(`"${item.title}" — 매장에 추가되었습니다.`);
      setTimeout(() => {
        onCopyToast(null);
        navigate(storeContentPath);
      }, 1500);
    } catch (e: any) {
  ...
  async function handleBulkPublish() {
    ...
    const result = await batch.executeBatch(
      (batchIds) => apiFetch(`${apiBase}/news/batch-publish`, {
        method: 'POST',
  ...
  async function handleBulkArchive() {
    ...
    const result = await batch.executeBatch(
      (batchIds) => apiFetch(`${apiBase}/news/batch-archive`, {
        method: 'POST',
        body: JSON.stringify({ ids: batchIds }),
      }),
      archivableIds,
  ...
  async function handleBulkHardDelete() {
    ...
    const result = await batch.executeBatch(
      (batchIds) => apiFetch(`${apiBase}/news/batch-hard-delete`, {
        method: 'POST',
        body: JSON.stringify({ ids: batchIds }),
      }),
      archivedIds,
  ...
  const contentActionPolicy = defineActionPolicy<CmsContent>(`${serviceKey}:content`, {
    rules: [
      {
        key: 'unpublish',
        label: '비공개',
      ...
      { key: 'edit', label: '수정', variant: 'primary', visible: (row) => row.status !== 'archived' },
      ...(assetCopyEnabled ? [{
        key: 'copy',
        label: '매장으로 복사',
        visible: (row: CmsContent) => row.status !== 'archived',
        confirm: (row: CmsContent) => ({
          title: '매장 복사',
  ...
  return (
    <div>
      ...
      <DataTable<CmsContent>
        ...
        tableId={`${serviceKey}-content-${type}`}
        selectable
