/**
 * OperatorStoreDetailPage — 매장 상세 (채널 + Capabilities + 상품) 공통 콘솔
 *
 * WO-O4O-STORE-HUB-OPERATOR-INTEGRATION-V1 (원본 업무):
 *   Operator HUB → Store HUB 연결. /api/v1/operator/stores/:storeId 기반 상세 조회.
 ...
 *   KPA / K-Cosmetics 중복 구현을 본 콘솔로 수렴. 서비스는 client adapter + 라벨/accent 만 주입.
 *   K-Cosmetics 는 이 수렴으로 DataTable 표준 · 섹션별 오류/재시도 · 상품 더보기를 획득한다
 *   (업무 의미·endpoint 변경 없음, 표준 정합 상향).
 */
...
import type {
  OperatorStoreDetail,
  StoreDetailChannel,
  StoreDetailCapability,
  StoreDetailProduct,
  OperatorStoreDetailPageProps,
} from './types';
...
export function OperatorStoreDetailPage({
  storeId,
  client,
  onBack,
  typeLabels,
  ...
  // ── 매장 정보 (실패 시 전체 차단) ──
  const [store, setStore] = useState<OperatorStoreDetail | null>(null);
  const [storeLoading, setStoreLoading] = useState(true);
  const [storeError, setStoreError] = useState<string | null>(null);

  // ── 채널 (섹션) ──
  const [channels, setChannels] = useState<StoreDetailChannel[]>([]);
  ...
  // ── 기능 capability (섹션) ──
  const [capabilities, setCapabilities] = useState<StoreDetailCapability[]>([]);
  const [capsLoading, setCapsLoading] = useState(true);
  const [capsError, setCapsError] = useState<string | null>(null);
  const [capabilityLoading, setCapabilityLoading] = useState<string | null>(null);

  // ── 상품 (섹션 + 더보기) ──
  const [products, setProducts] = useState<StoreDetailProduct[]>([]);
  ...
  const loadStore = useCallback(async () => {
    ...
    setStoreError(null);
    try {
      const data = await client.getStore(storeId);
  ...
  const loadChannels = useCallback(async () => {
    ...
    try {
      const data = await client.getChannels(storeId);
  ...
  const loadCapabilities = useCallback(async () => {
    ...
    try {
      ...
      const data = await client.getCapabilities(storeId);
  ...
  const loadProducts = useCallback(async (page: number, append: boolean) => {
    ...
    try {
      const data = await client.getProducts(storeId, page, PRODUCT_PAGE_SIZE);
      if (data.success) {
  ...
  const handleToggleCapability = async (capKey: string, currentEnabled: boolean) => {
    ...
    try {
      await client.toggleCapability(storeId, capKey, !currentEnabled);
  ...
  //   채널 영구 종료(되돌릴 수 없음)는 ConfirmActionDialog(danger) 게이트.
  ...
  const applyChannelStatus = async (channelId: string, newStatus: string) => {
    ...
    try {
      await client.updateChannelStatus(storeId, channelId, newStatus);
      setChannels((prev) =>
  ...
  const channelColumns: ListColumnDef<StoreDetailChannel>[] = useMemo(() => [
    {
  ...
  const productColumns: ListColumnDef<StoreDetailProduct>[] = useMemo(() => [
    {
    ...
    return (
      <div className="flex items-center justify-center" style={{ minHeight: 400 }}>
        <div className="flex flex-col items-center gap-3">
          <div className={`w-8 h-8 border-2 ${spinnerClass} border-t-transparent rounded-full animate-spin`} />
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">{typeLabels[store.type] || store.type}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        {channelsError ? (
          <SectionError message={channelsError} onRetry={() => void loadChannels()} />
        ) : (
          <DataTable<StoreDetailChannel>
            columns={channelColumns}
            data={channels}
            rowKey="id"
            loading={channelsLoading}
            emptyMessage="등록된 채널이 없습니다"
            tableId={tableIds?.channels ?? 'operator-store-channels'}
          />
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        {capsLoading ? (
          ...
          // 실패를 빈 목록으로 위장하지 않는다
          <SectionError message={capsError} onRetry={() => void loadCapabilities()} />
              ...
              <div key={cap.key} className="flex items-center justify-between p-4 rounded-lg border border-slate-100 bg-slate-50">
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${
                    cap.enabled ? capabilityOnClass : 'bg-slate-300'
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        ...
        {productsError ? (
          <SectionError message={productsError} onRetry={() => void loadProducts(1, false)} />
        ) : (
          <>
            <DataTable<StoreDetailProduct>
              columns={productColumns}
              data={products}
              rowKey="id"
              loading={productsLoading}
              emptyMessage="등록된 상품이 없습니다"
              tableId={tableIds?.products ?? 'operator-store-products'}
            />
