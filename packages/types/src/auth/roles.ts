 ...
 */
export const COMMERCE_ROLES: Role[] = [
  ROLES.SELLER,
  ROLES.VENDOR,
  ROLES.SUPPLIER,
  ROLES.BUSINESS,
 ...
 */
export function isCommerceRole(role: string): boolean {
  return COMMERCE_ROLES.includes(role as Role);
}

/**
 * Check if roleA has higher or equal privileges than roleB
 */
export function hasHigherOrEqualPrivilege(roleA: string, roleB: string): boolean {
  const levelA = ROLE_HIERARCHY[roleA as Role] ?? 0;
  const levelB = ROLE_HIERARCHY[roleB as Role] ?? 0;
 ...
 */
export function getRoleLabel(role: string, locale: 'ko' | 'en' = 'ko'): string {
