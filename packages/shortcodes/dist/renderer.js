import { cloneElement, createElement, Fragment, useCallback, useMemo } from 'react';
/**
 * 숏코드 렌더러 구현
 * 파싱된 숏코드를 React 컴포넌트로 렌더링합니다.
 */
export class DefaultShortcodeRenderer {
     ...
     * 단일 숏코드 렌더링
     */
    renderShortcode(shortcode, context) {
 ...
 */
export function useShortcodes(parser, registry) {
    const renderer = useMemo(() => new DefaultShortcodeRenderer(parser, registry), [parser, registry]);
    const render = useCallback((content, context) => renderer.render(content, context), [renderer]);
    const renderShortcode = useCallback((shortcode, context) => renderer.renderShortcode(shortcode, context), [renderer]);
