 ...
 */
export const parseAffiliateLink = (url: string): {
  affiliateCode?: string;
  productId?: string;
  campaignId?: string;
  source?: string;
...
export const generateShareMessage = (
  ...
  const templates: Record<string, string> = {
    twitter: `Check out ${product}! ${discount ? `Use my link for ${discount} off: ` : ''}${link} #affiliate #deals`,
    ...
    email: `Subject: Check out ${product}!\n\nHi,\n\nI found this great product and thought you might be interested: ${product}\n\n${discount ? `You can get ${discount} off using my affiliate link:\n` : 'Check it out here:\n'}${link}\n\nBest regards`,
    instagram: `${product} 🔥 ${discount ? `${discount} OFF! ` : ''}Link in bio 👆 #affiliate #${product.replace(/\s+/g, '').toLowerCase()}`
  };
...
/**
 * Calculate affiliate rank based on performance
 */
export const calculateAffiliateRank = (
  totalSales: number,
  totalRevenue: number,
